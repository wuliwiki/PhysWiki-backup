...
\begin{itemize}
\item 10.1 设 $I^k$ 是 $R^k$ 中的 $k$-方格， 它由满足 $a_i\leqslant x_i \leqslant b_i$（$i=1,\dots,k$） 的一切 $\bvec x=(x_1,\dots,x_k)$ 组成， $I^j$ 是 $R^j$ 中的 $j$-方格， 它由前 $j$ 个不等式来确定， $f$ 是 $I^k$ 上的连续函数． 令 $f = f_k$， 而用下式定义 $I^{k-1}$ 上的函数 $f_{k-1}$： $f_{k-1}(x_1,\dots,x_{k-1}) = \int_{a_k}^{b_k} f_k(x_1,\dots,x_{k-1},x_k)\dd{x_k}$． $f_k$ 在 $I^k$ 上的一致连续性标名 $f_{k-1}$ 在 $I^{k-1}$ 上连续． 因此， 我们能够重复应用这种手续， 得到 $I^j$ 上的连续函数 $f_j$，…… $k$ 步以后， 就能得到一个数 $f_0$， 我们就把这个数叫做 $f$ 在 $I^k$ 上的积分， 并写成 $\int_{I^k} f(x)\dd{x}$ 或者 $\int_{I^k} f$．
...
\item 10.6 在 $R^n$ 上， 只把标准基的某一队成员交换， 而其他成员不变的线性算子 $B$ 叫做\textbf{对换}． $B$ 也可以看成是交换两个坐标， 而基不变．
\end{itemize}
