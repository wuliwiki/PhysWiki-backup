...
\end{aligned}~
\end{equation}
即傅里叶变换是线性的。而任意的离散点 $u_j$ 都可以用Kronecker Delta 函数 
\begin{equation}
...
\end{aligned}\right.~
\end{equation}
表示为 $u_j=\sum\limits_{m}u_m\delta_{j-m}$。因此，若知道 $\delta_j$ 的傅里叶变换，就知道了 $u(x)=\sum\limits_{j=-\infty}^\infty u_m\delta(x)$ 的傅里叶变换，显然 $u(x_i)=u_m$
