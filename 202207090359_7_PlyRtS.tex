...
4．当$[\mathbb{K}:\mathbb{F}]=1$，有$\opn{Gal}(f:\mathbb{F})\cong S_4$；

\end{theorem}

\textbf{证明}：

设$f$的根为$\alpha_1, \alpha_2, \alpha_3, \alpha_4$，且令$\beta_i=\alpha_i+b/4a$．用 定义$u, v, w$，用\autoref{PlyRtS_the2} 定义它们的平方根$u', v', w'$．

由于$\opn{Dis}(s)=\opn{Dis}(f)\neq 0$，故$s$也是可分的，进而可知$u, v, w$互不相等．

令$\mathbb{E}$为$f\in\mathbb{F}[x]$的分裂域．

由\autoref{PlyRtS_the2} ，$\mathbb{E}=\mathbb{F}(\beta_1, \beta_2, \beta_3, \beta_4)\subseteq\mathbb{F}(u', v', w')$．
