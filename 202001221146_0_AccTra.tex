...
\subsection{一般情况}
类比\autoref{Vtrans_eq2}\upref{Vtrans}， 若两参考系之间有可能存在转动， 我们定义 $S'$ 系中的固定点相对于 $S$ 系的加速度为 $\bvec a_{r}$ 中的两个固定点取 $t$ 时刻点 $P$ 在两参考系中的坐标． 那么是否仍然有 $\bvec a_S = \bvec a_{S'} + \bvec a_r$ 呢？ 答案是否定的， 正确的表达式需要添加一项
\begin{equation}\label{AccTra_eq2}
\bvec a_S = \bvec a_{S'} + \bvec a_{rS} + 2 \bvec \omega \cross \bvec v
\end{equation}
