...
\end{equation}
其中，$\bvec \theta$表示的是模型参数，$w$表示模型权重，$\bvec X$表示的是训练数据的特征，$y$表示的是标签，$\alpha\in[0,+\infty)$是权衡范数惩罚项$\Omega$和标准目标函数$J(\bvec X; \bvec \theta)$相对贡献的超参数。将$\alpha$设为$0$表示没有正则化。该公式是假设模型参数中没有偏置，因此$\bvec \theta$=$\bvec w$。
