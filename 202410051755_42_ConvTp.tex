...
\end{definition}
 ...
 $M\subset\mathcal X$ 的\textbf{接触点} $x$ 是指 $x\mathcal X$ 的任一邻域都含有 $M$ 的点（即 $x\in[M]$\footnote{$[M]$ 指 $M$ 的闭包}）。
...
\begin{corollary}{}
设拓扑空间 $\mathcal T$ 如\autoref{lem_ConvTp_1} 定义。则 $(0,1]$ 中不存在收敛于 $0$ 的点列。
\end{corollary}
\textbf{证明：}由\autoref{lem_ConvTp_1} ，$(0,1]$ 中收敛
