...
\begin{definition}{对偶空间}
给定域$\mathbb{K}$上两个$n$维线性空间$V$和$W$．如果指定了一个双线性映射$m:V\times W\rightarrow\mathbb{K}$，即对于$a_i, b_i\in\mathbb{K}$、$\bvec{v}_i\in V$和$\bvec{w}_j\in W$，有$m(\sum\limits_{i=1}^na_i\bvec{v}_i,\sum\limits_{j=1}^nb_j\bvec{w}_j )=\sum\limits_{i, j=1}^n$
\end{definition}
