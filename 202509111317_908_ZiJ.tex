...
\subsection{包含关系的其他性质}
\begin{itemize}
\item 交集刻画：集合 $A$ 是集合 $B$ 的子集，当且仅当它们的交集等于 $A$。形式化地：
$$
A \subseteq B \iff A \cap B = A.~
$$
\item 并集刻画：集合 $A$ 是集合 $B$ 的子集，当且仅当它们的并集等于 $B$。形式化地：
$$
A \subseteq B \iff A \cup B = B.~
$$
\item 有限集的基数刻画：若 $A$ 是有限集，则 $A \subseteq B$ 当且仅当
$$
|A \cap B| = |A|.~
$$
\item 子集关系在集合上定义了一个偏序。事实上，给定集合的所有子集在子集关系下构成一个布尔代数，其中：并(join)运算是交集，交(meet)运算是并集，子集关系本身就是布尔代数中的包含关系。
\item 包含关系是典范的偏序，意思是：任意偏序集$(X, \preceq)$都同构于某个按包含关系排列的集合族。序数是一个简单的例子：如果把每个序数 $n$ 与集合$[n] = \{ \text{所有小于等于 } n \text{ 的序数} \}$对应起来，则有$a \leq b \iff [a] \subseteq [b]$.
\end{itemize}
\subsection{参见}
\begin{itemize}
\item 凸子集 —— 在几何学中，与任意一条直线的交集都是一个线段的集合
\item 包含序 —— 作为某类对象上的子集包含关系而产生的偏序
\item 部分论 —— 研究“部分”及其所构成的“整体”的学科
\item 区域 —— 拓扑空间中的连通开子集
\item 子集和问题 —— 计算机科学中的判定问题
\item 涵摄式包含 —— 元素彼此从属的系统
\item 子空间 —— 具有额外结构的数学集合
\item 全子集 —— 拓扑向量空间$X$的子集$T$，其线性生成集在$X$中稠密
\end{itemize}
\subsection{参考文献}
\begin{enumerate}
\item Rosen, Kenneth H. (2012). Discrete Mathematics and Its Applications (第7版). 纽约: McGraw-Hill. p.119. ISBN 978-0-07-338309-5.
\item Epp, Susanna S. (2011). Discrete Mathematics with Applications (第4版). Cengage Learning. p.337. ISBN 978-0-495-39132-6.
\item Stoll, Robert R. (1963). Set Theory and Logic. 旧金山, CA: Dover Publications. ISBN 978-0-486-63829-4.
\item Rudin, Walter (1987). Real and Complex Analysis (第3版). 纽约: McGraw-Hill. p.6. ISBN 978-0-07-054234-1. MR 0924157.
\item Subsets and Proper Subsets* (PDF), 原始版本存档于2013-01-23, 检索于2012-09-07.
\item Weisstein, Eric W. "Subset". mathworld.wolfram.com. 检索于2020-08-23.
\end{enumerate}
\subsection{参考书目}
\begin{itemize}
\item Jech, Thomas (2002). Set Theory. Springer-Verlag. ISBN 3-540-44085-2.
\end{itemize}
\subsection{外部链接}
\begin{itemize}
\item 维基共享资源中与子集相关的多媒体
\item Weisstein, Eric W. “Subset”. MathWorld.
\end{itemize}
