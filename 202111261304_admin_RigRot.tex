...
要讨论刚体的定轴转动和力矩之间的关系， 我们需要先来看\textbf{角动量矢量在延轴方向的分量}\footnote{矢量 $\bvec L$ 与矢量 $\bvec \omega$ 的关系见 “惯性张量\upref{ITensr}” 词条．}． 我们把转轴的某个正方向定义为 $z$ 方向． 对于刚体上的单个质点（编号为 $i$）， 角动量在 $z$ 方向的分量为
\begin{equation}
L_{iz} = \bvec L_i \vdot \uvec z = (\bvec r_i \cross \bvec p_i) \vdot \uvec z
\end{equation}
首先把质点的位矢在 $z$ 方向和垂直 $z$ 方向分解（称为水平方向）， $\bvec r_i = \bvec r_{iz} + \bvec r_{i\bot}$． 由于 $\bvec p_i$ 一直沿水平方向， 根据叉乘的几何定义， $\bvec r_z \cross \bvec p$ 也是沿水平方向， 只有 $\bvec r_ \bot \cross \bvec p$ 沿 $z$ 方向．另外， 在圆周运动中， 半径始终与速度垂直， 所以 $\bvec r_ \bot$ 始终与 $\bvec p$ 垂直．得出结论
\begin{equation}
...
在定轴转动的情况下， 根据\autoref{RigRot_eq6}， 当系统外对系统延轴方向的力矩 $\tau_z$ 为零时， 角动量 $\bvec L$ 的 $z$ 分量 $L_z$ 守恒．  这意味着刚体的角加速度为零（\autoref{RigRot_eq7}）， 也就是刚体做匀速转动． 类比到质点的直线运动， 就是当 $F = ma$ 中外力 $F = 0$ 时， 加速度为零， 质点做匀速运动， 动量守恒．

可见和匀速运动不需要外力维持一样， 匀速转动也并不需要延轴方向的外力矩维持． 然而在现实生活中， 正如水平直线轨道上具有初速度的小球会由于与轨道的摩擦力， 空气阻力等各种外力下最终停止运动， 具有初始角速度的物体也会在轴承的摩擦、空气阻力等外力矩的作用下最终停止转动， 这与延轴角动量守恒并不矛盾．
