...
设$V$是$n$维线性空间，取$v\in\bigwedge^k V$，则$\star \star v = (-1)^n v$．
...
复杂指标$\bar{I}$对应的置换$\bar{\sigma}$，相当于进行$\sigma$置换后再把后$n-k$个元素整体挪到前面，也就是进行$\sigma$后再进行$(n-k)k$次\textbf{对换}．因此，$\opn{sign}\bar{\sigma}=(-1)^{(n-k)k}\opn{sign}\sigma$
