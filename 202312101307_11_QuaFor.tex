...
\pentry{多重线性映射\upref{MulMap}，张量\upref{Tensor}，矩阵的秩\upref{MatRnk}}
...
若 $a = \sum_i a_i e_i$， $b = \sum_j b_j e_j$， 令对应的坐标列矢量为 $\bvec a = (a^1\ a^2\ \dots)\Tr$， $\bvec b = (b^1\ b^2\ \dots)\Tr$。 那么 $f(u, v)$ 可以表示为以下的矩阵运算\upref{Mat}。
\begin{equation}\label{eq_QuaFor_2}
f(a, b) = \bvec a\Tr \mat F \bvec b = \sum_{i,j} f_{ij}a^i b^j~,
\end{equation}
对应的二次型为
\begin{equation}\label{eq_QuaFor_3}
q(a) = \bvec a\Tr \mat F \bvec a = \sum_{i,j} f_{ij}a^i a^j~.
\end{equation}
因为 $f(e_i, e_j) = f(e_j, e_i)$， 所以 $\mat F$ 是一个\textbf{对称矩阵}。从\autoref{eq_QuaFor_2} 和\autoref{eq_QuaFor_3} 可见，二次型的矩阵形式实际上是把一个$(0,2)$型张量写为$(1,1)$型张量。

\subsection{二次型的规范型}
\begin{definition}{二次型的规范型（或对角型）}\label{def_QuaFor_1}
称二次型 $q$ 在 $V$ 的基底 $(e_1,\cdots,e_n)$ 之下具有\textbf{规范型}或\textbf{对角型}，如果对 $\forall x=\sum x^i e_i\in V$ ，$q(x)$ 的值可用公式
\begin{equation}
q(x)=\sum_{i}f_{ii}x^ix^i~
\end{equation}
