...
据\autoref{QMPrcp_def5} 和\autoref{QMPrcp_eq3} 知，取$\ket{s}$为$X$的任意\textbf{本征}矢量，其本征值为$a$，若$X$是厄米的，则$\bra{s}X$与$X\ket{s}$互为对偶表示，即$X\ket{s}=a\ket{s}$，$\bra{s}X=a^*\bra{s}$．
...
\begin{table}[ht]
\centering
\caption{请输入表格标题}\label{QMPrcp_tab1}
