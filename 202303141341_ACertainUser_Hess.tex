...
纯粹从数学上说，我们可以将这个变化写为积分的形式：
$$U(T_1,V_1)=U(T_0,V_0) + \int_{T_0}^{T_1} \left(\pdv{U}{T}\right)_V \dd T + \int_{V_0}^{V_1} \left(\pdv{U}{V}\right)_T \dd V $$
这两个偏导数就暗示了“设计路径”的原理。

在这一步我们似乎只玩了一个数学游戏，更关键的问题是，我们要如何确定 $\left(\pdv{U}{T}\right)_V$与$ \left(\pdv{U}{V}\right)_T $？这就需要我们灵活地化用热容\upref{ThCapa}与热力学关系式\upref{MWRel}。

根据以上定理，$\left(\pdv{U}{T}\right)_V = C_V$，就是热容，可以由实验测得；$ \left(\pdv{U}{V}\right)_T  = PT\beta-P $，其中$\beta$是定容压强系数，也是可以
