...
\pentry{压力体积图\upref{PVgraf}， 理想气体内能\upref{IdgEng}， 功 功率\upref{Fwork}}
...
\end{equation}
我们在 $Q$ 和 $W$ 用 $\delta$ 符号而不是全微分符号 $\dd{}$， 是因为 $Q$ 和 $W$ 和系统变化的过程本身有关： 也就是说即使系统的初末状态确定， 中间的过程不一样也会导致它们的不同． 这样的量被称为\textbf{过程量}\upref{StaPro}． $E$ 前面用的是全微分符号，是因为内能 $E$ 之和系统的状态有关， 被称为\textbf{状态量}\upref{StaPro}． 所以 $\Delta E$ 也只与系统的初始和最终的状态有关， 与中间的过程无关． $W$ 和 $Q$ 并不是系统状态的函数，它们用来描述在一个系统变化过程中功和热量的传递，是一个 ．
