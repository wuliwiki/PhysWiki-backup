...
\item Julia 完全使用 \textbf{Just-Ahead-of-Time（JIT）}。 并不是把部分代码使用该优化，而是全部。Julia （几乎?）不能直接理解并执行高级语言。 每个重载的函数第一次被调用的时候， julia 都会先给它生成专门的 llvm 机器码。 这个过程还有一个不那么正规的名字叫做 \textbf{Just Ahead-of-Time（JAOT）}。
