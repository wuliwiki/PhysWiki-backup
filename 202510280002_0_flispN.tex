...
\item \verb`(aset! 向量 索引 值)` 可以改变向量元素值。 如 \verb`(define x (vector 1 2 3 4))`， \verb`(aset! x 2 9)`
...
\item \verb`(string.inc 字符串 索引 字符数)` 向后跳 \verb`字符数` 个字符，返回新的索引（适用于。
\end{itemize}
...
\subsection{文件读写}
\begin{itemize}
\item \verb`(file 文件名 :write :create)` 打开文件用于写入，若没有就创建。 返回一个 \verb`iostream` 类型。
...
\item \verb`(io.eof? 怎么用)`
\item \verb`(iostream? 变量)` 怎么用？
\item \verb`(io.getc 流)` 用于 read char
\item \verb`(io.peekc 流)` 用于 peek char
\item \verb`(io.putc 流)` 用于 write char
\item 简单写入的例子
\begin{lstlisting}[language=none]
(define f (file "output.txt" :write :create))

; 以下不工作
(io.write "Hello, world!" f)
(newline f)
(write '(1 2 3) f)
(close f))
\end{lstlisting}
