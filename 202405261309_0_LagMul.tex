...
注意\autoref{eq_LagMul_1} 有 $N$ 条等式，结合\autoref{eq_LagMul_2} 共有 $M+N$ 条等式组成方程组。 而未知量 $x_1,\dot,x_N$ 和 $\lambda_1,\dots,\lambda_M$ 也有 $M+N$ 个。 所以一般存在唯一解或有限个解。
...
两式消去 $\lambda$， 得 $x = y$。 再结合约束条件 $x + y - c = 0$， 得极值点为 $(c/2, c/2)$。 可见拉格朗日乘数法的\autoref{eq_LagMul_1} 并没有包含约束条件， 而是需要再次联立约束条件才可以得出最后答案。
\end{example}
