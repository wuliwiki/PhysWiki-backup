...
这些命题有真有假，但都有一个共同的特点——需要在陈述中表达所述事物的“全体”的含义，这时使用的量词称为\textbf{全称量词}（universal quantifier），记作$\forall$\footnote{这个符号是倒写的A，取“任意”（any）或“所有”（all）的首字母A，并与一般的A做区分。}，可以读作“任意”（any）、“所有”（all）、“每一个”（each）等\footnote{在自然语言中，“每一”和“全体”强调的重点不同，前者强调每一个个体，后者强调整体，但在逻辑中不区分二者。}。
...
\end{itemize}
在陈述中表达所述事物的个体、部分或特例的含义时，使用的量词称为\textbf{存在量词}（existential quantifier，也称作\textbf{特称量词}，particular quantifier），记作$\exists$，可以读作“存在”(exist）、“有”（have）、“至少有一个”（at least）等。形如“集合$M$中存在某个元素$x$满足性质$P(x)$”的命题称为\textbf{存在量词命题}，记作：
