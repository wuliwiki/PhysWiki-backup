...
假设有两列数字$a_1,a_2,\cdots,a_n;b_1,b_2,\cdots,b_n$。若$\forall i<j,a_i\leq a_j\land b_i\leq b_j$则称二者为顺序排列，也就是两个序列的增长方向相同。若$\forall i<j,a_i\leq a_j\land b_i\geq b_j$，则称二者为逆序排列，也就是两个序列的增长方向相反。其他排列方式称为乱序排列。排序不等式表示：逆序和$\leq$乱序和$\leq$顺序和，规范表述如下：

\begin{theorem}{排序不等式}
设两列数字$a_1,a_2,\cdots,a_n;b_1,b_2,\cdots,b_n$，两列数满足$a_1\leq a_2\leq\cdots\leq a_n$和$b_1\leq b_2\leq\cdots\leq b_n$，那么有如下不等式：
\begin{equation}
\sum_{i=1}^na_ib_i\geq\sum_{i=1}^na_ib_j\geq\sum_{i=1}^na_ib_{n+1-i}~.
\end{equation}
