...
&= 4\pi \sum_{l=0}^{∞} \frac{(-1)^l}{\sqrt{2l + 1}} \frac{r_<^l}{r_>^{l+1}} \mathcal{Y}_{l,l}^{0,0} (\uvec r_1, \uvec r_2)
}\end{equation}
这就相当于将张量积空间中的线性算符分解成算符的张量积之和． 第二步使用了\autoref{SphCup_eq9}\upref{SphCup}， 即
\begin{equation}
