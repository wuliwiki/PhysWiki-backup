...
如果这 $N$ 个矢量\textbf{线性无关}， 就代表不存在一组 $c_i$ 使它们的线性组合为 0（见\autoref{linDpe_eq2}~\upref{linDpe}）
\begin{equation}
\sum_{i=1}^M c_i\bvec v_i = \bvec 0
\end{equation}
或者说， 任意一个一个矢量无法表示为其他矢量的线性组合（见\autoref{linDpe_eq3}~\upref{linDpe}）% 这一段中不应该引用几何矢量， 而是多维空间中矢量的公式．
\begin{equation}
