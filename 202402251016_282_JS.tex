...
\begin{itemize}
\item \verb``Scripts in the head: Execute first, in order, blocking the parsing until each is loaded and executed.
\item Inline scripts in the body: Execute in the order they appear as the parser reaches them, but potentially delayed by any preceding external scripts.
\item External scripts in the body: Block parsing and execute in the order they appear as the parser reaches them.
\end{itemize}
