...
\end{equation}
也就是说， 在每个时间步长 $\Delta t$ 中， 我们可以把波函数先根据方程
...
\end{equation}
演化 $\Delta t$． 最后再根据\autoref{HyTDSE_eq1} 演化 $\Delta t/2$． \autoref{HyTDSE_eq1} 和\autoref{HyTDSE_eq9} 的具体演化算法有许多， 将在下面介绍．
...
演化可以并使用 Crank-Nicolson 算法Crank-Nicolson 算法（一维）\upref{CraNic}演化． 但是 Lanczos 算法\upref{Lanc}效率更高， 而且可以变步长．
