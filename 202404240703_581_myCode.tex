...
网页版百科分为两个界面：
\begin{itemize}
\item \textbf{原界面} \verb|wuli.wiki/online|，数据以 html 方式存储。网址 \verb|wuli.wiki/online/AU.html| 中 AU 是文章的 id， 每个文章都有自己的唯一 id， 即使文章被重命名也不会变。  
\item \textbf{新界面} \verb|wuli.wiki/book|，数据保存在数据库中，增加了评论，搜索等功能，目录也更美观。文章内容全部来自静态词条，每天定时爬取。网址 \verb|wuli.wiki/book/AU|。
\item \textbf{文章编辑器} \verb|wuli.wiki/editor| 是一个相对独立的模块，百科文章的作者使用编辑器用 LaTeX 语言写作， 编辑器把 LaTeX 源码转为 \verb|wuli.wiki/online/| 目录中的 html 页面。
\end{itemize}
