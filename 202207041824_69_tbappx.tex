...
\sum_{m}\qty[\epsilon_i+(U(\bvec r)-V(\bvec r-\bvec R_m)]\phi_i(\bvec r-\bvec R_m)=E\sum_{m} \phi_i(\bvec r-\bvec R_m)
\end{equation}
并且将不同格点处的束缚态波函数 $\phi_i(\bvec r-\bvec R_m),m=1,\cdots,n$ 近似视为正交归一的．对\autoref{tbappx_eq2} 左乘 $\int \dd r\phi_i^*(\bvec r-\bvec R_n)$ ，可以得到
\begin{equation}
