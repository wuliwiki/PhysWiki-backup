...
当限制 $u(x_i)$ 对 $x_i$ 具有周期 $2L$ 时，即 $u(x_i+2L)=u(x_i)$，那么为了 $u$ 的单值性，限定 $x_i\in[-L,L]$。设 $x_i$ 有 $N$ 个，于是 $x\in \{h,\cdots,Nh=2L\}$，其中 $h:=\frac{2L}{N}$。和无界情形一样的理由，$k$ 应当限制在 $[-\pi/h,\pi/h]=[-\frac{N\pi}{2L},\frac{N\pi}{2L}]$ 上。此外由于周期性，应当认为点 $x+2L$ 和点 $x$ 是同一个。因此
