...
\pentry{对易厄米矩阵与共同本征矢\upref{Commut}， 定态薛定谔方程\upref{SchEq}}
...
对简并情况，首先要确保本征态 $\psi_n^0$ 取 “好本征态”， 也就是在每个本征子空间中， 要确保矩阵 $\mel{\psi_m^0}{H^1}{\psi_n^0}$ 是对角矩阵。 也就是对同一个能量的所有 $\psi_m^0$ 和 $\psi_n^0$， $\mel{\psi_m^0}{H^1}{\psi_n^0}$ 是对角矩阵。

加入了微扰后，每个简并空间可能会分裂成多个不同能级的简并空间，甚至使简并会完全消失。这也叫做\textbf{能级分裂}。 以后会学的绝热近似\upref{AdiaQM}告诉我们，如果初始时波函数处于某个简并空间的非 “好量子态”， 那么在含时薛定谔方程中给 $H$ 慢慢加上 $H'$ 后，波函数将被分割到若干个不同能级的简并空间中，测量其能量将可能随机得到对应的多个本征值中之一。
