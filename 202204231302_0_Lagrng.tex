...
若一个系统中每个质点的位置可以由几个独立的\textbf{广义坐标（generalized coordinates）} $q_i(t)$ $(i=1,2\dots,N)$ 和时间 $t$ 的函数描述， 把 $q_i$ 关于时间的导数记为 $\dot q_i$， 则广义坐标关于时间的函数可通过\textbf{拉格朗日方程（Lagrange equation）}\footnote{也叫\textbf{欧拉-拉格朗日方程（Euler-Lagrange Equation）}}\footnote{方程中对 $\dot q$ 求偏导\upref{ParDer}可能会使初学者看起来有些奇怪不知道该怎么求， 这里明确一下该偏导在数学上是如何操作的： 可将\autoref{Lagrng_eq2} 中 $L$ 的所有 $\dot q_i$ 记为 $v_i$， 然后将 $q_i$ 和 $v_i$ 看作是完全没有关联的变量， 求偏导时对 $q_1,\dots, q_N, v_1, v_2\dots,t$ 中的一个求， 而保持其他量不变．}解出．
...
\end{equation}
其中 $T$ 是系统的动能， $V$ 是势能． 注意我们为了简单起见暂且假设 $V$ 不显含 $\dot q$， 但在讨论广义势能（例如拉格朗日电磁势\upref{EMLagP}）时是可以显含的．

如果把系统看作质点组， 每个质点的位置矢量 $\bvec r_j$ 都可以看作 $(q, t)$ 的函数． 那么质点的速度 $\dot{\bvec r}_j$ 就是 $(q,\dot q, t)$ 的函数． 所以系统动能定义为
\begin{equation}
T(q, \dot q, t) = \frac{1}{2}\sum_j m_j\dot {\bvec r}_j^2
\end{equation}
而势能 $V$ 可以先看成所有 $\bvec r_j$ 和

若在\autoref{Lagrng_eq1} 右边加上广义力\upref{LagEqQ}， 该方程与牛顿三定律\upref{HSPM03}完全等效． 使用拉格朗日方程对比用牛顿定律的优势在于， 第一，只需写出拉格朗日量，就可由简单求导得到完整的动力学微分方程而无需进行受力分析．第二，方程的形式不随广义坐标的选取而改变．第三，无需解方程即可以得到一些守恒量．
