...
\end{equation}
的子代数．这使得可以通过定义其它的乘积，使得这二者各自成为代数．对于斜对称的 $\Lambda(V^*)$，就是本节强调的重点．使 $\Lambda(V^*)$ 成为一个代数的乘法称为\textbf{外积}(\textbf{wedge})，记号为 $\wedge$，它是通过交错化映射（\autoref{SIofTe_def1}~\upref{SIofTe}）定义的，具体便是
\begin{equation}
Q\wedge R=A(Q\otimes R)
\end{equation}

\subsection{外积}
事实上，基于张量的对称化和交错化\upref{SIofTe}引文处提到的利用，讨论任意张量的对称性和斜对称性，与 $(0,p)$型或 $(p,0)$ 型没有任何区别．为了多样性起见，本节取空间 $\Lambda(V)$．
