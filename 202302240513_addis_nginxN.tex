...
\end{lstlisting}
\item 要特别注意 \verb|静态网页根目录| 以及它的所有上层目录需要可以被 nginx 的用户 \verb|www-data| 读取和执行， 在非 ubuntu 系统可能会有别的用户名。
\item 要查看具体的用户名， 用 \verb`ps aux | grep nginx`， 看第一列中除了 root 都有哪些用户。
\item 要访问静态， 在本机或者其他机器的命令行用 \verb|curl ip地址或者域名|， 如果 \verb|ip| 不是公网 ip 就比较复杂。
\end{itemize}
