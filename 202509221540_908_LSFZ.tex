...
在该定义下，函数 $f$ 在点 $a$ 处可微，当且仅当存在一个在 $a$ 处连续的函数 $q$，使得

$$
f(x) - f(a) = q(x)(x-a).
$$

这样的函数至多只有一个，并且如果 $f$ 在 $a$ 处可微，那么 $f'(a) = q(a)$。
...
对于多元的连续可微（向量）函数，可以使用类似的方法。
这种分解的方法还能推广到更强形式的可微性，比如要求导数满足利普希茨连续性（Lipschitz continuity）、赫尔德连续性（Hölder continuity）等。
事实上，求导本身可以看作是**多项式余式定理**（小贝祖定理，或因式定理）在适当函数类中的推广。

---

要不要我接下来也帮你翻译并整理 **多元链式法则** 的表述？
