...
你的桌子上有一杯温度为$T_1$的水．这杯水会自发分为两部分，使一半杯的温度升为$T_1+\Delta T$、而另一半杯的温度降为$T_1-\Delta T$ 吗？这样你就不需要冰箱而能同时喝到冷饮与热饮了．

升温部分的熵变 $\Delta S_1=C_p \ln \frac{T+\Delta T}{T}$, 降温部分 的熵变$\Delta S_2=C_p \ln \frac{T-\Delta T}{T}$

总熵变 $\Delta S = C_p \ln \frac{T+\Delta T}{T} + C_p \ln \frac{T-\Delta T}{T} = C_p \ln \frac{(T+\Delta T)(T-\Delta T)}{T^2} $
\end{example}
