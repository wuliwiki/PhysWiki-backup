...
在恒等式关系中，很容易发现同一个值可以由不同的方式来表达，比如$1$既可以表示为$\sin^2\alpha+\cos^2\alpha$，也可以表示为$\sec^2\alpha-\tan^2\alpha$，这使得由三角函数构成的表达式与之前早就熟悉的幂形式表示不同，通常表示形式并不唯一。这被称作三角函数的（？）。这是三角函数一个非常重要的特点，也是在高中阶段经常需要处理的问题。

在三角函数的恒等关系中，很容易发现同一个值可以由不同的方式来表达，比如 $1$ 既可以表示为 $\sin^2\alpha+\cos^2\alpha$，也可以表示为 $\sec^2\alpha-\tan^2\alpha$，这使得由三角函数构成的表达式与之前早已熟悉的幂形式表示不同，通常表示形式并不唯一。这是三角函数一个非常重要的特点，也是在高中阶段经常需要处理的问题。
