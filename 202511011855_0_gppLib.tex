...
\item \verb`objdump -T yourlib.so | grep your_symbol` 可以查看 so 中的符号
\item \verb`readelf -Ws yourlib.so | grep your_symbol`  可以看到一个 so 中的符号是函数还是变量，是 global 还是 local（是否被 export）。
...
\verb`RTLD_LOCAL`（默认）\verb`RTLD_GLOBAL`
