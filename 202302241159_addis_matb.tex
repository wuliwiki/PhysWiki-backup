...
\subsection{文件格式}
我们先介绍二进制格式 matb。 matb 文件使用 little endian 储存（链接未完成）， 每个变量的前 8 个字节是变量名的长度 \verb|Nname|， 所有整数都用 \verb|long long| （8 字节有符号整型）储存（下同）， 然后是 \verb|Nname| 字节的变量名， 紧接着是 8 个字节的矩阵元类型编号（对照表未完成， 见 Matlab 的 \verb|type_num()| 函数）， 8 个字节的矩阵维度 \verb|Ndim| （标量的维数是 0， 向量维度是 1） 接着 \verb|8*Ndim| 字节记录每个维度的长度（标量不需要）。 最后， 再记录 \verb|N * sizeof(元素类型)|， 其中 \verb|N| 是元素个数， 等于各个维度的长度相乘。
