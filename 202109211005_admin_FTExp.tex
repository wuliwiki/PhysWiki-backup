...
如果实函数 $f(x)$ 的复数傅里叶变换为 $g(k)$， 即 $g(k)$ 需要满足什么条件才能使 $f(x)$ 是实函数呢？我们从\autoref{FTExp_eq1} 开始入手， 令 $f(x) = f^*(x)$ 得
\begin{equation}
\int_0^\infty [g(k)\E^{\I kx} + g(-k)\E^{-\I kx}] \dd{k} = \int_0^\infty [g^*(k)\E^{-\I kx} ] \dd{k}
\end{equation}
