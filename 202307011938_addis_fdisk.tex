...
\item \verb|defaults| 选项相当于 \verb|rw, suid, dev, exec, auto, nousers, async|.
\item \verb|rw| 是支持读写， \verb|suid| 支持 SetUID 权限， \verb|exec| 支持可执行文件。 \verb|dev| 选项可以支持挂载文件夹中的设备文件（block device 和 character device）。 \verb|nousers| 只允许管理员挂载或弹出硬盘。
