...
\item 4.13 \textbf{线性组合}指有限个向量的， \text{独立} 即线性无关， \text{张成空间} $[S]$， \textbf{规范正交}即正交归一

\item 如果 $\left\{u_{\alpha}: \alpha \in A\right\}$ 是规范正交的, 对于每个 $x \in H$, 我们都对应于指标 $A$ 上的一个复函数 $\hat{x}$, 它定义为 $\hat{x}(\alpha)=\left(x, u_{\alpha}\right) \quad(\alpha \in A)$. 有时, 称这些数 $\hat{x}(\alpha)$ 为关于集 $\left\{u_{\alpha}\right\}$ 的傅里叶系数.
