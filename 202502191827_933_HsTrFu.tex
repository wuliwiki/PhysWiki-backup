...
之前提到过第一象限角全部都是锐角，他们都对应着直角三角形中的某条边或边的比值，因此无论计算哪个三角函数，都是正的，不需要考虑符号问题，计算起来格外简便。

\textbf{诱导公式（Формулы приведения）}\footnote{приведение原意为“换算，导出，引导”，因此其实可以将这一组公式理解为“换算公式”，即：将其他象限角换算至一象限的情况。英语中原本无此名称，一般只称作对称性等，近年被人译作“induction formula”，但使用并不广泛。}是三角函数中的一类恒等式，它利用三角函数在单位圆上的对称性\footnote{高中教材通常采用三角函数线进行推导，而实际上，通过三角函数的定义结合单位圆的对称性，可以迅速得出这些结论。}，建立不同象限角与第一象限锐角之间的关系。
...
从上图，可以观察出，第二、三、四象限的角，只要分别对y轴、原点、x轴做终边的对轴，就能联合π、π/2这两位高中时期混得脸熟的朋友，将不同的角又变回成第一象限角啦~
因为人类偏爱为了把世界上所有的角都能够转换成友好的第一象限角，人类数学家发现了一个现象：

这些关系的总结即构成了诱导公式，它不仅反映了三角函数的对称性质，也为进一步研究三角恒等变换和计算提供了便利。
