...
解析表达式见论文． 现在讨论如何从波函数中提取每个 $(n_1, l_1, m_1, l_2, m_2)$ 分波的延迟． 首先需要使用\autoref{HeAnal_eq6} 的角向基底． 投影后得（bra 已经未对称化）
\begin{equation}
\braket{n_1,l_1,m_1,\bvec r_2}{\Psi} = \frac{\sqrt{2}}{r_2} \int r_1 R_{n_1,l_1}(r_1) \psi_{l_1,l_2}^{m_1,m_2}(r_1,r_2)\dd{r_1} Y_{l_2,m_2}(\uvec r_2)
\end{equation}
于是该分波的单电子分布为
\begin{equation}
P_{n_1,l_1,m_1}(\bvec r_2) = \abs{\braket{n_1,l_1,m_1,\bvec r_2}{\Psi}}^2
\end{equation}
径向分布为
\begin{equation}
P_{n_1,l_1,m_1}(r_2) = P_{n_1,l_1,m_1}(\bvec r_2)  \dd
\end{equation}
