...
\hline
9 & $ A \cap \complement_U( A) = \varnothing $ &$ A \cup \complement_U( A) = U $  & 排中律 \\
\hline
...
结合律（Associative Law）、分配律（Distributive Law）、吸收律（Absorption Law）、同一律（Identity Law）：幂等律（Idempotent laws）、排中律（Laws of the excluded middle）
...
\begin{array}{c} 
\displaystyle
\complement_U(A_1\cup A_2\cup\cdots\cup A_n)=\complement_UA_1\cup \complement_UA_2\cup\cdots\cup \complement_UA_n\\  
\displaystyle
