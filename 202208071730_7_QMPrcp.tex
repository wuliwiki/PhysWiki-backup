...
\begin{corollary}{可对角化算符的用本征矢展开}\label{QMPrcp_cor2}
设$X$是一个可对角化的算符，有一个由本征矢量$\{\ket{s_\alpha}\}$构成的基，其中$\alpha$是复数，且$X\ket{s_\alpha}=\alpha\ket{s_\alpha}$．则在这组基下有
\begin{equation}
X=\sum_{\alpha}\alpha\ket{s_\alpha}\bra{s_\alpha}
\end{equation}
\end{corollary}

\textbf{证明}：

设$X$在给定本征基下的矩阵表示为$\mathcal{M}$，其第$i$行$j$列的元素是$m^i_j$．
