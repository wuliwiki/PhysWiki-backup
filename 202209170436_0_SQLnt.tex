...
\item 选项 \verb|PRIMARY KEY| 可以使表格的某一列不出现重复的值 % 且不为 \verb|NULL|．
\item \verb|AUTO_INCREMENT| 选项可以在插入时自动增加， 例如 ID 号． 在 sqlite 中记为 \verb|AUTO_INCREMENT| 只能出现在 \verb|INTEGER PRIMARY KEY AUTOINCREMENT| 中．
\end{itemize}
