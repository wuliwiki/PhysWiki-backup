...
\end{equation}
\end{definition}
特别地，之前提到的常数列是 $q = 1$ 的等比数列。注意这里与等差数列的定义表述不同，并没有采用“每一项与前一项的比为同一个常数”的说法，尽管这个说法更像是“等比”这个名称的由来，但其实数列 $\{a_n\}$ 所有项都为$0$的情况也可以看作是等比数列，它是符合倍数是固定值的。另外，因为存在分母不能为0的情况，所以除非等比数列 $\{a_n\}$ 所有项都为$0$，等比数列中不会出现某一些的值为0，某一些不为0的情况。
