...
\subsubsection{逆元、除法}
可以证明每个不为零的复数 $z$， 都存在唯一的乘法\textbf{逆元}（简称\textbf{逆}） $z^{-1}$， 使得
...
z^{-1} = \frac{z^*}{\abs{z}^2}
\end{equation}
要证明该式， 把\autoref{CplxNo_eq8} 代入\autoref{CplxNo_eq9}， 再使用\autoref{CplxNo_eq2} 即可． 要证明逆元的唯一性， 假设 $z$ 的两个逆元分别为 $z_1, z_2$， 有 $zz_1 = 1 = zz_2$, 两边左乘 $z^{-1}$， 得 $z_1 = z_2$． 证毕．
