...
注意， 虽然四元数的加法显然满足交换律， 但由于一般来说向量的叉积不可交换， 因此四元数乘法一般不具有交换律． 只有 $\bvec{a}$ 平行于 $\bvec{b}$ （包括其中之一为 $\bvec 0$）的时候， 才会有 $\bvec{a}\times\bvec{b}= \bvec{0} =\bvec{b}\times\bvec{a}$． 四元数的乘法满足分配律（\autoref{Quat_exe1}）．
...
\end{equation}
其中$\uvec{a}$是和$\bvec{a}$同方向的单位矢量，$a$是 $\bvec{a}$ 作为四元数的模或者作为三维实向量的模．
\end{definition}
...
我们以上定义的四元数，可以看成四个基向量 $1$， $\uvec i$， $\uvec j$ 和 $\uvec k$ 的线性组合的集合， 而这些线性组合的系数取自实数域 $\mathbb R$． 如果系数还可以取自复数域 $\mathbb C$， 定义出来的数被称为\textbf{双四元数}， 或\textbf{复四元数}．一个双四元数可以看成一个复数和一个复数域上的三维向量的组合．
...
\qquad
\uvec i=\left[\begin{matrix}\I&0\\0&-\I\end{matrix} \right]
\qquad
\uvec j=\left[\begin{matrix}0&1\\-1&0\end{matrix} \right]
\qquad
\uvec k=\left[\begin{matrix}0&\I\\\I&0\end{matrix} \right]
\end{equation}
那么一个任意的四元数或双四元数$A=a_0+\bvec{a}$可以用矩阵表示为
\begin{equation}
\left[\begin{matrix}a_0+\I a_1&a_2+\I a_3\\-a_2+\I a_3&a_0-\I a_1 \end{matrix} \right]
\end{equation}

\begin{example}{矩阵表示的例子}\label{Quat_ex1}
\begin{equation}
1+\left(\begin{matrix}1\\0\\\I\end{matrix} \right)=1+1\uvec i+\I\uvec j =\left[\begin{matrix}1+\I&-1\\-1&1-\I\end{matrix} \right]
\end{equation}
\begin{equation}
u=\frac{1}{2}(1-\I\uvec i)=\left[\begin{matrix}1&0\\0&0\end{matrix} \right]
\end{equation}
\begin{equation}
v=-\frac{1}{2}(\uvec j+\I\uvec k)=\left[\begin{matrix}0&0\\1&0\end{matrix} \right]
\end{equation}
\end{example}
