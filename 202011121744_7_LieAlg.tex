...
设$\mathfrak{g}$为域$\mathbb{F}$上的$n$维李代数，它在基$\{\bvec{x}^i\}$下的\textbf{结构常数}为$C^k_{ij}$，在基$\{\bvec{y}^i\}$下的\textbf{结构常数}为$D^k_{ij}$，而且有过渡矩阵\upref{TransM}$a^i_j$使得$\bvec{y}^j=a^i_j\bvec{x}^i$，那么有变换法则：
\begin{equation}
