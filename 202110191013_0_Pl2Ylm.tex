...
\subsection{额外相位}
如果给 $j_l$ 加上额外相位 $\delta_l(k)$， $j_l[x+\delta_l(k)]$ 如何组合成 $\psi_{\bvec k}^{(\pm)}$ 呢？ $+$ 代表平面波入射球面波出射， $-$ 反之． 答案就是
\addTODO{有待数值验证}
\begin{equation}
\begin{aligned}
&\ket{\psi_\bvec k^{(\pm)}} = \frac{1}{(2\pi)^{3/2}}[\E^{\I \bvec k \vdot \bvec r} + f(\uvec k)\E^{\pm\I kr}]\\
