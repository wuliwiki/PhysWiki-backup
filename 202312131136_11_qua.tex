...
\subsection{二次型的坐标变换}
当我们用上述定义表示二次型时，如果基向量组变换，二次型的形式亦有所不同。具体而言，如果利用过渡矩阵$T$改变基向量组，由相似变换的知识可知，在新基下向量$\textbf {v'}=T\textbf{v}$，那么新的二次型形式为：
\begin{equation}
\textbf{v}^T Qv=(T^{-1}\textbf{v})^T Q(T^{-1}v)=\textbf{v}^T (T^{-1})^{T}QT^{-1}v~.
\end{equation}
