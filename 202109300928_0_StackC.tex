...
现在把所有的情况根据非空盒子的个数分类． 非空盒子个数可能为 $i=1$ 个（ $n$ 个小球都在里面）， $i=2$ 个， 一直到 $i=\min\qty{m,n}$ 个（若 $n\leqslant m$ 则每个小球都在不同的盒子）． 如果用 $i$ 个有编号的盒子装小球， 那么首先从 $m$ 个盒子里面选择 $i$ 个会有 $C_n^i$ 种情况． 然后要考虑这 $i$ 个有编号盒子装 $n$ 个小球（不为空）又有几种情况． 用隔板法\upref{BarCom}得到共有 $C_{n-1}^{i-1}$ 种情况． 所以一个 $i$ 对应 $C_m^i C_{n-1}^{i-1}$ 种情况．
