...
考虑实函数 $f(x)$． 若无论要求 $f(x)$ 和一确定实数 $A$ 的距离$\abs{a_n - A}$ 有多小， 都能通过不等式 $x > X$ （$X$ 是一确定实数）来满足， 那么我们说 $A$ 是函数 $f(x)$ 在 $x$ 趋于正无穷时的极限， 记为
\begin{equation}
...
考虑实函数 $f(x)$． 若无论要求 $f(x)$ 和确定实数 $A$ 的距离 $\abs{a_n - A}$ 有多小， 都能通过不等式 $\abs{x-x_0} < \delta$ （$\delta$ 是一确定实数）使要求成立， 那么我们说 $A$ 是函数 $f(x)$ 在 $x$ 趋于 $x_0$ 时的极限， 记为
\begin{equation}
