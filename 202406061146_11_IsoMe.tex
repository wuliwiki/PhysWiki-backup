...
非退化线性空间是可以有退化的子空间的。比如$f(\bvec e_1,\bvec e_1)=-1,f(\bvec e_2,\bvec e_2)=1$则$\bvec e_1+\bvec e_2$对应的一维子空间便是退化的。根据\autoref{the_OrSp_1}~\upref{OrSp}和\autoref{the_OrSp_2}~\upref{OrSp}可知，若$W$是非退化的，则$V$内总存在一组基使得$V=W\oplus W^{\bot}$。因此，如果$W$等距变换等距变换是比较简洁的块对角形式。