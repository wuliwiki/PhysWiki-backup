...
\begin{example}{}
对\autoref{mbdy_eq6} 的简要说明．
\begin{figure}[ht]
\centering
\includegraphics[width=5cm]{./figures/mbdy_1.png}
\caption{边界处的高斯面.仿自\cite{GriffE}} \label{mbdy_fig1}
\end{figure}
在两介质的边界处绘制一个高斯面，有麦克斯韦方程组（介质）\upref{MWEq1}的积分形式 $\oint \bvec D \vdot \dd{\bvec A} = q_f$．

当高斯面高度 $d\rightarrow0$时，D场的垂直分量只存在于上下表面，即$D^\perp_{above} A- D^\perp_{below} A= q_f$ (二者符号不同是因为积分时总是取曲面向外为面法向量的正方向)．两边同除以A，得
\end{example}
