...
有一种疾病A，它的发病率为 2 \% ，通过临床数据的统计，我们发现患者中有70\%吸烟，有百分之30\%不吸烟。我们调查发现某地区大概20\%吸烟，则一个不吸烟的人患疾病A的概率为多大？\\
我们用事件A表示患有疾病A，用事件$B_1$表示吸烟,$B_2$表示不吸烟，则有$P(B_1|A) = 0.7,P(B_2|A)=0.3$,我们要求的概率可以写为$P()$而根据贝叶斯公式
\end{example}
