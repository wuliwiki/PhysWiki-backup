...
\end{equation}
令$W_i=\opn{Span}\{x_i,B(x_i),B^2(x_i)...B^{l_i-1}(x_i)\}$，有$B^{l_i}|_{W_i}=0,i=1,2...k$。由于$B^{l_i-1}(x_i)\in \opn{ker}B$，因此可以扩充$\{B^{l_i-1}(x_i)\}^k_{i=1}$至$\opn{ker}B$的一组基，设其余线性无关部分为$\{\bvec v\}^m_{i=1}$。接下来我们证明$\{\bvec v\}^m_{i=1}\cup {x_i,B(x_i),B^2(x_i)...B^{l_i-1}(x_i)\}^{k}_{i=1}$
