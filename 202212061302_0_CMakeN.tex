...
\item \verb`set` 也可以把许多变量变成一个 list， 如 \verb`set(Foo a b c)` （等效于 \verb|set(Foo "a" "b" "c")|） 将 \verb`Foo` 变成 list， 当 \verb`${Foo}` 被使用时相当于 \verb`a b c`， 例如 \verb`command(${Foo})` 相当于 \verb`command(a b c)`． 另外 \verb`command("${Foo}")` 相当于 \verb`command("a b c")` （双引号中的是一个变量）
