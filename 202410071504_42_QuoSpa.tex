...
设 $W$ 是向量空间 $V$ 的一个子空间。我们想通过 $W$ 来定义 $V$ 中元素的一个等价关系，并由此得到 $V$ 的一个划分（商集）。对于 $v \in V$，$v + W$ 是 $V$ 的一个仿射集（\autoref{cor_AffSet_1}），全体（关于 $W$ 的）仿射集的集合构成里一个划分（商集），定义了一个等价关系——对于 $v_1, v_2\in V$，如果 $v_1 - v_2\in W$，则称它们是关于 $W$ 等价的，记为 $v_1 \sim v_2$。我们把这个划分（商集）记为$V / W: = V / \sim$，包含 $v$ 的等价类记为 $[v]: = v + W$.
