...
参照\autoref{fig_HsTrFu_5} ，既然任意角\aref{定义}{def_HsAngl_1}在坐标系的一个单位圆上，并且由任意角与单位圆的交点$P(u,v)$确定。自然地，就会想到三角函数的定义也应该与单位圆和点$P(u,v)$有关。另外，由于相差为$2\pi$或$360^\circ$的任意角终边相同，显然这些角的三角函数值应该相同。也就是说，所有的基本三角函数都应该是周期函数，且$T=2\pi$都应该是它们的一个周期。

下面的问题就是如何在单位圆中构造包含角$\alpha$的直角三角形。

在直角三角形中，$\sin\theta = \frac{\text{对边}}{\text{斜边}}$，而在单位圆上，斜边恒为 $1$，所以 $\sin\theta$ 直接就是 $y$ 坐标，$\cos\theta$ 直接就是 $x$ 坐标。
