...
序数乘法也可以通过**对 \(\beta\) 进行跨无限递归**来定义：  

- 当右因子 \(\beta = 0\) 时，普通乘法规则给出：
\[
\alpha \cdot 0 = 0
\]
- 当 \(\beta > 0\) 时，\(\alpha \cdot \beta\)是所有\((\alpha \cdot \delta) + \alpha\)（其中 \(\delta < \beta\)）中**最小的大于等于它们的序数**。
...
- 极限序数：
\[
\alpha \cdot \beta = \bigcup_{\delta < \beta} (\alpha \cdot \delta)
\]
...
\]
这个顺序型和 \(\omega + \omega\)相同。

对比 \(2 \cdot \omega\)，其顺序关系是：
\[
0_0 < 1_0 < 0_1 < 1_1 < 0_2 < 1_2 < 0_3 < 1_3 < \dots~
\]
如果重新标号，这与 \(\omega\)的顺序型相同。

因此：\(\omega \cdot 2 = \omega + \omega \neq \omega = 2 \cdot \omega
\)这表明*序数乘法通常不是可交换的**。  
