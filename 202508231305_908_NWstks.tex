...
已有研究证明，这种形式能得到与原始纳维–斯托克斯方程完全一致的速度解[19]：
$$
...
$$
其中：$\Gamma_D$：施加Dirichlet 边界条件的部分；$\Gamma_N$：施加 Neumann 边界条件的部分；且两部分互不重叠：$\Gamma_D \cap \Gamma_N = \emptyset$。该系统在时间区间 $(0, T)$ 内的方程为[20]：
$$
\begin{cases}
