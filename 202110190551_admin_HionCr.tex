...
\pentry{电磁波包的频谱\upref{WpEng}， 含时微扰理论\upref{TDPTc}}

本文使用原子单位制\upref{AU}．一阶微扰理论就是单光子电离， 即两能级之间的能量等于单个光子的能量．

\subsection{长度规范下的微扰跃迁理论}
含时微扰理论（\autoref{TDPTc_eq1}~\upref{TDPTc}） 得到的一阶系数为（$H'(t) = Wf(t)$， $\tilde f = \mathcal F[f]$）
\begin{equation}\label{HionCr_eq1}
c_{\alpha'}(+\infty) = -\I\sqrt{2\pi} \mel{\alpha'}{W}{\alpha} \tilde f(-\omega_{\alpha'\alpha})
\end{equation}
长度规范\upref{LenGau}中，
\begin{equation}
