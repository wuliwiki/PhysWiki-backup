...
本节将以一种“统一”的角度给出电磁力和引力的作用量。约定：重复指标代表求和，$\mu,\nu,\rho,\ldots$ 指标取值遍及所有分量 $$，$i,j,k,\ldots$ 范围除去时间分量。 
...
从而运动方程为
\begin{equation}
m\dv[2]{x^\rho}{\tau}=F^\rho_{\mu}\dv{x^\mu}{\tau}~.
\end{equation}

利用四动量的定义\upref{Comple} $p^\mu:=\dv{x^\mu}{\tau}$，上式写为
\begin{equation}
\dv{p^\rho}{\tau}=F^\rho_{\mu}\dv{x^\mu}{\tau}~.
\end{equation}
令 $\rho=1$，得 $\dv{p^1}{\tau}=F^1_{0}\dv{x^0}{\tau}+F^{1i}\dv{x^i}{\tau}$。
