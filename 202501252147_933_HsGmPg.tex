...
\end{equation}
它是\autoref{the_HsIden_2} 的一种特殊形式。通过它也可以进一步推得$a^n-b^n$的因式分解：
\begin{equation}
\begin{aligned}
a^n-b^n& = a^n(1-\left({b\over a}right)^n)\\
&= a^n\lfet(1-{b\over a}\right)(1+{b\over a}+\cdots+\left({b\over a}\right)^{n-})\\
&= a_1(1+q+ \cdots +q^{n-1})~.
\end{aligned}
\end{equation}
