...
\textbf{证明：}
1. $1\Rightarrow 2$：设 $\forall x,y\in T$。同于树是连通的，所以存在 $x$ 到 $y$ 的路 $R=xe_1\cdots y$。设 $R'=xe'_1\cdots e'_n y$ 是连接 $x,y$ 的另一路。由于 $T$ 无圈，所以链 $xe_1\cdots y|e'_n\cdots e'_1x$ 中在 $|$ 两边一定有两个内点相同，否则该链就是一个圈。由于左边和右边分别是路，因此两个内点只能一个在一边。设这两个内点是 $x_{i1},y_{i1}$，于是 $R_1=W(x_{i1},y_{i1})$ 是 $x_{i1}y_{i1}$ 链。

由于该链端点相同，我们仍可以重复上面的论述。设 $R',R$ 分别长 $k,l$，且，那么
