...
显然，$s,S$ 分别是对应该分划的积分和中的下界和上界（\autoref{def_SupInf_1}~\upref{SupInf}），或记作$\underline \sigma(f, \mathcal P),\bar \sigma(f, \mathcal P) $。如果 $f(x)$ 是连续函数，$s,S$ 则是\textbf{下确界}和\textbf{上确界}(\autoref{def_SupInf_2}~\upref{SupInf})。
