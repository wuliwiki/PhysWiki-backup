...
\subsection{初步推导}
设能级 $l$ 的简并度为 $\omega_l$（可以想象该能级上有 $\omega_l$ 个房间，代表不同的单粒子微观状态）。如果给能级 $\omega_l$ 分配 $a_l$ 个粒子，那么由排列组合可得到 $N!/\Pi_l a_l!$ 种分配方法。随后要给每个能级上的粒子分配到确定的房间。对于玻尔兹曼分布，\textbf{不涉及全同粒子假设}，我们认为粒子之间是可以分辨的。每种粒子都可以分配到 $\omega_l$ 种房间，于是共有 $\omega_l^{a_l}$ 种分配方式。总的微观状态数为
\begin{equation}
