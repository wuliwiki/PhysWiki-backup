...
形式上$H=A^2p^2+B^2x^2$，其中$A^2=1/2m, B^2=m\omega^2/2$．如果$H, x, p$是\textbf{数字}，那么有$H = (Ap+\I Bx)(Ap-\I Bx)$；然而实际上它们都是\textbf{算符}，所以有：
\begin{equation}\label{QSHOop_eq1}
(Ap+\I Bx)(Ap-\I Bx) = H+\I AB[x, p] = H-\hbar AB
\end{equation}

利用\autoref{QSHOop_eq1} 容易证明，算符$(Ap\pm \I Bx)$的对易关系为：
\begin{equation}\label{QSHOop_eq2}
[(Ap-\I Bx), (Ap+\I Bx)] = 2\hbar AB
\end{equation}
把\autoref{QSHOop_eq2} 代回\autoref{QSHOop_eq1} 易得
\begin{equation}
[H, (Ap\pm \I Bx)] = [H- \hbar AB, (Ap\pm \I Bx)] = \pm 2\hbar AB (Ap\pm \I Bx)
\end{equation}
由\autoref{RLop_eq1}~\upref{RLop}，显然$(Ap\pm \I Bx)$是$H$的升降算符，能把$H$的本征态的本征值改变$\mp $．
