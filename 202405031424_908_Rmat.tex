...
\end{equation}
这里使用了\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/89c3c2ae45d0c84c.png}
\caption{请添加图片标题} \label{fig_Rmat_1}
\end{figure}
狄拉克符号\upref{braket}表示积分。 这组基底如何具体计算呢？ 首先还是要明确 $x=a$ 处的边界条件。 一个简单的例子是令 $u(a) = 0$， 解 $H$ 的本征基底。 这相当于解 $[0,a]$ 中的无限深势阱加上势能 $V(x)$。 另一个简单的例子是把边界条件 $u(a) = 0$ 改成 $u'(a) = 0$， 也能得到一组基底。 更妙地， 也可以把这两组基底合并。 可以验证这三组基底都满足\autoref{eq_Rmat_3} 即\autoref{eq_Rmat_2}。
