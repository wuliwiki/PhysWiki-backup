...
\end{equation}
这说明对数运算和指数运算互为逆运算。如果觉得一会是$c$，一会是$b$和以往的逆运算感觉不一样，可以这样理解：对于乘除法，乘上一个数再除以这个数，原本的数不变，因此二者互为逆运算。而这里，给一个数$a$做指数再取a的对数，可以“消掉”底数  $a$ ，反过来，取一个。只不过给定  $a^b$ ，通过对数运算可以“消掉”底数  $a$ ，恢复原来的指数  $b$ 。
