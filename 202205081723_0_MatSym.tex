...
\item 要检查一个 \verb|sym| 类型的对象 \verb|x| 是否有误差， 只需要在命令行中把它显示出来（可以使用 \verb|disp()| 函数， 也可以直接运行 \verb|x| 不加分号）． 如果显示中出现了小数点， 那么他就是变精度浮点数． 对于 \verb|sym| 类型的整数，无论有多少位，如果没有误差则会把每一位显示出来而不是用科学计数法（科学计数法带小数点，表示有误差）．
