...
计算以下 Feynman 积分
\begin{equation}
I^\mu = \mu^{4-d}\int\frac{\dd[d]{k}}{(2\pi)^d} \frac{k^\mu}{(k^2-m^2+i\epsilon)((p-k)^2+i\epsilon)}
\end{equation}
\end{exercise}
利用 Feynman 参数化
\begin{equation}
\begin{aligned}
I^\mu 
&= \mu^{4-d} \int\frac{\dd[d]{k}}{(2\pi)^d}\int_0^1\dd x \frac{k^\mu}{(k^2+ xp^2-2xpk-(1-x)m^2 +i\epsilon)^2}\\
&=\mu^{4-d}\int_0^1\dd x\int\frac{\dd[d]{k}}{(2\pi)^d}  \frac{k^\mu}{((k-xp)^2-\Delta+i\epsilon)^2},\quad \Delta = (1-x)m^2-x(1-x)p^2\\
&=\mu^{4-d} \int_0^1 \dd x \int\frac{\dd[d]{l}}{(2\pi)^d} \frac{l^\mu + x p}{(l^2-\Delta+i\epsilon)^2}
\end{aligned}
\end{equation}


由于根据洛伦兹结构，上述积分是一个洛伦兹矢量，且是关于 $p^\mu$ 的函数，因此
\begin{equation}
I^\mu = A(p^2) p^\mu
\end{equation}
剩下的问题就是求解 $I^\mu$。利用 $2k\cdot p=k^2-(p-k)^2+p^2$，我们有
\begin{equation}
\begin{aligned}
A(p^2) = I^\mu p_\mu/p^2 = \frac{\mu^{4-d}}{p^2} \int \frac{\dd[d]{k}}{(2\pi)^d} \frac{(k^2-m^2+i\epsilon) - ((p-k)^2+i\epsilon) +p^2+m^2}{2(k^2-m^2+i\epsilon)((p-k)^2+i\epsilon)}
\end{aligned}
\end{equation}
