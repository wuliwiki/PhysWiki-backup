...
以下介绍的是 LLVM IR 的文本格式。 如果要写个程序生成 LLVM IR，那么应该直用 \enref{API}{LLVMpi} 而不是用程序生成文本格式。
...
define i32 @main() {
  ; 基本块标签
  entry:
    ; 分配栈上的 int32 变量，返回指针给虚拟寄存器 `i32* %var`
    ; 返回值的类型省略了
    %var = alloca i32
...
\item \verb`i32` 本身没有正负号，只是 32 个 bit！正负号约定取决于 instruction
\begin{lstlisting}[language=none]
%cmp_s = icmp slt i32 %a, %b  ; 有符号 < 算符
%cmp_u = icmp ult i32 %a, %b  ; 无符号 < 算符
\end{lstlisting}
\end{itemize}
...
%b = or i32 20, 0
\end{lstlisting}
但是不能用 \verb`%a = i32 10` （必须要一个 instruction）！
...
堆上数组：
\begin{lstlisting}[language=none]
; 声明 malloc 函数（也可以在函数体中声明）
declare i8* @malloc(i64)

define i64 @heap_memory_example(i64 signext %"my_var") #0 {
  top:
    ; 堆上分配：`double *array = (double *)malloc(10*sizeof(doubles));`
    %size = mul i64 10, 8
    ...
    ; 赋值 `value = 3.14`
    %value = fadd double 0.0, 3.14
    ; 元素指针 `ptr = array[0][2]`（也可以使用和栈一样的命令）
    %ptr = getelementptr inbounds double, double* %array, i64 2
    store double %value, double* %ptr, align 8
...
\item 缩进不重要
\item 注释用 \verb`;`
\item 单个指令中间不能换行（用 \verb`\` 也不行），字符串可以
\item 函数返回用 \verb`ret` 不是 \verb`return`
\item \verb`@名字` 用于声明全局函数或变量
\item \verb`signext`: 指定入参实参应该 sign-extended 到 64 位。一些机器上，入参可能通过小于 64 位的寄存器上
...
\item \verb`getelementptr` （GEP）的格式为 \verb`指针 = getelementptr 基类型, 基类型* 指针, 指标类型1 ind1, 指标类型2 ind2 ...`。 在取数组第 3 个元素时，最后是 \verb`i64 0, i64 2`， 第一个从 \verb`数组的指针` 中获取数组 \verb`array`， 第二个从 \verb`array` 中获取获取第 3 个元素，相当于 \verb`指针[0][2]`。
\end{itemize}
...
entry:
  ; 判断 `if x > 0, goto if.then; else, goto if.else`
  %cmp = icmp sgt i32 %x, 0
  br i1 %cmp, label %if.then, label %if.else

if.then:
  %value.then = mul i32 %x, 2
  br label %merge

if.else:
  %value.else = add i32 %x, 5
  br label %merge

merge:
  %y = phi i32 [ %value.then, %if.then ], [ %value.else, %if.else ]
  ret i32 %y
}
\end{lstlisting}
...
\begin{lstlisting}[language=none]
define void @countUpTo(i32 %n) {
  entry:
    ; 循环变量 i 的栈上指针
    ; 栈上变量可以被多次赋值而不需要 phi 函数！
    %i = alloca i32
    ; 赋值 `i = 0;`
    store i32 0, i32* %i
    br label %loop.cond
...
loop.cond:
  %i = phi i32 [ 0, %entry ], [ %next.val, %loop.body ]
  %cond = icmp slt i32 %i, %n
  br i1 %cond, label %loop.body, label %loop.end

loop.body:
  %next.val = add i32 %i, 1
  br label %loop.cond
...
\subsubsection{intrinsics：内建函数}
\begin{lstlisting}[language=none]
; Square root
%sqrt = call double @llvm.sqrt.f64(double %x)

; Absolute value
; false = don't trap on INT_MIN
...
%popcnt = call i32 @llvm.ctpop.i32(i32 %x)
\end{lstlisting}
