...
\item 如果上传时发现图片文件的 hash 已存在于 \verb|images.hash|， 则检查 \verb|images.figures| 中 \verb|figures.aka| 为空的图片环境（只有一个或者没有，否则就是 bug）。 如果有且未被标记 \verb|figures.deleted|， 有就提示已存在， 是否引用（\verb|\autoref{}|），若有多个就把标签全部列出。 如果没有， 或者
