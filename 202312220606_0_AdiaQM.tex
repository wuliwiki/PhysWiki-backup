...
\end{equation}
注意 $n=m$ 时该式两边恒为零，\autoref{eq_AdiaQM_10} 中 \textbf{$\mat A$ 的对角元也恒为零}。 且通过该式容易证明 \textbf{$\mat A$ 是厄米矩阵}。 要计算 $\mat A$ 的非对角元， 由\autoref{eq_AdiaQM_4} 得
\begin{equation}\label{eq_AdiaQM_11}
\braket*{\psi_m}{\dot\psi_n} = \frac{\mel*{\psi_m}{\dot H}{\psi_n}}{E_n-E_m} \qquad (E_m\ne E_n)~.
\end{equation}
这可以帮我们精确计算矩阵 $\mat A$。 但这并没有告诉我们若存在简并如何计算 $E_n=E_m$ 的非。
...
\end{equation}
这是齐次亥姆霍兹方程组，若把矩阵 $\I\braket*{\psi_m}{\dot\psi_n}\E^{\I(\theta_n-\theta_m)}$ 记为矩阵 $\mat A$，。 再令 $\mat A$ 中不包含在求和中的矩阵元为零（它们已经被绝热近似忽略），那么 $\mat A$ 就是一个块对角矩阵，每个对角块代表一个本征子空间（或者若干个可能在某时刻本征值相同的本征子空间张成的空间），不同子空间之间不存在耦合。把所有 $c_m(t)$ 按顺序排成列向量 $\bvec c(t)$，上式可以表示为矩阵乘法
\begin{equation}
