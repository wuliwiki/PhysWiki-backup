...
\begin{example}{}
假设两个粒子某时刻的态矢可以用单个张量积表示 $\ket{i}\ket{j}$． 显然若 $i = j$， 则这个态已经是对称的，不需要再对称化， 也不可能反对称化．  若 $i \ne j$， 则态矢是不对称的， 我们可以用以下方法对称化
\begin{equation}
\frac{1}{\sqrt{2}}(1 + P_{12})\ket{i}\ket{j} = \frac{1}{\sqrt{2}}(\ket{i}\ket{j} + \ket{j}\ket{i})
\end{equation}
为什么乘以 $1 + P_{12}$ 再归一化就可以对称化呢？ 这是利用了交换算符的性质 $P_{1,2}^2 = 1$
\end{example}
