...
设 $W$ 是向量空间 $V$ 的一个\enref{仿射集}{AffSet}。于是我们可以通过 $W$ 来定义 $V$ 上的一个等价关系（\autoref{def_Relat_1}），并由此得到 $V$ 的一个划分（商集）。



\begin{definition}{通过仿射集构造的等价关系}
设 $W$ 是向量空间 $V$ 的仿射集，其对应的向量子空间为 $U$（\autoref{the_AffSet_2}）。 则称 $v_1,v_2\in V$ （关于 $W$ ）是\textbf{等价}的，若
\begin{equation}
