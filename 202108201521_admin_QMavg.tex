...
本文使用原子单位制\upref{AU}． 我们先来回顾测量理论． 假设某个测量量 $q$ 对应的算符为 $Q$， 有离散的本征态 $\ket{\phi_i}$ （$i = 1,2\dots$）， 数量可以是有限或无穷多个， 对应的本征值为 $q_i$， 满足
...
\end{equation}
注意 $\ket{\psi}$ 必须要归一化， 即 $\sum_i \abs{c_i}^2 = 1$．
...
\ev{q} = \mel{\psi}{Q}{\psi}
\end{equation}
要验证等效， 可以将\autoref{QMavg_eq2} 代入该式， 得
\begin{equation}
\begin{aligned}
...
\end{equation}
其中 $\delta$ 是狄拉克 delta 函数\upref{Delta}． 那么波函数的展开变为
...
c(q) = \braket{q}{\psi}
\end{equation}
而 $\abs{c(q)}^2$ 则是测得 $q$ 的概率密度函数． 这时可以定义平均值为（未完成：链接到统计相关词条）
