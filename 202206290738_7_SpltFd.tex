...
对于任意$g\in\mathbb{F}[x]$和任意$k\in\mathbb{K}$，必有$\sigma(g(k))=g(\sigma(k))$．因此，$h$的根必须被$\sigma$映射为$h$的根．

记$h$的根为$\alpha_1, \alpha_2, \cdots, \alpha_r$，且$\opn{deg}$显然向量组$\{1, \alpha_1, \alpha_1^2, \cdots, \alpha_1^\}$\footnote{参见\autoref{FldExp_the1}~\upref{FldExp}的证明．}
