...
\subsubsection{自由能的物理翻译}
既然我们以及大费周章地引入了自由能的概念，我们为何不深入一些、继续探索自由能背后的含义，以及对热力学第二定律更深刻的理解？

同样地，我们假定系统等温等容，但是发生了一个变化，即系统的内能和熵改变了。现在，我们要求这个系统能对外做的最大非体积功（功可以被分为体积功与非体积功，体积功指因为系统体积变化而导致的机械功  $w_\text{体}=p\Delta V$，而非体积功指其余类型的功，例如电功等。）。根据热一和热二，我们写出：
\begin{equation}
\left \{
    \begin{aligned}
    \Delta U &= q-w_\text{非体}\\
...
\right.~.
\end{equation}
注意，根据状态量与路径无关的性质，我们假定内能的变化$\Delta U$和熵的变化$\Delta S$是已知的。
...
$$
我们注意到，等号右边的竟然就是我们之前引入的$\Delta F$!(差一个负号) 
\begin{equation}
w_\text{非体} \le - \Delta F~,
\end{equation}
因此，对于一个等温等容过程，$-\Delta F$的数值含义是过程后系统能对外做的最多的非体积功。因此，我们常说，自由能度量了系统的做功能力。同时，我们深刻明白了，热二是如何约束系统的能量转换的。
