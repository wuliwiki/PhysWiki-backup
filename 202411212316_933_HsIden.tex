...
恒成立的等式称作\textbf{恒等式（identities）}，某些时刻为了强调恒等关系会采用$\equiv$来代替$=$。对于一个恒等式，等号两侧的对应参数在代数关系上也需要完全一致。这是恒等式的本质属性，也是待定系数法成立的基础。比如：若等式$ax^2+bx+c=x^2+2x+1$是恒等式，则必有$a=c=1,b=2$，若不是恒等式，则需要研究求解方程$(a-1)x^2+(b-2)x+(c-1)=0$，根据$\Delta=b-2$。
