...
所以
$$\left|\int_{C(R)} f(z) \dd z - \I K (\theta_2 - \theta_1)\right| = \left| \int_{C(R)} \left[ f(z) - \frac{K}{z} \right] \right| \le \int_{C(R)} \left| z f(z) - K \right| \cdot \frac{|\dd z|}{|z|}~.$$

而在命题所述的条件下，当 $|z| \right \infty$ 时，$z f(z)$ 一致地趋于 $K$，这表明，$\forall \epsilon > 0, \exists M(\epsilon) > 0$（$M(\epsilon)$ 与 $\arg z$ 无关），当 $$
\textbf{证毕。}
