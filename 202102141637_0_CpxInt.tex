...
设 $C$ 为一条起点在 $a$，终点在 $b$ 的有向曲线， 其方程为
\begin{equation}
z(t)=x(t)+\mathrm{i} y(t) \qquad(\alpha \leqslant t \leqslant \beta, a=z(\alpha), b=z(\beta))
\end{equation}
...
\end{figure}
设$\Delta z_{k}=z_{k}-z_{k-1}$，$\zeta_k$ 为弧 $\overset{\hat}{z_{k-1}z_k}$ 上任意一点，作和$\displaystyle S_{n}=\sum_{k=1}^{n} f\left(\zeta_{k}\right) \Delta z_{k}$．
