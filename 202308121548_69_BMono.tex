...
\end{equation}
可以通过规范变换将一组磁矢势的解转换为另一组磁矢势的解。我们可以构造一个赤道上的标量函数 $\Lambda$ 使得赤道上的两种取值可以通过规范变换相联系： $\bvec A^N = \bvec A^S + \nabla\Lambda$ 。那么\autoref{eq_BMono_7} 转化为
\begin{equation}
...
\end{equation}
其中 $\bvec p=-i\hbar\bvec \nabla$，那么在规范变换 $\bvec A\rightarrow \bvec A+\nabla \Lambda$ 下，波函数以 $\psi(\bvec r)\rightarrow \psi(\bvec r)\exp(i\frac{q}{\hbar}\Lambda(\bvec r))$ 变换时，薛定谔方程仍然成立。