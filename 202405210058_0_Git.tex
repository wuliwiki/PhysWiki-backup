...
\end{lstlisting}
同时显示了不同版本的内容，需要手动把这部分解决，例如只保留其中之一。 注意这个显示的不同版本是当前分支第一个与 master 最新版的冲突，只要把这个解决，之后 commit 中对该文件的修改都会自动应用。 解决所有冲突后对这些文件用 \verb`git add` 标记冲突已经解决，再用 \verb`git rebase --continue` 即可。
