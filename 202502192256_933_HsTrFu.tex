...
然后是关于$y=x$对称的情况，当 $\alpha$ 与$\beta$都处于第一象限时，二者互余关系直接成立。因此，若 $P$ 对应的角 $\alpha$，则 $Q$ 对应的角 $\beta$ 满足$\alpha + \beta = \frac{\pi}{2} + 2k\pi, (k\in\mathbb{Z})$，
即：
\begin{equation}
\alpha = \frac{\pi}{2} - \beta + 2k\pi, \qquad (k\in\mathbb{Z})~.
\end{equation}

这种关系被称为互余关系，意味着 $\alpha$ 与 $\beta$ 是互为余角的。
\subsubsection{正弦与余弦}
