...
值得注意的是，在 $\sin x$ 的一个周期内，尽管几乎每个 $y$ 都对应着两个 $x$，但这两个 $x$ 的值互补，使得者说它们的导数符号相反，即在相同的 $y$ 值处，斜率方向不同。因此，每对 $(y, y’)$ 仅对应唯一的 $x$，从而在一个周期内确定 $\sin x$ 在坐标轴上的具体位置。这也正是将相位作为整体单独分析的重要性，它能够确保在给定的变换条件下，函数图像的位置变化具有明确的数学解释。相位 $\omega x + \varphi$ 直接对应于 $\sin x$ 函数图像中的位置。
