...
\end{equation}
最后一种记号在括号右下角声明所有保持不变的自变量，这在许多情况下能避免混淆。
...
\qty(\pdv{z}{x})_\theta~.
\end{equation}
按照\autoref{ParDer_eq1} 的定义，应该是仅用 $x$ 和 $\theta$ 表示 $z$，然后求偏导。 考虑极坐标系和直角坐标系的转换（\autoref{Polar_sub1}~\upref{Polar}）， 有 $y=x\tan\theta$，代入\autoref{ParDer_eq7} 得
\begin{equation}
