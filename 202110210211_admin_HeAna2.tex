...
\subsection{双电子系统}
双电子完全也可以分别使用\autoref{HeAna2_sub1} 和\autoref{HeAna2_sub2} 两种方法． 这是完全一样的． 一阶微扰的末态必须是能量本征态， 氦原子和 $H$ 对易的算符只有 $L^2, L_z$， 所以精确的球面波能量本征态记为 $\ket{H,L^2,L_z}$． 它可以分解为
\begin{equation}
\ket{H,L,M} = \frac{1}{r_1r_2}\sum_{l_1,l_2}\psi_{l_1,l_2}^{L, M}(r_1, r_2)\mathcal Y_{l_1,l_2}^{L, M}(\uvec r_1, \uvec r_2)
\end{equation}
\addTODO{是否存在精确的 $\ket{l_1,m_1, l_2, m_2, n_1, k_2}$ 本征态？ 是否可以把 $\ket{l_1,l_2,L,n_1,k_2}$ 做 CG 线性组合得到？}

理论上要解本征态 $\ket{l_1,l_2,L,n_1,k_2}$， 就用许多无限长的长方形网格， 每张网格是一个 $(l_1,l_2,L)$ 分波， 网格之间被 $1/r_{12}$ 势能耦合． 网格上 $r_1$ 在区间 $[0,a]$， $r_1 = a$ 处边界条件是波函数为零， 另外两条边 $r_1 = r_2 = 0$ 也有波函数为零． $r_2$ 的区间为 $[0,\infty)$， 这样才能保证 $E$ 可以连续取值而不是离散的． 所以我们无法控制 $r_2\to \infty$  的渐进相位， 可以记为 $\delta_l(E)$．
