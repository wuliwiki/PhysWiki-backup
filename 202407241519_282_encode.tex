...
如果你使用 \enref{Linux 命令行}{Linux}，那么 \verb`dos2unix` 命令也常用于检测换行符和 BOM 等信息，以及是文本文件还是二进制文件。 用法是 \verb`dos2unix -ih 文件1 文件2...`， 输出结果每个文件一行，格式为 \verb`DOS UNIX MAC BOM TXTBIN FILE`，分别是 \verb`CRLF的个数 LF的个数 CR的个数 是否有BOM 文本还是二进制 文件名`。 \verb`dos2unix 文件1 文件2...` 还可以自动转换
