...
由于$\bvec{r}$必定和$\bvec{\omega}$正交，因此从数值上来说，有$\abs{\bvec{\omega}}\abs{\bvec{r}}=\abs{\bvec{v}}$。这一点可以协助我们理解和推导旋转相关的性质。
...
给定参考点$P$和一个质点$X$，令$P$到$X$的位移向量为$\bvec{r}$，$X$的角速度为$\bvec{\omega}$，质量为$m$。则可以计算得$X$关于$P$的角动量为\footnote{设$\abs{\bvec{v}}=v$}
\begin{equation}
\bvec{L} = \abs{\bvec{r}}^2~. 
\end{equation}
