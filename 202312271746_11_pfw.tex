...
切空间是线性空间，因此可以通过过渡矩阵来进行坐标变换。拿不同图的切空间是否也存在一个过渡矩阵，使得基矢组可以变换到不同图sh？
...
\end{equation}
对于$p\in U\cap V$，我们有：$\widetilde\theta_i|_p=T^j_i \widetilde e_j|_p$，$T^j_i$是光滑函数构成的过渡矩阵，该假设是合理的，因为无论在什么图里，$p$点切向量作用在光滑函数上会得到相同的结果，即$Xf=a^i\widetilde e_if=b^i\widetilde \theta_i f$。两边作用在$\phi(p)$点上，利用$e_ix^k=\delta^k_i$，计算分量得：
\begin{equation}
\begin{aligned}
...
\end{aligned}~.
\end{equation}
于是，我们得到了不同图之间基矢的“联系”，类似多元函数微分学的结果，只不过雅克比矩阵的矩阵元为转移函数。
