...
\begin{corollary}{}
在\autoref{HlmPr2_the1} 中， 给 $\bvec G(\bvec r)$ 加上任意一个无旋场 $\bvec H(\bvec r)$ （满足 $\curl \bvec H = \bvec 0$）， 都能使\autoref{HlmPr2_eq4} 成立．
\end{corollary}
注意 $\bvec H$ 也可以表示为任意标量函数 $V(\bvec r)$ 的梯度 $\grad V$（引用未完成）．

我们可以认为\autoref{HlmPr2_eq1} 是旋度运算的逆运算， 这可以类比不定积分\upref{Int}是求导的逆运算． 

\begin{theorem}{}
令 $\bvec F(\bvec r)$ 为无散场， 则\autoref{HlmPr2_eq1} 得到的 $\bvec G(\bvec r)$ 仍然是一个无散场． 同理， 给 $\bvec G(\bvec r)$ 加上一个任意的调和场 $\bvec H(\bvec r)$ 后仍然是一个无散场且仍然满足\autoref{HlmPr2_eq4}．
\end{theorem}
...
\end{aligned}
\end{equation}
注意 $\bvec F$ 是 $\bvec r'$ 的函数而不是 $\bvec r$ 的函数， 所以第一项中 $\curl \bvec F = \bvec 0$． 另外由于 $\curl(\bvec R/R^3) = \bvec 0$ ， 上式恒为零．
