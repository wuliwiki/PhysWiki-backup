...
假设悬挂点左低右高， 相距为 $a'$， 高度差为 $h' > 0$， 绳长为 $L'$． 悬链线仍然具有形式\autoref{Catena_eq3}， 我们希望根据 $a', h', L'$ 求出 $k$． 若假设如果把左边的悬链线补全到和右边一样的高度后， 绳长为 $L$， 相距为 $a$， 那么有
...
\sinh(k\frac{a}{2}) - \sinh\qty(\frac{a}{2}-a') = L'
\end{equation}
...
\subsubsection{高度不相等的情况}
根据高度差的定义可以直接写出\autoref{Catena_eq4}， 而另一个条件\autoref{Catena_eq5} 是由绳长的约束得到：
\begin{equation}
L' = \int_{a/2-a'}^{a/2} \sqrt{1 + \dot y(x')^2} \dd{x'}
\end{equation}
把\autoref{Catena_eq3} 代入后化简可得．
