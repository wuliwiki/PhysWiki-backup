...
\end{equation}
使用\autoref{NatUni_eq1} ，\autoref{NatUni_eq2}  和\autoref{NatUni_eq3} 三个约束条件， 就可以求出 $\beta_x, \beta_m, \beta_t$（\autoref{NatUni_tab1} ）． 另外也顺便定义了 $\beta_F$．

\subsection{另一种思路}
注意普朗克单位制中， 所有转换常数都使用 $c, G, \hbar$ 来定义． 例如\textbf{普朗克长度（Plank length）}为
\begin{equation}
\beta_x = \sqrt{\frac{\hbar G}{c^3}}
\end{equation}
这是 $\hbar, G, c$ 使用幂的乘积拼凑出长度量纲的唯一组合： 令 $\beta_x = \hbar^a G^b c^c$ 解出 $a, b, c$ 即可解出 $a = b = 1/2, c = -3/2$．
