...
考虑整个$G$按伴随作用，作用在$\opn{Syl}_p(G)$上．由\autoref{Sylow_the1} ，这是一个可递作用，因此只有\textbf{一个}轨道，因此任意元素的轨道中元素数量是$n_p$．由\autoref{Group3_cor2}~\upref{Group3}，$n_p$等于$\opn{Syl}_p(G)$中任意元素的迷向子群\textbf{指数}．又由\textbf{拉格朗日定理}（\autoref{coset_the2}~\upref{coset}）可知子群指数整除群的阶，即$n_p\mid p^km$．
...
\autoref{Sylow_the2} 给出了$\opn{Syl}_p(G)$元素数量的
