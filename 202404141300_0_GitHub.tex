...
\item Git 本来是 Linux 操作系统的一个命令行工具，只能通过手动输入指令来操作。 虽然现在有了不少图形界面程序根据你的操作帮你输入这些命令，但它们很多只实现了常用的功能，高级功能往往仍然需要手敲命令。
\item 第三是这些图形界面软件往往不直接支持中文，想要中文还需要自己去找汉化版。
\item Git 的内容对比功能往往只支持 txt 等文本文件（毕竟程序员代码几乎都是文本文件），像 pdf, office 文档等二进制格式的文档虽然也可以 commit 和检测改变等，但是无法显示版本之间的具体区别。 虽然有第三方的插件可以把 pdf 等文档自动转换为文本文件供 Git 进行版本对比，但需要自己下载安装，效果也差强人意。
\end{itemize}
