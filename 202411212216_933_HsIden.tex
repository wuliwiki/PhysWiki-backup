...
之前已经聊了很久恒等式了，有两个与它相关的概念\textbf{公式（formula）}和\textbf{定义（式）（Definition）}很容易混淆。它们并不完全对立，事实上，公式和定义式都属于恒等式，定义式很好理解，因为如果不恒等就相当于定义改变了。而公式不仅在约定的范围内成立，而且一般应用的条件或范围也并不苛刻。

恒等式的本质是揭示数学中的普遍规律，是一种真理性陈述。而公式则更多地强调变量之间的联系以及它们在特定情境中的应用。例如，圆的面积公式不仅揭示了圆面积与半径之间的关系，还为具体的计算提供了工具。公式的作用更具工具性和操作性，适用于描述变量间的依赖关系，特别是在解决问题时被广泛使用。而定义通过一个明确的等号将数学对象与其性质或关系结合在一起，为公式的建立提供理论基础。

此外，公式与定义式的应用超越了纯数学的范畴，在物理、计算机科学和工程学等领域同样扮演着重要角色。例如，物理学中的牛顿第二定律公式$F=ma$描述了力与质量、加速度的关系，欧姆定律定律$V = IR$描述了电压、电流和电阻的关系和计算机算法复杂度公式 $T(n) = an^2 + bn + c$则用于衡量算法的效率。
...
最值是生活中很常见的一个概念，对于一组可以比较的量，人们总是希望能够快速地确定这组量的范围，而最值就是描述这个范围的量。寻找最值的过程可能各有不同，但最终的目标都是为了确定这组量的范围。对于不同的给定范围，最值有不同的定义或表示方法。
