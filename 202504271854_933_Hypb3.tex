...
注意上面$a,b$与虚实轴的对应关系。如果保持$x,y$对应的参数相等，只改变符号关系，即令两条双曲线的虚实轴调换，得到：
\begin{equation}\label{eq_Hypb3_14}
\frac{x^2}{a^2} - \frac{y^2}{b^2} = 1,\qquad \frac{y^2}{b^2} - \frac{x^2}{a^2} = 1    ~.
\end{equation}
就称二者为\textbf{共轭双曲线（conjugate hyperbola）}。这里的共轭与复数中介绍的共轭指的是同一个意思。其实可以看作\autoref{eq_Hypb3_14} 中左侧的方程把等号右侧的$1$bian zuo
