...
A^nB=\sum_{m=0}^{n}C_{n}^mA^{(m)}A^{n-m}=\sum_{m=0}^n\frac{n!}{(n-m)!m!}A^{(m)}A^{n-m}
\end{equation}
\end{lemma}
...
\begin{equation}
A^{k}B=A\sum_{m=0}^{k-1}C_{k-1}^mA^{(m)}A^{k-1-m}=\sum_{m=0}^{k-1}\frac{n!}{(n-m)!m!}A^{(m)}A^{n-m}
\end{equation}


现在，我们利用上面的两个引理来证明Baker-Hausdorff公式．
