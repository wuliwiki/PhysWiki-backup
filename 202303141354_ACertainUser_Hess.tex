...
这些都是方便实验测量（或查表得知）的量。因此，尽管我们不知道如何直接测量内能，但\textsl{阴差阳错之间}我们居然原则上能够计算内能了。
$$
U(T_1,V_1)=U(T_0,V_0) + \int_{T_0}^{T_1} C_V \dd T + \int_{V_0}^{V_1} P(T\beta-1) \dd V 
$$

那么你可能想问，那么我们如何确定$U(T_0,V_0)$？事实上我们\textsl{不需要确定}，因为在热力学中我们更关心的是状态量的变化，而非他的绝对数值。因此，将上式改写为变化形式更为实用：
$$
