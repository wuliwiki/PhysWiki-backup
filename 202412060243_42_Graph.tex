...
设 $V$ 是一个集合，$E\subset P_2(A)$，则称二元组 $G=(V,E)$ 为\textbf{图}（graph）。$V$ 的元素称为 $G$ 的\textbf{顶点}（vertex），$E$ 的元素称为 $G$ 的\textbf{边}（edge）。$V$ 的基数（元素个数）称为图 $G$ 的\textbf{阶数}（order），记作 $\abs{G}$，而 $E$ 的基数记作 $\norm{G}$。
\end{definition}
