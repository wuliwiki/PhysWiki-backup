...
考虑域$\mathbb{F}$，$\opn{ch}\mathbb{F}=p$．对于任意$\alpha, \beta\in\overline{\mathbb{F}}$和$k\in\mathbb{Z}^+$，都有$\alpha^{p^k}-\beta^{p^k}=(\alpha-\beta)^{p^k}$，其中$\overline{\mathbb{F}}$是$\mathbb{F}$的代数闭包．于是，如果设$a\in\mathbb{F}$在其代数闭包里有一个$p^k$次根$\alpha$，则$\alpha$是$a$唯一的$p^k$次根．这是因为
\begin{equation}
