...
可见由Fannes不等式可得，在$\forall 0<\epsilon\leqslant\frac{1}{2},~ T\left(\rho,\sigma\right)< \epsilon$时，$\abs{S\left(\rho\right)-S\left(\sigma\right)}<\epsilon\log_2\left(d-1\right) + H\left(\left(\epsilon,1-\epsilon\right)\right)$，所以可以说Fannes不等式给出了在迹距离的度量下，纠缠熵的连续性。