...
文件开头可以用注释声明关键词，格式为 \verb`keys 关键词1|关键词2|...`。

文件开头可以用注释声明所用协议， 格式为 \verb|license 协议id|（协议类型详见 “小时百科创作协议\upref{licens}”)。 \verb|协议 id| 有： \verb|Usr|（用户版权，无偿创作）, \verb|Xiao|（小时科技版权，有偿创作），\verb|CCBY3|，\verb|CCBYSA3|，\verb|CCBY4|，\verb|CCBYSA4|，\verb|Copy|（转载—只读）, \verb|Adap|（转载—可修改）, \verb|GPL3|, \verb|MIT|，\verb|Pub|（公有领域）。

文件开头可以用注释声明文章类型， 格式为 \verb|type 类型id|， 其中 \verb|类型id| 支持： \verb|Wiki|（类似于维基百科的条目，中立、全面、一般性的介绍）， \verb|Tutor|（类似于教材的一节）， \verb|Art|（学术论文或杂谈等）， \verb|Map|（介绍百科中的一章、一部分等，具有大量链接到其中的词条）， \verb|Note|（类似于讲义、复习资料，把个知识的要点列出）。
