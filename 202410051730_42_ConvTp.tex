...
\end{definition}
 ...
 \begin{theorem}{}
 取 $[0,1]$ 为基本集合构造如下拓扑 $\mathcal T$：$O$ 是 $\mathcal T$ 的开集，当且将当 $O$ 是从 $[0,1]$ 中去掉任意有限或可数个点得到的。那么 $\mathcal T$ 的序列 $\{x_n\}$ 是收敛的，当且将当从某一下标开时，所有元素相同，即当且将当存在 $N>0$，使得$x_n=x_{n+1}=\ldots,n\geq N$。
 \end{theorem}
 \textbf{证明：}
