...
\end{equation}
其中 $E$ 可以连续取值， 每个 $E$ 都对应不同的解． 当 $E< 0$ 时， 有可能存在束缚态也可能没有， 我们在散射问题中不考虑． 这是一个二阶常微分方程， 每个 $E$ 有两个线性无关的解， 记为 $\psi_{E,1}(x)$ 和 $\psi_{E,2}(x)$， 于是通解可以表示为
\begin{equation}
...
\end{equation}
注意由于我们规定 $V(\pm\infty)\to 0$， 在无穷远处 $\psi_{E,i}(x)$ 显然都是平面波， 波数都是 $\abs{k} = \sqrt{2mE}$．


\subsection{动量归一化}
于是我们同样可以像\autoref{FreeP1_eq3}~\upref{FreeP1} 那样用动量归一化的基底展开波函数， 只不过正交归一基底从平面波变成了本征函数．
\begin{equation}
