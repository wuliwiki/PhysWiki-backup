...
由质点匀速圆周运动容易知道，在同样的时间 $\dd t$ 内质点走过的路程(弧长) $\dd s$ 是不变量，而 $\abs{\dd r}=\dd s$．这表明，若将矢量 $\dd r(t)$ 头尾相连，则$\dd r(0)$到 $\dd r(t)$ 将画出一个圆弧，在一个周期内将画出一个完整的圆（请思考？）．由矢量加法的几何图像可知，那么积分
\begin{equation}
\int_{0}^{t}\uvec r(t) \dd{t}
\end{equation}
的结果将是从 $\dd r(0)$ 起点指向 $\dd r(t)$ 的终点．那么这个矢量微元 $\dd r$ 首尾相连构成的圆的半径是多少呢？在四分之一周期这个圆画了1/4，所以由\autoref{PLaw1_eq5} 可知，该圆半径便是 $\frac{1}{\omega}$.
\end{example}
