...
\end{equation}
其中 $\bvec p$ 为质点动量， $\bvec L$ 为轨道角动量， $\cross$ 表示\enref{矢量叉乘}{Cross}， $k$ 是一个常数（对\enref{万有引力}{Gravty} $k = GMm$， 对\enref库仑力\upref{ClbFrc} $k = -Qq/(4\pi\epsilon_0)$。 $\uvec r$ 为质点位矢 $\bvec r$ 的单位矢量。 在开普勒问题中， 可以证明 $\bvec A$ 是一个守恒量。
