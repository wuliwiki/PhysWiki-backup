...
\item \verb`stoi, stol, stoll` （C++）（\verb`sto` 代表 "string to"）（例如 \verb`int stoi(const string &str, size_t *pos = nullptr, int base = 10);`） \verb`pos` 用于输出处理了多少个字符。 但是缺点是如果处理子串， 需要生成一个新的字符串， 进行不必要的复制和内存分配。 如果出错会 \verb`throw`。
