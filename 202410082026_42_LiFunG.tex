...
若 $M'$ 是与子空间 $L'$ 平行、且不通过原点的任一超曲面，则存在唯一的线性泛函，使得 $M'=\{x|f(x)=c\neq0\}$：设 $M'=L'+x_0,x_0\in L$。则任一元素 $x\in L$ 可唯一表为 $x=\alpha x_0+y,y\in L'$ 的形式（因为超平面余维数为1）。令 $f(x)=\alpha$，于是 $f$ 是所需要的线性泛函。事实上，若 $x=\alpha x_0+y,x'=\alpha' x_0+y'$，则
\begin{equation}
f(\lambda x+\lambda' x')=f((\lambda\alpha+\lambda'\alpha')x_0+\lambda y+\lambda'y')=\lambda \alpha+\lambda'\alpha'=f()~
\end{equation}
