...
\end{itemize}
需要注意的是，尽管在很多推导之前都假设$\alpha$是第一象限角，但上面推导之后可知，所有任意角$\alpha$都满足前面的关系。由于后面会介绍的口诀中，为方便记忆会假设所有的角都是第一象限角，然后利用这个关系来推知其他象限角的情况。很多时候教师在教学时会语焉不详，只求让学生知道当他是就可以，使得很多学生对这个背后的原理不理解。上面对此进行了详细的论证。而利用对称性解决数学问题，是非常重要的一种视角。由此产生了\enref{群论}{Group}。
