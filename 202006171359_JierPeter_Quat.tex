...
四元数$A=(a_0, \vec{a})$的实数部分$a_0$被称为\textbf{标部}，矢量部分$\vec{a}$称为\textbf{矢部}．为了方便，有时候也把这个四元数记为$a_0+\vec{a}$，类比复数的表示法$(a, b)=a+b\I$．
...
令四元数为$A=a_0+\vec{a}$，则根据四元数的乘法定义（和向量的叉乘、点乘不一样），定义$$\mathrm{e}^A=\mathrm{e}^{a_0}\mathrm{e}^{\vec{a}}=\mathrm{e}^{a_0}\sum^\infty\limits_{n=0}\frac{\vec{a}^n}{n}=\mathrm{e}^{a_0}\sum^\infty\limits_{n=0}\frac{(a\vec{e})^n}{n}$$
