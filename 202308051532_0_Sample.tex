...
文件开头可以用注释声明关键词，以及所用协议（协议类型详见 “小时百科创作协议\upref{licens}”)。协议 id 有： \verb|Usr|（用户版权，无偿创作）, \verb|Xiao|（小时科技版权，有偿创作），\verb|CCBY3|，\verb|CCBYSA3|，\verb|CCBY4|，\verb|CCBYSA4|，\verb|Copy|（转载—只读）, \verb|Adap|（转载—可修改）, \verb|GPL3|, \verb|MIT|，\verb|Pub|（公有领域）。
