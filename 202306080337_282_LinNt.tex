...
\item 如果你对一个文件夹没有 \verb|rwx|， 那么文件夹和其子文件夹内的所有内容你都无法看到也无法进行任何操作， 无论内部的文件和子文件夹有多开放。 如果只有 \verb|rw| 没有 \verb|x|， 也只是好了那么一点点， 你可以对文件夹重命名， 只能看到文件夹内第一层的文件和子文件夹的名字，其他同样什么也做不了（包括）。
