...
\subsection{极限不存在}
如果说一个函数 $f(x)$ 的某种极限不存在，就说该极限\textbf{不收敛}。 但不收敛也有不同的细分。
...
\begin{example}{}
\autoref{eq_FunLim_1} 并不是极限不存在的唯一一种情况，例如 $\lim\limits_{x\to\infty}\sin x$ 和 $\lim\limits_{x\to 0}\sin 1/x$ 的极限同样不存在，但不满足\autoref{eq_FunLim_1}。
\end{example}
