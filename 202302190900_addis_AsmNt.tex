...
\item 注释用 \verb|#|
\item 缩进没有意义
\item \verb|movl $4, %eax| 把 register \verb|eax| 的值设为 4。 \verb|%| 表示 register， \verb|$| 表示 immediate value， 可以用十进制 \verb|4| 也可以 16 进制 \verb|0x4|。
\item （过时用法）\verb|int 0x80| 开始 software interrupt， 进行 \verb|syscall|。
