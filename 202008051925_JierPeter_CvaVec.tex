...
设线性空间$V$中有一组标准正交基$\{\bvec{e}_i\}^n_{i=1}$，令$\{\bvec{r}_i\}^n_{i=1}$和$\{\bvec{r}^i\}^n_{i=1}$分别是一组协变基和逆变基．设$\{\bvec{r}_i\}^n_{i=1}$在标准正交基中的坐标矩阵\footnote{将$\bvec{r}_i$写成列向量，作为坐标矩阵的第$i$列．}是$\bvec{M}$，而$\{\bvec{r}^i\}^n_{i=1}$的坐标矩阵是$\bvec{MT}$，其中$\bvec{T}$是从协变基到逆变基的过渡矩阵，那么由
