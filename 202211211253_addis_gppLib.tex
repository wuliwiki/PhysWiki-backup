...
\begin{itemize}
\item 实际上 \verb|-Wl,-rpath| 设置的是 \verb|runpath| 而不是 \verb|rpath|． 二者唯一区别在于动态库搜索路径顺序． 一般并不推荐使用 \verb|rpath|．
...
\item \verb|readelf -s 链接库.so| 可以检查里面的 symbols． 函数的类型是 \verb|FUNC|， 输出有两个表， 看 \verb|.dynsym| 就可以， 它是 \verb|.symtab| 中 allocable 的一个子集（详见\href{https://blogs.oracle.com/solaris/post/inside-elf-symbol-tables}{这里}）． 如果是 c++ 函数， 那么函数名是 mangle 的， 用 \verb|--demangle| 来显示 c++ 中的函数名．
