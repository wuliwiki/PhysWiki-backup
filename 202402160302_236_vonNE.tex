...
$$\ket{\psi} = \sum\limits_i^N\ket{a_i}\ket{u_i}~.$$
上式中$\left\{\ket{a_i}\right\}$是$A$区域上的一组正交归一基矢量，而$\left\{u_i\right\}$则是一组定义在$B$区域上的，不一定正交也不一定归一的矢量。而我们总可以通过$\left\{u_i\right\}$来张成一个$N$维线性空间（如果矢量不足$N$个，则需引入额外的基矢量来补足）。
