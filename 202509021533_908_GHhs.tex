...
具有指定闭支集的光滑函数常用于构造**光滑的单位分解**（参见 *partition of unity* 和 *topology glossary*）；这种构造在研究光滑流形时至关重要，例如用来证明黎曼度量可以从局部定义推广到整体。一个简单的情形是实数轴上的凸起函数，即一个光滑函数 $f$，在区间 $[a,b]$ 之外取值为 0，并且在区间内部满足：
$$
f(x) > 0, \quad a < x < b~
$$
