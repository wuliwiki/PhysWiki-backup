...
\item $\omega$ 称为\textbf{角频率（angular frequency，也译作圆频率）}；
\item $\omega x + \varphi$ 称为\textbf{相位（phase）}；
\item $\varphi$ 称为\textbf{初相（initial phase）}。
\end{itemize}
\end{definition}

如 \autoref{eq_HsTFFv_4} 所示，$\cos x$ 可以视为正弦型函数的一种特殊形式，其中 $\displaystyle A = \omega = 1, \varphi = \frac{\pi}{2}$。此外，通过适当的变形，可以将参数调整至更规范的范围。利用 $\sin(-x) = -\sin x$ 和 $-\sin x = \sin(x + \pi)$，可以确保 $A$ 和 $\omega$ 取正值，而所有的符号变化都体现在初相 $\varphi$ 上。这样，有$A\in\left(0,+\infty\right),$ $\omega\in\left(0,+\infty\right),$ $\varphi\in\left(-\pi,\pi\right]$，下面的讨论为避免麻烦，都默认已经将正弦型函数化为c。

$A\sin(\omega x)$ 与 $-A\sin(\omega x)$ 的相位相差 $\pi$，这一关系称为\textbf{反相（phase opposition）}。

如 \autoref{eq_HsTFFv_4} 提到的，$\cos x$ 可以视为一种正弦型函数，其中 $\displaystyle A = \omega = 1, \varphi = \frac{\pi}{2}$。另外，通过适当的变形，可以将参数限制在一个更小的范围内。利用 $\sin(-x) = -\sin x$ 以及 $-\sin x = \sin(x + \pi)$，可以确保 $A,\omega$ 始终为正数，并将所有的符号变化归结到初相 $\varphi$ 的不同上。显然，在这种情况下，$A\sin(\omega x)$ 和 $-A\sin(\omega x)$ 的相位相差 $\pi$，这种关系称为\textbf{反相（phase opposition）}。
...
\item 称 $\varphi$ 为初相的进一步原因在于，称 $\omega x+\varphi$ 为在 $x$ 处的相位，初相就是在 $0$ 处的相位。
\end{itemize}
，表示函数的最大偏离值；
，决定了函数的周期性变化速度；
，决定了函数在 $x=0$ 处的相位位置；
