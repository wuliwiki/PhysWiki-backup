...
\int_{\mathbb{R}^+}{e^{-zt}\phi(t)}\dd t∼\sum_{n=0}^{\infty}{c_n\frac{\Gamma\left(\lambda_n\right)}{z^{\lambda_n}}}  
\end{equation}
\end{lemma}
...
\]
若函数$\phi$满足定理中提到的三个条件，则$F(z)$变换对于$\operatorname{Re} z>c$是存在的．即：当$\operatorname{Re} z>c$时，积分收敛．注意\eqref{Lemma_eq1}意味着存在正整数N，当n>N时，对一切的0<t<\delta\left(0<\delta<\sfrac{\pi}{2}\right)，总有
\left|f\left(t\right)-\sum_{n=0}^{N-1}{a_nt^{\lambda_n-1}}\right|\le Mt^{\lambda_N-1}
其中M>0且为常数.同时\left(6\right)表明当时，存在正整数N，当n>N时，对一切的0<t<\delta\left(0<\delta<\sfrac{\pi}{2}\right)，总有
\left|f\left(t\right)-\sum_{n=0}^{N-1}{a_nt^{\lambda_n-1}}\right|\le Ke^{ct}t^{\lambda_N-1}
其中K>0且为常数．于是就有
\left|\int_{0}^{+\infty}{e^{-zt}f\left(t\right)}dt-\sum_{n=0}^{N-1}{a_n\int_{0}^{+\infty}{e^{-zt}t^{\lambda_n-1}d}t}\right|\le K0∞e-Rez-cttλN-1dt
注意到当Rez>0时
\int_{0}^{\infty}{e^{-zt}t^{\lambda_n-1}}dt=\frac{1}{z^{\lambda_n}}\int_{0}^{\infty}{e^{-u}u^{\lambda_n-1}}du=\frac{\Gamma\left(\lambda_n\right)}{z^{\lambda_n}}
因此
\left|F\left(z\right)-\sum_{n=0}^{N-1}{c_n\frac{\Gamma\left(\lambda_n\right)}{z^{\lambda_n}}}\right|\le KΓλNRez-cλN=KΓλNzλNzRez-cλN
由于z\in\mathfrak{G}，于是Rez≥zsinδ．这表明当\left|z\right|足够大时，Rez-c≥12zsinδ．因此
...
\end{equation}
其中 $c_n$ 由\autoref{LapAsm_eq3} 给出． 特别地， 由于 $\Gamma(1/2)=\sqrt{\pi}$， \autoref{LapAsm_eq1} 渐近公式的首项是
\[
