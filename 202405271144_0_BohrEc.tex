...
。}本文使用\enref{原子单位制}{AU}。 \textbf{Bohr-Sommerfeld 模型}（以下简称 Sommerfeld 模型）和玻尔原子模型一样属于量子力学发展早期的半经典原子模型， 它对玻尔模型进行了改进， 能更好地符合一些实验结果（如\enref{塞曼效应}{ZemEff}）。 玻尔模型假设电子以圆形轨道绕原子核旋转， 而 Sommerfeld 模型使用椭圆轨道。 为了简单我们同样先假设原子核固定不动， 要考虑原子核运动使用相对坐标和约化质量\upref{TwoBD}即可。
