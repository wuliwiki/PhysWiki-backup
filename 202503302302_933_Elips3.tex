...
下面尝试从圆的定义出发，尝试推广它的构造思路。圆通常被定义为：到某个定点（圆心）的距离为定值的所有点组成的轨迹。想要推广它的定义，就要在定义上动些手脚。由于圆的定义很简单，构成的元素包括：定点、距离、定值。参考割线到切线的变化过程，可以想象，想象圆心其实是两个重合的点，圆上的每个点到这两个点的距离都相等，且等于固定值。现在，若将这两个重合的点稍微分开，并考虑平面上所有“到这两个点距离相等”的点，那么得到的将是一条连结这两点的线段的垂直平分线。
