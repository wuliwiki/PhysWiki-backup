...
磁通量只与曲面的边界有关。设有两个曲面 $S_1$ 和 $S_2$ 的边界都是 $C$，而曲面 $S_1,S_2$ 围成的一个闭曲面（没有边界）的磁通量为
\begin{equation}
\int_{S_2} \bvec B(\bvec r)\cdot \dd \bvec s - \int_{S_1} \bvec B(\bvec r)\cdot \dd \bvec s~.
\end{equation}


 这是因为 $\bvec B$ 的散度恒为零（磁场的高斯定律）
\addTODO{证明}
