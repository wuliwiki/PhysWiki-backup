...
\end{equation}
\begin{theorem}{}
存在2对1的覆叠同态$\phi:SU(2)\times SU(2)\to SO(4)$，核元素为$\mathbb Z_2$。
\end{theorem}

由于$SU(2)\cong Q$，且\autoref{eq_Quat_5} 满足$\phi:Q\times Q\to SO(4)$，我们只需证明核元素只有2个即可。

设$(p,q)\in \opn{ker}Q\times Q$，则对于任意$u\in \mathbb H$都有$puq^{-1}=u$，对应单位矩阵。代入特例$u=1$得：$p=q$，$pup^{-1}$代表的是固定实轴，对$\opn{Im$