...
基的变换由\textbf{转移矩阵}\upref{TransM}$\omega^i_j(t)$表征：
\begin{equation}\label{HsbPic_eq13}
\ket{a_j; t} = \sum_i\ket{a_i; 0}\omega^i_j(t)
\end{equation}
其中$\sum_i \omega^i_j(t)$

联立\autoref{HsbPic_eq12} 和\autoref{HsbPic_eq13} ，或者根据\textbf{转移矩阵}\upref{TransM}中坐标的变换，得
\begin{equation}\label{HsbPic_eq14}
\sum_j c^j(t)\omega^i_j(t) = c^i(0)
\end{equation}

回到薛定谔绘景，此时保持基向量$\ket{a_i; 0}=\ket{a_i}$不变，而态矢量$\ket{s}$按时间演化算符改变．两个绘景下，态矢量的展开系数相同，于是有：
\begin{equation}\label{HsbPic_eq15}
\mathcal{U(t)}\ket{s} = \sum_i c^i(t) \ket{a_i}\implies \mathcal{U}(t)c^i(0) = c^i(t)
\end{equation}

比较\autoref{HsbPic_eq14} 和\autoref{HsbPic_eq15} 发现，转移矩阵$\omega^i_j(t)$表示的算子，和$\mathcal{U}(t)$互为逆算子．
