...
\begin{equation}
\psi_{l'_1,l'_2}^{L, M}(r_1, r_2) \overset{r_2\to\infty}{\longrightarrow}
\delta_{l_1,l'_1}\delta_{l_2,l'_2}r_1 R_{n_1,l_1}^{(Z=2)}(r_1) F_{l_2}(r_2 + \delta_{n_1,l_1,l_2}^{L,M})
\end{equation}
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
且所有其他的 $(l_1,l_2)$ 分波在 $r_2\to\infty$ 波函数都消失。 这样就可以把这个态记为 $\ket{L,M; l_1,l_2, n_1, k_2}$。 在 Pazourek 的论文中提到一个 R-matrix 方法可以解出这样的波函数\footnote{Philip G. Burke, R-Matrix Theory of Atomic Collisions - Application to Atomic, Molecular and Optical Processes, Springer}。 分号用于提醒后面的量子数只是无穷远处的边界条件， 这个态仍然只有 $E,L,M$ 是 well defined。 如果把这些态用 CG 系数线性组合一下， 就可以得到
\begin{equation}
\begin{aligned}
&\ket{n_1,l_1,m_1,l_2,m_2,k_2}
= \I \sum_{L} \bmat{l_1& l_2& L\\ m_1& m_2& M} \E^{-\I \delta_{n_1,l_1,l_2}^{L,M}} \ket{n_1,L,M,l_1,l_2,k_2}~,
\end{aligned}
\end{equation}
渐进形式为：
\begin{equation}
