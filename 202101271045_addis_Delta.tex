...
\begin{issues}
\issueTODO
\issueOther{这个定义并不适用于\autoref{Delta_eq10}， 所以\autoref{Delta_ex1} 也适用． 更广义的定义是直接使用\autoref{Delta_eq11}， 参考 \cite{Arfken}． 但是如何证明\autoref{Delta_eq10} 呢？}
\end{issues}
...
\end{equation}
注意\autoref{Delta_eq10} 不满足\autoref{Delta_def1} 却满足\autoref{Delta_eq11}． 所以根据某种更宽泛的定义也是 delta 函数．
\end{exercise}
