...
\begin{example}{}
显然， $z_n(x) = 0$ 是一个零函数（列）． 简谐函数 $\sin(nx + \phi)$， $\exp(\I n x)$ 都是零函数． 更一般地， 若 $f(x)$ 是一个性质良好的函数， $f(x)\sin(nx + \phi)$ 和 $f(x)\exp(\I n x)$ 也都是零函数， 例如高斯波包\upref{GausPk} $\E^{-x^2}\E^{\I kx}$．

笔者不会证明\footnote{即使会也会因太复杂而省略声明．}， 但从函数图像上这是符合直觉的： 若一个函数乘以一个另快速震荡的函数， 且震荡频率越来越快， 那么\autoref{F0_eq2} 的积分越接近 $0$．
\end{example}

\begin{theorem}{零函数的性质}
容易证明， 若 $z(x)$ 是零函数， 那么对它进行平移， 拉伸， 乘以常数等变换后仍然是零函数．
\end{theorem}

为什么定义零函数呢？ 我们可以以此判断一组连续的函数基底（例如量子力学的一维散射态\upref{ScaNrm}）是否正交． 在 $\delta$ 函数中， 我们知道若两个含参数的函数满足（$\delta_n$ 是 $\delta$ 函数列）
...
\end{equation}
那么两组函数基底间满足什么正交归一条件才可以使以下系数公式仍然成立呢？
\begin{equation}\label{F0_eq4}
C_i(k) = \int_{-\infty}^{\infty} f_i^*(k, x)g(x)\dd{x} \qquad (i = 1,2)
\end{equation}
可以证明除了要求 $f_1, f_2$ 各自满足\autoref{F0_eq1}， 还需要保证 $f_1, f_2$ 之间满足以下正交条件为
\begin{equation}
