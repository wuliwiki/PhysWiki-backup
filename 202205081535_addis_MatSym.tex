...
\item \verb|vpa(数值)| 和 \verb|sym(数值)| 一样会自动猜测 \verb|数值| 所代表的根式或符号， 结果相当于 \verb|vpa(sym(数值))|． 例如 \verb|vpa(0.866025403784439)| 的结果是表达式 $\sqrt{3}/2$， 又例如 \verb|sym(pi)| 的结果是精确的圆周率符号 $\pi$． 例如 \verb|vpa(0.866025403784439) - vpa(str2sym('sqrt(3)/2'))| 结果为零， 又例如 \verb|vpa(pi, 1000)| 都可以得到圆周率的 1000 位有效数字．

\item  若不希望 \verb|vpa| 猜测， 则应该使用引号， 例如 \verb|vpa('0.866025403784439') - vpa(str2sym('sqrt(3)/2'))| 不为零． 使用引号可以在当前 \verb|digits| 设置的精度内最精确地表示引号内的数字．
