...
G = \pm \sqrt{ab}~.
\end{equation}
这表明，对于给定的 $a$ 和 $b$，它们的等比中项并不唯一，分别对应 $q$ 为相反数的两种情况。其中，与 $a$ 和 $b$ 同号的值正好是 $a$ 和 $b$ 的几何平均值\footnote{关于几何平均值参见\aref{基本不等式}{sub_HsIden_2}。几何平均值的英语也与等比数列的英语使用了相同的单词“geometric”。}。

从几何的角度看，可以将这些离散的点通过两种方法连接起来，但相对简单的方法是选择单调的函数，因为它更简单和自然。
