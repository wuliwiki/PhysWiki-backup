...
其中函数 $H$ 有界． 当 $s$ 很大时， 邻域 $B(x_0,\delta)$ 以外的积分的贡献比 $s$ 的任何负幂衰减得都快：根据假定 2，当 $x\in I\setminus B(x_0,\delta)$ 时, 存在正数 $\epsilon_0>0$ 使得 $f(x)<f(x_0)-\epsilon_0$， 从而邻域 $B(x_0,\delta)$ 之外的积分估计为
$$
\begin{aligned}
