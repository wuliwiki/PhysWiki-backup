...
据\autoref{QMPrcp_def5} 和\autoref{QMPrcp_eq3} 知，若$X$是厄米的，则对于任意量子态$\ket{s}$，都有$\bra{s}X$与$X\ket{s}$互为对偶表示，则$X\ket{s}=a\ket{s}$，$\bra{s}X=a^*\bra{s}$．

取$\ket{s}$为$X$的任意\textbf{本征}矢量，其本征值为$a$．
