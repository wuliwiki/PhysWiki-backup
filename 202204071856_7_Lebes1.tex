...
2. 为方便，记$A$为对$E$进行任意分划后求上和的结果的集合；$A'$为先将$E$作分划$\{E_1, E_2\}$后，再分别对这两个$E_i$作分划后求上和，将两个上和相加后，所得值的集合．这样，按定义，\autoref{Lebes1_eq1} 的左边就是$A$的下确界，右边就是$A'$的下确界．

\autoref{Lebes1_eq1} 左边是对$E$作分划，右边则是先将$E$作分划$\{E_1, E_2\}$后，再分别对这两个$E_i$作分划，因此可知$A'\subseteq A$，故必有\footnote{子集的下确界大于等于母集的下确界．}
...
综合\autoref{Lebes1_eq2} 和\autoref{Lebes1_eq3} 即得\autoref{Lebes1_eq1} ．

由上下和与上下积分定义的对偶性，可直接推得\autoref{Lebes1_eq4} ．由此得证．
