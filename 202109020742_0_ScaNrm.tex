...
若我们得到两个不正交的线性无关解 $\psi_{k,1}, \psi'_{k,2}$， 这时我们可以使用施密特正交化\upref{SmdtOt}， 先把 $\psi_{k,1}$ 按照\autoref{ScaNrm_eq4} 归一化， 那么
\begin{equation}
...
\end{equation}
要得到行波的散射态， 需进行幺正变换， 令酉矩阵\upref{UniMat}为 $\pmat{C_{1} & C_{2}\\ C_{2}^* & -C_{1}^*}$， 满足 $\abs{C_1}^2 + \abs{C_2}^2 = 1$， 有
\begin{equation}
