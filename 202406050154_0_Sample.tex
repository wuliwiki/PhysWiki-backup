...
\subsubsection{预备知识}
预备知识的命令是 \verb`\pentry{}{}`，可以通过菜单上的 “插入预备知识” 按钮自动插入。 \verb`\pentry{}{}` 的第二个参数是该节点的 label， 格式为 \verb`nod_节点id`。 注意任何文章都有一个不出现在代码中的默认节点， label 为 \verb`nod_文章id`。 同一篇文章的每个节点默认依赖上一个节点， 默认节点依赖最后一个 \verb`\pentry{}{}` 定义的节点。 \verb`\pentry{}{}` 中第一个参数中可以用 \verb`\nref{}` 引用其他文章的节点， 可以通过 “引用其他文章内容” 按钮中的 “点” 插入，若不填节点编号则引用整篇文章。
