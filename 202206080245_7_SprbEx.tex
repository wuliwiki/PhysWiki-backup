% 可分扩张
% keys 可分多项式|完全域|完备域|形式微商|微商|导数|形式导数|separable polynomial|重根|自同构|形式微分|Frobenius同态
...
另外，由于本词条篇幅限制，关于可分扩张的更多讨论会在\textbf{本原元素与单代数扩张}\upref{PrmtEl}词条中继续．
...
% 下面这个定理描述的是可分扩张的中间域，在\textbf{本原元素与单代数扩张}\upref{PrmtEl}中很有用．

% \begin{theorem}{“有限中间域”}
% 如果域扩张$\mathbb{K}/\mathbb{F}$是\textbf{有限}的可分扩张，那么其中间域只有有限多个．
% \end{theorem}
...
% % 由\autoref{SpltFd_the2}~\upref{SpltFd}，$\mathbb{K}/\mathbb{F}$是正规扩张．任取$a\in\mathbb{K}-\mathbb{F}$，设$a$在$\mathbb{F}$上的最小多项式为$g(x)$，则$g(x)$的根都在$\mathbb{K}$中．设$\mathbb{K}_0$是$g\in\mathbb{F}[x]$的分裂域，则$\mathbb{K}_0\subseteq \mathbb{K}$，从而$\opn{deg}g\leq\opn{deg}f$．

% % 由于$f$是无重根的不可约多项式，故$\mathbb{K}$的保$\mathbb{F}$自同构一共有$\opn{deg}f$个．

% 任取$a\in\mathbb{K}-\mathbb{F}$，则存在
