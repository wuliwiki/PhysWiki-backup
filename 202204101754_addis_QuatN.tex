...
四元数的乘法运算（\autoref{Quat_eq1}~\upref{Quat}）可以表示两个旋转矩阵相乘， 即把两次旋转合并为一次旋转． 注意 $q_1q_2$ 表示先做 $q_2$ 的旋转， 再做 $q_1$ 的旋转． 另外易证 $(q_1q_2)^{-1} = q_2^{-1} q_1^{-1} = \tilde q_2\tilde q_1$．

证明：
\begin{equation}
(q_1q_2)\bvec v(q_1q_2)^{-1} = 
\end{equation}
