...
当系数比值相等时，能够定义$\mathcal{Q}(C_\alpha\ket{s_\alpha})=C_\alpha\ket{s'_\alpha}$．由$\ket{s'_\alpha}$的定义\footnote{在证明的一开始定义的，为免于读者翻找，这里复述一遍：$\ket{s'_\alpha}$是射线$f(\ket{\bar{s}_\alpha})$中任意一个\textbf{归一化}右矢．注意，已知$f$是一个对称变换．}，可知
\begin{equation}
\sum_\alpha C_\alpha
\end{equation}
