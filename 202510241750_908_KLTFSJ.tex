...
若两 Kuratowski 闭包算子 $\mathbf{c}_{1}, \mathbf{c}_{2} : \wp(X) \to \wp(X)$ 满足对任意 $A \in \wp(X)$ 都有 $\mathbf{c}_{2}(A) \subseteq \mathbf{c}_{1}(A)$，则它们分别诱导的拓扑 $\tau_{1}, \tau_{2}$ 满足 $\tau_{1} \subseteq \tau_{2}$，反之亦然。换句话说，$\mathbf{c}_{1}$ 支配（dominates）$\mathbf{c}_{2}$ 当且仅当后者所诱导的拓扑是前者所诱导拓扑的一个细化（refinement），或等价地，有 ${\mathfrak{S}}[\mathbf{c}_{1}] \subseteq {\mathfrak{S}}[\mathbf{c}_{2}]$ \cite{13}。

例如，$\mathbf{c}_{\top}$ 显然支配 $\mathbf{c}_{\bot}$（后者只是 $\wp(X)$ 上的恒等映射）。  
由于相同结论也可以通过将 $\tau_{i}$ 替换为包含其所有补集的族 $\kappa_{i}$ 得出，若我们在 $\mathrm{Cls}_{\text{K}}(X)$ 上定义偏序关系
$\mathbf{c} \leq \mathbf{c}' \iff \mathbf{c}(A) \subseteq \mathbf{c}'(A)$ 对所有 $A \in \wp(X)$ 成立，
而在 $\mathrm{Atp}(X)$ 上定义细化序，则可得出映射 ${\mathfrak{S}}$ 是偏序集之间的反单调映射（antitonic mapping）。

在任意诱导拓扑中（相对于子集 $A$），闭集诱导出新的闭包算子，该算子只是原闭包算子在 $A$ 上的限制：
$\mathbf{c}_{A}(B) = A \cap \mathbf{c}_{X}(B)$，对所有 $B \subseteq A$ 成立 \cite{14}。
\subsubsection{连续映射、闭映射与同胚}
一个函数 $f : (X, \mathbf{c}) \to (Y, \mathbf{c}')$ 在点 $p$ 处连续，当且仅当
$p \in \mathbf{c}(A) \Rightarrow f(p) \in \mathbf{c}'(f(A))$；
它在处处连续，当且仅当
$f(\mathbf{c}(A)) \subseteq \mathbf{c}'(f(A))$ 对所有 $A \in \wp(X)$ 成立 \cite{15}。

映射 $f$ 是闭映射（closed map）当且仅当上述包含反向成立 \cite{16}；
若它既连续又为闭映射，即满足等号成立时，$f$ 是同胚（homeomorphism）\cite{17}。
\subsubsection{分离公理}
设 $(X, \mathbf{c})$ 为一个 Kuratowski 闭包空间。则：

- $X$ 为 $T_{0}$ 空间，当且仅当 $x \neq y$ 蕴含 $\mathbf{c}(\{x\}) \neq \mathbf{c}(\{y\})$ \cite{18}；
- $X$ 为 $T_{1}$ 空间，当且仅当对所有 $x \in X$，有 $\mathbf{c}(\{x\}) = \{x\}$ \cite{19}；
- $X$ 为 $T_{2}$ 空间（Hausdorff 空间），当且仅当 $x \neq y$ 时，存在 $A \in \wp(X)$ 使得
$x \notin \mathbf{c}(A)$ 且 $y \notin \mathbf{c}(\mathbf{n}(A))$，
其中 $\mathbf{n}$ 为补集算子 \cite{20}。
\subsubsection{接近性与分离}
若 $p \in \mathbf{c}(A)$，则称点 $p$ 与子集 $A$ 是“接近的”（close）\cite{21}。
该概念可用于在集合的点与子集之间定义一种邻近关系（proximity relation）。

对于 $A, B \in \wp(X)$，当且仅当
$(A \cap \mathbf{c}(B)) \cup (B \cap \mathbf{c}(A)) = \varnothing$
时，称它们是“分离的”（separated）。  
空间 $X$ 是连通的（connected），当且仅当它不能表示为两个分离子集的并 \(^\text22}。
