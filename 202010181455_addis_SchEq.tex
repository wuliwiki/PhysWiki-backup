...
\end{equation}
这是一个二阶线性常微分方程． 数学上来看无论 $E$ 是多少， 必有两个线性无关的解， 它们的线性组合也是解（二维解空间）． 然而物理上可能存在的波函数必须要满足归一化条件， 所以只有对于一些离散的 $E$ 我们才能解出这些波函数． 我们把每个 $E$ 叫做\textbf{能级（energy level）}， 对应的波函数叫做\textbf{束缚态（bound state）}．

可以证明只有当 $E < \min V(\pm\infty)$ 且 $E > \min V(x)$ 时， 才可能解出束缚态．
