...
\item 下载完以后，可以备份一下，再次编译想要离线，见 \verb`doc/src/devdocs/build/build.md`：在 repo 根目录创建 \verb`Make.user` 里面加一行 \verb`USE_BINARYBUILDER=0`
...
\item 在新一代 x86 架构上编译的结果不能在直接拿到老一代跑，会直接无法启动。
\end{itemize}
