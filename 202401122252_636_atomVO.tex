...
\begin{enumerate}
\item store：原子地以非原子对象替换原子对象的值；
\item load：原子地获得原子对象的值；
\item is_lock_free：检查原子对象是否免锁；
\item compare_exchange_weak、compare_exchange_strong：原子地比较原子对象与非原子参数的值，若相等则进行交换，若不相等则进行加载。
\end{enumerate}
