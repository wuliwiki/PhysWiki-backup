...
给定拓扑空间 $B$ 和线性空间 $V$，如果存在一个拓扑空间 $E$ 和一个连续满射 $\pi:E\rightarrow B$，使得对于任意的 $x\in B$，都有 $\pi^{-1}(x)$\textbf{线性同构}于$V$，那么称这个结构 $(E, V, B, \pi)$ 为一个\textbf{向量丛（vector bundle）}。

%并且满足下列\textbf{相容条件（compatability）}：

%对于$B$中的任意开集$U$，存在\textbf{同胚映射}$\varphi: U\times V$，使得
%\begin{enumerate}
%\item $\varphi(p, v)\in \pi^{-1}(p)$；
%\item $v\mapsto \varphi(p, v)$是$V\to\pi^{-1}(p)$的线性同构，
%\end{enumerate}
%对于任意$p\in U, v\in V$恒成立，
\end{definition}


\autoref{def_Fibre_1} 中的相容条件是为了保证
