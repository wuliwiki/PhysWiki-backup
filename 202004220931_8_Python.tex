...
\begin{equation}
\left\{\begin{array}{lc}\begin{array}{c}A\\B\\C\end{array}&\begin{array}{c}x>90\\x>80\\x>60\end{array}\end{array}\right.\\\\
\end{equation}
\begin{lstlisting}[language=python]
x =99
if x>90:
...
plt.plot(A,C)
\end{lstlisting}
上述代码通过hold把两个图像放在一张画布上面．图像如~\ref{Python_fig1}
\begin{figure}[ht]
\centering
...
color='b'))
#添加文本注释
plt.text(2,0.8,r'普通文本$\sin(\pi x)$',color='b',weight='bold')
plt.title('图像标题')
plt.legend(loc='upper right',title='图例标题')#里面可有参数
...
\begin{figure}[ht]
\centering
\includegraphics[width=15cm]{./figures/Python_2.png}
\caption{matplotlib库使用} \label{Python_fig2}
\end{figure}
\begin{enumerate}
