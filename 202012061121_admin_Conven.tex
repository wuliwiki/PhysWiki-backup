...
\item 当区分矢量和坐标的不同时， 一般矢量空间中的矢量不能  矩阵的坐标表示为实数或复数的笛卡尔积（如 $\mathbb R^N, \mathbb C^N$ 或 $(c_1, c_2, c_3)$）， 也可以表示为列矩阵（如 $\bvec c, \bvec x$）．
\item  直接用普通变量字母（如 $x, y, u, v$）或者使用狄拉克符号（如 $\ket{x}, \ket{y}, \ket{u}, \ket{v}$）
\item 在粗体与正体矢量上方加 hat 表示单位几何矢量， 如 $\uvec x$．
\end{itemize}
