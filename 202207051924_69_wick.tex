...
\subsection{Wick 定理}
计算形如 $\bra 0 T\phi_I(x_1)\cdots\phi_I(x_n) \ket 0$ 的关键是，将 $\phi_I(x_i)$ 分解为正频率和负频率两个部分 $\phi(\bvec x)=\phi^{(+)}(\bvec x)+\phi^{(-)}(\bvec x)$（以下省略下角标 $I$）．
\begin{equation}
\phi^{(+)}(\bvec x) = \int \frac{d{}^3{\bvec p}}{(2\pi)^3} \frac{1}{\sqrt{2\omega_{\bvec p}}}a_{\bvec p} e^{-i\bvec p \cdot \bvec x} ,
\phi^{(-)}(\bvec x) = \int \frac{d{}^3{\bvec p}}{(2\pi)^3} \frac{1}{\sqrt{2\omega_{\bvec p}}}a_{\bvec p}^\dagger e^{i\bvec p \cdot \bvec x} 
\end{equation}
那么由于产生湮灭算符的性质，负频率部分作用于 $\ket 0$ 将得到 $0$．类似地，$\langle 0| \phi^{(+)}(\bvec x)=0$．将 $\bra 0 T\phi(x)\phi(y)\ket 0$ 中的场算符按正频率部分和负频率部分展开，可以得到
\begin{equation}
\bra 0 T\phi(x)\phi(y)\ket 0=\bra 0 \phi
\end{equation}
