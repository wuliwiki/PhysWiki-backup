...
\item 陈类（Chern class）** – 向量丛（vector bundles）的**特征类**
\item 欧拉示性数（Euler characteristic）** – 数学中的一种**拓扑不变量**
\item 不动点性质（Fixed-point property）** – 一种数学性质
\item 同调与上同调（Homology and cohomology）**
\item 同伦群与余同伦群（Homotopy group and Cohomotopy group）**
\item 结不变量（Knot invariant）** – 对等价结取相同值的函数
\item 链数（Linking number）** – 描述三维空间中两条闭曲线相互缠绕程度的数值不变量
\item 拓扑列表（List of topologies）** – 各种具体拓扑与拓扑空间的列表
\item 量子不变量（Quantum invariant）** – 数学结理论中的一个概念
\item 拓扑量子数（Topological quantum number）** – 因**拓扑量子效应**而取离散值的物理量
