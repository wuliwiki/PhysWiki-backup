...
更一般地，可以定义局部紧阿贝尔群$K$的海森堡群，其中$K$配备有一个 Haar 测度 [8]。这样的群有一个Pontrjagin 对偶群$\hat{K}$，它由$K$上所有取值于$U(1)$的连续特征组成，并在赋予紧开拓扑后同样是一个局部紧阿贝尔群。

与局部紧阿贝尔群$K$相关联的海森堡群，是希尔伯特空间$L^{2}(K)$的酉群中的一个子群，该子群由来自$K$的平移算子和来自$\hat{K}$的乘法算子生成。
