...
于是$f'(x_0)=0$. 不妨设$f''(x_0)>0$. 对于任何固定的$\delta>0$, 当$t$很大时, 区间$(x_0-\delta,x_0+\delta)$之外的积分的贡献都比$t$的任何负幂衰减得都快, 例如根据假定, 当$|x-x_0|>\delta$时有一$c>0$使得$f(x)<f(x_0)-c$, 从而区间$(x_0-\delta,x_0+\delta)$之外的积分估计为
$$
\begin{aligned}
...
从泰勒公式知道, 有一$\delta>0$使得在区间$(x_0-\delta,x_0+\delta)$上, 
$$
