...
显然， $z_n(x) = 0$ 是一个零函数（列）． 在任意积分区间 $[a,b]$ 上， 简谐函数 $\sin(nx + \phi)$， $\exp(\I n x)$ 都是零函数． 更一般地， 若 $f(x)$ 是一个性质良好的函数， $f(x)\sin(nx + \phi)$ 和 $f(x)\exp(\I n x)$ 也都是零函数， 例如高斯波包\upref{GausPk} $A\E^{-ax^2}\E^{\I nx}$．

笔者不会证明\footnote{即使会也会因太复杂而省略证明．}， 但从函数图像上这是符合直觉的： 若一个函数乘以一个另快速震荡的函数，震荡的平均值为零且震荡频率越来越快， 那么\autoref{F0_eq2} 的积分越接近 $0$．
\end{example}
