...
\item 10.11 设 $E$ 是 $R^n$ 中的开集． $E$ 中的 $k\geqslant 1$ 次\textbf{微分形式}（简称为 $E$ 中的 $k$-\textbf{形式}）是一个函数， 符号表示为 $\omega = \sum a_{i_1\dots i_k}(\bvec x)\dd{x_{i_1}} \wedge \dots \wedge \dd{x_{i_k}}$（指标 $i_1,\dots,i_k$ 各自从 $1$ 到 $n$ 独立变化） 的， 它给 $E$ 中的每个 $k$-曲面 $\Phi$， 按照规则 $\int_\Phi \omega = \int_D \sum a_{i_1,\dots,i_k}(\Phi(\bvec u)) \pdv*{(x_{i_1},\dots,x_{i_k})}{u_1,\dots,u_k}\dd{\bvec u}$ 规定一个数 $\omega(\Phi)=\int_\Phi \omega$， 这里的 $D$ 是 $\Phi$ 的参数域．
