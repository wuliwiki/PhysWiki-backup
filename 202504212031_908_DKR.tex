...
\end{enumerate}
检查 C-n 元组是否包含于 D-n 元组,设有 C-n 元组\(P = [P_1\quad P_2\quad \cdots \quad P_N]\)
和 D-n 元组
\[
Q = ]Q_1\quad Q_2\quad \cdots \quad Q_N[~
\]  
则有：\(P \subseteq Q \iff \text{至少存在一个 } i \text{ 使得 } P_i \subseteq Q_i\)

换句话说，**当且仅当在某一维上 \( P_i \subseteq Q_i \)，就可以认为整个 C-n 元组 \( P \) 包含于 D-n 元组 \( Q \)**。