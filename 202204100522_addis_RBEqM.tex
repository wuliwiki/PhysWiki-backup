...
\subsection{体坐标系}
在体坐标系中列出\autoref{RBEqM_eq6} 往往可以简化\autoref{RBEqM_eq6}． 这么做时， 通常令体坐标系的三个轴与刚体的三个主轴\upref{PrncAx}重合， 这样 $\mat I_0$ 就是三个主转动惯量构成的对角矩阵
\begin{equation}
\mat I_0 = \pmat{I_1 & 0&0 \\ 0& I_2 &0 \\ 0& 0& I_3}, \qquad
\mat I_0^{-1} = \pmat{1/I_1 & 0&0 \\ 0& 1/I_2 &0 \\ 0& 0& 1/I_3}
\end{equation}

 \autoref{RBEqM_eq6} 简化为
\begin{equation}\label{RBEqM_eq9}
\dot{\bvec \omega}_0 = \mat I_0^{-1} (\bvec \tau_0  - \bvec\omega_0\cross \mat I_0 \bvec\omega_0)
\end{equation}
其中
\begin{equation}
\bvec\omega_0\cross \mat I_0 \bvec\omega_0 = [(I_3-I_2)\omega_{03}\omega_{02},\ (I_1-I_3)\omega_{01}\omega_{03},\ (I_2-I_1)\omega_{01}\omega_{02}]
\end{equation}
代入得
\begin{equation}
\begin{aligned}
&\dot\omega_{01} = [\tau_{01} - (I_3-I_2)\omega_{03}\omega_{02}]/I_{1}\\
&\dot\omega_{02} = [\tau_{02} - (I_3-I_2)\omega_{03}\omega_{02}]/I_{2}\\
&\dot\omega_{03} = [\tau_{03} - (I_3-I_2)\omega_{03}\omega_{02}]/I_{3}
\end{aligned}
\end{equation}


\subsubsection{推导}
现在推导\autoref{RBEqM_eq9}． 令角（加）速度矢量和力矩在实验室坐标系中的坐标为
\begin{equation}
\bvec\omega_0 = \mat R\Tr \bvec\omega,\qquad
\dot{\bvec\omega_0} = \mat R\Tr \dot{\bvec\omega},\qquad
\bvec\tau_0 = \mat R\Tr\bvec\tau
\end{equation}
那么\autoref{RBEqM_eq6} 变为
\begin{equation}
...
\end{equation}
那么 $\mat\Omega_0$ 就是体坐标系中 $\omega_0\cross$ 代表的矩阵． 所以



注意此时式中的 $\bvec\omega$ 和 $\dot{\bvec\omega}$ 仍然是刚体相对于实验室坐标系的角（加）速度， 只是使用了体坐标系中的基底来计算坐标．
