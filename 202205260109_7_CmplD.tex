...
由\autoref{CmplD_cor3} 可知，特征为$0$的域都是完备域，因此我们接下来只讨论特征为素数$p$的情况．首先，我们给出两个相似的简单引理，对之后的定理证明有帮助：
...
由于$\mathbb{F}$是完备域，所以可以取$b_i\in\mathbb{F}$使得$b_i^p=a_i$．定义$h(x) = \sum_{i=0}^n b_ix^i$．由于$\opn{ch}\mathbb{F}=p$，据\autoref{CmplD_lem1} 知
\begin{equation}
\begin{aligned}
...
\end{equation}
即$f$也可约．但由于$f$是最小多项式，故矛盾，故$g$在$\mathbb{F}$上不可约．
