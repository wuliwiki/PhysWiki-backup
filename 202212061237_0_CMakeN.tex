...
\item 和 bash 一样， 变量都是 string 或者 list of strings， 使用变量格式为 \verb`${VAR}`．
\item 当字符串中没有空格时， 两边是否加双引号是等效的．
\item \verb`set` 可以对变量赋值， 例如 \verb`set(var, 1)`
\item \verb`set` 也可以把许多变量变成一个 list， 如 \verb`set(Foo a b c)` （等效于 \verb|set(Foo "a b c")|） 将 \verb`Foo` 变成包含 \verb`a b c` 的 list， 当 \verb`${Foo}` 被使用时相当于 \verb`a b c`， 例如 \verb`command(${Foo})` 相当于 \verb`command(a b c)`． 另外 \verb`command("${Foo}")` 相当于 \verb`command("a b c")` （双引号中的是一个变量）
