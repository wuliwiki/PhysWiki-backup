...
\end{equation}
对于量子体系，能够取到的$k$的值是离散的，以一维体系为例，设体系长度为$L$,动量$k$的所有取值为$2 n\pi/L$，即相邻动量值相差$2\pi/L$。如果我们把这个看作是在$k$空间中态的密度，则在三维情形中，单位体积里态的数目就是$V/(2 \pi)^3$。（这里假定的波函数是有周期边界条件的，如果我们假定波函数为驻波，此处的态数目相差一个$2^3$）。
