...
\item $\omega$ 称为\textbf{角频率（angular frequency，也译作圆频率）}，决定了函数的周期性变化速度；既然他表示周期性变化，之前也学过周期的概念，在这里给出周期T雨
\item $\varphi$ 称为\textbf{初相（initial phase）}，的进一步原因在于，称它即函数在在 $x=0$ 处的相位。
\item 
\item 。
\end{itemize}
