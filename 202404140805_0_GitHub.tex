...
如果一个 Git 仓库由多个人共同创作和修改，或者由同一个人在不同电脑上修改，那么就需要使用 Git 的联网功能。 这时 Git 可以发挥类似于同步网盘的作用。例如你在电脑 A 上创建了一个 Git 仓库并进行了若干 commit，你可以把该仓库和它的所有 commit 上传到 Git 服务器。 然后在电脑 B 上可以服务器把仓库\textbf{克隆（clone）}到本地。在这之后，无论那台电脑产生新的 commit，都可以将其\textbf{推送（push）}到服务器，然后在另一台电脑将其\textbf{拉下来（pull）}。
