...
我们只需要通过某种方法找到上式的某个解即可． 观察等式右边可以发现右边第一项对角块都为零（$E_m = E_n$）， 第二项是对角矩阵， 所以两项相加后矩阵的对角块都一定是对角矩阵（图未完成）． 所以等式左边的矩阵也需要满足同样的要求， 但 $H^1$ 是给定好的无法改变， 所以左边是否满足该要求只取决于基底 $\psi_n^0$ 的选取．
