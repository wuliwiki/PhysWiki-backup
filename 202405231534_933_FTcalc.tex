...
若函数$f(x)$在c处的极限值
 $$\lim_{x\to c}f(x)=f(c)\iff\lim_{x\to c}f(x)-f(c)=0$$
\end{definition}
\subsubsection{微分部份}
...
\item 这里由于之前使用的积分中值定理，可以知道$x\leq\xi(\Delta x)\leq x+\Delta x$，即$\xi$是$\Delta x$的函数，又\lim_{\Delta x\to0}x=x，\lim_{\Delta x\to0}x+\Detla x=x，由夹逼定理（\autoref{lem_FTcalc_3} ）可知：$\lim_{\Delta x\to0}c=x$。由于$f(x)$在$[a,b]$上连续，
\end{enumerate}
