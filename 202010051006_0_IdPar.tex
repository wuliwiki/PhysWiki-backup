...
\end{equation}
其中三个基底是对称的， 即\textbf{三重态（triplet）}， 一个是反对称的， 即\textbf{单态（singlet）}． % 链接未完成： 角动量加法中的例题
显然，三重态的任意线性组合再归一化都是对称的自旋态（3 维子空间）， 但反对称的只有单态． 我们可以把 $\uparrow\uparrow$ 和 $\downarrow\downarrow$ 视为自旋方向相同的态， 剩下两个视为自旋方向相反的态． 对于自旋相反的两个态， 我们只能说 “一个电子自旋为 $\uparrow$ 另一个为 $\downarrow$” 却不能说哪个是 $\uparrow$ 哪个是 $\downarrow$． 这正体现了全同粒子的不可区分性． 相比之下， $\uparrow\downarrow$ 就表明第一个电子是 $\uparrow$， 第二个是 $\downarrow$， 所以不能用于表示全同粒子．
