...
\mat B \mat A = \mat I
\end{equation}
那么 $\mat B$ 就叫做 $\mat A$ 的\textbf{逆矩阵}， 或者记为 $\mat A^{-1}$。 \footnote{从矢量空间的角度来看， 矩阵 $\mat A$ 把第一个矢量空间映射\upref{map}到第二个空间， 而 $\mat A^{-1}$ 就代表改映射的逆映射。} 这里的一个隐含要求是 $\mat A$ 必须代表一一映射， 如果是多对一的映射， 将不存在反映射。 一一映射要求 $\mat A$ 的每一列都线性无关，% 未完成： 在相关词条中说明
