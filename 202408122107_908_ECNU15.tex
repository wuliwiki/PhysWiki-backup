...
\item 三个全同电子处于一个圆频率为 $\omega$ 的一维谐振子势场中，忽略它们之间的相互作用。假设任一电子所处的本征波函数为 $\psi_{n}(x) = \varphi_{n}(x) \chi_{\sigma}$，其中 $\varphi_{n}(x)$ 表示一维谐振子的本征态（$n = 0,1,2,3$,...），$\chi_{\sigma}$ 表示电子自旋 $z$ 分量 $s_z$ 的本征态，$\sigma = \pm \frac{1}{2}$。该系统的基态能量是：
\end{enumerate}
