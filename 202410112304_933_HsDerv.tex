...
可以这样理解，$Df(x)$用$D$运算符代替了$\displaystyle\frac{\dd}{\dd x}f(x)$中的$\displaystyle\frac{\dd}{\dd x}$，$f_x$则是把$\displaystyle\frac{\dd}{\dd x}f(x)$中最重要的两部分拿出来显示。$\Delta f(x)$与$Df(x)$只是因为定义域不同，处理方法不同。$\dot{y}$和$y'$异曲同工，只是更着眼于“时间”。

在高中阶段，一般只要求使用拉格朗日记法，并且不建议使用其他记法。
