...
考察\autoref{RelDop_fig1} 的情况，我们直接在\textbf{$K_2$ 参考系中}进行分析，不妨假设光源 $S$ 正以 $v$ 的速度沿 $x$ 轴正方向运动，并且在 $K_2$ 参考系看来在 $t$ 时刻接收到的电磁波方向与 $x$ 轴成 $\phi$ 角。
\begin{figure}[ht]
\centering
...
假设在 $0$ 时刻光源位于 $P_0$ 点，它发出的电磁波于 $t$ 时刻被 B 点处的观测者接收到，并且观测者测得的电磁波方向与 $x$ 轴夹角为 $\phi$。那么 S 到 B 的距离为 $r_0=ct$。而经过 $\dd t$ 的时间后光源移动到了 P 点的位置，在这段时间内光源所经历的固有时（也就是原时）为 $\gamma\dd t=\dd t/\sqrt{1-v^2/c^2}$
