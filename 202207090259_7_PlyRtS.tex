...
\subsubsection{三次方程根式解与Cardano公式}
...
设$g(x)$在$\overline{\mathbb{F}}$上的根为$\beta_1, \beta_2, \beta_3, \beta_4$．由\autoref{VietaF_eq1}~\upref{VietaF}，$\sum_{i}\beta_i=0$，$\sum_{i<j}\beta_i\beta_j=p$，$\sum_{i<j<k}\beta_i\beta_j\beta_k=-1$，$\beta_1\beta_2\beta_3\beta_4=r$．
...
记$s(x)=(x-u)(x-v)(x-w)$，则$\opn{Dis}(f)=\opn{Dis}(g)=a^6\opn{Dis}(s)$．

由\autoref{PlyRtS_eq5} ，$u'=\beta_1+\beta_2$、$v'=\beta_1+\beta_3$和$w'=\beta_1+\beta_4$分别是$u$、$v$和$w$的平方根．且有：
\begin{equation}
\begin{aligned}
