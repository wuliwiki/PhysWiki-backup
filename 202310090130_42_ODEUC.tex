...
是 $C^r$ 类的。
\end{lemma}
\textbf{证明：}因为 $f\in C_x^r\cap C^{r-1}\Rightarrow F\in C_x^r\cap C_t^{r}\cap C_{t_0}^{r}$ ，所以 $F$ 对 $x$ （或 $t$ 或 $t_0$） 的 $r$ 阶偏导存在。剩下只需证明任意的 $r$ 阶混合导都存在，若先对 $t$ （或 $t_0$）求偏导，则 $\pdv{F(x,t)}{t}=f(x,t)$（或 $\pdv{F(x_1,t_1,t)}{t_1}=-f(x_1,t_1,t)$），由于 $f\in C^{r-1}$，所以剩下 $r-1$ 次混合导都存在；若先对 $x$ 求偏导，且第 1 次对$t$ 求偏导在第 $i$ 次，那么由
\begin{equation}
f\in C^{r-1}\Rightarrow\pdv[i-1]{}{x}\pdv{F(x,t)}{t}=\pdv[i-1]{f(x,t)}{x}\in C^{r-i} ~.
\end{equation}
...
\begin{equation}
g(x_1,t_1,t)=x+\int_{t_0}^tv(g(x_1,t_1,\tau))\dd\tau~.
\end{equation}
