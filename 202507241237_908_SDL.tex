...
仍以第 2×10¹⁷ 个素数 8512677386048191063 为例，假设尾项误差为零，计算得估计值为 8512681315554715386；前五位数字吻合，相对误差约为百万分之 0.46。Cipolla（1902 年）【41】【42】证明上述表达式是一个无限级数的前导项，该级数可以截断到任意次数，其通式为：
$$
...
Dusart（1999）【43】在 Cesàro/Cipolla 近似公式的基础上，通过调整最低阶常数项，给出了更紧的界。令 $B_k(x; C)$ 表示将上述 $B_k(x)$ 中最低阶常数项替换为参数 $C$ 后的函数。
...
Axler（2019）【44】进一步推广到了二阶表达式，得到了更高精度的上下界：
...
同样地，如果放宽参数 C，例如令 $C = 0$，则当 $n \geq 3468$ 时仍有：
