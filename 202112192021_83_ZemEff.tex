...
\end{align}
之前我们计算出来的$8$个态都是$H_{fs}^1$的本征态，因此$H_{fs}^1$在这一表象中是对角的．对于$j=2$的态，也就是$\psi_1,\psi_2,\psi_6,\psi_8$态，$H_{fs}^1=\gamma(3-8)=-5\gamma$.$j=3/2$的态,也就是$\psi_3,\psi_4,\psi_5,\psi_7$态，$H_{fs}^1=\gamma(3-8/2)=-\gamma$.
\begin{equation}
\begin{pmatrix}
