...
若令 $\bigwedge^k V=\Lambda^k_0 T$，其中 $\Lambda^k_0(V)$ 是所以 $(0,p)$ 型的斜对称张量（\upref{SIofTe}）构成的集合。那么此时得到的外代数便和张量里的\enref{外代数}{WegofT}一致。而外积 $\wedge$ 由张量的交错化映射定义，它满足这里外积的一切性质。
\end{example}
