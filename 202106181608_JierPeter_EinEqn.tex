...
\autoref{EinEqn_eq2} 最右边很长，我们可以把它分成两部分，分别是在度量$\eta_{ab}$下垂直和平行于$U^b$的项．显然，其中的$U_aU_b\partial^a\rho+\rho U_b\partial^aU_a+U_aU_b\partial^ap+p U_b\partial^aU_a$是平行于$U^b$的，因为它们都是在$U_b$前面乘以一个光滑函数的形式；剩下的部分，我们断言，是垂直于$U^b$的，即与$U^b$的内积为$0$．
