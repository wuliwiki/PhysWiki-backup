...
传递性 $\{x_n\}R\{x_n'\},\{x'_n\}R\{y_n\}\Rightarrow\{x_n\}R\{y_n\}$：因为 $\{x_n\}R\{x_n'\},\{x'_n\}R\{y_n\}$，所以对任一 $\epsilon>0$，存在 $N,N'$，使得只要 $n,m\geq N,n',m'\geq N'$，就有
\begin{equation}
d(x_n,x_m')<\epsilon/2,\quad d(x_{n'}',y_{m'})<\epsilon/2.~
\end{equation}
取 $N_1=\max\{N,N'\}$，则只要 $n,m>N_1$，就有
\begin{equation}
d(x_n,y_n)<d(x_n,x'_n)+d(x'_n,y_n)
\end{equation}
