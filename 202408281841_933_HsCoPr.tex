...
形如“若$P$，则$Q$”，用于表达两个命题之间的条件关系的命题，称为\textbf{条件命题}（conditional proposition或\textbf{蕴含命题}，implication）。
其中：命题$P$称为前件（antecedent），命题$Q$称为后件（consequent）$Q$组成，通过“如果……那么……”（if…then…）的形式来连接。


\end{definition}


“若 $P$ ，则 $Q$ ”只表示如果命题 $P$ 为真，那么命题 $Q$ 也为真。条件命题通常用符号$P\Rightarrow Q$或$P\rightarrow Q$表示，也读作“$P$蕴含$Q$”\footnote{事实上，用逻辑连接词表示的蕴含的定义是$\lnot P\lor Q$。但涉及这个定义的讨论，已超出高中的范畴，关于蕴含还有很多知识，可以参考\enref{命题及其表示法}{Propos}。}。
