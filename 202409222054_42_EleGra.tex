...
\subsubsection{G的改进}
对 G 改进的关键是，平等的对待 $\dd t$ 和 $\dd{\vec x}$。若 $\dd t^2$ 被某些函数乘，那么 $\dd{\vec x}$ 也应被某些函数乘。记 $\qty(1+\frac{2V}{m})$ 为 $g$，因此得到形如 $g(x)\dd t^2-\tilde g(x)\dd{\vec x}^2$ 的式子。而在进行Lorentz变换时，$\dd t^2$ 将变换为 $\dd t^2,\dd x^i\dd x^j$ 和 $\dd t\dd x^i$ 的线性组合。因此似乎根号下必须出现 $\dd t\dd x^i$ 的项。 
