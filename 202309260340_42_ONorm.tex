...
\end{theorem}
\textbf{证明：}我们只需要证明完备性（\autoref{def_cauchy_2}~\upref{cauchy}）即可。设 $A_i$ 是任一柯西序列，即对每一 $\epsilon>0$，存在 $N(\epsilon)>0$，使得只要 $m,k>N(\epsilon)$ 时就有 $d(\mathcal{A_m,A_k})<\epsilon$。记 $x_i\equiv\mathcal A_i x$，于是对 $m,k>N(\epsilon)$，就有
\begin{equation}
