...
\item Theorem A.18 (Fourier Inversion and Plancherel Formula, I) The Fourier transform on $\mathcal{S}\left(\mathbb{R}^{n}\right)$ has the following properties.
...
3. For all $\psi \in \mathcal{S}\left(\mathbb{R}^{n}\right)$, we have the Plancherel theorem: $\int_{\mathbb{R}^{n}}|\psi(\mathbf{x})|^{2} d \mathbf{x}=\int_{\mathbb{R}^{n}}|\hat{\psi}(\mathbf{k})|^{2} d \mathbf{k}$
\end{itemize}
