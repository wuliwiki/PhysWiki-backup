...
这里给出一个 Matlab 函数可以一次把三维空间中若干点绕非零矢量 $\bvec A$ 按照右手定则\upref{RHRul}旋转角度 $\theta$． 在程序中， 这些点可以用任意形状的 \verb|X, Y, Z| 数组来表示， 也可以用单个三列的矩阵 $P$ 来表示（每列分别为 $x, y, z$ 坐标）． $\bvec A$ 和确定的方式
