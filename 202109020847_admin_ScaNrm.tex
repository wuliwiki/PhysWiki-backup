...
把\autoref{ScaNrm_eq9} 的波函数代入正交条件\autoref{ScaNrm_eq7} 可知， 系数需满足的关系（令 $\Delta\phi_\pm = \phi_{\pm, 2} - \phi_{\pm, 1}$）
\begin{equation}
...
\begin{aligned}
\alpha &= \braket{}{} = \lim_{n\to\infty} \frac{\pi}{n} \int_{-n}^n \psi_{k,1}^*(x) \psi'_{k,2}(x) \dd{x}\\
