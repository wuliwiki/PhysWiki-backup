...
\begin{equation}
\mel*{r_i r_j}{\frac{r_<}{r_>^l}}{r_{i'} r_{j'}} \propto \delta_{i,i'} \delta_{j,j'}
\end{equation}
根据我的猜测， 泊松积分积分的结果应该和直接用数值积分的结果是一样的．
\begin{equation}
\mel*{r_i r_j}{\frac{r_<}{r_>^l}}{r_{i'} r_{j'}} = \delta_{i,i'} \delta_{j,j'} \qty(\frac{2l+1}{r_i r_j \sqrt{\omega_i \omega_j}} (T_l^{-1})_{i,j})
\end{equation}
参考 McCurdy, Solving the three-body Coulomb breakup problem using exterior complex scaling 式 68.
