...
\subsubsection{证明}
把\autoref{F0_eq3} 代入\autoref{F0_eq4}， 以 $i = 1$ 为例， 得
\begin{equation}
\begin{aligned}
&C_1(k) = \int_{-\infty}^{\infty} C_1(k') \int_{-\infty}^{\infty} f_1^*(k, x) f_1(k', x)\dd{x} \dd{k'} + \int_{-\infty}^{\infty} C_2(k') \int_{-\infty}^{\infty} f_1^*(k, x)f_2(k', x)\dd{x} \dd{k'}\\
&= \int_{-\infty}^{\infty} C_1(k) \ \dd{k} + \int_{-\infty}^{\infty} C_2(k) \int_{-\infty}^{\infty} f_1^*(k, x)f_2(k, x)\dd{x} \dd{k}
\end{aligned}
\end{equation}
