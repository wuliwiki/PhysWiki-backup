...
\item \verb`@param[in]` \verb`@param[out]` 和 \verb`@param[in,out]` 用于区分输入、输出和修改。
\item \verb`@return`: Describes the return value of a function. 仅限于 \verb`return` 的内容。
\item 如果 \verb`return` 的是 enum 等， 可以使用多个 \verb`@retval <VALUE> <description>` 描述每个返回值
\item 如果返回一个 tuple，可以这么描述
\begin{lstlisting}[language=cpp]
/** @return Tuple containing:
 *     - First element: User ID (int)
 *     - Second element: User name (std::string)
 *     - Third element: Account balance (double)
 */
\end{lstlisting}
\item \verb`enum` 和 \verb`enum class` 的每一项可以用 \verb`///< 文字` 描述。
...
\item \verb`@file`: Documents a file. 在文件的开头使用，后面跟 \verb`@brief`, \verb`@details` 等。
...
\subsection{安装使用}
\begin{itemize}
\item Ubuntu 可以直接 \verb`sudo apt install doxygen graphviz`（如果不装 graphviz，文档中的关系图会显示断裂的图标）。
