...
既然它们的拓扑由度量空间来保证，那么可以用度量定义的拓扑来验证。任意 $z_0=x_0+y_0$，设 $O_{z_0}$ 是 $z_0$ 邻域，于是其包含某一开球  $B(z_0,\epsilon)$， 选取 $x_0$ 的邻域 $B(x_0,\epsilon/2)$ 和 $y_0$ 的邻域 $B(y_0,\epsilon/2)$，于是任意 $x\in B(x_0,\epsilon/2),y\inB(y_0,\epsilon/2)$，对赋范空间和内积空间分别成立 
\begin{equation}
\begin{aligned}
d(x+y,z_0)=\norm{x+y-(x_0+y_0)}\leq \norm{x-x_0}+\norm{y-y_0}=d(x,x_0)+d(y,y_0)\leq \epsilon;\\
d(x+y,z_0)=\sqrt{{(x+y-z_0，x+y-z_0}\leq \norm{x-x_0}+\norm{y-y_0}=d(x,x_0)+d(y,y_0)\leq \epsilon;\\
\end{aligned}
\end{equation}
d(x+y,z_0)
\end{example}
