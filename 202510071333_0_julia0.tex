...
\item \verb`reflection.jl` 的 \verb`function method_instances(@nospecialize(f), @nospecialize(t), world::UInt)` 会返回 \verb`Core.MethodInstance` 数组，每个元素是 \verb`Core.Compiler.specialize_method(match)` 返回的。 其 \verb`def` 字段应该就是ya'suo
\item \verb`reflection.jl` 的 \verb`uncompressed_ir(m::Method)`
\end{itemize}
