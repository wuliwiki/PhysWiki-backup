...
\subsection{波的叠加}\label{sub_mcwave_1}
...
\begin{equation}\label{eq_mcwave_3}
\begin{aligned}
C^2
...
\begin{equation}\label{eq_mcwave_4}
\Delta\phi=k(r_1-r_2)+\phi_2-\phi_1~.
\end{equation}

中学物理常引入“振动步调”的概念来描述波源的振动相位。振动步调即振动位移，那么“振动步调一致”即波源的振动位移方程相同，$\phi_2-\phi_1=0$。当相位差为$2n\pi$，也就是振动加强时，由\autoref{eq_mcwave_4} 可知波程差$r_1-r_2=n\lambda$，当振动减弱，也就是相位差为$(2n+1)\pi$时，有$r_1-r_2=(2n+1)\lambda/2$。


若两个波源振动步调相反，则$\phi_2-\phi_1=\pi$。同理可得，振动加强时$r_1-r_2=(2n-1)\lambda/2$，振动减弱时$r_1-r_2=n\lambda$，观察可知，条件与步调一致时相反。

我们可以用图像记忆这两个规律。如下图所示，假设波源相位相同，用实线和虚线表示相位的传播，实线是波峰，虚线则是波谷。为了在$P$处总是振动加强，即振幅极大，只要两个波源到达$p$点的时间是周期的倍数即可，则波程差也是倍数波长。
\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/f58ffd16c6a5bd63.png}
\caption{} \label{fig_mcwave_2}
\end{figure}

如果波源反向，如下图所示，两个波源到达$P$的时间差是倍数半周期，才会振动加强。
\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/1277a75c4c0e5fee.png}
\caption{} \label{fig_mcwave_3}
\end{figure}
