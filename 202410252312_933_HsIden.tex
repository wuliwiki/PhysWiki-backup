...
注意，本节出现的$a,b$都是变量，因此可以给其赋予某个数或某种函数，例如：
\begin{itemize}
\item 取$\displaystyle a=x,b={1\over x}$，此时有：$\displaystyle x+{1\over x}\geq2,\quad(x>0)$
\item 取$\displaystyle a={y\over x},b={x\over y}$，此时有：$\displaystyle x+{y\over x}\geq2,\quad(x>0)$
\end{itemize}
