...
然后是关于$y=x$对称的情况，当 $\alpha$ 与$\beta$都处于第一象限时，二者的和为$\displaystyle\pi \over2$，这是之前学习过的“互余”。此时满足$\alpha + \beta = \frac{\pi}{2} + 2k\pi, (k\in\mathbb{Z})$，
即：
...
$\alpha’=\frac{\pi}{2} - \beta' + 2k'\pi, \qquad (k'\in\mathbb{Z})$，取$k=n+m-k'$：
\pi - \alpha+2m\pi


我们可以直接使用 $\alpha’$ 计算其关于 $y = x$ 的对称角 $\beta’$：

\beta{\prime} = \frac{\pi}{2} - \alpha{\prime}

代入 $\alpha’ = \pi - \alpha$：

\beta{\prime} = \frac{\pi}{2} - (\pi - \alpha) = -\frac{\pi}{2} + \alpha.

由于 $y = x$ 对称性仍然保持周期性关系，因此：

\beta = -\frac{\pi}{2} + \alpha + 2k\pi.


情况 2：$\alpha$ 处于第三象限

若 $\alpha$ 位于第三象限，可以先利用关于原点的对称变换，将其变换到第一象限：

\alpha{\prime} = \alpha - \pi.

此时，$\alpha’$ 位于第一象限，我们同样使用 $\alpha’$ 计算：

\beta{\prime} = \frac{\pi}{2} - \alpha{\prime}

代入 $\alpha’ = \alpha - \pi$：

\beta{\prime} = \frac{\pi}{2} - (\alpha - \pi) = \frac{3\pi}{2} - \alpha.

因此：

\beta = \frac{3\pi}{2} - \alpha + 2k\pi.


情况 3：$\alpha$ 处于第四象限

若 $\alpha$ 位于第四象限，可以先利用关于 $x$ 轴的对称变换，将其变换到第一象限：

\alpha{\prime} = -\alpha.

此时，$\alpha’$ 位于第一象限，同样使用 $\alpha’$ 计算：

\beta{\prime} = \frac{\pi}{2} - \alpha{\prime}.

代入 $\alpha’ = -\alpha$：

\beta{\prime} = \frac{\pi}{2} - (-\alpha) = \frac{\pi}{2} + \alpha.
