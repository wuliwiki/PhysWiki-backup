...
\eval{\pdv{z}{x}}_\theta~.
\end{equation}
按照定义\footnote{见偏导数\upref{ParDer}中的\autoref{ParDer_eq1}。}，应该是仅用 $x$ 和 $\theta$ 表示 $z$，然后求偏导。考虑极坐标系\upref{Polar}，$\theta$ 不变意味着 $y$ 与 $x$ 成正比即 $y=x\tan\theta$，代入\autoref{PChain_eq7} 得
\begin{equation}
