...
= 0.~
$$
这种柱坐标形式是不可压缩纳维–斯托克斯方程中第二常见的表示方法（最常见的是笛卡尔坐标表示）。
使用柱坐标的主要原因是利用对称性，从而使某个速度分量消失。

一个非常常见的情形是**轴对称流动**，假设**无切向速度**（即 $u_\varphi = 0$），且所有物理量都与角向 $\varphi$ 无关。在这种情况下，方程组简化为：
...
这种形式特别适用于管道流、射流、涡环等具有轴对称特性的流动建模，因为它能有效简化计算。
