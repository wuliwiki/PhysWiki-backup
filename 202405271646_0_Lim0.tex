...
如果一个数列不存在极限， 就称它是\textbf{发散（divergent）}的。 如果存在极限， 则称它是\textbf{收敛（convergent）}的。 如果说一个函数 $f(x)$ 是无穷大，那么就代表极限 $\lim\limits_{x\to \square} f(x)$ 不存在，或者说\textbf{不收敛}。 这通常可以表示为
\begin{equation}
\lim\limits_{x\to \square} f(x) = \infty~.
\end{equation}
\end{definition}
