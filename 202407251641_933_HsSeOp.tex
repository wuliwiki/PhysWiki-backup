...
\end{definition}
\addTODO{并集维恩图}
简单来讲，集合$A$和集合$B$的全部元素组成的集合就是集合$A$与$B$的并集。某种意义上，并集就好像两个集合做了一个“加法”，加法的过程是把所有的元素都放在一起，只不过由于\aref{集合}{def_HsSet_2}的性质比较特殊，导致并集的结果中，重复的元素只能存在一个。
...
\end{itemize}
下面的例题仅作为证明的展示，你可以按照类似的方法去证明上面的运算定律，而不仅依靠维恩图去作定性的理解。当然，哪怕现在不阅读这部分也是没问题的，仅作扩展视野。
...
1. 证明 $A \cup (B \cup C) \subseteq (A \cup B) \cup C $：
   ...
   取任意一个元素 $ x \in A \cup (B \cup C) $，有两种情况：
   
   如果 $ x \in A $，那么 $ x $ 也在 $ A \cup B $ 中，所以 $ x \in (A \cup B) \cup C $。
  
  如果 $ x \notin A $，但 $ x \in B \cup C $，那么又有两种情况：
     - 如果 $ x \in B $，那么 $ x \in A \cup B $，因此 $ x \in (A \cup B) \cup C $。
     - 如果 $ x \in C $，那么 $ x \in (A \cup B) \cup C $。

   综上所述，无论哪种情况，$ x \in (A \cup B) \cup C $。因此，$ A \cup (B \cup C) \subseteq (A \cup B) \cup C $。

2. 证明 $ (A \cup B) \cup C \subseteq A \cup (B \cup C) $：

   取任意一个元素 $ y \in (A \cup B) \cup C $，我们有两种情况：
   - 如果 $ y \in (A \cup B) $，那么 $ y \in A $ 或 $ y \in B $：
     - 如果 $ y \in A $，那么 $ y \in A \cup (B \cup C) $。
     - 如果 $ y \in B $，那么 $ y \in B \cup C $，因此 $ y \in A \cup (B \cup C) $。
   - 如果 $ y \in C $，那么 $ y \in B \cup C $，因此 $ y \in A \cup (B \cup C) $。

   综上所述，无论哪种情况，$ y \in A \cup (B \cup C) $。因此，$ (A \cup B) \cup C \subseteq A \cup (B \cup C) $。

由于我们已经证明了 $ A \cup (B \cup C) \subseteq (A \cup B) \cup C $ 以及 $ (A \cup B) \cup C \subseteq A \cup (B \cup C) $，我们可以得出结论：
$$ A \cup (B \cup C) = (A \cup B) \cup C .~$$
