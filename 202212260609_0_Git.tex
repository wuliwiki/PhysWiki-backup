...
安装后的设置
\item \verb|git fsck| 可以检查数据库是否出现错误（硬盘坏点等）， 但无法恢复， 只能通过备份。
\item \verb|git status| 一般只是检查 tree 中的文件的修改时间是否有更新， 而不会做一次完整的 checksum。 如果需要检查是否有硬件 bit rot， 可以先 touch 所有文件： \verb|find . -type f -exec touch {} +|。
\end{itemize}
