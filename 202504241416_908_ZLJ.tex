...
但对于一般的（交换）环上的模而言，并非每个模都是自由的。例如，模 \( \mathbb{Z}/n\mathbb{Z} \) 就不是自由的阿贝尔群（也即不是自由的 \( \mathbb{Z} \)-模）。与 \( \mathbb{Z}/n\mathbb{Z} \) 的张量积为：
\[
M \otimes_{\mathbb{Z}} \mathbb{Z}/n\mathbb{Z} = M / nM~
\]
更一般地，设\( M \)是某个\( R \)-模，且给定其一个表示，即一组生成元 \( m_i \in M,\, i \in I \) 和一组关系：
\[
\sum_{j \in J} a_{ji} m_i = 0,\quad a_{ij} \in R，
\]
则张量积可以通过以下**余核（cokernel）**来计算：
\[
M \otimes_R N = \operatorname{coker}(N^J \to N^I)
\]
其中 \( N^J = \bigoplus_{j \in J} N \)，\( N^I = \bigoplus_{i \in I} N \)，映射 \( N^J \to N^I \) 的定义为：将第 \( j \) 个拷贝中的某个 \( n \in N \) 发送到 \( a_{ij} n \)（作为 \( N^I \) 中第 \( i \) 个分量）。

通俗来说，可以理解为：\( M \) 的一个表示给出了 \( M \otimes_R N \) 的一个表示。这一计算方式反映出张量积是一个**右正合函子（right exact functor）**。

但张量积**一般不是左正合的**，也就是说，对于一个 \( R \)-模的**单射**（injective map）：
\[
M_1 \to M_2
\]
张量积下的映射：
...
例如，对映射 \( \mathbb{Z} \xrightarrow{n} \mathbb{Z} \)（乘以 \( n \)）施加张量 \( \otimes_{\mathbb{Z}} \mathbb{Z}/n\mathbb{Z} \)，得到的是零映射：
\[
0 : \mathbb{Z}/n\mathbb{Z} \to \mathbb{Z}/n\mathbb{Z}
\]
这显然不是单射。

这种张量积不具左正合性的“缺失”由所谓的**Tor 上推子函子（higher Tor functors）**来度量。所有高阶的 Tor 函子可以组合成所谓的**导出张量积（derived tensor product）**。
