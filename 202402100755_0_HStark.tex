...
那么如何确定非好本征态如 $\ket{2,0,0}$ 的极化率呢？ 考虑 TDSE， 根据绝热近似\upref{AdiaQM}，若波函数初始处于 $\ket{2,0,0}$，需要先分解为好本征态的线性组合，然后再考虑每个好本征态在电场中的变化。 把经过电场扭曲后的 $\ket{n,l,m}$ 记为 $\ket{n,l,m,*}$，则
\begin{equation}
\mel{n,l,m,*}{z}{n,l,m,*} = \sum_\alpha \abs{c_\alpha}^2\mel{n,\alpha,m,*}{z}{n,\alpha,m,*}~.
\end{equation}
在电场为零时， 对任意 $\ket{n,l,m}$ 左边都是零， 所以只有二阶修正起作用：
\begin{equation}
\sum_\alpha \abs{c_\alpha}^2 p_{(n,\alpha,m),z}^2~.
\end{equation}
\end{example}
