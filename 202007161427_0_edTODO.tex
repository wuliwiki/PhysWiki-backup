...
\begin{itemize}
\item 在设置窗口中增加 “自动编译” 开关（属于实验功能，默认关闭）， 如果开启， 在修改词条内容时每 10s 自动按一次保存按钮， 无修改则不保存．
...
\item （已完成）code 按钮支持新语言： python, pythonC（python 命令行）, julia, markdown, bash, makefile, json, git, cmake, regex, latex

\item （已完成）符号替换设置打开时， json 编辑窗自动排版格式太慢， 是否考虑直接使用正确的格式保存文件， 而不是打开后再排版？

\item （已完成）在加载页面（有转圈动画的那个）显示 “建议使用 Chrome 或 Chromium 内核的浏览器”

\item （已完成）替换模式下上传图片后不需要再提示 “该图片已存在， 是否替换？”
...
\item 如果 A 用户正在编辑， admin 改变了词条， 则立即将 A 用户对该词条的编辑窗变为只读， 自动备份也停止， 弹出提示 \verb|该词条已经被管理员临时修改， 需要发布后才能编辑|， 词条被 publish 以后也不需要自动恢复， 而是需要关闭 tab 再打开才能恢复．
...
\item 每五分钟的备份也要包括图片（figures 文件夹中的 png, svg, pdf）， 命名规则和 tex 文档一样． 只有图片文件内容被改变时才备份（可以通过 hash 判断或者逐字节对比）．
