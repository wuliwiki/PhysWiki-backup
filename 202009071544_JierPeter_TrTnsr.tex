...
为了方便将坐标变换推广，我们把二阶张量$f$的坐标表示为一个\textbf{行向量的行矩阵}$\bvec{F}$，也就是说，选定了$V_1$和$V_2$的基以后，用$\bvec{c}(\bvec{v}_1)$和$\bvec{c}(\bvec{v}_2)$分别表示$\bvec{v}_1\in V_1$和$\bvec{v}_2\in V_2$在这两组基下的坐标列矩阵，那么$f(\bvec{v}_1, \bvec{v}_2)=\bvec{F}\bvec{v}_1\bvec{v}_2$．
