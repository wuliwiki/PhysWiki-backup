...
%例题未完成 
在之前的例题中，我们就已经认识到直接的将电子看作为一个自旋的球体是危险的，这也就引发了经典推论的错误结论．实际结果于经典结果偏差的被称之为$g$因子：
...
\end{equation}
不过，这样的计算还是有一个严重的漏洞．于之前我们引入$g$因子类似的．我们所讨论静止的电子系并非一个惯性参考系．因为它在围绕原子核旋转时存在一定的加速度，因此我们还要引入一个适当的动力学修正——托马斯(Thomas)xu
微扰项为
\begin{equation}
