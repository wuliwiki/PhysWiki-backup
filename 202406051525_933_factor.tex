...
\begin{equation}
m\times(m+1)\times\cdots \times(n-1)\times n={1\times\cdots \times(m-1)\times m\times(m+1)\cdots \times(n-1)\times n\over 1\times\cdots \times(m-1)}={n!\over (m-1)!}.~
\end{equation}
\end{example}

\begin{example}{求从自然数n开始依次递减的m个数的乘积}
\begin{equation}
n\times(n-1)\times\cdots \times[n-(m-2)]\times [n-(m-1)]={n!\over m!}.~
\end{equation}
\end{example}
