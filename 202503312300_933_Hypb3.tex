...
将复数解中虚部所对应的位置标在另一条坐标轴上，得到\textbf{虚顶点（co-vertices）}，这两个虚顶点之间的连线称为\textbf{虚轴（conjugate axis）}，长度为 $2b$，其中 $b$ 被称为\textbf{半虚轴（semi-conjugate axis）}。
...
根据前文关于实解与虚解的讨论，这里引入了“\textbf{实轴（transverse axis）}”与“\textbf{虚轴（conjugate axis）}”的概念。双曲线与坐标轴的交点称为\textbf{顶点（vertices）}，连接两个顶点的线段称为\textbf{实轴（transverse axis）}，长度为 $2a$，其中 $a$ 被称为\textbf{半实轴（semi-transverse axis）}。双曲线的\textbf{焦点（foci）}始终位于实轴上。

将复数解中虚部所对应的位置标在另一条坐标轴上，得到\textbf{虚顶点（imaginary vertices）}，这两个虚顶点之间的连线称为\textbf{虚轴（conjugate axis）}，长度为 $2b$，其中 $b$ 被称为\textbf{半虚轴（semi-conjugate axis）}。

许多学生在学到这里时会产生疑问：既然是“虚”的，为什么还要在图像中标出来？这个\textbf{虚轴（conjugate axis）}到底有什么意义？事实上，在复变函数的研究中，这些\textbf{虚顶点（imaginary vertices）}与\textbf{虚轴（conjugate axis）}具有重要的几何意义。当函数的定义域扩展到复数范围时，这些结构会在图像中真实呈现。但由于高中阶段不涉及此部分内容，这里仅作简要说明，以满足对图形结构的进一步好奇。
根据前面所说的实解与虚解，这里也借用称为实轴虚轴。双曲线与坐标轴的交点称为\textbf{顶点}，顶点连线称为\textbf{实轴}，记作$2a$，其中$a$称为\textbf{半实轴}。焦点始终在实轴上。将复数解的系数作为坐标标记在另一根坐标轴上，称为\textbf{虚顶点}，称中间的线段为\textbf{虚轴}，记作$2b$，其中$b$称为\textbf{半虚轴}。与椭圆不同的是，双曲线的实轴与虚轴并不因为$a,b$值的大小而影响，而是完全取决于符号。
