...
\begin{equation}
\begin{aligned}
|\mathcal A(\bvec x+\bvec y)-(\mathcal Ax+\mathcal Ay)|^2&=(\bvec x+\bvec y)^2+\bvec x^2+\bvec y^2-2\left(\mathcal A(\bvec x+\bvec y),\mathcal Ax+\mathcal Ay\right)\\
&=(\bvec x+\bvec y)^2+\bvec x^2+\bvec y^2-2(\bvec x+\bvec y,\bvec x)-2(\bvec x+\bvec y,\bvec y)\\
&=0
\end{aligned}
~,\end{equation}

因而$\mathcal A(\bvec x+\bvec y)=\mathcal Ax+\mathcal Ay$。同理可得$\mathcal A(kx)=k\mathcal A(\bvec x)$。
...
$\mathcal A$是正交变换$\Longleftrightarrow $对任意$\bvec x\in V,|\mathcal Ax|=|\bvec x|$。
...
在复数域上，$f$有$n$个特征值。设有$k$个实特征值，则对应的特征向量是$V$上的一维不变子空间。设复特征值表示为$a+\mathrm ib$，对应特征向量表示为$\bvec x+\mathrm i \bvec y\,(\bvec x,\bvec y\in V)$。利用$f$的线性可得：
\begin{equation}
\begin{aligned}
...
若正交变换$A$有复根，对应特征向量$\bvec x=\bvec u+\mathrm i\bvec v$。则$\bvec u,\bvec v$是相互正交的特征向量。令$W=\opn{Span}\{\bvec u,\bvec v\}$，则有：
...
由于$A'=A^{-1}$且$Ax=\lambda \bvec x$，则$A'\bvec x=A^{-1}\bvec x=\frac{1}{\lambda}\bvec x=\lambda^* \bvec x$。左乘特征向量的转置得：
\begin{equation}
...
如果$A$有一特征值$\lambda_1$且$Ax=\lambda_1 \bvec x$，设单位向量$\bvec y\cdot \bvec x=0$，由\autoref{lem_ortho_2} 知，$\bvec y$也是$A$的特征向量。设$Ay=\lambda_2 \bvec y$，则
\begin{equation}
...
(\bvec x,\mathcal By)=(\mathcal Bx,\bvec y)~,
\end{equation}
则称$B$为对称变换。
\end{definition}
...
\begin{equation}
(By,\bvec x)=(\bvec y,Bx)=0~,
\end{equation}
因而$By\in U^{\bot}$，$ U^{\bot}$是$B$的不变子空间。
\begin{exercise}{}
