...
可以看到，推导得到的 \autoref{eq_Hypb3_3} 与 \autoref{eq_Hypb3_5} 在形式上完全一致\footnote{即此时$\left(\displaystyle\frac{m}{2}\right)^2=\displaystyle c^2-\left(\frac{m}{2}\right)^2=2\implies m=2\sqrt{2},c=4$}。而前面由果及因地用反比例函数来认识双曲线，现在正是从双曲线的几何定义出发得到了他的表达式。

为了便于记录，同时与椭圆的表示方式统一，一般规定符号为正的参数记作 $a^2$，符号为负的参数分母记作 $b^2$。需要注意的是，这里 $a$ 与 $b$ 的定义与椭圆中的选取略有不同，并未要求二者的大小关系。根据这一约定，有：
\begin{equation}
