...
\end{equation}
其中使用了酉矩阵的性质 $\mat R^{-1} = \mat R\Tr$ （\autoref{UniMat_eq2}~\upref{UniMat}）．

证明： 要计算 $\bvec L = \bvec I \bvec \omega$， 把\autoref{ITensr_eq6} 右边三个矩阵从右到左作用在 $\bvec \omega$ 上． 先算 $\mat R\Tr \bvec \omega$ 把实验室系中的角速度矢量 $\bvec \omega$ 变换到体坐标系中的 $\bvec \omega_0$． $\bvec I_0 \bvec \omega_0$ 得到实验室系中的角动量 $\mat L_0$， 再通过 $\mat R$ 变换到实验室系中的角动量 $\bvec L$． 证毕．
