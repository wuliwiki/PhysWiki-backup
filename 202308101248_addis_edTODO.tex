...
\item 如果上传时发现图片文件的 hash 已存在于 \verb|images.hash|， 则检查 \verb|images.figures| 中未标记 \verb|figures.deleted| 的图片环境 提示已存在， 是否引用（\verb|\autoref{}|）（推荐用户这么做）， 或者重复使用图片（等效于上传新图片， 但更新 \verb|figures.aka| 指向已存在的图片）。 
