...
\item 如果需要其他类型的 option， 可以用 cached variable： \verb|set(变量 "默认值" CACHE STRIN或BOOL或 "描述")|。 如果之前 \verb|变量| 已经被赋值（例如命令行 \verb|cmake -D 变量=...|）， 则值不会被覆盖。 这种变量叫做 \textbf{cached variable}。
