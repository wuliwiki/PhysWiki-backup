...
\item 打开词条后哪怕有一点改变都需要备份以及标记占用。
\item 词条备份文件确保换行符使用 LF 而不是 CRLF， 然后运行 \verb|./PhysWikiScan --backup 202X_xxx_xxx.tex| 更新数据库。 如果 exit code 为 1， 则说明 hash 已存在， 没有添加。
\item 关闭词条时， 如果内容有改变， 也备份一次。
\item 历史版本中的字符数加减使用 \verb|history.add/del| 中的数据。
...
\item 发布按钮改成 “审稿”， 弹出窗口， 给用户选择 \verb|review.action| 和 \verb|review.comment|。 插入数据库（补全其他列）， 确保审稿的 hash 存在于 \verb|history| 表。 如果选择发布， 则行为和现在的发布按钮相同。 若选择撤回， 则撤回到上次发布的内容。
\item 上传新的图片时， 插入信息到 \verb|figures| 表格， 弹出窗口让用户填写版权信息（附件功能先不做）。 如果发现上传图片文件的 hash 已存在于 \verb|images.hash|， 则提示已存在， 是否引用（\verb|\autoref{}|）， 或者重复使用图片（更新 \verb|figures.aka| 到已存在的图片）。
\item 若上传 pdf + svg 图片， 更新 \verb|figures.image_alt|。 同时支持上传其他文件（如果后缀名时 \verb|eps|
\item 若替换图片， 更新 \verb|figures.image_old|。
\item 在图片的下拉菜单中做一个图片浏览器按钮， 打开一个面板可以浏览所有图片。 选中图片后
\end{itemize}
