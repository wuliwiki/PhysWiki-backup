...
事实上，注意到 $\exp(\I q\bvec E\vdot\bvec r\dd t)$ 不过是一个普通函数而不是微分算符， 所以我们只需要把它和波函数相乘： $\exp(\I q\bvec E\vdot\bvec r\dd t)\Psi$。 根据 由于改算符只混合每个位置 $\bvec r$ 处不同的分波， 所以只需要计算 $\mel{Y_{l,m}}{\exp(\I q\bvec E\vdot\bvec r\dd t)}{Y_{l',m'}}$ 即可。 但话又说回来，做 $N_l^2/2$ 个球面积分，真的会比例如 Crank-Nicolson 要快吗？我看不见得。
