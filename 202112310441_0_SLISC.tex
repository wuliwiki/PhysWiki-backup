...
\item 实现了密矩阵和一些稀疏矩阵．
\item 实现了密矩阵的剪切， 这会有少量的额外运算， 但使用起来却比指针方便得多． 用户可以自行选择使用哪种接口．
\item 的底层使用 intel 的 Math Kernel Library (MKL) 来实现线性代数运算， 可以保证性能．
\item 所有的函数都尽量首先使用和 Lapack 类似的指针接口实现， 然后再封装一层更友好的接口．
\item 如无必要尽量不使用 \verb|unsigned| 类型．
\item 为了保证性能， SLISC 中的函数尽量不改变矩阵尺寸， 这是因为内存的动态分配往往耗时较多．
\end{itemize}
