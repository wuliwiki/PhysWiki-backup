...
\subsubsection{算术组合学}
算术组合学起源于数论（Number Theory）、组合数学（Combinatorics）、遍历理论（Ergodic Theory）和调和分析（Harmonic Analysis）之间的相互作用。它研究与算术运算（加、减、乘、除相关的\textbf{组合估计（Combinatorial Estimates）}。  
其中，当仅涉及加法与减法运算时的特殊情形称为\textbf{加性数论（Additive Number Theory）}  
或\textbf{加性组合学（Additive Combinatorics）}。  
算术组合学中的一个重要技术是\textbf{动力系统的遍历理论（Ergodic Theory of Dynamical Systems）}。

---

\textbf{无穷组合学（Infinitary Combinatorics）}  
（主条目：Infinitary Combinatorics）  

无穷组合学（又称\textbf{组合集论 Combinatorial Set Theory}）  
是将组合思想扩展到\textbf{无限集合（Infinite Sets）}的研究领域。  
它属于\textbf{集合论（Set Theory）}（数理逻辑的一个分支），  
但同时借鉴了集合论与\textbf{极值组合学（Extremal Combinatorics）}的思想与工具。  
该领域研究的对象包括\textbf{连续图与树（Continuous Graphs and Trees）}、  
\textbf{Ramsey 定理的扩展（Extensions of Ramsey’s Theorem）}、  
以及\textbf{Martin 公理（Martin’s Axiom）}等。  
近年的发展涉及\textbf{连续统的组合学（Combinatorics of the Continuum）}[23]  
与\textbf{奇异基数后继的组合学（Combinatorics on Successors of Singular Cardinals）}[24]。

\textbf{Gian-Carlo Rota}曾使用“\textbf{连续组合学（Continuous Combinatorics）}”一词来描述\textbf{几何概率论（Geometric Probability）}，  
因为\textbf{计数（Counting）}与\textbf{测度（Measure）}之间存在许多类比关系。
```
