...
\begin{equation}
F^{(v)}_{\lambda,\lambda'} = F^{(z1)}_{\lambda,\lambda'}\pdv{r} + \frac{F^{(v2z)}_{\lambda,\lambda'}}{r}~.
\end{equation}
其中 $F^{(z1)}$ eq_HeTDSE_20
\begin{equation}
F^{(v2z)} = -\cos\theta -\sin\theta\pdv{\theta}~.
\end{equation}

\begin{equation}\ali{
&\quad\mel{\mathcal{Y}_{l_1 l_2}^{LM}}{F^{(v2z)}}{\mathcal{Y}_{l_1' l_2'}^{L'M'}}\\
&= \delta_{l_2, l_2'} \sum_{m_1,m_2, m_1'} \bmat{l_1 & l_2 & L\\ m_1 & m_2 & M}\bmat{l_1' & l_2' & L'\\ m_1' & m_2 & M'} \times\\
& \quad\iint Y_{l_1m_1}^*(\uvec r_1) F^{(v2z)}(\uvec r_1)Y_{l_1' m_1'}(\uvec r_1)\dd{\Omega_1}~.
}\end{equation}
最后的积分可以参考\autoref{eq_HyTDSE_13}~\upref{HyTDSE}。
