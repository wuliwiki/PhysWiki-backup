...
与复数类似， 任何四元数都可以表示为模长乘以一个模长等于 $1$ 的四元数： $A = \abs{A} q$． 其中 $q$ 类似于复数的 $\E^{\I\theta}$． 而由于 $\abs{q} = q\tilde q = \tilde q q = 1$， 可见 $q$ 的共轭就是它的逆 $q^{-1} = \tilde q$． 这可以类比 $\E^{\I\theta}$ 的逆就是它的复共轭． $\E^{\I\theta}$ 可以代表一个角度为 $\theta$ 的平面旋转， 而在 “四元数与旋转矩阵\upref{QuatN}” 中会看到， 任意模长为 $1$ 的四元数都可以表示一个唯一的三维旋转．
