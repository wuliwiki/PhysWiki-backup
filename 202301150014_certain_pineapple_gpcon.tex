...
\subsection{与不变子群的关系}
在不变子群最初的定义中，我们曾要求其生成的左右陪集相等，即$\forall g \in G$ \\
$gH=Hg$，既$g^{-1}Hg=H$，用本节的语言便是$H$中包含完整的共轭类。

则由此可以引出找不变子群的方法，我们可以先写出所有的共轭类，在通过讲几个共轭类组合起来，找到运算封闭的共轭类组合便可以得到不变子群。

\begin{example}{找到$D3$群的不变子群}
由前一例题中结果可知$D3$群中的不变子群为
\end{example}