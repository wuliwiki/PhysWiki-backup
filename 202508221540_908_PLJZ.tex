...
$$
接着，考虑一个SU(2)矩阵 $U$ 在这类矩阵空间上的共轭作用：
$$
...
$$
其中 $R(U) \in \mathrm{SO}(3)$。

这个映射就是 $\mathrm{SU}(2)$ 对 $\mathrm{SO}(3)$ 的双覆盖的具体实现，因此说明了：$\mathrm{SU}(2) \;\cong\; \mathrm{Spin}(3)$.此外，$R(U)$ 的各个分量可以通过如下迹运算恢复出来：$R(U)_{ij} = \frac{1}{2} \operatorname{tr}(\sigma_i U \sigma_j U^{-1})$.
