...
之前已经聊了很久恒等式了，有两个与它相关的概念\textbf{公式（formula）}和\textbf{定义式（Definition）}很容易混淆。它们并不完全对立，事实上，公式和定义式都属于恒等式，定义式很好理解，因为如果不恒等就相当于di公式不仅在约定的范围内成立，而且一般应用的条件或范围也并不苛刻。

恒等式的本质是揭示数学中的普遍规律，是一种真理性陈述。而公式则更多地强调变量之间的联系以及它们在特定情境中的应用。例如，圆的面积公式 ￼ 不仅揭示了圆面积与半径之间的关系，还为具体的计算提供了工具。公式的作用更具工具性和操作性，适用于描述变量间的依赖关系，特别是在解决问题时被广泛使用。

这里需要注意的是，定义（式）一般通过等式的形式给出，它本质上也是恒等式，因为它通过一个明确的等号将数学对象与其性质或关系结合在一起。定义式常常为公式的建立提供理论基础。

此外，公式与定义式的应用超越了纯数学的范畴，在物理、计算机科学和工程学等领域同样扮演着重要角色。例如，物理学中的牛顿第二定律公式 ￼ 描述了力与质量、加速度的关系，而计算机科学中的复杂度公式则用于衡量算法的效率。

综上，恒等式是普遍的数学真理，而公式则是在特定情境中应用这些真理的工具。公式更关注变量之间的关系，并因其适用性广泛被引入到数学以外的学科中，成为跨领域沟通的桥梁。

在数学中，**恒等式（identity）和公式（formula）**是两个既密切相关又有区别的概念。恒等式是指在其定义域内，无论变量取何值，等式两边始终相等的表达式。例如，￼ 是一个恒等式，它在所有定义域范围内的变量取值下均成立。
