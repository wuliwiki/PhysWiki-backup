...
线性相关性的概念源自向量表示的唯一性。以几何向量为例，当给定一组向量$\bvec{v}_1, \bvec{v}_2, \cdots, \bvec{v}_n$，其线性组合$a_1\bvec{v}_1+a_2\bvec{v}_2+\cdots+a_n\bvec{v}_n$也是一个向量，我们可以称列矩阵$\pmat{a_1&a_2&\cdots&a_n}^{\mathrm{T}}$为该向量的\textbf{坐标}。两个向量相加后的坐标，就是两个向量的坐标矩阵直接相加。零向量显然有一个坐标，即零矩阵$\pmat{0&0&\cdots&0}^{\mathrm{T}}$；如果零向量还有一个\textbf{非零}的坐标$$
