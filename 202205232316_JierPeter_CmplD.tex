% 完备域
% complete domain|可分多项式|reducible polynomial|微商|导数|形式微商|形式导数|多项式|重根|自同构|形式求导|形式微分
...
设$\mathbb{K}$是非零多项式$f\in\mathbb{F}[x]$的分裂域，则$f$在$\mathbb{K}$中无重根的充分必要条件是$(f, \opn{D}f)=1$\footnote{即最大公因子为$1$．}．
\end{corollary}
...
设$\mathbb{K}$是$f\in\mathbb{F}[x]$的分裂域，且$f$是$\mathbb{F}[x]$上的非零\textbf{不可约}多项式，那么$f$在$\mathbb{K}$中无重根的充分必要条件是$\opn{D}f \neq 0$．
\end{corollary}

\textbf{证明}：

注意\autoref{CmplD_cor1} 和\autoref{CmplD_cor2} 的题设差异在于，后者多了“在$\mathbb{F}[x]$上不可约”这一要求．我们只需要证明加上这一要求时，“$(f, \opn{D}f)=1$”和$\opn{D}f\neq 0$等价即可．
