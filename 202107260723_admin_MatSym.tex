...
\item  特别注意显示成小数形式的 \verb|sym| 类型数字可能是不精确的． 例如运行 \verb|x = sym('1e-1')|， 输出显示 \verb|x = 0.1|， 这说明 \verb|x| 是一个变精度数字， 存在误差， 若把它进行运算， 则该误差会传递． 而 \verb|x = sym('1/10')| 显示为 \verb|x = 1/10| 无论参与什么计算都没有任何误差．， 例如令 \verb|x = vpa(sqrt(sym('2')), 5)|， 得到 \verb|x = 1.4142| （\verb|sym| 类型）， 然后计算 \verb|(x^(1/128))^128| 得到 \verb|1.41421356....| 而不是 \verb|1.4142|．
\end{itemize}
