...
考虑\textbf{Stern-Gerlach 实验}\upref{SGExp}中的序列实验（\autoref{SGExp_sub1}~\upref{SGExp}）．

让银原子束流先通过一台$\opn{SG}\uvec{z}$仪器，取出结果为$+\hbar/2$的态$\ket{S_z; +}$，让它通过一台$\opn{SG}\uvec{x}$仪器，取出其中某一束流$\ket{S_x; \pm}$，再让该束流通过一台$\opn{SG}\uvec{y}$仪器．最终得到$y$方向自旋为$+\hbar/2$束流$\ket{S_y; +}$的概率是多少呢？
