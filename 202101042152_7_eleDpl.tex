...
我们从\autoref{eleDpl_eq4} 出发，并且考虑到$\abs{\bvec{r}}\gg\\\abs{\bvec{r}_1}+\abs{\bvec{r}_2}$以及$\abs{\bvec{a}-\bvec{b}}=\sqrt{\bvec{a}^2+\bvec{b}^2-2\bvec{a}\cdot\bvec{b}}$，从而得到\footnote{符号$\doteq$此处指约等于，或者取极限后的结果．}：
\begin{equation}
\begin{aligned}
...
&=\frac{q}{4\pi\epsilon_0}\qty(\frac{\sqrt{\bvec{r}_1^2+\bvec{r}_2^2+\bvec{r}\cdot(\bvec{r}_1-\bvec{r}_2)}}{\abs{\bvec{r}-\bvec{r}_1}\abs{\bvec{r}-\bvec{r}_2}})\\
&\doteq \frac{1}{4\pi\epsilon_0}\qty(\frac{\bvec{r}\cdot\bvec{p}}{\bvec{r}^2})
\end{aligned}
\end{equation}
