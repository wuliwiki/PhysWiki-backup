...
\begin{itemize}
\item 可以用于同步和备份文件
\item 对于修改日期和大小相同的， 可以选择额外检查 hash 是否相同。
\item 可以选择复制后删除原来的文件
\item 可以选择删除目标文件夹中有而原文件夹中没有的文件吗？
\item \verb|–dry-run| 可以看看哪些文件会修改
\item \verb|-a| 是 archive
...
\subsection{rsync}
\begin{itemize}
\item 从远程传到本地： \verb|rsync -zvh 用户名@域名或IP:/路径/* 本地路径/| 其中 \verb|-v| 是 verbose， \verb|-h| 是 human readable， \verb|-z| 是在传输过程中压缩（通常只用于网络传输）
