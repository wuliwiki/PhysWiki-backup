...
\end{lstlisting}
\begin{itemize}
\item The @ symbol is used to denote global entities (like functions and global variables) in LLVM IR.
\item signext: This attribute indicates that the parameter should be sign-extended to the full 64-bit width when passed to the function. This is typically used in architectures where parameters might be passed in registers smaller than 64 bits.
\item \%"my_var": This is the name of the parameter. The \% symbol is used to denote local entities (like parameters and local variables) in LLVM IR. The name "my_var" is a descriptive name that indicates the parameter is a 64-bit integer named abc.
\item #0: This is an optional function attribute group. The #0 refers to a set of attributes that are defined elsewhere in the module. These attributes can control various aspects of the function's behavior, such as optimization hints or calling conventions.

top::

top: is a label. In LLVM IR, labels are used to mark specific points in the code, often used as targets for branches or jumps. The label top typically marks the entry point of the function, though the name top is arbitrary and could be anything.
