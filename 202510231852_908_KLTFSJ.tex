...
取$A = B = \varnothing$。则$\varnothing \cup c(\varnothing) \cup c(c(\varnothing)) = c(\varnothing) \setminus c(\varnothing) = \varnothing$即$c(\varnothing) \cup c(c(\varnothing)) = \varnothing$这立即推出\textbf{[K1]}。

* 取任意$A \subseteq X$，令$B = \varnothing$。应用\textbf{[K1]}，得$
  A \cup c(A) = c(A)$推出\textbf{[K2]}。

* 取$A = \varnothing$，任意$B \subseteq X$。应用\textbf{[K1]}，得
$c(c(B)) = c(B)$即\textbf{[K3]}。

* 取任意 (A, B \subseteq X)，结合 [K1]–[K3]，可推得 **[K4]**。

---

另一种替代方案由 **Monteiro（1945）** 提出，他提出了一个更弱的公理，仅能推出 [K2]–[K4]：[9]
...
条件 [K1] 与 [M] **相互独立**。确实，如果 (X \neq \varnothing)，定义算子
[
\mathbf{c}^\star : \wp(X) \to \wp(X)
]
为常值映射
[
A \mapsto \mathbf{c}^\star(A) := X
]
则该算子满足 [M]，但**不保持空集**，因为
[
\mathbf{c}^\star(\varnothing) = X
]
由定义可见，任何满足 [M] 的算子都是一个 **Moore 闭包算子**。

---

**M. O. Botelho 与 M. H. Teixeira** 提出了一个更对称的替代形式 [BT]，它同样蕴含公理 [K2]–[K4]：[2]
...
即：
[
A \cup B \cup c(c(A)) \cup c(c(B)) = c(A \cup B)
]
