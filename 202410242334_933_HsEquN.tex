...
当且仅当$a=b$时取等。
\end{theorem}
证明过程是显然的，将$(a-b)^2\geq0$打开并移项就可以得到，或后的写法也只是将$a,b$替换成了$\sqrt{a},\sqrt{b}$。但他背后的意义是深远的，一般称$\displaystyle a+b\over2$为\textbf{算术平均数（arithmetic mean，AM）}，也就是通常生活中提到平均值时所指的数，称$\sqrt{ab}$为\textbf{几何平均数（geometric mean，GM）}。几何平均数通常在计算增长率时使用，比如投资某个项目，第一年的收益率是$4\%$，第二年是$10\%$，则两年的平均收益率并非$\displaystyle {4\%+10\%\over2}=7\%$，而是$\sqrt{1.04\times1.10}-1=6.96\%$。根据基本不等式，如果直接计算算术平均数，则总会高估实际的收益率。

事实上，常见的均值有四种，对于一列数$a_1,a_2,\cdots,a_n$，称：
\begin{itemize}
\item $\displaystyle H = \frac{n}{\frac{1}{a_1} + \frac{1}{a_2} + \cdots + \frac{1}{a_n}}$为\textbf{调和平均数（Harmonic mean）}，其实
\item $\displaystyle A = \frac{a_1 + a_2 + \cdots + a_n}{n}$为\textbf{算术平均数（Arithmetic mean）}
\item $\displaystyle G = \sqrt[n]{a_1 \cdot a_2 \cdot \cdots \cdot a_n}$为\textbf{几何平均数（Geometric mean）}
\item $\displaystyle Q = \sqrt{\frac{a_1^2 + a_2^2 + \cdots + a_n^2}{n}}$为\textbf{平方平均数（quadratic mean）}，又称\textbf{均方根（root mean square，RMS）}
\end{itemize}
...
\textbf{平方平均数（quadratic mean）}，又称\textbf{均方根（root mean square，RMS）}
