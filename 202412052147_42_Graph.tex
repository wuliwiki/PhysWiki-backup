...
\begin{definition}{不交，子图，母图}
设 $G=(V,E),G'=(V',E')$ 是两个图。若 $G\cap G'=\varnothing:=(\varnothing,\varnothing)$， 则称 $G$ 和 $G'$ 是\textbf{不交的}（disjoint）；若 $G\cap G'=G$，则称 $G$ 是 $G'$ 的\textbf{子图}，而 $G'$ 称为 $G$ 的\textbf{母图}（s）。
\end{definition}
