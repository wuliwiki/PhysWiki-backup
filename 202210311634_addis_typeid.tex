...
\item 用 \verb|g++ -std=11| 测试， 如果 \verb|A| 中没有 \verb|virtual| 函数（即不存在多态）， 那么返回 \verb|false|．\item 如果 \verb|A| 不是抽象类但有 \verb|virtual|（存在多态）则返回 \verb|true|．
...
\item 所以（我猜） \verb|typeof| 一般来说和是静态编译的， 但如果
\end{itemize}
