...
\end{equation}
其时间导数恒为零，根据（\autoref{eq_AdiaQM_10}） $\mat A$ 恒为零。 所以绝热近似（\autoref{eq_AdiaQM_2} ）精确成立（即使开始时存在简并！）。把能量代入\autoref{eq_AdiaQM_8} 得
\begin{equation}
...
\end{equation}
那么绝热近似就是精确的。 若 $\mat H^1$ 的本征值和本征矢为
\begin{equation}
E_n^1~, \quad \bvec\psi_n \qquad (n=1,\dots,N)~,
\end{equation}
那么分裂后能级为
\begin{equation}
E_n(t) = E_0 + \alpha E_n^1 t~.
\end{equation}
\begin{equation}
\theta_n(t) = -\qty(E_0 t + \frac12 \alpha E_n^1 t^2)~.
\end{equation}
通解为
\begin{equation}
