...
\item 详细代码示例如下． 另外在所有 so 库中都定义了 \verb|conflict()|． 注意 \verb|main.x| 如果直接依赖 \verb|libbase.so.x|， 那么即使间接依赖也无法调用其中的函数． 所以使用的 \verb|conflict()| 会是最先链接的 \verb|libtest| 中的． 如果 main.x 直接依赖所有四个库， 那么编译的时候会警告可能发生冲突， 运行时会调用最先链接的库中的 \verb|conflict()|．
\end{itemize}

\begin{lstlisting}[language=bash, caption=make.sh]
export LD_LIBRARY_PATH=$PWD
g++ -shared -fPIC -o libbase.so.1 libbase.1.cpp
g++ -shared -fPIC -o libbase.so.2 libbase.2.cpp
g++ -shared -fPIC -o libtest1.so libtest1.cpp \
    -L . -l:libbase.so.1
g++ -shared -fPIC -o libtest2.so libtest2.cpp \
    -L . -l:libbase.so.2
g++ -o main.x main.cpp -L . -l:libbase.so.2 \
    -l:libbase.so.1 -l:libtest1.so -l:libtest2.so
\end{lstlisting}
