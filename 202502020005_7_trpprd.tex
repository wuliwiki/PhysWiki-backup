...
从几何上也能理解为什么向量三重积不具有结合性。如\autoref{fig_trpprd_2} 所示，$\color{Red}\vec{b}\color{Black}\times\color{NavyBlue}\vec{c}\color{Black}$是垂直于$\color{NavyBlue}\vec{a}\color{Black}, \color{Red}\vec{b}\color{Black}$平面的向量，它跟$\color{OliveGreen}\vec{a}\color{Black}$做叉乘后又垂直于$\color{Red}\vec{b}\color{Black}\times\color{NavyBlue}\vec{c}\color{Black}$自身了，因此最终结果还是在$\color{NavyBlue}\vec{b}\color{Black}, \color{Red}\vec{b}\color{Black}$平面上。
