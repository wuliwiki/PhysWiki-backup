...
给定一个直角三角形ABC，其中AC是斜边，构造一个以AC为直径的圆Ω。设O为Ω的圆心。设D为Ω与射线OB的交点。根据塔勒斯定理，∠ADC是直角。但此时D必须等于B。（如果D位于△ABC内部，∠ADC将是钝角；如果D位于△ABC外部，∠ADC将是锐角。）
...
\end{enumerate}
设\(\angle ABC\)为直角，M为以AC为直径的圆。为了简化计算，设M的圆心位于原点。那么我们知道：
\begin{itemize}
\item \(A = -C\)，因为以原点为圆心的圆的直径是AC，并且  
\item \((A - B) \cdot (B - C) = 0\)，因为∠ABC是直角。
\end{itemize}