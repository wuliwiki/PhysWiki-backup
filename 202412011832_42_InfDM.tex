...
当限制 $u(x_i)$ 对 $x_i$ 具有周期 $2L$ 时，即 $u(x_i+2L)=u(x_i)$，那么为了 $u$ 的单值性，限定 $x_i\in[-L,L]$。设 $x_i$ 有 $N$ 个，于是 $x\in \{h,\cdots,Nh\}$，其中 $h:=\frac{2L}{N}$。因而当 $k_1-k_2=\frac{2\pi}{h}=\frac{N\pi}{L}$ 时，$e^{\I k_1x}=e^{\I k_2x}$，从而可将 $k$ 限制在 $[-\frac{N\pi}{2L},\frac{N\pi}{2L}]$ 上。
