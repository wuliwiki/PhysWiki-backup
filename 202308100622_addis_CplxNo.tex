...
特殊地， 我们把复数 $(0, 1)$ 称为\textbf{虚数单位}， 用 $\I$ 表示\footnote{为了与变量 $i$ 区分， 小时百科中虚数单位使用正体的 $\I$。}。 最后我们把虚部为零的复数 $(x, 0)$ 等同于实数 $x$。 把所有复数的集合\upref{Set}记为 $\mathbb C$， 那么全体实数的集合 $\mathbb R$ 就是它的一个真子集\upref{Set}， 即 $\mathbb R \subset \mathbb C$。
\end{definition}
