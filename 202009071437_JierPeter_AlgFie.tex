...
\item 同样由于反对称性，若干元素$\bvec{v}_i$中只要有两个相等，它们的外积就为零．这就意味着我们最多可以取到$A^n(V)$，而对于$m>n$，都有$A^m(V)=\{0\}$．这样，将各$A^k(V)$视为互相不同的线性空间，其中只有$n+1$个非平凡，它们作为向量空间的直和$\bigoplus\limits_{k=1}^n A^k(V)$也是一个向量空间，记为$A(V)$．由前面的步骤可知，任取
\end{itemize}
