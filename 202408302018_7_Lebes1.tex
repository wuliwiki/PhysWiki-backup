...
如果柱子的高度都选成最高点（函数值上确界），那么柱子总面积一定大于等于真正的积分值；柱子的高度都选成最低点（函数值下确界），那么柱子总面积一定小于等于真正的积分值。当柱子划分得越来越细，这两种选择高度的策略所计算出来的面积差有可能会趋于$0$，此时我们就根据两边夹的原则，认为两种计算结果的共同极限就是真正的积分值；但对于一些奇怪的函数，
