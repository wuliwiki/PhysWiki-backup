...
从三角形的一个顶点出发，向其对边所在直线作的垂线称为三角形的\textbf{高（altitude）}。垂足可能在边上、与定点重合或在边的延长线上，所以高可能位于三角形内部、与边重合或在三角形外部。高所对的边称作这个高所对的底。

由于高中阶段研究的都是欧几里得几何，根据前面提及的平行公设的等价的表达方式，任意三角形的内角和等于 $180^\circ$。因此，三角形的内角不会大于$180^\circ$，同时至多有一个内角大于等于$90^\circ$。按角的大小或边的长度的特殊情况，有下面几种三角形：
\begin{itemize}
...
三角形有五个重要的特殊点，称为“五心”：
\begin{itemize}
\item \textbf{重心（centroid）}：三条\textbf{中线}（连接顶点与对边中点的线段）交于一点，重心将每条中线分为 $2:1$ 的比例（靠近顶点的一段较长）。它也是三角形的\textbf{质心}，即平衡点。
\item \textbf{垂心（orthocenter）}：三条\textbf{高}的交点。垂心位置取决于三角形类型，锐角三角形的垂心在内部；直角三角形的垂心是直角顶点；钝角三角形的垂心在外部。
\item \textbf{内心（incenter）}：三条\textbf{角平分线}的交点，是三角形的\textbf{内切圆}的圆心，且到三边的距离相等。
\item 三条边的垂直平分线交yu，是三角形\textbf{外接圆}的圆心，且到三个顶点的距离相等。
\item \textbf{旁心（excenter）}：\textbf{旁切圆}（外接某一边及其余两边延长线的圆）的圆心。一个三角形有三个旁心，每个旁心对应一个旁切圆。
\end{itemize}
