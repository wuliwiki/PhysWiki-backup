...
\subsection{四}
解：
偶极跃迁，$\mathbf{H'} \propto X,Y, \text{或} Z$，$\mathbf{H'}$ 偏振方向固定。
...
\\[3zY_{lm}(\\theta, \\varphi) = b_1 Y_{l+1,m\\pm1} + b_2 Y_{l-1,m\\mp1}\\]

\\text{可得极化矩阵元} $H'_{n\\epsilon m, n'\\epsilon'm'} \\neq 0$ \\text{的条件：}

\\[\\Delta l = \\pm1, \\Delta m = 0, \\pm1\\]

\\text{径向积分不构成选择定则，……无} $\\Delta n$ \\text{及} $n$ \\text{的规则}。

\\text{又由自旋李矩阵的正交归一性：} \\alpha = \\beta = 0, \\alpha' = 1 \\text{和} \\beta' = 1,
