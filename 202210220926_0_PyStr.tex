...
\verb|"一些字符 {0}, 一些字符 {1}".format(变量0, 变量1)| 可以把字符串中的 \verb|{数字}| 替换为对应的变量．

\verb|a = 123; b = 1.3e-14; f"blablabla {a}, {b}"| 可以直接把变量展开到字符串中．

类似 c 语言一样的字符串格式化 \verb|"a = %d\n , b = %s\n" % (a, b)|

\verb|ord('a')| 把字符转换为整数（utf 编码）， \verb|chr(97)| 把整数转换为字符．
