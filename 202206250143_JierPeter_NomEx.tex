...
\end{equation}
则由于已设定$\varphi_1$是恒等映射，可知$f(\alpha)=0$；由于各$\varphi_i\alpha$互不相同，可知$f$是可分多项式\footnote{注意，这里没法证明$f=\opn{irr}(\alpha, \mathbb{F})$，因为无法保证$f\in\mathbb{F}[x]$．最多只能确定$\opn{irr}(\alpha, \mathbb{F})=f^k$，其中$k$是正整数．}．

任取$\overline{\mathbb{F}}$的保$\mathbb{F}$自同构$\sigma$，则$\sigma\mid_{\{\varphi_i\alpha\}}$是一个$\{\varphi_i\alpha\}$上的置换．于是
