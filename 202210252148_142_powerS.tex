...
\end{equation}
的函数叫做\textbf{幂级数（power series）}． 其中 $f(0) = c_0 (x - x_0)^0$ 始终视为 $c_0$， 即使取 $x = x_0$．
...
\end{equation}
的简写． 对给定的 $x$， 当极限存在时， 我们就说级数\textbf{收敛（converge）}， 反之就说级数\textbf{不收敛}或\textbf{发散（diverge）}． 广义来说， 幂级数中的 $c_n, x, x_0$ 可以复数\upref{CplxNo}．
