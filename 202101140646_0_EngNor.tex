...
本文使用原子单位制\upref{AU}． 以平面波 $\exp(\I kx)$ 为例， 在动量归一化% 链接未完成
中， 我们把波函数（包括归一化系数）看成是以动量（即波数） $k$ 为参数的， 满足
\begin{equation}\label{EngNor_eq1}
\int \psi_{k'}(x)^* \psi_{k}(x) \dd{x} = \delta(k' - k) \qquad (k \in \mathbb R)
\end{equation}
其中 $*$ 表示复共轭，
\begin{equation}
...
有时候我们也希望用能量 $E = k^2/(2m)$ 作为平面波的参数， 即把任意波函数分解为
\begin{equation}
