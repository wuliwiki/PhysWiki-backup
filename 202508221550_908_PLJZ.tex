...
$$
\psi_{+} =
\frac{1}{\sqrt{2|\vec{a}|(a_{3}+|\vec{a}|)}}
\begin{bmatrix}
a_{3} + |\vec{a}| \\ 
a_{1} + i a_{2}
\end{bmatrix},
...
\end{bmatrix}.~
$$
当 $a_{3} \to -|\vec{a}|$ 时，这些表达式会出现奇异性。
为解决这个问题，可以令：

$$
\vec{a} = |\vec{a}|\bigl(\epsilon, 0, -(1-\epsilon^2/2)\bigr)
$$

并取极限 $\epsilon \to 0$，即可得到 $\sigma_z$ 的正确特征向量：
