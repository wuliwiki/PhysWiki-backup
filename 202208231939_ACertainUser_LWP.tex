...
那么，要如何确定$t=t_b$时的推迟时刻$t_r=t_b-R/c$呢？（很明显，根据推迟时刻的物理含义，此时有$t_r=t_a$，不过我们希望找到关于推迟时刻的更有意义的表达）麻烦的是，$R$（“场源与场点的相对距离”）到底指的是什么？在场源固定时，R与时间无关，可以被很清晰地描述；但在场源运动时，R应该指什么？事实上，$t=t_b$时刻，R应该指\textsl{场点所知道的场源}$\bvec r'(t=t_a)$到自己的距离： $\bvec R = \bvec r - \bvec r'(t=t_a)$．
那么，推迟时刻就是$t_r=t_b-\frac{\abs{\bvec r - \bvec r'(t=t_a)}}{c}$，或整理为$c(t_b-t_r)=\abs{\bvec r - \bvec r'(t=t_a)}$．

广而论之，任意时刻t时的推迟时刻就被隐性描述为$c(t-t_r)=\abs{\bvec r - \bvec r'(t=t_r)}$．
