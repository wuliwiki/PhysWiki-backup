...
\footnote{参考 Wikipedia \href{https://en.wikipedia.org/wiki/Runge\%E2\%80\%93Kutta_methods}{相关页面}。}\textbf{龙格库塔法（Runge-Kutta）}是一类数值解\enref{微分方程}{ODE}或方程组的算法， 其中较常见的是\textbf{四阶龙格库塔法}。 这里不进行推导， 仅仅给出算法。 如果你不想了解算法只想用 Matlab 自带的解算器直接求解方程，见“Matlab 解常微分方程组（ode45）\upref{MatODE}”。
