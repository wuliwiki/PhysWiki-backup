...
从定义上，可以从几何的视角直接看出 $\sin x$ 和 $\cos x$ 的单调性。在单位圆上，点 $P$ 的横坐标表示 $\cos x$，纵坐标表示 $\sin x$。当 $x$ 处于上半平面（$0 < x < \pi$）时，随着 $x$ 增加，$P$ 的横坐标从 $1$ 变为 $-1$，即 $\cos x$ 递减；而在下半平面（$\pi < x < 2\pi$）时，$P$ 的横坐标从 $-1$ 变为 $1$，即 $\cos x$ 递增。因此，根据定义可知：
	•	在 $0 + 2k\pi < x < \pi + 2k\pi$ 内，$\cos x$ 递减。
	•	在 $\pi + 2k\pi < x < 2\pi + 2k\pi$ 内，$\cos x$ 递增。

同理，$\sin x$ 和 $\tan x$ 的单调性也可以用类似方式观察。然而，这种分析方法虽然直观，但并不严谨，特别是在三角函数的代数计算中，直接通过做差来判断单调性较为复杂。

更有效的方法是使用导数。在此前已经介绍过三角函数的导数，接下来可以基于
