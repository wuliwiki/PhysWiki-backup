...
Git 还可以给仓库创造不同的\textbf{分支（branch）}，例如一个人使用 Git 管理自己的简历，他每写一部分就做一次 commit，这些 commit 按时间顺序连成一串就是一个分支（通常该分支默认的名字是 ma。 但有一天要根据不同类型的公司提交不同版本的简历。这些不同版本中大部分文字和图片都相同，但给不同公司的版本略有不同，且每个公司的版本。
