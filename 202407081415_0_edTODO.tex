...
\item \verb`lstlisting` 环境（见 sample）转换后开头为：\verb|```语言 标题|（占一行），结尾 \verb|```|（占一行）。 其中 \verb`标题` 可选。
\item \verb`\begin{equation}\begin{aligned} ... \end{aligned}\end{equation}` 环境变为 \verb`:::E ... :::` 注意两个相邻命令之间可能有空格或回车。
\item \verb`equation` 环境（如果不直接嵌套 \verb`aligned` 环境）变为 \verb`:::e ... :::`。
