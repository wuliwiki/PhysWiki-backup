...
\item 要下载 image， 用 \verb`docker pull 用户名/repo名:TAG` 其中 \verb|用户名/| 对于一些官方镜像可以忽略， 例如 \verb`docker pull ubuntu:22.04` 中， \verb|ubuntu| 是 repo， \verb|22.04| 是 TAG． \verb|docker pull hello-world| 同理．
\item 注意区分 container 和 image 和 repo． repo 相当于 VirtualBox 虚拟机， image 相当于 snapshot， 而 container 相当于现在在运行的虚拟机．
\item \verb|docker pull ubuntu:22.04| 中， \verb|ubuntu| 是一个
\item \verb`docker run -it -d IMAGE_NAME/IMAGE_ID` 从 image 创建 container． 反复使用会创建多个．
\item \verb`docker exec -it CONTAINER_ID bash` 进入某个 container 的 bash． 如果默认 user 不是 root， 那么在 \verb|-it| 前面加上 \verb|-u 0| 选项可以以 root 的身份 login．
...
\item \verb`docker commit CONTAINER_ID 用户名/REPO_NAME` 会将 container commit， 也可以在后面加 \verb`:TAG_NAME` 指定 tag
\item \verb`sudo docker login` 用于登录 docker hub
\item \verb`docker push 用户名/REPO_NAME` 可以直接将 commit 的 image push 到 docker hub， 也可以用 \verb`REPO_NAME:TAG_NAME` 指定 tag
