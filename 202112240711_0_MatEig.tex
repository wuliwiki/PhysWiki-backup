...
\subsection{对角化与相似变换}
求解矩阵的本征方程有时候也称为矩阵的\textbf{对角化}， 原因如下： 若我们把矩阵 $\mat A$ 的第 $i$ 个本征值和本征列矢量记为 $\lambda_i$ 和 $\bvec v_i$． 如果把本征值按顺序组成对角矩阵 $\mat \Lambda$， 把 $\bvec v_i$ 按顺序从左到右组成方阵 $\mat P$， 那么根据矩阵乘法\upref{Mat}的定义 $\mat A \mat P$ 相当于分别计算 $\mat A\bvec v_i$ 再从左到右排列． 而 $\mat P\mat\Lambda$ 相当于把 $\lambda_i\bvec v_i$ 从左到右排成方阵． 二者应该相等， 所以有
\begin{equation}
