...
其实，通过刚才的概念，你可以注意到，尽管条件命题研究的也是两个命题的关系，但是它和“且”、“或”不同，$\Rightarrow$符号的左右两个命题并不满足交换律，也就是$P$（前件）和$Q$（后件）不能随意互换。这使得二者地位并不“平等”，因此下面我们来研究一下二者的关系。
...
首先我们考虑 $P$ 对 $Q$ 的关系。显然，由 $P$ 可以推出 $Q$， 说明 $P$ 中有充分的信息能得到 $Q$， 所以叫做 $Q$ 的\textbf{充分条件}。 $P$ 中包括得到 $Q$ 所必要的信息，还\textbf{可能}包括一些其他信息，例如由命题 $P$ 可以得出四边形任意两条临边垂直。 这些多出来的信息并不一定是得到 $Q$ 所必须的，因为还有许多其他的四边形四条边相等但并不是正方形。
...
\begin{example}{}
命题 $P$ ：三角形 $X$ 的其中两内个角分别为 $90^\circ$ 和 $45^\circ$。

命题 $Q$ ：三角形 $X$ 有两个 $45^\circ$ 的内角。

利用三角形三个内角和为 $180^\circ$ 的事实，可以从 $P$ 推出 $Q$， 说明 $P$ 是 $Q$ 的充分条件， $Q$ 是 $P$ 的必要条件。但也可以从 $Q$ 推出 $P$， 说明 $Q$ 是 $P$ 的充分条件， $P$ 是 $Q$ 的必要条件。所以 $P$ 和 $Q$ 既是彼此的充分条件也是彼此的必要条件。所以我们说 $P$ 和 $Q$ \textbf{互为充分必要条件}。若 $P$ 是 $Q$ 的充分必要条件， $Q$ 一定也是 $P$ 的充分必要条件。因为两种表述都意味着 $P$，  $Q$ 命题\textbf{等效}，所提供的信息都是一样的，两者都没有任何多余的或者缺失的信息。
\end{example}
...
\item 在证明 $P$ 是 $Q$ 的充分必要条件时，需要分别证明 $P$ （相对于 $Q$）的充分性和必要性。充分性需要由 $P$ 证明 $Q$， 必要性需要由 $Q$ 证明 $P$。 
\item 在证明 $P$ 是 $Q$ 的充分非必要条件时，除了需要证明 $P$ 的充分性，还需非必要性，即 $Q$ 不能推出 $P$。 只要我们可以举出一个 $Q$ 成立 $P$ 不成立的反例，就立刻证明了不可能由 $Q$ 推出 $P$。 
\end{enumerate}
