...
理论上从定义可以直接看出sinx和cosx的单调性，当x在上半平面时，随着x增加点P的横坐标从1变到-1，在下半平面时，随着x增加点P的横坐标从-1变到1，从而根据定义可知，当0+2kpi<x<pi+2kpi时，cosx递减，当pi+2kpi<x<2pi+2kpi时，cosx递增。sinx和tanx也可以认为这样看出。当然，这是朴素的，尽管在幂函数等代数为基础的函数中可以做差进行考量，但三角函数做差进行讨论比较复杂。之前已经介绍过三角函数的导数，下面就基于\autoref{eq_HsDerB_4}\autoref{eq_HsDerB_5}
