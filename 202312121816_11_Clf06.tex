...
\end{theorem}
proof.
回顾伴随映射的定义。设上述线性映射$f$在这两组基下的矩阵为$F^a_b$，伴随映射$f^*$在这两组基下的矩阵为$G^a_b$。那么我们有：$G^a_b =q^{ja}p_{ib}F^i_j$。如果二次型是标准的，上式简化为$G^a_b=q(e_a)p(e_b)F^b_a$。因此求矩阵的伴随有两个过程：取负及转置。因此该定理可以表述为："对$F^a_b$生成外同态，取负，转置”等同于“对$F^a_b$取负，转置，生成外同态”。
