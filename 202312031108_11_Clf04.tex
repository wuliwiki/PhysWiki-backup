...
回忆二次型的表示方式，该双下标二次型表示为行矩阵的行矩阵。由上式得：$q(e_1,e_2)=0,q(e_1,e_1)=q(e_2,e_3)=1,q(e_2,e_2)=0$。$e_1$和$e_2$是一组正交基，但其张成的空间无法扩展为全空间的正交基。这是由子空间的退化性导致的。反设存在$v\in V$并与$e_x,e_y$构成标准正交基，且$v=ae_1+be_2+ce_3$，由正交性解得$a=b=0$，则该向量与$e_y$平行，所以假设不成立。