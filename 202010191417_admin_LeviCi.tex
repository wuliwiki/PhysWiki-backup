...
列维—奇维塔符号是一个函数， 记为 $\epsilon_{i_1, i_2, \dots, i_N}$， $N$ 叫做它的\textbf{维数（dimension）}． 它的自变量是 $N$ 个正整数 $i_1, \dots, i_N$， 可以称为角标． 每个角标从 $1, 2, \dots, N$ 中取值． 函数值只能取 $0, 1, -1$ 中的一个． 当 $i_1, \dots, i_N$ 中有任意两个重复时函数值为 0； 若没有重复， 函数值取 $1$ 还是 $-1$ 取决于角标的排列顺序．
