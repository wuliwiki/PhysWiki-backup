...
取一列单调递减趋近于零的正数$\{\epsilon_k\}$．由于$f_n\overset{m}\to f$，故对于每个$\epsilon_k$，总存在一个$N_k$使得只要$n>N_k$，就有$\opn{m}\qty(\{x\in E|\abs{f_n-f}<\epsilon_k\})<1/2^k$．以这些$N_k$为编号，构建子序列$f_{N_k}$．
