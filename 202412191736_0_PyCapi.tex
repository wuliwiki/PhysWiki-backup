...
\item \verb`PyObject* PyObject_GetAttrString(PyObject *obj, const char *attr_name);` 获取对象的属性（attribute）。 模块也是对象，如 \verb`math.pi` 就是模块 \verb`math` 对象的 \verb`pi` 属性。 它也可以用于获取函数如 \verb`math.sin`。 如果失败，返回 \verb`NULL`， 可以用 \verb`PyErr_Print()` 打印错误信息。也可以用于获取对象的成员函数指针（绑定该对象）
\end{itemize}
