...
\end{equation}
利用黎曼度量的正定性，易见$\widetilde g$是单射，由定义可知这又是满射，所以该丛映射实际上是丛同构。因此，利用黎曼度量，我们可以将切场（余切场）对偶到余切场（切场）。下面我们来看对偶后的坐标表示。

由定义可知$g(\partial_i,\partial_j)=g_{ij}(\partial_i\equiv\pdv{x^i})$，则
\begin{equation}
g(X,Y)=g_{ij}X^iY^j=\widetilde g(X)Y^j\partial_j~,
\end{equation}
所以
\begin{equation}
\widetilde g(X)=g_{ij}X^idy^j~.
\end{equation}
