...
\end{lstlisting}
第一位 0 说明是正数， 1 是复数． 接下来 11 bit 先看成 unsigned， 然后再减 $2^{10}-1 = 1023$， 这里指数 \verb|10000000010| 表示 3， 于是指数项为 $2^{3} = 8$ （指数无论正负都要加 1， 也比较奇怪）． 小数项应该是 $10/8 = 1+1/4$． 第 13 bit 表示 1/2， 14 bit 表示 1/4， 所以第 14 bit 是 1， 后面的小数全是零． 另外， 由于 intel 的 x86 和 x86-64 都使用 little endian， 所以在内存中实际上 byte 的顺序是相反的．
