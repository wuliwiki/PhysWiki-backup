...
\subsection{另一种推导}
类比\autoref{Vtrans_eq1}\upref{Vtrans}， 若 $S'$ 系相对 $S$ 系以角速度 $\bvec\omega$ 旋转， 对任意一个随时间变化的矢量（假设一阶导数存在）， 我们把它在 $S$ 和 $S'$ 系中的时间导数分别记为 $(\dot{\bvec A})_{S}$ 和 $(\dot{\bvec A})_{S'}$， 则有
...
\bvec a_{S} = (\dot{\bvec v}_{S'})_{S} + \bvec\omega\cross\bvec v_{S} + \dot{\bvec\omega} \cross\bvec v_{S}
\end{equation}
...
\begin{equation}
\bvec a_{S} = \bvec a_{S'} + 2\bvec\omega\cross\bvec v_{S'} + \bvec\omega\cross(\bvec\omega\cross\bvec r) + \dot{\bvec\omega} \cross\bvec v_{S}
\end{equation}
所以旋转参考系中的总惯性力（\autoref{Iner_eq1}\upref{Iner}）为
\begin{equation}
\bvec f = m(\bvec a_{S'} - \bvec a_{S}) = 2m\bvec v_{S'}\cross\bvec\omega - m\bvec\omega\cross(\bvec\omega\cross\bvec r)
\end{equation}
