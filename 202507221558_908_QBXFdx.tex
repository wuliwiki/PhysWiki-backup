...
根据**等振定理（Equioscillation Theorem）**，在所有次数不超过 $n$ 的多项式中，若某个多项式 $f$ 在区间 $[−1, 1]$ 上的无穷范数 $\|f\|_\infty$ 取得最小值，当且仅当存在 $n + 2$ 个点 $-1 \leq x_0 < x_1 < \cdots < x_{n+1} \leq 1$，使得：
...
然而，在上面的讨论中，$|f|$ 仅在 $n + 1$ 个点上达到最大值，这是因为我们寻找的是\*\*次数正好为 $n$（且 $n \geq 1$）\*\*的最佳多项式。因此，前述等振定理在此情形下并不适用。
