...
对每个子空间 $M\subseteq X$， 我们定义对应的\textbf{（正交）投影算符} $P$ 根据\autoref{projOp_eq1} 将每个 $u\in X$ 映射到 $v\in M$：
\begin{theorem}{}
令投影算符 $P$ 将 $N$ 维空间 $X$ 中的矢量（正交）投影到其子空间 $M$ 中， 若 $\qty{\mu_i}$ 是 $M$ 的一组\textbf{正交归一基底}． 那么投影算符可以用狄拉克符号（\autoref{braket_eq1}~\upref{braket}）表示为
\begin{equation}
