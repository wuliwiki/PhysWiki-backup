...
若我们把 $S'$ 相对于 $S$ 的运动分解为原点的平移加绕原点的转动， 那么牵连加速度 $\bvec a_r$ 也可以分解为平移加速度和旋转加速度的矢量和， 而旋转加速度又可以分为向心加速度（\autoref{CMAD_eq9}~\upref{CMAD}）和角加速度引起的切向加速度．
\addTODO{1. 链接到变速圆周运动的例题， 2. 该式需要验证}
\begin{equation}\label{AccTra_eq5}
\bvec a_{r} = \bvec a_T + \bvec\omega\cross(\bvec\omega\cross\bvec r_S) + \dot{\bvec\omega} \cross\bvec r_S
\end{equation}
