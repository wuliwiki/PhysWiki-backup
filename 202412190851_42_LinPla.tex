...
其中 
\begin{aligned}
x_B=(x_1,\cdots,x_m)^T,x_N=(x_{m+1},\cdots,x_n)^T,\lambda=(\lambda_1,\cdots,\lambda_{n-m})^T,c=(c_1,\cdots,c_n)^T.
\end{aligned}
...
\textbf{必要性：}若 $\lambda_i<0$，则一定存在可行解 $x$，使得 $x$ 中仅 $x_i$ 分量不为0，而其它分量为0，并且 $x_i<0$。事实上，此时 $x_B=c-Cx_N=(c_1-C_{1i}x_i,\cdots,c_m-C_{mi}x_i)^T$，只要 $x_i$ 足够的靠近0，就能使 $c_j-C_{ji}x_i$
