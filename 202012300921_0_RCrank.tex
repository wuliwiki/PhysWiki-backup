...
我们从矢量空间的角度\upref{MatLS} 证明， 假设矩阵 $\mat A$ 的尺寸为 $N_Y \times N_X$， 对应的线性算符为 $A: X\to Y$． $X, Y$ 空间的维数分别为 $N_x, N_y$． $\mat A$ 的厄米共轭\upref{HerMat}记为 $\mat A\Her$， 对应算符记为 $A\Her$．

我们前面讲过 $X$ 空间中满足 $\mat A \bvec x = \bvec 0$ 的所有 $\bvec x$ 构成的空间为 $\mat A$ 的零空间（链接未完成）， 记为 $X_0$． $Y$ 空间中满足 $A\Her |y\rangle = 0$ 的子空间为 $Y$ 的零空间， 记为 $Y_0$． 定义整个 $X$ 空间通过 $A$ 映射后得到 $Y$ 的另一个子空间为 $Y_1$ 空间， 即 $A(X) = Y_1$， 同理定义 $A\Her(Y) = X_1$． % 未完成： 矩阵与矢量空间中添加空间的映射及对应符号
