...
不妨设$f$的各不可约因子互不相同．我们主要用数学归纳法和对单扩张情况的讨论来证明．

当$\opn{deg}f=1$时，定理显然成立．下设定理对于任意$\opn{deg}f<n$的情况成立．

设$\sigma:\mathbb{K}\to\mathbb{K}$是保$\mathbb{F}$自同构，$h$是$f$在$\mathbb{F}$上的一个不可约因子．
...
对于任意$g\in\mathbb{F}[x]$和任意$\kappa\in\mathbb{K}$，必有$\sigma(g(\kappa))=g(\sigma(\kappa))$．因此，$h$的根必须被$\sigma$映射为$h$的根．或者说，$\sigma$是$\{\alpha_i\}$上的一个置换．由\autoref{FldExp_the4}~\upref{FldExp}第2条，遍历所有$\sigma$，则$\sigma(\alpha_1)$遍历所有$\alpha_i$．

考虑单扩张$\mathbb{F}(\alpha_1)$的保$\mathbb{F}$自同构，则这种自同构的数量等于$\mathbb{F}(\alpha_1)$中所包含的$h$的根的数量．因此，如果$\mathbb{F}(\alpha_1)$包含所有$\alpha_i$，则自同构数量等于所有根的数量，
