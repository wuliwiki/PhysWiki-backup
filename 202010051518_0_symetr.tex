...
\subsection{子空间的维度}
先来看有限维的情况， 假设单个粒子态所在空间是 $M$ 维的（例如只考虑自旋空间， $M = 2s+1$）， 那么只有当粒子数 $N \le M$ 时， 才能允许全同费米子的态矢． 通过基底的不同排列以及对称化， 我们一共可以找到 $C_M^N$ 种不同的反对称基底， 所以反对称子空间是 $C_M^N$ 维的． 要得到这 $C_M^N$ 个基底， 我们只需要从 $M$ 个单粒子基底中无顺序地选出 $N$ 个（$\ket{i_1}, \dots, \ket{i_N}$）然后计算 $\mathcal A \ket{i_1}\dots\ket{i_N}$ 即可．

要解决这个问题， 我们不妨从物理上着手． 将所有 $M^N$ 种不同排列的 $\ket{i_1}\dots\ket{i_N}$ 作为张量积空间的基底， 我们通过组合\upref{combin}把他们划分为 $C_M^N$ 组， 在每一组基地构成的子空间中， 我们都能找到唯一反对称的单位矢量．
