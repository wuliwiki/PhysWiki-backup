...
\item \verb`cmake -L CMakeLists.txt的路径` 列出编译选项， \verb`-LH` 列出选项以及帮助说明。 \verb`-LAH` 列出所有选项（会多出很多 \verb`CMAKE_` 开头的， 不是作者提供的选项）， 用 \verb`cmake -D 选项1=值1 -D 选项2=值2 CMakeLists.txt的路径` 来设置选项（\verb`-D` 后面可以没有空格）。 ru'guo
