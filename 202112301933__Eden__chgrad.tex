...
\begin{equation}
\bvec E=\frac{q}{R(t')^2}\frac{(1-v'^2)(\bvec{\hat n}-\bvec v')}{(1-\bvec v'\cdot \bvec{\hat n})^3}
\end{equation}
注意上式中 $R(t'),\bvec{\hat n}$ 是在 $t'$ 时刻的物理量，而 $t'$ 时刻满足 $t-t'=|\bvec r-\bvec r'|$．现在我们希望用 $t$ 时刻的物理量来表示电场，要将这些物理量进行变换．

设 $R(t_1)$ 为 $t_1$ 时刻电荷与 $\bvec r$ 的距离．$R(t')=|\bvec r-\bvec r'|=t-t'$．而在 $t$ 时刻，电荷所在的位置是 $\bvec r'(t)=\bvec r'+(t-t')\bvec v'$，那么 $R(t)$ 为
\begin{equation}
\begin{aligned}
R(t)&=|\bvec r-\bvec r'-(t-t')\bvec v'|\\
R^2(t)&=R^2(t')+(t-t')^2v'^2-2\bvec R(t')\cdot (t-t')\bvec v'\\
&=R^2(t')(1+v'^2)-2R(t')\bvec v\cdot \bvec R(t')\\
&=2R^2(t')(1-\bvec v'\cdot \bvec {\hat n})-R^2(t')(1-v'^2)
\end{aligned}
\end{equation}
\end{example}