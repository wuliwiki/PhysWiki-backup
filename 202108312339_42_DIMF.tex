...
A'=kA_1'\cdots A_n'
\end{equation}
其中， $A,A_1,\cdots,A_n$ 和 $A',A_1',\cdots ,A_n'$ 分别是在 $\mathscr{Z}$ 和 $\mathscr{Z'}$ 中用基本单位测得的数．
\end{theorem}
\textbf{证明：}假设定理不成立，则由\autoref{DIMF_the2} ，可将 $A_1,\cdots,A_n$ 用基本量类对应的数 $J_1,\cdots,J_l$ 表示之，只需把 $A_1,\cdots,A_n$ 各自的终定方程（配以所依托的现象类）带入\autoref{DIMF_eq9} ，