...
\autoref{HelmTh_eq1} 和\autoref{HelmTh_eq2} 可以分别类比静电学中的库伦定律（\autoref{Efield_eq9}~\upref{Efield}）和静磁学中的比奥萨法尔定律（\autoref{BioSav_eq3}~\upref{BioSav}）： 把 $\bvec F_c$ 看作静电场， $\rho(\bvec r)$ 看作电荷密度， 满足高斯定律（\autoref{EGauss_eq1}~\upref{EGauss}）； 把 $\bvec F_c$ 看作静磁场， $\bvec j(\bvec r)$ 看作电流密度， 满足安培环路定理（\autoref{AmpLaw_eq2}~\upref{AmpLaw}）． 也可以把 $\bvec j(\bvec r)$ 看成磁场 $\bvec B$
