...
\begin{issues}
\issueOther{卢瑟福散射的模拟需要支持不同的入射距离， 初速度和方向相同， 生成动画}
\end{issues}
...
\begin{figure}[ht]
\centering
\includegraphics[width=8cm]{./figures/KepNum_3.png}
\caption{卢瑟福散射\upref{RuthSc}产生的双曲线轨道} \label{KepNum_fig3}
\end{figure}
...
% === 参数设置 ===
k = 1; % -GMm, (m=1)
R = 5; alpha = 0; % 初始位置（极坐标）
v0 = 1.6; % 初速度（支持行矢量或标量）
beta = -pi/2 + linspace(pi/50, pi/20, 6); % 发射仰角（支持行矢量或标量）
t_max = 6.7; Nt = 100; % 模拟时间和步数
axis_param = [-5,5,-5,5]; % 坐标范围 [xmin,xmax,ymin,ymax] 或 'auto'
% ==============
\end{lstlisting}
