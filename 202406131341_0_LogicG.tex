...
\item \textbf{有界律（identity laws）} $A+0=A$，$A\cdot 1=A$（这就是为什么要用加号和乘号，很多性质都很相似）
\item \textbf{德摩根律（De Morgan's laws）} $\overline{A+B} = \overline A \cdot \overline B$； $\overline{A \cdot B} = \overline A + \overline B$
\item 易证德摩根律可以拓展到多个变量如 $\overline{A+B+C} = \overline A \cdot \overline B \cdot \overline C$， $\overline{A \cdot B\cdot C} = \overline A + \overline B + \overline C$
