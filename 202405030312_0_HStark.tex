...
\addTODO{这不太可行，如果把 $n_\text{max}$ 增加到 5-15，就会得到奇怪的结果（尤其是直接计算 $\mel{\psi_n}{H^1/\mathcal{E}}{\psi_n}$，\autoref{fig_HStark_2} 也会出现奇怪的 avoided crossing。 这是因为，波函数被电场扭曲以后并不完全能够用束缚态展开，而是同时\textbf{需要许多连续态}！从数值方法上，我们可以规定给一个 $r_\text{max}$ 把氢原子束缚到一个盒子中，用离散的 $E>0$ 盒子束缚态来近似连续态。}
