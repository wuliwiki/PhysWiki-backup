...
\end{equation}
其中 $N$ 称为\textbf{时移函数}（\textbf{lapse function}），$N^i$ 称为\textbf{位移函数}（\textbf{shift function}）．其几何解释为：$N\dd t$ 是相邻两超曲面的间的固有时，$N^i\dd t$ 是上超曲面 $X^\mu(x^i,t+\dd t)$ 上空间坐标为 $x^i$ 的点，到上超曲面对应于下超曲面 $X^\mu(x^i,t)$ 上空间坐标为 $x^i$ 的点在法矢量方向上的对应点的距离．用图表示，就是
