...
y=a_0x^n+\sum\limits^\infty_{k=2}\frac{(-1)^ka_0}{2^{2k}k!(n+1)(n+2)\cdots(n+k)}x^{n+2k}
\end{equation}
...
Gamma函数有两个性质：对正整数$n$, $\Gamma(n+1)=n!$；$\Gamma(s+1)=s\Gamma(s)$．
 ...
 中的解\autoref{ODE2P_eq22} 仍有一个待定常数$a_0$．如果令$a_0=\frac{1}{2^n\Gamma(n+1)}$，那么我们就能得到：
\begin{equation}
\begin{aligned}
y&=\sum\limits^\infty_{k=0}\frac{(-1)^k}{k!(n+1)(n+2)\cdots(n+k)\Gamma(n+1)}\qty(\frac{x}{2})^{n+2k}\\
