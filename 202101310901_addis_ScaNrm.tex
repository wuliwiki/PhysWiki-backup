...
和上文类似， 我们可以保证 $i = i'$ \autoref{ScaNrm_eq1} 成立． 但由于我们缺失了函数的奇偶性， 一般无法保证 $i \ne i'$ 时也成立． 这时我们可以使用一个类似于施密特正交化\upref{SmdtOt}的操作： 令正交化以前的两个满足\autoref{ScaNrm_eq4} 但不正交的线性无关解为 $\psi_{k,1}, \psi'_{k,2}$， 其中 $\psi_{k,1}$ 已经归一化， 那么与 $\psi_{k,1}$ 正交的 $\psi_{k,2}$ 可以由下式得到
\begin{equation}
...
&= \frac{\pi}{2} \qty[A_{-,1}A_{-,2}\cos(\phi_{-,1}-\phi_{-,2}) + \frac{\pi}{2}A_{+,1}A_{+,2}\cos(\phi_{+,1}-\phi_{+,2})]
\end{aligned}
\end{equation}
