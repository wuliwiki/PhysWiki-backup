...
注意$\alpha$的任意性．因此$f$的最小多项式的根全都在$\mathbb{F}$中，故$\mathbb{F}/\opn{Fix}_\mathbb{F}(G)$是正规扩张．由于各$\alpha_i$不相等，故$\alpha$是$\opn{Fix}_\mathbb{F}(G)$的可分元，故$\mathbb{F}/\opn{Fix}_\mathbb{F}(G)$是可分扩张．由此得证$\mathbb{F}/\opn{Fix}_\mathbb{F}(G)$是Galois扩张．

由不变子域的定义，显然$G\subseteq\opn{Gal}(\mathbb{F}/\opn{Fix}_\mathbb{F}(G))$．

由于任意元素$\alpha\in\mathbb{F}$的轨道中元素数量不会超过$\abs{G}$，$f_\alpha$的次数就是$\alpha$轨道中的元素数量，以及$f_\alpha$是$\alpha$的零化多项式，可知\textbf{任意}$\alpha$的最小多项式次数不会超过$\abs{G}$．因此，应用\autoref{PrmtEl_cor3}~\upref{PrmtEl}，可知$[\mathbb{F}:\opn{Fix}_\mathbb{F}(G)]$
