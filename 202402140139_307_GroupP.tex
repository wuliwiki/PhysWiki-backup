...
对于$n$种旋转的情况(我们把保持不变的操作算作旋转$0$次，当然也可以看成旋转$n$次，即$r=0$和$r=n$表示同一种操作)。易见，当旋转一次时，当且仅当所有珍珠为同一颜色时，旋转后的颜色才会和旋转前重合，一共可能的颜色有$k$种，对应着$k$个不动点。对于一般情况$d^r$，即旋转$r$次，$1\le r\le n$，令$m=\gcd(n,r)$。那么有$k^m$种可能随意选择编号为$1$到$m$颗珍珠的颜色，之后需要按照相同的样式，重复$n/m$次直到刚好填满全部珍珠。于是我们得到旋转$r$次时，不动点的个数为：$k^{\gcd(n,r)}$
