...
\item \verb`\aref{链接文字}{标签}` 给\aref{任意文字}{sub_Sample_2}加链接
\item \verb`\autoref{标签}` 自动生成文字加链接，如\autoref{eq_Sample_2}（本文）或\autoref{eq_AU_10}（其他文章）。
\end{itemize}
其他命令合并到这三个之中：\verb`\enref` 并入 \verb`\aref`， \verb`\nref` 并入 \verb`\upref`。

在 pdf 中，需要支持打印，所以变为以下效果：
\begin{itemize}
\item \verb`\upref{标签}` 变为一个脚注如 “见 103 页《向量空间》定义 2。”）
\item \verb`\aref{链接文字}{标签}` 相当于给\verb`任意文字` 加下划线然后使用 \verb`\upref{标签}`）
\item \verb`\autoref{标签}` 如 \verb`式 1`（本文）或 \verb`式 1`$^{\color{green}[103]}$（其他文章），但书签图标
\end{itemize}
