...
\begin{equation}
\begin{aligned}
\nabla_X\nabla_Y\bvec e_j& =\nabla_X\sum_k(\omega_j^k(Y)\bvec e_k) \\
&=\sum_kX\omega_j^k(Y)\bvec e_k+\sum_k\omega_j^k(Y)\nabla_X\bvec e_k \\
&=\sum_iX\omega_j^i(Y)\bvec e_i+\sum_{i,k}\omega_j^k(Y)\omega_k^i(X)\bvec e_i.
\end{aligned}~,
\end{equation}
根据对称性，同理可得
\begin{equation}
\nabla_Y\nabla_X\bvec e_j =\sum_iY\omega_j^i(X)\bvec e_i+\sum_{i,k}\omega_j^k(X)\omega_k^i(Y)\bvec e_i~.
\end{equation}
