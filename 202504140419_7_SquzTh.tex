...
由于$\lim_{n\to\infty}a_n=m$，可知对于任意$\varepsilon>0$，存在$N_a$使得对于任意$n>N_a$，都有$\abs{a_n-m}<\varepsilon$。同理，对于同一个$\varepsilon>0$，存在$N_b$使得对于任意$n>N_b$，都有$\abs{b_n-m}<\varepsilon$。

令$N=\max\{N_a, N_b\}$。由$a_n<b_n$，可知对于$n>$
