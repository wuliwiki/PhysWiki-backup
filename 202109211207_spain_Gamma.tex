...
\begin{definition}
		($\Gamma$函数的定义)由积分
		\begin{equation}
			\Gamma(z)=\int_{0}^{\infty}e^{-t}t^{z-1}\mathrm{d}t\label{Gamma(z)}
		\end{equation}
		定义的函数称为$\Gamma$函数.
	\end{definition}

	由Morera定理,由\eqref{Gamma(z)}式所定义的函数$\Gamma(z)$在$\mathrm{Re}(z)>0$是解析的,且
	\[
	...
	设
	\begin{equation}
		f_{n}(z)=\int_{0}^{n}\left(1-\frac{t}{n}\right)^{n}t^{z-1}\mathrm{d}t\qquad\mathrm{Re}(z)>0
	\end{equation}
	作变量替换$u=\dfrac{t}{n}$并分部积分$n$次,得到
	\begin{equation}
		f_{n}(z)=n^{z}\int_{0}^{n}(1-u)^{n}u^{z-1}\label{Guass f_n(z)}\mathrm{d}u
	\end{equation}
	当$0\leqslant t\leqslant n,n>1$时
	\begin{equation}
		0\leqslant1-e^{t}\left(1-\frac{t}{n}\right)^{n}\leqslant\frac{et^2}{4n}\label{Inequality}
	\end{equation}
	事实上,由于
	\[
		D\left\{1-e^{t}\left(1-\frac{t}{n}\right)^{n}\right\}=-\frac{1}{n}te^{t}\left(1-\frac{t}{n}\right)^{n-1}
	\]
	因此
	\[
		1-e^{t}\left(1-\frac{t}{n}\right)^{n}=\frac{1}{n}e^{t}\int_{0}^{t}ve^{v}\left(1-\frac{v}{n}\right)^{n-1}\mathrm{d}v
	\]
	而函数$ve^{v}\left(1-\dfrac{v}{n}\right)^{n-1}$在$v=1$取得最大值
	\[
		\sup_{v\geqslant0}\left\{ve^{v}\left(1-\frac{v}{n}\right)^{n-1}\right\}=e\left(1-\frac{1}{n}\right)^{n-1}\leqslant\frac{e}{2}
	\]
	故有
	\[
		1-e^{t}\left(1-\frac{t}{n}\right)^{n}\leqslant\frac{e}{2n}\int_{0}^{t}v\mathrm{d}v=\frac{et^2}{4n}
	\]
	此外当$v>0$时,$e^{-v}\geqslant1-v$,则$e^{-t/n}\geqslant1-\dfrac{t}{n}$,于是$e^{-t}\geqslant\left(1-\dfrac{t}{n}\right)^{n}$,故
	\[
		1-e^{t}\left(1-\frac{t}{n}\right)^{n}\geqslant0
	\]
	利用不等式\eqref{Inequality},得到
	\[
	...
	\]
	结合\eqref{Guass f_n(z)}式和\eqref{Inequality}式,可得
	\begin{equation}
		\Gamma(z)=\lim_{n\to\infty}\frac{n!n^{z}}{\displaystyle{\prod_{k=0}^{n}(z+k)}}\label{Gauss's formula}
	\end{equation}
	...
	且
	\begin{equation}
		\lim_{n\to\infty}\left(\sum_{k=1}^{n}\frac{1}{k}-\log n\right)=\gamma\label{Euler constant}
	\end{equation}
	从而
	\begin{equation}
		\frac{1}{\Gamma(z)}=ze^{\gamma z}\prod_{k=1}^{\infty}\left(1+\frac{z}{k}\right)e^{-z/k}\label{Weierstrass's formula}
	\end{equation}
	\eqref{Weierstrass's formula}式称为Weistrass公式.因此
	\begin{equation}
		\Gamma(z)\Gamma(1-z)=\frac{\pi}{\sin(\pi z)}\qquad 0<\mathrm{Re}(z)<1\label{Eluer's reflection formula}
	\end{equation}
	\autoref{Eluer's reflection formula}式称为余元公式.
