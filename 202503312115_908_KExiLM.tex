...
\]
现在，如果\( \Delta z \)是实数，则有\( \frac{\Delta \bar{z}}{\Delta z} = 1 \)，而如果它是虚数，则有\( \frac{\Delta \bar{z}}{\Delta z} = -1 \)。因此，第二项在\( \Delta z \to 0 \)的极限路径上是独立的，只有当它恒等地消失时才成立：\(f_x + i f_y = 0\),这正是柯西–黎曼方程的复数形式。这个证明还表明，在这种情况下，
\[
