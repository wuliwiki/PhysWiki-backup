...
在体坐标系中列出转动方程\autoref{RBEqM_eq6} 和\autoref{RBEqM_eq4} 往往可以简化计算． 这时往往令体坐标系的三个轴与刚体的三个主轴\upref{PrncAx}重合， 这样 $\mat I_0$ 就是三个主转动惯量构成的对角矩阵
\begin{equation}
...
\end{aligned}
\end{equation}
注意此时式中的 $\bvec\omega$ 和 $\dot{\bvec\omega}$ 仍然是刚体相对于实验室坐标系的角（加）速度， 只是使用了体坐标系中的基底来计算坐标． 第二条中特地使用 $\bvec\alpha_0$ 表示角加速度而不用 $\dot{\bvec\omega}_0$， 因为后者容易被误解为 $\bvec\omega_0$ 的时间导数， 这显然是错误的． 又令
