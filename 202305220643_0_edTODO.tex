...
\item 词条备份文件确保换行符使用 LF 而不是 CRLF， 保存前先计算文件 SHA1 的前 16 位， 确保 \verb|history.hash| 不存在（否则不备份）。 然后保存到 \verb|PhysWiki-backup/YYYYMMDDHHMM_authorID_entry.tex|， 其中 \verb|authorID| 是数据库的 \verb|authors.id|。 然后运行 \verb|./PhysWikiScan --backup authorID entry < 备份文件内容(utf8 编码)| 更新数据库（scan 会更新 \verb|add, del| 等所有备份信息）。 如果输出了 \verb|cerr| 就说明运行失败（详见错误信息）。
