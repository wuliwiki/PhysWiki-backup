...
所以在进行泰勒展开时， 如果函数在除了若干发散点（如\autoref{Taylor_eq4} 中的 $x = -1$ 或者\autoref{Taylor_eq6} 中的 $x = \mp 1$）外都无穷阶可导， 令离 $x_0$ 最近的发散点为 $a$， 那么 $f(x)$ 关于 $x_0$ 泰勒展开后， 幂级数的收敛半径就是 $r = \abs{a - x_0}$．
