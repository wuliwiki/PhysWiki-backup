...
而对于等式$f(x)=g(x)$，若它是指恒等关系，那么这一表述是平凡的，因为它只意味着改了个名称符号。因此，这种表达式出现时通常指的是条件等式，也就是方程。它仅在特定条件或某些点上成立，。即通过求解找出使等式成立的点，这些点通常对应于两个函数的交点。
然而，对于一般的条件等式（即方程），这样的操作是不可行的，因为条件等式仅在特定点上成立，运算可能会破坏其成立的条件。
