...
考虑实函数 $f(x)$． 若无论要求 $f(x)$ 和确定实数 $A$ 的距离$\abs{a_n - A}$ 有多小， 都能给出一个相应的条件 $x > X$ （$X$ 为某实数）使要求成立， 那么我们说 $A$ 是函数 $f(x)$ 在 $x$ 趋于正无穷时的极限， 记为
\begin{equation}
...
考虑实函数 $f(x)$． 若无论要求 $f(x)$ 和确定实数 $A$ 的距离 $\abs{a_n - A}$ 有多小， 都能给出一个相应的条件 $\abs{x-x_0} < \delta$ （$\delta$ 为某实数）使要求成立， 那么我们说 $A$ 是函数 $f(x)$ 在 $x$ 趋于 $x_0$ 时的极限， 记为
\begin{equation}
...
我们还可以区分函数在某点的\textbf{左极限}和\textbf{右极限}． 简而言之就是 $x$ 分别从左边和右边两个方向趋近 $x_0$ 时的极限， 具体定义留做思考． 左右极限记为
\begin{equation}
...
在 $x_0 = 0$ 处的左右极限显然分别为 $0$ 和 $1$， 但它在 $x_0 = 0$ 处不存在极限．
\end{example}
