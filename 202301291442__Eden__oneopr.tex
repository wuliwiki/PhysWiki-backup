...
现在考察多体系统，多粒子态 $\ket{\psi_1,\cdots,\psi_N}$ 不再是 $N$ 个单粒子态的简单的张量积，因为交换对称性，我们需要仔细考虑。假设单粒子 Hilbert 空间的一组正交完备基为 $\ket{1},\cdots,\ket{N}$，那么 Fock 空间可以表达为一系列多粒子态 $\ket{ijk\cdots}$ 的直和。我们期待单粒子算符能够被写为
\begin{equation}
\hat A^{(1)}=\sum_{i=1}^N A^{(1)}(\hat x_i,\hat p_i)
\end{equation}
其中算符 $A^{(1)}(x_i,p_i)$ 仅仅作用于多粒子态的 $\ket{i}$ 成分上。