...
\]
若函数$\phi$满足定理中提到的三个条件，则$F(z)$变换对于$\operatorname{Re} z>c$是存在的．即：当$\operatorname{Re} z>c$时，积分收敛．注意 \autoref{LapAsm_eq3} 意味着存在正整数N，当$n>N$时，对一切的$t\in B_{+}(0,\delta)$，总有
\[
  \left|f\left(t\right)-\sum_{n=0}^{N-1}{c_nt^{\lambda_n-1}}\right|\le Mt^{\lambda_N-1}
\]
