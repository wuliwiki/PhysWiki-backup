...
$\delta(x)$ 在泛函分析中并不是数学中一个严格意义上的函数， 而是被称为\textbf{广义函数（generalized function）}或\textbf{分布（distribution）， 详见泛函分析}． 但这里我们给出另含有 $\delta(x)$ 等式的一种严谨的理解． 令一个含有参数 $\alpha$ 的一元函数 $f_\alpha(x)$， 若满足
\begin{equation}
\lim_{\alpha\to\infty} f_\alpha(x) = 
\end{equation}
