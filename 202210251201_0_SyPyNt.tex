...
\subsection{微积分}
\begin{itemize}
\item 积分如 \verb|sm.integrate(sm.exp(x)*(sm.sin(x) + sm.cos(x)), x)|
\item 格式为 \verb|sm.integrate(sm.exp(x)*(sm.sin(x) + sm.cos(x)), x)| 
\end{itemize}
...
\item 例如令 \verb|expr = sm.sin(x)**2 + sm.cos(x)**2|， 那么 \verb|expr| 的类型是 \verb|sympy.core.add.Add|， \verb|expr.args| 是 \verb|(cos(x)**2, sin(x)**2)|． \verb|expr.args[0]| 的类型是 \verb|sympy.core.power.Pow|， \verb|expr.args[0].args| 是 \verb|(cos(x), 2)|， 以此类推． 这样就可以生成一个树状结构．
\end{itemize}
