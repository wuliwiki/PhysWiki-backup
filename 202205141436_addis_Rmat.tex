...
代入并左乘 $\chi_i$ 得
\begin{equation}
\sum_j \qty[2mH_{ij} - \delta_{ij}\frac{k^2}{2m}] c_j = 0
\end{equation}
这是一个其次线性方程组， 可以解出坐标 $c_j$． 事实上这里有一个小技巧， 把\autoref{Rmat_eq4} 改写为
\begin{equation}
(2mH + \mathscr L)\psi = \qty(k^2 + \mathscr L)\psi
\end{equation}
把

另外可以证明关系
\begin{equation}
\frac{\psi'(a)}{\psi(a)} = \frac{1}{aR(k)}
\end{equation}
其中 $R(k^2)$ 就是 $R$-矩阵
\begin{equation}
R(k) = \frac{1}{a} \sum_{i=1}^\infty \frac{\chi_i^2(a)}{k_i^2 - k^2}
\end{equation}
事实上目前这只是一个数， 即 $1\times 1$ 的矩阵． 在多通道问题中才会称为真正的矩阵．
