...
\end{figure}
我们定义了几个参数以表示这些光学器件对光线的作用，也就是图中的$r_1,\theta_1$和$r_2,\theta_2$（这里的$r$是图中的那个标量）。我们认为后者和前者是有线性关系的，也就是有：
...
\end{cases}
\text{    即：   }\pmat{r_2\\ \theta_2}=\pmat{A&B\\C&D}\pmat{r_1\\ \theta_1}~, \quad
\end{equation}
这里就出现了所谓的ABCD矩阵（这命名有点随意）。对于特定的光学器件，A、B、C、D都是常数。为什么会有这样的关系？我们距离解释
