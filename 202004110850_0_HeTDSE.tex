...
\begin{equation}
\begin{aligned}
(T_l)_{ij} &= \mel*{\phi_i}{-\frac{1}{2m}\dv[2]{r} + \frac{l(l+1)}{2mr}}{\phi_j}\\
&= -\frac{1}{2m} \mel*{\phi_i}{\dv[2]{r}}{\phi_j} + \delta_{i,j}\frac{l(l+1)}{2mr_i^2}
\end{aligned}
\end{equation}
其中 $\phi_i$ 是一维 FEDVR 格点在 $r_i$ 处的正交归一基底\upref{FEDVR}， $r_{max}$ 是计算 $T_l$ 时最后一个 FE 的边界（而不是最后一个格点）． 虽然\autoref{HeTDSE_eq3} 中含有 $r_{max}$， 但结果确与 $r_{max}$ 无关， 原因是 $T_l$ 也和 $r_{max}$ 相关， 两项中的 $r_{max}$ 抵消了．
