% 欧几里得内积空间的正交化、同构及正交群
% keys 欧氏内积空间|正交化|同构|正交矩阵|正交群

\begin{issues}
\issueTODO 需说明欧几里得内积空间和 $\mathbb{R}^n$ 的关系．
\end{issues}

\pentry{欧几里得内积空间\upref{EuVS}}
\footnote{柯斯特利金,代数学引论,第二卷.}对欧几里得内积空间 $V$（\autoref{EuVS_def1}~\upref{EuVS}），其上任一基底都可正交标准化，而空间 $V$ 上的对称双线性型 $(*|*)$ 实际上给出了 $V$ 中的度量性质，即长度（\autoref{EuVS_def2}~\upref{EuVS}）和夹角（\autoref{EuVS_def3}~\upref{EuVS}），这意味着就度量性质来说，欧几里得内积空间 $V$ 和 $\mathbb{R}^n$ 没有差别．由于标准正交基底的重要作用，往往要研究不同标准正交基底之间的转换关系，即两基底对应的过渡矩阵（或转换矩阵），与两标准正交基底对应的过渡矩阵称为正交矩阵，它们将构成一个群，称之正交群．
\subsection{正交化过程}
\begin{theorem}{标准正交化过程}\label{EVOIOG_the1}
设 $\{\bvec e_1,\cdots,\bvec e_m\}$ 是 $n$ 维欧几里得内积空间 $V$ 中的一组 $m$ 个线性无关的矢量．那么，存在一个标准正交矢量组 $\{\uvec e'_1,\cdots,\uvec e'_m\}$ ，使得线性包络（或称张成空间\autoref{VecSpn_def1}~\upref{VecSpn}）
\begin{equation}
...
设 $L$ 是有限维欧几里得内积空间 $V$ 的一个子空间，$L^{\perp}$ 是它的正交补，那么
\begin{equation}
...
\textbf{证毕！}
\subsection{欧几里得内积空间的同构}
\begin{theorem}{}
任意两个维数相同的欧几里得内积空间 $V,V'$ 都是同构的．即存在矢量空间的同构映射 $f:V\rightarrow V'$，它还保持内积：
\begin{equation}
...
由于同构的双方可认为是同一事物的不同表现形式，这意味着，欧几里得内积空间中每一矢量 $\bvec v$ 都可看成是一个线性函数 $\bvec v:V\rightarrow\mathbb{R}$．（即把 $\bvec v$ 看成 $V$ 的线性函数时，$\bvec v$ 相当于 $\varphi_{\bvec v}=(\bvec v|*)$）．
