...
\frac{n}{\pi}\sinc(n x) \qquad (n = 1, 2, \dots)
\end{equation}
...
\subsection{性质}
若一个等式中出现了所谓的 $\delta$ 函数 $\delta(x)$， 那么其严格的定义是先将 $\delta(x)$ 替换为\autoref{Delta_def1} 中的 $\delta_n(x)$． 该\textbf{等式在 $n\to\infty$ 的极限时成立}．
