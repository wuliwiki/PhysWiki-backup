...
\begin{enumerate}
\item $S(a_{(1)})a_{(2)} = 1_{(1)} \, \epsilon(a1_{(2)}), \quad \forall a \in H$（右侧是一个投影，通常记作 $\Pi_R(a)$ 或 $\epsilon_s(a)$，其像是一个称为 $H_R$ 或 $H_s$ 的可分子代数）；
\item $a_{(1)} S(a_{(2)}) = \epsilon(1_{(1)}a) \, 1_{(2)}, \quad \forall a \in H$（这是另一个投影，通常记作 $\Pi_L(a)$ 或 $\epsilon_t(a)$，其像是一个可分代数 $H_L$ 或 $H_t$，并且通过 $S$ 与 $H_L$ 反同构）；
\item $S(a_{(1)})a_{(2)}S(a_{(3)}) = S(a), \quad \forall a \in H$.需要注意的是，如果 $\Delta(1) = 1 \otimes 1$，那么这些条件便化简为霍普夫代数反元的通常两条条件。
\end{enumerate}
\end{itemize}
