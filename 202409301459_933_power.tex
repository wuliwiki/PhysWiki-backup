...
这里独立说一下正实数的无理数次幂，它的定义是由极限得到的。如果有两列数字分别为$p_i,q_i$，满足对任意的$i$都有$p_i<b<q_i$，如果随着$i$增加$a^{p_i},a^{q_i}$之间的区别越来越小，直至二者几乎相等，那么就认为$a^b$就是那个值。这里采用了比较模糊的表述，在高中教材中也是如此描述。它实际上反映了极限运算的本质。此处参照\autoref{ex_power_1} 有一个感性认识就可以。
