...
定义每个电子的电荷精确等于 $1.602176634\e{-10} \Si{C}$．

\subsubsection{电势/电压：伏特（V）}
等效于 $\Si{J/C}$ 或者 $\Si{kg\cdot m^2 s^{-3} A^{-1}}$． 伏特的定义使得 $1\Si{C}$ 的电荷增加 $1\Si{V}$ 电势， 需要 $1\Si{J}$ 的能量．

\subsubsection{电场}
等效于 $\Si{V/m}$ 或 $\Si{N/C}$ 或 $\Si{kg\cdot m s^{-3} A^{-1}}$．

\subsubsection{电容：法拉（F）}
等效于 $C/V$ 或者 $\Si{A^2 s^4 kg^{-1} m^{-2}}$． 法拉是电容量的单位， 一个 $1\Si{F}$ 的电容器两端施加 $1\Si{V}$ 电压， 可以储存 $1\Si{C}$ 净电荷．

\subsubsection{电容率}
单位 $\Si{F/m}$ 或者 $\Si{A^2 s^4 kg^{-1} m^{-3}}$．
