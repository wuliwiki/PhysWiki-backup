...
如果给$V_1$和$V_2$进行基变换，过渡矩阵分别为$m_{ij}$和$n_{ij}$，则在新的基下，$\bvec{a}'_i=m_{ki}\bvec{a}_k$，$\bvec{b}'_j=n_{lj}\bvec{b}_l$，因此张量$f$在新基下的坐标为：

\begin{equation}\label{TrTnsr_eq2}
T'_{ij}=f(\bvec{a}'_i, \bvec{b}'_j)=f(m_{ki}\bvec{a}_k, n_{lj}\bvec{b}_l)=m_{ki}n_{lj}f(\bvec{a}_k, \bvec{b}_l)=m_{ki}n_{lj}T_{kl}
\end{equation}
...
给定域$\mathbb{F}$上的一组有限个线性空间$\{V_\alpha\}^n_{\alpha=1}$，设$f:V_1\times\cdots\times V_n\rightarrow\mathbb{F}$．给各$V_\alpha$指定一组基，使得$f$在这些基下的坐标为$T_{i_1i_2\cdots i_n}$．如果对每个$V_\alpha$进行过渡矩阵为$m^\alpha_{ij}$的基变换，那么
