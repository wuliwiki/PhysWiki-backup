...
\subsection{另一种推导}
\footnote{参考 \cite{Goldstein} 相关章节．}类比\autoref{Vtrans_eq1}~\upref{Vtrans}， 若 $S'$ 系与 $S$ 系原点始终重合， 且相对 $S$ 系以角速度 $\bvec\omega$ 旋转， 对任意一个随时间变化的矢量（假设一阶导数存在）， 我们把它在 $S$ 和 $S'$ 系中的时间导数分别记为 $(\dot{\bvec A})_{S}$ 和 $(\dot{\bvec A})_{S'}$， 则有
