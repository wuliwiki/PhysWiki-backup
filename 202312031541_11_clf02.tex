...
正交性要求$\theta_1\theta_2=-\theta_2\theta_1$，对应$\theta_i\wedge\theta_j=-\theta_j\wedge\theta_i$。从上式可以看到，基的正交性等价于\textbf{正交变换条件}$\sum_k(a^k b^k s(e_k))=0$，这其实是正交矩阵在任意空间的推广。在配备了正定二次型的欧几里得空间中，这意味着正交矩阵定义为列向量组中任意两个列向量内积为$0$。
\subsubsection{左内积}
\begin{definition}{}
给定Clifford代数$\mathrm {Cl(X,R,s)}$，对于$A,B\in 2^x$，定义
\begin{equation}
