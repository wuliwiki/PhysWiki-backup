...
\begin{equation}
G^\mu(k,p,p+k)=\int \dd[4]{x}\dd[4]{y}\dd[4]{z} e^{-ikx}e^{-ipy}e^{i(p+k)z}\bra{\Omega} \psi(z) \bar\psi(y) A^\mu(x) \ket{\Omega}
\end{equation}
应当满足
\begin{equation}
k_\mu G^\mu(k,p,p+k)=0
\end{equation}
利用三点函数 Feynman 规则（\autoref{qedfey_the1}~\upref{qedfey}），我们总是可以将三点函数的 Feynman 图表示为截肢图和外腿图的组合，因此
