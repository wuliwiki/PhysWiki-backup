...
考虑一个由 \(N\) 个质点组成的系统的动能。如果假设 \( T(\mathbf{q}, \mathbf{\dot{q}}, t) = T(\mathbf{q}, \mathbf{\dot{q}}) \)，则可以证明 \(\dot{\mathbf{r}}_k(\mathbf{q}, \mathbf{\dot{q}}, t) = \dot{\mathbf{r}}_k(\mathbf{q}, \mathbf{\dot{q}}) \)（参见 Scleronomous § 应用）。因此，动能为

\[
T(\mathbf{q}, \mathbf{\dot{q}}) = \frac{1}{2} \sum_{k=1}^{N} \left( m_k \dot{\mathbf{r}}_k(\mathbf{q}, \mathbf{\dot{q}}) \cdot \dot{\mathbf{r}}_k(\mathbf{q}, \mathbf{\dot{q}}) \right)
\]

使用多变量的链式法则可以将速度展开：

\[
\dot{\mathbf{r}}_k(\mathbf{q}, \mathbf{\dot{q}}) = \frac{d \mathbf{r}_k(\mathbf{q})}{dt} = \sum_{i=1}^{n} \left( \frac{\partial \mathbf{r}_k(\mathbf{q})}{\partial q_i} \dot{q}_i \right)
\]

得到的结果为

\[
T(\mathbf{q}, \mathbf{\dot{q}}) = \frac{1}{2} \sum_{k=1}^{N} \left( m_k \left( \sum_{i=1}^{n} \left( \frac{\partial \mathbf{r}_k(\mathbf{q})}{\partial q_i} \dot{q}_i \right) \cdot \sum_{j=1}^{n} \left( \frac{\partial \mathbf{r}_k(\mathbf{q})}{\partial q_j} \dot{q}_j \right) \right) \right)
\]

进一步化简为

\[
=\sum_{i=1}^{n}\sum_{j=1}^{n} \left( \sum_{k=1}^{N} \left( \frac{1}{2} m_k \frac{\partial \mathbf{r}_k(\mathbf{q})}{\partial q_i} \cdot \frac{\partial \mathbf{r}_k(\mathbf{q})}{\partial q_j} \right) \dot{q}_i \dot{q}_j \right)
\]
