...
\end{lemma}
\subsection{磁矢势的多级展开}
类似于\enref{电多级展开}{EMulPo}的讨论，我们考虑一个局域在原点附近的电流密度分布 $\bvec J(\bvec x')$ 在远离电流区域的 $\bvec x$ 处所产生的磁矢势（也就是说 $|\bvec x|\gg|\bvec x'|$）。这里我们用的是库仑规范\upref{Cgauge}（实际上在静磁问题中，库仑规范和洛伦兹规范等价）。电流分布在小区域 $V$ 内。
\begin{equation}
