...
\autoref{BAHA_eq9} 对 $\tau$ 微分，并利用\autoref{MatExp_eq3}~\upref{MatExp}，得
\begin{equation}
\begin{aligned}
(A+B)\E^{\tau(A+B)}&=B\E^{\tau(A+B)}+\E^{\tau B}\pdv{K(\tau)}{\tau}\E^{\tau A}+\E^{\tau B}K(\tau)\E^{\tau A}A\\
\pdv{}{\tau}K&=\E^{-\tau B}A\E^{\tau B}K-KA
\end{aligned}
\end{equation}
