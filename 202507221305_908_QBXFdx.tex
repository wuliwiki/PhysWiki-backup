...
$$
x^n = 2^{1 - n} \sum\nolimits^{\prime}_{j = 0 \atop j \equiv n \pmod{2}}^{n} \binom{n}{\tfrac{n - j}{2}} T_j(x)~
$$
...
用带负上下标的组合数写成：
$$
= \sum_{k=0}^{\left\lfloor \frac{n}{2} \right\rfloor} \binom{2k - (n + 1)}{k} (2x)^{n - 2k}, \quad \text{当 } n > 0~
$$
用交错符号的组合数写成：
$$
= \sum_{k=0}^{\left\lfloor \frac{n}{2} \right\rfloor} (-1)^k \binom{n - k}{k} (2x)^{n - 2k}, \quad \text{当 } n > 0~
$$
用阶乘和幂级数形式表示：
$$
