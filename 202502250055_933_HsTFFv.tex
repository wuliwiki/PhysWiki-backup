...
\item $\omega$ 决定了函数变化的快慢，即图像在水平方向上的压缩或拉伸程度。$\omega$ 越大，周期越短，函数振荡得越快。\textbf{周期（cycle）} $T$ 与 $\omega$ 之间的关系为：
\begin{equation}
    T = \frac{2\pi}{\omega}~.
\end{equation}
...
\item 在日常生活中，另一个常见的概念是\textbf{频率（frequency）}，它可以从圆周运动的角速度角度来理解。频率表示单位时间内完成的完整振荡次数，在圆周运动中，它对应于物体转过的圈数。而角频率 $\omega$ 是圆周运动特有的量，表示单位时间内转过的弧度。由于一个完整的圆周对应 $2\pi$ 弧度，因此频率 $f$ 与角频率 $\omega$、周期 $T$ 的关系为：
\begin{equation}
    f = \frac{1}{T} = \frac{\omega}{2\pi}~.
\end{equation}
这一关系表明，角频率和频率本质上是同一现象的不同刻画方式。值得注意的是，这里的分析基于圆周运动的概念，这也正体现了“三角函数”也称为“圆函数”这件事。
\item $\varphi$的作用主要体现在比较具有相同角频率的两个正弦型函数时（补充，相位前后的概念）。
\end{itemize}

\item $\varphi$ 的作用主要体现在比较具有相同角频率的两个正弦型函数时。在给定$\omega$ 的情况下，两个正弦型函数的形状将完全相同，区别则是如果具有不同的初相 $\varphi$，它们的图像将会沿 $x$ 轴方向发生平移。只要具有不同的初相，则两个函数的相位则始终保持相同的\textbf{相差（phase difference）}。更清晰地描述两个周期信号之间的相对位置。

\


关于初相，他的影响主要是对具有相同角频率或最小正周期的两个正弦型函数时做比较。设具有相同角频率或最小正周期的两个正弦型函数：
\begin{equation}
