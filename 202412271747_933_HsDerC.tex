...
根据定义，可能是极值点的点包括驻点，但并非所有驻点都是极值点。例如，对于函数 $y = x^3$，在 $x = 0$ 处满足 $f'(x) = 0$，但两侧的函数值有的大，有的小，因此 $x = 0$ 并不是极值点。此外，极值点的判定并不依赖于一阶导数是否存在。只要满足在某个邻域内满足要求就是极值点。例如，对于函数 $y = |x|$，在 $x = 0$ 处，函数的一阶导数不存在，但由于在任意的区间$(-\delta,\delta)$上都满足$|0|$最小，因此 $x = 0$ 是一个极值点。

事实上，极值点对应着在某点两侧导数值变号，而与这一点是否可导无关。因此，驻点如果是极值点必须满足导数在该点附近的符号发生变化。只不过是因为驻点作为零点的特殊性，往往对应着导数符号变化的情况，从而造成概念上的混淆。
