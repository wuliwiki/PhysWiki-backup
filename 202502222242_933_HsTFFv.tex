...
在不严谨的情况下，当然可以从几何的视角直接看出三角函数的单调性，高中教材在介绍三角函数的单调性时，大多数也会采取这样的方法。在单位圆上，对于角$x\in(0,2\pi)$的情况，点 $P$ 的横坐标表示 $\cos x$，纵坐标表示 $\sin x$。当 $x$ 处于上半平面（$0 < x < \pi$）时，随着 $x$ 增加，$P$ 的横坐标从 $1$ 变为 $-1$，即 $\cos x$ 递减；而在下半平面（$\pi < x < 2\pi$）时，$P$ 的横坐标从 $-1$ 变为 $1$，即 $\cos x$ 递增。因此，结合周期性可知：
\begin{itemize}
\item 在 $0 + 2k\pi < x < \pi + 2k\pi$ 内，$\cos x$ 递减。
\item 在 $\pi + 2k\pi < x < 2\pi + 2k\pi$ 内，$\cos x$ 递增。
\end{itemize}
同理，$\sin x$ 和 $\tan x$ 的单调性也可以用类似方式观察。然而，如前面所说，这种分析方法虽然直观，但并不严谨。而如果想要根据单调性的定义来分析三角函数的单调性，则略显困难，因为三角函数做差进行符号的讨论会较为复杂。更有效的方法是使用导数。在此前已经介绍过三角函数的导数，接下来可以基于
\aref{$\sin x$的导数}{eq_HsDerB_4}、\aref{$\cos x$的导数}{eq_HsDerB_5}、\aref{$\tan x$的导数}{eq_HsDerB_6} 来更严格地分析其单调性。

理论上从定义可以直接看出sinx和cosx的单调性，当x在上半平面时，随着x增加点P的横坐标从1变到-1，在下半平面时，随着x增加点P的横坐标从-1变到1，从而根据定义可知，当0+2kpi<x<pi+2kpi时，cosx递减，当pi+2kpi<x<2pi+2kpi时，cosx递增。sinx和tanx也可以认为这样看出。当然，这是朴素的，尽管在幂函数等代数为基础的函数中可以做差进行考量，但三角函数做差进行讨论比较复杂。好在研究单调性还有导数的方法，之前已经介绍过三角函数的导数，下面就基于\aref{sinx的导数}{eq_HsDerB_4} \aref{cosx的导数}{eq_HsDerB_5}，\aref{tanx的导数}{eq_HsDerB_6}进行讨论。

首先，之前提及过，当x在第一象限时，即$x\in$时，因为此时他们对应的都是直角三角形中的边的关系，所以此时所有的三角函数值都是正的。
由于(\sin x)'=\cos x，因此在第一象限时sinx是递增的，
