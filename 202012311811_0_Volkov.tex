...
\end{equation}
其中 $E_0 = \bvec k^2/(2m)$． 注意以下所有规范中， 广义动量算符都是 $\bvec p = m\bvec v + q\bvec A = -\I \grad$．
...
\end{equation}
其中 $\bvec p(t)$ 是经典粒子的动量（\autoref{Volkov_eq5} ）． 由于长度规范下矢势 $\bvec A_L \equiv 0$（\autoref{LenGau_eq4}~\upref{LenGau}）， 广义动量就是 $m\bvec v$．
\begin{equation}
\bvec p(t) = m\bvec v(t) = \bvec k - q\bvec A(t)
\end{equation}
注意本文中的 $\bvec A(t)$ 都是指库仑规范或速度规范中的矢势而不是长度规范．
