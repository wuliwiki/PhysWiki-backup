...
另外需要注意的是，题目中$|PF_1| - |PF_2| = m>0$隐含了一个默认条件，即 $|PF_1| > |PF_2|$，这使得最终得到结果其实只对应 $x > 0$ 的区域。但在实际作图后会发现，在$x > 0$ 的区域也有正好与右侧对称的图像，对应的就是$|PF_2|>|PF_1|$的部分。因此，如果将条件改为 $||PF_1| - |PF_2|| = m$，其中 $0 < m < 2c$，就会完整对应 \autoref{eq_Hypb3_3} 表达式，图像也将呈现左右对称的完整双曲线。而题目所给出的，正是双曲线的几何定义，也被称为\textbf{双曲线的第一定义}。
