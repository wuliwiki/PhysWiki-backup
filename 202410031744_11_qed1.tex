...
\begin{enumerate}
\item 正规正时（proper orthochronous）$\quad$($\Lambda_{+}^{\uparrow}/SO^+(1,3)$)，对应 $\det\Lambda=1\quad \Lambda_{0}^{0}\geqslant1$。
\item 正规负时（proper non-orthochronous）$\quad$($\Lambda_{+}^{\downarrow}$)，对应 $\det\Lambda=1\quad \Lambda_{0}^{0}\leqslant-1~.$
