...
\begin{issues}
\issueDraft
\issueOther{这甚至不是量子力学问题， 直接作为厄米矩阵本征值的微扰理论来讲可能会更好}
\end{issues}
...
\end{equation}
下面分为 $H_0$ 是否简并来具体讨论． 简并是指 $H_0$ 的一个本征值可能对应多个线性无关的本征矢， 每个本征值对应的所有本征矢的集合构成\textbf{本征子空间}（\autoref{HerEig_sub1}~\upref{HerEig}）．
...
我们只需要通过某种方法找到上式的某个解即可． 观察等式右边可以发现右边第一项对角块都为零（$E_m = E_n$）， 第二项是对角矩阵， 所以两项相加后矩阵的对角块都一定是对角矩阵， 非对角块则未必（图未完成）． 所以等式左边的矩阵也需要满足同样的要求， 但 $H^1$ 是给定好的无法改变， 所以左边是否满足要求只取决于基底 $\psi_n^0$ 的选取．

但考虑到 $H_0$ 简并， $\psi_n^0$ 并不是完全确定的： 每个本征子空间中正交归一基底的选取都仍有一定自由——可以进行任意基底变换． 所以我们在每个本征子空间中把 $H^0$ 和 $H^1$ 同时对角化即可满足上述要求． 注意这并不要求 $[H^0, H^1] = 0$ 对易， 因为对易要求整个 $\mel{\psi_m^0}{H^1}{\psi_n^0}$ 矩阵可以被彻底对角化而不只是每个对角块对角化． 这样通常就能唯一地确定一组 $\psi_n^0$， 但不唯一也关系不大．
