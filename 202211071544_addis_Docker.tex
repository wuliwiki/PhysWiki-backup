...
\item 要下载镜像用 \verb`docker pull 用户名/repo名:TAG` 其中 \verb|用户名/| 对于一些官方镜像可以忽略， 例如 \verb`docker pull ubuntu:22.04` 中， \verb|ubuntu| 是 repo， \verb|22.04| 是 TAG． \verb|docker pull hello-world| 同理．
\item 注意区分 container 和 image 和 repo． repo 相当于 VirtualBox 虚拟机， image 相当于 snapshot， 而 container 相当于现在在运行的虚拟机．
\item \verb`docker run -it -d 镜像名或ID` 从 image 创建 container． 反复使用会创建多个．
...
\item \verb`docker image rm [-f] 镜像名或ID` 删除镜像
\item \verb`docker commit CONTAINER_ID 用户名/repo名` 会将 container commit， 也可以在后面加 \verb`:TAG` 指定 tag
