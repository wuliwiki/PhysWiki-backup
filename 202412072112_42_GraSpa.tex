...
\begin{exercise}{}
试证明：点空间 $\mathcal V(G)$ 和边空间 $\mathcal E(G)$ 满足\aref{向量空间}{def_LSpace_2}的定义。上加法和数乘定义如下：
\begin{equation}
\begin{aligned}
&(f+g)(v_i):=f(v_i)+g(v_i),\quad &f,g\in\mathcal V(G),v_i\in\mathbb V(G),\\
&(\lambda f)(v_i):=\lambda f(v_i),\quad &f\in\mathcal V(G),\lambda_\mathbb R,v_i\in V(D).
\end{aligned}~
\end{equation}
