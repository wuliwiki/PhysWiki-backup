...
\addTODO{？？？，每个时刻 $t$ 都对分块厄米矩阵 $\mat A$ 的每个块分别对角化不就行了吗？？？ 如果不是分块矩阵，就直接整个对角化不就好了吗？ 但是！但是！这样一来 $t$ 改变时难道要不断的对角化？}
...
$\mat A$ 就是一个块对角矩阵，每个对角块代表一个本征子空间（或者若干个可能在某时刻本征值相同的本征子空间张成的空间），不同子空间之间不存在耦合。把所有 $c_m(t)$ 按顺序排成列向量 $\bvec c(t)$，
...
？？ 的两个求和中，至于哪些矩阵元被忽略，可能取决于时刻 $t$。 这就是说矩阵 $\mat A(t)$ 的对角块可能有些时候会发生拆分或合并（多个对角块发生耦合后合并为一个）。 但若这种合并的持续时间只有很短乃至一瞬间，那我们可以认为合并前后波函数不发生变化，也就是假设合并不存在。
