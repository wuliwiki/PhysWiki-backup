...
\end{equation}
这样， 二阶导数只存在于左边的 $\ddot q_j$， 其他项都只是 $q,\dot q, t$ 的函数。 该式中的偏微最好计算出解析表达式， 但也可以通过\enref{差分}{ParDf}来计算会引入更多误差）。 数值解线性方程\footnote{如果 $N$ 不大可以直接用\enref{克拉默法则}{kramer}，甚至写出解析解。}， 就可以得到（令 $v_i = \dot q_i$）（\autoref{the_GO2SOD_2}~\upref{GO2SOD}）
\begin{equation}
\leftgroup{
&\dot v_i = f_i(q, v_i, t)\\
&\dot q_i = v_i
