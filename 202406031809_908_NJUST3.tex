...
5. 已知电子自旋在空间任一方向上的投影只有两个可能取值：$\\pm \\frac{\\hbar}{2}$，试求电子自旋在空间任意方向 $\\mathbf{n}$ 的投影 $s_n = \\hat{\\mathbf{s}} \\cdot \\mathbf{n} = \\hat{s}_x \\cos \\alpha + \\hat{s}_y \\cos \\beta + \\hat{s}_z \\cos \\gamma$ 的归一化本征矢量。设单位矢量 $\\mathbf{n}$ 的方向余弦为 $(\\cos \\alpha, \\cos \\beta, \\cos \\gamma)$。
