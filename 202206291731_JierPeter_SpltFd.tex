...
由\autoref{FldExp_the4}~\upref{FldExp}第2条，可知，域自同构一定把每个多项式的根映射到其它根上，并且
...
不妨设$f$的各不可约因子互不相同．设$\sigma:\mathbb{K}\to\mathbb{K}$是保$\mathbb{F}$自同构，$h$是$f$在$\mathbb{F}$上的一个不可约因子，其在$\overline{\mathbb{F}}$上的全体根为$\{\alpha_i\}_{i=1}^n$．

考虑单扩张$\mathbb{F}(\alpha_1)$的保$\mathbb{F}$自同构，则这种自同构的数量等于$\mathbb{F}(\alpha_1)$中所包含的$h$的根的数量．因此，如果$\mathbb{F}(\alpha_1)$包含所有$\alpha_i$，则自同构数量等于所有根的数量，定理成立：由\autoref{FldExp_the1}~\upref{FldExp}和“多项式的根小于等于其次数”即可．
