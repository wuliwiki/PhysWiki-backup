...
\subsection{运动的计算}
我们使用 “开普勒问题的运动方程\upref{EqMoKp}” 给出的方法求解三种圆锥曲线的含时运动． 易得 $\cos\beta > 0$ 时质点绕中心天体逆时针运动， $\cos\beta < 0$ 时顺时针运动． 令 $t=0$ 时质点位于近日点， 对应极角为 $\theta_0 + \pi$． 令相对近日点的极角增量为 $\Delta\theta = \theta - \theta_0 - \pi$． 接下来， 先计算函数 $t(\Delta\theta)$， 默认逆时针旋转， 然后数值求解反函数 $\Delta\theta(t)$ 即可． 若要考虑顺时针旋转， 乘以 $-1$ 即可．

\textbf{抛物线}： 时间 $t$ 在 $(-\infty,\infty)$ 取值， \autoref{EqMoKp_eq3}~\upref{EqMoKp}就是 $t(\Delta\theta)$．

\textbf{椭圆}： 时间 $t$ 在 $[-T/2,T/2]$ 取值， 周期 $T$ 由\autoref{Keple2_eq1}~\upref{Keple2} 计算． 

当然， 要生成 $t$ 的格点， 最好一开始就求出发射质点的初始时刻 $t_0$， 满足 $\theta(t_0) = \alpha$． 可以先求 $r(\alpha)$， 再代入\autoref{EqMoKp_eq1}~\upref{EqMoKp} 求 $\psi$， 最后代入（？） 得 $t_0$．

 同样求出 $t_0$， 生成时间格点， 解方程得到 $t\in[0,\infty)$ 的 $\theta(t)$， 再使用（？） 即可．
