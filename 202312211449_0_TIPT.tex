...
\subsection{与绝热近似结合}
接下来考虑一个应用问题： 含时薛定谔方程中如果初始波函数处于任意一个态 $\Psi(0)$， 然后非常缓慢地增加含时微扰 $\lambda(t) H'$ 的强度直到 $\lambda(t)=1$， 这个过程中 $\Psi(t)$ 会如何随时间变化？

\textbf{绝热近似（adiabatic approximation）}\upref{AdiaQM}告诉我们， 如果 $\Psi(0)$ 是一个非简并本征态（或好量子态） $\psi_n(0)$， 那么该过程中 $\Psi(t)$ 将始终保持 $H+\lambda(t) H'$ 的本征态（好量子态），但需要乘以一个额外的相位因子 $\E^{\I\alpha_n(t)}$。
