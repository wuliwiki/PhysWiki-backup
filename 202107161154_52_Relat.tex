...
如果一个集合$A$上定义了一个（偏）序关系$\prec$，那么称这个集合是带有偏序$\prec$的\textbf{偏序集（partially ordered set）}或\textbf{半序集（semi-ordered set）}．
\end{definition}
...
\end{example}
\begin{example}{幂集}
一个集合$A$的幂集\footnote{即集合$A$所有子集的集合．}$2^X$与子集关系构成偏序集.
\end{example}

\begin{definition}{全序集}
偏序集$(A,\prec)$称为\textbf{全序集（totally ordered set）}，当且仅当对任意$a,b \in A$，$a \prec b$或$b \prec a$．
\end{definition}
\begin{example}{}
...
全序集$(A,\prec)$称为\textbf{良序集（well-ordered set）}，当且仅当它的任意子集都有
\end{definition}