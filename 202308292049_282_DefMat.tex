...
\begin{definition}{}
若一个对称矩阵\upref{SymMat} $\mat A$， 对任意非零实数列向量 $\bvec v$ 满足 $\bvec v\Tr \mat A \bvec v > 0$， 那么它就是\textbf{实正定矩阵}。

若一个厄米矩阵\upref{HerMat} $\mat A$， 对任意非零复数列向量 $\bvec v$ 满足 $\bvec v\Her \mat A \bvec v > 0$， 那么它就是\textbf{复正定矩阵}。
\end{definition}
