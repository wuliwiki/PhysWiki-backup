...
\subsubsection{通用规范}
\begin{itemize}
\item 数域用双线字母表示， \textbf{实数} $\mathbb R$、 \textbf{复数} $\mathbb C$、 \textbf{有理数} $\mathbb Q$、 \textbf{整数} $\mathbb Z$、 \textbf{正整数} $\mathbb Z^+$、 \textbf{自然数} $\mathbb N = \mathbb Z^+$、 非负整数（非零自然数） $\mathbb Z^{0+}$．
