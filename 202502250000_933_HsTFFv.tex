...
值得注意的是，在 $\sin x$ 的一个周期内，尽管几乎每个 $y$ 都对应着两个 $x$，但这两个 $x$ 的值互补，使得它们的导数符号相反，即在相同的 $y$ 值处，斜率方向不同。因此，每对 $(y, y')$ 仅对应唯一的 $x$，故而，尽管$\sin x$在一个周期上并非单调的，但每个$x$的位置都是独一无二的。不会出现sin x和sinx+2pi这种无法区分的样子。从而在一个周期内确定 $\sin x$ 在坐标轴上的具体位置。这也正是将相位作为整体单独分析的重要性。尤其是确定y和y'在物理上具有重要的意义，如位移和速度等。相位 $\omega x + \varphi$ 直接对应于 $\sin x$ 函数图像中的位置。

在这些新概念中，相位 $\omega x + \varphi$ 尤其值得关注。与以往的参数不同，它不是单独作为一个数字出现，而是作为整体引入，从而提供了一种新的视角来理解三角函数的变化。具体而言，由于$\sin x$ 是一个非线性函数，直接分析其变化规律并不直观。于是，可以借鉴指数函数的处理方式，将 $\sin(\omega x + \varphi)$ 视为一个复合函数，其中 $\omega x + \varphi$ 对 $x$ 进行线性变换，作为 $\sin$ 的输入，而 $\sin$ 仅在最后起到非线性映射的作用，将输入限制在 $[-1,1]$ 之间，而不直接影响 $x$ 的变换过程。而这里相位整体其实对应的就是$\sin x$函数图像中的位置。对于sinx一个周期内，尽管几乎每一个y都对应着两个x，但这两个x是互补的，或者说这两个点的导数是不同的，也就是说每一组y,y'都对应着唯一的x，因此尽管sinx并不是单调的，但只要找到x，就可以找到在一个周期内，sinx唯一对应的y的位置。这也就是将相位单独考虑的重要性。
