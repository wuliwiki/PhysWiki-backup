...
站在稍微抽象一点的数学角度上，数据集中的每一个特征（例如表1中的每一列）都可以视为一个坐标分量，对应于一个坐标轴，那么全部特征就能够构成一个向量空间。我们称此向量空间为\textbf{特征空间}（feature space），\textbf{属性空间}（attribute space）、或者\textbf{输入空间}（input space）。一个样本（或实例）就可以被称为\textbf{特征向量}（feature vector）。


\textbf{参考文献：}
\begin{enumerate}
\item 周志华. 机器学习[M]. 清华大学出版社. 
\end{enumerate}