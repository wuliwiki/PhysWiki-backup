...
再回头考虑力学量系综平均$\left \langle A \right\rangle\left(t\right)$的时间演化。仍然考虑解析，有：

\begin{equation}
\begin{aligned}
\frac{\mathrm{d}}{\mathrm{d} t}\left \langle A \right\rangle 
&= \int \frac{\partial}{\partial t}\left(A \rho\right) \mathrm{d}^Nq~\mathrm{d}^Np \\
&= \int \left[ 
    \left( \frac{\partial}{\partial t}A \right)\rho + A\left( \frac{\partial}{\partial t}\rho \right)
     \right]\mathrm{d}^Nq~\mathrm{d}^Np
\end{aligned}~
\end{equation}
