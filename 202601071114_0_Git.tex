...
\item \verb`git grep` 可以在仓库中快速搜索关键词， \verb`grep --exclude-dir='.git'` 则往往较慢。 注意 \verb`git grep` 不能搜到没有 \verb`add` 的文件， 除非加了 \verb`--no-index`。 \verb`-l` 可以仅列出文件名（有些文件一行很长）。 \verb`-i` 选项不区分大小写。 \verb`-w` 
