...
\subsection{基础}
\begin{itemize}
\item [一个教程](http://faculty.salina.k-state.edu/tim/unix_sg/index.html)
\item Linux 命令行区分大小写
...
## dd
\item 这个命令可以直接在当前位置生成一个大小为 100 MB 的文件， 内容为随机， 还会显示写入速度
\verb`dd if=/dev/urandom of=./dump.txt bs=1048576 count=100`
\end{itemize}
...
\item top 中的 `NI` 就是 nice value
\item 在 top 界面中用 `r` (renice) 可以调整进程的 nice value （优先级，越小越优先）按 ESC 可以退出
\item 在 top 界面中用 `k` 可以杀进程，输入进程的 PID，两次回车即可
\item `pstree` 可以看到哪些进程生成了哪些子进程， 例如 shell 里面运行 .sh 文件会生成一个子进程用于运行一个新的 shell
\item `kill -STOP 进程号` 可以暂停程序
\item `kill -CONT` 可以继续暂停的程序
\end{itemize}
