...
首先考虑$\rho\left(p,~q,~t\right)$的时间演化，考虑$\rho\left(p,~q,~t\right)$解析。用\autoref{eq_denMat_7} 中第二式两边对$t$求导。

\begin{equation}
\begin{aligned}
\frac{\mathrm{d}}{\mathrm{d}t}\int \rho\left(p,~q,~t \right) \mathrm{d}^Np~\mathrm{d}^Nq &= 0
\end{aligned}~
\end{equation}
