...
\caption{gdb 调试最痛苦的地方：调试时调用函数不存在。 解决办法：\verb`set print pretty on` 或者直接用 IDE。 如果调试显示函数不存在，那就是被优化掉了，无解，只能查看数据成员。} \label{fig_gdbNt_1}
\end{figure}
...
\item 快捷键 \verb`Ctrl+R` 可以搜索之前输入的命令（退出 gdb 后不会保留命令历史）。
...
\item \verb`backtrace` 或 \verb`bt` 显示每层正在执行的子程序的信息 （stack frame）. 这样就可以追踪当前的 function 是被哪一行调用， 直到 main() 程序。调用顺序从下到上，数字从大到小。
\item \verb`up` 和 \verb`down` 可以在 stack 中移动， 例如 \verb`up` 可以移动到调用当前函数的函数中， 检查那里面的变量。
\item \verb`frame 2` 或 \verb`f 2` 可以直接跳到 \verb`bt` 显示的第 2 个 frame。 \verb`frame` 可以显示当前在 stack 中的位置。
...
\item \verb`catch load /my/path/file.so` 可以在加载动态库的时候 break。也可省略 \verb`/my/path/`。
\end{itemize}
