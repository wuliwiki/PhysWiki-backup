...
假设$K_3$系相对$K_2$系的速度为$v_1$（依然假设只有水平相对速度），“水平”坐标轴的夹角为$\theta_1$，那么其与$x$轴的夹角是多少呢？作图可知必是$\theta+\theta_1$。\footnote{由\autoref{eq_SRLrtz_3} 可得洛伦兹变换的速度叠加公式为：
$v_2=\frac{v+v_1}{1+vv_1/c^2}$，再结合双曲正弦的性质展开$\opn{tanh}(\theta+\theta_1)$亦可验证。}，这就很像相继旋转的结合了。
