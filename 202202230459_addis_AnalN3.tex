...
\item 11.12 如果集合 $X$ 存在子集（称为可测集）组成的 $\sigma$-环 $\mathfrak M$， 以及其上的一个非负可数可加集函数 $\mu$ （称为\textbf{测度}）， 就说 $X$ 是\textbf{测度空间}． 如果还有 $X\in \mathfrak M$， 那么 $X$ 称为\textbf{可测空间（measurable space）}．
