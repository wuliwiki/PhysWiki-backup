...
更一般地，可以定义**局部紧阿贝尔群** $K$ 的海森堡群，其中 $K$ 配备有一个 Haar 测度 [8]。这样的群有一个 **Pontrjagin 对偶群** $\hat{K}$，它由 $K$ 上所有取值于 $U(1)$ 的连续特征（characters）组成，并在赋予紧开拓扑后同样是一个局部紧阿贝尔群。

与局部紧阿贝尔群 $K$ 相关联的海森堡群，是希尔伯特空间 $L^{2}(K)$ 的酉群中的一个子群，该子群由来自 $K$ 的**平移算子**和来自 $\hat{K}$ 的**乘法算子**生成。

---

要不要我帮你补充解释一下，这里的构造为什么与经典的 **傅里叶变换** 紧密相关？
