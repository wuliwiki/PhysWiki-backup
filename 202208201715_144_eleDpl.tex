...
\begin{figure}[ht]
\centering
\includegraphics[width=5cm]{./figures/eleDpl_1.png}
\caption{电偶极子示意图} \label{eleDpl_fig1}
\end{figure}
给定两个等量异号$\pm q (q\ge 0)$、且间距始终为$r$的电荷，那么我们把这两个粒子构成的系统看成一个叫\textbf{物理电偶极子（physical electric dipole）}的对象．

一个物理电偶极子的总电荷量为 $0$，但是它有另一个性质叫\textbf{电偶极矩（electric dipole moment）}$\bvec p$，定义为
\begin{equation}
\bvec p = q\bvec r
\end{equation}
其中$\bvec r$是\textbf{从负电荷指向正电荷}的向量，数值上等于两电荷的间距．
