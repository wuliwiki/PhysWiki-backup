...
-\frac{1}{2m} \dv[2]{\psi_l}{r} + \qty[V(r) + \frac{l(l + 1)}{2mr^2}]\psi_l = E_{n,l}\psi_l
\end{equation}
对每个固定的 $l$， 该式可能存在若干个不同的能级， 每个能级可能对应多个束缚态． $n$ 就是这些能级的序号， 称为\textbf{主量子数（principal quantum number）}， 一般从 0 开始， 而 $l$ 叫做\textbf{角量子数（angular quantum number）}．

该方程的解取决于 $V(r)$ 的具体形式． 若 $V(r)$ 始终为有限值， 对于每个较小的 $l$， 可能不存在束缚态， 也可能存在有限个束缚态， 即有限个 $n$． 束缚态能量 $E_{n,l}$ 由这两个量子数共同决定． 随着 $l$ 不断增加， 方括号中的等效势能越来越浅， 最终会导致径向方程不存在束缚态解． 一个简单的例子是有限深球势阱\upref{FiSph}．
