...
\begin{corollary}{}
设$\overline{\mathbb{F}}/\mathbb{K}/\mathbb{F}$是代数扩域链，$\overline{\mathbb{F}}$是$\mathbb{F}$的代数闭包，$\sigma:\mathbb{K}\to\overline{\mathbb{F}}$是保$\mathbb{F}$的域单同态．
...
域单同态总把不可约多项式的一个根映射为另一根．参考后续词条中\autoref{FldExp_the4} 的证明过程，可知对于任意不可约多项式，只要确定了单同态单对其中一个根的映射，也就知道了所有根的映射了——换句话说，单同态是根的置换，但不是所有置换都可以．

将此推论中的$\mathbb{F}$当作$\mathbb{K}_1$，$\sigma(\mathbb{F})$当作$\mathbb{K}_2$，$\overline{\mathbb{F}}$当作$\mathbb{K}_1$和$\mathbb{K}_2$．
