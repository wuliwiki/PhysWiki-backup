...
这是李代数范畴中的积。注意，\( \mathfrak{g} \) 和 \( \mathfrak{g'} \) 在 \( \mathfrak{g} \times \mathfrak{g'} \) 中的副本彼此交换：

\[
[(x,0),(0,x')] = 0.
\]

设 \( \mathfrak{g} \) 是一个李代数，\( \mathfrak{i} \) 是 \( \mathfrak{g} \) 的一个理想。如果标准映射 \( \mathfrak{g} \to \mathfrak{g}/\mathfrak{i} \) 是可分裂的（即存在一个李代数同态的截面 \( \mathfrak{g}/\mathfrak{i} \to \mathfrak{g} \)），则称 \( \mathfrak{g} \) 是 \( \mathfrak{i} \) 和 \( \mathfrak{g}/\mathfrak{i} \) 的半直积，记作：
