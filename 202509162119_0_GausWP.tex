...
我们这里要讨论的是量子力学中单个粒子的一类特定的一维波函数 $\psi(x,t)$。 它描述的粒子的位置分布是一个\enref{高斯分布（正态分布）}{GausPD}。
...
\end{equation}
那么\enref{动量表象}{moTDSE}的波函数具有对称的形式\footnote{也可以把\autoref{eq_GausWP_1} 和\autoref{eq_GausWP_2} 同时除以常数 $\E^{\I p_0 x_0}$ 使\autoref{eq_GausWP_1} 最后的 $x$ 变为 $x-x_0$， \autoref{eq_GausWP_2} 最后的 $p-p_0$ 变为 $p$。 }% \addTODO{链接}
\begin{equation}\label{eq_GausWP_2} % 已验证
\psi (p,0) = \frac{1}{(2\pi\sigma_p^2)^{1/4}} \E^{-(p - p_0)^2/(2\sigma_p)^2} \E^{-\I\frac{x_0}{\hbar }(p - p_0)}~,
\end{equation}
其中 $\sigma_x, \sigma_p$ 分别为位置和动量概率分布的标准差，满足\enref{不确定原理}{Uncert}
\begin{equation} % 已验证
\sigma_x\sigma_p = \frac{\hbar}{2}~.
\end{equation}
让\autoref{eq_GausWP_1} 根据含时薛定谔方程（\autoref{eq_TDSE11_3}，令 $V\equiv 0$）演化，可得含时波函数为
...
让波包随时间变长的原因和产生啁啾的原因都是下面推导中的动能因子 $\exp(-\I k^2 t)$， 它使得高斯波包在 $t > 0$ 时不再是 \textbf{transform limited}，即不再恰好满足 $\sigma_x \sigma_p = \hbar/2$。
