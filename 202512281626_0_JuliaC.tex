...
\end{lstlisting}
其中 \verb`@__DIR__` 是当前路径。 注意 \verb`ccall` 是一个关键字， 不是函数。

\verb`:函数名` 的类型是 \verb`Symbol`。 第二个参数是返回类型， 第三个参数是函数的参数类型， 后面是具体的参数。 如果只有一个参数， 用 \verb`(类型,)` 即 1-Tuple。 没有参数时， \verb`()` 表示 0-Tuple。
