...
\item \verb`包名` 会在默认路径或者 \verb`CMAKE_MODULE_PATH` 的路径里面搜索名为 \verb`Find包名.cmake` 或者 \verb`包名Config.cmake` 的文件。 该文件通常会设置一些变量， 可以在 \verb`find_package` 结束后访问。
