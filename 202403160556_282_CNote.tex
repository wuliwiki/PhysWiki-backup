...
\item 宏 \verb`__STDC_VERSION__` 可以在程序中判断编译器的 C 标准。例如 \verb`199901L` 就是 \verb`c99` 标准， \verb`201112L` 就是 \verb`c11` 标准。 \verb`L` 表示 \verb`long int` 的 literal。 要打印如 \verb`printf("C version: %ld\n", __STDC_VERSION__);`
\end{itemize}
...
\subsection{struct, enum 和 union}
\begin{lstlisting}[language=cpp]
