...
\end{equation}
\begin{equation}
y-y_0={y_0\over p}(x-x_0)\quad\implies\quad y_0y=2p\left(\frac{x+x_0}{2}\right)~.
\end{equation}
...
这证明起来也并不困难。设抛物线的标准形式为 $y^2 = 2px$，则其焦点为 $\displaystyle F\left(\frac{p}{2}, 0\right)$，准线为 $\displaystyle l: x = -\frac{p}{2}$。取抛物线上一点 $P(x_0, y_0)$，连接 $PF$，作点 $P$ 到准线 $l$ 的垂线，垂足为 $\displaystyle Q\left(-\frac{p}{2}, y_0\right)$，连接 $FQ$，设 $FQ$ 与 $y$ 轴交于点 $R$，再连接 $PR$。
...
点 $F$ 的坐标为 $\displaystyle \left(\frac{p}{2}, 0\right)$，点 $Q$ 的坐标为 $\displaystyle\left(-\frac{p}{2}, y_0\right)$，则线段 $FQ$ 的斜率为
\begin{equation}
k_{FQ} = \frac{y_0 - 0}{-\frac{p}{2} - \frac{p}{2}} = \frac{y_0}{-p}~.
\end{equation}

由于 $FQ$ 与 $y$ 轴相交于点 $R$，代入 $x = 0$ 可得 $R$ 点坐标：
\begin{equation}
y = y_0 + k_{FQ}(0 - (-\frac{p}{2})) = y_0 - \frac{y_0}{p} \cdot \frac{p}{2} = y_0 - \frac{y_0}{2} = \frac{y_0}{2}~.
\end{equation}
即 $R = (0, \frac{y_0}{2})$。
...
另外$k_{PR}$与P处的切线斜率一样，由于$PR$过点P，因此PR即$P$处的切线。根据全等关系和对顶关系，显然有

\begin{equation}
\angle RPF=\angle RPQ=\angle M~.
\end{equation}
...
另一方面，点 $P$ 处的切线斜率可由 $y^2 = 2px$ 的导数得到：

又由于 $x_0 = \frac{y_0^2}{2p}$，代入得：
\begin{equation}
k_{\text{切}} = \frac{p}{y_0} = \frac{1}{\frac{y_0}{p}} = \frac{2x_0}{y_0} \Rightarrow k_{PR} = \frac{y_0}{2x_0} = \frac{1}{k_{\text{切}}}~,
\end{equation}
即 $PR$ 与切线互为垂线。

因此，$PR$ 是 $P$ 处的法线。又因为 $|FP| = |PQ|$，且 $\angle FPR = \angle QPR$，从而 $\triangle FPR \cong \triangle QPR$，于是 $FP$ 与 $PQ$ 关于切线对称。
