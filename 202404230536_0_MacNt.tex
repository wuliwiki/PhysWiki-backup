...
苹果会像在外部硬盘或网络硬盘拉出大量 \verb`.DS_Store` 和 \verb`._XXX`（这是为了使 metadata 兼容， \verb`XXX` 是对应的正常文件名），这些文件在 Finder 中默认隐藏，但其他系统可以看到。
\begin{itemize}
\item 其他系统中可以指定一个全局的 gitignore 文件， 如 \verb|git config --global core.excludesfile ~/.gitignore_global|，然后把恶心的 \verb`.DS_Store` 放进去。
...
\item 要停止在非苹果文件系统的硬盘生成 \verb`._XXX` 文件，用 \verb`defaults write com.apple.desktopservices AppleDouble -bool false`
