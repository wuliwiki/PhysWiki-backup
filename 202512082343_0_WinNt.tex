...
\item Win 11 装系统的时候会要求登录在线账号，若想用本地账号用 \verb`Shift + F10` 打开 cmd， 输入 \verb`OOBE\BYPASSNRO` 然后按回车，自动重启后就可以选择没有网络。
...
\item 若控制面板的电源选项里面没有 High Performance，命令行输入 \verb`powercfg /list` 查看，若还是没有，用 \verb`powercfg -duplicatescheme 8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c` 然后 \verb`powercfg -setactive 8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c`。 再次 \verb`powercfg /list` 应该就有了。重新进入控制面板也有了。
\end{itemize}
...
\item 如果控制面板删除程序的时候，uninstaller 报错导致删不掉，就手动删掉程序文件夹，然后在任务管理器中找到 uninstaller 的进程所在的文件，把该文件删掉。再 uninstall，就会问你要不要从列表中移除。
\end{itemize}
...
\subsection{中文乱码问题}
\begin{itemize}
\item 修改区域语言，在开始菜单中打开控制面板，找到“区域和语言”这一项。
2、进入区域和语言窗口后，先选择“格式”这个选项卡，在这个界面，把格式设置为“中文(简体，中国)”。
\item 接着再打开“位置”选项卡，在这个界面，把当前位置设置为“中国”这一项。
\item 再打开“管理”这个选项卡，在非Unicode程序语言下选项下，点开“更改系统区域设置”按钮，设置为“中文(简体，中国)”，确定。
\item 这一项重要，Unicode在程序里为宽字符，占2个字符，很多软件都是英文编程的，占一个字符，如果系统没有设置为中文(简体，中国)，即(Unicode程序语言)，那些非Unicode编写的程序，如果显示中文时，就会显示乱码。
\item 如果以上方法均不行，就要使用下面的设置即可（我就是设置这里才成功的）
\item 更改系统区域设置- 把 Beta版：使用Unicode UTF-8 提供全球语言支持 这个复选框的勾选去掉，再重启系统，就成功了
\item 如果字体还是怪怪的甚至一些软件出现方块乱码，那就是缺少字体，可以装一个 office 中文拓展包解决（在 word 里面打几个中文就会提示安装）。
\end{itemize}
