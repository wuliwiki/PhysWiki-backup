...
对于一元实函数$y=f(x)$， $x$只有正负两个方向趋于零; 但对于复函数$w=f(z)=f(x+iy)$，$z$可以从复平面$x-y$上的任意路径趋于零，因此复变函数可导的条件更复杂．\autoref{CauRie_eq4} 会让我们联想到二元函数的方向导数\upref{DerDir}, 但由于\autoref{CauRie_eq4} 中的除法是复数的除法， 所以它比方向导数\upref{DerDir}要更复杂一些．
