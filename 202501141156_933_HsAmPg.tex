...
\autoref{ex_HsAmPg_3} 是另一个常见的判断方法。事实上，对于等差数列，易证知道任意一项即可通过公差推出所有其他项，满足关系式：
\begin{equation}
a_n=a_k+(n-k)d~.
\end{equation}
由此还能推导出另一个重要关系：
...
d={a_n-a_k\over n-k}~.
\end{equation}
这与\aref{平均变化率}{def_HsFunC_3}或\aref{斜率}{def_HsFunC_5}的定义极为相似。其实，如果将等差数列的通项公式变形可以得到：
\begin{equation}
a_n=dn+(a_1-d)~.
\end{equation}
...
另外，根据\autoref{eq_HsAmPg_1} 还能得到等差数列的一个重要性质，取四个整数，满足$m+n=p+q$，即$p-n=m-q$，那么根据\autoref{eq_HsAmPg_1} 有：
\begin{equation}
