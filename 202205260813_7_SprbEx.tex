% 可分扩张
% keys 可分多项式|完全域|完备域|形式微商|微商|导数|形式导数|separable polynomial|重根|自同构|形式微分
...
\begin{theorem}{}\label{SprbEx_the4}
...
由\autoref{SprbEx_the3} ，存在\textbf{可分}的不可约多项式$h(x)\in\mathbb{F}[x]$和正整数$k$，使得$f(x)=h(x^{p^k})$．
...
\autoref{SprbEx_eq1} 的最后两步，应用了\autoref{SprbEx_lem1} ．这么一来，$f(x)$就可约了，和“不可约”的假设矛盾．故反设不成立，故不存在不可分的不可约多项式，也就是说$\mathbb{F}$是完备域．
...
由\autoref{FldExp_cor2}~\upref{FldExp}，$\mathbb{K}$的代数扩张都是$\mathbb{F}$的代数扩张，从而是$\mathbb{F}$的可分扩张，从而是$\mathbb{K}$的可分扩张．
...
\begin{example}{\autoref{SprbEx_cor5} 的正例}
...
\begin{example}{\autoref{SprbEx_cor5} 的反例}

考虑$\mathbb{Z}_p$的单超越扩域$\mathbb{Z}_p(\pi)$．则$x^p-\pi$是$\mathbb{Z}_p(\pi)$上的不可约多项式\footnote{考虑到$\mathbb{Z}_p(\pi)$是整环，且$\pi$是其素元素，应用\textbf{爱森斯坦判别式}\upref{EsstCr}即可．}，但$\opn{D}(x^p-\pi)=0$，从而是一个不可分的不可约多项式．因此，$\mathbb{Z}_p(\pi)$不是完备域．
