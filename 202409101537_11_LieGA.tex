...
[X_p, Y_p]=[X, Y]_p~.
\end{equation}
可以代入切场计算得证，这样定义的李括号具有双线性和Jacobi 结合性。由习题1可知，李括号运算对左不变切场是封闭的，因此 $T_p G$ 是一个李代数，即\textbf{李群的李代数}。由于左不变切场同构于切空间，且$l_p^*(X)=X_p$，因此无论$p$点取何值，都不改变左不变切场之间的李代数关系。一般我们用$\opn{Lie}(G)$表示李群上全体左不变切场构成的李代数。
\end{definition}
