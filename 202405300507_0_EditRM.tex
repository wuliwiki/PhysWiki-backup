...
\item 若一篇文章被某个作者编辑， 则该文章被该作者占用。 其他用户无法修改该文章，只能以只读模式打开，除非有 “占用管理” 权限。 当文章被取消占用或被发布时，才会取消占用。目前，管理员会每天对所有新建和被修改的文章进行粗略的审核，并对其进行发布（即使处于草稿阶段）。
\item 新建的文章不会自动出现在目录中，因为程序不可能知道应该把它放在目录的什么位置，需要手动编辑 \verb`main.tex`。 该文件和参考文献列表 \verb`bibliography.tex` 都会每 6 分钟会自动取消占用。
\end{itemize}
...
\item 最开始的加粗、斜体、大标题、小标题按钮，可以在您选中一段文字后按下，给这段文字添加相应格式。 也可以按下以后再填写需要的文字。
\item 编辑器会将有改动的文章每隔 5 分钟备份一次， 可以用 “恢复” 按钮（红色的时钟）查看历史版本，可以勾选其中两个进行对比，也可以点击其中一个进行恢复。
\item 编辑器支持各种自动引用（被引用对象没有 label 时会自动插入 label）， 工具栏上的\textbf{内部引用}按钮（实心双引号）可以引用同一文章的公式， 图表， 例题，子节等。 \textbf{外部引用}按钮（空心双引号）可以引用其他文章的各种环境。
\item 如果要在网页预览和 LaTex 代码之间跳转到对应位置， 可以通过搜索关键词实现。 例如在预览窗口复制一段文字， 在编辑窗口搜索就可以跳转到对应内容。（更新：现在可以直接选中后用 \verb`Alt` + \verb`F` 或者定位按钮）
\item 预备知识按钮用于插入预备知识列表 \verb`\pentry{知识点1\nref{节点id1}，知识点2\nref{节点id2}}{节点id}`。 一个预备知识列表定义了\href{https://wuli.wiki/tree/}{知识树}中的一个节点（简单说就是知识点），它的范围是它到下一个预备知识列表之间的全部内容。 该节点的 ID 就是 \verb`节点id`。 如果读懂该节点中的内容，\textbf{必须}先弄懂一些\textbf{其他文章}中的节点，就在第一个花括号中把它们列出来。 这里的 \verb`\nref{}` 命令可以通过 “外部引用” 按钮中的 “节点” 按钮来插入。 注意每篇文章无论是否存在预备知识列表，都会有一个默认的节点 \verb`nod_XXX` 其中 \verb`XXX` 是该文章的 \verb`文章id`。
\end{itemize}
