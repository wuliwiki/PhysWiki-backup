...
SLISC 库提供了两种特殊的文件格式用于储存各种变量和矩阵， 每个文件可以存如多个不同的变量（包括矩阵）并指定名称， 读取时可以指定要读取的变量名。 两种文件分别使用拓展名 \verb|.matt| 和 \verb|.matb|。 其中 \verb|mat| 表示 matrix， \verb|t| 表示 text， \verb|b| 表示 binary。 顾名思义， 第一种是文本文件， 第二种是二进制文件。 二进制文件具有占用硬盘空间少， 读写速度快的优点。 文本文件可以方便使用其他语言读取。
