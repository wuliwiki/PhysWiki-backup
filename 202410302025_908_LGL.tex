...
\]
其中矩阵定义为\(W_{ij} = \frac{\partial^{2} L}{\partial \dot{q}_{i} \partial \dot{q}_{j}}.\)如果矩阵 \( W \) 是非奇异的，上述方程可以被求解，以将 \( \ddot{q} \) 表示为 \( (\dot{q}, q, t) \) 的函数。如果矩阵是不可逆的，则无法将所有的 \( \ddot{q} \) 表示为 \( (\dot{q}, q, t) \) 的函数，并且哈密顿运动方程也不会呈现标准形式。