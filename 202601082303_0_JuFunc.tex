...
\item 也可以定义函数模板： \verb`my_fun(a::Vector{T}) where T = (a[2])`； 使用： \verb`my_fun([1,2,3])` 返回 \verb`2`。
...
\begin{table}[ht]
\centering
\caption{算符和对应的函数（\href{https://docs.julialang.org/en/v1/manual/functions/}{官方文档}）}\label{tab_JuFunc_1}
\begin{tabular}{|c|c|}
\hline
Expression & Calls \\
\hline
...
\item \verb`function fun(a,b,x...)` 可以定义任意变量个数的函数。 在函数体内 \verb`x` 是一个 tuple， 包括后面的所有变量。
\item 调用函数的时候， \verb`fun(a, bb..., c; dd...)` 中的 \verb`...` 叫做 splat，就是把 Tuple 等的每个元素展开一个入参。
...
\item \verb` fun() = (a = 1; b = 2; return a + b);` 右边用括号囊括多条命令。
\end{itemize}
...
\item \verb`identity(val)` 会返回输入的任何对象。
\end{itemize}
...
\end{lstlisting}
\begin{itemize}
\item \verb`@noinline` 禁止内联
\end{itemize}


\subsection{可变入参数、变量名展开}
\begin{itemize}
\item 函数定义： \verb`myfun(a, b = 1, others...; b, c = 2, kw_others...) = ...`。
\item 注意默认值只能对最后几个未知入参用，但可以对任意关键字入参使用。
\item 默认参数也可以是一个表达式，在每次被调用且未提供该实参时执行。
\item 在函数体中， \verb`others` 的类型是 \verb`Tuple`， \verb`kw_others` 的类型是 \verb`Base.Pairs`（几乎可以当作只读的字典使用），见\enref{数据类型笔记}{JuType}.
\end{itemize}
