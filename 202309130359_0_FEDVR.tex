...
再看含 bridge function 的矩阵元：
\begin{equation} \ali{
D_{(im), (iN)} &= \frac{\sqrt{w_{0m}}}{\sqrt{a_i} \sqrt{a_i + a_{i+1}}} f'_N(t_m)
\qquad (1< m < N)~,
} \end{equation}

\begin{equation} \ali{
D_{(iN), (iN)} &= \frac{\sqrt{w_{01}}}{a_i + a_{i+1}} \qty[f'_N(t_N) + f'_1(t_1)]~,
} \end{equation}

\begin{equation} \ali{
D_{(iN), (i+1,n)} &= \frac{\sqrt{w_{01}}}{\sqrt{a_{i+1}} \sqrt{a_i + a_{i+1}}} f'_n(t_1)
\qquad (1< n < N)~,
} \end{equation}

\begin{equation}\label{eq_FEDVR_5} \ali{
D_{(iN), (i+1,N)} &= \frac{\sqrt{w_{01}}}{\sqrt{(a_i + a_{i+1})(a_{i+1}+a_{i+2})}} f'_N(t_1)~.
} \end{equation}
