...
# will skip files with long names, and list them in the end,
# rename and run again
def encrypt_names_in_folder(directory, password):
	...
	if not long_names:
		print('======================================================')
		print('WARNING: the following names are too long and skipped:')
		print('======================================================')
	...
	for root, dirs, files in os.walk(directory, topdown=False):
		for name in files:
			if (name[-4:] != '.dEc'):
			 	continue
			path_old = os.path.join(root, name)
			name_new = decrypt_str(name[:-4].replace('-', '/'), password)
			print(path_old, ' -> ', name_new)
			os.rename(path_old, os.path.join(root, name_new))
		for name in dirs:
			if (name[-4:] != '.dEc'):
