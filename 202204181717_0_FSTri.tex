...
\subsection{任意区间的展开}
有时候我们希望对函数 $f(x)$ 在某个任意区间 $[a, b]$ 做类似\autoref{FSTri_eq1} 那样的展开， 却并不关心 $f(x)$ 或者它的展开在 $[a, b]$ 外是否有周期性， 甚至是否有定义． 那么此时我们将有多种可能的选择．

第一种方法是令 $g(x)$ 是周期为 $2l$ 的周期函数， 且在区间 $[a, b]$ 内 $g(x) = f(x)$． 令\autoref{FSTri_eq1} 中 $l = (b-a)/2$， 那么容易证明（留做习题）只需要把系数公式的积分区间改为 $[a,b]$ 即可
\begin{equation}
...
第二种方法是令\autoref{FSTri_eq1} 中 $l = (b-a)/2$， 把 $f(x)$ 先向左平移 $x_0 = (a+b)/2$， 也就是使原来 $[a,b]$ 的中点移动到坐标原点， 展开成\autoref{FSTri_eq1} 的傅里叶级数， 再向右平移 $x_0$ 即可． 这样傅里叶展开变为
\begin{equation}
...
\end{equation}
类似地， 也可以只用余弦函数展开， 把 $\sin$ 替换为 $\cos$ 即可．

若

把函数 $f(x)$ 向左平移 $a$， 使 $[a, b]$ 移动到 $[0, l]$． 然后令 $g(x)$ 是
