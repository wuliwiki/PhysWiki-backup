...
我们称一个多线性映射 $f: V^{\times k} \to W$ 为\textbf{对称多线性映射}（简称\textbf{对称映射}）如果它是 $S_k$-不变的（参考\autoref{def_GrpRep_1}~\upref{GrpRep}），即对任意置换 $\pi \in S_k$，
\begin{equation}
f(R_\pi(v)) = f(v)~.
\end{equation}
...
类似的我们可以定义的\textbf{反对称多线性映射} （简称\textbf{反对称映射}）$f: V^{\times k} \to W$ 如果它满足 $\pi \in S_k$，
\begin{equation}
