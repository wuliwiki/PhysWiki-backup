...
\begin{figure}[ht]
\centering
\includegraphics[width=5cm]{./figures/Spin_1.pdf}
\caption{请添加图片描述} \label{Spin_fig1}
\end{figure}
物理学的理论发展往往是由实验和现象所推动的，因此再直接进入抽象的自旋理论之前，我们可以先了解最初斯特恩–格拉赫实验实验观察到的“异常”现象．在\textbf{斯特恩–格拉赫（Stern-Gerlach）}实验中，一束银原子朝一个方向（我们设这个方向为 $\bvec y$ 轴方向）发射并穿过不均匀的磁场．除了有力矩（类比经典电磁理论中的\upref{EBLoop}），还有另外一个力作用在磁偶极子上（类比经典电磁理论中的\upref{EBTorq}）：
...
$S_x$的可能值由此看来和$S_z$是一样的，同样我们可以计算出本征旋量：
\begin{equation}
\frac{\hbar}{2}\pmat{0\quad \I\\-\I\quad0}\pmat{\alpha\\\beta}=\pm\frac{\hbar}{2}\pmat{\alpha\\\beta} \Rightarrow -\I\beta=\pm\alpha
\end{equation}
接下来由旋量必须归一化性质可得：
\begin{equation}
...
接下来由本征旋量可得：
\begin{equation}
c_+^{(x)}=\left(\chi_+^{(y)}\right)\chi=\frac{1}{3}\frac{1}{\sqrt{2}}\pmat{1\quad 1}\pmat{1-2\I\\2}=\frac{3-2\I}{3\sqrt{2}}\Rightarrow |c_+^{(x)}|^2=\frac{13}{18}
\end{equation}
因此可能值$+\hbar/2$的概率为$13/18$．接下来类似地我们继续计算可能值$-\hbar/2$的概率为$5/18$：
\begin{equation}
c_-^{(x)}=\left(\chi_-^{(x)}\right)\chi=\frac{1}{3}\frac{1}{\sqrt{2}}\pmat{1\quad -1}\pmat{1-2\I\\2}=\frac{1+2\I}{3\sqrt{2}}\Rightarrow |c_+^{(x)}|^2=\frac{5}{18}
