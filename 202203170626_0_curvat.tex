...
\footnote{参考 Wikipedia \href{https://en.wikipedia.org/wiki/Curvature}{相关页面}}我们来看一个平面上的一个光滑曲线（即处处存在切线）， 我们如何描述它某点处的弯曲程度呢？ 一种常用方法是在这点附近取曲线的一小段， 然后做一个尽量与它吻合的圆， 当这小段的长度趋近于 0 时， 这个圆可以唯一确定． 我们把这个圆叫做\textbf{密切圆（osculating circle）}， 把密切圆的半径叫做曲线在该点的\textbf{曲率半径（radius of curvature）}， 曲率半径的倒数 $1/R$ 叫做\textbf{曲率（curvature）}． 下面我们来具体定义曲率半径．
\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{./figures/curvat_1.pdf}
\caption{请添加图片描述} \label{curvat_fig1}
\end{figure}

我们先来看一个半径为 $R$ 的圆的一小段圆弧， 令其长度为 $\Delta l$． 作这段圆弧两端的切线， 令它们的夹角为 $\Delta \theta$， 那么显然满足 $R \Delta\theta = \Delta l$． 同理对于\autoref{curvat_fig1} 中光滑曲线上一点， 取该点附近长度为 $\Delta l$ 的一段， 在其两端分别作切线， 并分别作切线的垂直线， 那么当 $\Delta l \to 0$ 时两个垂直线的交点就是密切圆的圆心． 我们也可以做相同的方法定义曲率半径 $R$， ， 但需要
