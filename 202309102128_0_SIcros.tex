...
从\autoref{eq_HionCr_5}~\upref{HionCr} 来看， 若电子从束缚态 $\ket{j}$ 跃迁到另一个束缚态 $\ket{i}$， 跃迁概率（即吸收能量）和光谱中的 $\omega_{ij}$ 的强度成正比。 所以自然地，可以用以下思路定义\textbf{束缚态之间的跃迁截面}： 想象有许多原子，每个跃迁需要的能量为 $\omega_{ij}$，所以平均每个原子吸收的能量为
\begin{equation}
