...
我们还可以将微分形式局部表示为指标形式，比如在$U\subseteq M$上给定基$\{\uvec{e}_i\}$后，设$X=x^i\uvec{e}_i$和$Y=y^i\uvec{e}_i$，则将$f$表示为$f_i$、$g$表示为$g_i$后有$f(X)=f_ix^i$．此时还可以将$1$-形式的楔积和外微分表示为
\begin{equation}
f_i\wedge g_j=f_ig_j-f_jg_i
\end{equation}
和
\begin{equation}
(\dd f)_{ij}=\nabla_{\uvec{e}_i}f_j-\nabla_{\uvec{e}_j}f_i
