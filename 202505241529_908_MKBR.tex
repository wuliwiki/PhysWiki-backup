...
$$
速度矢量的每个分量都服从正态分布，其均值为：
$$
\mu_{v_x} = \mu_{v_y} = \mu_{v_z} = 0
$$
标准差为：$\sigma_{v_x} = \sigma_{v_y} = \sigma_{v_z} = \sqrt{ \frac{k_{\text{B}} T}{m} }$因此，整个速度向量服从一个三维正态分布，这是多元正态分布的一种特例，其均值为：$\mu_{\mathbf{v}} = \mathbf{0}$协方差矩阵为：

$$
\Sigma_{\mathbf{v}} = \left( \frac{k_{\text{B}} T}{m} \right) I
$$

其中 $I$ 是 $3 \times 3$ 的**单位矩阵**。
