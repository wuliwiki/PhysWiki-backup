...
\subsection{一阶近似中简单的情况}
大多数情况下我们只使用一阶近似，一种较简单的情况是，若初始波函数为某一个 $\ket{\psi_j}$， 且 $H'$ 中的时间可以分离出来
\begin{equation}
H' = W f(t)
\end{equation}
令 $W_{ij} = \mel{\Psi}{W}{\Psi}$， 有
\begin{equation}
H'_{ij}(t) = W_{ij}f(t)
\end{equation}
此时一阶微扰公式（\autoref{TDPT_eq9}） 变为 $f(t)$ 的反傅里叶变换\upref{FTExp}．
\begin{equation}
c_i^{(1)}(t) = \frac{W_{ij}}{\I\hbar} \int_0^t f(t)\E^{\I\omega_{ij} t} \dd{t}
\end{equation}
所以在一阶近似中，波函数在 $t$ 时刻出现在 $\ket{\psi_i}$ 的概率约为
\begin{equation}
P_{i}(t) = \abs{c_j^{(1)}(t)}^2
\end{equation}
