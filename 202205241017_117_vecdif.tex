...
\]
则称 $F(x)$ 在 $x=p$ 处 \textbf{可微}, 将 $F(x)$ 在 $x=p$ 处的 \textbf{全微分} (简称 \textbf{微分}) 记作  $\mathrm{d}F(p)$， 同时将线性变换 $A$ 称为 $\mathrm{d}F(p)$ 的主部， 写成  $\mathrm{d}F(p)=A \mathrm{d}x,$ 其中 $\mathrm{d}x=(\mathrm{d}x_1,\mathrm{d}x_1,\cdots,\mathrm{d}x_n).$ 
\end{definition}
