...
我们把\textbf{不需要任何作用}的数矩阵称为 $(0,0)$ 型张量，把\textbf{需要一次列向量作用}的行矩阵称为 $(0,1)$ 型张量，把\textbf{需要一次行向量作用}的列矩阵称为 $(1,0)$ 型张量，把\textbf{需要一次列向量作用和一次行向量作用}方列矩阵称为 $(1,1)$ 型张量．

注：这里我不称呼它们为 $(s, t)$ 型矩阵是因为这很容易和 $s \times t$ 型矩阵搞混，由于我还没有讲什么是张量，如果读者自认不会混淆，可以称呼它们为 $(s, t)$ 型矩阵．

很自然的我们就会考虑， $s$ 和 $t$ 如果不是 $0$ 或 $1$，我们能把 $(s, t)$ 型张量定义成\textbf{需要 $t$ 次列向量作用和 $s$ 次行向量作用}的
