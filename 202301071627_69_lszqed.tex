...
\begin{equation}
\begin{aligned}
&\epsilon_\mu^{(\lambda)*}(\bvec k)g_{\lambda\lambda}\int_{\Rnum 1} \dd[4]{x} e^{ikx} \bra{\Omega}T[A^\mu(x)\cdots]\ket{\Omega}
\stackrel{k^0\rightarrow E_{\bvec k}}{\sim}
\frac{i\sqrt{Z_3}}{k^2+i\epsilon} \bra{\bvec k,\lambda} T[\cdots]\ket{\Omega}\\
&
\epsilon_\mu^{(\lambda)}(\bvec k)g_{\lambda\lambda}\int_{\Rnum 3} \dd^4{y} e^{-iky}
...
\begin{enumerate}
\item 入射光子：$\wick{\c1A^\mu(x) \c1{\ket{\bvec k,\lambda}} }=\epsilon^{(\lambda)\mu}(\bvec k)$。
\item 出射光子：$\wick{\c1{\bra{\bvec k',\lambda'}}\c1A^\nu(x) }=\epsilon^{(\lambda)\nu*}(\bvec k')$。
\end{enumerate}
