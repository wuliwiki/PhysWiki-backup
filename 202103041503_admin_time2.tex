...
如果把一年中的视太阳时做一个线性拟合， 就得到了\textbf{平太阳时（mean solar time）}． 平太阳时和 UT1 几乎相等（除了每百年增加 1.7ms 误差）． 当前的平太阳日约为国际单位的 $86400.002\mathrm{s}$． 视太阳时和平太阳时之间的差距叫做\textbf{均时差（equation of time）}

\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{./figures/time2_1.pdf}
\caption{均时差} \label{time2_fig1}
\end{figure}
