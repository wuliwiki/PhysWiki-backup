...
\red{新：}对于有限维度矢量空间来说，我们可以用对偶空间 $V^*$ \upref{DualSp}和多线性映射空间 $\mathcal L(V_1,V_2;\mathbb F)$ \upref{MulMap}来定义来定义它们之间的张量积。
...
在张量的张量积\upref{TsrPrd}中我们已经定义过线性映射（对偶向量）的张量积了：考虑 $f \in V_1^*,g \in V_2^*$，我们有 $(f \otimes g)(v_1, v_2) = f(v_1) f(v_2)$，其中 $f \otimes g \in \mathcal L(V_1,V_2;\mathbb F)$ 是双线性映射。
...
主空间的双线性函数空间 $\mathcal L(V_1,V_2;\mathbb F)$ 就是对偶空间的张量积：
