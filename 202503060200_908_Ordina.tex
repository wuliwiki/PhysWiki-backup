...
- **自然和**和**自然积**都是**交换的**（commutative）和**结合的**（associative）。  
- **自然积**对**自然和**是分配的**（distributive），即：
\[
\alpha \otimes (\beta \oplus \gamma) = (\alpha \otimes \beta) \oplus (\alpha \otimes \gamma)
\]

这些运算还具有单调性（monotonic），具体来说：
    - 如果\(\alpha < \beta\)，那么：
    \[
    \alpha \oplus \gamma < \beta \oplus \gamma
    \]
    - 如果\(\alpha \leq \beta\)，那么：
    \[
    \alpha \otimes \gamma \leq \beta \otimes \gamma
    \]
    - 如果\(\alpha < \beta\)且\(\gamma > 0\)，那么：
    \[
    \alpha \otimes \gamma < \beta \otimes \gamma
    \]
