...
这是一个简单的、但却\textsl{有点难以理解}的结论．在处理一些刁钻的问题时，局部保号性偶尔会派上用场．
...
\end{figure}
如\autoref{FunLim_fig9} 所示（其实就是\autoref{FunLim_fig7} ），我们可以取一个$0< \epsilon < A$，极限的定义保证了我们总能找到对应的$\delta$．且在$(x_0-\delta, x_0+\delta)$区间内，$f(x)>0$．
