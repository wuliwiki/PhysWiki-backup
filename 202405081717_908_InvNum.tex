...
逆序数主要的应用有定义\enref{行列式}{Deter}和\enref{列维—奇维塔（Levi-Civita）符号\upref{LeviCi}。 在这两个应用中， 我们只对逆序数的奇偶性感兴趣。 我们把逆序数为奇数的排列叫做\textbf{奇排列}， 逆序数为偶数的排列叫做\textbf{偶排列}。 把按从小到大的顺序排列叫做\textbf{标准排列}。 标准排列逆序数为零， 是一个偶排列。
