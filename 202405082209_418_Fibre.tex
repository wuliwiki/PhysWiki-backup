...
\begin{definition}{纤维丛}
	\textbf{纤维丛(biber bundle)}是一四元组 $(E, B, \pi, F)$, 其中 $E, B$ 与 $F$ 都是\textbf{拓扑空间}, 仍称 $B$ 为\textbf{底空间(base space)} 而 $E$ 为\textbf{全空间(total space)}, 而称 $F$ 是\textbf{纤维型(fiber type)}. 仍称 $\pi$ 是一个\textbf{投影映射(projection map, bundle projection)}, 但要求 $\pi: E\rightarrow B$ 是一个\textbf{连续满射}, 满足下述的\textbf{局部平凡条件(local triviality condition)}.
	下面我们假设 $B$ 是连通的.

	要求 $\forall b \in B$, 存在一个在 $B$ 中的包含 $b$ 的\textbf{开邻域} $U$, 且有同胚映射 $\varphi: \pi^{-1}[U] \rightarrow (U \times F)$, 显然 $U \times F$ 是一个乘积拓扑.
...
\end{CD}\\
	...
	对于开覆盖 $\{(U_i, \varphi_i)\}$ 称为这个丛的\textbf{局部平凡化(local trivialization)}.

\end{definition}

\begin{corollary}{纤维丛上的纤维}
根据纤维上的定义, 
\end{corollary}
