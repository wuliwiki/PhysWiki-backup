...
\pentry{定态薛定谔方程\upref{SchEq}， 二阶常系数齐次微分方程\upref{Ode2}， 函数平移\upref{FunTra}}
...
\subsection{讨论与拓展}
一维定态薛定谔方程的解的一般性质（无论势能函数 $V(x)$ 是什么）：
\begin{itemize}
\item 不存在任何简并。
\item 任何关于某点 $x_0$ 对称的势能函数 $V(x)$ 都会导致波函数是关于 $x_0$ 的奇函数或偶函数（证明略）。
\end{itemize}
