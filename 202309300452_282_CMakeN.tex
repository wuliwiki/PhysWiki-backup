...
\item 如果字符串 literal 中没有空格或者特殊符号，两边的双引号可以省略。 
...
\item 变量是区分大小写的。
\item \verb`set` 也可以把许多变量变成一个 list， 如 \verb`set(Foo aaa bbb ccc)` （等效于 \verb|set(Foo "aaa" "bbb" "ccc")|） 将 \verb`Foo` 变成 list，其实就是单个字符串 \verb|aaa;bbb;ccc|。
