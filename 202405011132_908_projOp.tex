...
令 $M$ 为 $N$ \enref{维内积空间}{InerPd} $X$ 的子空间。 那么任意 $u\in X$ 有唯一的正交分解：
...
\enref{三维几何向量}{GVec} 空间 $X$ 中， 一个过原点的平面是它的一个子空间 $M \subset X$， 该平面过原点的法线就是它的正交补空间 $M^\bot$。  任意 $\bvec u \in X$ 可以唯一地正交分解成 $M$ 上的一个向量 $\bvec v$ 和法线上的一个矢量 $\bvec w$， 且满足勾股定理 $\bvec u^2 = \bvec v^2 + \bvec w^2$。
\addTODO{图}
