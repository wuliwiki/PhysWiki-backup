...
\item \verb`du /some/path` 用于查看指定目录中所有子文件夹的大小（单位默认为 block，512 或 1024 字节）, \verb`-a` 选项可以显示文件夹和文件. \verb`--max-depth=N` is used to control the level of subfolders to display, \verb`-h` is for human readable file size (\verb`Mb`, \verb`Gb`, etc)
