...
\begin{example}{}
某一维势阱中有两个互不作用的全同粒子， 我们只考虑束缚态， 单个粒子在该势阱中可能有 $N$ 个正交归一的束缚态 $\psi_i(x)$ （$i=1,2,\dots, N$）， 它们可以看作是 $N$ 维希尔伯特空间 $\mathcal V$ 中的基底．

那么双粒子波函数将处于张量积空间 $\mathcal V\otimes \mathcal V$ 中． 该空间一组基底数学上可以取 $\psi_i(x_1) \psi_j(x_2)$ （$i,j=1,2,\dots, N$） 但当 $i\ne j$ 时这些函数基底显然不满足交换对称或反对称．
