...
先来回顾为什么要把散射态做归一化。 若我们知道初态波函数 $\psi(x, 0)$， 要求解接下来波函数如何演化，一种比较解析的办法就是类似自由粒子\upref{GausWP}那样，先把它投影到不同束缚态和散射态上，也就是定态薛定谔方程的能量本征态上。我们知道这些能量本征态关于时间的演化只是乘以一个相位因子 $\E^{-\I Et}$，所以在一定shi
