...
\end{lemma}
\subsection{磁矢势的多级展开}
类似于电多级展开的讨论，我们考虑一个局域在原点附近的电流密度分布 $\bvec J(\bvec x^{\prime})$ 在远离电流区域所产生的磁矢势．
% 链接未完成
\begin{equation}
\bvec A(\bvec x)=\dfrac {\mu_0}{4\pi} \int_V \dfrac{ \bvec J(\bvec x^{\prime})\dd V^{\prime}}{|\bvec x - \bvec x^{\prime}|}\label{edy33_eq1}
\end{equation}
势能零点在无穷远点，而电流分布在小区域 $V$ 内．为了计算远处任意一点 $\bvec x$ 处的磁矢势（ $R$ 为 $\bvec x$ 到体积 $V$ 内的势能零点的距离）
\begin{equation}
