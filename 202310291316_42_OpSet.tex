...
f_{X\cup Y}(z)&=f_X(z)\cup f_Y(z)~.
\end{aligned}
\end{equation}
\end{theorem}
\textbf{证明：}以第一式证明为例：
若 $f_{X\cap Y}(z)=1$，则由\autoref{eq_OpSet_3} ，$z\in X\cap Y$，由\autoref{def_OpSet_3} 和\autoref{def_OpSet_5} ，有
\begin{equation}
z\in X\land z\in Y=f_X(z)\land f_Y(z)=1~.
\end{equation}
