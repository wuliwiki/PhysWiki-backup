...
想象一下这样一个场景，老师要求你计算从1到100的所有自然数的和，这时，你旁边的一个名叫高斯的同学脱口而出5050。当然，这个故事里，你不是高斯，而是他旁边那个看起来笨笨的同学。你和其他的同学一样，从$1$开始一个一个往上加。当然，这篇文章的内容不是介绍如何快速的计算这个和的，否则你就应该附身到高斯身上了。下面要介绍的求和符号将只用一个比较简短的记号符号就描述之前你的求和过程，对于计算机随处可见的现在，这样的操作不再是笨笨的，而是具有相当广泛的实际意义的。
它的好处有以下四点：
求和符号的写法可以快速转换成计算机语言
求和符号的适用范围非常广，绝不仅限于连续自然数的和
在对性质熟练掌握后，可以快速化简
与定积分联系紧密，二者很容易转写
，而且。而是我们怎么样把这件事情用一个符号来表示出来，就是如何才能把这个一个一个往上加的行为用一个比较简短的记号来表示出来。
