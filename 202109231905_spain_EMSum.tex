...
\end{tabular}
\end{table}
实际上，通过一定的计算可以发现似乎对于任意的$n\in\mathbb N^+$,有$B_{2n+1}=0$．下面我们借助复变函数理论给出一个较为简洁的证明.\\
不妨设辅助函数
\[
...
由留数定理可得
\[
\oint_{C} \frac{f(z)}{z^{2 n+2}} d z=2 \pi i \cdot \frac{B_{2 n+1}}{(2 n+1) !}
\]
于是就有
\[
B_{2 n+1}=\frac{(2 n+1) !}{2 \pi i} \oint_{C} \frac{f(z)}{z^{2 n+2}} d z
\]
应用Euler公式代换 $z=\E^{\mathrm i\theta}$,则有
$$
\begin{aligned}
&B_{2n+1}=\frac{(2n+1) !}{2\pi\mathrm i} \int_{0}^{2 \pi} \frac{\E^{i \theta}}{e^{\E^{i \theta}}-1} e^{-2 n i \theta-2\mathbrm i \theta} i e^{i \theta} d \theta=\frac{(2 n+1) !}{2 \pi} \int_{0}^{2 \pi} \frac{e^{-2 n i \theta}}{e^{i \theta}-1} d t \\
&=\frac{(2 n+1) !}{2 \pi} \int_{0}^{\pi} \frac{e^{-2 n i \theta}}{e^{e^{i \theta}}-1} d \theta+\frac{(2 n+1) !}{2 \pi} \int_{0}^{\pi} \frac{e^{-2 n i \theta+2 n \pi i}}{e^{e^{i \theta}}-1} d \theta \\
&=\frac{(2 n+1) !}{2 \pi} \int_{0}^{\pi} \frac{e^{-2 n i \theta}}{e^{e^{i \theta}}-1} d \theta+\frac{(2 n+1) !}{2 \pi} \int_{0}^{\pi} \frac{e^{-2 n i \theta}}{e^{e^{-i \theta}}-1} d \theta=\frac{(2 n+1) !}{2 \pi} \int_{0}^{\pi}-e^{-2 n i \theta} d t
\end{aligned}
$$
