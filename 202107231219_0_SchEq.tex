...
\addTODO{什么情况下定态薛定谔方程存在解？在 1 维和多维中分别讨论． 必要条件是各个方向渐进势能都高于束缚态能量，也就是存在真正的势阱（而不是版． 有时候虽然不存在严格的解，但存在很长 life time 解， 例如类氢原子的 Stark 效应（抛物线坐标系）\upref{HStrk2}．充分条件不太好说}
\addTODO{在一维情况下， 无论}
