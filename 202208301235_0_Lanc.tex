...
\end{equation}
可以做个实验把该式和直接计算的结果进行对比． 推导： 把\autoref{Lanc_eq2} 做 $K-1$ 次幂， 左下角的矩阵元就是 $\prod_i \beta_i$． 此时如果把\autoref{Lanc_eq2} 拓展到无限大的矩阵， 那么做 $K-1$ 次幂后结果也相同． 也就是说， $\exp(-\I \mat H \dd{t})$ 的泰勒展开的前 $K$ 项都是精确的
