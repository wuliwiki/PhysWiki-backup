...
$g(v_x^2)$ 为高斯分布，由分布函数的归一化条件 $\int_{-\infty}^\infty g(v_x^2)\dd v_x=1$ 可以求得常数 $C$ 和 $a$ 的关系．我们再进一步利用分子平均动能与温度的关系\autoref{PVnRT_eq3}~\upref{PVnRT} （也就是说，分子速度平方的平均值应当满足 \autoref{MxwBzm_eq1} ，$v_x^2$ 的平均值是它的 $1/3$ 倍）求得 $a$．最终得到以下等式
