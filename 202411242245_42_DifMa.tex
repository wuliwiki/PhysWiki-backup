...
为方便起见，以 $n$ 为偶数为例，即可设为 $2n$ 阶多项式插值，其过 $2n+1$ 个点 $u_{i+j},j=-n,-n+1,\cdots ,n$ 只需注意 $p_i(x_{i-j})=u_{i-j}$，注意\autoref{eq_DifMa_3} 的规律，因此 $2n$ 阶多项式插值公式为
\begin{equation}
p_i(x)=\sum_{j=-n}^n u_{i+j}a_{j}(x),
\end{equation}
其中 
\begin{equation}
a_{j}(x)=\Pi_{k=j+1}^{j+2n+1}(x-x_{i-k})$
