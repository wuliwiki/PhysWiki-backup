...
3）我们用1）和2）方法的平均值，就得到了一个新的方法：  $ \frac{y(t+h)-y(t)}{h}= \frac{1}{2}\left(f(y(t+h),t+h)+f(y(t),t)\right)$  ．这个方法事实上就是所谓的梯形公式（\textbf{Trapezoidal rule}）．它仍然是一个隐式方法，但他的离散化误差提高到了是  $e=\mathcal{O}(h^2)$ ．有兴趣的小伙伴可以自己用泰勒展开验证一下．
