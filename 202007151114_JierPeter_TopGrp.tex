...
我们知道，欧几里得空间中可以用乘积拓扑来构造基本开集，也就是说，$\mathbb{R}^k$的基本开集形如$(a_1, b_1)\times(a_2, b_2)\times\cdots\times(a_k, b_k)$，其中各$a_i, b_i$都是实数；$\mathbb{R}^k$的开集都是基本开集的并．全体$\mathbb{R}$上的$k$阶矩阵构成一个$k^2$维欧氏空间中的基本开集，可以表示成开区间的矩阵，其第$i$行$j$列的矩阵元是$(a_{ij}, b_{ij})$；这是因为这个开区间的矩阵，可以看成是若干矩阵的集合：$\{k\times k$实矩阵|第$i$行$j$列的矩阵元在区间$(a_{ij}, b_{ij})$中$\}$．
