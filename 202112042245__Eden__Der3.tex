...
提示：观察图像一点切线的斜率容易知道 $g'(y)=1/f'(x)$（其中 $x=f^{-1}(y)$），严格证明可以参考书上定理．但二阶导不是那么容易求，逐步推导过程如下：
\begin{equation}
\begin{aligned}
...
&=\frac{-1}{f'(x)^2}\frac{1}{f'(x)}\frac{\dd f'(x)}{\dd x}f''(x)=-\frac{f''(x)}{f'(x)^3}
\end{aligned}
\end{equation}
...
\begin{exercise}[2.2]
设 $x=r\sin\theta\cos\phi,y=r\sin\theta\sin\phi,z=r\cos\theta$，其中 $r>0,0<\theta<\pi,0<\phi<2\pi$．求下列表达式的值：
\begin{enumerate}
\item $\sqrt{(\frac{\partial x}{\partial r})^2+(\frac{\partial y}{\partial r})^2+(\frac{\partial z}{\partial r})^2}$．
\item $\sqrt{(\frac{\partial x}{\partial \theta})^2+(\frac{\partial y}{\partial \theta})^2+(\frac{\partial z}{\partial \theta})^2}$．
\item $\sqrt{(\frac{\partial x}{\partial \phi})^2+(\frac{\partial y}{\partial \phi})^2+(\frac{\partial z}{\partial \phi})^2}$．
\end{enumerate}
\end{exercise}