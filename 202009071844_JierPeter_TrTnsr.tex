...
如果给$V_1$指定一组基$\{\bvec{a}_1, \bvec{a}_2, \cdots, \bvec{a}_n\}$， 给$V_2$指定一组基$\{\bvec{b}_1, \bvec{b}_2, \cdots, \bvec{b}_n\}$，在这两组基下，张量$f$被表示为一个矩阵$\bvec{F}$，而向量$\bvec{v}_1\in V_1$和$\bvec{v}_2\in V_2$在这两组基下的坐标列矩阵分别为$\bvec{c}(\bvec{v}_1)$和$\bvec{c}(\bvec{v}_2)$．如果给$V_1$和$V_2$进行基变换，过渡矩阵分别为$\bvec{P}$和$\bvec{Q}$，则在新的基下$f$的矩阵变成了$\bvec{Q}^{-1}\bvec{F}\bvec{P}$．

为了方便将坐标变换推广，我们把二阶张量$f$的坐标表示为一个\textbf{行向量的行矩阵}$\bvec{F}$，也就是说，选定了$V_1$和$V_2$的基以后，用$\bvec{c}(\bvec{v}_1)$和$\bvec{c}(\bvec{v}_2)$分别表示$\bvec{v}_1\in V_1$和$\bvec{v}_2\in V_2$在这两组基下的坐标列矩阵，那么$f(\bvec{v}_1, \bvec{v}_2)=\bvec{F}\bvec{v}_1\bvec{v}_2$．此时，
