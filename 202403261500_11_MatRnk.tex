...
设行秩为$L_r$，列秩为$L_c$。
$\mathcal R(AB)=\opn{dim}\opn{Im}AB$，设$\bvec x$为任意列向量，任意列向量$\bvec y\in \opn{Im}AB$，即$\bvec y=AB\bvec x$。所以$\bvec y\in \opn{Im}{A}$，则$ L_c(AB)\leq \mathcal R(A)$（象空间为列空间）。

设$\bvec z$为任意行向量，同理得：$\bvec z AB=\bvec y^T\in \opn {Im}B$（象空间为行空间），所以$\mathcal R(AB)\leq \mathcal R(B)$。定理得证。
