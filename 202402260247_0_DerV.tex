...
\subsection{几何意义}
矢量函数 $\bvec r(t)$ 可以看成一条含参曲线，也就是把矢量的起点固定在坐标原点，改变 $t$ 时，矢量终点画出的曲线。 那么该曲线在任意 $\bvec r(t)$ 处如果存在切线\upref{TanL}， 那切线的方向就是它的导函数 $\dot{\bvec r}(t)$ 的方向。

从物理上这是容易理解的，若 $t$ 是时间， $\bvec r$ 是一点的位置矢量\upref{Disp}，那么 $\dot{\bvec r}(t)$ 就是这点的速度矢量，速度矢量总是沿运动轨迹的切线方向。

证明使用\autoref{eq_DerV_1}：对每个具体的 $\Delta t$，$\bvec v(t+\Delta t)-\bvec v(t)$ 就是曲线的\textbf{割线}，而当 $\Delta t\to 0$ 时， 若 $\bvec v(t+\Delta t)$ 无限接近 $\bvec v(t)$，割线的极限就是切线。 若不是无限接近，说明曲线在该处不连续，切线也不存在。 注意 $\Delta t\to 0$ 的极限存在要求从正负两个方向趋近于零时极限都存在且相等， 所以类似\autoref{fig_TanL_3}~\upref{TanL}拐角处的情况不满足该条件。
