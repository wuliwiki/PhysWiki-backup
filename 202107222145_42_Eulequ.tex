...
$\varphi(D_t)f(t)=\lambda f(t)$（$\lambda$为常数），则称 $f(t)$为其具有特征值 $\lambda$ 的特征函数．显然，$e^{rt}$($r$为常数)为$\varphi(D_t)$ 的特征函数，对应特征值为 $\varphi(r)$．对方程\autoref{Eulequ_eq4} ，这等价于找 $\varphi(D_t)$的具有特征值0的特征函数，
\addTODO{证明结论}