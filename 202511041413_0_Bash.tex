...
\item 如果变量 \verb`var` 以 \verb`字符串` 结束， \verb`${var%字符串}` 移除最后的 \verb`字符串`。 例如 \verb`${0%/*}` 获取当前脚本所在的目录
...
\item 同理， \verb`./文件名.sh > 123.log` 只会把 stdout 导入文件， 而不包含 stderr。 还要在后面加上 \verb`2>&1`。 或者直接用 \verb`./文件名.sh &> 123.log`。
...
默认情况下，所有函数本地变量都是全局变量，除非声明了 \verb`local`。 如 \verb`bar() { local var="hi" }`。

\begin{itemize}
\item \verb`return` 等效于 \verb`return 0`。
\item 如果没有 \verb`return` 关键字，那么函数的 exit code 就是最后一个命令的 exit code。 例如 \verb`command_exist() { command -v "$1" &> /dev/null }` 可以用于检查一个命令是否存在（包括 alias）。 用法： \verb`command_exist ls; echo $?`。 如果存在则打印 0， 因为成功的退出码是 0。
\end{itemize}
