...
\end{aligned}
\end{equation}
这样就得到了矩阵乘法公式（\autoref{Mat_eq6}~\upref{Mat}） $y_i = \sum_{i,j} A_{i,j} x_j$． 要特别强调\autoref{braket_eq1} 中的基必须是正交归一的， 否则在\autoref{braket_eq2} 中就不可能得到克罗内克 delta 函数\upref{Kronec}．
...
\begin{equation}
\mel{\eta_i}{A}{\xi_j} = \sum_{i',j'} A_{i',j'} \ket{\eta_i'}\bra{\xi_j}
\end{equation}
