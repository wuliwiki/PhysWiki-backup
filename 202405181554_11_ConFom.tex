...
$M$是黎曼流形，$\nabla$是丛上的联络。
\begin{itemize}
\item 若联络与度规兼容，且 $\{\uvec{e}_i\}$ 是 $\mathfrak{X}(U)$ 上的任意\textbf{正交基}，那么在这组基下，联络形式矩阵 $\omega^j_i$ 必是\textbf{反对称}，即 $\omega^j_i=-\omega^i_j$。
...
\begin{corollary}{}
令$M$是黎曼流形，$\{\uvec e_i\}$是$\mathfrak{X}(U)$上的一组正交基。令$\{\uvec\theta^i\}$是1形式的对偶基。那么存在\textbf{唯一的}\textbf{反对称}联络形式矩阵使得
\begin{equation}
