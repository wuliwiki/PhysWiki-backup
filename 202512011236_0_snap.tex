...
\begin{itemize}
\item 换源：\verb`sudo cp /etc/apt/sources.list /etc/apt/sources.list.backup` 然后 \verb`sudo sed -i 's/archive.ubuntu.com/mirrors.aliyun.com/' /etc/apt/sources.list`
...
\item \verb`dpkg -I 名字.deb` 查看包信息，例如依赖的库。
\item \verb`dpkg -s 包名` 列出包的详细信息
\item \verb`dpkg -S 文件路径` 查找一个文件属于哪个包
\item \verb`dpkg -L 包名` 列出一个包安装了什么文件到什么路径。
\end{itemize}
