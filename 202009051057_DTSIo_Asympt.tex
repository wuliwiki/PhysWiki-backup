...
$$
\sum_{k=0}^\infty \frac{(-1)^kt^k}{x^{k+1}},
$$
代入并计算得到如下的形式等式:
$$
f(x)=\sum_{k=0}^\infty\frac{(-1)^kk!}{x^{k+1}}.
$$
当然, 右边的级数对于任何$x$都不收敛, 因为$t$的几何级数收敛半径是有限的. 然而这个等式却在渐近展开的意义下成立. 实际上, 通过换元可得
$$
f(x)=e^{x}\int_x^\infty \frac{e^{-t}}{t}dt.
$$
反复进行分部积分, 得到
$$
f(x)=\sum_{k=0}^n\frac{(-1)^nn!}{x^{k+1}}
+n!e^x\int_x^\infty\frac{e^{-t}}{t^{n+2}}dt
$$
\end{example}