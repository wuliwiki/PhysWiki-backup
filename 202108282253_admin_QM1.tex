...
\pentry{量子力学基本原理（科普）\upref{QM0}， 狄拉克符号\upref{braket}， 狄拉克 delta 函数\upref{Delta}， 平面波\upref{PWave}， 厄米矩阵的本征值问题\upref{HerEig}， 概率密度函数\upref{RandF}}
...
现在， 测量结果的概率就很自然地从离散的概率 $\abs{C_i}$ 变为概率密度函数\upref{RandF} $\abs{C(\lambda)}^2$ 了． 也就是说， 测量值落在某个区间 $\lambda \in [a, b]$ 
