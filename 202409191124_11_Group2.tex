...
\textbf{证明：}
\begin{enumerate}
\item 设任意$f(x),f(y)\in f(B)$，则$f(x)f(y)^{-1}=f(xy^{-1})$。又因为$xy^{-1}\in B$，所以$f(xy^{-1})\in f(H)$。所以$f(B)$是$G_2$的子群。反之也同理易证$f^{-1}(K)$是$G_1$的子群\footnote{需要注意映射的逆和群元的逆的区别，$[f^{-1}(a)]^{-1}=f^{-1}(a^{-1})$}。设$e'=f(e)$，由于$e'\in K$，所以$f^-1{e'}=N\in f^{-1}$
\end{enumerate}
