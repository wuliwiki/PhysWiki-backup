...
\end{equation}
的极值曲线，令 $x'=\dv{x}{y}$，$\phi(y,x')=x'F(y,\frac{1}{x'})$，则
\begin{equation}
y_C=\int_{-\frac{a}{2}}^{\frac{a}{2}} \phi(y,x')\dd y
\end{equation}
现在，函数 $\phi(y,x')$ 包含自变量 $y$，我们就可以使用拉格朗日方程求解．回想拉格朗日方程的推导，不难看出，$\phi(y,x')$代表拉格朗日函数$L(t,q,q')$, $y$代表拉格朗日方程中的 $t$,而$x'$代表$$
对不依赖与 $x$ 的被积函数 $F(y,y')$ 的情形，下面的方程成立：
\begin{equation}
F(y,y')-y'F_{y'}(y,y')=C
\end{equation}
其中 $C$为常数．
