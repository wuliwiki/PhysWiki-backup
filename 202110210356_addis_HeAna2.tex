...
\begin{equation}
\psi_{l_1,l_2}^{L, M} \overset{r_2\to\infty}{\longrightarrow} r_1 R_{n_1,l_1}(r_1) F_{l_2}(r_2 + \delta)
\end{equation}
所有其他的 $(l_1,l_2)$ 分波在 $r_2\to\infty$ 波函数都消失． 这样就可以把这个态记为 $\ket{L,M; l_1,l_2, n_1, k_2}$． 分号用于提醒后面的量子数只是无穷远处的边界条件， 这个态仍然只有 $E,L,M$ 是 well defined． 如果把这些态用 CG 系数线性组合一下， 就可以得到 $\ket{l_1, m_1, l_2,m_2,n_1,k_2}$， 注意此时除了能量， 里面的量子数都只是在 $r_2\to+\infty$ 处是 well defined．

于是微扰理论中的矩阵元可以是 $\mel{l_1, m_1, l_2,m_2,n_1,k_2}{z_1 + z_2}{i}$， 剩下的论述就和氢原子的一样了． 注意氢原子
