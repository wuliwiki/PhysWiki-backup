...
\end{aligned}~
\end{equation}
只要求出 $p'(x_i)$ 即可得到对应的 $n$ 阶差分方程。（读者只需知道具体的方法，无需大量的计算，因为计算量会随着 $n$ 的增大而增大，shi'ji以下的具体实例由Mathematica辅助计算完成）例如，对4阶($n=2$)，就有
\begin{equation}
...
\end{aligned}~
\end{equation}
因此，若设有 $8$ 个数据点，即 $x_i$ 的个数为 $8$。那么，对应 $4$ 阶和6阶的差分矩阵分别就是
\begin{equation}
\left(
