...
\bra{\delta(\bvec r'_1 - \bvec r_1)}\braket{C_{l_2,m_2}(\bvec r_2)}{\Psi(\bvec r_1, \bvec r_2)} = \frac{1}{r_1} \sum_{l_1, m_1} \psi_{l_1,m_1}(r_1) \cdot Y_{l_1, m_1}(\uvec r_1)
\end{equation}
\begin{equation}
\psi_{l_1,m_1}(r_1) = \int_{r_{2,min}}^{+\infty} \sqrt{\frac{2}{\pi}}F_{l_2}(\eta_2, k_2 r_2)\psi_{l_1, l_2}^{m_1, m_2}(r_1, r_2)\dd{r_2}
\end{equation}
然而 streaking trace 需要的不是上式的概率而是某个方向的概率． 我们使用\autoref{CulmWf_eq6}~\upref{CulmWf}
\begin{equation}
\bra{\delta(\bvec r'_1 - \bvec r_1)}\braket*{\psi_{\bvec k_2}^{(-)}}{\Psi} = \frac{1}{k_2}\sum_{l_2,m_2} \I^{-l_2} \E^{\I\sigma_{l_2}} \bra{\phi_{n_1, l_1, m_1}}\braket{C_{l_2, m_2}(k_2)}{\Psi} Y_{l_2,m_2}(\uvec k_2)
\end{equation}
...
\begin{aligned}
P(k_2) &= \int_0^{r_{1,max}} \abs{\bra{\delta(\bvec r'_1 - \bvec r_1)}\braket{C_{l_2,m_2}(\bvec r_2)}{\Psi(\bvec r_1, \bvec r_2)}}^2 \dd[3]{r_1}\\
&= \sum_{l_1,m_1} \int_0^{r_{1,max}} \abs{\psi_{l_1,m_1}(r_1)}^2 \dd{r_1}
\end{aligned}
\end{equation}
．
...
\end{equation}
然而， streaking trace 需要的不是上式的概率而是某个方向的概率． 我们只需使用\autoref{CulmWf_eq6}~\upref{CulmWf} 即可
\begin{equation}
\bra{\phi_{n_1, l_1, m_1}}\braket*{\psi_{\bvec k_2}^{(-)}}{\Psi} = \frac{1}{k_2}\sum_{l_2,m_2} \I^{-l_2} \E^{\I\sigma_{l_2}} \bra{\phi_{n_1, l_1, m_1}}\braket{C_{l_2, m_2}(k_2)}{\Psi} Y_{l_2,m_2}(\uvec k_2)
\end{equation}
\begin{equation}\label{HeAnal_eq10}
P(n_1, l_1, m_1, \bvec k_2) = \abs{\bra{\phi_{n_1, l_1, m_1}}\braket*{\psi_{\bvec k_2}^{(-)}}{\Psi}}^2
\end{equation}

当 $M \equiv 0$ 时这里对 $m_2$ 求和其实只有一项就是 $m_2 = - m_1$， 也是一个常数． 理论上， 如果把\autoref{HeAnal_eq10} 对所有 $n_1, l_1, m_1$ 求和， 应该得到\autoref{HeAnal_eq11} ．
