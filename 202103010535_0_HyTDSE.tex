...
\end{equation}
在原子单位中电子电荷 $q = -1$， 该势能使用了长度规范\upref{LenGau}， 此外我们还可以使用速度规范速度规范\upref{LVgaug}， 也是等效的． 在长度规范中， 
...
\end{aligned}
\end{equation}
可见， 当没有外场的时候每一个项（即每一个分波）都可以独立演化， 而电场将不同的分波耦合起来． 可以证明， 矩阵 $\mat F$ 只有两条副对角线上的元不为零． 即 $\Delta l = \pm 1$．
