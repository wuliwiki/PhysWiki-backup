...
角的度量值称为\textbf{角度（measure of angle）}。角的大小可以用\textbf{度（degree）}、\textbf{分（minute）}、\textbf{秒 （second）}表示，例如 $30^\circ34′56″$。这种表示角大小的单位制称为\textbf{角度制（Degree System）}。角度制要求一个\textbf{平角}的度数为 $180^\circ$，而\textbf{周角}的度数为 $360^\circ$。按角度的大小角可以分为：

其中\textbf{平角（straight angle）}的度数规定为 $180^\circ$，而\textbf{周角（full angle）}的$\frac{360分之一为1度的角$度数为 $360^\circ$。

根据角度的大小，可以将角分类如下：
\begin{itemize}
\item \textbf{直角（right angle）}：角的大小等于 $90^\circ$。
\item \textbf{锐角（acute angle）}：角的大小小于 $90^\circ$。
\item \textbf{钝角（obtuse angle）}：角的大小大于 $90^\circ$ 但小于 $180^\circ$。
\end{itemize}

对于大于 $180^\circ$ 的角，通常采用其与周角互补的角度进行描述，以便更直观地理解其几何意义。
...
对于大于$180^\circ$的角，通常会使用其相对于周角另一侧的角度进行描述。
