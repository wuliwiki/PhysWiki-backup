...
对于庞加莱半平面度量的描述，更简洁的表达是：$\frac{\dd x\otimes \dd x+ \dd x\otimes\dd x}{y^2}$．这里的$\dd x$是指$\mathbb{H}$上的函数$f(x, y)=x$的微分$\dd f$，同样地$\dd y$是$g(x, y)=y$的微分$\dd g$，它们都是$\mathbb{H}$上的$1$-形式，而$\otimes$是它们的张量积．

如果用通常的$(x, y)\in \mathbb{R}^2$作为坐标来描述$\mathbb{H}$，那么在$\mathbb{R}^2$中的两个点，在保持$x$坐标不变时同步向$y$的正方向移动，那么它们的距离会缩短，并在$y$坐标趋于正无穷时距离趋于零．反过来，如果两个点的$y$坐标始终相同，$x$坐标不变，那么它们同步趋近于$x$轴时，彼此距离会趋近于正无穷．

\subsection{高斯曲率}

\pentry{高斯绝妙定理\upref{Egreg}}

由\textbf{高斯绝妙定理}\upref{Egreg}，如果要计算$\mathbb{H}$的高斯曲率，我们就要去计算某个基下的曲率形式$\Omega^i_j$，从而有$K=\Omega^1_2(\uvec{e}_1, \uvec{e}_2)$．

由\textbf{反对称定理}\autoref{ConFom_the1}~\upref{ConFom}，选标准正交基来进行计算最为方便，因为$\mathbb{H}$是二维的，标准正交基下的联络形式矩阵是一个反对称的$2$阶方阵，也就是说可以被一个分量唯一确定．
...
\begin{equation}
\begin{aligned}
\dd\theta^1=-\omega^1_2\\theta^2\\
\dd\theta^2=\frac{1}{y}\dd y
\end{aligned}
\end{equation}
