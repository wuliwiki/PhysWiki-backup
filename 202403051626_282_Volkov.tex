...
本文使用原子单位制\upref{AU}。 若初始时刻 $t_0$ 一个一维自由粒子的波函数 $\Psi(x,t_0)$ 已知，接下来空间中出现了随时间变化的电场 $\bvec {\mathcal E}(t)$ （我们采用偶极子近似\upref{DipApr}， 即假设电场不随位置变化， 感生磁场可以忽略）\upref{DipApr}， 那么接下来波函数会如何变化呢？

要求解该问题， 可以用 “一维自由粒子高斯波包\upref{GausWP}” 相似的思路，先把初始波函数拆解为无数平面波， 再分别把每个平面波在电场 $\bvec {\mathcal E}(t)$ 中演化到任意时间 $t$， 最后组合成为 $\Psi(x,t)$。

我们若把该电场中的含时波函数称为 \textbf{Volkov 波函数}。
