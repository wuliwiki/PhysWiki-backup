...
\item \verb`%class{ ... }` 定义 lexer clas (custom lexer implementation)，如 \verb`%class{ class MyLexer : public reflex::AbstractLexer { ... } }`
\item \verb`%init{ ... }` 定义 lexer 初始化的代码，如 \verb`%init{ printf("Lexer initialized!\n"); %}`
\end{itemize}
