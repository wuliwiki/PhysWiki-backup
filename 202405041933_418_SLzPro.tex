...
\end{lemma}
这使得可以证明，对于 $u_a(\lambda)$，其是一个本征解，当且仅当在 $b$ 处满足条件，即 $\theta_a(\lambda, b) = \beta \mod \pi$。而当 $\lambda \rightarrow -\infty$ 时 $\theta_a \rightarrow 0$，这就使得可以取到一个最小 $\beta \in (0, \pi]$ 满足刚才的sh'zi