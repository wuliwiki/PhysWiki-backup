...
\quad (i=1,\dots,N)
\end{equation}
其中 $L$ 是系统的\textbf{拉格朗日量（lagrangian）}，等于系统动能减势能（我们先假设势能不是 $\dot q_i$ 的函数）． 为了方便， 我们也会把 $q_1, \dots, q_N$ 和 $\dot q_1,\dots,\dot q_N$ 分别记为 $q$ 和 $\dot q$． 我们可以把 $N$ 个广义坐标看作 $\mathbb R^N$ 空间中的一点， 把这个空间叫做\textbf{位形空间（configuration space）}． 当系统随时间变化时， 系统在位形空间中的点随时间移动形成一条轨迹．
