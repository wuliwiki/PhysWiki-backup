...
\begin{itemize}
\item \textbf{动量算符} $A:D(A)\subseteq X\to X$ 定义为 $(A\phi)(x) := -i\hbar \dv*{\phi(x)}{x}$ （$\forall x\in\mathbb R$）
\end{itemize}
...
\item \textbf{动量算符}： 令 $p\in\mathbb R$， $T_p(\phi) := \int_{-\infty}^\infty \overline{\phi_p(x)}\phi(x) \dd{x}$ （$\forall \phi\in\mathcal S$） 其中 $\phi_p(x) := \exp(ipx/\hbar)$． $T_p \in \mathcal S'$．
\end{itemize}
