...
不同线性空间之间可以用直和组合在一起，因此以上这些空间也都可以作直和，得到一个 $\bigwedge V=\bigoplus^{\opn{dim}V}_{k=0}\bigwedge^k V=\mathbb{F}\oplus\bigwedge^1V\oplus\bigwedge^2V\cdots$。这个 $\bigwedge V$，就被称作 $V$ 上的\textbf{外积空间（exterior product space）}或\textbf{楔积空间（wedge product space）}。$\mathbb{F}$ 是 $V$ 的基域，视为 $\mathbb{F}$ 自身上的一维线性空间。总结上述限制条件，我们便得到了外代数的运算结构。
...
\begin{enumerate}
\item \textbf{结合性：}$x\wedge (y\wedge z)=(x\wedge y)\wedge z$；
...
故构成 $\mathbb{F}$ 上的有限维结合代数。称$\bigwedge V=\bigoplus^{\opn{dim}V}_{k=0}\bigwedge^k V$为 $V$ 上的\textbf{外代数（exterior algebra）}、\textbf{楔积（wedge product）}或\textbf{格拉斯曼代数（Grassmann algebra）}。
\end{definition}
