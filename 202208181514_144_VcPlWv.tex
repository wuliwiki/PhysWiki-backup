...
其中，$\bvec E, \bvec B$均为矢量场\upref{Vfield}（即空间任意一点处，都存在一个矢量的电场$\bvec E$与磁场$\bvec B$）．$\bvec E$的各分量即为
$\bvec E = 
\begin{pmatrix}
...
$
\subsubsection{复指数形式}
或写为复数形式\upref{PWave}．复数形式的波动方程更为简洁、也更便于求导、相乘等计算．最终有用的结果是实数部分 $\bvec E = \Re (\tilde E)$

\begin{align}
\tilde E = \tilde E_0 \E^{\I(\bvec k \cdot \bvec r - \omega t)}\\
\tilde B = \tilde B_0 \E^{\I(\bvec k \cdot \bvec r - \omega t)}\\
\end{align}

$\tilde E$的含义指$\tilde E$是一个复数shi
...
\begin{equation}
\bvec B_0 = \frac{1}{\omega} \bvec k \times \bvec E_0
\end{equation}
由此，可推导出以下结论：
\begin{itemize}
