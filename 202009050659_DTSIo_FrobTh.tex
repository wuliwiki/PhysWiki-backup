...
如果$u=u(x)$是这方程组的解, 那么根据偏导数的交换性, 显然可得
\begin{equation}\label{FrobTh_eq2}
\frac{\partial f_i^\alpha}{\partial x^j}+f_j^\beta\frac{\partial f_i^\alpha}{\partial u^\beta}=\frac{\partial f_j^\alpha}{\partial x^i}+f_i^\beta\frac{\partial f_j^\alpha}{\partial u^\beta},\forall i,j,\alpha,\beta
\end{equation}
在自变量取值$(x,u(x))$时成立. 欲使得\autoref{FrobTh_eq1}对于任何初值$u(x_0)=u_0$都可积, 则必须$U\times V$中存在由图像给出的, 经过点$(x_0,u_0)$的$n$维子流形$\{(x,u(x)):x\in U\}$, 使得$u$满足\autoref{FrobTh_eq1}. 因此\autoref{FrobTh_eq2}必须在$U\times V$中处处成立. 显然\autoref{FrobTh_eq2}处处成立当且仅当向量场
$$
X_i=\frac{\partial}{\partial x^i}+f_i^\alpha(x,u)\frac{\partial}{\partial u^\alpha},\,i=1,...,n
$$
两两对易. 而根据费罗贝尼乌斯定理, 可以给出可积性的充分必要条件: \autoref{FrobTh_eq1}的可积性等价于$U\times V$上的$n$维分布$\text{span}(X_1,...,X_n)$的可积性. 