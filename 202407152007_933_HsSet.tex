...
\subsection{前言}
经过一代代数学家们的不断努力，告别了古代数学，现代数学展现在了世人面前。古代数学的每个部分都有它自己的一套体系，也有它自己的语言（比如，几何证明时的$\mathbin{\!/\mkern-5mu/\!}$就不会出现在解方程的过程中。）。集合语言是的基本语言，这种语言可以简洁、准确的表达数学内容。在高中数学中，集合的内容相对简单，但集合是高中数学的基础，请重视集合的学习。

提前提醒一下，由于集合是高中阶段第一个接触到的概念，这篇文章会涵盖许多与集合相关的新概念，请不要被这些它们的数量吓倒，它们其实都是一些自然而然的概念，在高中阶段也并不要求对他们进行过深的探索。同时，请务必理解这部分的内容，哪怕暂时不熟悉这些概念的名称，但一定要理解他们的意思，理解不同概念之间的差异点会很有帮助，后面在介绍时也会尽可能指明它们来帮助理解。由于这些概念很常用，在使用时再逐步熟悉名称就可以，不必因为记不住名称而紧张。
