...
\subsection{矩阵视角}\label{sub_TIPT_3}
为了读者们不在上面的推导中迷失。 我们可以从矩阵的角度来看看我们以上究竟在解决什么问题。 若我们使用 $H^0$ 的本征矢 $\psi_n^0$ 作为基底， 那么 $H^0$ 就可以表示为一个对角矩阵
\begin{equation}
...
总哈密顿矩阵为 $\mat H = \mat H^0 + \lambda\mat H^1$， 注意 $\mat H$ 往往是一个无穷大的矩阵（例如量子简谐振子），但在实际计算中我们往往会把它截断为有限维的，并通过增加矩阵大小来逼近精确结果。

理论上，通过直接求解矩阵的本征方程
\begin{equation}
(\mat H^0 + \lambda\mat H^1)\bvec{v}_n = E_n \bvec{v}_n~.
\end{equation}
就可以精确得到 $E_n$ 和 $\bvec{v}_n$ 而无需任何阶的微扰修正
\begin{equation}
...
\end{equation}
当 $\lambda$ 逐渐变大时， $E_n(\lambda)$ 一般会发生非线性的变化， 可以泰勒展开为多项式：
\begin{equation}
E_n(\lambda) = E_n^0 + \sum_{m=1}^\infty C_{n,m} \lambda^m~.
\end{equation}
而其中 $C_m\lambda^m$ 就是第 $n$ 个本征值的 $m$ 阶修正 $E_n^m$。
