...
\addTODO{完成\textbf{Sylow定理}\upref{Sylow}中Sylow子群定义后，在此引用．}

除去$V_4$中各元素的单元素集合的中心外，$S_4$中还剩下$8$个元素，都是三元轮换．

这样分类后容易验证，$V_4$的中心就是$V_4$本身．

另外，如果把一个正方形的四个顶点顺时针依次编号为$1, 3, 2, 4$，则不难看出$V_4$实际上就是
...
如果$[\mathbb{K}:\mathbb{F}]=3$，而$3$是素数，故$\mathbb{K}/\mathbb{F}$应是一次单扩张，又由$uvw=q^2\in\mathbb{F}$可知$\mathbb{K}=\mathbb{F}(u)=\mathbb{F}(v)=\mathbb{F}(w)$．故$\abs{\opn{Gal}(\mathbb{K}:\mathbb{F})}=\deg\opn{Irr}(u, \mathbb{F})=3$．于是$\abs{\opn{Gal}(f:\mathbb{F})}=3\cdot 4=12$．于是$\opn{Gal}(f:\mathbb{F})=A_4$，唯一阶数为$12$的$S_4$的子群．

如果$[\mathbb{K}:\mathbb{F}]=2$，而$2$是素数，故同理可知$\abs{\opn{Gal}(\mathbb{K}:\mathbb{F})}=2$，进而$\opn{Gal}(f:\mathbb{F})=V_4\times B$，其中$B$是二元群．因此$\opn{Gal}(f:\mathbb{F})=C$（见\autoref{PlyRtS_eq10} ）．
