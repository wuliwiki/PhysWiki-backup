...
\subsection{无穷阶差分矩阵}
由\autoref{eq_InfDM_4} 可知，若 $u(x)=\sum\limits_{j}a_jf_j(x)$，$a_i\in\mathbb C$， 那么
\begin{equation}
\begin{aligned}
\hat u(k)&=h\sum_{i=-\infty}^{\infty}e^{-\I kx_i}\sum\limits_{i}u(x_i),\quad k\in[-\pi/h,\pi/h],\\\sum_{i}a_i\int_{-\infty}^{\infty} e^{-\I kx}f_i(x)\dd x=\sum_{i}a_i\hat f_i(k).
\end{aligned}~
\end{equation}
...
\end{aligned}\right.~
\end{equation}
表示为 $u(x_j)=\sum\limits_{m=-\infty}^{\infty}u(x_m)\delta(x_j-x_m)$。因此，若知道 $\delta(x)$ 的傅里叶变换，就知道了 $p(x)=\sum\limits_{m=-\infty}^\infty u(x_m)\delta(x-x_m)$ 的傅里叶变换，显然 $p(x_i)=u(x_i)$。
