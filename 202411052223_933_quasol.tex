...
\textbf{单项式（monomial）}是由一个变量或多个变量的非负整数指数幂及常数系数的乘积组成的项。包含其他运算（如 $\sin x$、$\cos x$、$2^x$等）或其他非整数指数（如 $x^{1/2}$、$x^{-1}$）的表达式均非单项式。\textbf{多项式（polynomial）}是由一个或多个单项式通过加法或减法连接组成的表达式。
