...
在无法获得精确解的情况下，麦克斯韦方程可以通过\textbf{数值方法}来近似求解，例如有限元法和时域有限差分法。更多细节可参考\textbf{计算电磁学}。


麦克斯韦方程组似乎是**超定的**，因为它涉及六个未知量（即电场 \( E \) 和磁场 \( B \) 的三个分量），但包含八个方程（两条高斯定律各有一个方程，而法拉第定律和安培环路定律各有三个分量）。（电流和电荷并非未知量，它们在满足电荷守恒的前提下可以自由指定。）这与麦克斯韦方程组中的某种有限的冗余性有关：可以证明，任何符合法拉第定律和安培环路定律的系统，只要其初始条件满足这两条高斯定律，并假设电荷守恒和不存在磁单极子，该系统将自动满足这两条高斯定律。这一解释最早由朱利叶斯·亚当斯·斯特拉顿（Julius Adams Stratton）在1941年提出。

尽管在数值算法中可以简单地忽略两条高斯定律（除初始条件外），计算的精度不完美可能导致这些定律的逐步违背。通过引入表征这些偏差的虚拟变量，四个方程实际上并不是超定的。采用这种方法可以使算法更准确，并考虑所有四条定律。

两个恒等式 
\[
\nabla \cdot \nabla \times \mathbf{B} \equiv 0, \quad \nabla \cdot \nabla \times \mathbf{E} \equiv 0
\]
将八个方程减少为六个独立方程，是真正造成超定的原因。