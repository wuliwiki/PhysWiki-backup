...
在 Stern-Gerlach 实验中，一束银原子朝一个方向（我们设这个方向为 $\bvec y$ 轴方向）发射并穿过不均匀的磁场．除了有力矩（类比经典电磁理论中的\upref{EBLoop}），还有另外一个力作用在磁偶极子上（类比经典电磁理论中的\upref{EBTorq}）：
...
\end{equation}
这里的 $S_x$ 表示银元子自旋角动量 $x$ 分量的期待值，$S_z$ 表示自旋角动量 $z$ 分量的期待值．由于电子的磁矩和自旋角动量成正比，$\gamma$ 表示这一比例系数（称为磁旋比\upref{BohMag}．不同于经典理论的 $q/2m$，这里的 $\gamma$ 实际上是 $q/m$\upref{BohMag}）．
...
银元子束穿过磁场受到偏转后打在检测器屏幕上．经典上，我们预期的结果是一条模糊带（$S_z$ 没有量子化）．但实验结果却是原子束分成了 $2s+$
