...
\end{equation}
\end{example}
根据\autoref{ex_Hypb3_1} 的推导可知反比例函数$\displaystyle y={k\over x}$逆时针旋转$45^\circ$后对应的表达式为：
\begin{equation}
{x^2\over(\sqrt{2k})^2}- {y^2\over(\sqrt{2k})^2}=1~.
\end{equation}
由于$a=b=\sqrt{2k}$，两条渐近线互相垂直。从而围成的平行四边形即矩形，根据\autoref{eq_Hypb3_27} 面积为：
\begin{equation}
\frac{ab}{2}=\frac{\sqrt{2k}\sqrt{2k}}{2}=k~.
\end{equation}
