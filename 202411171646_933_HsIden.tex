...
对于具备内积运算的任意两个向量$\bvec{a},\bvec{b}$，有
\begin{equation}
\bvec{a} \cdot \bvec{b} \leq (\bvec{a}\cdot\bvec{a})(\bvec{b}\cdot\bvec{b})=||\bvec{a}||\cdot||\bvec{b}||~.
\end{equation}
，当且仅当两个向量平行时取等。其中$||\bvec{a}||$是与内积对应的范数。
\end{theorem}

鉴于这部分是为开阔视野，上面采用了比较严谨的表述，但涉及到很多高中未曾了解的知识。在高中阶段由于只涉及到几何向量的运算，这时的内积指的就是$\bvec{x}\cdot\bvec{y}=|\bvec{x}|\cdot|\bvec{y}|\cos\theta$，而所谓对应的范数指的就是向量的模长。
