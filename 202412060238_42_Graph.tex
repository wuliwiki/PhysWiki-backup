...
\begin{definition}{边的分类，有向性，环}
设 $G=(V,E)$ 是图，$e\in E$。若 $e\in[V]^2$，则称 $e$ 是\textbf{无向的}（undirected）；若 $e\in V\times V$，则称 $e$ 是\textbf{有向的}（directed）。对有向边 $e=(v_1,v_2)$，称 $v_1$ 是 $e$ 的\textbf{起点}（origin），$v_2$ 是 $e$ 的\textbf{终点}（terminus）。起点和终点统称为\textbf{端点}（end vertex）。称边 $(v,v)$ 的边为环
\end{definition}
