...
根据三角函数的定义，正弦函数和正切函数的零点均出现在角$x$的终边与x轴重合的情况，根据弧度的定义，此时角度分别为$0+2k\pi$和$\pi+2k\pi$，整理合并可以得到$\sin x,\tan x$的零点为：
\begin{equation}\label{eq_HsTFFv_1}
x=k\pi,\qquad (k\in\mathbb{Z})~.
\end{equation}
同理，余弦函数和余切函数的零点出现在角$x$的终边与y轴重合的情况，此时角度分别为$\displaystyle{\pi\over2}+2k\pi$和$\displaystyle{3\over2}\pi+2k\pi$整理可得$\cos x,\cot x$的零点为：
\begin{equation}
x={\pi\over2}+k\pi,\qquad (k\in\mathbb{Z})~.
\end{equation}

另外由于$\sec x$和$\csc x$都是单位圆外一点与圆心的连线长度，因此其值至少为$1$，即不存在零点。

\subsubsection{周期性}

据\aref{之前的分析}{sub_HsTrFu_1}，所有的三角函数都是周期函数，且$2\pi$一定是他们的一个周期。现在分析$2\pi$是否是他们的最小正周期。

对于正弦函数，设$T$是它的最小正周期，则根据周期函数的性质，必有$f(x+T)=f(x)$，即
...
对于余弦函数，与正弦的推导过程相同，取$\displaystyle x={\pi\over2}$就可得到相同的结论。而$\sec x,\csc x$由于与$\cos x,\sin x$之间存在yi yi ying she

对于正切函数的情况，首先根据诱导公式$\tan(\pi+x)=\tan x$显然有$\pi$是他的一个周期，下面讨论$\pi$是不是它的最小正周期。与之前讨论的方法类似，设$T$是它的最小正周期，则有：
\begin{equation}
\tan(T+x)=\tan x~.
\end{equation}
取$x=0$，可知$T$必然是$\tan x$的零点，而根据\autoref{eq_HsTFFv_1} $\tan x$不存在比$\pi$小的正的零点，因此$T=\pi$是正切函数的最小正周期。根据类似的分析，可以知道$\cot x$的最小正周期也是$T=\pi$。
