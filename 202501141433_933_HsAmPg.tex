...
d={a_n-a_k\over n-k}~.
\end{equation}
这一公式与\aref{平均变化率}{def_HsFunC_3}或\aref{斜率}{def_HsFunC_5}的定义极为相似，也符合之前等差数列与一次函数关系的判断。根据\autoref{eq_HsAmPg_1}，还可以得到等差数列的一个重要性质：取四个整数，满足 $m+n=p+q$，即 $p-n=m-q$，则有：
\begin{equation}
