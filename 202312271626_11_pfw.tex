...
\end{equation}
对于$p\in U\cap V$，我们有：$\widetilde\theta_i|_p=T^j_i \widetilde e_i|_p$，T^j_i是光滑函数构成的过渡矩阵，该假设是合理的，因为两边的导子作用在光滑函数上，结果还是光滑函数。两边作用在$\phi(p)$点上，计算分量得：
\begin{equation}
\ali
\theta |_{\psi(p)x^k=T^j_i e
\end{equation}
