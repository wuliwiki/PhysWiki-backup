...
\item GCC 若要默认隐藏（推荐），可以通过（即 \verb`-fvisibility=hidden`）控制某个编译单元，或者用 linker 选项 \verb`-Wl,--exclude-all-symbols` 隐藏当前编译的库的所有符号。
...
\end{lstlisting}
\item \verb`MYLIB_API` 只需要在声明符号的时候使用，不需要在定义时使用（除非定义和声明写在一起，例如 C++ template、inline 函数等）。
\end{itemize}

\subsection{跨编译器编译库}
例如要在使用 MinGW 编译一个库给 MSVC 使用，除了使用 \verb`extern "C" {...}`， 为了避免该库依赖 MinGW 特有的依赖库，应该把它们设置为静态链接。
