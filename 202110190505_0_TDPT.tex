...
为简单起见我们暂时假设 $H_0$ 只有离散的束缚， 例如简谐振子\upref{QSHOop}． 我们假设已经知道 $H'(t) = 0$ 的情况下含时薛定谔方程的通解：先解出对应的定态薛定谔方程的波函数 $\ket{\psi_n}$ 和能级 $E_n$，通解可表示为（\autoref{TDSE_eq5}~\upref{TDSE}）
\begin{equation}
...
c_i^{(n)}(t) = \frac{1}{\I\hbar} \sum_j \int_{0}^{t} \mel{\psi_i}{H'(t)}{\psi_j} \E^{\I\omega_{ij}t} c_j^{(n-1)}(t) \dd{t}
\end{equation}
...
其中 $W$ 是一个不含时的算符
\begin{equation}
H'_{ij}(t) = \mel{\psi_i}{H'}{\psi_j} = \mel{\psi_i}{W}{\psi_j}f(t)
\end{equation}
此时一阶微扰公式（\autoref{TDPT_eq9}） 变为 $f(t)$ 的反傅里叶变换\upref{FTExp}． 末态 $\ket{\psi_i}$ （$i \neq j$）的系数为
\begin{equation}
c_i^{(1)}(t) = \frac{\mel{\psi_i}{W}{\psi_j}}{\I\hbar} \int_0^t f(t)\E^{\I\omega_{ij} t} \dd{t}
\end{equation}
当 $f(t)$ 只在有限时间段不为零时候， 令 $t\to+\infty$ 该积分就是 $f(t)$ 的傅里叶变换\upref{FTExp} $\sqrt{2\pi}\tilde f(-\omega_{ij})$．
\begin{equation}
c_i^{(1)}(+\infty) = \frac{\sqrt{2\pi}}{\I\hbar} \mel{\psi_i}{W}{\psi_j}\tilde f(-\omega_{ij})
\end{equation}
