...
设 $G=(V,E,\varphi)$ 是图，$v\in V$。则 $G$ 中与 $v$ \aref{关联}{def_Graph_1}的边数称为 $v$ 在 $G$ 中的\textbf{度}（degree），记作 $d_G(v)$。$G$ 中以 $v$ 为起点的有向边数称为 $v$ 在 $G$ 中的\textbf{出度}（out degree），记作 $d_G^+(v)$。$G$ 中以 $d$ 为终点的有向边数称为 $v$ 在 $G$ 中的\textbf{入度}（in degree），记作 $d_G^-(v)$。度为 $d$ 的点称为 $d$ 度点（d degree vertex）。
\end{definition}
