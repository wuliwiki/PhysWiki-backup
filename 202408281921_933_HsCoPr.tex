...
总的说来，否定条件命题时，只需要否定后件，而前件则不用动。另外，“$P$不能推出$Q$”是一种符合语言习惯的表达，但很多同学会因为这个表达而对条件命题的否定造成混淆，不知道应该否定哪个部分。
\item 这里的否定和后面的否命题不是同一回事，具体辨析可参见\autoref{ex_HsCoPr_1} 。
\end{itemize}

在前面的讨论中，我们给出的命题并未涉及其真值的判定。实际上，条件命题既可以是真命题，也可以是假命题。然而，在高中教材中，有时对这些概念的表达不够明确，容易导致初学者在理解条件命题的否定以及充分条件和必要条件时产生困惑。因此，需要注意本文和教材在表达上的区别。具体说来：
\begin{itemize}
\item 在教材中，如果$P$能够推出$Q$，则通常记作$P\Rightarrow Q$；否则，记作$P\not\Rightarrow Q$。在这种表达中，$P\Rightarrow Q$或$\Rightarrow$符号已经隐含了真值的判断，即认为“$P$能推出$Q$”这一命题本身就包含了真假判定。
...
到此为止，给出的命题都不涉及真值的判定，事实上条件命题既可以是真命题，也可以是假命题。但在高中教材中，有一些语焉不详，造成初次接触时，容易对条件命题的否定以及充分、必要条件的理解出现问题。因此，请注意文本的表达和教材表达的区别。具体说来：教材中认为，如果$P$能推出$Q$，则记作$P\Rightarrow Q$，否则记作$P\not\Rightarrow Q$，也就是认为“$P$能推出$Q$”这句话或者$\Rightarrow$这个符号是包含了真值判断的。本文则采用命题“如果$P$能推出$Q$”，记作$P\Rightarrow Q$，否定记作$P\not\Rightarrow Q$，然后再讨论命题的真值：如果命题为真，则否定为假，如果命题为假，则否定为真，的方式。
