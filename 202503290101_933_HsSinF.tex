...
感兴趣的读者可以参考之前的函数的变换中的 \aref{例子}{ex_FunTra_1}，自行验证五点作图法是否正确。事实上，正是由于函数的变换对于高中阶段的学生而言理解较为困难，才需要引入五点作图法。

另外，初学者容易混淆此处 $\alpha = 0$ 与之前介绍初相时所说 $\varphi$ 对应着 $x = 0$ 的含义。$\alpha = 0$ 表示的是相位整体为 0，是相对于正弦函数 $\sin x$ 而言的，指的是 $\sin x$ 在初始位置的相位。而初相 $\varphi$ 则是相对于 $x$ 轴而言的，由于在信号分析中 $x$ 轴通常表示时间，因此初相表示的是初始时刻的相位。

感兴趣的读者可以联系之前的函数的变换中的\aref{例子}{ex_FunTra_1}，自行验证五点作图法是否正确。事实上，正是由于函数的变换对于高中阶段的学生而言，理解较为困难，才要引入五点作图法。另外，初学者容易混淆此处$\alpha=0$与之前介绍初相时说$\varphi$对应着$x=0$的含义，$\alpha=0$对应的是相位整体为$0$，是对正弦函数$\sin x$而言的，是$\sin x$在初始位置的相位。而初相则是对$x$轴而言的，由于在信号分析时，$x$轴通常代表时间，所以初相指的是初始时刻的相位。
...
\begin{itemize}
\item 相位视角：将 $\displaystyle2x + \frac{\pi}{3}$ 视为整体，表示函数的输入值变化方式。$\omega = 2$ 使得输入值增长速度变快，而 $\displaystyle\varphi = \frac{\pi}{3}$ 使得初始位置发生偏移。如果要用平移和伸缩来表示的话，理解为先拉伸后平移，但在图像上平移的量应根据尺度变化为。
...
这两种分析方式在数学上是等价的。但之前提到过，为避免因尺度变化带来的问题通常存在平移和伸缩时先进性平移在进行伸缩变换。相位视角能够更直接地解释 $\omega$ 和 $\varphi$ 对图像的影响，也避免了伸缩变换不保持距离的影响。掌握这两种不同的理解方式，有助于更灵活地分析正弦型函数的变化。
...
另外，根据上述经验，对于正切函数，图像的关键特征在于其渐近线和对称点的位置。因此，通常只需在一个周期内确定三个关键点，便可大致勾勒出函数图像的形态。
