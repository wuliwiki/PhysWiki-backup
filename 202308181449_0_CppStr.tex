...
\item \verb|char *strcat(char *dest, const char *src);| 把 \verb|src| 添加到 \verb|dest| 末尾（若没有分配足够的空间则会内存溢出） 返回 \verb|dest|。 一个更安全的函数是 \verb|char *strncat(char *destination, const char *source, size_t num);|。
\end{itemize}
