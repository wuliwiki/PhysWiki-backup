...
重要提示：WSL1 用于 NTFS 文件系统最合适，包括移动硬盘。 exFAT 兼容性较差（不兼容软链， git 也会有权限 bug）。 NTFS 用 WSL1 也不完美， 有时候 \verb|chmod -R| 不能真正的深入到每个子目录，还可能要 cd 进去再次用 \verb|chmod -R|，或者直接 \verb|find . -type f -exec sudo chmod 777 {} \;|。且这么做只能临时有效，打开另一个命令行就没用了。
