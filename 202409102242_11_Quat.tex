...
设单位四元数$q=\left(\cos\frac{\theta}{2},\hat{\mathbf{n}}\sin\frac{\theta}{2}\right),\bvec v$为任意向量，则$q\bvec vq^{-1}$表示$\bvec v$绕$\hat {\bvec n}$旋转$\theta$\footnote{证明见\href{https://wuli.wiki/online/QuatN.html}{四元数与旋转矩阵}一节。}。易证，$q$与$-q$都代表同一个旋转，因此这是一个覆叠映射。设
又因为$q_1q_2\bvec v q_2^{-1}q_1^{-1}$
