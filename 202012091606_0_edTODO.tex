...
\item 自动补全括号 \verb|()[]{}| 的算法需要和 vscode 的行为一样， 以 \verb|{| 为例： 1. 插入 \verb|{| 时， 只有右边是空格、换行符、enf of file、 \verb|()[]{}| 时才补全右括号． 2. 选中几个字符并插入 \verb|{| 时， 自动在选中字符的左边插入 \verb|{|， 右边插入\verb|}|． 3. 在插入 \verb|}| 时， 如果右边存在自动配对的 \verb|}|， 则不插入， 光标移动到 \verb|}| 的右边．
