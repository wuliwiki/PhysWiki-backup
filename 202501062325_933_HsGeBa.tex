...
\textbf{圆（circle）}是平面内与某个定点距离等于定值的所有点组成的图形。该定点称为\textbf{圆心（center）}，定值称为\textbf{半径（radius）}，通常记作$r$。
...
由上述定义可以看出，两点 $A$ 和 $B$ 唯一确定一条弦 $AB$，一条$\overset{\frown}{AB}$ 和一个圆心角。因此可以称他们三者对应，如：是互相关联的几何对象。通过圆心的弦称为圆的\textbf{直径（diameter）}，其长度是半径的两倍，即：
\begin{equation}
d = 2r~.
\end{equation}
