...
\subsection{非平动参考系}
非平动参考系相对于惯性系除了平移运动还可能做相对旋转运动。对于非平动参考系， \autoref{eq_Iner_2} 并不满足（即使把 $\bvec a$ 看做位置和时间的函数也不行）， 以后在“\enref{科里奥利力}{Corio}” 中我们会推导 $\bvec a_{S}$ 和 $\bvec a_{S'}$ 间的具体关系。 所以我们不能得到\autoref{eq_Iner_5}。 由\autoref{eq_Iner_3} 和\autoref{eq_Iner_4}， 对任何非惯性系都成立的结论只有
\begin{equation}
