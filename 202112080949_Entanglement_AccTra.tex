...
\subsection{无相对转动}
若两个参考系之间只有平移没有转动， 两个参考系中任意两个固定点之间的加速度都是相等的（类比\autoref{Vtrans_eq1}~\upref{Vtrans}）． 令某时刻点 $P$ 相对于 $S$ 系和 $S'$ 系的加速度分别为 $\bvec a_S$ 和 $\bvec a_{S'}$， 再令两坐标系中任意两个固定点（例如各自的原点）之间的加速度为 $\bvec a_r$， 那么有
...
解： 我们令 $S'$ 的原点为点 $A$， 且 $S'$ 系无相对转动． 那么在 $S'$ 系中， 边缘上一点 $P$ 的角速度等于 “相对于接触点的角速度” 加 “接触点的角速度”
\begin{equation}
...
另一种方法是假设 $S'$ 相对 $S$ 系以 $\omega$ 逆时针旋转， 此时必须考虑科里奥利加速度， 见\autoref{AccTra_exe1}．
\end{example}
