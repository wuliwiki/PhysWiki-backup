...
$\delta(x)$ 在泛函分析中并不是数学中一个严格意义上的函数， 而是被称为\textbf{广义函数（generalized function）}或\textbf{分布（distribution）， 详见泛函分析}． 但这里我们给出另含有 $\delta(x)$ 等式的另严谨的理解： 即把 $\delta$ 函数看作是\textbf{对一系列函数取极限的过程}．
...
\subsubsection{三维 delta 函数}
\addTODO{……}

\subsection{一些应用}
若一个表达式中出现了 $\delta$ 函数， 那么其意义是先将 $\delta(x)$ 替换为 $f_\alpha(x)$． 
