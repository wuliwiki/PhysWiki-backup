...
\centering
\includegraphics[width=8cm]{./figures/FunLim_7.png}
\caption{在$(x_0-\delta, x_0+\delta)$区间内，$f(x)>0$} \label{FunLim_fig9}
\end{figure}
局部保号性的一个幼稚“证明”：如\autoref{FunLim_fig9} 所示（其实就是\autoref{FunLim_fig7} ），我们可以取一个$0< \epsilon < A$，极限的定义保证了我们总能找到对应的$\delta$，且在$(x_0-\delta, x_0+\delta)$区间内，$f(x)>0$．
