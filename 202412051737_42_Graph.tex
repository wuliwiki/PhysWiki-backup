...
设 $G$ 是图，若 $\abs{G}\in\mathbb N$，则称 $G$ 是\textbf{有限的}（finite）；若 $\abs{G}$ 和自然数集的基数相同，即 $\abs{G}=\aleph_0$（记号见\enref{集合的基数}{CardiN}），则称 $G$ 是\textbf{可数的}（countable）；否不是以上两种情况，则称 $G$ 是\textbf{无限的}（infinite）。 $\abs{G}=0,1$ 的有限图 $G$ 称为\textbf{平凡的}（trival）。其中 $\abs{G}=0$ 的图 $G$ 又称为\textbf{空图} （empty graph）。 
\end{definition}
显然，空图就是 $(\varnothing,\varnothing)$。平凡图由于没有边，
