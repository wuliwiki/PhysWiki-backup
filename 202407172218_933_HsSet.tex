...
\begin{definition}{集合}
一定范围内，某些能够确定的（well-defined，也称良定义的）、不同的对象（object）构成的整体（collection）称为集合\textbf{集合}（set），常用大写拉丁字母指代$A,B,C,D,\cdots$。

构成集合的每个对象则称为该集合的\textbf{元素}（element），常用小写拉丁字母指代$a,b,c,d,\cdots$。
\end{definition}
