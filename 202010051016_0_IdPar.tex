...
我们可以把 $\uparrow\uparrow$ 和 $\downarrow\downarrow$ 视为自旋方向相同的态， 剩下两个视为自旋方向相反的态． 对于自旋相反的两个态， 我们只能说 “一个电子自旋为 $\uparrow$ 另一个为 $\downarrow$” 却不能说哪个是 $\uparrow$ 哪个是 $\downarrow$． 这正体现了全同粒子的不可区分性． 相比之下， $\uparrow\downarrow$ 就表明第一个电子是 $\uparrow$， 第二个是 $\downarrow$， 所以不能对两个全同粒子使用．

一般地， 对于自旋量子数为 $s$ 的两个全同粒子， 我们可以将双粒子自旋态记为 $\chi_{1,2}$， 是 $(2s+1)^2$ 维空间中的态矢． 那么全同粒子要求
\begin{equation}
