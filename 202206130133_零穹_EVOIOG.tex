...
设 $A,B$ 是两正交矩阵，则 $(AB)^T(AB)=B^TA^TAB=E$（几何意义就是将一个标准正交基底 $e_i$ 通过 $A$ 转换到另一标准正交基底 $e'_i$，再将 $e'_i$ t'g转到另一标准正交基底 $e''_i$，所得结果），即正交矩阵的矩阵乘法也是正交矩阵（封闭性）．由于矩阵乘法满足结合律，并且 $A$ 有逆元 $A^T$ ，且 $E$ 也是正交矩阵，所以 $O(n)$ 构成一个群\upref{Group}．
