...
\end{lemma}
    ...
    \]
    由迫敛性即可得证.
\begin{corollary}{Stirling公式}

\end{corollary}
现在给出的Stirling公式的一种推导方法.应用\autoref{EMSum_eq3} 就有
\begin{equation}\label{EMSum_eq24}
    \log n!=\sum_{k=1}^{n}\log k=\left(n+\frac{1}{2}\right)\log n-n+C_{1}+R_{n}
\end{equation}
其中
\[
    C_{1}=1+\int_{1}^{\infty}\frac{\psi(x)}{x}\mathrm{d}x,R_{n}=-\int_{n}^{\infty}\frac{\psi(x)}{x}\mathrm{d}x
\]
将\autoref{EMSum_eq24} 代入 \autoref{EMSum_eq22} 可得
\[
    \frac{\pi}{2}=\lim_{n\to\infty}\frac{ne^{4R_n-2R_{2n}}}{2(2n+1)}e^{2C_{1}}=\frac{e^{2C_{1}}}{4}\Rightarrow C_{1}=\frac{1}{2}\log(2\pi)
\]
这就得到了Stirling公式．
