...
\begin{definition}{模}
称 $\norm{  v}=\sqrt{(  v|  v)}$ 为矢量 $  v$ 的\textbf{模}．
\end{definition} 
...
\begin{theorem}{}
设 $(  e_1,\cdots,  e_n)$ 是埃尔米特空间 $V$ 上的一个标准正交基底，那么
\begin{enumerate}
\item \begin{equation}\label{HVorUV_eq3}
\forall  x\in V,  x=\sum_{i}(  e_i|  x)  e_i
\end{equation}
\item 帕塞瓦尔等式
\begin{equation}
\forall  x,  y\in V,(  x|  y)=\sum_{i}(  x|  e_i)(  e_i|  y)
\end{equation}
\item \begin{equation}
\  x\in V,\Rightarrow\norm{  x}^2=\sum_{i}\abs{(  e_i|  x)}^2.
\end{equation}
\end{enumerate}
\end{theorem}
\textbf{证明：}\begin{enumerate}
\item 设 $  x=\sum_{i}x_i  e_i$，则
\begin{equation}
(  e_i|  x)=\sum_{j}x_j(  e_i|  e_j)=x_i
\end{equation}
\item 由\autoref{HVorUV_eq3} 
\begin{equation}
(  x|  y)=(  x|\sum_{i}(  e_i|  y)  e_i)=\sum_{i}(  e_i|  y)(  x|  e_i)=\sum_{i}(  x|  e_i)(  e_i|  y)
\end{equation}
