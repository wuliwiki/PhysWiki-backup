...
\end{equation}
其中，$\qty(\dv{X}{\zeta})^2=-\eta_{\mu\nu}\dv{X^\mu}{\zeta}\dv{X^\nu}{\zeta}$。这一作用量在 $m=0$ 时是有效的。因此我们，只需要验证\autoref{eq_RAct_8} 和\autoref{eq_RAct_9} 等价即可。注意\autoref{eq_RAct_9} 的动力学变量不光是 $X^\mu$，也包含 $\sigma(\xi)$。而 $\dv{\sigma}{\zeta}$ 不在作用量中出现，因此由Euler-Lagrange方程，得 $即得
\begin{equation}
