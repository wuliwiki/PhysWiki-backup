...
\begin{enumerate}
    ...
    \begin{enumerate}
        \\item 质点作平面运动，运动方程为 $\\vec{r} = (3t + 1) \\hat{i} + (t^2 - 1) \\hat{j}$，则t时刻质点的速度为\\underline{\\hspace{3cm}}（1），加速度为\\underline{\\hspace{3cm}}（2）。
        
        \\item 质量 $m$ 均匀分布的链条，总长为 $L$，有长度b伸在桌外。若自静止释放，则链条全部脱离光滑桌面的速度为\\underline{\\hspace{3cm}}（3），加速度为\\underline{\\hspace{3cm}}（4），若以桌面为势能零点，此时的势能为\\underline{\\hspace{3cm}}（5）。
        
        \\item 如图，曲线I表示 \$27^\\circ C$ 的氧气分子的Maxwell速度分布，则图示中 $v_1 = \\underline{\\hspace{3cm}}$（6）。设曲线II也表示氧气分子某一温度下的Maxwell速度分布，且 $v_2 = 600 \\, m/s$，则曲线II对应的理论体温标 $T_2 = \\underline{\\hspace{3cm}}$（7）。
