...
$$
需要注意的是，在**可压缩流**的情况下，压力 $p$ 不再单纯与各向同性应力项成比例，因为多了一个**体黏度项**：
...
需要强调的是，**体黏度** $\zeta$ 和**动力黏度** $\mu$ **不一定是常数**。
对于**单一化学组分**的流体，这两个传输系数通常取决于两个热力学变量，例如**压力**和**温度**。任何显式地将这些传输系数与守恒变量联系起来的方程，都被称为**状态方程（equation of state）**\[8]。
