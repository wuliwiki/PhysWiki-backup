...
\item 如果想在本地使用 Jupyter Notebook 中的 Cling （推荐）， 用 \enref{\verb|conda|}{CondaN} 安装即可。 Xeus-Cling 不支持 Windows 版的 conda（2023/8/13）。可以用 WSL2 安装 conda 再装 Cling。 WSL1 不行，安装成功，但启动 jupyter notebook 失败（2023/8/13）。
