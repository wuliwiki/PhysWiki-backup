...
\item 假设对 $n=k-1$ 的矢量空间 $V_{k-1}$，命题成立．设 $(\bvec e_1,\cdots,\bvec e_k)$ 是 矢量空间 $V_k$ 的初始基底，考查 $k-1$ 维子空间
\begin{equation}
\langle\bvec e_1,\cdots,\bvec e_{k-1}\rangle
\end{equation}
设 $\overline q=q|_L$ 是 $q$ 在 $L$ 上的限制，则型 $\overline q$ 的矩阵 $\overline F$ 是由 $F$ 去掉最后一行与最后一列得到的．
\end{enumerate}