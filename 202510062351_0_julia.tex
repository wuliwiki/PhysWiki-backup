...
\item 定义后无法动态增减字段。
\item \verb`fieldnames(类型)` 查看所有字段，\verb`nfields(类型)` 是数量， \verb`fieldtypes(类型)` 查看对应的类型。 \verb`dump(类型)` 查看详情， \verb`methods(类型)` 查看有哪些函数（似乎只能显示构造函数）。
...
\item \verb`for (i, e) in enumerate(my_tuple)`
\end{itemize}
