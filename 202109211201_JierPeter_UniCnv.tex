...
设给定定义域上，函数列$\{f_n\}$和$\{g_n\}$分别一致收敛到$f$和$g$，且$f_n, g_n$都是有界函数，那么$\{f_ng_n\}$一致收敛到$fg$上．
\end{theorem}

\autoref{UniCnv_the2} 的一个证明思路提示：$g_n$有界且一致收敛，那么$g$也就有界．由一致收敛的定义，任取$\epsilon>0$，存在正整数$N$，使得编号$n>N$时，所有
