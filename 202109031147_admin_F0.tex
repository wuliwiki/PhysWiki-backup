...
\end{equation}
\end{theorem}
该定理笔者同样不会证， 但同样符合直觉． \autoref{F0_eq5} 告诉我们， 震荡的频率未必需要是恒定， 也可以改变．

\subsection{两组正交的函数基底}
定义零函数有什么用呢？ 我们可以以此判断两组连续的函数基底（例如量子力学的一维散射态\upref{ScaNrm}）是否正交． 在 “$\delta$ 函数\upref{Delta}” 中， 我们知道若两个含参数的函数满足（$\delta_n$ 是 $\delta$ 函数列）
