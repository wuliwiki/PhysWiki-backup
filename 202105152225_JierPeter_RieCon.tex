...
给定一个实流形$M$，定义其上一个映射$g$，它将$M$上任意点处的两个切向量映射为一个数字．如果$g$满足对于任意向量$x, y, z$，都有：
\begin{enumerate}
\item \textbf{线性性}：$g(ax+bz, y)=a\cdot g(x, y)+b\cdot g(z, y)$，其中$a, b$为任意实数；
...
假设存在向量场$X, Y$使得对于任意$Z$都有$g(X, Z)=g(Y, Z)$，那么由黎曼度量的线性性知，$g(X-Y, Z)\equiv 0$．

由$Z$的任意性，取$Z=X-Y$，知$g(X-Y, X-Y)\equiv 0$．
