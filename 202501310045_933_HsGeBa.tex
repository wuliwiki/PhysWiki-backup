...
有一个角为直角的三角形称作\textbf{直角三角形（right triangle）}，\textbf{三角函数（Trigonometric Functions）}描述了直角三角形中角度与边长之间的关系，是几何和数学分析中的重要概念。

考虑一个直角三角形，其中一个锐角为 $\theta$，则三角形的三个边分别称作对边、邻边和斜边，具体说来：
\begin{itemize}
\item 直角三角形中最长的一边称作\textbf{斜边（Hypotenuse）}，对应于直角的对边。
\item 角 $\theta$ 所在的边非斜边的边称作\textbf{邻边（Adjacent）}。
\item 所讨论角 $\theta$ 对面的边称作\textbf{对边（Opposite）}。
\end{itemize}

角的大小或

在直角三角形中，三角函数主要包括\textbf{正弦（sine）}、\textbf{余弦（cosine）}和\textbf{正切（tangent）}。

\paragraph{正弦、余弦、正切的定义}

考虑一个直角三角形，其中一个锐角为 $\theta$，其对边、邻边和斜边分别记为：
\begin{itemize}
    \item 斜边（Hypotenuse）：直角三角形中最长的一边，对应于直角的对边。
    \item 对边（Opposite）：所讨论角 $\theta$ 对面的边。
    \item 邻边（Adjacent）：角 $\theta$ 所在的边（但不是斜边）。
\end{itemize}
...
常用的记忆方法：
\[
\sin = \frac{\text{对边}}{\text{斜边}}, \quad \cos = \frac{\text{邻边}}{\text{斜边}}, \quad \tan = \frac{\text{对边}}{\text{邻边}}
\]
可以使用“\textbf{正弦对斜，余弦邻斜，正切对邻}”的口诀来帮助记忆。

\paragraph{特殊角的三角函数值}
常见角度 $0^\circ, 30^\circ, 45^\circ, 60^\circ, 90^\circ$ 的三角函数值如下：
\[
\begin{array}{c|c|c|c}
...
这些基本定义在后续学习中会扩展到单位圆、余切、正割等更广泛的三角函数概念。
