...
网页版百科分为两个界面：
\begin{itemize}
\item \textbf{原界面} \verb|wuli.wiki/online| 静态词条。数据以 html 方式存储。网址 \verb|wuli.wiki/online/AU.html| 中 AU 是文章的 id， 每个文章都有自己的唯一 id， 即使文章被重命名也不会变。  
\item \textbf{新界面} \verb|wuli.wiki/book| 动态词条。数据保存在数据库中。相当于在原界面上套壳，增加了评论，搜索等功能，目录也更美观。 每天准点爬取 \verb|wuli.wiki/online/| 中的内容进行更新， 新界面中文章的网址如 \verb|wuli.wiki/book/AU|。
