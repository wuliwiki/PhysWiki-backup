...
\begin{enumerate}
\item proper orthochronous($\Lambda_{+}^{\uparrow}/SO^+(1,3)$)，对应 $\det\Lambda=1\quad \Lambda_{0}^{0}\geqslant1$。
...
首先，可以证明，所有的洛伦兹变换都可以写成 $\Lambda_{+}^{\uparrow}$ 与时间反演变换 $ \Lambda_{T}$ 以及宇称变换 $\Lambda_{P}$ 的乘积。在一些书上，会把 $\Lambda_{+}^{\uparrow}$ 写成 $\mathcal{S O}(3,1)^{\uparrow}$，表明这是\textbf{Special Orthogonal groups}，\textbf{Special}指的就是行列式为1这个性质。所以
...
$$
所以只需要对 $\mathcal{S O}(3,1)^{\uparrow}$，即 $\Lambda_{+}^{\uparrow}$ 进行研究即可。
\subsection{举例}
\begin{enumerate}
