...
\item \verb`docker run -it -d 镜像` 从镜像创建容器． 反复使用会创建多个． 这里的 \verb|镜像| 是指 \verb|用户/repo名：|
\item \verb`docker exec -it 容器ID bash` 进入某个 container 的 bash． 如果该镜像的默认 user 不是 root， 那么在 \verb|-it| 前面加上 \verb|-u 0| 选项可以以 root 的身份 login．
