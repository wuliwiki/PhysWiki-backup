...
\begin{equation}
\begin{aligned}
&C_a A_{+,1}\E^{-\I\phi_{+,1}} + C_b A_{+,2}\E^{-\I\phi_{+,2}} = 0\\
&C_a A_{-,1}\frac{\E^{-\I\phi_{-,1}}}{2\I} + C_b A_{-,2}\frac{\E^{-\I\phi_{-,2}}}{2\I} = \frac{1}{\sqrt{2\pi}}
\end{aligned}
\end{equation}
这就可以求出 $C_a, C_b$， 以及 $\psi_{k,a}, \psi_{k,b}$．

可以证明如果 $V(x)$ 是对称的（$A_{\pm,i} = 1/\sqrt{2}$， $\phi_{+,i}=\phi_{-,i}$）， 那么 $\psi_b$ 就和 $\psi_a$ 镜像对称（\autoref{ScaNrm_fig1} ）
\begin{equation}
