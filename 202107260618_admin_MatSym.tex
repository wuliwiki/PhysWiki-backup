...
\subsection{任意精度计算}
注意任意精度计算功能往往和符号计算一同使用， 但该功能本身却可以独立使用． 任意精度计算本质上还是数值浮点计算， 存在数值误差． 只不过我们可以任意规定每个变量的有效数字位数， 而不是统一使用双精度类型的 16 位有效数字．

\begin{itemize}
\item 如果使用 \verb|eval| 求一个表达式的值， 那么它的有效数字将不足 16 位． 但 Matlab 也提供更精确的
\end{itemize}
