...
于是有
\begin{equation}
\Omega(t)\ket{s} = \sum_ic^i(0)\ket{a_i; t} = \sum_ic^i(0)\Omega(t)\ket{a_i; 0}
\end{equation}



回到薛定谔绘景，此时保持基向量$\ket{a_i; 0}=\ket{a_i}$不变，而态矢量$\ket{s}$按时间演化算符$\mathcal{U(t)}$改变．两个绘景下，态矢量的展开系数相同，于是有：
\begin{equation}\label{HsbPic_eq14}
\mathcal{U(t)}\ket{s} = \sum_i c^i(t) \ket{a_i; 0}
\end{equation}
