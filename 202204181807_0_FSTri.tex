...
\end{figure}
理论上虽然随着项数变多三角级数会收敛于方波， 但是容易发现在函数不连续点的附近， 这个收敛会比较慢， 而且会出现高频振荡． 这被称为\textbf{龙格现象}， 在多项式插值\upref{InterP}中同样会遇到．
\end{example}
...
在实际操作中要如何选择各种方法呢？ 前两种方法实际上是等价的， 把\autoref{FSTri_eq9} 利用两两角和公式（\autoref{TriEqv_eq1}~\upref{TriEqv}）拆开即可证明． 另一个主要的考虑因素是\autoref{FSTri_ex1} 提到的龙格效应． 若 $f(a) = f(b)$， 那么用前两种方法把 $f(x)$ 拓展为周期函数后， $[a,b]$ 的两个端点不是间断点， 这就可以避免此处的龙格效应． 特殊地， 若 $f(a) = f(b) = 0$， 那么用第三种方法中的 $\sin$ 展开也无妨， 若区间两端的导数 $f'(a) = f'(b) = 0$， 那么当然最好只用 $\cos$ 来展开．
