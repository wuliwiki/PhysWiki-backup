...
\item 打开词条后如果有一次备份（见下文\textbf{新备份算法}）则标记占用。 如果不产生新的备份（即新的文件 hash）则不要占用。
...
\subsubsection{新备份算法}
\begin{itemize}
\item editor 备份步骤： 更新 \verb|contents/文件名.tex|， 然后运行 \verb|PhysWikiScan --backup 文件名|，不需要修改数据库。以下备份规则和用户是否重复关闭打开词条无关。
