...
\begin{theorem}{典则形式}
标准形式\autoref{eq_LinPla_2} 等价于如下的典则形式
\end{theorem}
...
z=c^T x, Ax=b,~
\end{equation}
其中 $x=(x_1,\cdots,x_n)^T,A=(a_{ij}),b=(b_1,\cdots,b_n)^T,c=(c_1,\cdots,c_n)^T$。设 $\rank A=m$（否则可将某些约束条件去掉变成这一情形，），总可以调换\autoref{eq_LinPla_2} 中 $x_i$ 之间的顺序，使得 $A$ 中 $m$ 个线性无关的列在前 $m$ 列。因此，总可以认为矩阵 $A$ 的前 $m$ 列是线性无关的。
