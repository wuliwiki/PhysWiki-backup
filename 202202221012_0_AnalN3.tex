...
\item 11.22 设 $f$ 为可测函数， 考虑两积分 $\int_E f^\pm \dd{\mu}$． 如果其中至少有一个有限， 我们定义 $\int_E f\dd{\mu} = \int_E f^+\dd{\mu} - \int_E f^-\dd{\mu}$． 若两个积分都有限， 那么左边也有限， 就说 $f$ 在 $E$ 上对 $\mu$ 是 \textbf{Lebesgue 可积}的， 记为 $f\in \mathscr L(\mu)$
\end{itemize}
