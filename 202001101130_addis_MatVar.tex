...
a = 1.5625
\end{lstlisting}
如果新的变量第一次被赋值，它会自动出现在 Workspace 窗口中．注意 Workspace 中的一个特殊的变量 \lstinline|ans|，如果命令的输出结果没有赋值给变量，就会自动赋值给 \lstinline|ans|．注意一般不要对 \lstinline|ans| 赋值． 另外两个特殊的变量是 \lstinline|pi| （圆周率）和 \lstinline|i| （虚数单位）， 一般也不要对他们赋值\footnote{一种好习惯是在虚数单位前加数字， 而不单独使用 \lstinline|i|． 例如 \lstinline|1i|， \lstinline|1+3i|}． 自然对数底没有对应的变量， 若要使用自然对数底， 用 \x{exp(1)} 即可．
