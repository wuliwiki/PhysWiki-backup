...
\item 如果上传时发现图片文件的 hash 已存在于 \verb|images.hash|， 则检查 \verb|images.figures| 中 \verb|figures.aka| 为空的图片环境（只有一个或者没有，否则就是 bug）。 如果有但被标记了 \verb|figures.deleted|， 那就看看 \verb|images.figures| 中其他图片环境有没有没被 \verb|deleted| 的， 有就提示已存在， 是否引用（\verb|\autoref{}|）（推荐用户这么做）
