...
\begin{itemize}
\item 先用 VScode 打开文件夹， 按左边的 debuug 图标 （圆圈里面有小虫子被斜线划掉）， 弹出来的窗口中按齿轮按钮， 会自动创建 \verb`.vscode/launch.json` 设置文件， 不用做任何改动，现在就可以 debug 了．
\item 可以在行号左边点击添加 breakpoing， 快捷键 F5，F10，F11 都跟 Visual Studio 中的一样．
\end{itemize}
