...
应用斯涅尔定律\upref{Snel}，可以进一步使记号简化．别害怕，它们只是看起来很复杂：
...
\end{equation}
\begin{equation}%\label{Fresnl_eq}
...
\subsection{布儒斯特角}
我们这里考虑常见的 $n_2>n_1$ 且 $\mu_1 = \mu_2$ 情况．由\autoref{Fresnl_eq8} 容易证明当 $\theta_i + \theta_t = \pi/2$时， $r_p = 0$, 反射光的平行分量消失．此时，入射角称为\textbf{布儒斯特角（Brewster's angle）} ． 由斯涅尔公式 $$布儒斯特角等于
\begin{equation}
