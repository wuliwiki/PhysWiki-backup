...
ii) 当 $\lambda < 0$ 时。此时通解为 $y = C_1 \cos(\sqrt{\lambda} x) + C_2 \sin(\sqrt{\lambda} x)$ 即 $y= C_1 \cosh(\sqrt{-\lambda} x) + C_2 \sinh(\sqrt{-\lambda} x)$。考虑到 $y(0)=0$ 故 $C_1 = 0$，代入 $y(L) = 0$ 可得：$C_2 \sinh(\sqrt{-\lambda} L) = 0$，故 $\sinh(\sqrt{-\lambda} L) = 0$。