...
\item 2.7 数列是 $J$ 的映射 $f(n) = x_n$， 记为 $\{x_n\}$． $x_n$ 叫做一项． 如果 $x_n \in A$， 那该序列就叫 $A$ 中（元素）的序列．

\item 2.8 可数集的无穷子集仍然是可数的

\item 2.9 \textbf{Sequence of set} $\{E_\alpha\}$， 每个 $\alpha \in A$ 都对应一个 $E_\alpha$（其实就是 set of sets， 但不这么叫）
