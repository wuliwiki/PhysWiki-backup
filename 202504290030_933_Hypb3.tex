...
有了引理的支持，证明过程就比较容易了。如\autoref{fig_Hypb3_12} 所示，设 $P$ 是双曲线上的任意一点\footnote{以下假设在$F_1$一侧，不影响证明。}，$l$ 是过 $P$ 的切线，在 $l$ 上任取另一点 $P'$（$P'\neq P$），连接 $PF_1$、$PF_2$ 以及 $P'F_1$、$P'F_2$。由于 $P'$ 在双曲线外部，而 $F_1,F_2$ 位于双曲线内部，因此线段 $P'F_1,P'F_2$ 必定与双曲线两支分别交于某一点，记为 $Q,R$，并连接 $QF_2,RF_1$。

\begin{figure}[ht]
\centering
\includegraphics[width=12cm]{./figures/0be46ab66e6c7f2e.png}
\caption{证明反射性质} \label{fig_Hypb3_12}
\end{figure}

根据双曲线的几何定义，因为 $P$ 和 $Q$ 位于双曲线的同一支上，$P$ 和$R$位于双曲线的不同支上，所以有：
...
&\leq |( |PF_2|-|PF_1|)|+|(|P'F_2|- |P'Q|  - |QF_2|)| \\
&= (|PF_2|-|PF_1|) - (|QF_2|+|P'Q|-|P'F_2|)\\
&< ||PF_1| - |PF_2||~.
\end{split}
\end{equation}
...
\item $\leq$处利用了三角不等式$|x+y|\leq|x|+|y|$。
\item 第三个等号处利用了\autoref{eq_Hypb3_21} 和\autoref{eq_Hypb3_20} 中的符号打开了绝对值。
\end{itemize}
