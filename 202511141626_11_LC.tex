...
设在某一时刻，电容器极板上的电荷量为 $q$, 电感的电流为 $i$， 使用被动符号规定（\autoref{sub_Resist_1}）， 并取 LC 回路的顺时针方向为电流的正方向， 那么电势沿该方向降低为正， 有环路电压之和为零：
\begin{equation}
\frac{q}{C} + L \frac{\mathrm{d} i}{\mathrm{d} t} = 0~.
\end{equation}
因为假定的电感电流是流入电容正极板，则假定的电容器正极板电荷量增加，所以有 $i=\dfrac{\mathrm{d} q}{\mathrm{d} t}$， 代入得
\begin{equation}
...
\end{equation}
则实际电路电流为$\omega Q_{0} \sin \left(\omega t+\phi_{0}\right)$。

令 $\omega Q_0=I_0$ 表示电流的最大值，称为\textbf{电流振幅}，则有
