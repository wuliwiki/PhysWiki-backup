...
&=\sum_{i,j,k}\sum_{m=0}^{n}(-1)^{n-m}C_n^m{a_i}^m{a_k}^{n-m}B^{i}_{\;k}B^k_{\; j}\ketbra{a_i}{a_j}\\
BA^{(n)}&=\sum_{i,j,k}\ketbra{a_i}{a_i}B\ketbra{a_k}{a_k}A^{(n)}\ketbra{a_j}{a_j}\\
&=\sum_{i,j,k}B^{i}_{\;k}\qty(A^{(n)})^{k}_{\; j}\ketbra{a_i}{a_j}\\
&=\sum_{i,j,k}\sum_{m=0}^{n}(-1)^{n-m}C_n^m{a_k}^m{a_j}^{n-m}B^{i}_{\;k}B^k_{\; j}\ketbra{a_i}{a_j}
\end{aligned}
\end{equation}
