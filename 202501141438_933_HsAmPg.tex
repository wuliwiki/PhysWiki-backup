...
d={a_n-a_k\over n-k}~.
\end{equation}
这一公式与\aref{平均变化率}{def_HsFunC_3}或\aref{斜率}{def_HsFunC_5}的定义极为相似，也符合之前等差数列与一次函数关系的判断。根据直线的经验，可以得到：
\begin{corollary}{等差数列的增减性}
对于公差为$d$的等差数列$\{a_n\}$：
\begin{itemize}
\item 如果$d>0$，则$\{a_n\}$是递增数列；
\item 如果$d<0$，则$\{a_n\}$是递减数列。
\end{itemize}
\end{corollary}这一点从等差数列的递推公式中也可以看出。

根据\autoref{eq_HsAmPg_1}，还可以得到等差数列的一个重要性质：取四个整数，满足 $m+n=p+q$，即 $p-n=m-q$，则有：
\begin{equation}
...
对于更特殊的从1开始的自然数列，（补充高斯求和的故事），得到的“首项加末项，乘以项数除以2”，于此处也是契合的。
