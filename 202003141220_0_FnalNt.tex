...
\item 令 $\mathcal D(G) := C_0^\infty(G)$． 令 $\phi_n, \phi \in \mathcal D(G)$． $\phi_n \to \phi$ 的定义是： (1) 存在 $G$ 的紧子集使得 $\phi_n(x) = 0$ 对所有 $x \in G-K$ 和 $n$ 成立， (2) 对所有的多重指标 $\alpha$， $K$ 上有一致收敛\upref{UniCnv} $\partial^\alpha \phi_n \to \partial^\alpha \phi(x)$．
...
\item $L_2(G) \subseteq \mathcal D'(G)$ 的意思是每个 $u\in L_2(G)$ 都对应（identified with）一个广义函数 $U(\phi) := \int_G u(x) \phi(x) \dd{x}$ 对所有 $\phi \in \mathcal D(G)$ 成立． $U \in \mathcal D'(G)$
\end{itemize}
