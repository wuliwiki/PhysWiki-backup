...
第一类切比雪夫多项式 $T_n$ 和第二类切比雪夫多项式 $U_n$ 都构成了一族**正交多项式**。

---

第一类多项式 $T_n$ 在区间 $[−1, 1]$ 上，相对于权函数：
...
这一点可以通过令 $x = \cos(\theta)$，以及利用恒等式 $T_n(\cos(\theta)) = \cos(n\theta)$ 推导而得。

---

类似地，第二类多项式 $U_n$ 在区间 $[−1, 1]$ 上相对于权函数：
...
（其中的测度 $\sqrt{1 - x^2} \, dx$，在归一化后正是**Wigner 半圆分布**。）

---

这些正交性来源于这样一个事实：切比雪夫多项式是下列**Sturm–Liouville 微分方程**的解：
...
而 Sturm–Liouville 微分方程普遍具有一组特殊的正交归一解。实际上，切比雪夫多项式也可以直接通过这些方程来定义。
