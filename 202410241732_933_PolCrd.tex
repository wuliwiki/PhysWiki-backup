...
此时$a=0$，代入\addTODO{引用式子出错。}可知，此时表达式为：
\begin{equation}
\theta=\alpha~.
\end{equation}
...
此时$\displaystyle\alpha={\pi\over2}$，代入\addTODO{引用式子出错。}可知，此时表达式为：
\begin{equation}
\rho\cos\theta=a~.
\end{equation}
若$a>0$则在极点右侧，否则在极点左侧。

2.与极轴平行，极点到直线距离为$|a|$。

此时$\alpha=0$，将\addTODO{引用式子出错。}变形有：
\begin{equation}
\rho\left(\sin\alpha\cos\theta-\cos\alpha\sin\theta\right)=a\sin\alpha~.
\end{equation}
可知，此时表达式为：
\begin{equation}
\rho\cos\theta=a~.
