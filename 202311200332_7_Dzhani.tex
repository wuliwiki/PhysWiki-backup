...
计算角速度大小的方式如\autoref{fig_Dzhani_10} 所示。影响位移向量角度变化的是切向速度$\bvec{v}_t$，在一小段时间$\Delta t$后，质点$X$运动到了$\bvec{r}+\Delta \bvec{r}$的位置，$\bvec{r}$和$\bvec{r}+\Delta \bvec{r}$之间的夹角为
\begin{equation}
...
我们分两步来解释贾尼别科夫效应：首先说明旋转刚体的旋转方向会受到扰动，然后说明扰动必须沿着特定路线进行，从而解释旋转
