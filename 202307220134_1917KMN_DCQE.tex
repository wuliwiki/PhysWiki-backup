...
后来，人们发现，根本不会出现以上那些诡异的现象，因为如果只看$D_0$，将不可能看到任何干涉条纹。由\autoref{eq_DCQE_9} 可以发现，不论后面对纠缠光子进行什么操作，$A$光子的概率分布中都没有干涉项。如果后面选择对光子的路径信息进行擦除，也就是将$BS_a$和$BS_b$去掉，就可以在$1$类分布图与$2$类分布图中分别看到干涉条纹。但是，这种方法仅能对$1$类路径和$2$类路径进行分别擦除，所以总分布仍然没有干涉条纹。至此，所有的矛盾都能顺利得到解释了。