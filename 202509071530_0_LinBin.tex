...
\item dll/so 中的符号可以设置编译器属性（非标准 C/C++）如 GCC 的 \verb`__attribute__((attribute_name))` MSVC 的 \verb`__declspec(attribute_name)`。注意双层括号不可省略为单层。其中 \verb`attribute_name` 在 Windows 中可以是 \verb`dllimport` 或 \verb`dllexport`；若用 GCC/Clang 在非 Windows 系统跨平台使用则用 \verb`visibility("default")`（不区分 import/export）。
\item GCC 默认 dll/so 库中的符号全部导出，而 MSVC 默认全部隐藏。 只有导出的符号才可以被其他库或可执行文件使用。
\item GCC 若要默认隐藏，可以通过编译器选项
...
#else
    #define API  // Unknown compiler
#endif

class API MyClass {  // Exported on all platforms
public:
    void method();
};
\end{lstlisting}
