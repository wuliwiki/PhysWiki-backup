...
\item $\omega x + \varphi$ 是之前未曾见过的表达形式。其思路是，将外部的 $\sin$ 视为一个黑箱，只需计算内部表达式的值，再通过 $\sin x$ 映射到函数值，相当于将其看作一个复合函数。这种方式可以避免三角函数带来的非线性直觉困难，使分析更加直观。

\item $\omega$ 称为\textbf{角频率（angular frequency，也译作圆频率）}，反映了函数的周期性变化速率。此前已经学习过\textbf{周期（period）}的概念，这里给出 $T$ 与 $\omega$ 的关系：
\begin{equation}
T = \frac{2\pi}{\omega}~.
\end{equation}
