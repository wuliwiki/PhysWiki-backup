...
与复数类似， 任何四元数都可以表示为模长乘以一个模长等于 $1$ 的四元数： $A = \abs{A} q$． 其中 $q$ 类似于复数的 $\E^{\I\theta}$． 而由于 $\abs{q} = q\tilde q = \tilde q q = 1$， 可见 $q$ 的共轭就是它的逆 $q^{-1} = \tilde q$． 这可以类比 $\E^{\I\theta}$ 的逆就是它的复共轭． $\E^{\I\theta}$ 可以代表一个角度为 $\theta$ 的平面旋转， 而在 “四元数与旋转矩阵\upref{QuatN}” 中会看到， 任意模长为 $1$ 的四元数都可以表示一个唯一的三维旋转． 两个复数相乘， 就是把它们的模长相乘， 幅角相加（\autoref{CplxNo_eq10}~\upref{CplxNo}）， 同理两个四元数相乘的模长也等于各自的模长相乘
\begin{equation}
\abs{AB} = \abs{A}\abs{B}
\end{equation}
...
定义四元数的除法为
\begin{equation}
\frac{A}{B} = AB^{-1} = \frac{A\tilde B}{\abs{B}^2} \qquad (B \ne 0)
\end{equation}

根据以上讨论不难发现， 所有的非零四元数及其乘法运算构成一个群\upref{Group}．
