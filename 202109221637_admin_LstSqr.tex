...
上式存在的一个问题是， 各个 $c_i$ 偏导为零， 就一定会是全局最小值吗？ 乍看之下我们较难证明任意阶情况的系数行列式都不为零， 那么会不会解出的是局部最小值，最大值，甚至鞍点\upref{DerMax}呢？ 首先要明确的是方差 $S_2$ 是一个关于 $c_i$ 的二次函数， 处处可导， 所以全局最小值必定满足各阶偏导为零， 也就是\autoref{LstSqr_eq2}． 再来考虑有多个解的情况， 根据线性方程组解的结构\upref{LinEq}， 若线性方程组存在多于一个解， 那么这些解必定不可能是分立的而是会构成一个连续的平面（不包含零点）， 在这个平面上， 由于各阶偏导处处为零， $S_2$ 必定处处相等， 所以只可能所有解都是全局最小值．
