...
我们把这个切线的斜率定义为$x=x_A$处$f(x)$的导数，一般记为$f'(x_0)$或$\dv{f}{x}|_{x=x_0}$．因此，我们说函数一点处的导数值就等于这点处切线的斜率．
...
类似于单侧极限\upref{FunLim}，我们也可以引入单侧导数的概念．如果B点从右侧趋近A点，但始终不运动到A点的左侧，那么此时切线的斜率即为该点处函数的右导数值，可以记为$f'_+(x_0)$．用极限的语言可以写为：
