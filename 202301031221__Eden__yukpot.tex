...
\end{aligned}
\end{equation}
在非相对论极限下，两个费米子之间是可区分的，那么第二项可以被忽略。此外由于 $|\bvec p|\ll 1$，有如下的近似：$p=(m,{\bvec p})+O(|{\bvec p}|^2),k=(m,{\bvec k})+O(|{\bvec k}|^2),\cdots$,$(p-k)^2=-|{\bvec p}-{\bvec k}|^2+O(|{\bvec p}|^4)$，因此对于 Dirac 旋量\autoref{diracs_eq4}~\upref{diracs}
\begin{align*}
