...
\end{itemize}
在子流形刚性问题的背景之外，丘成桐能够借鉴于尔根·莫泽证明 Caccioppoli不等式的方法，从而证明了完备黎曼流形上函数的新刚性结果。他的一个特别著名的结果是：一个次调和函数如果既是正的又是Lp可积的，除非它是常数。\(^\text{[Y76][52][65]}\)类似地，在完备的Kähler流形上，一个全纯函数如果是Lp可积的，除非它是常数。\(^\text{[Y76]}\)
