...
\item 文章类型， 格式为 \verb|type 类型id|， 其中 \verb|类型id| 支持： \verb|Wiki|（类似于维基百科的条目，中立、全面、一般性的介绍）， \verb|Tutor|（类似于教材的一节）， \verb|Art|（学术论文或杂谈等）， \verb|Map|（介绍百科中的一章、一部分等，具有大量链接到其中的词条）， \verb|Note|（类似于讲义、复习资料，把个知识的要点列出）。
\end{itemize}
