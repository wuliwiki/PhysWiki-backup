...
则光滑函数$\omega_{\sigma(1), \sigma(2), \cdots, \sigma(k)}$表示为$\omega_I$，微分形式$\dd x^{\sigma(1)}\wedge\dd x^{\sigma(2)}\wedge\cdots\wedge\dd x^{\sigma(k)}$表示为$\dd x^I$．

复杂指标也应用爱因斯坦求和约定：$\omega_I\dd x^I$是$I$遍历所有$k$次有序子集、或者特别声明的范围后，所得结果的乘积．
\end{definition}
