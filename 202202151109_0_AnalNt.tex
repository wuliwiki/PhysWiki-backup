...
\item 3.16 设 $\{s_n\}$ 是实数序列． $E$ 是所有可能的子序列的极限组成的集（可能含有 $\pm\infty$）． $s^* = \sup E$， $s_* = \inf E$ 这两个数叫做序列的\textbf{上极限}和\textbf{下极限}． 记为 $\lim_{n\to\infty} \sup s_n = s^*$， $\lim_{n\to\infty} \inf s_n = s_*$．
\end{itemize}
