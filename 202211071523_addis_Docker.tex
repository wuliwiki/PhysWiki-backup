...
\item 要下载 image， 用 \verb`docker pull 用户名/repo名:TAG` 其中 \verb|用户名/| 对于一些官方镜像可以忽略， 例如 \verb`docker pull ubuntu:22.04` 中， \verb|ubuntu| 是 repo， \verb|22.04| 是 TAG．
...
\item \verb`docker run -it -d IMAGE_NAME/IMAGE_ID` 从 image 创建 container
