...
\item \verb`add_library(exe_name source_name)` 单独编译一个 library （在 library 路径的 CMakeLists 中使用这个命令而不是 \verb`add_executable` 命令）
...
\item \verb`add_subdirectory(dir1 [dir2] ...)` 执行子路径中的 CMakeLists
