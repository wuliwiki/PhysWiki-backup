...
对数列$\{a_n\}$，若对于$n>1$的所有项，均有每一项与前一项的差是同一个常数$d$，则称数列$\{a_n\}$为\textbf{等差数列}，$d$为等差数列的\textbf{公差}，即等差数列$\{a_n\}$的递推公式为：

如果数列 ${a_n}$ 满足对于 $n > 1$ 的所有项，每一项与前一项的差为同一个常数 $d$，则称 ${a_n}$ 为\textbf{等差数列（arithmetic sequence）}，$d$ 称为\textbf{公差（common difference）}。等差数列的递推公式为：
\begin{equation}
