...
总是要两个数进行比较书写太麻烦了，根据集合的概念，得到了针对集合的最值。
...
\end{equation}
\end{definition}
如果一个集合是闭区间，也就是可以取到端点的，那么就在这个方向上存在最值，而如果一个集合是开区间，就无法取到端点，那么根据\aref{实数的稠密性}{sub_HsFunB_1}，任意给出的最值和端点之间都有其他的值，于是无法给出一个明确的最值\footnote{这时的端点称为区间的\textbf{确界}。}，这时称最值不存在。

对于有限集合，最值的确定相对简单，可以通过比较直接得到。而当集合包含无限多个元素时，讨论最值的范围超出了高中数学的要求，但可以借助区间的概念进行直观理解。

如果一个集合是闭区间，例如$[a, b]$，由于端点$a$和$b$属于集合，因此集合在这两个方向上都存在最值，最小值为$a$，最大值为$b$。相反，对于开区间$(a, b)$，端点$a$和$b$并不属于集合。根据\aref{实数的稠密性}{sub_HsFunB_1}，可以在任意给定的最值与端点之间找到其他的值，因此无法明确确定一个最值。这种情况下，称集合的最值不存在。

值得注意的是，对于开区间$(a, b)$，虽然最值不存在，但端点$a$和$b$在数学上被称为区间的\textbf{确界（supremum和infimum）}\footnote{确界是闭区间和开区间的统一描述，用于研究集合的上界与下界性质。}。确界并不一定属于集合，但它是所有集合元素的上界或下界中“最紧密的界”。例如，对于$(a, b)$，$a$是其下确界（infimum），$b$是其上确界（supremum）。
