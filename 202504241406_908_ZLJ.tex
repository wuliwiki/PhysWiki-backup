...
则存在唯一的映射
\[
\overline{q} : A \otimes_R B \to G
\]
使得：
\[
\overline{q}(a \otimes b) = q(a, b)
\]
对所有 \( a \in A \)、\( b \in B \) 成立。

这个性质说明：\( A \otimes_R B \) 是使所有中线性映射 \( A \times B \to G \) 因式分解的“**自由对象**”，从而赋予张量积强有力的范畴论刻画。
...
\item 若\( B \)是一个\( (R, S) \)-双模，则\(A \otimes_R B\)是一个**右 \( S \)-模**，其右作用定义为：\((a \otimes b)s := a \otimes (bs)\)。
