...
由于散射态不能用 $\braket{\psi}{\psi} = 1$ 的方式归一化， 它们并不是物理的， 即粒子不能处于某个散射态． 然而散射态的线性组合\footnote{这里指积分}却可能可以归一化． 例如自由粒子（$V \equiv 0$）的高斯波包\upref{GausPk} 就可以看作由平面波线性叠加而来， 即反傅里叶变换． 某种意义上， 给定一个势能函数， 所有的束缚态和散射态可以构成一组完备的正交归一的函数基底， 可以展开所有可归一化的波函数．

由于散射态不需要归一化， 一维散射态存在双重简并
