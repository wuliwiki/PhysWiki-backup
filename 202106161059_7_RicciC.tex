...
用抽象指标，将$X, Y, Z$分别表示为$x^i, y^j, z^k$，那么$T_{Y, Z}(X)=R^r_{ijk}y^jz^kx^i$，因此$T_{Y, Z}=R^r_{ijk}y^jz^k$．于是，$\opn{trace}T_{Y, Z}=R^i_{ijk}y^jz^k$\footnote{注意，就是把上指标$r$换成了$i$，因为对于矩阵（或者抽象指标表示的线性变换）$a^i_j$，其\textbf{迹}的定义就是$\opn{trace}$}
