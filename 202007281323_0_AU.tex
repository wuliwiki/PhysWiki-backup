...
在量子力学的许多理论或数值计算中，选用\textbf{原子单位（atomic unit）}会更方便． 需要特别注意的是， 原子单位下的物理公式在使用时的习惯和其他单位制有所不同， 例如会出现 $E = \omega$ （能量等于角频率， 见\autoref{AU_eq2}）这种看似不符合量纲分析的公式． 要理解这种公式， 一种方法是把使用原子单位的公式都理解成无单位的物理公式\upref{NoUnit}．
