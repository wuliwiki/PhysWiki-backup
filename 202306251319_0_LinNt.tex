...
\item 要改变 swap 文件的尺寸： \verb|sudo swapoff /swapfile|， \verb|sudo dd if=/dev/zero of=/swapfile bs=1M count=1024|（如果写入比较慢也可以 append： \verb|sudo dd ... oflag=append conv=notrunc|）， \verb|sudo mkswap /swapfile|， \verb|sudo swapon /swapfile| 一般无需重启， 但如果系统不允许关闭 swap， 也可以考虑用多个 swap 文件。有时候 \verb|/swapfile| 也叫做 \verb|/swap|。
