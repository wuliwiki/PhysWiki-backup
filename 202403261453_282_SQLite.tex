...
例如很常见的一个情况是，想把数据库以后的表中所有（或满足某个条件的）记录删除再重新写入新的版本。 但这样会导致程序很慢，尤其是如果有很多记录且新版本只修改了少数几条记录。另一个坏处就是会导致 dump 出来的 sql 文本文件有大量不一致，即使一模一样的记录 dump 后位置也会改变，不利于版本管理。 sql 没有一个稳定的 dump 算法，需要借助第三方工具。
