...
外代数最重要的例子是外微分\upref{ExtDif}，它是微分形式作为线性空间的外代数．$n$维线性空间$V$上的$k$次外代数可以视为$V$上的$k$维体积，而当$k=n$时就是我们所熟悉的行列式：取定线性空间$V$的基$\{\bvec{e}_i\}$，那么$n$个元素的外积$\sum\limits_{i}(a_{1i}\bvec{e}_i)\wedge\sum\limits_{i}(a_{2i}\bvec{e}_i)\wedge\cdots\wedge\sum\limits_{i}(a_{ni}\bvec{e}_i)=\vmat{a}$
