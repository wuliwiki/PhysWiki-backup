...
赋权矩阵是用来体现赋权函数的，具体地，设图 $G$ 的点集为 $V(G)=\{v_1,\cdots,v_n\}$，$\omega_1:V(G)\rightarrow\mathbb R,\omega_2:E(G)\rightarrow\mathbb R$ 分别是点赋权函数和边赋权函数，则点赋权矩阵是一个 $n$ 元行矢量或列矢量，其第 $i$ 个元反映了 $\omega_1(v_i)$ 的值；而边赋权矩阵的第 $(i,j)$ 个元反映了以 $v_i$ 为起点 $v_j$ 为终点的所有边的权值之和。
\begin{definition}{邻接矩阵}
设 $G$ 是图，$V(G)=\{v_1,\cdots,v_n\}$，$A=(a_{ij})$ 是 $n$ 阶矩阵。若 $G$ 是无向图，$a_{ij}$ 等于连接 $v_i,v_j$ 的边数；若 $G$ 是有向图，$a_{ij}$ 等于以 $v_i$ 为起点，$v_j$ 为终点的有向边数。则称 $A$ 为 $G$ 的\textbf{邻接矩阵}（adjacency matrix）。
\end{definition}
