...
给定域\upref{field} $\mathbb F$ （通常取实数域 $\mathbb R$ 或复数域 $\mathbb C$）上的线性空间 $V$ 和 $U$。 如果有映射\upref{map} $f:V\rightarrow U$ 满足： 对于任意的向量 ${\bvec{v}}_1, {\bvec{v}}_2\in V$ 和标量 $c_1, c_2 \in \mathbb{F}$，都有
\begin{equation}
f(c_1 {\bvec{v}}_1+c_2 {\bvec{v}}_2)=c_1f({\bvec{v}}_1)+c_2f({\bvec{v}}_2)~.
\end{equation}
