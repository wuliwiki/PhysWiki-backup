...
\item 要安装 ubuntu live 镜像, 先用命令行重写一个 partition table， 新建一个分区(fdisk 比 gparted 要可靠), 用 \verb|sudo dd bs=4M if=/path/to/ISOfile of=/dev/sda# status=progress oflag=sync| 注意 sda 后面一定要有数字！
\end{itemize}
