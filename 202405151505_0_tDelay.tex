...
\end{equation}
如果 $A(k)$ 是以 $k_0$ 为中心的偶函数和实函数，那么根据傅里叶变换的\autoref{the_FTExp_1}~\upref{FTExp} 和平移性质， $\psi(x,0)$ 就是一个偶函数和实函数乘以 $\E^{\I k_0 x}$。 则波包的运动轨迹就是 $x_c = k_0 t$。\addTODO{当 $t>0$ 时是否波包的形状也像高斯波包那样关于 $x_c$ 对称的呢（虽然肯定有 chirp）？}

根据以上结论，在此基础上若给 $A(k)$ 加上一个相位 $E$ 变化的相位变为 $C(k)$，那么此直线运动的延迟就是
\begin{equation}
