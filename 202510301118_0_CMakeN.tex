...
\item 当然， 也不是什么平台都可以用 \verb`make`， 所以也可以用 \verb`cmake --build 生成Makefile的路径 [--target 可执行文件] -- -j 12` 其中 \verb`--` 后面的选项会传给 \verb`make` 或者别的具体用于编译的程序。 更跨平台的多线程编译方法也可以 \verb`--parallel [线程数]`。
...
\item 像 \verb`include_directories(), link_directories(), add_definitions(), add_link_options()` 这些改变 global 状态的命令，只对在它们之后定义的目标生效
\end{itemize}
...
\item 为了更好的跨平台， \verb`命令` 可以使用 \verb`${CMAKE_COMMAND} -E 一些命令`。 其中 \verb`${CMAKE_COMMAND}` 就是当前使用的 \verb`cmake` 命令，\verb`-E` 代表 cmake 的命令执行模式， \verb`一些命令` 如 \verb`echo 字符串`（注意这不是 Linux 的而是 cmake 的 \verb`echo`），\verb`copy 文件 目标文件`，\verb`copy_if_different 文件 要被更新的文件`， \verb`make_directory 目录`， \verb`remove_directory 目录`， \verb`remove 文件` 等。
\item 例如：手动在命令行执行 \verb`cmake -E copy_if_different <source> <destination>` 就可以更新文件（而不用在乎是在 Linux 还是在 PowerShell）。
...
\item \verb`find_library(变量 NAMES 库名1 库名2 PATHS /usr/lib /usr/local/lib HINTS $ENV{LIBRARY_PATH} $ENV{LD_LIBRARY_PATH})` 可以寻找 \verb`lib库名.so或a[.版本号]`，然后把路径写入 \verb`变量`（不做其他事情）。 注意不会搜索 \verb`HINTS` 的子文件夹。 若要指定版本可以用例如 \verb`gfortran.3`， 也可以指定 \verb`gfortran.so或a`。 注意指定目录的子目录不会被搜索。
