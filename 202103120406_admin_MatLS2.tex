...
特殊地， 若 $A$ 是双射， 则定理中 $X_0 = \qty{0}$ 是零维空间， 补空间 $X_1 = X$． 此时可以把\autoref{MatLS2_fig1} 中表示 $X_0$ 的三角形去掉．
...
\begin{corollary}{}
线性映射 $A:X\to Y$ 是单射当且仅当\autoref{MatLS2_eq1} 中 $N_x = R$ 或 $N_0 = 0$．
\end{corollary}
证明： 前者证明后者显然． 后者证明前者： 令\autoref{MatLS2_the1} 中的 $Y = Y_1$ （即满射）满足 $N_X = R$， 根据\autoref{MatLS2_eq1}， $N_0 = 0$， 即零空间只有零矢量一个元素， 所以 $X = X_1$， 与 $Y_1$ 一一对应（双射）． 证毕．
