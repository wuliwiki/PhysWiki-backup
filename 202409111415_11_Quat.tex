...
设单位四元数$q=\left(\cos\frac{\theta}{2},\hat{\mathbf{n}}\sin\frac{\theta}{2}\right),\bvec v$为任意向量，则$q\bvec vq^{-1}$表示$\bvec v$绕$\hat {\bvec n}$旋转$\theta$\footnote{证明见\href{https://wuli.wiki/online/QuatN.html}{四元数与旋转矩阵}一节。}。易证，$q$与$-q$都代表同一个旋转，因此这是一个二对一的覆叠映射。
...
当我们讨论单位四元数同态于$SO(3)$时，$q\bvec v q^{-1}$所代表的旋转过程实际上是把四元数的矢量部分$\opn{Im}\mathbb H=\{a_1\uvec i+a_2\uvec j+ a_3\uvec k\}$同构为一般意义上的矢量空间$\mathbb R^3$。