...
\item list 的指标从 1 开始．
\item Mathematica 的矩阵是行主序\upref{MatSto}的， 可以把三维矩阵/数组第一个指标看作列， 第二个看作行， 第三个看作指向屏幕内的方向． 元素写成一行的顺序是先增加最右边（最内层）的指标， 再增加右边第二个…… 越左边的指标是最外层的 list． \verb|MatrixForm| 显示的时候仍然把前两个指标显示为矩阵， 但把矩阵元拓展为矢量或矩阵．
...
\item 对列表求和用 \verb|Total[{1,2,3}]|， 矩阵默认对第一个维度求和
\end{itemize}
