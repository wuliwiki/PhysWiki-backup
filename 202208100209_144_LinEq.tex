...
若 $m-r>0$，则解可能不存在.此时，若$\bvec y$是$\mat A$的列向量的线性组合($\bvec y \in Col(\mat A)$)，则$\mat A \bvec x=\bvec y$一定存在解．
\end{theorem}
...
\begin{theorem}{解的结构}
$\mat A \bvec x=\bvec y$的通解等于$\mat A \bvec x=\bvec y$的一个特解加上$\mat A \bvec x=0$的各个线性无关的解的线性组合，即$\bvec x = \bvec x_p +k_1\bvec x_{n1}+k_1\bvec x_{n1}$
