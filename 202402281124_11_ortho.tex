...
\end{corollary}
\textbf{证明：}
实数域上易证推论2，下面讨论复数域的情况。由于特征值是特征多项式的根，且一元多项式方程里复根成对出现（若$\lambda_0$为特征值，则$\lambda_0^{*}$也为特征值），所以$A$有偶数个模为$1$的复根，这些特征值连乘为$1$。结合行列式等于特征值连乘\autoref{the_MatEig_3}~\upref{MatEig}，可知剩下奇数个连乘为1的实特征值，得证。
\subsection{对称变换}
