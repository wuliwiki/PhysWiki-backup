...
\end{equation}
如果 $f(x)$ 和 $g(k)$ 分别只在区间 $[-L_x/2, L_x/2]$ 和 $[-L_k/2, L_k/2]$ 内不为零， 积分就可以只在这两个区间内进行． 如果函数不为零的区间 $[a,b]$ 并不关于原点对称， 那么使用平移公式（\autoref{FTExp_eq4}~\upref{FTExp}）即可．
...
要用 DFT 计算傅里叶级数\upref{FSExp} 的系数（\autoref{FSExp_eq2}~\upref{FSExp}）， 只需要把\autoref{DFT_eq3} 前面的系数乘以 $\sqrt{2\pi}/L_x$ 即可， 所以
\begin{equation}
