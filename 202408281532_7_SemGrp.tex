...
类比到非负整数的质因数分解，合成序列就好比是每次只拿走一个质因子（单群）。非负整数的“合成序列”不一定唯一，比如$60$可以写成$2\times 3\times 2\times 5$，但是所有“合成序列”的长度一定是一样的。群的合成序列也类似，合成序列不唯一，但其长度必然是一样的。这就是以下Jordan-Hölder定理在群论中的情形：
