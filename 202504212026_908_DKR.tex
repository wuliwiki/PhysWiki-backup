...
它们的交集是一个新的 C-系统，包含了 \( P \) 中每一个 C-n 元组与 \( Q \) 中每一个 C-n 元组之间所有**非空交集**所构成的 C-n 元组。

\item 检查 C-n 元组是否包含于 D-n 元组,设有 C-n 元组：\(P = [P_1\quad P_2\quad \cdots \quad P_N]\)和 D-n 元组：\(Q = ]Q_1\quad Q_2\quad \cdots \quad Q_N[\)则有：\(
P \subseteq Q \iff \text{存在某个 } i \text{ 使得 } P_i \subseteq Q_i
\]

即：**只要有一个位置上的分量包含于对应的 D 元组分量，即可认为 \( P \subseteq Q \)。**

---

**检查 C-n 元组是否包含于 D-系统**

设有 C-n 元组 \( P \) 和 D-系统 \( Q \)，则有：

\[
P \subseteq Q \iff \text{对于 } Q \text{ 中的每一个 D-n 元组 } Q_i，都有 } P \subseteq Q_i
\]

换言之，**只有当 \( P \) 包含于 D-系统中的每一个 D-n 元组时，才有 \( P \subseteq Q \) 成立。**
\end{enumerate}