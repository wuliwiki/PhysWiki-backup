...
\item import wsl 以后， 会默认用 root 登录， 先试试 xeyes 可不可以（\verb`apt install x11`）。 可以的话 \verb`cp ~/.Xautority /home/用户名/` 然后 \verb`su 用户名` 再试试 xeyes 可不可以。 可以的话就把这两个命令添加到 \verb`/root/.bashrc` 中。
\end{itemize}
