...
设 $y = g(x)$ 有反函数，记其反函数为 $f$，则有：

$$
x = f(y).
$$

存在一个公式可以用 $g$ 的导数表示 $f$ 的导数。为此，注意到 $f$ 与 $g$ 满足：
...
考虑函数 $g(x) = e^x$。它的反函数是 $f(y) = \ln y$。由于 $g'(x) = e^x$，上式给出：
...
例如，考虑 $g(x) = x^3$。它的反函数是 $f(y) = y^{1/3}$，在 0 点不可导。如果我们试图用上式计算 $f$ 在 0 点的导数，需要计算：

$$
\frac{1}{g'(f(0))}.
$$

由于 $f(0) = 0$ 且 $g'(0) = 0$，我们得到 $\tfrac{1}{0}$，这是未定义的。

因此，该公式在此情形下失效。这并不奇怪，因为 $f$ 在 0 点确实不可导。

---

要不要我帮你把这个结论总结成一个 **“反函数可导公式 + 失效条件表”**，方便你快速查阅？
