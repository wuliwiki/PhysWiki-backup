...
注意一维定态薛定谔方程对于同一个能量 $E = k^2/(2m)$ 有两个不同的解， 一个能量对应正负两个 $k$ 的平面波， 他们都被包含在了 的积分中．
...
类似于平面波， 一维散射态也有不同的归一化方式， 但情况要更为复杂． 我们令定态薛定谔方程的两个线性无关解为 $\psi_{E,1}(x), \psi_{E,2}(x)$． 令 $k = \sqrt{2mE} > 0$．

写作参考\href{https://chaoli.club/index.php/4541/last}{这篇帖子}． 假设 $V(x)$ 关于原点对称， $\psi_{k}(x) = \sin(kx + \phi)$， $\phi$ 是 $k$ 的函数． 首先有
\begin{equation}
