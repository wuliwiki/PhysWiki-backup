...
"唯一可解"的意思实际上比定理表述得还要更多. 在定理的表述中, 初值问题 \autoref{PiLin_eq1} 只是局部唯一可解的. 但是, 如果$t_0+T\in I$, 那么还可以将$u(t_0+T)$作为点$t_0+T$处的新初值, 从而将解沿着时间轴继续延拓下去. 按照这个思路, 就可以得到唯一的极大解\upref{MaxSol}.
...
$$
给出, 而且这个序列收敛到真解的速度是指数式的. 这个算法叫做\textbf{逐次迭代法}. 由于极大解的存在, 这个迭代算法实际上并不受$t\in [t_0-T,t_0+T]$的限制, 而可以
