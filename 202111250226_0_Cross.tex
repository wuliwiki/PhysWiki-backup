...
根据\autoref{Cross_eq1} 的定义， 叉乘结果的模长等于平行四边形的面积． 这个平行四边形就是用平行四边形法则\upref{GVecOp}计算 $\bvec A + \bvec B$ 的平行四边形． 通常来说面积是一个标量概念， 但在微积分和大学物理中， 我们可以把它拓展为矢量． \textbf{面积矢量}

\begin{example}{求三角形或平行四边形面积与法向量}\label{Cross_exe1}
