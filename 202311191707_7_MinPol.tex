...
由\autoref{the_MinPol_1} ，任何一个线性变换都有零化多项式。取线性变换$A$的\textbf{零化多项式}$f$和$g$，那么存在$\mathbb{F}$上的多项式$h$和$r$使得
\begin{equation}
f=gh+r~, 
\end{equation}
且$\opn{deg}r<\opn{deg}f$。显然，$f$和$gh$都是$A$的零化多项式，因此$r$也是。

因此，任取$A$的零化多项式$f$和$，如果它不是极小多项式，那么可以取
