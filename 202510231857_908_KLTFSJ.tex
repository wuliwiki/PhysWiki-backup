...
$\text{[M]}\quad \forall, A,B \subseteq X, \quad A \cup c(A) \cup c(c(B)) \subseteq c(A \cup B)$即：$A \cup c(A) \cup c(c(B)) \subseteq c(A \cup B)$

条件\textbf{[K1]}与\textbf{[M]}相互独立。确实，如果$X \neq \varnothing$，定义算子$\mathbf{c}^\star : \wp(X) \to \wp(X)$为常值映射
[
...
由定义可见，任何满足 \textbf{[M]} 的算子都是一个 Moore 闭包算子。
