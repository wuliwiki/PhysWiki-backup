...
物体或物体的一部分在某个位置附近所做的\textbf{往复运动}叫做\textbf{机械振动}，简称\textbf{振动}。
...
x=A\cos(\omega t + \varphi_0),\omega \equiv \sqrt{k/m}~.
\end{equation}
$A$ 为振幅，表示物体偏离平衡距离的最大距离，是未定常数；$\omega$ 为角频率，表示简谐运动物体振动的快慢；称$\omega t+\varphi_0$为相位，知道物体所处的振动相位，就能得到其状态信息如位移，速度等。称初始时刻的相位$ \varphi_0$ 为初相位，表示 $t=0$ 时，简谐运动物体所处的状态。
...
假设物体在恢复力的作用下回到势能零点，结合胡克定律的图像可得弹性势能的表达式，即
\begin{equation}
E_p=\frac{1}{2}kx^2~,
\end{equation}
因此弹性势能的最大值为$E_p=1/2kA^2$，此时物体速度为0，则该势能便是此时的机械能。又因为系统在这个过程中机械能守恒，因此结合\autoref{eq_HSPM09_2} 可得物体的速度-时间表达式为
\begin{equation}
v=-\omega A \sin(\omega t+\varphi_0)~,
\end{equation}
取负号是因为物体在零时刻处于正向位移最大值，此后物体在恢复力的作用下往负方向运动。

我们可以进一步拓展简谐运动的模型，假设物体除了弹性恢复力外，还受到一个恒定的外力如$F_0$，那么物体所受的合外力为
\begin{equation}
F=-kx+F_0=-k(x-x_0)~,
\end{equation}
其中$x_0=F_0/k$。比较典型的一种情况为用弹簧竖直悬挂物体。
\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/bf8f562ae0bff99a.png}
\caption{} \label{fig_HSPM09_7}
\end{figure}
显然，作变量代换$x'=x+x_0$后，我们可以得到新的位移-时间规律，即
\begin{equation}
x'=x-x_0=A'\cos(\omega t+\varphi_0)~.
\end{equation}
该位移零点为$x_0$，也就是说，物体的新平衡位置是合力为0的位置，物体在新平衡位置附近作往复运动。
...
质量为 $m$ 的质点绕点 $O$ 做半径为 $A$、角速度为 $\omega$ 的匀速圆周运动（\autoref{fig_HSPM09_4}），$t=0$ 时质点与点 $O$ 的连线与 $x$ 轴的夹角为 $\varphi_0$。质点相对圆心 $O$ 的位移在 $x$ 轴上的投影为 $x=A\cos(\omega t + \varphi_0)$，可见做匀速圆周运动的质点在$x$轴上的投影的运动是简谐运动。正是因为简谐振动和匀速圆周运动等价，每一个角速度和初相位联合对应唯一一个匀速圆周运动，所以这确实是“简单”，“和谐”的。
...
单摆是一个理想模型，只有在满足小角近似，小球可被视作质点下才是一个简谐振动。回顾这一简化过程可知，模型成立的另一关键在于物体除了受到拉力，还受到另一平面内的恒力。因此，如下图所示，轻绳下端拴着一个可视作质点的小球，上端固定在某倾角的光滑斜面上，这个装置也可以等效为单摆，此时等效重力为$mg\sin\theta$。
\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/d5a000e1b8398a28.png}
\caption{} \label{fig_HSPM09_8}
\end{figure}
