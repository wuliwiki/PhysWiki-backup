...
\caption{$\theta = \pi/2$时的电场强度大小随r关系示意图．z轴高度为电场强度．在一个局域截面上，辐射出的球面波行为类似于平面电磁波\upref{VcPlWv}} \label{DipRad_fig1}
\end{figure}
...
\caption{$s$的“等强度面”.注意平行于偶极子的$\bvec z$方向上没有能流．仿自\cite{GriffE}} \label{DipRad_fig2}
\end{figure}
总辐射功率（时间平均）为
\begin{equation}
