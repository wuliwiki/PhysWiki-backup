...
在某种情况下，方程组\autoref{ODEPr_eq1} 确定变量 $y_i^{(n_i)}$ 为自变量
\begin{equation}
x,y_1,y_1',\cdots,y_1^{(n_1-1)},\cdots,y_n,y_n',\cdots,y_n^{(n_m-1)}
\end{equation}
的单值隐函数．这时，微分方程组\autoref{ODEPr_eq1} 等价于形为 
\begin{equation}
\begin{aligned}
&y_i^{(n_i)}=f_i\qty(x,y_1,y_1',\cdots,y_1^{(n_1-1)},\cdots,y_n,y_n',\cdots,y_n^{(n_m-1)}),\\ i=1,\cdots n
\end{aligned}
\end{equation}
的微分方程组．