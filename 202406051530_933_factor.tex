...
\begin{example}{用阶乘表示从某个自然数m到另一个自然数n的连续整数的乘积}
\begin{align*}
...
\begin{example}{用阶乘表示从自然数n开始依次递减的m个数的乘积}
\begin{align*}
n\times(n-1)\times\cdots \times[n-(m-2)]\times [n-(m-1)]&={n\times(n-1)\times\cdots \times[n-(m-2)]\times [n-(m-1)]1\times\cdots \times(n-m)\times[n-(m-1)]\cdots \times(n-1)\times n\over 1\times\cdots \times(n-m)}\\
&={n!\over (n-m)!}~.
\end{align*}
\end{example}
