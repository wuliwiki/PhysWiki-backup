...
c_i^{(n)}(t) = \frac{1}{\I\hbar} \int_{0}^{t}  \sum_j \mel{\psi_i}{H'(t)}{\psi_j} \E^{\I\omega_{ij}t} c_j^{(n-1)}(t) \dd{t}
\end{equation}
...
\end{equation}
当 $f(t)$ 是一个波包的时候， 令 $t\to+\infty$ 该积分就是 $f(t)$ 的傅里叶变换\upref{FTExp} $\sqrt{2\pi}\tilde f(-\omega_{ij})$．
\begin{equation}
c_i^{(1)}(+\infty) = \frac{\sqrt{2\pi}}{\I\hbar} W_{ij}\tilde f(-\omega_{ij})
\end{equation}
