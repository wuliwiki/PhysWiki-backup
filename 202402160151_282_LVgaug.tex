...
本文使用原子单位制\upref{AU}。 和长度规范中的思路一样， 我们只在使用偶极子近似\upref{DipApr}时讨论\textbf{速度规范（velocity gauge）}。 用角标 $V$ 表示速度规范， 先从规范不变的哈密顿算符（\autoref{eq_QMEM_1}~\upref{QMEM}）出发
...
速度规范在数值计算中有一定的优势，若带电粒子的波包在外电场中加速， 其动量增量和 $-q\bvec A$ 成正比， 会产生 $\exp(-\I q\bvec A\bvec r)$ 相位因子， 而\autoref{eq_LVgaug_6} 的
