...
\end{equation}
与椭圆不同的是，这里不仅分母位置发生了变化，符号也要随之调整，体现出双曲线在形式上的对称性与差异性。

\item 对于 \autoref{eq_Hypb3_3}，当令 $y = 0$ 时，得到 $x$ 轴上的两个交点：
$$
x = \pm \frac{m}{2}~.
$$
而当令 $x = 0$ 时，方程在实数范围内无解，但在复数范围内的解为：
$$
y = \pm \I \sqrt{c^2 - \left( \frac{m}{2} \right)^2}~.
$$
这与椭圆存在显著区别：标准椭圆图像与坐标轴有四个交点，而该双曲线仅与 $x$ 轴相交于两点，与 $y$ 轴无实数交点。
\end{itemize}
...
为了记录方便也与椭圆统一，记符号为正的参数为$a^2$，为负的为$b^2$。注意这与椭圆$a,b$的选取不完全一样，并没有要求二者的大小关系。于是得到总有：
\begin{equation}
...
根据前面所说的实解与虚解，这里也借用称为实轴虚轴。双曲线与坐标轴的交点称为\textbf{顶点}，顶点连线称为\textbf{实轴}，记作$2a$，其中$a$称为\textbf{半实轴}。焦点始终在实轴上。将复数解的系数作为坐标标记在另一根坐标轴上，称为\textbf{虚顶点}，称中间的线段为\textbf{虚轴}，记作$2b$，其中$b$称为\textbf{半虚轴}。

许多学生在学到这里时很困惑，既然是虚的，为什么要在图像中标出来呢？这个虚轴又有什么意义呢？事实上，这是在复变函数中的重要性质，当把定义域改换为复数时，这些虚顶点、虚轴便是真实存在的。但由于高中阶段不涉及，因此此处只是稍稍提及以满足好奇心。
