...
\item 正文只支持非常有限的 LaTeX 命令． 工具栏有大部分常用命令， 所有命令见 “词条示例\upref{Sample}”．
...
\item 独立公式只能用 equation 环境（推荐）， align 环境或者 gather 环境． equation 环境可以通过工具栏的公式图标插入， 也可以在行首打 \lstinline|\beq| 然后按 Tab 键或者回车插入， 如
...
\subsection{自动标签和引用}
工具栏中的 “内部引用” 按钮可以引用同一页面的公式图片等， 如\autoref{editor_eq1}．
