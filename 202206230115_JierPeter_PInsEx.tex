...
\autoref{PInsEx_ex1} 启发我们得到纯不可分扩张的另一等价定义．为了描述这一等价定义，我们先看一个引理：

\begin{lemma}{}
设域$\mathbb{F}$的特征是素数$p$，且$\alpha$是其代数元．

则“存在非负整数$k$使得$\alpha^{p^k}\in\mathbb{F}$”$\iff$“存在$a\in\mathbb{F}$使得$\opn{Irr}(\alpha, \mathbb{F})(x) = x^{p^k}-a$”．
