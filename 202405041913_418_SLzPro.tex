...
接下来我们继续考虑 $\theta_u$ 的问题：$\theta_u$ 应该随 $\lambda$ 增加。所以我们考虑 $\hat Lu = \lambda u$ 的解 $u (\lambda, x)$ ，并沿用 Prufer 变量的记号 $\rho_u (\lambda, x)$，$\theta_u (\lambda, x)$。若 $u(\lambda, x)$ 是 $\hat Lu = \lambda u$ 的解，对于 $\lambda_1 > \lambda_0$，有 $\theta_u (\lambda_1, x) > \theta_u (\lambda_0, x)$ 对于 $x > c$。）由刚才的引理，$\theta_u (\lambda_1, c)\ge \theta_u (\lambda_0,c)$。对于 $x < c$，颠倒不等号方向。