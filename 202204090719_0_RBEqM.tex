...
沿用\autoref{ITensr_sub1}~\upref{ITensr}中的符号， 令体坐标系中惯性张量为 $\mat I_0$， 体坐标系到实验室坐标系的旋转矩阵为 $\mat R$， 那么 $\mat R$ 和 $\bvec \omega$ 完整描述了刚体绕原点转动的\textbf{状态}， $\mat R$ 确定了刚体的朝向， 而 $\bvec \omega$ 确定了刚体上每一点的速度， 这就好比位置和动量可以完整描述了一个质点运动的状态． 若已知刚体在初始时间的运动状态， 且力矩 $\bvec \tau$ 是一个关于时间和刚体运动状态的已知函数 $\bvec \tau(t, \mat R, \bvec\omega)$， 那么求解以下运动方程可以得到刚体接下来的运动
...
事实上旋转矩阵 $\mat R$ 的 9 个矩阵元中只有三个自由度\upref{DoF}， 如果我们能用三个变量表示 $\mat R$， 就可以得到只含 6 个未知数的 6 个常微分方程． 一种方法是使用欧拉角（链接未完成）． 另一种折衷的方法是用 4 元数， 即用 4 个变量表示 $\mat R$， 可以得到相对简单的方程， 详见 “刚体运动方程（四元数）\upref{RBEMQt}”．
