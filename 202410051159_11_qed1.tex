...
实际上，洛伦兹群的四个分支里只有$SO^+(1,3)$是子群。我们来简单证明一下。首先，非正规的洛伦兹分支总是违背封闭性的，因为两个行列式为$-1$的相乘结果是行列式为$1$的矩阵。其次，由于非正时的洛伦兹变换会改变类时矢量的时间分量符号，所以被两个非正时的洛伦兹变换作用后，类时矢量的时间分量符号会恢复。所以非正时的洛伦兹矩阵相乘结果必为正时矩阵。相较之下，由于子群相交结果是子群，作为正规与正时群的交集，$SO^+(1,3)$必为$O(1,3)$的子群，实际上还是一个李子群。因此我们可以用指数映射表示该李子群的群元。即设$X\in \opn{Lie} SO^$
