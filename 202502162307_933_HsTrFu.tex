...
从图中可以清楚地看出：
\begin{itemize}
\item $\sin\alpha$ 和 $\cos\alpha$ 对应于单位圆内的直角三角形中的直角边，分别表示弦的一半，即以原点为圆心、半径为 1 的单位圆上点的 $y$ 坐标和 $x$ 坐标。
...
\item $\sec\alpha$ 和 $\csc\alpha$ 分别对应于从 $T,C$ 作通过圆心的割线所得到的线段 $\overline{OT}$ 和 $\overline{OC}$，即切线、坐标轴和角所在直线截取出的三角形的斜边。
\end{itemize}
...
这样，每个正角的弦、切、割都涉及两个三角形：一个是单位圆内的三角形$\triangle OPP_x$；另一个是包含圆外一点的三角形$\triangle OTX_0$。对于余角，对应关系类似，只是对应的三角形是以余角展开的三角形。
