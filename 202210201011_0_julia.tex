...
\item \verb|1:3:9| 等效于 \verb|StepRange(1, 3, 9)|， 类型是 \verb|StepRange{Int64, Int64}| 注意 \verb|StepRange| 类型的参数只能是整数．
\item \verb|1.2:5.1| （等效于 \verb|range(1.2,5.1)|）和 \verb|1:0.2:4.1| （等效于 \verb|1:0.2:4| 和 \verb|range(1,4,16)|）的类型是 \verb|StepRangeLen{Float64, Base.TwicePrecision{Float64}, Base.TwicePrecision{Float64}, Int64}|
...
\item 都可以像数组一样用 \verb|size(x, dim)| 来获取尺寸， \verb|length()| 获取元素个数．
\end{itemize}
...
\item 要指定 \verb|using| 的内容， 用 \verb|using 模块名: 函数1, 函数2, 变量3|．
