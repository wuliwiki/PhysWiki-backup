...
综合上面所有的分析，如果 $n_0 \leq 1$，则数列和的最小值是 $S_1$。此时可以推导出 $a_1 \geq 0$，即当 $a_1$ 非负时，数列和的最小值在 $n=1$ 处取得。反之，当 $a_1 < 0$ 且 $d > 0$ 时，存在 $a_1 < a_2 < \dots < a_k \leq 0$，并且可以验证$a_{k+1} =0$且$a_{k+m}>0(m\in\mathbb{N}^+)$，或$a_{k+m}>0(m\in\mathbb{N})$。从而，可以直接声称$S_n$的最小値$S_k=S_{k+1}=S$或$S_k=S$
