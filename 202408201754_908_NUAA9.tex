...
将(2)式化入(1)式得:$i\\hbar \\sum_n \\frac{d c_n(t)}{dt} Ψ_n(x) e^{-\\frac{i}{\\hbar} E_n t} = \\sum_n H' Ψ_n c_n e^{-\\frac{i}{\\hbar} E_n t}\\]
以 $Ψ_n^*(x)$ 乘方程两侧，并对 $x$ 从 $-\\infty$ 到 $\\infty$ 积分，利用正交归一性：
\\[\\int Ψ_n^*(x) Ψ_m(x) \, dx = \delta_{mn}$