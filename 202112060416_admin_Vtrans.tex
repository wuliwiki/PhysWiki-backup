...
\subsection{证明}
现在用旋转变换矩阵\upref{Rot3D}来证明\autoref{Vtrans_eq2}， 下面的黑体字母如无声明一律表示 $S$ 系中的坐标， 即 $(...)_S$． 令 $S'$ 中坐标到 $S$ 坐标的旋转变换矩阵为\footnote{即对任意矢量 $\bvec A$， $\mat R (\bvec A)_{S'} = (\bvec A)_S$．} $\mat R$， 则
\begin{equation}
