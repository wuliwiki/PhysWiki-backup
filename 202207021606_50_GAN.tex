...
生成器在数据x上的分布记为$p_g$，输入生成器的随机变量，即噪音，记为$z$，其分布为$p(z)$.生成器是一个从潜在空间（latent space），即输入的随机噪音，到数据空间（data space）的映射，记为$G(z;\theta_g)$.生成器本质上是由多层神经网络表示的带有参数$\theta_g$的可微分函数．参数$\theta_g$的值是模型训练时学习而确定的．
