...
Git for Windows 的 Git bash 是不支持 symlink 的， symlink clone 下来以后都会变成文本文件（文本就是 symlink 指向的路径）， 所以还是建议在 WSL\upref{WSLnt} 中安装 Git 使用。 但就算使用 wsl， checkout 出的 symlink 也可能只是一个文本文件，
