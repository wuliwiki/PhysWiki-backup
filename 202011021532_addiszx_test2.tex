...
\footnote{参考 Wikipedia \href{https://en.wikipedia.org/wiki/Hartree_atomic_units}{相关条目}．}在量子力学的许多理论或数值计算中，选用\textbf{原子单位（Hartree atomic unit）}会更方便． 需要特别注意的是， 原子单位下的物理公式在使用时的习惯和其他单位制有所不同， 例如会出现 $E = \omega$ （能量等于角频率， 见）这种看似不符合量纲分析的公式． 要理解这种记号， 这里推荐的方法是把使用原子单位的公式都理解成无单位的物理公式\upref{NoUnit}．
...
代入， 各项同除 $\beta_E\beta_\Psi$， 得\footnote{根据偏微分的定义， 常数可以移到偏微分算符外， 如 $\pdv*[2]{(\beta_x x_a)} = (1/\beta_x^2) \pdv*[2]{x_a}$}
\begin{equation}
...
我们再看波函数的归一化公式
\begin{equation}
1 = \int \abs{\Psi}^2 \dd{x} = \beta_\Psi^2 \beta_x \int \abs{\Psi_a}^2 \dd{x_a} = \beta_\Psi^2 \beta_x \int \abs{\Psi_a}^2 \dd{x_a}
\end{equation}
\begin{equation}
1 = \int \abs{\Psi}^2 \dd{x} 
\end{equation}
为了使归一化公式的形式不变， 必须令
\begin{equation}
\beta_\Psi = \beta_x^{-1/2} = \beta_x^{-1/2}
\end{equation}
同理， 对 $N$ 维波函数有 $\beta_\Psi = \beta_x^{-N/2}$．
由 可知我们只剩下两个自由的常数， 例如只要确定 $\beta_x$ 和 $\beta_m$， 剩下的 $\beta$ 也就确定了．
...
\subsection{原子单位}
最常见的情况下， 原子单位定义 $\beta_m$ 等于电子的质量， $\beta_x$ 等于玻尔半径， 再由确定 $\beta_E, \beta_t, \beta_\Psi$， 如所示\footnote{为了区别能量与电场，以下用 $E$ 表示能量，用 $\mathcal{E}$ 表示电场．} ． 注意许多常数都与氢原子的玻尔模型\upref{BohrMd}（原子核不动）的基态（表中简称基态）有关．

\begin{table}\label{test2_tab1}[ht]
\caption{原子单位转换常数表}
...
U &= \frac{q}{r} = \mathcal{E} x \qquad \text{（点电荷电势）} = q\mathcal{E} x = q\mathcal{E} x\\
\mathcal{E} &= \frac{q}{r^2} \qquad \text{（点电荷电场）}\\
V &= qU = q\mathcal{E} x \qquad \text{（匀强电场电势能）} 
\end{align}
薛定谔方程为
\begin{equation}
-\frac{1}{2m} \pdv[2]{\Psi}{x} + V\Psi= \I\pdv{\Psi}{t} = \I\pdv{\Psi}{t}= \I\pdv{\Psi}{t}= \I\pdv{\Psi}{t}
\end{equation}
...
令 中 $m = 1$， $q = 1$， 再将 代入， 得
\begin{equation}
\mathcal{E} x-\frac12 \pdv[2]{\Psi}{x} + \mathcal{E} x \Psi= \I\pdv{\Psi}{t}
\end{equation}
\end{example}

\begin{exercise}{氢原子的基态能量}
计算玻尔模型中氢原子基态的能量\autoref{test3_eq1}~\upref{test3}（答案：$-1/2$）．
\end{exercise}
...
当问题涉及一主要角频率 $\omega$ 的时候，可选择 $\beta_E = \hbar\omega$ 做能量单位． 同样令 $\beta_m$ 等于电子质量， $\beta_q$ 等于元电荷， 由 得
\begin{equation}
...
现在我们按照“无单位的物理公式\upref{NoUnit}” 中介绍的方法将 转换为含单位的公式． 即先把所有无单位的物理量替换成有单位的物理量除以对应的 $\beta$ 常数， 得（两边已同乘 $\beta_\Psi$）
\begin{equation}
-\beta_x^2\frac12 \pdv[2]{\Psi}{x} + \frac{1}{\beta_x^2}\frac12 x^2 \Psi= \beta_t\I\pdv{\Psi}{t}
\end{equation}
将 代入， 两边乘以 $\omega\hbar$ 得
\begin{equation}
