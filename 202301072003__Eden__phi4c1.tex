...
\end{aligned}
\end{equation}
需要注意的是，在 $\overline{MS}$ 方案中，两点编时格林函数在物理质量处的留数不为 $1$，而是 $Z=\sqrt{Z^{OS}_\phi/Z^{\overline{MS}}_\phi}$，这可以从 $\phi^{\overline{MS}}$ 与裸场之间的关系看出：
\begin{equation}
\phi_0 = \sqrt{Z_\phi^{\overline{MS}}} \phi^{\overline{MS}} = \sqrt{Z_\phi^{OS}} \phi^{OS},\quad \Rightarrow \quad \phi^{\overline{MS}} = Z \phi^{OS}
\end{equation}
