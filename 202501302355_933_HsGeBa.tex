...
\textbf{三角形的相似}（Similar）指两个三角形的对应角相等，对应边成比例。相似三角形的判定法：
\begin{itemize}
\item \textbf{AA（角-角）}：两个对应角相等；
\item \textbf{SAS（边-角-边）}：两边成比例，夹角相等；
\item \textbf{SSS（边-边-边）}：三边成比例。
\end{itemize}

三角形的全等是相似的特殊情况，对应比例系数为 $1$。
