...
\textbf{堆（heap）}又称\textbf{二叉堆}是一个以一颗完全二叉树的结构存储值的数据结构．
...
因为是随机插入和删除，所以在堆中删除或者插入某个值的话必定要进行调整．分为 $up$ 操作和 $down$ 操作，$up$ 是如果一个数不符合堆的性质（太小了或太大了）就要往上调整，$down$ 也同理，因为某个数不符合堆的性质，就要往下调整．
