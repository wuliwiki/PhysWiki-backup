...
\begin{equation}\ali{
\int \frac{1}{\sqrt{2(k^2/2+Z/r)}} \dd{r} &= \frac{r}{k}\alpha-\frac{2 Z}{k^3}\tanh ^{-1}\qty(\alpha) + C\\
&=F(r)+ C'~.
}\end{equation}
其中
\begin{equation}\label{eq_HeAna2_7}
F(r) = \frac{r}{k}\alpha-\frac{Z}{k^3}\ln(\frac{\alpha+1}{\alpha-1})~,
\qquad \alpha = \sqrt{\frac{2 Z}{k^2 r}+1}~.
\end{equation}
$C' = C + \I \pi/2$。 这是因为（\autoref{eq_atanh}~\upref{TrigH}）
\begin{equation}
\tanh ^{-1}\qty(x)
