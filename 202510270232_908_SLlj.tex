...
\subsubsection{数学基础（Foundations of Mathematics）}
...
而更严格的建构主义版本只处理：自然数；数论函数；自然数集（可用以表示实数，从而研究分析）。

其核心思想是：  
只有当一个函数的取值可通过具体计算过程得到时，才能断言该函数存在。

\paragraph{直觉主义与真理的哲学}
20 世纪初，\textbf{布劳威尔}（L. E. J. Brouwer）创立了\textbf{直觉主义}（intuitionism），  
作为数学哲学的一个分支。  
起初该哲学并未被充分理解。  
它主张：一个数学命题若要对数学家而言为真，  
该数学家必须能\textbf{直观地把握其真理性}——  
不仅相信它是真的，还要理解其为何为真。  
由此，布劳威尔拒绝了\textbf{排中律}（law of excluded middle），  
因为在他看来，某些命题既不能断言为真，也不能断言其否定为真。  

布劳威尔的思想引发了极大的影响与激烈争论。  
后来，\textbf{克莱尼}（Stephen Kleene）与\textbf{克赖瑟尔}（Georg Kreisel）  
研究了直觉逻辑的形式化版本（布劳威尔本人拒绝形式化，  
并以自然语言表达其思想）。  
随着\textbf{BHK 诠释}（Brouwer–Heyting–Kolmogorov interpretation）  
与\textbf{克里普克模型}（Kripke models）的出现，  
直觉主义逐渐与经典数学建立了更和谐的联系。
```
