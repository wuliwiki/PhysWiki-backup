...
拆分后的每个算符（矩阵）演化的算法可以一样或不一样。 事实上，注意到 $\bvec E\vdot \bvec r$ 不含微分，我们可以不计算它矩阵而是一步到位直接计算 $\exp(\I q\bvec E\vdot\bvec r\dd t)$ 矩阵！ 由于改算符只混合每个位置 $\bvec r$ 处不同的分波， 所以只需要计算 $\mel{Y_{l,m}}{\exp(\I q\bvec E\vdot\bvec r\dd t)}{Y_{l',m'}}$ 即可。 但话又说回来，做 $N_l^2/2$ 个球面积分，真的会比例如 Crank-Nicolson 要快吗？我看不见得。
