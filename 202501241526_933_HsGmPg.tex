...
如果数列 $\{a_n\}$ 所有项都为$0$的情况，如果按照当前的递推公式来判断，也可以看作是等比数列，但事实上，他却因为存在分母为0的情况，使得并不能符合等比数列的定义。并它是符合倍数是固定值的。另外，因为存在分母不能为0的情况，所以除非等比数列 $\{a_n\}$ 所有项都为$0$，等比数列中不会出现某一些的值为0，某一些不为0的情况。特别地，非0的常数列都是 $q = 1$ 的等比数列。
