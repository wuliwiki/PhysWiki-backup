...
\end{equation}
由于机械波和电磁波的能量密度（光强）都正比于振幅平方，所以我们经常会讨论分布函数 $\abs{f(x)}^2$ 的性质。

$\abs{f(x)}^2$ 的方差为（对比\autoref{eq_GausPD_1}）
\begin{equation}
\sigma^2= \frac{1}{4a}~.
\end{equation}

FWHMI （光强半高宽）为 $f(x)^2$ （\autoref{eq_GausPD_8}）
\begin{equation}\label{eq_GausPk_2}
\mathrm{FWHMI} = \sqrt{\frac{2\ln 2}{a}} = 2\sqrt{2\ln 2}\ \sigma \approx 2.35482\sigma~.
\end{equation}
满足 $\abs{f(\text{FWHMI/2})}^2 = \abs{f(0)}^2/2$。
