...
Pervin（1964）进一步给出了库拉托夫斯基外点算子（exterior operator）和边界算子（boundary operator）的类似公理化形式。[3][10]这些算子同样可以通过以下关系定义出对应的闭包算子：$\mathbf{c} := \mathbf{n e},\mathbf{c}(A) := A \cup \mathbf{b}(A)$.
