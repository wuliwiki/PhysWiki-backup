...
\end{equation}
证明见下文． 由 $Y_{l,m}^* = (-1)^m Y_{l,-m}$ 易证这里的复共轭可以加在任意一个球谐函数上． 另外由 $Y_{l,m}(-\uvec k) = (-1)^l Y_{l,m}(\uvec k)$ 易得 $\ket{\bvec k}\Cj = \ket{-\bvec k}$．
...
若三维函数具有球谐展开的形式
\begin{equation}
f(\bvec r) = \frac{1}{r}\sum_{l,m} u_{lm}(r) Y_{lm}(\uvec r)
\end{equation}
要做三维傅里叶变换
\addTODO{多维傅里叶变换， 链接}
\begin{equation}
