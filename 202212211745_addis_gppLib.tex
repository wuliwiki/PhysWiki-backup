...
\item 动态链接库文件内部的 \verb|DT_SONAME| entry 会保存本来的文件名给运行时的 linker 确认。 用 \verb|patchelf --print-soname 文件| 可以显示。 所以把 so 文件重命名以后， 还要用 \verb|patchelf --set-soname 新文件名 新文件名|。
