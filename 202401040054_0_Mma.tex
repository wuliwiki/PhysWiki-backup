...
\item 求数值用 //N 或者 N[expr,digit]
\item 如果 expression 和 literal constant 运算, 是非常精确的.
\item for 循环是 do[expr,{x,x1,x2}] (expr 可以是许多用分号隔开的语句)
\item 如果函数不支持 List 输入, Map[Function, {arg1,arg2...}] 可以把一个函数依次作用在一个 List 上面.
\item var1=var2=var3=val 可以给 var1,var2,var3 同时赋值, 用 . 给变量赋值可以清除变量的值, 相当于 Clear[var1,var2,var3]
\item 如果 x2 = [x1 的表达式], x3=[x2 的表达式], 那么如果输入 x3 会返回 x1 的表达式. 但是 [x2 的表达式] 的信息还在, 因为如果这是把 x2 清空, 输入 x3 又会出现 x2 的表达式.
\item 变量名建议用小写开头
\item 两个变量用空格隔开代表相乘
\item expr_of_x/.x->3 可用于临时赋值表达式中的变量
\end{itemize}
