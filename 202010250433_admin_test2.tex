...
\begin{definition}{群}\label{test2_def1}
...
实际上，我们可以用更为弱化的公理系统来定义群，比如第 4 条只要求存在左逆元， 即只要求$\forall x\in G, \exists y\in G, y\cdot x=e$． 在这种情况下我们仍然可以证明左逆元都是右逆元（\autoref{test2_the1}）．有很多不同的弱化版本公理系统也能等价地定义出群来，但是为了方便理解，我们用了以上对称的公理系统．

\autoref{test1_eq1}~\upref{test1}集合的元素数量被称为集合的\textbf{基数}或\textbf{势}\upref{Set}，而群$G$的元素数量也可以称为群的\textbf{阶（order）}，记作$|G|$． 以后如果没有特别说明，默认将群元素$x$的逆元记为$x^{-1}$，将群的单位元记为$e$．\autoref{test2_eq1} 
...
\end{equation}
\begin{exercise}{二元群}\label{test2_exe1}
...
\item 请根据\autoref{test2_def1} 验证这个二元集合配上运算 $\cdot$ 构成一个群
\item 尝试改变\autoref{test2_eq1} 的规则使其同样构成一个群
\end{itemize}
\end{exercise}
...
\begin{example}{整数加法群}\label{test2_ex1}
所有整数的集合 $\mathbb Z$， 配合通常的整数加法运算构成一个群．
\end{example}

\begin{example}{$n$ 元循环群}\label{test2_ex2}
...
\begin{example}{$n$ 元置换群}\label{test2_ex3}
...
\begin{example}{对称群}\label{test2_ex4}
...
\begin{example}{$n$阶可逆方阵群}\label{test2_ex5}
...
\begin{theorem}{群运算的唯一性}\label{test2_the2}
...
从\autoref{test2_the2} 可知左右单位元分别是唯一的． 请证明左单位 $e_1$ 元等于右单位元 $e_2$ （提示： 将它们相乘）．
\end{exercise}

\begin{theorem}{逆元的唯一性}\label{test2_the1}
