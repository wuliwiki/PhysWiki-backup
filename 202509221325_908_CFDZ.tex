...
\end{aligned}~
$$
\subsubsection{利用倒数法则或链式法则的证明}
设
$$
h(x) = \frac{f(x)}{g(x)} = f(x) \cdot \frac{1}{g(x)}.~
$$
那么根据乘积法则：
$$
h'(x) = f'(x)\cdot \frac{1}{g(x)} + f(x)\cdot \frac{d}{dx}\left[\frac{1}{g(x)}\right].~
$$
为了计算第二项中的导数，可以应用倒数法则，或者结合幂函数求导法则与链式法则：
$$
...
\end{aligned}~
$$
\subsubsection{利用对数微分法的证明}
设$h(x) = \frac{f(x)}{g(x)}$.对等式两边取绝对值并取自然对数：
$$
...
$$
解出 $h'(x)$，并将 $h(x)$ 代回为 $\tfrac{f(x)}{g(x)}$，可得：
$$
\begin{aligned}
...
\end{aligned}~
$$
在对数微分中引入函数的绝对值是必要的，因为当函数可能取负值时，$\ln u$ 仅在 $u>0$ 时有实数值。这是合理的，因为：$\frac{d}{dx}(\ln |u|) = \frac{u'}{u}$,这说明在对数微分中取函数的绝对值是正确且合理的做法。
\subsection{高阶导数}
隐函数求导可以用来计算商函数的第 $n$ 阶导数（部分结果会用到其前 $n-1$ 阶导数）。例如，对$f = g h$进行两次求导，得到：$f'' = g''h + 2g'h' + gh''$
然后解出 $h''$，可得：
$$
