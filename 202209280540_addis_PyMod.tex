...
\subsection{模块目录}
\begin{itemize}
\item \verb|__init__.py| （通常为空）用于把所在目录标记为模块目录． 例如创建 \verb|mydir/__init__.py| 以及 \verb|mydir/mymodule.py| 后， 如果 \verb|mydir| 在搜索路径下， 那么就可以 \verb|import mydir.mymodule| 或者 \verb|from mydir import mymodule|．
\end{itemize}
