...
此外，如前文在讨论任意角和弧度制时所述，通常对三角函数的研究也放在单位圆中，这时，由于单位圆的半径恒定为 $r=1$，三角函数的定义解除了对$r$的依赖（而正切 $\tan\alpha$ 的表达式本身就不依赖于 $r$）因为三角函数本身就就只和角有关，而与斜边长度无关。这种推广方式不仅使三角函数的定义更为普遍，也为后续的函数分析和计算提供了更清晰的数学框架。
