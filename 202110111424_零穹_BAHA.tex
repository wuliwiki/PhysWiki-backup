...
&=\sum_{m=0}^{n}(-1)^{n-m}C_n^m{a_i}^m{a_j}^{n-m}\bra{a_i}B\ket{a_j}\ketbra{a_i}{a_j}\\
&=\sum_{m=0}^{n}(-1)^{n-m}C_n^m{a_i}^m{a_j}^{n-m}B^i_{\; j}\ketbra{a_i}{a_j}
\end{aligned}
\end{equation}
由此，有
\begin{equation}\label{BAHA_eq7}
\begin{aligned}
A^{(i)}B&=\sum_{m,n,l}\ketbra{a_m}{a_m}A^{(i)}\ketbra{a_l}{a_l}B\ketbra{a_n}{a_n}\\
&=\sum_{m,n}a_m^i\bra{a_m}B\ket{a_n}\ketbra{a_m}{a_n}\\
&=a_m^iB^m_{\enspace n}\ketbra{a_m}{a_n}\\
