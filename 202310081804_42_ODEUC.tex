...
\dot x=v(x,t)~
\end{equation}
中向量场 $v$ 在扩张相空间（\autoref{def_PSaPF_2}~\upref{PSaPF}）中的一区域 $U$ 上有直到 $r$ 阶连续的导数 $v\in C^r$，则对任一点 $(t_0,x_0)\in U$，存在相空间中点 $x_0$ 的一邻域 $M$，使得任一给定的 $y\in M$，都存在一个 $t_0$ 的邻域区间，使得唯一存在满足初始条件 $\varphi(t_0)=x$ 的微分方程
