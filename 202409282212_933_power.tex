...
因此，当$a_1>a_2$时，有$a_1-a_2>0$。此时，对$x>1$，有$x^{a_1-a_2}>1$，即$\displaystyle \frac{x^{a_1}}{x^{a_2}}>1$，由于在第一象限$x^{a_2}>0$，从而有$x^{a_1}>x^{a_2}$；同理，对$0<x<1$，$x^{a_1}<x^{a_2}$。也就是说，在第一象限内，如果在$x=1$的右侧从下到上画一条垂线，则会先穿过a值较小的函数图象，后穿过a值较大的图象，在$x=1$的左侧从下到上画一条垂线，则会先穿过a值较大的函数图象，后穿过a值较小的图象。

单调性：
a>0递增，a<0递减。

根据前面的分析，如果将第一象限以$x=1,y=x,y=1$三条线分成六个区域。这三条线的交点是$(1,1)$，由于不论$a$取何值，$1^a=1$，因而幂函数一定会过点$(1,1)$。如果将$x=1,y=x$之间大于$1$的部分记作区域$\rm{I}$，则可顺时针得到$\rm{I, II, III, IV, V, VI}$一共六个区域：
\begin{itemize}
\item $a>1$时，幂函数通过区域$\rm{I,IV}$
\item $0<a<1$时，幂函数通过区域$\rm{II,V}$
\item $a<0$时，幂函数通过区域$\rm{III,VI}$
\end{itemize}
并且$a$越大，越会靠近直线$x=1$。\autoref{fig_power_1} 展示了幂函数 $f(x) = x^a$ 第一象限的函数图象。
