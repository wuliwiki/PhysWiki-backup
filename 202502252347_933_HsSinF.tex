...
这个结果表明，$\sin x$ 和 $\cos x$ 的导数具有周期性的循环结构。针对sinx而言，每求导一次，相位都会提前 $\frac{\pi}{2}$ \footnote{这也与之前说cosx和sinx相差$\frac{\pi}{2}$个相位相联系，这在\autoref{eq_HsSinF_2} 中也有体现，cosx总是比sinx落后一个，希望能带来一些关于相和相位的新理解。}，而因为sinx的周期为2pi，因此这样的变化每进行两次就反相，进行四次就同相，也就回到了初始状态。这种求导的循环性质具有很强的对称性，并在许多数学和物理问题中发挥作用。下面对此进行一些探讨。
