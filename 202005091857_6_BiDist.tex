...
二点分布b(l ,p) 主要用来描述一次伯努利试验中成功（记为A) 的次数(0
或1) .
很多随机现象的样本空间0 常可一分为二，记为A 与A, 由此形成伯努利试
验.n 重伯努利试验是由n 个相同的、独立进行的伯努利试验组成，若将第i 个伯
努利试验中A 出现的次数记为x, (i= 1,2, …, n), 则x, 相互独立，且服从相同
的二点分布b(l ,p). 此时其和
X = X1 + X2 +… +X 几
就是n 重伯努利试验中A 出现的总次数，它服从二项分布b(n,p). 这就是二项
分布b (n ,p) 与二点分布b(l,p) 之间的联系，即服从二项分布的随机变量是n
个独立同为二点分布的随机变量之和．