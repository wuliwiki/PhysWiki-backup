...
\item \verb`sudo sysctl vm.swappiness=90` adjusts swapiness immediately, but will be cleared after reboot
\item to adjust swapiness after every reboot, use \verb`sudo vim /etc/sysctl.conf`, add \verb`vm.swappiness=20` to the end of the file. To apply the config file imediately, use \verb`sudo sysctl -p`
