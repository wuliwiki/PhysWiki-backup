...
而 $(\tan x)' = \sec^2 x$，由于 $\sec x$ 在其定义域内无零点，故 $\sec^2 x > 0$ 恒成立。这表明 $\tan x$ 在每个连续区间上严格递增。然而，需要注意的是，导数恒为正只能保证函数在各个连续区间内递增，但因为 $\tan x$ 存在间断点，所以不能直接推出在整个定义域内任取两个点都满足递增关系。

以下是其余三个三角函数的导数及其单调性：

\begin{itemize}
\item $(\cot x)' = -\csc^2 x$。$\cot x$ 在其定义域的连续区间上递减。
\item $(\cot x)' = -\csc^2 x$，$\cot x$ 在 $(k\pi, (k+1)\pi), k \in \mathbb{Z}$ 上递减。
\item $(\sec x)' = \sec x \tan x$，$\sec x$ 在 $\left(-\pi + 2k\pi, -\frac{\pi}{2} +2k\pi\right), k \in \mathbb{Z}$ 和$\left(-\frac{\pi}{2} + 2k\pi, 2k\pi\right), k \in \mathbb{Z}$ 上递减，在 $\left(\pi + 2k\pi, \frac{\pi}{2} +2k\pi\right), k \in \mathbb{Z}$ 和$\left(-\frac{\pi}{2} + 2k\pi, 2k\pi\right), k \in \mathbb{Z}$ 上递减，上递减。
\item $(\csc x)' = -\csc x \cot x$，$\csc x$ 在 $\left(2k\pi, (2k+1)\pi\right), k \in \mathbb{Z}$ 上递减，在 $\left((2k+1)\pi, (2k+2)\pi\right), k \in \mathbb{Z}$ 上递增。
\end{itemize}
