...
=&\int\dd \tau\dv{q^0(\tau)}{\tau}\delta^{(4)}(x-q(\tau)).\\
\end{aligned}~
\end{equation}
注意上式使用了 $q^0(\tau)=\tau,\vec q(\tau)=0,\delta^{(4)}\equiv\delta(x^0)\delta^{(3)}(\vec x)$。因为 $\int\dd x^4\delta^{(4)}(x)=1$，$\dd{^4}x$ 是Lorentz标量，因此 $\delta^{(4)}(x)$ 是Lorentz标量。
