...
\subsection{简介}
\begin{itemize}
\item 编辑器可从网站首页的链接进入（必须是登录状态）． 如果不想注册， 可以使用测试网址 \href{http://wuli.wiki/editor}{wuli.wiki/editor}， 测试账号为 \lstinline|test1| 到 \lstinline|test100|， 密码 6 个 8． 注意测试文件会被定期删除， 可以自行使用下载按钮备份
\item 目前该编辑器仅用于编辑小时物理百科的词条， 我们正在测试个人笔记功能（见\href{http://wuli.wiki/editor?target=home}{测试页面}）
...
\item 一个简单的 LaTeX 介绍见 “LaTeX 简介\upref{latxIn}”
