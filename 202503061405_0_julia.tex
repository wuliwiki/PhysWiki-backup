...
\item 如果存在启动脚本 \verb`~/.julia/config/startup.jl`， \verb`julia <file>` 会先运行启动脚本再运行 \verb`<file>`。 要跳过启动脚本，用 \verb`julia --startup-file=no <file>`（只能是 \verb`[yes|no]`）。 如果存在环境变量 \verb`JULIA_DEPOT_PATH`， 默认启动脚本路径就是 \verb`JULIA_DEPOT_PATH/config/startup.jl`（参考 \verb`julia --help`）
\end{itemize}
