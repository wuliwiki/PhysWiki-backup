...
给定域 $\mathbb F$ 上的线性空间 $V$，设 $W$ 是其子空间，则 $W$必有补空间（\autoref{def_DirSum_1}）。
\end{theorem}
...
\textbf{满射性：}设 $v+U\in V/U$。由于 $V=U\oplus W$，所以 $v=x+y,x\in U,y\in W$。于是
\begin{equation}
v+U=x+y+U=(x+U)+(y+U)=U+(y+U)=y+U=f(y).~
\end{equation}
上面第三个等式用到 $x\in U\Rightarrow x+U=U$。

\textbf{Ker f=0：}由于 $\ker f:=\{x|f(x)=0,x\in W\}$，其中 $0$ 代表 $V/U$ 的零元，由\autoref{the_QuoSpa_2} 中的证明可知这个零元就是 $[0]=0+U=U$。因此若 $u\in\ker f$，则 $f(u+U)=u+U=U$。但是 $u\in W,U\cap W=0$，所以只能是 $u=0$。
