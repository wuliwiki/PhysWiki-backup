...
\subsection{额外相位}
如果给 $j_l$ 加上额外相位 $j_l(x+\delta_l)$ 如何拼成 $\psi_{\bvec k}^{(\pm)}$ 呢？ $+$ 代表平面波入射球面波出射， $-$ 反之．

答案就是
\begin{equation}
\ket{\psi_\bvec k^{(\pm)}} = \frac{1}{(2\pi)^{3/2}}\E^{\I \bvec k \vdot \bvec r} + f(\uvec k)\E^{\I kr} = \sqrt{\frac{2}{\pi}} \sum_{l=0}^{\infty} \I^l  j_l(kr) \sum_{m=-l}^l Y_{l,m}^*(\uvec k) Y_{l,m}(\uvec r)
\end{equation}
