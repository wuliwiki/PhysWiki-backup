...
\item 据说如果把 \verb|sync| 变为 \verb|async|， 可以用 ram 作为文件缓存。 但是一但服务器断电缓存中的文件将丢失。
...
\item 命令行： \verb|sudo apt install cifs-utils -y|
...
\item 临时 mount： 用 \verb|sudo mount -t cifs -o rw,vers=3.0,credentials=/root/.SMBcredentials //ip地址/共享名称 /mount文件夹|
\item 要开机自动 mount， 在 \verb|/etc/fstab| 加入一行： \verb|//ip地址/共享名称 /mount文件夹 cifs vers=3.0,credentials=/.SMBcredentials
|
