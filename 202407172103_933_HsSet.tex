...
由于数字在数学领域有特别的地位，也非常常用，于是偷懒地数的集合简称\textbf{数集}（number set），一般对这些特殊的集合采用一些特定的记号\footnote{在高等数学领域，这些数集的记号为$\mathbb{N,N^+,Z,Q,R}$，既表示他们的地位特殊，同时这些集合的定义都是广泛明确的，使用这个记号会方便交流。}：
\begin{itemize}
