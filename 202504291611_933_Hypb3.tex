...
可以联想到，双曲线的两条渐近线，其实正对应着正切函数或正割函数中的渐近线。另外，如果想要表述左支或下支，对于使用双曲函数表示的参数方程，需要将 $a$ 取为 $-a$；而对于使用三角函数表示的参数方程，则需要将 $\theta$ 的取值范围改为 $\displaystyle\left( \frac{\pi}{2}, \frac{3\pi}{2} \right)$。
...
同样地，连接双曲线上任意两点的线段称为弦，但由于双曲线有两支的特性，双曲线的弦可能在双曲线内部，也可能在双曲线外部。但是，由于双曲线是一个二次方程，它与直线方程联立后只能最多的到两个实数解，因此，弦所在的直线与双曲线最多只可能有两个交点，不存在弦跨过双曲线的情况。

通过一个焦点且垂直于实轴的弦称为\textbf{通径（Latus Rectum）}。通径的长度反映了双曲线在焦点位置的“张开程度”。


以右焦点 $F(c,0)$ 为例，通径所在的直线为 $x=c$，代入双曲线方程得：
\begin{equation}
...
通径长度与双曲线的参数 $a$ 和 $b$ 有关，$b$越大或$a$越小，通径越长，双曲线在焦点附近张开的角度越大。

\begin{definition}{焦点到双曲线的最短距离}
在标准双曲线

中，焦点 $F(c,0)$ 到双曲线的最短距离，指的是从焦点到对应支的最接近点的距离，即焦点到顶点的距离减去原点到顶点的距离。
\end{definition}

由于顶点坐标为 $(a,0)$，焦点坐标为 $(c,0)$，两者共线，因此焦点到顶点的距离为
\begin{equation}
c - a~,
\end{equation}
其中
...
焦点到双曲线的最短距离反映了焦点在双曲线张开结构中的位置关系。$a$越小，或$b$越大，焦点距离双曲线越远。
