...
更具体地，我们可以写出分子速度（方向确定）的概率分布函数 $f(\bvec v)=f(v)/(4\pi v^2)$．则速度在 $(v_x\text{~}v_x+\Delta v_x,\ v_y\text{~}v_y+\Delta v_y,\ v_z\text{~}v_z+\Delta v_z)$ 内的分子总数：

\begin{equation}
\begin{aligned}
&N f(v_x,v_y,v_z)\Delta v_x\Delta v_y,\Delta v_z=N\qty(\frac{m}{2\pi k T})^{3/2}\exp(-\frac{mv^2}{2kT})\Delta v_x\Delta v_y\Delta v_z\\
&=N\qty[\frac{m}{2\pi kT}\exp(-\frac{mv_x^2}{2kT})]\qty[\frac{m}{2\pi kT}\exp(-\frac{mv_y^2}{2kT})]\qty[\frac{m}{2\pi kT}\exp(-\frac{mv_z^2}{2kT})]
\end{aligned}
\end{equation}
