...
首先证明初等行变换不改变行秩。交换两行或者将某行乘以$k$倍都不改变行向量组的线性相关性。现设行向量组为$\bvec e_1,\bvec e_2...\bvec e_n$，则i行的k倍加到j行后行向量组为$\bvec e_1...\bvec e_i,...k\bvec e_i+\bvec e_j$，合并$\bvec e_i$项，则行变换不改变行向量组的线性相关性。

然后证明初等行变换不改变列秩。设该初等变换矩阵为$M$，列向量组为$\{\theta_i\}$，初等行变换相当于
