...
\textbf{单项式（monomial）}是由常数系数与变量的非负整数次幂的乘积构成的数学表达式。一个单项式可能包含一个变量或多个变量，但它们的指数必须是非负整数。例如，$3x^2y$ 和 $7z$ 是单项式，而包含其他运算（如 \sin x、\cos x、2^x）或非整数次幂（如 x^{1/2} = \sqrt{x}、\displaystyle x^{-1} = \frac{1}{x}）的表达式均不是单项式。


\textbf{单项式（monomial）}是由一个变量或多个变量的非负整数指数幂及常数系数的乘积组成的项。包含其他运算（如 $\sin x$、$\cos x$、$2^x$等）或其他非整数指数（如 $x^{1/2}=\sqrt{x}$、$\displaystyle x^{-1}={1\over x}$）的表达式均非单项式。\textbf{多项式（polynomial）}是由一个或多个单项式通过加法或减法连接组成的表达式。不含变量的单项式通常称为\textbf{常数项}。

\textbf{因式分解（factorization）}就是将一个多项式分解成多个多项式或单项式的乘积形式的操作，构成乘积的式子称为\textbf{因式}。通常分解获得的每个因式要求是\textbf{不可约的（irreducible）}，也就是。由于多项式方程一般都可以整理成某个多项式为$0$的形式，而乘积为$0$则意味着每项都可以取$0$，因式分解的操作可以降低求解多项式方程的难度。
