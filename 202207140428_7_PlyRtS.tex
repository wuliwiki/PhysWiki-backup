...
由\autoref{PlyRtS_the1} ，$\opn{Gal}(f:\mathbb{F})$是$S_4$的可递子群，且有$4\mid\abs{\opn{Gal}(f:\mathbb{F})}$和$\abs{\opn{Gal}(f:\mathbb{F})}\mid \abs{S_4}=24$，故$\abs{\opn{Gal}(f:\mathbb{F})}$只有四种情况：$4, 8, 12, 24$．


\subsubsection{对各情况的讨论}
...
所以如果$[\mathbb{K}:\mathbb{F}]=1$，即$\mathbb{K}=\mathbb{F}$，则按\autoref{PlyRtS_eq9} 知$\opn{Gal}(f:\mathbb{F})/V_4$是一元群，即\textbf{得证}$\opn{Gal}(f:\mathbb{F})=V_4$．
