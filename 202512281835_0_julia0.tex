...
\item typed ir 中常见的 \verb`Core.Intrinsics` 模块的 \verb`Core.IntrinsicFunction` 类（\verb`Core` 中定义的类）函数都有唯一的编号（例如 \verb`add_int (intrinsic function #2)`）这些函数在 \verb`Base` 模块中也放了一份： \verb`add_int, and_int, checked_smul_int, lt_float, mul_float, not_int, or_int, sle_int, slt_int, sub_int, ule_int, ult_int, xor_int`。
