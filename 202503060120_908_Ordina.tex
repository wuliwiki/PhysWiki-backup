...
\[
\left(\omega^{\omega^{\beta_1}} \cdots \omega^{\omega^{\beta_m}}\right)n_k(\omega^{\alpha_{k-1} - \alpha_k} + 1)^{n_{k-1}} 
\cdots 
(\omega^{\alpha_1 - \alpha_2} + 1)^{n_1}~
\]

其中，每个 \(n_i\) 都要替换为其**按非递增顺序排列的有限素数分解**。
...
并且：
\[
\beta_1 \geq \beta_2 \geq \cdots \geq \beta_m~
\]
