...
\subsubsection{历史版本}
\begin{itemize}
\item 做一个历史版本页面， 网址使用 \verb|wuli.wiki/history/|． 直接d 可以通过参数查看任意词条的任意历史（html 通过 scan 程序临时生成）． 该页面需要同时提供对比功能， 使用 \verb|diff2html| 对比两个版本． 注意页面上需要可以选择 \verb|diff2html| 程序的 \verb|diffStyle| （char, word, line 等， 其中 char 非常重要， 否则连续的中文字符都会被看成一个 word）．
