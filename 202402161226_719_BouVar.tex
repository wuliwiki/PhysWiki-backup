...
假设 $f\in V[a,b]$，并且令 $v(x)=V_{\sigma}^{x}(f)$，$x\in(a,b]$，$v(a)=0$。显然，$f(x)=v(x)-[v(x)-f(x)]$。我们将证明 $v(x)$ 和 $v(x)-f(x)$ 是递增的。对于任意的 $x_{1}<x_{2}$，我们有


\begin{align}
 v(x_{2})-v(x_{1})=V_{x_{1}}^{x_{2}}(f)\geq0\Leftrightarrow v(x_{2})\geq v(x_{1}) 
\end{align}
