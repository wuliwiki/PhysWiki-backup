...
对简并情况（即存在多个 $\psi_n^0$ 具有相同的能量本征值），首先要确保所有 $\psi_n^0$ 取 “\textbf{好本征态（good eigen states）}”， 也就是在每个本征子空间中， 要确保矩阵 $\mel{\psi_m^0}{H^1}{\psi_n^0}$ 是对角矩阵。 也就是对同一个能量的所有 $\psi_i^0$， $\mel{\psi_m^0}{H^1}{\psi_n^0}$ 是对角矩阵。

加入了微扰后，每个简并空间可能会分裂成多个不同能级的简并空间，甚至使简并会完全消失。这也叫做\textbf{能级分裂}。

\subsection{绝热近似}
接下来考虑一个应用问题： 含时薛定谔方程中如果初始波函数处于任意一个态， 然后非常缓慢地增加 $H'$ 的强度， 这个态会如何变化？ 是否先分解为好量子态的线性组合， 然后再对好本征态分别修正？
