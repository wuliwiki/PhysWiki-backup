...
区域 $V$ 为 $z=0$ 上方空间．导体给出了边界条件 $\phi(\bvec r')|_{\partial V}=0$．用 Dirichlet 格林函数方法来求解．设 $\bvec r_1=(x,y,z),\bvec r_2=(x,y,-z)$，容易构造出以下函数
\begin{equation}
G_D(\bvec r_1,\bvec r')=\frac{1}{|\bvec r_1-\bvec r'|}-\frac{1}{|\bvec r_2-\bvec r'|}
\end{equation}
它在区域 $V$ 内满足 Dirichlet 格林函数的两个条件．然后就可以利用\autoref{EleGr_eq4} 求解．得
\begin{equation}
