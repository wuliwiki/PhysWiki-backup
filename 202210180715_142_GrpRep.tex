...
设有群 $G$ 和一个线性空间 $V$，记 $V$ 上的全体可逆线性变换为 $\opn{GL}(V)$\footnote{一般线性群\autoref{GL_def1}~\upref{GL}}．如果存在\textbf{同态}$\phi: G\rightarrow \opn{GL}(V)$，那么我们称 $(V, \phi)$ 是群 $G$ 在 $V$ 上的一个\textbf{群表示}．
\end{definition}
...
设有群 $G$ 到集合 $S$ 的群作用 $\rho: G \to \opn{Aut}(S)$，我们可以得到一个 $G$ 在 $\mathbb{F}[S]$ 上表示，
$$\begin{aligned}
