...
由三角约束（$\abs{l-l'} \leqslant 1 \leqslant l + l'$）得 $\Delta l = 0, \pm 1$． 但由\autoref{ThreeJ_eq8}~\upref{ThreeJ} 得 $l + l' + 1$ 为奇数时\autoref{SelRul_eq1} 为零， 所以只有满足
\begin{equation}\label{SelRul_eq2}
 \Delta l = \pm 1
\end{equation}
时跃迁矩阵元可能不为零．

注意不满足\autoref{SelRul_eq4} \autoref{SelRul_eq2} 的矩阵元必为零， 但满足了也可能为零． 这时因为 3j 符号还有其他选择定则， 找到所有 3j 符号（或 CG 系数）为零的情况是十分困难的．
\addTODO{需核实}
