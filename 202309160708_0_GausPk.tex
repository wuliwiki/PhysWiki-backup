...
$f(x)$ 的积分为（用于求电场矢势）
\begin{equation}
\int f(x) \dd{x} = -\frac{\I A_0}{2} \sqrt{\frac{\pi}{a}} \exp(-\frac{k^2}{4a}) \erfi\qty(\frac{k + 2 \I a x}{2 \sqrt{a}}) + C~.
\end{equation}
...
\begin{aligned}
\int A_0\cos^2\qty(a x) \E^{\I k_0x} \dd{x} &= -\I\frac{A_0}{4}\E^{\I kx}\qty(\frac{2}{k} +\frac{\E^{2\I ax}}{2a+k} -\frac{\E^{-2\I ax}}{2a-k})\\
&= \frac{A_0}{4} \qty(\frac{2\sin(k x)}{k} +\frac{\sin[(2 a+k)x]}{2 a+k} +\frac{\sin[(2 a-k)x]}{2 a-k})\\
&+\I\frac{A_0}{4} \qty(-\frac{2\cos(k x)}{k} -\frac{\cos[(2 a+k)x]}{2 a+k} +\frac{\cos[(2 a-k)x]}{2 a-k})~.
\end{aligned}
\end{equation}
