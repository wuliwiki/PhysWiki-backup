...
设 $t$ 为未知变量，称多项式 $\mathrm{det}(A-tE)$ 是矩阵 $A$ 的\textbf{特征多项式}，$\mathrm{det}(A-tE)=0$ 为 $A$ 的\textbf{特征方程式}（或\textbf{长期方程式}）．$\lambda$ 作为 $A$ 的特征多项式根的重数称为算子 $\mathcal{A}$ 的特征值 $\lambda$ 的\textbf{代数重数}．
\end{definition}