...
\end{equation}
其中 $\mat B$ 为基底 $( e_i)$ 向基底 $( e'_j)$ 的过渡矩阵\upref{TransM}.
\end{theorem}
...
& e_i = \sum_i b_{ji} e'_j~.
\end{aligned}
\end{equation}
取任意向量 $v \in V$ 在两个基底下分别表示为 $\sum v_i e_i$ 和 $\sum v'_i e'_i$，我们有
\begin{equation}
\begin{aligned}
\sum v_i e_i &= \sum v'_j e'_j \\
\sum v_i \mathcal{A} e_i &= \sum v'_j \mathcal{A} e'_j \\
\sum v_i \sum_{k} a_{ki} e_k &= \sum v'_j \sum_{k} a'_{kj} e'_k \\
\end{aligned}~
\end{equation}
...
% \begin{equation}
% \mathcal{B} e_j=\sum_i b_{ij} e_i= e'_j~.
% \end{equation}
