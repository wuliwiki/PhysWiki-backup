...
% \item 如果把下标顺序排列的 $e_{i_1}\wedge e_{i_2}\wedge \cdots\wedge e_{i_k}$ 表示为集合 $\{i_1, i_2, \cdots, i_k\}$，那么 $\star\{i_1, i_2, \cdots i_k\}=\{1, 2, \cdots, n\}-\{i_1, i_2, \cdots, i_k\}$．
% \end{enumerate}
...
取$V$的标准正交基$\{e_i\}$，任取$k$次复杂指标$I$、$I_1$和$I_2$，分别对应置换$\sigma$、$\sigma_1$和$\sigma_2$，则
\begin{equation}
\ali{
    e_{\overline{I}}\wedge \star a_1 e_I &= a_1\langle  e_{\overline{I}}, e_I \rangle \cdot (\opn{sign}\sigma)e_I\wedge e_{\overline{I}}\\
...
设$V$是$n$维线性空间，取$v\in\bigwedge^k V$，则$\star \star v = v$．

\end{theorem}

\textbf{证明}：

取$V$的标准正交基$\{e_i\}$，任取一个$k$次复杂指标$I$，其对应的置换为$\sigma$，补为$\bar{I}$，则由\autoref{HodgeO_the1} ，
\begin{equation}
\ali{
