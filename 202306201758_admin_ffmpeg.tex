...
\subsection{视频压缩}
\begin{itemize}
\item \verb|ffmpeg -i 输入视频 -c:v libx264 -crf 23 -b:v 1M -c:a copy 输出视频|。
\item 其中 \verb|-c:v libx264| 使用 H264 编码。 \verb|-crf| （Constant Rate Factor）用于控制转换质量， 数值越小质量越高。 \verb|-b:v 1M| 用于指定码率（1 Mbps）。 \verb|-c:a copy| 直接复制音频不做改动。
\end{itemize}
