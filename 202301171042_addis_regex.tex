...
Linux 命令行中的 regular expression 可以用于匹配一定格式的文字, 通常用于文字搜索和替换 (grep, sed, 等命令)

例如 \verb`c[aeiou]+t` 可以匹配 \verb`cat`, \verb`coat` 等. 其中 \verb`[aeiou]` 代表 \verb`aeiou` 中的任意一个, \verb`+` 代表 \verb`aeiou` 可以出现多次.

\subsubsection{Quantifier}
\begin{itemize}
\item \verb`{m,n}` 连续至少 n 到 m 次
\item \verb`{m,}` 连续至少 m 次
\item \verb`{n}` 连续 n 次, 相当于 \verb`{n,n}`
\item \verb`*` 连续 0 或多次, 相当于 \verb`{0,}`
\item \verb`+` 连续 1 或多次 \verb`{1,}`
\end{itemize}

\subsubsection{Metacharacter}
\begin{itemize}
\item \verb`.` 可以代表任何一个字符包括空格
\item \verb`[abc]` 代表 \verb`abc` 中的任意一个字符
\item \verb`[a-z]` 代表 \verb`a` 到 \verb`z` 中的任意一个字符
\item \verb`\w` 相当于 \verb`[a-zA-Z_0-9]`
\item \verb`\W`
\end{itemize}
...
\subsubsection{转义字符}
\begin{itemize}
\item \verb`\n` 换行, \verb`\r` 回车, \verb`\t` horizontal tab, \verb`\v` vertical tab
\item 另外任何在 regular expression 中有特殊意义的字符都可以用 \verb`\` 变为 literal, 如 \verb`\\` 等
\end{itemize}
