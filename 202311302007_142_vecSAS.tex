...
我们定义（向量的）\textbf{对称积}\footnote{如果域的特征 $\opn{char} \mathbb{F}$ 大于零（比如 $\mathbb{F}_p$），$\frac{1}{2}$的存在性依赖于 $\opn{char} \mathbb{F}$ 是否大于$2$，更一般的，$\frac{1}{n!}$ 存在要求$\opn{char} \mathbb{F} > n$；当我们只考虑 $\mathbb{R}, \mathbb{C}$的时候。}
\begin{equation}
\begin{aligned}
...
\begin{aligned}
\rho(\sigma): V^{\otimes n} &\to V^{\otimes n}~, \\
v_1 \otimes \dots \otimes v_n &\mapsto \opn{sgn}(\sigma) v_{\sigma(1)} \otimes \dots \otimes v_{\sigma(n)}~.
\end{aligned}
\end{equation}
...
v_1 \wedge \cdots \wedge v_n &:= \frac{1}{n!} \sum_{\sigma \in S_n} \text{sgn}(\sigma) v_{\sigma(1)} \otimes \dots \otimes v_{\sigma(n)}~,
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
\rho(\mu)(v_1 \wedge \cdots \wedge v_n) &= \frac{1}{n!} \sum_{\sigma \in S_n} \text{sgn}(\sigma) \rho(\mu)(v_{\sigma(1)} \otimes \dots \otimes v_{\sigma(n)}) \\
&= \frac{1}{n!} \sum_{\sigma \in S_n} \text{sgn}(\sigma) \text{sgn}(\mu) (v_{\mu(\sigma(1))} \otimes \dots \otimes v_{\mu(\sigma(n))}) \\
&= \frac{1}{n!} \sum_{\sigma \in S_n} \text{sgn}(\mu \sigma) (v_{\mu \sigma(1)} \otimes \dots \otimes v_{\mu \sigma(n)}) \\
...
\nu_1 \wedge \nu_n &:= \sum_{\sigma \in S_n} \text{sgn}(\sigma) v_{\sigma(1)} \otimes \dots \otimes v_{\sigma(n)}~,
\end{aligned}
\end{equation}
