...
\end{equation}
这个方程是关于$\lambda$的$n$次多项式方程，其中$n$是$\bvec{M}$的阶数。根据\enref{代数学基本定理}{BscAlg}，此方程一定有$n$个解（计入重数），记为$\lambda_1, \cdots, \lambda_n$。接下来，求解线性方程组（向量的线性方程）
\begin{equation}
\bvec{M}\bvec{X} = \lambda_i\bvec{X}~, 
\end{equation}
每个这样的方程组一定有非零解（为什么？），每个非零解作为列向量，排成一排，所得的矩阵便是$\bvec{Q}$。
