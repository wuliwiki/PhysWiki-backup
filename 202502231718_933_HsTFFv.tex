...
如 \autoref{eq_HsTFFv_4} 所示，$\cos x$ 可以视为正弦型函数的一种特殊形式，其中 $\displaystyle A = \omega = 1, \varphi = \frac{\pi}{2}$。此外，通过适当的变形，可以将参数调整至更规范的范围。利用 $\sin(-x) = -\sin x$ 和 $-\sin x = \sin(x + \pi)$，可以确保 $A$ 和 $\omega$ 取正值，而所有的符号变化都体现在 $\varphi$ 的取值变化上\footnote{需要注意的是，这并不意味着仅仅改变 $\varphi$ 的符号，具体情况将在后续讨论中说明}。因此，在规范化的表达中，参数满足 $A \in (0, +\infty)$，$\omega \in (0, +\infty)$。

由于三角函数的周期性，通常规定 $\varphi \in (-\pi, \pi]$，如果不在此范围内，可以利用 $\sin x = \sin(x + 2k\pi)$ 进行调整，使其化为符合规范的形式。教科书中通常使用 $|A|$ 和 $|\omega|$ 进行表示，但为了简化讨论，后续内容默认正弦型函数已转换为上述标准形式。


如 \autoref{eq_HsTFFv_4} 所示，$\cos x$ 可以视为正弦型函数的一种特殊形式，其中 $\displaystyle A = \omega = 1, \varphi = \frac{\pi}{2}$。此外，通过适当的变形，可以将参数调整至更规范的范围。利用 $\sin(-x) = -\sin x$ 和 $-\sin x = \sin(x + \pi)$，可以确保 $A$ 和 $\omega$ 取正值，而所有的符号变化都体现在 $\varphi$ 的取值变化上\footnote{需要注意的是，这并不意味着只对应改变 $\varphi$ 的符号，具体情况将在后续讨论中说明}。因此，在规范化的表达中，参数满足 $A \in (0, +\infty)$，$\omega \in (0, +\infty)$。而由于三角函数的周期性，通常认为$\varphi\in\left(-\pi,\pi\right]$，如果不在此范围内，可以利用$sinx=sinx+2kpi$化成这种样式。教科书上通常会用$|A|,|\omega|$来表示，但为了简化讨论，后续内容均默认正弦型函数已转换为前面所说的形式。

一下子引入了很多参数让人眼花缭乱，但他们都描述了正弦型函数与正弦函数的关系。一切都是为了找到变化之后的函数与朴素的$\sin x$之间的对应关系。下面对$f(x)$中的参数一个个介绍：
\begin{itemize}
\item $A$表示了函数的最大偏离值，它决定了 $f$ 的值域，即$\left[-A,A\right]$ ，也就是 $f$ 的图像的垂直高度，最高与最低点的差为$2A$。
\item $\omega x + \varphi$，用于描述函数在不同 $x$ 位置的状态。研究的就是
