...
\item named tuple： \verb`x = (a=2, b="abc")`， 类型为 \verb`NamedTuple{(:a, :b), Tuple{Int64, String}}`，语法糖为 \verb`@NamedTuple{a::Int64, b::String}`。然后可以使用 \verb`x.a` 和 \verb`x.b` 来获取元素。 和 \verb`Tuple` 一样，创建后值无法改变。 \verb`typeof(x)` 
