...
以一维单粒子情况为例：位置表象下的波函数$\psi(x)$，可以视为取位置算符的本征矢量（在位置表象下的波函数表示为$\delta(x)$\footnote{这是\textbf{狄拉克 delta 函数}\upref{Delta}．}，在动量表象下则为$\exp(xp/\I\hbar)$．）线性组合而成；动量表象下的波函数$\phi(p)$，可以视为取动量算符的本征矢量（在位置表象下表为$\exp{(\I p x/\hbar)}$，在动量表象下则为$\delta(p)$．）线性组合而成．
