...
\subsubsection{欧氏几何公理}

\textbf{欧几里得几何体系（）}建立在以下五条基本公设之上：
\begin{enumerate}
\item 任意两点可以确定一条直线。
\item 一条直线可以无限延长。
\item 可以以任意点为圆心，任意长度为半径作圆。
\item 所有直角相等。
\item 若一条直线与两条直线相交，并使得同侧内角之和小于两直角（即小于 $180^\circ$），则这两条直线在该侧必相交。
\end{enumerate}

其中，第五公设，即\textbf{平行公设（Parallel Postulate）}，是欧几里得几何与\textbf{非欧几何（non-Euclidean geometry）}的核心区别。改变平行公设会产生不同的几何体系，例如：
...
欧几里得几何体系是基于以下五条基本公设（Postulates）：
	1.	任意两点可以确定一条直线。
	2.	一条直线可以无限延长。
