...
\subsection{库仑平面波作为基底} \label{sub_HeAna2_2}
...
\end{equation}
注意 $\ket{C(\bvec k)}$ 的无穷远平面波是不含库仑相移的（已经在系数中抵消掉了）。 把 $\ket{\psi(t)}$ 投影到 $\ket{C(\bvec k)}$ 上有（\autoref{eq_CulmWf_10}~\upref{CulmWf}）
\begin{equation}
...
可见使用平面波和球面波计算延迟是一样的。
 ...
 根据边界条件\autoref{eq_CulmWf_4}~\upref{CulmWf}， $C(\bvec k)$ 本身也自带一个不收敛的相位 $-\eta \ln 2kr$， 而电场的傅里叶变换 $\tilde f(-\omega)$ 相位恒定， 所以对某个方向 $\uvec k$， 有
\begin{equation}
