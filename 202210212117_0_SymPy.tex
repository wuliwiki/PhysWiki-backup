...
\end{lstlisting}
对于多个变量的定义方法类似．需要说明的是\verb|sy.symbols('x0, x1')|中的\verb|,|用空格隔开也是可以的．即\verb|sy.symbols('x0, x1')=sy.symbols('x0 x1')|．
...
\begin{lstlisting}[language=python]
sy.sin() sy.asin() sy.sinh() sy.exp() sy.log() sy.sqrt() # 返回的类型是各自的函数名
\end{lstlisting}
