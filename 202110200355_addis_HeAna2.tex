...
\subsection{直接投影}
对于氢原子， 一阶微扰也可以直接投影到库仑平面波（\autoref{CulmWf_eq7}~\upref{CulmWf}， 取减号） $\ket{C(\bvec k)}$
\begin{equation}
\ket{\psi(t_f)} = \int c(\bvec k)\ket{C(\bvec k)} \E^{-\I k^2 t/2} \dd[3]{k}
\end{equation}
\begin{equation}
c(\bvec k) = -\sqrt{2\pi}\I\mel{C(\bvec k)}{z}{i} \tilde f(-\omega)
\end{equation}
根据边界条件\autoref{CulmWf_eq4}~\upref{CulmWf}， $C(\bvec k)$ 本身也自带一个不收敛的相位 $-\eta \ln 2kr$， 而电场的傅里叶变换 $\tilde f(-\omega)$ 相位恒定， 所以对某个方向 $\uvec k$， 有
\begin{equation}
\pdv{E}[c(\uvec k k(E)) -\eta \ln 2kr] = \pdv{E} \mel{C(\bvec k)}{z}{i} - \eta \pdv{E} \ln 2kr
\end{equation}
