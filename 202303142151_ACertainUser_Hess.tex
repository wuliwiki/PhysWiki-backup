...
\subsection{更数学的表述}
我们尝试使用更数学的方式说明“设计路径”的含义。假设我们已经知道某一状态下系统的内能，
$$U_0=U(T_0,V_0)$$
那么怎么求解另一状态下系统的内能
$$U_1=U(T_1,V_1)$$
？
...
这两个偏导数就暗示了“设计路径”的原理：先固定体积不变，将温度从$T_0$升高至$T_1$；再控制温度不变，将体积从$V_0$升高至$V_1$。
...
根据以上定理，$$\left(\pdv{U}{T}\right)_V = C_V$$，就是热容，可以由实验测得；$$ \left(\pdv{U}{V}\right)_T  = PT\beta-P $$，其中$\beta$是定容压强系数，也是容易实验测量的量。

因此，尽管我们不知道如何直接测量内能，但阴差阳错之间我们居然原则上能够计算内能（的差值）了。
$$
U(T_1,V_1)=U(T_0,V_0) + \int_{T_0}^{T_1} C_V \dd T + \int_{V_0}^{V_1} P(T\beta-1) \dd V 
$$


那么你可能想问，那么我们如何确定$U(T_0,V_0)$？事实上我们\textsl{不需要确定}，因为在热力学中我们更关系的是状态量的变化，而非他的绝对数值。

同理，熵也可以写出类似的表达式\upref{MacroS}：
\begin{equation}
