...
# module1.jl
module Module1
export function1 # using Module1 时，可以把 Module1.function1 写为 function1
function function1()
    println("This is function1 from Module1")
end
end
\end{lstlisting}
\item \verb`export` 的名字只影响 \verb`using Module1`，不会影响 \verb`import Module1`。
\item \verb`using MyMod: <name>` 不需要 \verb`export <name>`
...
\begin{itemize}
\item 之前为了解决 Tim to first plot 问题，需要手动创建 system image，也就是一个 so 文件。 但 1.10 之后这个问题已经系统性解决（预编译），用户不需要这么做了。 之前在包安装时就并行地预编译，但是用户加载的时候也有预编译，这部分现在也变为并行了。
\item System Image 使用的是 PackageCompiler 模块。
\item 
\end{itemize}
