...
在群表示一节中曾提到群元可与线性变换建立同态关系，由此可以给出群的线性表示。矩阵表示是一种特殊的有限维度表示 $(\mathbb{F}^n, \rho)$，其中 $\rho: G \to \opn{GL}(n; \mathbb{F})$\footnote{间一般线性群\upref{GL}}。

对于一般的群表示 $(V, \rho)$，在向量空间 $V$ 中选取合适的基后，可以将线性变换写成矩阵的形式，即 $\opn{GL}(V) \cong \opn{GL}(\mathbb{F}^n) = \opn{GL}(n; \mathbb{F})$。
