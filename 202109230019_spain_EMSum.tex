...
\end{equation}
Euler-Mascheroni常数\autoref{Masche_eq1}~\upref{Masche}由调和级数与自然对数的差值的极限所给出，利用\autoref{EMSum_eq3} 和该常数可以给出类调和级数前$n$项和较好的计算公式.\\
应用\autoref{EMSum_eq3} 式,从而可得
...
\end{equation}
由Dirichlet定理可知，\autoref{EMSum_eq5} 积分存在，因此将\autoref{EMSum_eq4} 结合代入\autoref{EMSum_eq3} 就有
\[
    C_{2}=\gamma
\]
	因此可得
