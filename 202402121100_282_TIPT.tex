...
我们只讨论量子力学中束缚态， 因为它们的本征值和厄米矩阵的本征值一样是离散的。 唯一需要注意的是， 线性代数中的厄米矩阵是有限维的（即行数和列数有限）， 而量子力学中的厄米矩阵可能是无穷维的（例如量子简谐振子\upref{SHO}）。实际操作中，我们可以截取一个有限维矩阵，并通过增加维数来趋近正确结果。

另一个要注意的是，一些量子系统（例如氢原子）中不光有离散的束缚态还有连续态（continuum），且考虑二阶或以上微扰时往往不可忽略，详见“含连续态的不含时微扰（量子力学）\upref{TIPTc}”。
