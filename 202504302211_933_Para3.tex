...
\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{./figures/e3f63aab20497a5c.png}
...
设抛物线的焦点为 $F$，准线为 $l$，点 $P$ 是抛物线上的任意一点。则过 $F$ 到 $P$ 的连线与点 $P$ 处的法线所成的角，等于从点 $P$ 向准线发射的垂线与法线所成的角，即：
\begin{equation}
\angle(FP, n) = \angle(PQ, n)~,
\end{equation}
其中 $n$ 是 $P$ 点处的法线方向，$Q$ 是 $P$ 向准线作垂线的垂足。
\end{theorem}

这一性质说明，从焦点 $F$ 发出的光线若射向抛物线上任一点 $P$ 并发生镜面反射，则其反射光线将沿着与准线垂直的方向传播。也就是说，抛物线能够将来自焦点的光线反射为平行光线，这是抛物线广泛应用于探照灯、雷达、卫星天线等的根本原理。

从几何上理解这一性质，需要借助如下引理。

\begin{lemma}{设点 $A$ 到某定点 $F$ 和某定直线 $l$ 的距离相等，则 $A$ 在以 $F$ 为焦点、$l$ 为准线的抛物线上。反之，若点 $P$ 满足 $|PF| = \text{dist}(P, l)$，则该点在抛物线上，且 $P$ 到 $F$ 的连线和 $P$ 到 $l$ 的垂线关于该点的法线对称。}\label{lem_Para3_1}
\end{lemma}

设抛物线的标准形式为 $x^2 = 2py$，则其焦点为 $F(0, \tfrac{p}{2})$，准线为 $y = -\tfrac{p}{2}$。取抛物线上一点 $P(x, y)$，显然有：
\begin{equation}
