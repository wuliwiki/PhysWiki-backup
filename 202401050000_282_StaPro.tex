...
\end{equation}
例如在二维或三维状态空间， 若令矢量函数为 $\bvec f(\bvec x) = \sum_i f_i(\bvec x) \uvec x_i$， 那么当旋度 $\curl \bvec f = \bvec 0$ 时， $\bvec f(\bvec x)$ 就是一个保守场\upref{V}， 必存在势函数 $V(\bvec x)$。 对于高维情况， 需要使用外导数\upref{ExtDer} 来判断保守场。
...
为了防止这种误解， 一些教材中把过程量的微小变化记为 $\delta Q$ 而不是 $\dd Q$。后者一般表示全微分\upref{TDiff}， 而只有 $\bvec x$ 的函数可以做全微分。
