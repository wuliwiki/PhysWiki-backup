...
\abs{\overline{F}'_y-F'_y}<\epsilon,\quad\abs{\overline{F}'_{y'}-F'_{y'}}<\epsilon
\end{equation}
所以
\begin{equation}\label{Varia_eq3}
\begin{aligned}
&J(\overline{y})-J(y)\\
&=\int_a^b\qty[F'_y\eta(x)+F'_{y'}\eta'(x)]\dd x+\int_a^b\qty[(\overline{F}'_y-F'_y)\eta(x)+(\overline{F}'_{y'}-F'_{y'})\eta'(x)]\dd x\\
