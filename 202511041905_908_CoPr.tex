...
在这一背景下，一个突出的例子是\emph{非均匀宇宙学}（inhomogeneous cosmology），其目的是模拟观测到的宇宙加速膨胀以及宇宙常数（cosmological constant）现象。该类模型并不采用当前被广泛接受的“暗能量”（dark energy）概念，而是假设宇宙的非均匀性远比现有认知更强——例如，假设太阳系位于一个极其庞大的低密度空洞（low-density void）中。\cite{ref32} 然而，为使模型符合观测结果，太阳系必须非常接近该空洞的中心，这一假设立即违背了哥白尼原理。
