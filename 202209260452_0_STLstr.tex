...
\item \verb|key| 如果不存在， 调用 \verb|umap[key]| 会生成一个新的 \verb|pair|， 其 value 是默认值 （例如 \verb|int| 返回 \verb|0|）， 并返回该 value 的左值．
...
\item \verb|priority_queue<T, vector<T>, std::greater<T>>|
