...
下面的内容高中目前不再涉及，作为开阔视野。这四种均值都有广泛的应用\footnote{此处只给出这四种，其实有很多种均值的定义，它们也都可纳入下面的均值不等式中。}。事实上，上面的四种平均数都可以扩展至多个数，对于一列数$a_1,a_2,\cdots,a_n$，有如下均值形式。

\begin{table}[ht]
\centering
\caption{常见均值}\label{tab_HsIden2}
\begin{tabular}{|c|c|c|c|}
\hline 名称 & 常用字母 & 表达式 & * \\
\hline
平方平均数（Quadratic mean，或平方均值）\footnote{也称作\textbf{均方根（root mean square，RMS）}}&$Q$ & $\displaystyle \sqrt{\frac{a_1^2 + a_2^2 + \cdots + a_n^2}{n}}$ & * & * \\
\hline
算术平均数（Arithmetic mean，或算数均值）&$A$ & $\displaystyle \frac{a_1 + a_2 + \cdots + a_n}{n}$ & * & * \\
\hline
几何平均数（Geometric mean，或几何均值）&$G$ & $\displaystyle \sqrt[n]{a_1 \cdot a_2 \cdot \cdots \cdot a_n}$ & * & * \\
\hline
调和平均数（Harmonic mean，或调和均值）&$H$ &  $\displaystyle \frac{n}{\frac{1}{a_1} + \frac{1}{a_2} + \cdots + \frac{1}{a_n}}$ & * & * \\
\hline
\end{tabular}
\end{table}
...
分别为：
\begin{itemize}
\item $Q$——\textbf{}，又称\textbf{}，可以看作是$\displaystyle \sum_{i=1}^nQ^2= \sum_{i=1}^na_i^2$
\item $A$——\textbf{}，可以看作是$\displaystyle \sum_{i=1}^nA= \sum_{i=1}^n{a_i}$
\item $G$——\textbf{}，可以看作是$\displaystyle \prod_{i=1}^nG= \prod_{i=1}^n{a_i}$
\item $H$——\textbf{}，可以看作是$\displaystyle \sum_{i=1}^n\frac{1}{H}= \sum_{i=1}^n\frac{1}{a_i}$
\end{itemize}
...
\textbf{柯西-施瓦茨不等式（Cauchy-Schwarz Inequality）}，也称\textbf{柯西不等式}，为两个向量的内积（inner product）与它们的模长（norm）之间建立了重要的不等关系。柯西不等式是根据“内积”这种运算的定义推知的属性，因此所有满足“内积”定义要求的运算，或者说可以称为“内积”的运算，都会满足这个不等式\footnote{关于内积更详细的内容可以参考\enref{内积、内积空间}{InerPd}。}。
...
鉴于这部分是为开阔视野，上面采用了比较严谨的表述，但涉及到很多高中未曾了解的知识。不必担心，在高中阶段由于只涉及到几何向量的运算，这时的内积指的就是$\bvec{u}\cdot\bvec{v}=|\bvec{u}|\cdot|\bvec{v}|\cos\theta$，而所谓对应的“范数”指的就是向量的“模长”$|\bvec{u}|$。这时，利用坐标将两个向量表示出来，即$\bvec{u}=(a,b),\bvec{v}=(c,d)$，就得到了下面的表达方法：
\begin{equation}
ac+bd\leq\sqrt {a^2 + b^2}\cdot\sqrt{c^2 + d^2}\quad\text{或} \quad(ac+bd)^2\leq(a^2 + b^2)\cdot(c^2 + d^2)~.
\end{equation}
这种表示方法在高中的题目中更为常见。

其实，以高中常见的二维向量为例，柯西不等式不难推出。由于此时内积定义为：
\begin{equation}
\bvec{u} \cdot \bvec{v} = \|\bvec{u}\| \|\bvec{v}\| \cos \theta~.
\end{equation}
