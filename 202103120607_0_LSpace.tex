...
\textbf{说明}： 加法运算 $X \times X \to X$ 是一个二元映射（\autoref{map_sub1}~\upref{map}）, 注意运算的结果必须仍然落在 $X$ 中． 我们把这种运算叫做\textbf{封闭（closed）}的\footnote{一些文献中也叫 “闭合”}． 数乘运算同样也是封闭的， 一个矢量数乘后仍然落在 $X$ 中． 以后会看到一个矢量空间的子集也可以是矢量空间， 称为子空间\upref{SubSpc}， 子空间上的两种运算的结果只能落在同一个子空间上．
