...
\end{equation}
当系统不存在简并时， 渐进近似下含时薛定谔方程的通解可以表示为
\begin{equation}\label{eq_AdiaQM_2}
\Psi(t) = \sum_n c_n(0) \psi_n(t) \E^{\I\gamma_n(t)}\E^{\I\theta_n(t)}~.
\end{equation}
其中 $\psi_n(t)$ 是 $H(t)$ 一组正交归一本征态（为了方便我们把它们限制为实值函数），满足不含时薛定谔方程
\begin{equation}
...
\end{equation}
\begin{equation}
\theta_n(t) = -\I\frac{1}{\hbar} \int_0^t E_n(t')\dd{t'}~.
\end{equation}
