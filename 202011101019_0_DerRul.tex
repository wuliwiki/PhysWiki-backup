...
\end{equation}
从几何上来看， 我们可以把 $f(x), g(x)$ 看做一个矩形的两条边长， 它们的乘积 $f(x)g(x)$ 就是矩形的面积， 而 $f(x+h)g(x+h)$ 就是把矩形两条边长分别改变一个很小的长度后的面积． 当 $x$ 增加 $h$ 后， 令 $f$ 和 $g$ 分别增加 $\Delta f$ 和 $\Delta g$， 那么矩形面积的增量可以分解为三部分
\begin{equation}
