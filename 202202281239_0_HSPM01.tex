...
其中，$\bvec v$ 是速度矢量， $\Delta \bvec x$ 是 $\Delta t$ 时间内的位移． 其中 $\Delta t$ 是一段极小的时间． 速度采用比值定义法，不能说 $\bvec v$ 与 $\Delta \bvec x$ 成正比．

在直线运动的情况下， 可以把直线作为 $x$ 轴， 使用一个坐标 $x$ 表示位置（标量）， 用坐标之差 $\Delta x$ （标量）表示位移， 当 $\Delta x > 0$， 就代表位移矢量延 $x$ 轴正方向， $\Delta x < 0$ 则指向负方向． 用标量代替矢量是为了方便， 但在概念上位移仍然是矢量．
