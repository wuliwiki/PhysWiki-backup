...
\item \verb|cmake| 中的变量本质上都是字符串。 即使是 list， 也不过是特殊的字符串（\verb|str1;str2;str3|）。 所谓的 \verb|bool| 类型不过是 \verb|ON| 和 \verb|OFF| 字符串。
\item 如果字符串中没有空格或者特殊符号，两边不需要加双引号。 例如 \verb|set(my_var abc)|。
\item \verb`message(STATUS "...")` 可以输出到 stdout。 如果 \verb`message(STATUS ${变量})` 中的变量是 list， 那么输出中不会有空格， 应该用 \verb`message(STATUS "${变量}")`， 输出中每个元素会用分号隔开。 \verb`message(STATUS ${变量1} ${变量2} ...)` 相当于把若干变量合成 list。
