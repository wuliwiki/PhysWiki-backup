...
设有一初始量子态$\ket{s}=\sum_a c^a\ket{a}=\sum_a c^a\ket{a; 0}$．在薛定谔绘景中，一段时间$t$后，这个态变为$\sum_a c^a\mathcal{U}(t)\ket{a}=\sum_ac^a\E^{-\I at}\ket{a}$，即系数的变化是$c^a\to c^a\E^{-\I at}\ket{a}$．

根据\textbf{转移矩阵}\upref{TransM}的\autoref{TransM_sub1}~\upref{TransM}，如果固定$\ket{s}$不变，而基根据转移矩阵$\Omega(t)$改变，那么$\ket{s}$的系数应变为$\omega^i_j(t)$
