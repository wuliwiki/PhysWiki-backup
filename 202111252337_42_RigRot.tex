...
\begin{example}{箱子倾倒}
如\autoref{RigRot_fig3} ，一长方体箱子初始时倾角为 $\theta_0$，以初速度为0绕其和地面固定轴转动，求其运动方程．
\begin{figure}[ht]
\centering
\includegraphics[width=2.1cm]{./figures/RigRot_3.pdf}
\caption{箱子倾倒} \label{RigRot_fig3}
\end{figure}
记倾角为 $\theta$，则箱子受到的对转轴的力矩为 $\bvec\tau=M\bvec r_c\cross \bvec g=Mgr_c\sin\theta$ ，由角动量定理\autoref{AMLaw_eq1}~\upref{AMLaw}
\begin{equation}
I\ddot \theta = Mg r_c \sin\theta
\end{equation}

这显然和上例属于同一模型．只需把质量为 $M$ 的长方体看成质量为 $M$ 的长方形即可．事实上，长方体箱子的旋转运动和其截面长方形的旋转一致，那么只需将\autoref{RigRot_ex1} 的不规则的薄片看成规则的长方形即可．
