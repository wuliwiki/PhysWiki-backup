...
\pentry{序列的极限\upref{SeqLim}，函数的极限\upref{limfx}}
...
\end{theorem}
\begin{theorem}{局部有界性}
设函数 $f(x)$，$\lim\limits_{x\rightarrow x_0}f(x)$ 存在（不为无穷大量），那么存在 $x_0$ 的一个去心邻域  $U_0(x_0,\delta)$，满足 $\exist M>0$，$\forall x\in U_0(x_0,\delta)$，$|f(x)|<M$，即 $f(x)$ 在 $U_0(x_0,\delta)$ 上有界．
\end{theorem}
