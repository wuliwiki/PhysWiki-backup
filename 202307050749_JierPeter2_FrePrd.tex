...
将上述步骤最后所得的词按\textbf{共合积}的定义化简，即先将所有形如$\varphi(x)\phi(x^{-1})$和$\phi(x)\varphi(x^{-1})$的元素化为$1$，然后将所得结果按\textbf{自由积}的定义化简。此时如果所得结果是$1$，那么$W$确实是$S$中的元素相乘而得\fo；如果不是，那么它已经不可再施行上述\textbf{补充、化简的程序}，从而$W$不可能等价为$1$。
