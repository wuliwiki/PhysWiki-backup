...
本文使用原子单位制\upref{AU}。 若初始时刻 $t_0$ 一个自由粒子的波函数 $\Psi(\bvec r,t)$ 已知，接下来空间中出现了随时间变化的电场 $\bvec {\mathcal E}(t)$ （我们采用偶极子近似\upref{DipApr}， 即假设电场不随位置变化， 感生磁场可以忽略）\upref{DipApr}， 那么接下来波函数会如何变化呢？
...
\end{equation}
它的初始动能为 $E_0 = \bvec k_0^2/(2m)$。

\begin{equation}\label{eq_Volkov_4}
% 已代入薛定谔方程验证
\Psi_{\bvec k_0}^L(\bvec r, t) = (2\pi)^{-3/2} \exp\qty[\I m\bvec v(t)\vdot \bvec r - \I\int_{t_0}^t E(t') \dd{t'} - \I E_0 t_0]~.
\end{equation}
