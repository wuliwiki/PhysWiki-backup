...
计算$n=2$的八个能级$|2lm_lm_s\rangle$，找到在强场赛曼分裂下的不同态的能量，并且将每个答案都表示为三个项：波尔能级，精细结构，和赛曼效应部分之和的形式．
...
\end{align}
在这一组基中，$H_{\rm{fs}}$的非零矩阵元都是对角元，根据\autoref{ZemEff_eq11} 可得，$H_{\rm{z}}$有四个非对角元素．接下来我们计算出$H_{\rm{fs}}$和$H_{\rm{z}}$的矩阵元，进而构造出之前提到的在$n=2$的情况下，完整的$W$矩阵：
\begin{equation}
\begin{pmatrix}
