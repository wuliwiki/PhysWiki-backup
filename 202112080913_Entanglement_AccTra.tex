...
\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/AccTra_1.png}
\caption{例 1 图示} \label{AccTra_fig1}
\end{figure}
\addTODO{画图， 轨道在 $S$ 系， 以圆盘加速度为 $S'$ 系的原点， 无相对转动， 使用\autoref{AccTra_eq1}．}

解： 圆盘中心 $A$ 点对于圆形轨道中心 $O$ 点的加速度为 $\bvec a_r$ 圆盘边缘对于圆盘中心 $A$ 点的加速度为 $\bvec a_{S'}$ ,那么圆盘边缘上一点在任意时刻的加速度 $\bvec a_S = \bvec a_r + \bvec a_{S'}$ ．

圆盘中心$A$ 点对于 $O$ 点的加速度为
\begin{equation}
\bvec a_{r} = \bvec \omega^{2} R
\end{equation}

圆盘绕圆轨道一周所用的时间为$\frac{2 \pi}{\bvec \omega}$ ，$A$ 点走过的路径长为$2 \pi (\bvec R + \bvec r)$ ，所以圆盘边缘的点转动的线速度为 $\frac{2 \pi (R+r)}{\frac{2 \pi}{\bvec \omega}}$ （圆心速度等于圆边缘的线速度），所以圆盘边缘上任意点对圆心 $A$ 点的加速度 $\bvec a_{S'}$ 为
\begin{equation}
\bvec a_{S'} = \frac{\bvec v^2}{\bvec r}= \frac{(\frac{2 \pi (\bvec R+\bvec r)}{\frac{2 \pi}{\bvec \omega}})^2}{\bvec r} = \frac{(\bvec R + \bvec r)^2 \bvec \omega^2 }{\bvec r}
\end{equation}
