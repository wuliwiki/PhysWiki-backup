...
\end{lstlisting}
第一位 0 说明是正数， 1 是复数． 接下来 11 位指数的第一个 \verb|1| 表示指数是正数， \verb|0| 表示负数且后十位采用补码（这和一般的整数编码不同， 比较奇怪）， 这里指数最后两位是 2， 于是指数项为 $2^{2+1} = 8$ （指数无论正负都要加 1， 也比较奇怪）． 小数项应该是 $10/8 = 1+1/4$． 第 13 bit 表示 1/2， 14 bit 表示 1/4， 所以第 14 bit 是 1， 后面的小数全是零． 另外， 由于 intel 的 x86 和 x86-64 都使用 little endian， 所以在内存中实际上 byte 的顺序是相反的．
...
\end{lstlisting}
其中 \verb|denorm_min()| 比较特殊， 这似乎说明当指数 bit 全为 0 的时候， 指数为 \verb|-1022|， 且小数部分前面不加 1．
