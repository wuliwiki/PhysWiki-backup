...
事实上，注意到 $\exp(\I q\bvec E\vdot\bvec r\dd t)$ 不过是一个普通函数（一个平面波）而不是微分算符， 所以我们只需要把它和波函数相乘： $\exp(\I q\bvec E\vdot\bvec r\dd t)\Psi$。 为了使相乘后的函数仍然具有\autoref{eq_HyTDSE_3} 的形式， 可以先根据\autoref{eq_Pl2Ylm_1}~\upref{Pl2Ylm} 对其展开为分波。
\begin{equation}
\begin{aligned}
&\exp(\I q\bvec E\vdot\bvec r\dd t)\Psi = ~
