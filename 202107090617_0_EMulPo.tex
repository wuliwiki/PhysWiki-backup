...
当 $m = 0$ 时， $l = 1$ 的项就是\textbf{电单极子（electric monopole）} 具有球对称的电荷分布和势能分布， $l = 2$ 的项就是\textbf{电偶极子（dipole）}\upref{eleDP2}（$\propto\cos \theta$）， $l = 3$ 的项是\textbf{电四极子（quadrupole）}…… $l = N$ 的项叫做电 $2^N$ 极子．

为什么这么叫呢？ 因为电 $2^N$ 极子可以用 $2^{N-1}$ 个电荷为 $q$ 的正电荷以及 $2^{N-1}$ 个电荷为 $-q$ 的负电荷an．
