...
\end{equation}
令变换矩阵为 $\pmat{C_{1} & C_{2}\\ C_{2}^* & -C_{1}^*}$， 即
\begin{equation}
\psi_{k,r}(x) = C_1\psi_{k,1}(x) + C_2\psi_{k,2}(x)
\end{equation}
\begin{equation}
\psi_{k,l}(x) = C_{2}^*\psi_{k,1}(x) - C_{1}^*\psi_{k,2}(x)
\end{equation}

令他们满足
\begin{equation}
\psi_{k,r}(x) = C_{1}\psi_{k,1}(x) + C_{2}\psi_{k,2}(x) \propto \exp(\I kx) \qquad (x > L)
\end{equation}
得到条件
\begin{equation}
C_{1}A_{+,1}\E^{-\I\phi_{+,1}} + C_{2}A_{+,2}\E^{-\I\phi_{+,2}} = 0
\end{equation}
这就可以求出 $C_1, C_2$， 以及变换矩阵．
