...
更一般地， 函数值为复数的多元函数都可以叫做复变函数． 例如 $f: \mathbb R \to \mathbb C$ 或者 $f:\mathbb R^N \to \mathbb C$． 事实上 $f: \mathbb C \to \mathbb C$ 本质上就是 $f: \mathbb R^2 \to \mathbb C$ 或者更直接地， $f: \mathbb R^2 \to \mathbb R^2$．
...
如果只需满足 “兼容” 的条件， 那么拓展的方式可以有无限多种， 但一般我们还需要添加另一个条件， 就是拓展后的函数必须是解析的， 这叫做\textbf{解析拓延} % 链接未完成
