...
\autoref{HDipM_tab1} 给出了 $Z=1$ 时的 $\mel{\psi_{n',l',m'}}{z}{\psi_{n,l,m}}$， 由于这是一个对称矩阵， 只给出矩阵的上半三角． 当 $Z > 1$ 时把表中每个系数除以 $Z$ 即可． 这是因为 $\psi_{n,l,m}$ 与 $Z$ 成反比进行缩放， 导致 $\abs{\psi_{n,l,m}}^2$ 和 $z$ 的平均值也是如此．
\begin{table}[ht]
\centering
