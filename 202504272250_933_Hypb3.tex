...
就像其他类型的方程一样，只要一个表达式符合双曲线标准方程的形式，就可以确定它的图像是双曲线。有了学习圆和椭圆时的经验，自然也会希望在这里了解双曲线的参数方程。在双曲线的参数方程中，可以使用三角函数中满足双曲线需要关系的$\sec x$和$\tan x$，或者更简单地可以使用\enref{双曲函数}{TrigH}。但双曲线的参数方程在高中阶段并不要求掌握。这里给出相关内容，主要是为了知识的完整性。
