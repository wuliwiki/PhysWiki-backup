...
这里引入一个族的概念，由一个或多个参数连续变化而生成的一组对象就称为\textbf{族（family）}。通过对椭圆的标准方程代换可以得到，取$p=\lambda a,q=\lambda b(\lambda>0)$，以\autoref{eq_Elips3_3} 为例，带入整理可得：
\begin{equation}
\frac{x^2}{a^2} + \frac{y^2}{b^2} = \lambda^2,\qquad(a>b>0)~.
\end{equation}
