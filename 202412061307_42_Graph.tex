...
\begin{equation}
\begin{aligned}
\pi:E\rightarrow P_2(V),\pi(\varphi(e))=\{x,y\},\\
\text{其中} e=\{x,y\}\text{或}(x,y)\text{或}(y,x)
\end{aligned}~
\end{equation}
为（关于 $G$ 的）无向化映射。
...
\begin{definition}{边的无向等价}
设有图 $G=(V,E,\varphi)$ 和如下定义的映射 $\pi$。称边 $e_1,e_2\in E$ 是\textbf{无向等价}的，若
...
设 $G_1=(V_1,E_1,\varphi_2)$ 是有向图，$G_2=(V_2,E_2,\varphi_2)$ 是无向图。
