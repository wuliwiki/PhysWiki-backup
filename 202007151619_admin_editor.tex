...
如果你已经会用 LaTeX， 请仔细阅读这段话． 我们必须强调这个编辑器只是在 \href{https://www.mathjax.org/}{MathJax} 的基础上做了一个简单的拓展， MathJax 只支持公式环境， 我们另外实现了基本的 LaTeX 命令， 如标题， 粗体， 常用的环境的简单代码以及对他们的引用． 这些功能大部分可以在工具栏上找到（找不到就是大概率不支持） 编辑器只支持唯的模板（百科使用的模板）． 最后， 整个百科是一个 document 环境， 除了 main.tex 外其他所有 tex 文件都将作为一个 section 插入．
