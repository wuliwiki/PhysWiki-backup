...
Kruskal 算法的思想较为简单，思想是基于贪心算法．具体做法是：把每条边的权值从小到大排序，然后依次枚举每条边 $(a,b,w)$，如果 $a$ 和 $b$ 不联通，就合并这两个点所在的集合，并把答案加上这条边的权值，不连通就忽略这条边．初始化每个结点都是一个集合．维护不相交森林可以用并查集，该算法的时间复杂度的瓶颈在排序上，所有 Kruskal 的时间复杂度为 $\mathcal{O}(m \log_2 m)$．