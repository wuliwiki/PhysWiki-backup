...
设域$\mathbb{F}$上有\textbf{可分的不可约}多项式$f(x)=ax^4+bx^3+cx^2+dx+e$，$\opn{ch}\mathbb{F}\neq 2$．设$\mathbb{K}\subseteq\overline{\mathbb{F}}$是$f$的预解式\autoref{PlyRtS_eq7} 的分裂域．
...
3．当$[\mathbb{K}:\mathbb{F}]=2$，如果$f$在$\mathbb{F}$上不可约则$\opn{Gal}(f:\mathbb{F})\cong D_4$\footnote{$D_n$是正$n$边形的对称群．}，否则$\opn{Gal}(f:\mathbb{F})\cong \mathbb{Z}_4$；
