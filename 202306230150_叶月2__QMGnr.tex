...
设$Q$为多粒子体系的一算符，对应的单粒子算符为$q$，那么总的测量结果为单粒子测量结果之和。引入粒子数算符$N(q_i)=a(q_i)^\dagger a(q_i)$，其中${q_i}$为单体算符的本征值谱，该粒子数算符统计处于该本征值的粒子数目。那么$Q=\sum q_i a(q_i)^\dagger a(q_i) $。为了得到从单粒子算符到多粒子算符的形式，我们需要给这个式子“加上”一个表象。
