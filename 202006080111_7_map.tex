...
如果我们在 $B$ 中取一个子集 $C$， 那么 $A$ 中所有能映射到 $C$ 的元素就称为 $C$ 的\textbf{原像（inverse image 或 preimage）}， 记为
\begin{equation}
...
对于映射$f:A\to B$，可以定义其逆映射$f^{-1}:2^B → 2^A$：

\begin{equation}
f^{-1}(C) = \qty{x | x \in A, f(x) = C}
\end{equation}

 逆映射$f^{-1}$将$B$中的每个元素映射到$A$的某个子集上（而不是$A$的某个元素上）．特别地，不在$f$的值域中的元素被$f^{-1}$映射到空集上，而空集也是$A$的一个子集．如果$f$是一个双射，那么$f^{-1}$都映射在$A$的单元素子集上，那么我们也可以认为此时$f^{-1}$实际上是映射在单个元素上，也是从$B$到$A$的映射．
