...
\item 一个 method instance 中可能有多个编译好的 \verb`jl_code_instance_t`（链表），这是因为可能有不同的 world age。 每一个 \verb`jl_code_instance_t` 的 \verb`next` 字段指向下一个 instance， \verb`invoke` 字段（类型 \verb`jl_callptr_t`）是调用该 code instance 的函数指针， 如果参数类型等不匹配导致调用失败，会返回 \verb`NULL`。
\end{itemize}
