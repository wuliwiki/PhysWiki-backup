...
\subsubsection{特殊点 $z = \tfrac{1}{2}$ 处的函数值}

#### 高斯第二求和定理（Gauss’s Second Summation Theorem）：
...
在某些参数为有理数的特殊情况下，超几何函数还可简化为**代数数**。相关结果可参考 Gessel & Stanton（1982）和 Koepf（1995）所列的公式。
...
当 $-\pi < x < \pi$ 时成立，其中 $T_a$ 是**广义切比雪夫多项式**（generalized Chebyshev polynomial）。

这些恒等式说明超几何函数在特定参数和变量值下可以退化为三角函数、多项式或代数表达式，体现了它的统一性与丰富结构。
