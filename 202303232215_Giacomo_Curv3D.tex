...
\begin{definition}{曲线}
令 $I$ 是实数轴 $\mathbb{R}$ 上的一个开区间，则称\textbf{连续函数}$f:I\to \mathbb{R}^n$ 为 $\mathbb{R}^n$ 中的一条\textbf{曲线（curve）}，（若$I$是一个闭区间则$f$被称为\textbf{道路（path）}）。此处连续是指函数的 $n$ 个分量都是 $\mathbb{R}\to\mathbb{R}$ 的连续函数。
\end{definition}
