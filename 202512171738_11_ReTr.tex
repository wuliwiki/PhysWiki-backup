...
反射变换的概念来源于平面上的轴反射。根据初中几何学里就已接触的轴反射定义，我们可以用矩阵表示关于$y$轴的轴反射，。
\begin{equation}
\mat M=\pmat{-1&1\\0&0}~.
\end{equation}
可以验证，$\mat M\bvec e_x=-\bvec e_x,\mat M \bvec e_y=\bvec e_y$。因此这确实是一个保持向量在y方向的分量不变，x轴分量反向的轴反射。任意向量的模长不因轴反射而改变，因此任何图像的形状在反射后不变。如果我们想进一步把“轴反射”的概念拓展到高维，只需要满足两个性质：依然是保距的线性变换，且兼容轴变换。
