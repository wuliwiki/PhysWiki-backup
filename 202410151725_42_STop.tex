...
\textbf{证明：} 因为 $\bar{E}$ 是 $E$ 的完备化空间，所以对任一 $x\in \bar{E}$，存在 $E$ 中的收敛于 $x$ 的序列 $\{x_n\}$ （由\autoref{the_cauchy_2}，它是柯西序列）。设 $f$ 是 $E$ 上的线性连续泛函，则 $\{f(x_n)\}$ 是柯西数列，因而必收敛（实数集上的柯西序列必收敛），定义 $\bar f(x):=\lim\limits_{n\rightarrow\infty}f(x_n)$，则 $\bar f$ 就是 $f$ 在 $\bar E$ 上的连续延拓。该延拓是唯一的，否则 $\f$
