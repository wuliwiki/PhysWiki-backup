...
\item 3.3 假定 $\{s_n\}, \{t_n\}$ 是复数序列，且极限为 $s, t$ 那么 (a) $\lim_{n\to\infty} (s_n+t_n) = s+t$， (b) 对任何数 $c$， $\lim_{n\to\infty}cs_n = cs$； (c) $\lim_{n\to\infty}(s_n t_n) = st$； (d) $\lim_{n\to\infty} (c+s_n) = c+s$
