...
\item Julia 1.10 基于 \textbf{LLVM 15}： Julia 编译器先把 Julia 语言变为 \textbf{Julia IR} （和 LLVM IR 相似，而且有若干层）， 再变为 \enref{LLVM IR}{llvmIR}, 最后交给 LLVM 进行优化。
...
Expr <: Any
  ...
  args::Vector{Any}
\end{lstlisting}
\item 创建表达式用 \verb`Expr(头, 入参1, 入参2, ...)` 如 \verb`ex = Expr(:call, :+, 1, 2)`
\item 其中 \verb`头` 可以是 \verb`:call, :block, :if, :(=), :quote`
\item 当 \verb`Expr(...)` 无法打印出正常的表达式时，会直接打印 \verb`:($(Expr(...)))`。 这只是打印效果，并没有改变数据。
          ...
          args: Array{Any}((2,))
            ...
            2: Expr
              ...
              args: Array{Any}((2,))
                ...
                2: Symbol String
\end{lstlisting}
\item 什么是 \verb`QuoteNode`？ \verb`:(f(x))` 会得到
\begin{lstlisting}[language=julia]
Expr
  head: Symbol call
  args: Array{Any}((2,))
    ...
    2: Symbol x
\end{lstlisting}
而如果 \verb`g()` 函数接收 \verb`Symbol` 类型的入参，那么 \verb`:(g(:abc))` 就是
\begin{lstlisting}[language=julia]
Expr
  head: Symbol call
  args: Array{Any}((2,))
...
typed_ir = @code_typed add(1, 2) # 生成 typed Julia IR
# 高亮 <真·动态类型> 的 typed Julia IR（源码中标注 Union Any 未必是动态类型）
...
\item \verb`@code_lowered` 返回一个 \verb`Core.CodeInfo` 类型的结构体，最后经过一次 \verb`remove_linenums!(code)`，打印出来就是我们看到的文本格式 Julia IR，其 \verb`code` 字段（\verb`Vector{Any}`）就是 IR 内部的数据结构。
\item \verb`%数字` 并不是连续的，而是行号也就是 \verb`code[数字]`
\item 每一个 \verb`%数字` 的类型是
\begin{lstlisting}[language=julia]
struct Core.SSAValue <: Any
  id::Int64
\end{lstlisting}
...
\begin{itemize}
\item 返回 \verb`Pair{Core.CodeInfo, DataType}` 第二个是出参类型。
\item 之前 untyped 没有常量折叠，typed 有，以及死分支剪切，函数内联。所以看起来比较冗长（GPT 可以解释！）。
...
\item 还有一个 pi 节点 \verb`Core.PiNode`，有两个字段 \verb`val, typ`。 这是为了表示在当前分支中，一个变量确定为某种类型。 例如
\item \verb`Core.ReturnNode` 会被打印为 \verb`unreachable`
\begin{lstlisting}[language=julia]
1 ─ %1 = x isa Int
└── goto #3 if not %1
2 ─ %2 = π(x, Int64)
...
\item typed ir 中常见的 \verb`Core.Intrinsics` 模块中的 \verb`Core.IntrinsicFunction` 类（\verb`Core` 中定义的类）函数都有唯一的编号（例如 \verb`add_int (intrinsic function #2)`）这些函数在 \verb`Base` 模块中也放了一份： \verb`add_int (+), add_ptr (指针+偏移量) , and_int (按位与), bitcast (类型转换), checked_smul_int (有符号乘法，带溢出检查), eq_int (==), lshr_int (按位>>), lt_float (<), mul_float (*), not_int (按位非), or_int (按位或), sle_int (<=), slt_int (<), sub_int (-), trunc_int
(截取低位)， ule_int (<=), ult_int (<), xor_int (按位异或)`。
\item \verb`Base.bitcast(类, 值)` 类似于 C++ 的 \verb`reinterpret_cast`，把 \verb`值` 强制理解为指定的 \verb`类`。
\item \verb`Base.checked_smul_int(x, y)` 返回 \verb`Tuple{Int64, Bool}`，溢出时，第二个出参为 \verb`true`。
\item \verb`Base` 模块中 \verb`setfield!, getfield` 是 built-in function。
\item \verb`Expr(:foreigncall, ...)` 表达式，一般来说由 \verb`ccall()` 调用生成，见 \enref{Julia 调用 C 语言（笔记）}{JuliaC}。
\begin{lstlisting}[language=julia]
Expr(
    :foreigncall,
    fptr_or_symbol, # 函数指针或符号
    return_type, # 出参类型
    arg_types_svec, # 入参类型
    nreq, # 大部分情况同 length(arg_types_svec)，除非 C 函数是 var_arg，例如 printf()
    calling_convention, # `:ccall` 是最常见的，使用 C ABI。
                        # 其他的如 `:stdcall`（用于 32bit windows），
                        # `:llvmcall` （用于 llvm intrinsic call），罕见
    args...
)
\end{lstlisting}
\item 等效于 \verb`ccall((:函数名, 共享库路径), 出参类型, 入参类型表::Tuple, 入参1, 入参2, ...)`
\item Julia 函数调用有两种节点，一个是 \verb`Expr(:call, ...)` 另一个是 \verb`Expr(:invoke, ...)` 前者走多重派发，后者直接调用函数实例
\begin{lstlisting}[language=julia]
Expr(:invoke,
     mi::Core.MethodInstance,   # 函数实例
     f,                         # 函数对象（或一个可调用对象）
     arg1, arg2, ... )          # 实参（已经在 IR 里被整理成 SSA 值、常量、类型对象等）
\end{lstlisting}
\end{itemize}
...
\item 要编译 debug 版本，命令行用 \verb`make debug`（\href{https://docs.julialang.org/en/v1/devdocs/backtraces/}{文档}，好像加不加 \verb`-j` 都行）。 注意 make 的之前可能需要挂个梯子（OpenVPN 比较稳定）。remote 名字 \verb`origin` 不能改。
\item 编译说明见 \verb`doc/src/devdocs/build/build.md`：在 repo 根目录 \verb`echo JULIA_CPU_TARGET=generic > Make.user` 可以让编译结果适用于其他同架构 cpu。 \verb`USE_BINARYBUILDER=0` 不要用，会导致用 fortran 重新编译 openblas。
\item 下载完以后，可以备份一下，再次编译想要离线，就用 \verb`make clean`（不会删除下载文件），再 \verb`make -j debug`。
\item 编译不会在仓库中删除只会新增文件（删掉所有 \verb`.gitignore` 就能看到）。
\item 下载阶段新增文件夹：deps (175M) usr-staging (3.8M) usr (915M)，括号中的尺寸是编译后的。
\item 其他新增文件夹：julia (1.7GB)
\item 已有文件夹新增的文件（感觉都可以删掉）：
\item $\qquad$ base (jl, version_git_dirty/)
\item $\qquad$ cli (dbg.obj, expmap)
\item $\qquad$ deps
\item $\qquad$ src (o,dbg.obj,json,a,)
\item $\qquad$ stdlib (toml, <hashfiles>)
...
\end{lstlisting}
\item 最新的 v1.12.0 中 \verb`make debug` 失败，提了 \href{https://github.com/JuliaLang/julia/issues/57675}{issue} 正在修复。撤回到 v1.10.10 编译成功。 最新：20260107 的 e8bf6c。
...
\item 许多 \verb`jl_` 开头的函数和类都是 \enref{Julia C API}{JuCAPI} 中的，建议先熟悉这个，可以用于辅助学习 Julia 源码。
...
\begin{itemize}
\item \verb`jl_task_t` 是 Julia 的 Task， 也叫\textbf{协程（coroutines）}，也叫绿线程。 携程不是操作系统层面的线程，而是 julia runtime 实现的。
\item 每个 method 定义都会被分配一个单调递增的整数，称为其\textbf{世界编号（world number，或 world age）}。当你定义一个新方法时，全局的世界计数器会递增。
