...
电子层：从半径最小的轨道开始，用数字 1，2，3 (或K,L,M,...)等依次命名每条轨道，这些数字也叫主量子数，用 $n$ 来表示． 

电子亚层：一个电子层可以分为若干个电子亚层，以角量子数$l$计．角量子数从 0 增加到 $n-1$（或称$S,P,D,F...$ ）．即第 $n$ 个电子层有 $n$ 个电子亚层．把行标和列标组合起来， 就能得到任意一个格子的名称，例如第三行第二列的格子叫做 $3P$． 

亚层的轨道：一个电子亚层可以继续分为若干个轨道，以角量子数$m$计．对于角量子数 $l$，$m$ 可以取 $ - l, - l + 1...0,...l - 1,l$ 等 $2l + 1$ 个不同的值．因此，一个 
一个角量子数为$l$的电子亚层
接下来，每个格子又能装不同数目的电子，任意一格能装的电子数等于 $2(2l + 1)$． 最后， 每一个小格子里面能装两个电子． 总共算下来，第 $n$ 行刚好可以装 $2n^2$ 个电子（见\autoref{Ptable_fig2}）．
