...
\subsection{二次型的正定性}
二次型正定意味着$f(\textbf{v})> 0$，负定则意味着$f(\textbf{v})< 0$。将二次型化为对角矩阵的形式，那么可以很清楚地看到，正定意味着对角元大于0，反之则小于0.
实际上，在\textbf{实数域}内做合同变换并不改变对角元的正负性，比如列变换表示第$n$列乘以系数$k$时，合同变换里结合相应的行变换使得第$n$个对角元乘以$k^2$