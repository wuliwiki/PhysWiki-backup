...
* **I – 反三角函数（包括双曲函数的反函数）**：$\arctan(x),\ \operatorname{arcsec}(x),\ \operatorname{arsinh}(x)$ 等
* **A – 代数函数（如多项式）**：$x^{2},\ 3x^{50}$ 等
* **T – 三角函数（包括双曲函数）**：$\sin(x),\ \tan(x),\ \operatorname{sech}(x)$ 等
* **E – 指数函数**：$e^{x},\ 19^{x}$ 等
\end{itemize}

而作为 $dv$ 的函数则是列表中**排在最后的函数**。原因是：列表越靠后的函数通常具有比前面函数更简单的原函数。

该规则有时也写作 **“DETAIL”**，其中 **D** 代表 $dv$，即取列表顶部的函数作为 $dv$。

另一个变体是 **ILATE 法则**，它与 LIATE 的区别在于：**反三角函数排在对数函数之前**。
