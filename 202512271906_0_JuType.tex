...
\item \verb`getproperty(a, b)` 相当于算符 \verb`a.b`，会根据 \verb`a, b` 的类型调用 \verb`getfield(::Type, ::Symbol)`（结构体获取字段）， \verb`getfield(::Tuple, ::Int)`（Tuple 获取元素），\verb`getfield()`\verb`getglobal(::Module, ::Symbol)`（从模块获取符号）
