...
考虑单qubit的密度矩阵的好处是此时密度矩阵可以写成\autoref{eq_Qubit_2}~\upref{Qubit}$\rho = \frac{1}{2}I + \frac{1}{2}\vec{n}\cdot \vec{\sigma}$。
...
&= \frac{1}{4}\opn{tr}\abs{\left(\vec{n}_1 - \vec{n}_2\right) \cdot \vec{\sigma}}~,
\end{aligned}~
\end{equation}
考虑$\left(\vec{n}_1 - \vec{n}_2\right) \cdot \vec{\sigma}$的本征值是$\pm \abs{\vec{n}_1 - \vec{n}_2}$，则：
\begin{equation}
\begin{aligned}
D\left(\rho,~\delta\right) 
&= \frac{1}{4}\opn{tr}\abs{\left(\vec{n}_1 - \vec{n}_2\right) \cdot \vec{\sigma}} \\ 
&= \frac{1}{2}\abs{\vec{n}_1 - \vec{n}_2}~.
\end{aligned}~
\end{equation}
