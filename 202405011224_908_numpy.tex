...
创建一个二维数组，如（\enref{默认是行主序}{MatSto}）
\begin{lstlisting}[language=python]
...
修改数组的shape属性，在保持数组元素个数不变的情况下，改变数组每个轴的长度。下面的例子将数组 \verb|c| 的shape改为 \verb|(4,3)|，注意从 \verb|(3,4)| 改为 \verb|(4,3)| 并\textbf{不是对\enref{数组进行转置}{Mat}}，而只是改变每个轴的大小，数组元素在内存中的位置并没有改变：
...
\subsubsection{矩阵的操作}
下面分别介绍对矩阵\upref{Mat}进行一些常见的操作处理，包括转置，拼接，翻转，\enref{内积}{Dot}，外积，\enref{计算本征值}{MatEig}，\enref{行列式}{Deter}，\enref{逆矩阵}{InvMat}，等等在python中的实现。
\begin{lstlisting}[language=python]
A = [1, 2, 3]
B = [[2, 1, 4],[2, 2, 3],[1, 1, 3]]
C = [4, 5, 6]
