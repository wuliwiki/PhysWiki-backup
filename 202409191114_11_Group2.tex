...
\begin{enumerate}
\item 若$B$是$G_1$的子群，则$f(B)$是$G_1$的子群。反之，若$K$是$G_2$的子群，则$f^{-1}(K)$是$G_1$的子群\textbf{且$f^{-1}(K)\supseteq N$}。
...
\textbf{证明：}
\begin{enumerate}
\item 设任意$x,y\in B$，则$f(x)f(y)^{-1}=f(xy^{-1})$。又因为$xy^{-1}\in B$，所以$f(xy^{-1})\in f(H)$。
\end{enumerate}
