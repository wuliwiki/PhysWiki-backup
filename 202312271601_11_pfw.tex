...
设$(U,\phi),(V,\psi)$是$M$上相容的图。$\{\widetilde e_i\}$与$\{\widetilde \theta_i\}$分别是$U$和$V$上切空间的基矢，且图上的坐标分别为$\phi(p)=(x_1,x_2...),\psi(p)=(y_1,y_2...y_n)$。则$\psi\circ\phi^{-1}:\phi(U\cap V)\rightarrow\psi(U\cap V)$是光滑的。又设\textbf{该转移映射的坐标表示}为
\begin{equation}
\psi\circ \phi^{-1}(x)=(y'_1,y'_2...y'_n)~.
\end{equation}
对于交集部分，我们有：$\widetilde e$