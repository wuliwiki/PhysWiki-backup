...
\subsubsection{性质}
下面的性质都是可以从定义中直接推知的，因此不加证明地给出。由于交集和并集是\textbf{对称运算}，他们在性质上也互相：
\begin{table}[ht]
\centering
...
\hline
3 & $A\cap A = A$ & $A\cup A = A$ & 幂等性 \\
\hline
4 & $A\cap \varnothing = \varnothing$ & $A\cup \varnothing = A$ & 与空集的关系 \\
\hline
5 & $ A \cap (B \cap C) = (A \cap B) \cap C$  &$ A \cup (B \cup C) = (A \cup B) \cup C$ & 结合律 \\
\hline
6 & $ A \cap (B \cap C) = (A \cap B) \cap C$  & $ A \cup (B \cap C) = (A \cup B) \cap (A \cup C) $ & 结合律 \\
\hline
7 & $ A \cap (B \cap C) = (A \cap B) \cap C$  &$ A \cup (B \cup C) = (A \cup B) \cup C$ & 结合律 \\
\hline
