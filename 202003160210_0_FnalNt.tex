...
\begin{itemize}
\item 令 $X := L_2^{\mathbb C}(\mathbb R)$， $A:D(A) \subseteq X\to X$ 为对称算符， $\mathcal S\subseteq D(A)$ 那么非零的 tempered distribution $T \in\mathcal S'$ 叫做 $A$ 的 \textbf{广义本征函数（generalized eigenfunction）} （具有实数本征值） 当且仅当 $T(A\phi) = \lambda T(\phi)$
\end{itemize}
