...
\item 如果上传时发现图片文件的 hash 已存在于 \verb|images.hash|， 则检查该文件的 \verb|images.figures| 中 \verb|figures.deleted==0| 的图片环境， 有就提示图片已存在，列出所有标签（\verb|fig_| 加 \verb|figures.id|）和它们所在的词条 id 和 caption 让用户选择引用（\verb|\autoref{}|）或者重复使用图片。 如果 \verb|images.figures| 中没有 \verb|figures.deleted==0| 的图片环境（就不用选了）或者用户选择重复使用图片， 就检查 \verb|images.figures| 中 \verb|figures.deleted==1| 的环境中有没有属于当前词条的， 如果有就更新 \verb|figures.deleted=0| 然后根据该记录中的信息插入图片环境代码，否则新建标签和环境，并在 \verb|figures| 表中插入新的记录， \verb|aka| 指向 \verb|images.figures或figures_old| 中 \verb|aka| 为空的 figure（有且仅有一个，否则是bug）。
