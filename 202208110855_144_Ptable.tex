...
电子亚层：对于主量子数为 $n$ 的行，又有额外的电子亚层，以角量子数计．角量子数从 0 增加到 $n-1$，也就是说第 $n$ 行有 $n$ 个格子．
在上图中，每条轨道从第一列开始依次用 $S,P,D,F...$ 命名，每一列有相同的角量子数，用 $l$ 来表示．  $S,P,D,F\dots$ 的角量子数依次是 $l = 0,1,2,3\dots$． 把行标和列标组合起来， 就能得到任意一个格子的名称，例如第三行第二列的格子叫做 $3P$． 
