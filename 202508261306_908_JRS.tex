...
上述曲线绕某点的例子有一个简单的拓扑解释：平面去掉一点后的空间与一个圆 $S^1$ 是同伦等价的，因此从圆到自身的映射是需要重点考虑的对象。可以证明，每一个这样的映射都可以连续变形（同伦）为某个标准映射：$S^1 \;\to\; S^1: \; s \;\mapsto\; s^n$,其中，圆上的乘法是通过将其识别为复平面上的单位圆来定义的。

从一个圆映射到某个拓扑空间的所有同伦类组成一个群，这个群被称为该空间的**第一同伦群（fundamental group 或 $\pi_1$）。圆的第一同伦群是整数群 $\mathbb{Z}$，而复平面上一条曲线的绕数，实际上就是它对应的同伦类。
