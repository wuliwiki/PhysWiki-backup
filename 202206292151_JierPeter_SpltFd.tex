...
给定$\mathbb{F}_1$作为$\mathbb{F}$上线性空间的一组基$\{\nu_i\}\subseteq\{\alpha_j^k\}$，并任挑一个$\alpha_i$来构造保$\mathbb{F}$单同态$\sigma: \mathbb{F}(\alpha)\to\mathbb{F}_1$，其中$\sigma(\alpha_1)=\alpha_i$．则根据\autoref{FldExp_the5}~\upref{FldExp}的证明过程，可知总能\textbf{唯一地}把$\sigma$开拓为与$\{\nu_i\}$关联的保$\mathbb{F}$自同构$\sigma: \mathbb{F}_1\to\mathbb{F}_1$．yush

由\autoref{FldExp_the3}~\upref{FldExp}，$[\mathbb{F}_1:\mathbb{F}]=[\mathbb{F}_1:\mathbb{F}(\alpha_1)][\mathbb{F}(\alpha_1):\mathbb{F}]$．
