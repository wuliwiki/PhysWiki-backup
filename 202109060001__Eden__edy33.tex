...
\label{edy33_eq7}
\end{equation}
其中 $\dd{\bvec x}^{\prime}=\dd l^{\prime}$ 是因为 $ \bvec x^{\prime}$ 为闭合流管上的坐标．为了进一步计算，我们利用：
\begin{equation}
\oint_L d(\bvec x_i \bvec x'_i \bvec x'_j)=0 \label{edy33_eq8_0}
\end{equation}
\begin{equation}
\oint_L \dd{(\bvec x^{\prime}\cdot \bvec x)\bvec x^{\prime}}=\oint_L (\bvec x^{\prime}\cdot\bvec x)\dd l^{\prime}+\oint_l (\dd l^{\prime}\cdot \bvec x)\bvec x^{\prime}\label{edy33_eq8}
\end{equation}
所以可以推出
\begin{equation}
