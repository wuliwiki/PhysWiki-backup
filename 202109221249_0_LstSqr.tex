...
上式存在的一个问题是， 各个 $c_i$ 偏导为零， 就一定会是全局最小值吗？ 会不会是局部最小值，最大值，甚至鞍点呢？ 这就涉及到线性方程组解的结构\upref{LinEq}， 若线性方程组存在多余一个解， 那么这些解必定不可能是分立的而是连续的， 具体来说会构成一个线性子空间 $V$， 向量 $(c_2, c_1, c_0) \in V$， 在整个子空间中，  由于各阶偏导处处为零， $S_2$ 必定处处相等， 所以只可能都是全局最小值．
