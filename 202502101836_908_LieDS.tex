...
\]
李代数 \( \mathfrak{g} \) 的李括号由矩阵的交换子给出：

\[
[X, Y] = XY - YX.
\]

给定李代数 \( \mathfrak{g} \subset \mathfrak{gl}(n, \mathbb{R}) \)，可以通过矩阵指数运算从 \( \mathfrak{g} \) 中的元素恢复李群。具体地说，若 \( G \) 是连通的，那么这个过程给出了 \( G \) 的单位成分。这里的指数映射
...
相同的评论也适用于复数李子群 \( \mathrm{GL}(n, \mathbb{C}) \) 和复矩阵指数：
