...
\textbf{6.(15 分)}设平面转子的哈密顿量为 $\hat{H} = \frac{l_z^2}{2I}$, ($l_z = -i\hbar \frac{\partial}{\partial \varphi}$)，试求归一化的能量本征函数、能量本征值、能级简并度。

\textbf{7.(15 分)}设量子体系的哈密顿量为 $\hat{H}$，力学量算符 $\hat{A}$ 不显含时间 $t$，$t$ 时刻的量子态为 $\psi(t)$，$\hat{A}$ 的平均值为 $A(t)$，求证：
\[
i\hbar \frac{dA(t)}{dt} = [A, H]~
\]
