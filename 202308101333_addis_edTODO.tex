...
\item 如果上传时发现图片文件的 hash 已存在于 \verb|images.hash|， 则检查 \verb|images.figures| 中 \verb|figures.aka| 为空的图片环境（只有一个或者没有，否则就是 bug）。 如果有且未被标记 \verb|figures.deleted|， 有就提示已存在， 是否引用（\verb|\autoref{}|）（推荐）或者创建新图片环境。 若有多个就把标签全部列出。 如果没有， 或者用户选择创建新环境， 就检查 \verb|images.figures| 中被标记 \verb|figures.deleted| 的环境有没有原本属于当前词条的， 如果有就更新 \verb|figures.deleted=0| 然后根据该记录中的信息插入图片环境代码。
