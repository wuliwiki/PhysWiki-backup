...
这是一个二阶线性常微分方程． 数学上来看无论 $E$ 是多少， 必有两个线性无关的解， 它们的线性组合也是解（二维解空间）． 然而物理上可能存在的波函数必须要可归一化： 即随着 $\abs{x} \to \infty$，  $\abs{\psi}^2$ 下降得比 $1/x^2$ 要快， 所以只有对于一些离散的 $E$ 我们才能解出这些波函数． 我们把每个 $E$ 叫做\textbf{能级（energy level）}， 对应的波函数叫做\textbf{束缚态（bound state）}．

可以证明只有在能量区间
\begin{equation}
\min V(x) < E < \min V(\pm\infty)
\end{equation}
才存在束缚态： 当 $E$ 太小时， 波函数必定会在无穷远处爆炸（无穷大）， 而当 $E$ 太大时， 又不拿呢． 这就要求势能函数中存在某种形状的势阱．
