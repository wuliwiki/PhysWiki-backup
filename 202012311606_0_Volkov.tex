...
\begin{equation}
\Psi_{\bvec k}^L(\bvec r, t) = (2\pi)^{-3/2} \exp\qty[\bvec p_V(t)\vdot \bvec r - \frac{1}{2}\int_{-\infty}^t \bvec p_V(t')^2 \dd{t'}]
\end{equation}
其中 $\bvec p_C(t)$ 是库仑规范或速度或中的广义动量（\autoref{QMEM_eq6}~\upref{QMEM}）
\begin{equation}
\bvec p_V(t) = \bvec k + q\bvec A(t)
\end{equation}
\addTODO{长度规范下 $\bvec A = \bvec 0$， 这里的 $\bvec A$ 应该是库仑/速度规范下的？}
