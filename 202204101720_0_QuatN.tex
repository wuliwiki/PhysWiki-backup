...
\begin{equation}
q= (\cos{\frac{\theta}{2}}, {\uvec{n}}\sin{\frac{\theta}{2}})
\end{equation}
那么旋转后的向量就可以表示为
\begin{equation}
q\bvec vq^{-1}
...
比如说，取 $\bvec v = (0, 1, 0, 0)$，它代表一个 $x$ 轴上的单位向量．如果想要把它绕着 $z$ 轴上的单位向量转 $\pi/2$，那么结果向量的四元数表示应该是 $(0, 0, 1, 0)$．按照我们定义的规则，旋转表示为四元数 $q=(\sqrt{2}/2, 0, 0, \sqrt{2}/2)$，按照四元数的乘法规则易得 $q\bvec v q^{-1}=(0,0,1,0)$．
