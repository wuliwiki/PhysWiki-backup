...
如果把系统看作质点组， 每个质点的位置可以由几个独立的\textbf{广义坐标（generalized coordinates）} $q_i$ $(i=1,2\dots,N)$ 和时间 $t$ 的函数确定， 那么整个系统随时间的变化就可以用函数 $q_i(t)$ 描述． 把 $q_i(t)$ 对时间的导数记为 $\dot q_i$， 则每个 $q_i(t)$ 可通过\textbf{拉格朗日方程（Lagrange equation）}\footnote{也叫\textbf{欧拉-拉格朗日方程（Euler-Lagrange Equation）}}\footnote{方程中对 $\dot q$ 求偏导\upref{ParDer}可能会使初学者看起来有些奇怪不知道该怎么求， 这里明确一下该偏导在数学上是如何操作的： 可将\autoref{Lagrng_eq2} 中 $L$ 的所有 $\dot q_i$ 记为 $v_i$， 然后将 $q_i$ 和 $v_i$ 看作是完全没有关联的变量， 求偏导时对 $q_1,\dots, q_N, v_1, v_2\dots,t$ 中的一个求， 而保持其他量不变．}解出：
