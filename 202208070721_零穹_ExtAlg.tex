...
交错化映射 $A$ 将任意张量映射为斜对称张量（\autoref{SIofTe_the1}~\upref{SIofTe}），而 $\Lambda(V)$ 的子空间是斜对称子空间，因此有可能通过 $A$ 定义一种乘法，使得 $\Lambda(V)$ 在该乘法下封闭．注意 $ \Lambda(V)$ 上的任意元素 $Q$ 可记作（\autoref{TenAlg_eq5}~\upref{TenAlg} ）
\begin{equation}
