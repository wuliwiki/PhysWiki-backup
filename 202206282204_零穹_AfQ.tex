...
\end{example}
\begin{theorem}{}
所以保持点 $\dot o$ 不动的仿射自同构构成的集合是一个子群 $A_n(\mathbb F)_{\dot o}\in A_n(\mathbb F)$，且同构于完全线性群 $GL(V)=GL_n(\mathbb F)$（\autoref{Group_ex5}~\upref{Group}）．空间 $\mathbb A$ 的所有平移构成的子群 $T=\{t_v|v\in V\}$ 在群 $A_n(\mathbb F)$ 中是正规的（\autoref{NormSG_def1}~\upref{NormSG}）．
\end{theorem}
\textbf{证明：}先证明定理第1部分：
