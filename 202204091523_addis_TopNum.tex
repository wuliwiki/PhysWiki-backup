...
\begin{equation}
\bvec\omega = \dot\psi\uvec r + \dot\theta\uvec\phi + \dot\phi\uvec z = (\dot\psi + \dot\phi\cos\theta)\uvec r - \dot\phi\sin\theta\ \uvec\theta + \dot\theta\uvec\phi
\end{equation}
其中使用了 $\uvec z = \cos\theta\ \uvec r - \sin\theta\ \uvec\theta$． 于是角动量为
\begin{equation}
\bvec L = L_1(\dot\psi + \dot\phi\cos\theta)\uvec r - L_2\dot\phi\sin\theta\ \uvec\theta + L_2\dot\theta\uvec\phi
\end{equation}
动能为
\begin{equation}
T = \bvec\omega\vdot\bvec L = L_1(\dot\psi + \dot\phi\cos\theta)^2 + L_2(\dot\phi^2\sin^2\theta + \dot\theta^2)
\end{equation}
