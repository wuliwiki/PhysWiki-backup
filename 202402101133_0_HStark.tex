...
\begin{example}{氢原子斯塔克效应（能级截断）}
事实上，在\autoref{ex_HStark_1} 中使用\autoref{eq_HStark_2} 计算二阶微扰十分繁琐，通常需要编程来数值计算。 除了按不同阶来计算微扰外，还有一个更直接的近似方法就是直接把总哈密顿 $H = H^0+H^1$ 用有限个 $\ket{n,l,m}$ （可以用 $n_\text{max}$ 作为截断条件）完整表示（例如\autoref{tab_HDipM_1}~\upref{HDipM}），然后数值将其对角化。 当 $n_\text{max}$ 无穷大，结果就是精确的（包含任意阶微扰）。
\end{example}
