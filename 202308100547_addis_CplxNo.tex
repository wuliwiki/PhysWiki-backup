...
\subsection{复平面}
\pentry{几何矢量的运算\upref{GVecOp}， 四象限 Arctan 函数\upref{Arctan}}
\begin{figure}[ht]
\centering
...
\end{definition}
\textbf{}为（$x_i,y_i$ 分别是 $z_i$ 的实部和虚部）
...
其中最后一步用到了两角和公式（\autoref{eq_TriEqv_1}~\upref{TriEqv}）。 容易看出， 最后得到的是一个模为 $A_1 A_2$， 幅角为 $\theta_1 + \theta_2$ 的复数。 \textbf{证毕}。
