...
\item 如果要强制设置 \verb|rpath| （不推荐）而不是 \verb|runpath|， 先用 \verb|patchelf --remove-rpath 文件| 移出已有的 \verb|runpath| 或 \verb|rpath|， 然后 \verb|patchelf --force-rpath --set-rpath '路径' 文件| 即可．
\item \verb|patchelf --print-needed 文件| 可以显示直接依赖的动态库（不包含依赖的依赖）． 这应该和 \verb|lddtree| 的第一层是一样的．
