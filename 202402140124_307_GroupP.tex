...
我们给每个珍珠从$1$到$n$编号。设$F=\{f_1,...,f_k\}$是不同颜色的集合，$X=F^n$是在固定顺序下，手环的所有颜色排列方案。显然在顺序固定情况下有$k^n$种可能性，对应的是$|X|=k^n$。我们现在要做的，是把所有旋转或翻转可以重合的情况，只保留记作一种。这等价于二面体群$D_{2n}$作用在集合$X$上。对于任意元素$x_1,x_2\in X$，$x_1$在$D_{2n}$作用下得到$x_2$，等价于$x_1,x_2$位于同一个轨道。而我们想求的曲重复的不同排列数，也正是轨道的数量。
...
对于$n$种旋转的情况(我们把保持不变的操作算作旋转$0$次，当然也可以看成旋转$n$次，即$r=0$和$r=n$表示同一种操作)。易见，当旋转一次时，当且仅当所有珍珠为同一颜色时，旋转后的颜色才会和旋转前重合，这对应着$n$个不动点，对于一般情况$d^r$，即旋转$r$次，$1\le r\le n$，令$m=\gcd(n,r)$。那么有$k^m$种可能，随意选择前$m$颗珍珠的颜色，之后按照相同的样式，重复$n/m$次刚好填满全部珍珠。于是我们得到旋转$r$次时，不动点的个数为：$k^{\gcd(n,r)}$
