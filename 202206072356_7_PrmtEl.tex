...
显然，由于$x^p$的$p$次方根不在$\mathbb{Z}_p(x^p, y^p)$中，根据\autoref{SprbEx_the4}~\upref{SprbEx}，$\mathbb{Z}_p(x^p, y^p)$不是完备域．这就提醒我们，这个扩张可能不是可分的．
...
最后要处理的还是关于可分扩张的一个性质．首先，考虑下面两个引理．

\begin{lemma}{}
设$\mathbb{F}$是特征为素数$p$的域．

$\mathbb{F}$的单代数扩张$\mathbb{F}(a)$为可分扩张$\iff$ $\mathbb{F}(a)=\mathbb{F}(a^p)$．
\end{lemma}

\textbf{证明}：

设$a$在$\mathbb{F}$上的最小多项式为$f(x)=\opn{Irr}(a, \mathbb{F})=\sum_{i=0}^n c_ix^i$，$a^p$在$\mathbb{F}$上的最小多项式为$g(x)=\opn{Irr}(a^p, \mathbb{F})=\sum_{i=0}^n b_ix^i$．

则$g(x^p)$是$a$的一个零化多项式，故$f(x)\mid g(x^p)$．
