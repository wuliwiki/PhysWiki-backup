...
\subsection{基本定义}
无穷维空间里，本征值、谱和正则点的基本概念是一样的。为正式起见，这里给出基本的定义。一般线性拓扑空间中第三种可能的存在将导致一些本质性区别。
\begin{definition}{本征值，谱，正则点，预解式}
设 $A$ 是任意线性拓扑空间中的线性算子（定义空间和象空间一样），如果方程
\begin{equation}
Ax=\lambda x~
\end{equation}
有非零（矢量）解，则数 $\lambda$ 称为算子 $A$ 的\textbf{本征值}。所有本征值的总体称为算子的\textbf{谱}。而其它的 $\lambda$ 值称为\textbf{正则点}。若算子 $(A-\lambda I)^{-1}$ 存在，则称其为算子 $A$ 的\textbf{预解式}。若预jie'shi
\end{definition}
