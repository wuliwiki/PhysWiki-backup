...
由前面的讨论，单扩张$\mathbb{F}(\alpha_1)$的保$\mathbb{F}$自同构数量，等于$\mathbb{F}(\alpha_1)$中所包含的$h$的根的数量，设该数量为$k$．则$h$剩下的$m-k$个根，组成了$\mathbb{F}(\alpha_1)$上的不可约多项式$h'$，且其次数小于$n$．于是按照归纳假设，$\mathbb{F}_1$的保$\mathbb{F}(\alpha_1)$自同构数量小于等于$[\mathbb{F}_1:\mathbb{F}(\alpha_1)]$，当且仅当$h_1$无重根时等号成立．

由\autoref{FldExp_the3}~\upref{FldExp}，$[\mathbb{F}_1:\mathbb{F}]=[\mathbb{F}_1:\mathbb{F}(\alpha_1)][\mathbb{F}(\alpha_1):\mathbb{F}]$．而$\mathbb{F}_1$的保$\mathbb{F}$自同构，都是$\mathbb{F}(\alpha_1)$的保$\mathbb{F}$自同构和$\mathbb{F}_1$的保$\mathbb{F}(\alpha_1)$自同构结合而得的，故
