...
其中函数 $\phi$ 有界． 当 $s$ 很大时， 邻域 $B(x_0,\delta)$ 以外的积分的贡献比 $s$ 的任何负幂衰减得都快：根据假定 2，当 $x\in I\setminus B(x_0,\delta)$ 时, 存在正数 $\varepsilon_0_0>0$ 使得 $f(x)<f(x_0)-\varepsilon_0$， 从而邻域 $B(x_0,\delta)$ 之外的积分估计为
\[
...
\tilde{\psi}(y):=\psi(x(y))\frac{\dd}{\dd x}[x(y)]\sim c_0+c_1y+\cdots \quad y\to 0
\end{equation}
