...
\begin{definition}{乘方运算}
形如
\begin{equation}
a^b=c~.
\end{equation}
的运算称为\textbf{乘方运算（exponentiation，或幂运算）}，其中a称为\textbf{底数（base）}，b称为\textbf{指数（exponent）}，c称为\textbf{幂值（result）}
\end{definition}

下面是一些乘方运算的法则，如果不了解原理，直接默认其成立即可：
\begin{itemize}
\item 积的幂：$(a \cdot b)^n = a^n \cdot b^n$
\item 分数次幂：$a^{\frac{m}{n}} = \sqrt[n]{a^m}=(\sqrt[n]{a})^m, \quad a \geq 0$
