...
G = \pm \sqrt{ab}~.
\end{equation}
这说明，对于给定的$a,b$，他们的等比中项并不唯一，分别对应着$q$为相反数的两种情况。而$G$与$a,b$同号的值正好是$a$ 与 $b$的\textbf{几何平均值（geometric mean）} \footnote{关于几何平均值参见\aref{基本不等式}{sub_HsIden_2}。几何平均值的英语也和等比数列的英语使用了一样的单词 “geometric”。}。这里可以看成有两种方法来把这些离散的点连起来。但相对简单的，即单调的那个函数，更（补充）。
