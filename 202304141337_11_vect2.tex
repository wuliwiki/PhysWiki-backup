...
\partial_\mu J_\mu=0
\end{equation}
显然，这是一个洛伦兹协变方程。但这样的扩展到底是否可行呢？我们需要检验电流密度是否满足洛伦兹变换。设带电粒子速度为$v$,$\rho_0$为该区域的电荷密度（以粒子为参考系），则$\vec J=\rho \vec v$。由于转换惯性系时，线元仅在boost的方向上收缩，那么体积元的变化为$dV'=\frac{1}{\gamma}dV$，则$\rho=\gamma\rho_0$，因此四维矢量为$(\rho,\gamma\rho_0\vec)$，
\subsubsection{矢势}
\subsection{拓展：洛伦兹张量}
