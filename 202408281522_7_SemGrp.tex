...
给定群 $G$，如果有 $n+1$ 个群 $\{G_i\}_{i = 1}^n$，满足 $\{e\} = G_0 \triangleleft G_1 \triangleleft \cdots \triangleleft G_n = G$，即满足 $G_{i}$ 都是 $G_{i+1}$ 的正规子群，称之为$G$的一个\textbf{次正规序列（subnormal series）}，$G_{i+1} / G_{i}$ 被称为该序列的\textbf{因子群（factor group）}， $n$被称为该序列的\textbf{长度（length）}。
\end{definition}

我们有 $G_{i + 1} = G_{i} \rtimes (G_{i+1} / G_{i})$.

\addTODO{factor group的翻译待定}
%注释原因：已修正为“次正规序列”，参考朱富海《抽象代数》的称呼
...
类比到非负整数的质因数分解，合成序列就好比是每次只拿走一个质因子（单群）。非负整数的“合成序列”不一定唯一，但是所有“合成序列”的长度一定是一样的。群的合成序列也类似，合成序列不唯一，但其长度必然是一样的。这就是以下Jordan-Hölder定理在群论中的情形：

\begin{theorem}{Jordan-Hölder定理（有限群）}
