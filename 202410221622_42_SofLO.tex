...
设 $A$ 是任意线性拓扑空间中的线性算子（定义空间和象空间一样），则称 $(A-\lambda I)^{-1}$ 为算子 $A$ 的\textbf{预解式}，使 $Ax=\lambda x$ 有非零矢量解的数 $\lambda$ 称为算子 $A$ 的本征值。若 $A$ 的预解式 $(A-\lambda I)^{-1}$ 存在且有界、并定义在整个空间上，则称 $\lambda$ 对 $A$ 是\textbf{正则的}。所有其余的值 $\lambda$ 称为 $A$ 的\textbf{谱}。
\end{definition}
...
\textbf{证明：}若 $\lambda$ 时本征值，则预解式不存在，即 $\lambda$ 不对 $A$ 正则，因而属于谱。
