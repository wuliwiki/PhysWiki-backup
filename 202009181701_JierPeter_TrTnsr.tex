...
\begin{equation}
T'_{i}=f(\bvec{e}'_i)=f(a_{ji}\bvec{e}_j)=a_{ji}f(\bvec{e}_j)
\end{equation}
...
给定线性空间$V_1$和$V_2$，其中$\opn{dim}V_1=\opn{dim}V_2=k$．$f:V_1\times V_2\rightarrow\mathbb{F}$是一个二阶张量．
...
\begin{equation}
T'_{ij}=f(\bvec{a}'_i, \bvec{b}'_j)=f(m_{ki}\bvec{a}_i, n_{lj}\bvec{b}_j)=m_{ki}n_{lj}f(\bvec{a}_)
\end{equation}
