...
\item typed ir 中常见的 \verb`Base` 和 \verb`Core.Intrinsics` 模块的 \verb`Core.IntrinsicFunction` 类（\verb`Core` 中定义的类）函数都有唯一的编号（例如 \verb`add_int (intrinsic function #2)`） \verb`add_int, sub_int, or_int, not_int, xor_int, mul_float, lt_float, sle_int`。
\item  中定义的函数有 \verb`checked_smul_int, ule_int, or_int`
