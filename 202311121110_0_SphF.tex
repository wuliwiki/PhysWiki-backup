...
\footnote{参考 Wikipedia \href{https://en.wikipedia.org/wiki/Shell_theorem}{相关词条}。}一般来说，若我们要把两个物体看成质点来去计算它们之间的万有引力（\autoref{eq_Gravty_1}~\upref{Gravty}），前提需要是这两个物体之间的距离远大于它们的大小。但事实证明在经典力学中，对于一个满足一定条件的球体，在计算它对任何其他物体的引力时无论两个物体的远近都可以把该球体等效为球心处质量相同的质点。

\textbf{壳层定理（shell theorem）}指的是在经典力学中，计算一个球对称质量分布的球壳形物体产生的引力场时， 我可以将其做以下等效。 令该物体总质量为 $M$， 半径为 $R$， 要计算 $\bvec r$ 处的引力场。
\begin{itemize}
