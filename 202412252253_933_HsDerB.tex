...
而对$\Delta x\to 0$的情况下，根据\autoref{def_HsDerB_1} ，可知这时有$\Delta t$也趋于0\footnote{否则导数值会变成无穷大。}，而且。于是有：

\begin{equation}
\begin{split}
\left[f(g(x))\right]'&=\lim_{\Delta x\to 0}\frac{f(g(x+\Delta x))-f(g(x))}{\Delta x}\\
&=\lim_{\Delta x\to 0}\frac{f(g(x+\Delta x))-f(t)}{\Delta t}\cdot\frac{\Delta t}{\Delta x}\\
&=\lim_{\Delta x\to 0}\frac{f(t+\Delta t)-f(t)}{\Delta t}\cdot\frac{g(x+\Delta x)-g(x)}{\Delta x}\\
&=f'(x)g(x)+f(x) g'(x)~.
\end{split}
\end{equation}
