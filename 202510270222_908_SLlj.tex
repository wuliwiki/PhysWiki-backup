...
自动定理证明与逻辑编程（Automated Theorem Proving and Logic Programming）计算机科学反过来也对数学产生了重要影响。它发展出用于自动验证甚至自动发现数学证明的技术，如自动定理证明（automated theorem proving）与逻辑编程（logic programming）。  
这些技术不仅促进了数学形式化验证的发展，也构成了人工智能和符号计算的重要基础。

\paragraph{描述性复杂性理论（Descriptive Complexity Theory）}
\textbf{描述性复杂性理论}（descriptive complexity theory）研究\textbf{逻辑系统与计算复杂性}（computational complexity）之间的联系。  
这一领域的首个重要成果是\textbf{法金定理}（Fagin’s theorem, 1974），  
该定理指出：
\[
\mathbf{NP} \text{ precisely equals the class of languages expressible by existential second-order logic.}
\]
换言之，\textbf{NP 类问题}正好对应于\textbf{存在二阶逻辑}（existential second-order logic）所能表达的语言集合。
