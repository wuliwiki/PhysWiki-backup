...
\[
\frac{Z_i(N)}{Z_i(N-1)} = e^{-\mu / k_B T}.~
\]
将上述代入 \( \bar{n}_i \) 的方程，并使用先前定义的 \( \beta \) 来将 \( \frac{1/k_B T} \) 替换为 \( \beta \)，得到费米–狄拉克分布：
\[
\bar{n}_i = \frac{1}{e^{(\varepsilon_i - \mu)/k_B T} + 1}.~
\]
像麦克斯韦–玻尔兹曼分布和玻色–爱因斯坦分布一样，费米–狄拉克分布也可以通过达尔文–福勒的均值法推导出来。[29]