...
不要误以为这条式子是说当 $n = \infty$ 时， $a_n=\pi$． 有两个理由可以说明这种理解不正确：首先，按定义，每个$a_n$都是有理数，而$\pi$是无理数，所以不应该有任何一个$a_n=\pi$；其次，$\infty$不是一个整数或实数，不存在 $n=\infty$ 的说法． 这里的$n\to\infty$只是表示 $n$ 的不断增加的过程．


，而要理解成数列 $a_n$ 经过算符 $\lim\limits_{n \to \infty }$ 的作用以后，得出其极限是 $\pi$． 类比函数 $\sin x = y$，并不是说 $x=y$， 而是说 $x$ 经过正弦函数作用后等于 $y$． 
