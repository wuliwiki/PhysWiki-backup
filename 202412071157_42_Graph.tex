...
若有图 $H=(V(H),E(H),\varphi_H)$，使得 $V(H)=V(G),E(H)\subset E(G),\varphi_H=\varphi|_{E(H)}$（其中 $\varphi|_{E(H)}$ 是 $\varphi$ 在 $E(H)$ 上的\aref{限制}{def_map_3}），则称 $H$ 是 $G$ 的\textbf{支撑子图}（spanning subgraph）或生成子图。
\end{definition}
