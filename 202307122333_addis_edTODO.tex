...
\subsubsection{图片相关}
\begin{itemize}
\item 上传新的图片时， \verb|PhysWiki/figures/| 中的文件使用 \verb|images.hash| + 拓展名命名。 \verb|\label{}| 格式为 \verb|fig_词条名_序号|。这里的序号可以由小到大逐个查看数据库中存不存在 \verb|figures.id=词条名_序号|，使用。
\item 插入信息到 \verb|images| 中的 \verb|hash, ext, figures|，再插入 \verb|figures| 表格的 \verb|id,entry,image|， 再插入 \verb|entries| 表格的 \verb|figures| （注意格式，不要把已有的数据覆盖）。 
