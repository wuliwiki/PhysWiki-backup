...
\begin{definition}{高维空间的球坐标}
是 $(x^1,\cdots,x^{N+1})$ 是 $N+1$ 维空间的笛卡尔坐标，则称如下定义的坐标 $(r,\theta,\varphi)$ 为该空间上的球坐标
\begin{equation}
\begin{aligned}
&x^{N+1}=r\theta\cos \theta^N,\\
&x^{N}=r\theta\sin \theta^N\cos\theta^{N-1},\\
&\cdots\\
&x^{2}=r\theta\sin \theta^N\cos\theta^{N-1}\ldots\sin\theta_2.
\end{aligned}
\end{equation}
