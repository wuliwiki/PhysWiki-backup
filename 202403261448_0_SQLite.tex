...
高性能的做法需要复杂得多的编程： 首先把整个表读到程序中（读比写快得多，且这是必要的，否则不知道哪个记录删除了，更优化的做法是先只读所有 primary key，需要时再读其他列）， 在程序中鉴定三种情况：新记录、需要更新的记录、需要删除的记录。 然后把使用 \verb`INSERT`， \verb`UPDATE`， \verb`DELETE` 三种不同的命令分别处理即可。 如果要把这个过程通过函数模板实现则更难（code gen 会更容易）。
