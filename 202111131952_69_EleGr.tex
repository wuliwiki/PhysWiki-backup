...
不幸的是，这个积分如此复杂，以至于我们只能借助计算软件得到数值结果．对于 $\bvec r=(0,0,z)$ 的情况，$\gamma=\theta'$．我们可以推出解析解：
\begin{equation}
\begin{aligned}
\phi(0,0,z)=&\frac{Va(a^2-r^2)}{2}\int_0^{\pi/2}\sin\theta'\dd \theta'\\
&\qty(\frac{1}{(a^2+r^2-2ar\cos\theta')^{3/2}}-\frac{1}{(a^2+r^2+2ar\cos\theta')^{3/2}})\\
=&\frac{Va(a^2-r^2)}{2}
\end{aligned}
\end{equation}
