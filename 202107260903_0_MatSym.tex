...
\item 变精度浮点数的运算结果是当前的默认位数 \verb|digits()|， 无论参与该运算的变精度浮点数有多少个有效数字． 例如设置 \verb|digits(32)| 后， \verb|vpa(sym(pi)/2, 50) + vpa(sym(pi)/2, 100)| 的结果仍然是 32 位有效数字．

\item 可以用 \verb|digits()| 设置变精度计算的默认精度， 如果不设置， 默认为 32 位有效数字． 例如运行 \verb|digits(50)|， 再运行 \verb|1 + sym('1e-40') - 1| 就会得到 \verb|0.000...000999...99938892...|， 这就比默认的 32 位有效数字精确多了， 但还是存在误差．
