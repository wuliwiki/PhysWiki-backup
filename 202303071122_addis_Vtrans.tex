...
\subsubsection{证明}
现在用旋转变换矩阵\upref{Rot3D}在 $S$ 系中证明\autoref{Vtrans_eq2} 。 令 $S'$ 中坐标到 $S$ 坐标的旋转变换矩阵\upref{Rot3D}为 $\mat R$， 即对任意几何矢量 $\bvec r$ 有 $\mat R (\bvec r)_{S'} = (\bvec r)_S$， 那么令 $\bvec r_{O'}$ 为 $S$ 的原点指向 $S'$ 的原点的几何矢量， $\bvec r, \bvec r'$ 分别是质点在两参考系中的坐标， 有
\begin{equation}
