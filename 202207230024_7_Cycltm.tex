% 分圆域
% keys 分圆域|cyclotomy|域扩张|单位根|本原根|原根|primitive element|切圆域|割圆域
...
反设$\Phi_n=f(x)g(x)$，其中$f$和$g$都是次数大于$0$的有理系数多项式．不妨\footnote{我们总可以通过乘以一个正整数，将一个有理系数多项式化为本原多项式．}设$f$的首项系数是$k$，且$f$是整系数多项式，则$f$是本原多项式，$g$的首项系数是$1/k$．

由\autoref{Cycltm_the2} ，$\Phi_n$也是本原多项式．于是由\autoref{PPlyR_lem2}~\upref{PPlyR}，知$g(x)$应为本原多项式，故$1/k$也是整数，故$k=1$．因此$g$和$f$都是首一整系数多项式．

任取$\Phi_n$的一根$\varepsilon_k$，则由\autoref{Cycltm_def2} 的互素条件可知，$\varepsilon_k$的全体幂包含了全体$n$次单位根．
