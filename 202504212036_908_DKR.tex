...
\item 检查 C-n 元组是否包含于 D-系统,设有 C-n 元组 \( P \) 和 D-系统 \( Q \)，则有：\(P \subseteq Q \iff \)对于\(Q\) \text{ 中的每一个 D-n 元组 } Q_i，均有 } P \subseteq Q_i~
\]
换句话说，当且仅当\( P \)被包含于 D-系统中所有的 D-n 元组中，才有\( P \subseteq Q \)成立。
\end{enumerate}
