...
若微分方程\autoref{eq_ODEUC_2} 解 $\varphi$ 存在唯一且对初始点 $(t_0,y)$ $r$ 次连续可微。记 
\begin{equation}
...
\end{aligned}
\end{equation}
由假设，\autoref{eq_ODEUC_3} 的解 $\varphi(t)$存在唯一，且 $r$ 次连续可微的依赖于起始点 $(t_0,y)$（即解可写为 $\varphi(y,t_0,t)\equiv\varphi(t)$ 且 $\varphi(y,t_0,t_0)=y$），于是解也就 $r$ 阶连续可微的依赖于 $(t_0,y)$；
...
\dot x=v(x,t)~
\end{equation}
中向量场 $v$ 在扩张相空间（\autoref{def_PSaPF_2}~\upref{PSaPF}）中的一区域 $U$ 上有直到 $r$ 阶连续的导数 $v\in C^r$，则对任一点 $(t_0,x_0)\in U$，存在相空间中点 $x_0$ 的一邻域 $M$，使得任一给定的 $x\in M$，都存在一个 $t_0$ 的邻域区间，使得唯一存在满足初始条件 $\varphi(t_0)=x$ 的微分方程
...
\end{theorem}
\textbf{证明（存在唯一和对起始点的连续性）：}由\autoref{the_PicMap_2}~\upref{PicMap}，对任一点 $(t_0,x_0)$，都有要求的 $x_0$ 的领域 $M$（对应 $\abs{x-x_0}\leq b'$），使得 $\forall x\in M$ ，都有  $t_0$ 的一领域（对应 $\abs{t-t_0}\leq a'$）存在，使得皮卡映射（\autoref{def_PicMap_1}~\upref{PicMap}）
...
\end{equation}
上式只能在 $\norm{g_1-g_2}=0$ 时成立，这意味着唯一性。对 $g\in C_x^{r-1}$ 单独放在下面进行证明。
...
\end{theorem}
\textbf{证明：}对初始点 $x\in M$ 的映射 $g_0(x,t)$ 和初始矩阵为单位矩阵 $\mat E$ 的矩阵映射 $f_0(x,t)$，我们分别用 $g_n,f_n$ 表示其皮卡近似\footnote{对矩阵映射来说，只不过曲线所在空间维度变大了}：
\begin{equation}\label{eq_ODEUC_8}
\begin{aligned}
g_{n+1}(x,t)&=x+\int_{t_0}^{t}v(g_n(x,\tau),\tau)\dd\tau,\\
f_{n+1}(x,t)&=\mat E+\int_{t_0}^{t}v_{*x}(g_n(x,\tau),\tau)f_n(x,\tau)\dd\tau~.
\end{aligned}
\end{equation}
其中 $v_{*x}$ 表示 $v$ 对 $x$ 的导数。
注意 $(g_{0})_{*x}=f_0$，于是由数学归纳法可从\autoref{eq_ODEUC_8} 推得 $(g_{n+1})_{*x}=f_{n+1}$。
