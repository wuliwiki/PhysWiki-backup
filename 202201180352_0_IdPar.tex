...
\begin{example}{}
某一维势阱中有两个互不作用的全同粒子， 我们只考虑束缚态， 不考虑自旋． 单个粒子在该势阱中可能有 3 个正交归一的束缚态 $\psi_i(x)$ （$i=1,2,3$）， 它们可以看作是 3 维希尔伯特空间 $\mathcal V$ 中的基底．

那么双粒子波函数将处于张量积空间 $\mathcal V\otimes \mathcal V$ 中． 该空间的 9 个函数基底在数学上可以取 $\psi_i(x_1) \psi_j(x_2)$ （$i,j=1,2,3$） 但其中只有 $\psi_i(x_1)\psi_i(x_2)$ 3 个基底是对称的． 剩下 6 个 $i\ne j$ 的基底显然不满足交换对称或反对称．

对 $i\ne j$ 的基底， 我们可以稍作幺正变换， 由 $\psi_i(x_1)\psi_j(x_2)$ 和 $\psi_j(x_1)\psi_i(x_2)$ 张成的子空间中， 把一个对称基底取为 $[\psi_i(x_1) \psi_j(x_2) + \psi_j(x_1) \psi_i(x_2)]/\sqrt{2}$， 一个反对称基底取为 $[\psi_i(x_1) \psi_j(x_2) - \psi_j(x_1) \psi_i(x_2)]/\sqrt{2}$．

这样， 所有 9 个新的基底中， 共 6 个是对称的， 3 个是反对称的． 显然对称基底张成的 6 维空间就是
\end{example}
