...
\begin{equation}
\dot c_m(t) = - \sum_{n}^{E_n=E_m} c_n \braket*{\psi_m}{\dot\psi_n}\E^{\I(\theta_n-\theta_m)}
- \sum_{n}^{E_n\ne E_m} c_n \frac{\mel*{\psi_m}{\dot H}{\psi_n}}{E_n-E_m}\E^{\I(\theta_n-\theta_m)}~.
\end{equation}
其中两个求和分别对所有使 $E_n=E_m$ （在 $t）以及 $E_n\ne E_m$ 的 $n$ 进行。 现在还没有使用任何近似。 绝热近似就在于假设 $\dot H$ 非常小，从而忽略该式中第二项。
