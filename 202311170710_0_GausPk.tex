...
% properties of cos2 wave packet

fun = @(k) sqrt(2)*pi^1.5./(4*pi^2-k.^2) .* sinc(k/2);
fun2 = @(k) fun(k).^2;
...
xlabel k;
hold on;
scatter((2:6)*2*pi, 0, 'k');
scatter((-6:-2)*2*pi, 0, 'k');
axis([-40, 40, -0.02, 0.21]);

A = 1/sqrt(integral(fun2, -inf, inf));
integral(@(k)fun2(k).*abs(k).*A^2, -inf, inf)
\end{lstlisting}
