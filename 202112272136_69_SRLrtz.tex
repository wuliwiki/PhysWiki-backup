...
\end{aligned}
\end{equation}
两个洛伦兹变换的组合还是洛伦兹变换，这意味着洛伦兹变换构成一个\textbf{洛伦兹群}\upref{qed1}，每个群元都代表了从一个参考系到另一个参考系的变换．

对于两个四矢量 $p^\mu,q^\mu$，可以构造洛伦兹不变量 $p_\muq^\nu\eta_{\mu,\nu}p^\mu q^\nu$，代表的意义是 $p$ 和 $q$ 的“点乘”，只不过是闵可夫斯基空间度规下的点乘．由于 $\eta_{\mu,\nu}p^\mu q^\nu$ 在洛伦兹变换下是保持不变的，