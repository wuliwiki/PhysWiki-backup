...
把 UT1 儒略日的小数部分除以 86400 （$24\times 60\times 60$）， 就可以定义 UT1 时间， 即上一个 UT1 正午经过的 UT1 秒数（注意儒略日是从中午开始计算的）． 注意这里的 UT1 秒是由\autoref{time2_eq1} 定义的而不符合国际单位． 由于地球自转速度会在潮汐力的作用下发生改变， 所以 UT1 中一秒的长度也会在几天到几十天的周期内变化． UT2 就是在 UT1 的基础上把周期性潮汐力对地球自转的影响过滤掉了． 测量得出， 现在地球自转一圈约比 100 年前的 1 天慢 1.7 毫秒．
