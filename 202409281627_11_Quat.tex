...
这是数乘特殊酉矩阵的一般形式。满足$$f(A)^{\dagger}=f(A)^{-1}$。若$A$是单位四元数，则$\opn{det}f(A)=1$，。也就是说$f$将单位四元数映射到$SU(2)$中。容易验证，先运算（加法和乘法）后映射的结果与先运算后映射的结果相同，也就是说这是同态映射。在群意义上，单位四元数与$SU(2)$同构。
