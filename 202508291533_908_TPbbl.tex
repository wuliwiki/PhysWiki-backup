...
\item 第一可数空间（First-countable space）**
  如果空间中每个点都有一个**可数的局部基**，则该空间是**第一可数空间**。

\item 第二可数空间（Second-countable space）**
  如果空间的拓扑存在一个**可数的基**，则该空间是**第二可数空间**。
  第二可数空间总是**可分的**、**第一可数的**，并且是**Lindelöf 空间**。

\item \textbf{Lindelöf 空间}.如果空间的**任意开覆盖**都有一个**可数的子覆盖**，则该空间是**Lindelöf 空间*。
\item \textbf{σ-紧空间}.如果一个空间可以表示为**可数多个紧子空间的并集**，则该空间是σ-紧空间。
\end{itemize}