...
进而可以得到， 对于任意两个包含一个点电荷 $q$ 的曲面（正方向都向外）， 电场在它们上的通量都等于 $q/\epsilon_0$， 而 $q$ 也不一定需要在原点． 再根据电场的叠加原理和点乘的分配律（\autoref{CrossP_eq1}~\upref{CrossP}）， 如果曲面内有多个点电荷 $q_1, \dots, q_N$， 那么它们在曲面上的通量等于每个点电荷产生的通量之和， 即
\begin{equation}
...
\end{equation}
注意如果电荷密度 $\rho(\bvec r)$ 处处为有限值， 那么电场的散度也将处处有定义． 对比散度定理可得电场的散度与电荷密度成正比．
\begin{equation}
\div \vdot \bvec E(\bvec r) = 
\end{equation}
