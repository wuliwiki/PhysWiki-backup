...
\end{equation}
它看上去和标准方程不完全一样，直接对比的话相当于将等号右侧的$1$变成了参数。其实前面说的的代换也可以看作是取$x=\sqrt{\lambda} x',y=\sqrt{\lambda} y'(\lambda>0)$得到的，而根据\aref{图像变换的规律}{sub_FunTra_2}，很显然，这就是在$x,y$两个方向进行了等比例的放缩，就像同心圆那样。
假设 $p=2$, $q=1$，则表达式为 $\frac{x^2}{4} + \frac{y^2}{1} = \lambda$
以下是六个 $\lambda$ 值对应的椭圆：
$\lambda = 0.25$: $\frac{x^2}{4} + \frac{y^2}{1} = 0.25$ (最小的椭圆)
$\lambda = 0.5$: $\frac{x^2}{4} + \frac{y^2}{1} = 0.5$
$\lambda = 0.75$: $\frac{x^2}{4} + \frac{y^2}{1} = 0.75$
$\lambda = 1$: $\frac{x^2}{4} + \frac{y^2}{1} = 1$ (标准椭圆)
$\lambda = 1.5$: $\frac{x^2}{4} + \frac{y^2}{1} = 1.5$
$\lambda = 2$: $\frac{x^2}{4} + \frac{y^2}{1} = 2$ (最大的椭圆)
