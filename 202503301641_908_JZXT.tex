...
\item 如果原子周围有一个理想的电子气，其电子密度为 \( n_e \)，则有：\(\exp \left( \frac{\mu + q\varphi}{kT} \right) = n_e \lambda_{\text{th}}^{3}/2\)代入这一公式后得到萨哈电离方程。[10]
\item 在半导体中，掺杂原子的电离可以很好地用这个集合描述。[11]在半导体中，导带边缘\( \epsilon_C \)充当真空能级的角色（取代 \( -q\varphi \)），而 \( \mu \) 被称为费米能级。当然，掺杂原子的电离能和电子亲和能与它们在真空中的值相比有很大改变。典型的硅中供体掺杂物磷，其电离能\( \Delta E_I = 45 \, \text{meV} \)。[13]
\end{itemize}