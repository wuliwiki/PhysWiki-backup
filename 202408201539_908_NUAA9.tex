...
解：
\begin{enumerate}
    \item 设量子化转动惯量 $I = \frac{1}{2}MR^2$，取 $\xi$ 轴为转动轴。
    \item 哈密顿算符：
   \hat{H}_z = \frac{1}{2I}\hat{L}_z^2 = -\frac{\hbar^2}{2I}\frac{\partial^2}{\partial\varphi^2}
    \item 本征方程：
    -\frac{\hbar^2}{2I}\\frac{\\partial^2}{\\partial\\varphi^2}\\Psi = E\\Psi 
    \item 对应本征函数：
    \\[    \\chi_m^{(0)} = \\frac{1}{\\sqrt{2\\pi}}e^{im\\varphi} \\quad \\text{其中 } m = 0, \\pm1, \\pm2, \\ldots    \\]
    \\item 能量本征值：
    \\[    E_m^{(0)} = m^2\\frac{\\hbar^2}{2I} = m^2\\frac{\\hbar^2}{MR^2} \\quad \\text{其中 } m = 0, \\pm1, \\pm2, \\ldots    \\]
    \\item 除基态($m = 0$)外，$m \\neq 0$ 的本征态都是简并态：
    \\[    \\chi_m^{(0)} = \\frac{1}{\\sqrt{2\\pi}}e^{im\\varphi}, \\quad \\chi_{-m}^{(0)} = \\frac{1}{\\sqrt{2\\pi}}e^{-im\\varphi}    \\]
    或
    \\[    \\chi_m^{(0)} = \\frac{1}{\\sqrt{2\\pi}}\\cos m\\varphi, \\quad \\chi_{-m}^{(0)} = \\frac{1}{\\sqrt{2\\pi}}\\sin m\\varphi    \\]
\\end{enumerate}
