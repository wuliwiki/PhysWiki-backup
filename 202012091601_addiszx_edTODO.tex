...
\item 自动补全括号 \verb|()[]{}| 的算法需要和 vscode 的行为一样： 1. 在字符左边插入 \verb|{| 时不需要补全右括号． 2. 选中几个字符并插入 \verb|{| 时， 自动在选中字符的左边插入 \verb|{|， 右边插入\verb|}|． 3. 在 \verb|}| 的左边插入 \verb|}| 时， 如果前面存在没有配对的 \verb|{|， 则不作为．
