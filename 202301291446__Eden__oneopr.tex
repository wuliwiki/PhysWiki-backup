...
现在考察多体系统，多粒子态 $\ket{\psi_1,\cdots,\psi_N}$ 不再是 $N$ 个单粒子态的简单的张量积，因为交换对称性，我们需要仔细考虑。假设单粒子 Hilbert 空间的一组正交完备基为 $\ket{1},\ket{2},\cdots$，那么 Fock 空间可以表达为一系列多粒子态 $\ket{ijk\cdots}$ 的直和。此时单粒子 Hilbert 空间上单体算符可以表达为
\begin{equation}
A^{(1)}(\hat x,\hat p)=\sum_{\alpha\beta}A_{\alpha\beta} \ket{\alpha}\bra{\beta}
\end{equation}
其中 $A_{\alpha\beta}$ 为 $A^{(1)}$ 算符的矩阵表示。
