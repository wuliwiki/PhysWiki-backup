...
证明在第 $k$ 步算法选择的不在 $S$ 集合中距离 $S$ 集合最近的点，它在被加入 $S$ 集合前，它的路径就是最短路径．
...
第 $k$ 步算法选择的点为 $u$，它的最短路径已经被确定，并且加入到了 $S$ 集合中．考虑第 $k + 1$ 步算法选择了 $u \to v$ 这条边的 $v$ 结点．证明 $dist[v]$ 就是从源点 $s$ 到 $v$ 的最短路径．若存在另一条路径 $s \to y \to v$，即证：$d_v \leq d_y + w(y \to v)$．
