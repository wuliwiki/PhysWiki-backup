...
\begin{equation}
\begin{aligned}
&\sum a_m \int \dd{}^3 \bvec r \phi_i^*(\bvec r-\bvec R_n)(U(\bvec r)-V(\bvec r-\bvec R_m))\phi_i(\bvec r-\bvec R_m)\\
&=a_n(E-\epsilon_n)\phi_i(\bvec r-\bvec R_m)
\end{aligned}
\end{equation}
注意到 $U(\bvec r)$ 是周期性势场．可以作换元  $\xi=\bvec r-\bvec R_m$ 上式可以简写为
\begin{equation}
\sum a_m \int \dd{}^3 \bvec r \phi_i^*(\bvec r-\bvec R_n)(U(\bvec \xi)-V(\bvec \xi))\phi_i(\bvec \xi)=a_n(E-\epsilon_n)\phi_i(\bvec r-\bvec R_m)
\end{equation}
