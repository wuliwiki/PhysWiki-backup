...
\begin{definition}{共轭算符}
对于算符$X$，若存在算符$X^\dagger$使得对于任意量子态$\ket{s}$，$X\ket{s}$都和$\bra{s}X$互为对偶表示，则称$X^\dagger$为$X$的\textbf{共轭算符（conjugate operator）}，或\textbf{厄米共轭（hermitian conjugate）}．
\end{definition}
...
\begin{itemize}
\item \textbf{位置算符}为$\hat{x}=x$（一维情况）或$\uvec{r}=x\hat{x}+y\hat{y}+z\hat{z}$（三维情况）；
\item \textbf{动量算符}为$\hat{p}=-\I\hbar\frac{\partial}{\partial x}$（一维情况）或$\uvec{p}=-\I\hbar\nabla=-\I\hbar(\hat{x}\frac{\partial}{\partial x}+\hat{y}\frac{\partial}{\partial y}+\hat{z}\frac{\partial}{\partial z})$（三维情况）；
...
\begin{itemize}
\item \textbf{位置算符}为$\hat{x}=\I\hbar\frac{\partial}{\partial p}$（一维情况）或$\uvec{r}=\I\hbar (\hat{p}_{x}\frac{\partial}{\partial p_x}+\hat{p}_{y}\frac{\partial}{\partial p_y}+\hat{p}_{z}\frac{\partial}{\partial p_z})$（三维情况）；
\item \textbf{动量算符}为$\hat{p}=p$（一维情况）或$\uvec{p}=p_x\hat{p}_x+p_y\hat{p}_y+p_z\hat{p}_z$（三维情况）；
\item \textbf{能量算符}为$\hat{E}=\I \frac{\partial}{\partial t}$；
\item \textbf{角动量算符}为$\uvec{L}=\uvec{r}\times \uvec{p}$，展开后得
\begin{equation}
\hat{\bvec{L}}=
...
其中角动量要特别注意，算符相乘不是数字相乘，而是函数复合．所以位置算符和动量算符做叉乘的时候，应代入一个“测试函数”$\psi$，根据复合的定义得$[(\frac{\partial}{\partial_x})\cdot(p_y)]\psi=\frac{\partial}{\partial_x}(p_y\psi)=p_y\frac{\partial}{\partial_x}\psi$．
\end{example}
\addTODO{动量表象的正确性有待核验．核验后删除此“未完成”．}
