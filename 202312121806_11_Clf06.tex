...
\begin{theorem}{}
给定线性映射：$f:V\rightarrow W$，令$q,p$分别为$V,W$上的\textbf{非退化duic}
\end{theorem}
proof.
回顾伴随映射的定义。设上述线性映射$f$在这两组基下的矩阵为$F^a_b$，伴随映射$f^*$在这两组基下的矩阵为$G^a_b$。那么我们有：$G^a_b =q^{ja}p_{ib}F^i_j$。如果二次型是标准的，上式简化为$G^a_b=q(e_j)p(e_b)F^i_j$。
