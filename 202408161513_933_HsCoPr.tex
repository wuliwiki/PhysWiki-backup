...
比如：“如果一个整数是偶数，那么它能被2整除。”的否定是“如果一个整数是偶数，那么它不能被2整除。”
...
\begin{exercise}{求“任意平面四边形，若这个四边形是矩形，则这个四边形的对角线长度相等且有一个内角是直角。”的否定、否命题、逆否命题，并判断真值。}
题目给出的命题是真命题。
\begin{itemize}
\item 否定：存在一个平面四边形，这个四边形是矩形，且这个四边形的对角线长度不相等或所有内角都不是直角。（假命题）
\item 否命题：任意平面四边形，若这个四边形不是矩形，则这个四边形的对角线长度不相等或所有内角都不是直角。（假命题）
