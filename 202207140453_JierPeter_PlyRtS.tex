...
显然，$\mathbb{K}=\mathbb{F}(u, v, w)$，其中$u$在$\mathbb{F}$上的最小多项式次数不大于三，$v$在$\mathbb{F}(u)$上的最小多项式次数不大于前者减一，故$[\mathbb{K}:\mathbb{F}]$只有四种可能：$6$，$3$，$2$和$1$．这还意味着
...
\begin{equation}\label{PlyRtS_eq11}
...
参考\autoref{PlyRtS_lem1} ，如果$[\mathbb{K}:\mathbb{F}]=6$，则$\opn{Gal}$
...
而$\abs{V_4}=4$．因此\autoref{PlyRtS_lem1} 中的四种情况正对应\autoref{PlyRtS_the3} 中的四种情况．
...
如果$[\mathbb{K}:\mathbb{F}]=6$，则考虑到\autoref{PlyRtS_eq10} 和\autoref{PlyRtS_eq11} ，可知$\abs{\opn{Gal}(f:\mathbb{F})}=24$，即$\opn{Gal}(f:\mathbb{F})=S_4$．
