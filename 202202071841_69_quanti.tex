...
\begin{equation}
|\bvec k\rangle = \sqrt{2\omega_{\bvec k}}a^\dagger_{\bvec k} |0\rangle
\end{equation}
可以继续将产生算符 $a^\dagger$ 作用于这个单粒子态，这相当于有多个粒子占据这一量子态，即 Klein-Gordon 场描述的是玻色子系统．$\langle \bvec k|\bvec k'\rangle = 2\omega_{\bvec k}(2\pi)^3\delta(\bvec k-\bvec k')$．容易证明这是洛伦兹不变量．与这些单粒子态对应的完备关系可表达为
\[
...
在 Fock 空间上，坐标的洛伦兹变换体现为量子化的算符的幺正变换．对于一个洛伦兹变换 $\Lambda$，它将体系的坐标 $x$ 变为 $x'=L x$，动量 $\bvec k$ 变为 $\bvec k'$，记为 $k'=L \bvec k$．设 $\Lambda$ 对应的 Hilbert 空间的算符为 $U(\Lambda)$，那么我们记
\[
