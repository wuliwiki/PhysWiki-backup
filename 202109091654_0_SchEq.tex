...
\subsubsection{简并}
束缚态的能量\textbf{简并（degenerate）}是指一个能量本征值 $E$ 对应多个线性无关解的束缚态． 一个能量最多有几个线性无关的束缚态， 那就说该能级有多少\textbf{重}简并． 这和线性代数的厄米矩阵本征值问题\upref{HerEig}中的 “简并” 用法类似．
...
由于散射态不能用 $\braket{\psi}{\psi} = 1$ 的方式归一化， 它们并不是物理的， 即粒子不能处于某个散射态． 然而散射态的线性组合却可能可以归一化． 这里指的是无穷多个散射态的线性组合， 求和变为积分
\begin{equation}
\psi_{wp}(x) = \int C(E) \psi_s(E, x)\dd{E}
\end{equation}


例如自由粒子（$V \equiv 0$）的高斯波包\upref{GausPk} 就可以看作由平面波线性叠加而来， 即反傅里叶变换． 某种意义上， 给定一个势能函数， \textbf{所有的束缚态和散射态可以构成一组完备的正交归一的函数基底}， 可以展开所有可归一化的波函数． 散射态有特殊的正交归一化条件， 这需要用到较为复杂的数学知识， 我们留到 “一维散射态的正交归一化\upref{ScaNrm}” 介绍．
