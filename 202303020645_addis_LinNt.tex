...
\item 要改变 swap 文件的尺寸： \verb|sudo swapoff /swapfile|， \verb|sudo dd if=/dev/zero of=/swapfile bs=1M count=1024 oflag=append conv=notrunc|， \verb|sudo mkswap /swapfile|， \verb|sudo swapon /swapfile| 一般无需重启。 如果系统不允许关闭 swap， 也可以考虑用多个 swap 文件。
