...
可以运用Lagrange中值定理与积分中值定理．%极限中出现变上限积分时，自变量往往位于上（下）限，且上下限往往趋于一致．

\begin{example}{}
求解 $\lim_{x\to0} \frac{\int^x_0 \cos(x)}{x} \dd x$

可以运用洛必达法则，原式化为$\lim_{x\to0} \cos(x) = 1$

另一种思路是运用积分中值定理．根据中值定理，$\exists \xi \in (0,x)$, $\int^x_0 \cos(x) \dd x = \cos(\xi) (x-0)$，因此原式化为 $\lim_{x\to0} \frac{\cos(\xi)x}{x} = \lim_{x\to0} \cos(\xi)$．由于$\xi \in (0,x)$，当$x\to0$时，$\xi$别无选择，只能也趋向0.因此，$\lim_{x\to0} \cos(\xi)=1$

尽管在这里微分中值定理看起来更为繁琐，但这是因为\textsl{这道题太简单了}，在一些复杂的问题中，微分中值定理比洛必达更为方便．
\end{example}