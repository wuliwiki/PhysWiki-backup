...
\end{figure}
图2表示的是一个简单的感知机。其中有两层，分别为输入层和输出层。输入层有两个输入神经元，分别接收输入信号$x_1$和$x_2$。输出层有一个输出神经元，产生输出结果$y$。这个的简单的感知器只有输出层的那个神经元有计算功能，因此，其实只有一层功能神经元（Functional neuron）。

感知机能够表示所有的原子逻辑函数：与（AND）、或（OR）、与非（NAND）、或非（NOR）[2]。然而，对于某些逻辑函数，单一的感知器无法表示。比如，异或运算。


感知机可以堆叠，层数也能加深，每层也可以有多个神经元。由多层神经元堆叠而成的具有层级结构的神经网络，称为\textbf{多层感知机}（Multi-layer perceptron, MLP），或者\textbf{前馈神经网络}（Feedforward neural network）.之所以称之为“前馈”神经网络，是因为数据在网络中的流动方向始终是单向的，数据从浅层网络向深层网络传播，而不会返回。

事实上，仅用拥有两层深度的功能神经元的多层感知机就能表示所有的逻辑函数[2]。
\begin{figure}[ht]
\centering
\includegraphics[width=8cm]{./figures/d2e6a4c6b99c91b4.png}
\caption{多层感知机} \label{fig_NN_3}
\end{figure}
图3表示的是一个有两层功能神经元的多层感知机。其中，中间一层为隐含层。原始的输入信号，经过隐含层的处理，送到输出层，再由输出层做处理，最终产生结果作为整个网络的输出值。隐含层的存在使得神经网络具有非线性函数的表示能力。
