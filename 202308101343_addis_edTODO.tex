...
\item 如果上传时发现图片文件的 hash 已存在于 \verb|images.hash|， 则检查该文件的 \verb|images.figures| 中 \verb|figures.deleted==0| 的图片环境， 有就提示图片已存在，列出并显示词条 id 和 caption 让用户选择引用（\verb|\autoref{}|）（推荐）或者重复使用图片。 如果没有或者用户选择重复使用图片， 就检查 \verb|images.figures| 中被标记 \verb|figures.deleted| 的环境有没有原本属于当前词条的， 如果有就更新 \verb|figures.deleted=0| 然后根据该记录中的信息插入图片环境代码； 如果没有。
