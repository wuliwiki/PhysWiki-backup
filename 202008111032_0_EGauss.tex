...
% =============== 未完成 ==========
% 这里给出的是一个比较幼稚直观的证明
% 系统的做法是先使用散度公式， 证明点电荷的电场除了那一点外都是零
% 所以通量和曲面的选取无关， 只要曲面不穿过电荷
% 然后再计算球形曲面的通量， 就是任意曲面的通量
% ===============================
...
\dd{\Phi_1} = \dd{\Phi_2}
\end{equation} 
\item 若把整个闭合曲面划分成无数个小面元（如\autoref{EGauss_fig1} ），每个小面元 $\dd{\bvec s_i}$ 根据上面推理，都等效为半径 $R$ 上的一块垂直面元 $\dd{\bvec s_i}$． 这样，这些等效面元可以重新组成一个半径为 $R$ 的球体．而球体的通量为
