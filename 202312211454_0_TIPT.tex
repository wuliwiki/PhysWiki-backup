...
\textbf{绝热近似（adiabatic approximation）}\upref{AdiaQM}告诉我们， 如果 $\Psi(0)$ 是一个非简并本征态（或好量子态） $\psi_n(0)$， 那么该过程中 $\Psi(t)$ 将始终保持 $H+\lambda(t) H'$ 的本征态（好量子态） $\psi_n(\lambda)$，但需要乘以一个额外的相位因子 $\E^{\I\alpha_n(t)}$。 而 $\psi_n(\lambda)$ 又可以进一步通过不含时微扰进行计算。
