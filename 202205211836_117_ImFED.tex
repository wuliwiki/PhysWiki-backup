...
\end{equation}
由连续性（布尔查诺-柯西第一定理（or连续函数介值定理），编辑词条并引用），必有 $\overline{A}\overline{B}$ 间一点 $y=\overline{y}$，\footnote{错误的原因可能在于这里的$\overline{y}$并不是唯一的，条件3只说$x=x_0$时，函数$F(x,y)$关于$y$严格单增, 而在其他位置，不一定严格单增！！}使得
