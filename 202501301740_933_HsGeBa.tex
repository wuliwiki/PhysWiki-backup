...
由无数个点排列组成的没有宽度的理想化几何对象称为\textbf{线（line）}。如果一条线上的所有点都在一个固定的方向上，则称其为\textbf{直线}。直的线有三种不同的形式包括：
\begin{itemize}
\item \textbf{直线}：无限延伸，不存在端点。
\item \textbf{射线}：从一个端点出发，向另一方向无限延伸。
\item \textbf{线段}：有两个端点，表示有限长度。
\end{itemize}
...
对于线段而言，可以定义其长度。长度表示线段的度量值，在欧氏几何中是最基本的几何量之一，无法通过更基本的概念进行定义。线段的长度可用于计算距离、建立比例关系等。将线段分成两段等长部分的点称为线段的中点。
