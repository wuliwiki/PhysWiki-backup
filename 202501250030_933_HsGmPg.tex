...
这一性质说明，等比数列的任意两项乘积只要满足项数之和相等，就可以通过另一对项的乘积表示。与等差中项类似，如果在 $a$ 和 $b$ 中插入一个数 $G$，使得 $a,G,b$ 成等比数列，即：
\begin{equation}
{G\over a} = {b\over G}~.
\end{equation}
...
G^2 = ab~.
\end{equation}
结合\autoref{cor_HsGmPg_1} 可以知道，对于一个等比数列，如果某两项的项数和恰好是另一项的两倍，那么这第三项就是那两项的等比中项。不过需要注意，这要求那两项的项数和必须是偶数，否则无法在等比数列中找到符合条件的项\footnote{与此不同的是，对于直线，无论什么情况下都可以找到对应的点。}。由于等比数列中不含 $0$，因此 $ab > 0$，即条件中给出的 $a$ 和 $b$ 必须同号。另外由此可以得到：
\begin{equation}
G = \pm \sqrt{ab}~.
\end{equation}
这说明，$G$ 的正值正好是$a$ 与 $b$的\textbf{几何平均值（geometric mean）} \footnote{关于几何平均值参见\aref{基本不等式}{sub_HsIden_2}。几何平均值的英语也和等比数列的英语使用了一样的单词 “geometric”。}。
