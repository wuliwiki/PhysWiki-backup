...
绝热近似的关键就在于假设 $H$ 随时间变化缓慢，即 $\dot H$ 非常小，以至于两个能级 $E_n$ 和 $E_m$ 如果相距不是太近，可以近似认为\autoref{eq_AdiaQM_11} 在矩阵 $\mat A$ 中可以忽略不计。

\subsection{非简并情况}
若在考虑的时间区间内， $H(t)$ 始终没有发生简并，不同的能级之间也没有太接近， 那么可以假设\autoref{eq_AdiaQM_11} 全部为零，也就是 $\mat A = \bvec 0$。 此时\autoref{eq_AdiaQM_9} 直接变为
\begin{equation}
\dot{\bvec c}(t) = \bvec 0~.
\end{equation}
这说明所有系数都不随时间变化，令 $C_n = c_n(0)$，得到\autoref{eq_AdiaQM_2}。

\subsection{简并情况}
若\autoref{eq_AdiaQM_11} 右边不能全部忽略，则
