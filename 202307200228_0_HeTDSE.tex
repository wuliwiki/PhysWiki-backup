...
\end{equation}
列出薛定谔方程的 “矩阵形式”， 就得到了一组耦合的径向波函数的薛定谔方程
\begin{equation}
...
\end{equation}
其中 $H_i \ \ (i = 1, 2)$ 是单个电子的哈密顿算符（\autoref{eq_RYTDSE_2}~\upref{RYTDSE}）， 不耦合分波， 对应对角矩阵
\begin{equation}
...
\autoref{eq_HeTDSE_7} 中剩下的哈密顿项都会耦合分波： $V_{12}$ 是两电子之间的库仑势能， $V_{Fi}$ 是电场对每个电子的作用势能。
