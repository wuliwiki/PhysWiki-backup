...
\subsection{静态和动态库}
\begin{itemize}
\item \verb`BUILD_SHARED_LIBS` 变量用于让 \verb`add_library()` 默认编译成动态库。 即 \verb`add_library(example SHARED ${sources})` 而不是 \verb`add_library(example STATIC ${sources})`。 手动指定是，还是会按照手动的设置。
\end{itemize}
