...
\entry{Bloch}{布洛赫理论}
\entry{bnlext}{伯努利数拓展}
\entry{BscAlg}{代数学基本定理}
\entry{BvecA}{磁矢势}
\entry{CanTra}{正则变换}
\entry{CatTns}{张量的分类}
\entry{CEfDiv}{点电荷的散度}
\entry{CenG}{重心}
\entry{CirISW}{无限深圆形势阱}
\entry{CM1}{动量和能量}
\entry{CM2}{角动量}
\entry{cppOp}{C++ 基本运算 }
\entry{CurMag}{电流产生磁场}
\entry{CylHlm}{柱坐标中的亥姆霍兹方程}
\entry{CylNab}{柱坐标系中的矢量算符}
\entry{DGGRNt}{微分几何入门与广义相对论笔记}
\entry{Diode}{二极管}
\entry{DipApr}{偶极子近似（量子）}
\entry{DipEle}{跃迁偶极子矩阵的三种形式}                     
\entry{DirPro}{张量积空间}
\entry{editIn}{小时物理百科编辑器简介}
\entry{editor}{编辑器使用说明}
\entry{edTODO}{编辑器事项}
\entry{EqEng}{能均分定理}
\entry{FD}{单缝的夫琅禾费衍射}
\entry{FitPdf}{分布函数的数值拟合}
\entry{FSWtwo}{有限深势阱中的双粒子}
...
LSFC      LSFC                
MathHi    数学发展史     
MBdisN    麦克斯韦—玻尔兹曼分布的数值模拟
NtLcns    LaTeX 云笔记协议
NumVec    代数矢量        
PanBak    用百度网盘安全高效地备份文件
PanLcs    网盘协议        
PrncAx    刚体的主轴     
Propos    命题及其表示法
PyExcp    Python 异常处理 
...
rules     百科编写规范  
Sample    词条示例        
SEopBC    TDSE Open Boundary Condition
spiral    心脏螺旋波     
SqPtNu    方势垒散射的数值计算
t         个人目录        
Td2Law    热力学第二定律
