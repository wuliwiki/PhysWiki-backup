...
\autoref{Topo2_def2} 的描述比较绕，更多的材料里使用的是以下\textbf{等价}定义：

\begin{definition}{局部紧空间（等价定义）}\label{Topo2_def3}
给定拓扑空间 $X$ 和 $x\in X$．如果对于某个 $x\in X$，存在一个紧邻域$C\ni x$，那么称空间 $X$ 在 $x$ 处\textbf{局部紧（locally compact）}．如果 $X$ 在每一个点都局部紧，称这个空间是局部紧空间．
\end{definition}

我们简单证明一下\autoref{Topo2_def2} 和\autoref{Topo2_def3} 为什么等价．

\textbf{证明}：

由\autoref{Topo2_def2} 可直接推出\autoref{Topo2_def3} \footnote{$x$总存在邻域$X$，根据\autoref{Topo2_def2} 又可知$x$必有紧邻域，从而推出\autoref{Topo2_def3} .}，因此下面用\autoref{Topo2_def3} 来推出\autoref{Topo2_def2} ．

取$x$的一个紧邻域$C$，再任取$x$的一个邻域$U_x$．
