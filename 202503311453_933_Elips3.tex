...
椭圆C含于以椭圆长轴为直径的圆P内，椭圆的的任一条弦（除长轴外）都是圆P弦的一部分。
圆P的弦不大于圆P的直径（即椭圆长轴长），所以椭圆的任一条弦不大于圆P的直径，所以椭圆最长的弦是它的长轴，即长轴是椭圆中唯一最长的弦。同理，短轴是椭圆中唯一最短的弦。


任何穿过两个焦点的直线段，其在椭圆上的两个端点之间的距离等于 $2a$。
...
根据这个性质，可以得到开始时提到的那个反射的性质，即任意从一个焦点 $F_1$ 发出的光线射到椭圆上的某点 $P$，会被反射到另一个焦点 $F_2$，路径 $F_1PF_2$ 是 $F_1 \to F_2$ 所有路径中最短的一条绕射路径。在椭圆上任意一点 $P$，连到两个焦点 $F_1$, $F_2$ 的线段与该点的切线夹角相等。即：
\begin{equation}
