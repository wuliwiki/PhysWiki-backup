...
下面概念给出了边的有向性。
\begin{definition}{边的有向性}
设 $G=(V,E,\varphi)$ 是图，$e\in E$。若 $\varphi(e)\in[V]^2$，则称 $e$ 是\textbf{无向的}（undirected）；若 $\varphi(e)\in V\times V$，则称 $e$ 是\textbf{有向的}（directed）。若 $\varphi(e)=\{v_1,v_2\}$，则简记 $e=\{v_1,v_2\}$，而 $v_1,v_2$ 称为 $e$ 的端点，且记 $v_1\in e$；若 $\varphi(e)=(v_1,v_2)$，则简记 $e=(v_1,v_2)$。对有向边 $e=(v_1,v_2)$，称 $v_1$ 是 $e$ 的\textbf{起点}（origin），$v_2$ 是 $e$ 的\textbf{终点}（terminus）。起点和终点统称为\textbf{端点}（end vertex）。
\end{definition}
...
设 $G=(V,E,\varphi)$ 是图，$x,y\in V,e=,f\in E$。则：

 \begin{enumerate}
 \item 若 $x,y\in e$，则称 $x$（$y$ 也是同样的） 和 $e$ 是\textbf{关联的}（incident）；
