...
另外在严格意义上，Linux 只是一个\textbf{系统内核（kernel）}，不是一个操作系统．用户往往使用的是Linux的\textbf{发行版（distribution）}，比如Ubuntu、Debian、Redhat、CentOS、Deepin、Fedora、Arch等．如果把Linux系统内核比喻为发动机引擎，那么发行版就是可以上路的汽车．除Linux系统内核外，这些来自开发者社区的发行版还拥有GNU 工具、附加软件和软件包管理器，大部分也有美观的图形界面． 但使用这些操作系统大同小异．\textbf{本百科选用 Ubuntu进行介绍．}
