...
\end{equation}
如果 $A(k)$ 是以 $k_0$ 为中心的偶函数和实函数，那么根据傅里叶变换的\autoref{the_FTExp_1}~\upref{FTExp} 和平移性质， $\psi(x,0)$ 就是一个偶函数和实函数乘以 $\E^{\I k_0 x}$。 则波包的运动轨迹就是 $x_c = k_0 t$。 根据 Ehrenfest 定理的 \autoref{eq_Ehrenf_2}~\upref{Ehrenf}，无论初始形状的具体形状如何 $\ev{x} = k_0 t$ 都是精确成立的，甚至对任意单调的 $V(x)$， $\ev{x}$ 的轨迹也精确等于经典粒子。
