...
a^b=c~.
\end{equation}
的运算称为\textbf{幂运算（exponentiation，或乘方运算）}，其中a称为\textbf{底数（base）}，b称为\textbf{指数（exponent）}，c在研究与a的关系时称为\textbf{幂值}。在研究c与b的关系时称为\textbf{指数运算}，此时c称为\textbf{真数}，此时运算，要求$a>0,a\neq1$。$a,b$不同时为$0$。
