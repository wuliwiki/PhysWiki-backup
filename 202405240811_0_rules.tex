...
每篇文章的文件必须有一个独一无二的标签， 文章标签必须限制在 6 个字符内，必须在 \verb|main.tex| 和文章文件名中一致。 文章的中文名必须在主文件和文章文件的第一行注释中一致。 中文名中空格用 “\textbackslash 空格” 实现， 不能出现公式环境（尽量用英文单词代替比如 gamma 代替 $\Gamma$）。 文章文件一般放在 contents 目录下， 并在主文件中用 \verb|\entry{文章名}{文章标签}| 命令输入中文名和标签。 该命令大致等效于
