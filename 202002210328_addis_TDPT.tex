...
\end{equation}
我们已经知道 % 链接未完成
$H'(t) = 0$ 的情况下含时薛定谔方程的通解：先解出对应的定态薛定谔方程的波函数 $\ket{\psi_n}$ 和能级 $E_n$，通解可表示为\footnote{假设 $H_0$ 只有离散的束缚态而没有连续态， 例如简谐振子\upref{QSHOop}}
\begin{equation}
...
考虑到
\begin{equation}
H_0\ket{\phi_n(t)} = \I\hbar \dv*{\ket{\phi_n(t)}}{t}
\end{equation}
% 引用公式未完成
可化简为
\begin{equation}
...
其中
\begin{equation}
H'_{ij}(t) = \bra{\psi_i} H'(t)\ket{\psi_j}
\end{equation}
\begin{equation}
...
\begin{equation}
c_i^{(n)}(t) = \frac{1}{\I\hbar} \int_{0}^{t}  \sum_j \bra{\psi_i} H'(t)\ket{\psi_j} \E^{\I\omega_{ij}t} c_j^{(n-1)}(t) \dd{t}
\end{equation}

大多数情况下我们只使用一阶近似，一种较简单的情况是，若初始波函数为某一个 $\ket{\psi_i}$， 且 $\bra{j} H'\ket{i} = W_{ji}f(t)$，$W_{ji}$ 为常数，此时\autoref{TDPT_eq9} 变为 $f(t)$ 关于角频率 $\omega_{ji} = (E_j-E_i)/\hbar$ 的反傅里叶变换\upref{FTExp}．
\begin{equation}
