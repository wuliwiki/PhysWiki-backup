...
\subsection{复变函数的导数}
复变函数的导数定义与实函数的导数\upref{Der}十分类似， 只是其中的除法 $\Delta f/ \Delta z$ 由实数的除法变为复数的除法． 一般复变函数的导数要更复杂， 因为它不但取决于 $z$ 的值， 还取决于求导的方向（即 $\Delta z$ 的方向）， 这有点类似于方向导数\upref{DerDir}． 但如果复变函数在某个区域上解析， 那么在该区域上导数与方向无关．对于复数域初等基本函数，求导的结果也和实数域的求导一样． 详见 “复变函数的导数 柯西—黎曼条件\upref{CauRie}”．
