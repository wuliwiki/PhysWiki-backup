...
以下我们分别在长度、速度、加速度规范中求解 Volkov 波函数． 电场和矢势满足 $\bvec E(t_0) = \bvec A(t_0) = \bvec 0$． 三种规范下的初始波函数都是（不同规范仅当存在电磁场时才有区别）
...
\subsection{长度规范}
要求长度规范长度规范\upref{LenGau}下的 Volkov 波函数只需要对\autoref{Volkov_eq1} 再次做规范变换即可（\autoref{LVgaug_eq6}~\upref{LVgaug}）， 得\footnote{如果你不在乎波函数的初相位， 可以把\autoref{Volkov_eq4} 方括号最后一项去掉．}
