...
\begin{equation}
f:\uvec x=\sum_{i=1}^{n}x_i\uvec e_i\mapsto\uvec x'=\sum_{i=1}^{n}x_i\uvec e'_i
\end{equation}
...
设 $(\uvec e_1,\cdots,\uvec e_n)$ 和 $(\uvec e'_1,\cdots,\uvec e'_n)$ 是矢量空间 $V$ 的不同标准正交基底．设 $\uvec e'_i$ 到 $\uvec e_i$ 的转换矩阵为 $A=(a_{ij})$，即
...
设 $\mat A,\mat B$ 是两正交矩阵，则 $\mat{(AB)}^T\mat{(AB)}=\mat B^T\mat A^T\mat{AB}=\mat E$（几何意义就是将一个标准正交基底 $\{ \uvec e_i\}$ 通过 $B$ 转换到另一标准正交基底 $\{\uvec e'_i\}$，再将 $\{\uvec e'_i\}$ 通过 $\mat A$ 转到另一标准正交基底 $\{\uvec e''_i\}$），即正交矩阵的矩阵乘法也是正交矩阵（封闭性）．由于矩阵乘法满足结合律，并且 $\mat A$ 有逆元 $\mat A^T$ ，且 $E$ 也是正交矩阵，所以 $O(n)$ 构成一个群\upref{Group}．
