...
\item 10.30 …… 复合映射 $\Phi=T\circ\sigma$ 是 $V$ 中的以 $Q^k$ 为参数域的 $k$ 曲面． 称 $\Phi$ 为 $\mathscr C''$ 类的有向 $k$-单形． $V$ 中 $\mathscr C''$ 类的有向 $k$-单形 $\Phi_1,\dots,\Phi_r$ 的有限ji

\item 10.33 设 $\Psi$ 是开集 $V\subset R^m$ 中的 $\mathscr C''$ 类 $k$-链， $\omega$ 是 $V$ 中的 $\mathscr C'$ 类 $(k-1)$-形式， 那么 $\int_\Psi\dd{\omega}=\int_{\partial \Psi}\omega$．
\end{itemize}
