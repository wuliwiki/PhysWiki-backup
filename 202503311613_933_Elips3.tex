...
即椭圆上某点$(x_0,y_0)$处，切线斜率为$\displaystyle-\frac{b^2}{a^2}\frac{x_0}{y_0}$，这也可以通过连立求解方程得到。有了斜率之后，就可以谈一谈椭圆的反射性质了。
...
这个性质在最开始讲“回声墙”的例子时就已经提过，但与\autoref{sub_Elips3_1} 不同，在\autoref{sub_Elips3_2} 推导椭圆的几何定义时，并没有直接从这个例子出发，下面解释一下为什么。
...
而现在，构建回音墙的条件就是，希望找到平面上所有经过一次反射且满足与$M\to P\to N$同时到达的路线，也就是路线长度一样。直接找到所有反射的直线比较困难，因此反过来第一步找到所有满足$|MP'|+|NP'|=|MP|+|NP|$点$P'$，然后再从中筛选符合反射条件的。于是，第一步得到了椭圆，而巧的是，人们发现，椭圆上的所有点都满足反射条件，于是就有了反射性质。因此\autoref{sub_Elips3_2} 直接使用距离做为条件就足够了。


\begin{example}{证明对椭圆上任意一点$p(x_0,y_0)$，}
