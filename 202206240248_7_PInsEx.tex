...
类比\autoref{SprbEx_the3}~\upref{SprbEx}一开始的证明过程，可知存在可分的不可约多项式$h(x)\in\mathbb{F}$和非负整数$k$，使得$h(x^{p^k})=f(x)=x^n-a$，因此$p^k\mid n$．设$n=p^km$，那么$h(x)=x^m-a$．由于$f$只有唯一的根$\alpha$，
