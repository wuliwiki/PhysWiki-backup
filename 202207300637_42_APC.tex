...
\subsection{排列}
\begin{theorem}{}
设 $X,Y$ 分别是基数为 $\abs{X}=n,\abs{Y}=m$ 的有限集，则单射 $f:X\rightarrow Y$ 的个数为 $[m]_n=m(m-1)\cdots(m-n+1)$．
\end{theorem}
...
同样的，上述定理相当于：
\begin{theorem}{}
将 $n$ 个物体的集合在 $m$ 个房间的集合里进行布置，要求每个房间至多包含一个物体的布置个数为 $[m]_n=m(m-1)\cdots(m-n+1)$．
\end{theorem}
\begin{definition}{}
