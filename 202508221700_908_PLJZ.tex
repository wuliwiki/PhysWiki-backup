...
直接应用公式 (2) 可以得到 SU(2$) 群合成律** 的一种参数化形式。\[c]
我们可以通过解如下方程直接求出参数 $c$：

$$
\begin{aligned}
e^{ia(\hat{n}\cdot\vec{\sigma})} \; e^{ib(\hat{m}\cdot\vec{\sigma})}
&= I\big(\cos a\cos b - \hat{n}\cdot\hat{m}\,\sin a \sin b\big) \\
&\quad + i\big(\hat{n}\,\sin a \cos b + \hat{m}\,\sin b \cos a - \hat{n}\times\hat{m}\,\sin a \sin b\big)\cdot\vec{\sigma} \\
&= I\cos c + i(\hat{k}\cdot\vec{\sigma})\sin c \\
&= e^{ic(\hat{k}\cdot\vec{\sigma})},
\end{aligned}
$$

该公式给出了 **SU(2)** 的一般群乘法关系，其中：
...
因此，该群元素中的 **合成旋转参数**（即该情况下 BCH 展开的闭合形式）可以简写为：
...
**特殊情况**
当 $\hat{n}$ 与 $\hat{m}$ 平行时，$\hat{k}$ 也平行于它们，此时有：
