...
\autoref{GExt_the3} 暗示了自同构群和其不变子域的一一对应关系，即“给定自同构群$G$，则能被$G$保持不变的元素，就只能被$G$保持不变”——但是只针对$G$有限的情况．这也体现在以下性质中，注意此处不再需要\textbf{有限}性：
...
\autoref{GExt_the4} 可以简述为：全体保$\mathbb{F}$不变的同构，只能保$\mathbb{F}$不变．
