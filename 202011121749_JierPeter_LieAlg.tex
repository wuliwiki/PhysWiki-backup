...
设$\mathfrak{g}$为域$\mathbb{F}$上的$n$维李代数，它在基$\{\bvec{x}^i\}$下的\textbf{结构常数}为$C_k^{ij}$，在基$\{\bvec{y}^i\}$下的\textbf{结构常数}为$D^k_{ij}$，而且有过渡矩阵\upref{TransM}$a^i_j$使得$\bvec{y}^i=a^i_j\bvec{x}^j$，那么有变换法则：
\begin{equation}
...
\begin{equation}
[\bvec{y}^i, \bvec{y}^j]=D^k_{ij}\bvec{y}^i\bvec{y}^j=D^k_{ij}a^i_m\bvec{x}^ma^j_n\bvec{x}^n=D^k_{ij}a^i_ma^j_n\bvec{x}^m\bvec{x}^n
\end{equation}

\begin{equation}
[\bvec{y}^i, \bvec{y}^j]=[a^i_m\bvec{x}^m, a^j_n\bvec{x}^n]=a^i_ma^j_n[\bvec{x}^m, \bvec{x}^n]=a^i_ma^j_nC^k_{mn}\bvec{x}^m\bvec{x}^n
\end{equation}
