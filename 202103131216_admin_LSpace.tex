...
标量域 $\mathbb F$ 上的矢量空间定义了矢量的集合 $X$、 两个矢量间的加法运算 $X\times X \to X$ 以及标量和矢量间的数乘运算 $\mathbb F \times X \to \mathbb X$． 其中 $X$ 的元素叫做\textbf{矢量}， $\mathbb F$ 的元素叫做\textbf{标量}． 令矢量 $u,v,w \in X$， 标量 $a,b \in \mathbb F$， 矢量加法记为 $u + v$， 数乘记为 $a u$． 两种运算必须满足以下性质：
