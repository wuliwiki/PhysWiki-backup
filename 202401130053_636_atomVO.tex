...
\subsection{原子变量}
\verb`atomic` 是一个模板类，可以通过 \verb`atomic<_typName> varName;` 来声明类型为 \verb`_typName` 的、变量名为 \verb`varName` 的原子变量，前提是 \v\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/6a8cf899ee18cddc.png}
\caption{请添加图片标题} \label{fig_atomVO_4}
\end{figure}
erb`_typName` 类型合法。
