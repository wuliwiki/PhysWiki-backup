...
\item $\varphi$ 的作用主要体现在比较具有相同角频率的两个正弦型函数时。在给定 $\omega$ 的情况下，两个正弦型函数的形状完全相同，但如果它们具有不同的初相 $\varphi$，则它们的图像将在 $x$ 轴方向上发生平移。两个函数的相位便始终保持固定的\textbf{相差（phase difference）}，因此得知初相便可计算不同函数的相差，进而清晰地描述两个周期信号之间的相对位置。
\end{itemize}
...
\end{equation}
$\Delta \varphi$ 直接决定了两个信号在时间轴上的相对位置：
\begin{itemize}
