...
\subsection{不对称势能}
\subsubsection{归一化}
由于势能 $V(x)$ 不对称， 我们仍然可以求出两组线性无关的实函数解 $\psi_{k,1},\psi_{k,2}$， 却无法自动保证它们的奇偶性， 但可以进行正交化以满足\autoref{ScaNrm_eq6}． 先给出结论： 令区间 $[-L,L]$ 外的波函数为
...
\end{equation}
即\textbf{振幅的平均模方}为 $1/\pi$． 当 $A_{+,i} = A_{-,i}$ 时就有\autoref{ScaNrm_the1} 中的归一化系数 $1/\sqrt{\pi}$．
