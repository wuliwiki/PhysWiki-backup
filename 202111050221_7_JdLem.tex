...
但是我们要算的是两条有向线段上的积分，在$\epsilon\to 0$下的极限，得排除掉小圆弧上的积分值．那么小圆弧上的积分值是多少呢？把上下两半小圆弧合起来，就得到一个包围原点的逆时针回路，它的回路积分值同样是\autoref{JdLem_eq7} ．而两个半小圆弧上相对于$x$轴对称的点，也就是共轭的点，其函数值也是共轭的\footnote{比如取上半小圆弧上的点$z$，其函数值为$\E^{\I z}/z$，而其共轭的函数值为$\E^{\I z^*}/z^*$，两个函数值是共轭的．}．而共轭点在求路径积分时，所乘的$\dd$
