...
公式之外的命令如 \verb|\subsection{}|， \verb|\textbf{}|， 常用环境的简单格式以及对他们的自动引用等。 绝大部分支持的命令都可以在菜单栏上找到。 编辑器\textbf{仅支持网站提供的模板}， 整个百科是一个 \verb|document| 环境， 主文件是 \verb|main.tex|， 其他所有 tex 文件都将作为一个 \verb|section| 插入 main.tex 中（使用我们定义的 \verb|\entry{词条名}{词条标签}|）， 不需要手动添加 \verb|\section{词条名}|。
