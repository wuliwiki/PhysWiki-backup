...
%Jier：注释原因：原本的定义是用极大线性无关组定义列秩和行秩，但我改成张成空间维度以后就不需要了。两种定义下，初等变换不改变行秩和列秩 的证明是一样的。
...
给定\enref{矩阵}{Mat}$\mathcal{A}$，将其每一列看成一个列向量，每一行看成一个行向量。所有列向量所张成空间的维度，称为$\mathcal{A}$的\textbf{列秩（column rank）}；所有行向量所张成空间的维度，称为$\mathcal{A}$的\textbf{行秩（row rank）}。
\end{definition}
