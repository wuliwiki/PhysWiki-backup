...
\begin{itemize}
\item 如果没有 \verb`return` 关键字，那么函数的 exit code 就是最后一个命令的 exit code。 例如 \verb`command_exist() { command -v "$1" &> /dev/null }` 可以用于检查一个命令是否存在（包括 alias）。 用法： \verb`command_exist ls; echo $?`。 如果存在则打印 0。
\end{itemize}
