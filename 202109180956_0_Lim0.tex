...
对于\autoref{Lim0_eq1} 可以发现， 一般地如果给定 $\varepsilon  = b\e{-q}\,\, (b > 1)$， 就令 $N = q$， 第 $N$ 项以后的项就满足要求． 这就严谨地证明了 $\lim\limits_{n \to \infty } a_n = \pi$． 

我们来看几个简单的例题，加深一下印象．

\begin{exercise}{}
从定义证明 $\lim\limits_{n\to\infty}1/2^n = 0$．
\end{exercise}

\begin{exercise}{}\label{Lim0_exe1}
考虑数列$a_n=(-1)^n$．这个数列存在极限吗？ （不存在）
\end{exercise}
