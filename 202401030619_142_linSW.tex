...
由于 $L(V \otimes V', W \otimes W')$ 是一个 $\mathbb{F}$-向量空间，我们可以定义映射张量积的数乘和加法。它们和张量积满足，
% Giacomo: 从线性算子的张量积\upref{TPofLO}复制而来。
\begin{enumerate}
\item $(\mathcal A+\mathcal C)\otimes \mathcal B=\mathcal A\otimes \mathcal B+\mathcal C\otimes \mathcal B$
\item $\mathcal A\otimes(\mathcal B+\mathcal D)=\mathcal A\otimes \mathcal B+\mathcal A\otimes \mathcal D$
\item $\lambda(\mathcal A\otimes \mathcal B) = \mathcal A\otimes(\lambda\mathcal B) = (\lambda\mathcal A)\otimes \mathcal B$
\end{enumerate}

如果定义域与值域合适，我们也可以定义映射张量积之间的复合
\begin{equation}
(\mathcal A\otimes \mathcal B) \circ (\mathcal C\otimes \mathcal D)=\mathcal A \circ \mathcal C\otimes \mathcal B \circ \mathcal D~,
\end{equation}
