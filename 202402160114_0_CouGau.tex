...
库仑规范\upref{Cgauge}推导起来较为简单， 用角标 $C$ 表示库仑规范， 则矢势满足 $\div \bvec A_C = 0$， 标势 $\varphi_C$ 满足（\autoref{eq_Cgauge_1}~\upref{Cgauge}）。但原子核的库仑势能一般包含在\autoref{eq_QMEM_1}~\upref{QMEM} 的 $V(\bvec r)$ 而不是 $\varphi$ 中，空间中也无其他电荷，所以有（\autoref{eq_Cgauge_5}~\upref{Cgauge}，\autoref{eq_Cgauge_2}~\upref{Cgauge}）
\begin{equation}
\varphi_C \equiv 0~.
\end{equation}
\begin{equation}
