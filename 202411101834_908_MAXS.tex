...
| 形式化 | 齐次方程 | 非齐次方程 |
| --- | --- | --- |
| 场 | 闵可夫斯基空间 |  ∂_[α] F^βγ] = 0 |  ∂_α F^αβ = μ₀ J^β |
| 势 (任意规范) | 闵可夫斯基空间 |  F^αβ = 2 ∂_[α A_β] | 2 ∂_α ∂^[α A^β] = μ₀ J^β |
| 势 (洛伦兹规范) | 闵可夫斯基空间 | F^αβ = 2 ∂_[α A_β] | ∂_α ∂^α A^β = μ₀ J^β |
| 场 | 任意时空 | ∂_[α] F^βγ] = ∇_[α F^βγ] = 0 | 1/√(-g) ∂_α (√(-g) F^αβ) = ∇_α F^αβ = μ₀ J^β |
| 势 (任意规范) | 任意时空 (带拓扑限制) | F^αβ = 2 ∂_[α A_β] | 2/√(-g) ∂_α (√(-g) g^αμ g^βν ∂_[μ A_ν]) = 2 ∇_α (∇^[α A^β]) = μ₀ J^β |
| 势 (洛伦兹规范) | 任意时空 (带拓扑限制) | F^αβ = 2 ∂_[α A_β] | ∇_α ∇^α A^β - R^β_α A^α = μ₀ J^β |

- 其中 ∂ 表示偏导数，∇ 表示协变导数，g 是度规张量的行列式，μ₀ 是真空磁导率。