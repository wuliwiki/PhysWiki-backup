...
% license Usr
% type Tutor
我们知道，对切向量场积分可以得到一族曲线，比如地球的一系列纬线。如果加上初始条件，便可以确定具体是哪一条曲线。因此，流形$M$上的积分曲线实际上有两个含参变量，设其为$\theta(t,p),\forall p\in M$。我们可以固定$p$点为曲线的初始位置，得到一条曲线：$\theta_p(t)$；亦可以固定$t$，得到$\theta_t(p):M\rightarrow M$，这是光滑的左作用。我们统一这两种视角为：$\theta^p(t_0)=\theta^{t_0}(p)=\theta(t,p)$。
\subsection{全局流}
