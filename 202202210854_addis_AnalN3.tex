...
\item 11.2 如果 $\phi$ 能给每个集合 $A\in \mathscr R$ 指派扩大实数系内的一个数 $\phi(A)$， 就说它是定义在 $\mathscr R$ 上的\textbf{集函数}． 如果能从 $A\cap B=0$ 得出 $\phi(A\bigcup B)=\phi(A)+\phi(B)$， 那么 $\phi$ 就是\textbf{可加的}．
\end{itemize}
