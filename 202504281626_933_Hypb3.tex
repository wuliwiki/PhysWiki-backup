...
双曲线的反射性质与椭圆非常相似，都是指反射光线所在的直线经过另一个焦点。不过，它们之间有一个关键的区别：在椭圆中，从一个焦点发出的光线经过反射后，会实际到达另一个焦点；而在双曲线中，从一个焦点出发的光线经过反射后，本身不会直接到达另一个焦点，而是反射线的延长线穿过另一个焦点。

这种差别的原因在于：椭圆上任意一点到两个焦点的距离之和是定值，因此可以通过实际路径连接两个焦点；而双曲线上任意一点到两个焦点的距离之差是定值，因此需要通过延长线来保持这一特性。

这种反射性质在一些通信设备和雷达设计中尤其重要，因为它能有效地控制信号的传播路径，实现精确的信号收发。

类似地，也可以从几何角度解释双曲线为何具备这样的反射性质。

首先回顾双曲线的定义：
平面上到两个定点 $F_1$、$F_2$ 的距离差的绝对值为定值 $2a$ 的点的轨迹，称为双曲线，即
\begin{equation}
|PF_1 - PF_2| = 2a~.
\end{equation}
...
从物理直觉出发：
设 $l$ 是过 $P$ 的切线，要求 $\angle (F_1P, l) = \angle (PF_2, l)$，这意味着 $PF_1$ 和 $PF_2$ 关于切线对称。而由双曲线的定义，$|PF_1 - PF_2|$是定值，这与反射条件自然吻合。
