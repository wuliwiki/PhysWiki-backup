...
可以证明当 $x_i$ 各不相同时， \autoref{LstSqr_eq2} 存在唯一解： \autoref{LstSqr_eq2} 也可以用范德蒙矩阵\upref{VandDe} $\mat X$ 表示为
\begin{equation}
(\mat X\Tr \mat X) \bvec c = \bvec b
\end{equation}
当 $x_i$ 各不相同时， 范德蒙矩阵的各列都线性无关， 根据\autoref{OvrDet_the1}~\upref{OvrDet}， 系数行列式 $\abs{\mat X\Tr \mat X} > 0$， 方程有唯一解． 证毕．
