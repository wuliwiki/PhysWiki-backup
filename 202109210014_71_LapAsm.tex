...
设实数 $\alpha>0$， 函数 $\phi:X\to Y$ 在区间 $[0,b]$ 上连续， 且当 $x\to0$ 时有渐近展开式
\[
...
证明是直接的计算 固定一个正整数 $n$，按照渐近展开的定义， 存在正数 $\delta >0$ 使得当 $0\leq x<\delta$ 时， $\phi(x)-(c_0+...+c_nx^n)=O(x^{n+1})$． 邻域$B(0,\delta
)$外的点 $x\in[0,b]$ 对积分的贡献是可以忽略的：
$$
