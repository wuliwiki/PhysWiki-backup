...
第一种方法是令\autoref{FSTri_eq1} 中 $l = (b-a)/2$， 把 $f(x)$ 先向左平移 $x_0 = (a+b)/2$， 也就是使原来 $[a,b]$ 的中点移动到坐标原点， 展开成\autoref{FSTri_eq1} 的傅里叶级数， 再向右平移 $x_0$ 即可． 这样\autoref{FSTri_eq1} 变为
\begin{equation}
f(x) = \frac{a_0}{2} + \sum_{n = 1}^\infty a_n \cos (\frac{n\pi}{l}(x-x_) + \sum_{n = 1}^\infty b_n \sin (\frac{n\pi}{l}x)
\end{equation}
