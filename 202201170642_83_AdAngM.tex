...
接下来我会按照一步步特定的步骤找到结果．首先假设有两个粒子，一个的自旋为$s_1 = 2$另一个自旋为$s_2 = 1$，那么假设它们的总自旋$s=3$，并且$m=0$．那么我们首先找到$2\times 1$的那个表格\autoref{AdAngM_fig1} ．然后在最上面的方格上找到$3\quad 0$的那一列，那些分数就是我们所要找的系数（记得带上根号）．
现在我们就有了：
\begin{equation}
