...
= 0~
$$
对于密度 $\rho$ **恒定**的流体介质，称为**不可压缩流体**。
因此，密度 $\rho$ 对时间的变化率
$\displaystyle \left(\frac{\partial \rho}{\partial t}\right)$
和密度的梯度$\displaystyle (\nabla \rho)$都等于 **0**。



在这种情况下，通用的连续性方程：$\frac{\partial \rho}{\partial t} + \nabla \cdot (\rho \mathbf{u}) = 0$简化为：$\rho (\nabla \cdot \mathbf{u}) = 0$进一步假设密度 $\rho$ 是非零常数 $(\rho \neq 0)$，则可将方程两边同时除以 $\rho$，得到：
$$
\nabla \cdot \mathbf{u} = 0~
$$
这一关系 $\nabla \cdot \mathbf{u} = 0$ 表明：

* 流速向量 $\mathbf{u}$ 的**散度为零**；
* 换句话说，对于不可压缩流体，流速场是一个**无散场（solenoidal vector field）**或**散度为零的向量场**。

---

值得注意的是，这一关系可以结合向量拉普拉斯算子展开：
...
* $\vec{\omega} = \nabla \times \mathbf{u}$ 表示**涡量（vorticity）**。
