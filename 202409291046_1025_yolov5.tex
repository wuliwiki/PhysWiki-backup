...
\end{figure}
对于github文件夹是放配置文件的，这个不重要，可以忽略。对于data文件是放一些超参数的配置文件(如.yaml)，是用来配置训练集、测试集、验证集的路径的。还有一些官方提供的测试的图片。特别注意，如果训练自己的数据集的话需要修改这里的yaml文件。但是自己的数据集最好在外面新建一个dataset的文件夹。
