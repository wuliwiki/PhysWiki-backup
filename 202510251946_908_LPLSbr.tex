...
\[
\Delta_{H^{n-1}} f
= \left. \Box\, f\!\left( \frac{x}{q(x)^{1/2}} \right) \right|_{H^{n-1}}~
\]
其中$f\!\left( x / q(x)^{1/2} \right)$表示$f$ 向未来光锥内部的
\emph{零次齐次延拓}（degree-zero homogeneous extension），
而 $\Box$ 为\emph{波动算子}（wave operator）：
\[
\Box
...
该算子也可在极坐标中表示。
设 $(t, \xi)$ 为双曲空间上的球坐标，
其中 $t$ 表示某点 $p$（例如庞加莱圆盘中心）的
\emph{双曲距离}（hyperbolic distance），
$\xi$ 表示测地线方向在 $S^{n-2}$ 上的参数。
此时，双曲拉普拉斯算子可写为：
\[
\Delta_{H^{n-1}} f(t, \xi)
= \sinh(t)^{2-n}
...
这一表达式与球面拉普拉斯算子的形式非常相似，
只是将 $\sin$ 替换为 $\sinh$，
反映出双曲几何与球面几何在曲率符号上的根本对称关系。
