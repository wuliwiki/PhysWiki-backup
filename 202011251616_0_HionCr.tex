...
\end{equation}
对于氢原子， 见\autoref{HyIon2_eq1}~\upref{HyIon2}．
...
P_{j\to i} = \frac{\pi q^2}{c\epsilon_0 m^2 \omega_{ij}^2} \abs{\uvec e \vdot\mel{i}{\bvec p}{j}}^2 s(\omega_{ij})
\end{equation}
...
\end{equation}
对于束缚态 $\ket{i}$， $P_{j\to i}$ 是概率， 而对于连续态的 $\ket{i}$ （如原子电离）， $P_{j\to i}$ 是概率 $\ket{i}$ 的 $\ket{\bvec k}$ 的三维概率密度\upref{PTCont}． 当波包经过以后， $\bvec A = 0$， 有 $\omega_{ij} = k^2/(2m) + I_0$， $-I_0$ 是束缚态 $\ket{j}$ 的能量．
\begin{equation}
...
\begin{equation}
\pdv{\sigma}{\Omega} = \frac{\pi m\omega k q^2}{c\epsilon_0} \abs{\mel{i}{\uvec e \vdot\bvec r}{j}}^2
\end{equation}
速度规范下有（\autoref{HionCr_eq5} ）
\begin{equation} % 已验证与 Merzbaucher eq 19.86 相同
\pdv{\sigma}{\Omega} = \frac{\pi k q^2}{c\epsilon_0 m \omega} \abs{\mel{i}{\uvec e \vdot \bvec p}{j}}^2
\end{equation}
