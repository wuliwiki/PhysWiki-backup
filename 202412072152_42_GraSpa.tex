...
注意到 $\forall f\in\mathcal V(G)$ 可由在 $V(G)$ 上 $n$ 个点处的取值 $f(v_1),\cdots f_(v_n)$ 唯一确定，且 $f(v_i)=f(v_i)\cdot 1$，于是若令 $u_i(v_j)=\delta_{ij},i,j=1,\cdots,n$，其中 $\delta_{ij}$ 试Dirac $\delta$ 函数。那么
\begin{equation}
...
f=\sum_{i=1}^n f_iu_i.~
\end{equation}
由于 $f$ 的任意性，这表明 $\{u_i|i=1,\cdots,n\}$ 是 $\mathcal V(G)$ 的一组基。因此，$(f_1,\cdots,f_n)$ 就是 $f$ 在该基下的坐标。
