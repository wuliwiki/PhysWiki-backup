...
\item dll/so 中的符号可以设置编译器属性（非标准 C/C++）如 GCC 的 \verb`__attribute__((attribute_name))` MSVC 的 \verb`__declspec(attribute_name)`。 其中 \verb`attribute_name` 可以是 \verb`dllimport` 或 \verb`dllexport`。 注意双层括号不可省略为单层。
