...
具体来说，设$f:M\rightarrow N$是光滑映射，$c(t):[0,1]\rightarrow M$是光滑道路。$V$是$M$上沿着该道路的任意切场：$V(t)=v^i(t)e_{i,c(t)}$，其中$\{e_{i,c(t)}\}$为点$c(t)$邻域上的一组基。为方便计，重新表示$V(t)=v^ie_i$。根据前推的定义，我们可以定义切场的推前为$f_*V(t)=v^i\widetilde e_{i,f\circ c(t)}$，其中$\widetilde e_i=f_*e_i$。由于前推是线性映射，因此${\widetilde e_i}$线性无关，是$f\circ c(t)$上的一组基。
