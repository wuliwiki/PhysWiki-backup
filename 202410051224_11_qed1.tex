...
因此我们可以用指数映射表示该李子群的“部分”群元。设任意$X\in \mathfrak{so}^+(1,3)\equiv\opn{Lie} SO^+(1,3),t\in \mathbb R$，则$\E^{tX_e}$表示一条从单位元切空间$T_eSO^+(1,3)$出发的左不变切场的积分曲线。那么该曲线经过的点都能被指数映射所表示。由此观之，其他连通分支的元素均不能被指数映射所表示，毕竟单位元$I$是正规正时矩阵，在$SO^+(1,3)$内，而不同连通分支是彼此不相交的。
%最好补充指数映射是满射的证明。

有意思的是，虽然指数映射并非总是满射，但在$SO^+(1,3)$上可以证明是满的。
\textbf{所以该群群元都可以被指数映射所表示}。因此，研究$SO^+(1,3)$上的李代数是非常有实践意义的。由于$SO^+(1,3)$群包含了通俗的洛伦兹变换，即旋转和推促，所以也有称其为\textbf{狭义洛伦兹变换（restricted lorentz group）}的。为了叙述简洁，下面我们沿用这一称呼。

\subsubsection{狭义洛伦兹变换的群元形式}
...
所以狭义洛伦兹群的李代数为六维，分别是三个转动自由度和三个平动自由度。
