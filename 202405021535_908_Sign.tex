...
\end{equation}
\end{definition}
显然，形如 $\sum\limits_{i=0}^\infty a_iD^i(a_i\in\mathbb{C})$ 的所有记号因子构成的集合 $\qty{\sum\limits_{i=0}^\infty a_iD^i|a_i\in\mathbb{C}}$ 为\enref{域\upref{field} $\mathbb C$ 上的线性空间\upref{LSpace}，而集合中的元素 $\sum\limits_{i=0}^\infty aD^n$ 自然叫作该空间中的向量。
