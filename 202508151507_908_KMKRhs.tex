...
这意味着：模 $n$ 的整数乘法群中每个元素的阶都整除 $\lambda(n)$。卡迈克尔将这样一种元素 $a$ 称为模 $n$ 的本原 $\lambda$-根（primitive $\lambda$-root modulo $n$）：它的最小正整数指数，使得$a^{\lambda(n)} \equiv 1 \pmod{n}$
（不要与模 $n$ 的本原根混淆，卡迈克尔有时会称其为**模 $n$ 的本原 $\varphi$-根）。
