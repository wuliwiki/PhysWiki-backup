...
\addTODO{一般来说，相当长一段时间内你都做不出任何图形界面，也就是你打交道的都是命令行输入和输出。甚至xu}
...
\subsubsection{调试程序}
如果你选择简单的文本编辑器和命令行的环境， 那么命令行调试器几乎是必备的， 详见 “\enref{调试 C++ 程序}{gdbcpp}”。 当然有许多不那么专业的用户选择完全不使用调试器。 g++ 和 icpc 编译器对应的调试器是 \enref{gdb}{gdbNt}， clang++ 编译器对应的是 \enref{lldb}{LLDB}， 二者的用法几乎一样。
