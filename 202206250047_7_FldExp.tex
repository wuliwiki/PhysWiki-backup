...
域单同态总把不可约多项式的一个根映射为另一根．参考后续词条中

将此推论中的$\mathbb{F}$当作$\mathbb{K}_1$，$\sigma(\mathbb{F})$当作$\mathbb{K}_2$，$\overline{\mathbb{F}}$当作$\mathbb{K}_1$和$\mathbb{K}_2$．将$\overline{\mathbb{F}}$中的元素进行等价类划分，等价关系为“关于$\mathbb{F}$共轭”，或者说“是$\mathbb{F}$上同一个不可约多项式的根”．每个等价类里选一个代表元素
