...
交错化映射 $A$ 将任意张量映射为斜对称张量（\autoref{SIofTe_the1}~\upref{SIofTe}），而 $\Lambda(V)$ 的子空间是斜对称子空间，我们试着将 $A$ 作用到  $\Lambda(V)$ 上，以希望其上两元素进行运算还是其上的元素．注意 $ \Lambda(V)$ 上的任意元素 $Q$ 可记作（\autoref{TenAlg_eq5}~\upref{TenAlg} ）
\begin{equation}
Q=\sum_{i=0}^\infty Q_i=(Q_0,Q_1,\cdots),\quad Q_i\in\Lambda^i(V)
\end{equation}

$A:\Lambda^p(V)\rightarrow\Lambda^p(V)$，和 $A$ 的线性性，$\forall Q=\sum_$
\begin{equation}
A()
\end{equation}
