...
\begin{equation}
\Psi(x, t) = \int_0^{+\infty} [C_1(E) \psi_{E,1}(x) + C_2(E) \psi_{E,2}(x)] \E^{-\I E t} \dd{E}
\end{equation}
系数 $C_i(E)$ 由初始波函数决定， 满足
\begin{equation}
\Psi(x, t_0) = \int_0^{+\infty} [C_1(E) \psi_{E,1}(x) + C_2(E) \psi_{E,2}(x)] \E^{-\I E t_0} \dd{E}
\end{equation}
...
\begin{equation}
\int_{-\infty}^{+\infty} \psi_{E,i}(x)\Cj \psi_{E',i'}(x)\dd{x} = \delta(E - E')\delta_{i,i'}
\end{equation}
那么就有
\begin{equation}
C_i(E) = \braket{\psi_{E,i}}{\Psi(t_0)} \qquad (i = 1,2)
\end{equation}
这就是\textbf{能量归一化}． 当然我们同样有不同的归一化方式， 例如类似于\autoref{FreeP1_eq3}~\upref{FreeP1} 的动量归一化， 令\autoref{Sca1D_eq2} 的本征函数为 $\psi_{k}(x)$（$k \in \mathbb R$， $k^2/(2m) = E$）， 满足归一化条件
\begin{equation}
