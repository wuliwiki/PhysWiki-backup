...
线性代数中基底的排列顺序十分重要， 顺序会影响坐标和矩阵的值． 我们通常令 $\ket{l_1, m_1, l_2, m_2}$ （$m_2 = M - m_1$）以 $m_1$ 从大到小的顺序排列， $\ket{L, M}$ 以 $L$ 从大到小排序．

\subsection{CG 系数}
既然每个 $M$ 子空间可以通过 $N_M$ 个正交归一的 $\ket{l_1, m_1, l_2, m_2}$ 基底或者 $\ket{L,M}$ 基底展开， 那么就有必要讨论它们之间的酉变换矩阵 $\mat U_M$． 其矩阵元 $\braket{l_1, m_1, l_2, m_2}{L, M}$ 被称为 \textbf{Clebsch–Gordan 系数}或者简称 \textbf{CG 系数}． 详见 “CG 系数\upref{SphCup}”．
