...
结合电磁感应定律与原有的静场方程，我们就得到了如雷贯耳的\enref{Maxwell方程组 }{MWEq}：
\begin{table}[ht]
\centering
...
同静场问题一样，势的取值依旧具有任意性，只是此时电势与磁势得\textsl{此消彼长}：在磁势加上 $\grad \lambda$的同时，电势得加上 $ - \pdv{\lambda}{t}$，才能再得到\enref{相同的场。}{Gauge}

同样地，我们可以做规范变换；不过与静场问题不同，我们一般不选取 $\div \bvec A =0$ （\enref{库伦规范 }{Cgauge}），反而选取$\div \bvec A = -\mu_0 \epsilon_0 \pdv{\varphi}{t}~$（洛伦兹规范 \upref{LoGaug}）。尽管洛伦兹规范形式上看起来更复杂，但它使势的达朗贝尔方程高度对称、简明（见下）。
