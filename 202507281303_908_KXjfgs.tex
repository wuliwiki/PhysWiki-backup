...
现在，我们可以使用柯西积分公式来计算每一个较小的积分，但在此之前需要对其进行改写以便应用该定理。对于围绕 $C_1$ 的积分，定义$ f_1(z) = (z - z_1)g(z)$。由于路径不包含另一个奇点，因此这是一个全纯函数。我们可以将 $f_1$ 简化为：$ f_1(z) = \frac{z^2}{z - z_2}，$从而得到：$g(z) = \frac{f_1(z)}{z - z_1}$。
