...
下面估计 $D(x)$。而 $D(x)$ 就代表不超过 $x$ 且不能被任何素数 $p > p_j$ 整除的数 $n$ 的个数。显然可以把 $x$ 表示为 $x = n^2 m$，其中 $m$ 无平方因子，也就是说利用算术基本定理将 $m$ 表示为
\begin{equation}
m = 2^{a_1} \times 3^{a_2} \times \cdots \times p_{i}^{a_i } ~,
\end{equation}
则各 $a_i$ 要么为 $0$，要么为 $1$。而 $n$ 必定有 $n \le \sqrt x$。这就指出
