...
\item 最简单的命令： \verb`cmake CMakeList的路径`，可以是相对路径。 如果不想改变源码路径， 就用在别的文件夹。
\item 一般的步骤是，在 \verb`CMakeLists.txt` 的路径下，新建 \verb`build`（名字任意）文件夹，然后 cd 进去，使用 \verb`cmake ..` 在当前目录生成一些缓存文件和 \verb`Makefile`（或 ninja 文件）， 然后 \verb`make -j` 多线程编译。
