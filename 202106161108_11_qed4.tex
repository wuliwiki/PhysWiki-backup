...
\begin{equation}
i \frac{\partial}{\partial t} \psi_{L}=m \psi_{R}, \quad i \frac{\partial}{\partial t} \psi_{R}=m \psi_{L}
\end{equation}
该式耦合了$\psi_{L}$与$\psi_{R}$，然而只要令
\begin{equation}
\psi_{+}=\frac{1}{\sqrt{2}}\left(\psi_{L}+\psi_{R}\right), \quad \psi_{-}=\frac{1}{\sqrt{2}}\left(\psi_{L}-\psi_{R}\right)
\end{equation}
\subsubsection{描述对象}
\begin{enumerate}
\item \textbf{XXXX}
