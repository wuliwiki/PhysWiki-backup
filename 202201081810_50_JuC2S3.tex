...
正因为有了这样的一个源码文件，使得\verb|Programs|项目可以被 Julia 视为一个程序包．更明确地讲，如果存在一个名为`X`或`X.jl`的 Julia 项目，只要该项目包含一个相对路径为`src/X.jl`的源码文件，并且在该文件中定义的最外层模块名为`X`，那么它就是一个有效的程序包．
