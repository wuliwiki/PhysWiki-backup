...
若我们已经解得两个不正交的线性无关解 $\psi_{k,1}, \psi'_{k,2}$， 可以使用施密特正交化\upref{SmdtOt}： 先把 $\psi_{k,1}$ 按照\autoref{ScaNrm_eq4} 即\autoref{ScaNrm_eq7} 归一化， 那么
...
令两组不正交的平面波为
\begin{equation}
\psi_{k,1}(x) = \frac{1}{\sqrt{\pi}}\sin(kx - \delta), \qquad
\psi'_{k,2}(x) = \sin(x)
\end{equation}
代入\autoref{ScaNrm_eq14} 后再做归一化可得
\begin{equation}
\psi_{k,2}(x) = \frac{1}{\sqrt{\pi}}\cos(kx - \delta)
\end{equation}
显然它们是正交的， 因为它们分别是关于 $x = k/\delta$ 的奇函数和偶函数．
\end{example}
