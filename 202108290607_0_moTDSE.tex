...
在位置表象中， 狄拉克 delta 函数\upref{Delta} $\delta(x-x_0)$ 是位置算符 $\hat x$ 的本征函数， 而 $\exp(\I kx)/\sqrt{2\pi}$ 是动量算符 $\hat p$ 的本征函数． 若使用 $\delta(x-x_0)$ 函数基底展开波函数 $\psi(x, t)$， 系数同样为． 若使用 $\exp(\I kx)/\sqrt{2\pi}$ 基底展开态矢， 坐标记为 $\psi(k, t)$， 那么我们就使用了动量表象， $\psi(k, t)$ 就是动量表象下的波函数．
