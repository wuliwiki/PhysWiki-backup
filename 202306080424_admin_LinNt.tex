...
\item 要开启 SetGID， 用 \verb|chmod +s|。 在显示权限时， 文件夹会显示为 \verb|rws|。
\item 所以作为系统管理员，要想让不同组的用户可以访问不同的文件夹，那就首先把每个用户的 \verb|umask| 设为 \verb|007|， 然后把每个文件夹的所有子文件夹和子文件设置为同一个组， 文件权限设为 \verb|664|，
