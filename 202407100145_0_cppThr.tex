...
\item \verb`std::thread th(函数, arg1, arg2, ...)`; 创建一个线程， 调用\verb`函数`（可以是函数指针， 函数对象， lambda）， \verb`arg` 是\verb`函数`的变量。 但如果 \verb`函数` 有参数是 reference， 对应的 \verb`arg1` 等需要用 \verb`std::ref(变量)` 或者 
