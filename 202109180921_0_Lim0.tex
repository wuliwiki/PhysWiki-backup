...
不要误以为这条式子是说当 $n = \infty$ 时， $a_n=\pi$． 有两个理由可以说明这种理解不正确： 首先， $\infty$ 不是一个整数或实数， 不存在 $n=\infty$ 的说法， 这里的$n\to\infty$只是表示 $n$ 的不断增加的过程． 其次， 按照定义每个 $a_n$ 都是有理数，而 $\pi$ 是无理数， 所以不应该有任何一个 $a_n=\pi$． 类比函数 $\sin x = y$ 并不是说 $x = y$， 而是说 $x$ 经过正弦函数作用后等于 $y$．

所以从概念上来说，极限中的“趋于” 和“等于” 是不同的．趋于是数列整体的性质，而不是某一个项性质．我们可以像这样粗略理解 “趋近”：
\begin{itemize}
\item 越来越接近，但不一定相等
