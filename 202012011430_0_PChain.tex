...
\subsection{显含}
在物理中， 尤其在分析力学中， 我们通常会遇见\textbf{显含（explicitly depends on）}的概念． 当\autoref{PChain_eq2} 中的某个 $u_i$ 有 $\pdv*{f}{u_i}$ 不恒为零时， 我们说 $f$ 显含 $u_i$． 


等于某个 $x_j$ 且 的时候， 我们就说函数 $f$ \textbf{显含（explicitly depends on）} $x_j$． 例如令 $u, v$ 都是 $x, y$ 的函数， 又令 $z(x, y) = f(u, v, x)$， 且满足$\pdv*{f}{x}$ 不恒为零， 那么函数 $f$ 显含 $x$． 但注意这里有一个条件是 
