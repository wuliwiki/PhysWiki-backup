...
设 $A$ 为可逆热机，则卡诺定理可以表述为：$\eta_A \ge \eta_B$。由于采用反证法，为此，设 $\eta_A < \eta_B$。也即有 $\frac{W}{Q_1}<\frac{W'}{Q_1'}$。令 $Q_1=Q_1'$，则 $W<W'$，那么可以让 $B$ 热机输出的功的一部分 $W$ 推动热机 $A$ 逆向运行，从低温热源 $\theta_2$ 吸收热量 $Q_2$，运行后输出 $Q_1=Q_1'$ 热量到高温热源 $\theta_1$。此时热机 $B$ 还有多余的功 $W'-W$ 可以输出。等价于
