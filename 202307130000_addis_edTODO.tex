...
\item 首先插入 \verb|images| 表的 \verb|hash, ext, figures, author, time|， 再插入 \verb|figures| 表的 \verb|id, caption, author, entry, image|， 再插入 \verb|entries| 表格的 \verb|figures| （注意格式，不要把已有的数据覆盖）。 
