...
SLISC 的底层使用 intel 的 Math Kernel Library (MKL) 来实现线性代数运算， 可以保证性能．

所有的函数都尽量首先使用和 Lapack 类似的指针接口实现， 然后再封装一层更友好的接口．

SLISC 实现了矩阵剪切类， 这会有少量的额外运算， 但却可以方便使用． 由用户自己权衡是使用 slicing 还是指针 interface．
