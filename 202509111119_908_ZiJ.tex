...
\caption{} \label{fig_ZiJ_3}
\end{figure}
\begin{itemize}
\item 集合$A = {1, 2}$是集合$B = {1, 2, 3}$的真子集，因此表达式
$A \subseteq B（A \subseteq B）$和 **A ⊊ B（A \subsetneq B）** 都成立。
\item 集合 **D = {1, 2, 3}** 是集合 **E = {1, 2, 3}** 的子集（但不是**真子集**），因此
**D ⊆ E（D \subseteq E）** 成立，而 **D ⊊ E（D \subsetneq E）** 不成立（即为假）。
