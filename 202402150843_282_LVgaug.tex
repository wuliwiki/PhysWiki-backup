...
本文使用原子单位制\upref{AU}。 和长度规范中的思路一样， 我们只在使用偶极子近似\upref{DipApr}时讨论\textbf{速度规范（velocity gauge）}。 用角标 $V$ 表示速度规范， 先从规范不变的哈密顿算符（\autoref{eq_LenGau_2}~\upref{LenGau}）出发
...
\begin{equation}
H_V = H_0 - \frac{q}{m} \bvec A_V \vdot \bvec p~.
\end{equation}
薛定谔方程为
