...
\end{theorem}
设 $\sigma$ 是一个n阶置换， 其可以写成s个不相交的轮换的积， $\sigma=\tau_1\tau_2\cdots\tau_n$, 我们定义一个函数 $N(\sigma)=(-1)^{n-s}$。设 $(a\ b)$ 为任意对换， 则不难验证 $N((a\ b)\sigma)=-1N(\sigma)$\footnote{只需注意到 $(a\ b)(a\ c\cdots\ d\ b\ e\cdots\ f)=(a\ c\ \cdots\ d)(b\ e\ \cdots f)$ 和 $(a\ b)(a\ c\ \cdots\ d)(b\ e\ \cdots\ f)=(a\ c\ \cdots\ d\ b\ e\cdots\ f)$。}, 则 $N(\sigma)=N(\tau_1\cdots \tau_k)I)=(-1)^kN(I)$, 同理有 $N(\sigma)=(-1)^sN(I)$, 这就说明$k,s$的奇偶性相同。(其中I为恒同映射)
