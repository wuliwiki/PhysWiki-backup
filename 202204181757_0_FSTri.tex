...
在实际操作中要如何选择各种方法呢？ 前两种方法实际上是等价的， 把\autoref{FSTri_eq9} 利用两两角和公式（\autoref{TriEqv_eq1}~\upref{TriEqv}）拆开即可证明． 另一个主要的考虑因素是\autoref{FSTri_ex1} 提到的龙格效应． 若 $f(a) = f(b)$， 那么用前两种方法把 $f(x)$ 拓展周期为 $2l$ 的函数后， $[a,b]$ 的两个端点
