...
\subsection{OS 重整化方案}
不同的重整化方案实际上是对裸场的不同的归一化。设裸场的拉氏量为 $\mathcal{L}=\frac{1}{2} (\partial_\mu \phi_0)^2 - \frac{1}{2} m_0^2 \phi^2 - \frac{\lambda_0}{4!}\phi_0^2$，对裸场而言，根据其 Lehmann-Kallen 谱表示，其两点格林函数在物理质量处的留数为 $Z$ 而不是 $1$，也就是说 $\bra{\Omega}\phi(x) \ket{k}=\sqrt{Z}e^{-ikx}$。因此我们可以对裸场进行归一化得到重整化的场 $\phi_0 = \sqrt{Z}\phi$
\subsubsection{重整化条件}
