...
从某种意义上来说，\autoref{LieMor_def1} 中$\mathfrak{g}$的每个元素都被$\rho$映射到了$



\begin{example}{伴随表示}
设$\mathfrak{g}$是域$\mathbb{F}$上的一个李代数，$x\in\mathfrak{g}$，定义映射$\opn{ad}_x :\mathfrak{g}\to\mathfrak{g}$为：对于任意$y\in\mathfrak{g}$，有$\opn{ad}_x(y)=[x, y]$．
\end{example}
