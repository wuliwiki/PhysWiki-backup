...
\item git 在 pull 的时候如果本地和上有有不同的 commit， 通常会默认自动 merge 并产生新的 commit。 若要默认报错什么都不做，用 \verb`git config --global pull.rebase false; git config --global merge.ff only`
\end{itemize}
