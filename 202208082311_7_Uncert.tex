...
\begin{definition}{相容性与不确定性原理}
设$X, Y$分别是两个可观测量对应的算符，若$[X, Y]=0$，则称它们是\textbf{相容}的．
\end{definition}

相容的可观测量具有一个关键性质：

\begin{theorem}{}\label{Uncert_the1}
...
因此，如果$[X, Y]=0$且无简并，根据\autoref{Uncert_the1} ，可知当我们对量子态进行$X$测量后，结果是$Y$的本征矢，故再进行$Y$测量时所得结果是确定的．

但是，如果$[X, Y]\neq 0$，那么两个可观测量通常不共享本征矢，此时先进行$X$测量后，再测量$Y$，所得结果是不确定的．这就是不确定性原理．
