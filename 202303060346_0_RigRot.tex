...
我们可以把刚体的定轴转动类比质点的直线运动\upref{VnA1}， 把 $\theta$， $\omega$ 和 $\alpha$ 分别类比为直线运动中的位置 $x$， 速度 $v$ 和加速度 $a$， 因为后三个变量之间的数学关系是完全相同的： $v, \omega$ 分别是 $x, \theta$ 的一阶导数， 而 $a, \alpha$ 分别是 $x, \theta$ 的二阶导数。 于是我们可以立即同理得到\textbf{匀变速转动}（即 $\alpha$ 为常数）的一些公式， 如（引用未完成）
\begin{gather}
...
\tau_z = I \alpha
\end{equation}
其中 $\tau_z$ 是外力对系统的力矩\upref{Torque}在转轴方向的分量， $I$（有时也用$J$）是一个和刚体的质量分布有关的量角转动惯量，它相当于线性力学中的质量。
