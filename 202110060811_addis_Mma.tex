...
\item \verb|&@| 可以把右边的结果在左边用 \verb|#| 代替， 如 \verb|#.#\[Transpose] &@RandomInteger[{-5, 5}, {6, 6}]| 可以随机生成对称ju'z
\end{itemize}
...
\item \verb|Eigenvectors[{{1,2},{3,4}}]|， \verb|Eigenvalues[{{1,2},{3,4}}]| 计算矩阵本征值． 或者 \verb|EigenSystem[矩阵]|， 一次返回 \verb|{本征值，本征矢}|，和分别计算是一样的． 注意返回的本征矢矩阵每行是一个本征矢而不是每列． 注意返回的本征矢未必是归一化也未必是正交的．
