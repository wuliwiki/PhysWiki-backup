...
\begin{itemize}
\item 速度 $v(t)$ 比位移 $x(t)$ 超前 $\displaystyle\frac{\pi}{2}$；
\item 加速度 $a(t)$ 比速度 $v(t)$ 超前 $\displaystyle\frac{\pi}{2}$，即与位移完全反相。
\end{itemize}
...
在上述推导中，角频率 $\omega$ 在每次求导时都会作为系数出现，影响速度和加速度的大小。这一点在振动系统中尤为关键。
