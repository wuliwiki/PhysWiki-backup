...
\end{equation}
该式在傅里叶分析和量子力学中有重要应用（下文以及\cite{Arfken}）， 但证明起来比较困难暂且从略（参考\autoref{eq_sinc_2}~\upref{sinc}）。 注意即使对于 $x \ne 0$ 上式也不存在 $n\to\infty$ 的极限而是不断摆动， 可见\autoref{eq_Delta_2} 是十分不严谨的。 \autoref{fig_Delta_1} 的三个例子也较为片面。
\end{example}
\begin{figure}[ht]
\centering
