...
全集是一个相对的概念，是为了规定研究范围而定下来的，请不要把它和“包含世间万事万物的集合”这句话等同起来\footnote{这个集合是不存在的。}。
...
什么叫区间呢？形象上的理解就是在数轴上割出来一段，比如从a到b的一段吧。区间有三种，但它们其实对应的都是同一段内容，他们的长度也都一样，区别就只是能不能取到端点：
\begin{itemize}
\item 两端都能取到，称为\textbf{闭区间}，记作$[a,b]$；
\item 都取不到，称为\textbf{开区间}，记作$(a,b)$；
\item 一边能取到，一边取不到，称为\textbf{半开半闭区间}，记作$[a,b)$或$(a,b]$。
\end{itemize}
