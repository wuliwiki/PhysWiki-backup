...
为了得到正确的作用量，我们分析量纲：作用量的量纲为质量乘长度 $ML$（只需注意 $S_{\text{粒子}}=-m\int\dd\tau$，当然这里选定了 $c=1$）。度规无量纲，$R$ 由度规和两个导数缩并构造，因此量纲为 $1/L^2$，所以 $\int\dd{}^4x\sqrt{-g(x)}R(x)$ 的量纲为 $L^4/L^2=L^2$。因此 $k$ 的量纲需要为 $M/L$，注意到引力常数 $G$ 的量纲为 $L/M$ （只需注意 $V=-\frac{GMm}{r}$）
