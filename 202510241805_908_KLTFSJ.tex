...
\subsection{性质}
\begin{itemize}
\item 由于任意 Kuratowski 闭包算子都是单调的（isotonic），而包含映射显然也是单调的，因此我们得到一个单调的 Galois 连接：$\langle\, \mathbf{c} : \wp(X) \to \mathrm{im}(\mathbf{c});\ \iota : \mathrm{im}(\mathbf{c})\hookrightarrow \wp(X) \,\rangle$，其中 $\wp(X)$ 被视为按包含关系排列的偏序集（poset），而 $\mathrm{im}(\mathbf{c})$ 是 $\wp(X)$ 的一个子偏序集。事实上，可以容易验证，对于任意 $A \in \wp(X)$ 与 $C \in \mathrm{im}(\mathbf{c})$，
有 $\mathbf{c}(A) \subseteq C$ 当且仅当 $A \subseteq \iota(C)$。
\item 若 $\{A_{i}\}_{i \in \mathcal{I}}$ 是 $\wp(X)$ 的一个子族，则有
\[
\bigcup_{i \in \mathcal{I}} \mathbf{c}(A_{i})
\subseteq
\mathbf{c}\!\left( \bigcup_{i \in \mathcal{I}} A_{i} \right),
\qquad
\mathbf{c}\!\left( \bigcap_{i \in \mathcal{I}} A_{i} \right)
\subseteq
\bigcap_{i \in \mathcal{I}} \mathbf{c}(A_{i})~
\]
\item 若$A,B \in \wp(X)$，则有$\mathbf{c}(A) \setminus \mathbf{c}(B) \subseteq \mathbf{c}(A \setminus B)$
\end{itemize}
\subsection{以闭包形式表述的拓扑概念}
\subsubsection{细化与子空间}
若两 Kuratowski 闭包算子 $\mathbf{c}_{1}, \mathbf{c}_{2} : \wp(X) \to \wp(X)$ 满足对任意 $A \in \wp(X)$ 都有 $\mathbf{c}_{2}(A) \subseteq \mathbf{c}_{1}(A)$，则它们分别诱导的拓扑 $\tau_{1}, \tau_{2}$ 满足 $\tau_{1} \subseteq \tau_{2}$，反之亦然。换句话说，$\mathbf{c}_{1}$ 支配（dominates）$\mathbf{c}_{2}$ 当且仅当后者所诱导的拓扑是前者所诱导拓扑的一个细化（refinement），或等价地，有 ${\mathfrak{S}}[\mathbf{c}_{1}] \subseteq {\mathfrak{S}}[\mathbf{c}_{2}]$\(^\text{[13]}\)。例如，$\mathbf{c}_{\top}$ 显然支配 $\mathbf{c}_{\bot}$（后者只是 $\wp(X)$ 上的恒等映射）。由于相同结论也可以通过将 $\tau_{i}$ 替换为包含其所有补集的族 $\kappa_{i}$ 得出，若我们在 $\mathrm{Cls}_{\text{K}}(X)$ 上定义偏序关系$\mathbf{c} \leq \mathbf{c}' \iff \mathbf{c}(A) \subseteq \mathbf{c}'(A)$ 对所有 $A \in \wp(X)$ 成立，而在 $\mathrm{Atp}(X)$ 上定义细化序，则可得出映射 ${\mathfrak{S}}$ 是偏序集之间的反单调映射（antitonic mapping）。

在任意诱导拓扑中（相对于子集 $A$），闭集诱导出新的闭包算子，该算子只是原闭包算子在 $A$ 上的限制：$\mathbf{c}_{A}(B) = A \cap \mathbf{c}_{X}(B)$，对所有 $B \subseteq A$ 成立\(^\text{[14]}\)。
\subsubsection{连续映射、闭映射与同胚}
一个函数$f : (X, \mathbf{c}) \to (Y, \mathbf{c}')$在点$p$处连续，当且仅当$p \in \mathbf{c}(A) \Rightarrow f(p) \in \mathbf{c}'(f(A))$它在处处连续，当且仅当
$$
f(\mathbf{c}(A)) \subseteq \mathbf{c}'(f(A))~
$$
对所有 $A \in \wp(X)$ 成立 \(^\text{[15]}\)。映射 $f$ 是闭映射（closed map）当且仅当上述包含反向成立\(^\text{[16]}\)；若它既连续又为闭映射，即满足等号成立时，$f$ 是同胚（homeomorphism）\(^\text{[17]}\)。
\subsubsection{分离公理}
设 $(X, \mathbf{c})$ 为一个 Kuratowski 闭包空间。则：
\begin{itemize}
\item $X$ 为 $T_{0}$ 空间，当且仅当 $x \neq y$ 蕴含 $\mathbf{c}(\{x\}) \neq \mathbf{c}(\{y\})$\(^\text{[18]}\)；
\item $X$ 为 $T_{1}$ 空间，当且仅当对所有 $x \in X$，有 $\mathbf{c}(\{x\}) = \{x\}$\(^\text{[19]}\)；
\item $X$ 为 $T_{2}$ 空间（Hausdorff 空间），当且仅当 $x \neq y$ 时，存在 $A \in \wp(X)$ 使得$x \notin \mathbf{c}(A)$ 且 $y \notin \mathbf{c}(\mathbf{n}(A))$，
其中 $\mathbf{n}$ 为补集算子\(^\text{[20]}\)。
\end{itemize}
\subsubsection{接近性与分离}
若 $p \in \mathbf{c}(A)$，则称点 $p$ 与子集 $A$ 是“接近的”（close） \(^\text{[21]}\)。该概念可用于在集合的点与子集之间定义一种邻近关系（proximity relation）。

对于 $A, B \in \wp(X)$，当且仅当$(A \cap \mathbf{c}(B)) \cup (B \cap \mathbf{c}(A)) = \varnothing$时，称它们是“分离的”（separated）。空间 $X$ 是连通的（connected），当且仅当它不能表示为两个分离子集的并 \(^\text{[22]}\)。
\subsection{另见}
\begin{itemize}
  \item 拓扑空间范畴的各种刻画（Characterizations of the category of topological spaces）
  \item Čech 闭包算子 — 闭包算子（Čech closure operator – Closure operator）
  \item 闭包算子 — 数学算子（Closure operator – Mathematical operator）
  \item 闭包代数 — 代数结构（Closure algebra – Algebraic structure）
  \item 库拉托夫斯基的闭包–补运算问题（Kuratowski's closure–complement problem）
  \item 前闭包算子 — 闭包算子（Preclosure operator – Closure operator）
  \item 前拓扑空间 — 广义拓扑空间（Pretopological space – Generalized topological space）
  \item 拓扑空间 — 具有“接近”概念的数学空间（Topological space – Mathematical space with a notion of closeness）
\end{itemize}
\subsection{注释}
\begin{enumerate}
  \item Kuratowski (1922)。
  \item Monteiro (1945)，第 160 页。
  \item Pervin (1964)，第 44 页。
  \item Pervin (1964)，第 43 页，习题 6。
  \item Kuratowski (1966)，第 38 页。
  \item Arkhangel'skij 与 Fedorchuk (1990)，第 25 页。
  \item “Moore closure”，nLab，2015 年 3 月 7 日。检索于 2019 年 8 月 19 日。
  \item Pervin (1964)，第 42 页，习题 5。
  \item Monteiro (1945)，第 158 页。
  \item Pervin (1964)，第 46 页，习题 4。
  \item Arkhangel'skij 与 Fedorchuk (1990)，第 26 页。
  \item 当 $\lambda = \aleph_{0}$ 时的证明可见 Stack Exchange 上的 “Is the following a Kuratowski closure operator?!”（2015 年 11 月 21 日）。
  \item Pervin (1964)，第 43 页，习题 10。
  \item Pervin (1964)，第 49 页，定理 3.4.3。
  \item Pervin (1964)，第 60 页，定理 4.3.1。
  \item Pervin (1964)，第 66 页，习题 3。
  \item Pervin (1964)，第 67 页，习题 5。
  \item Pervin (1964)，第 69 页，定理 5.1.1。
  \item Pervin (1964)，第 70 页，定理 5.1.2。
  \item 证明可见相关链接。
  \item Pervin (1964)，第 193–196 页。
  \item Pervin (1964)，第 51 页。
\end{enumerate}
\subsection{参考文献}
\begin{itemize}
  \item Kuratowski, Kazimierz (1922) [1920], Sur l'opération A de l'Analysis Situs（关于分析位形中的 A 运算），Fundamenta Mathematicae，第 3 卷，第 182–199 页。
  \item Kuratowski, Kazimierz (1966) [1958], Topology, Vol. I，J. Jaworowski 译，Academic Press 出版，ISBN 0-12-429201-1，LCCN 66029221。
  \item —— (2010). On the Operation Ā Analysis Situs，ResearchGate，译者 Mark Bowron。
  \item Pervin, William J. (1964), 编者 Ralph P. Boas Jr., Foundations of General Topology，Academic Press 出版，ISBN 9781483225159，LCCN 64-17796。
  \item Arkhangel'skij, A.V.; Fedorchuk, V.V. (1990) [1988], 编者 R.V. Gamkrelidze, A.V. Arkhangel'skij, L.S. Pontryagin, General Topology I, 收录于 ncyclopaedia of Mathematical Sciences 第 17 卷，D.B. O'Shea 译，柏林：Springer-Verlag，ISBN 978-3-642-64767-3，LCCN 89-26209。
  \item Monteiro, António (1945), Caractérisation de l'opération de fermeture par un seul axiome（通过单一公理刻画闭包运算）{Portugaliae Mathematica，第 4 卷，第 4 期，第 158–160 页，MR 0012310，Zbl 0060.39406。
\end{itemize}
\subsection{外部链接}
\begin{itemize}
  \item \href{https://ncatlab.org/nlab/show/Alternative+Characterizations+of+Topological+Spaces}{Alternative Characterizations of Topological Spaces}
\end{itemize}
