...
\autoref{FldExp_def2} 中要给定$R$来讨论其子环$S$，是为了保证$a$已经定义好了与$S$中各元素的运算．如果有别的办法定义，那自然不需要那么麻烦．比如我们在\autoref{FldExp_exe1} 中讨论的$\mathbb{Q}(\sqrt{2})$，虽然$\sqrt{2}$不在$\mathbb{Q}$中，但我们已经定义好了它与各有理数的运算关系．这个运算关系是怎么确定的呢？那就是其在$\mathbb{Q}$上的\textbf{最小多项式}，$$
