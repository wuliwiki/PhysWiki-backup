...
同样，在\autoref{RicciC_eq3} 中交换$j$和$k$指标即可证明．
...
用\autoref{RicciC_eq1} 可以证明上标版本的恒等式$\partial_aR_{kij}^r+\partial_iR_{kja}^r+\partial_jR_{kai}^r=0$，把$g_{rt}$乘进去，展开\footnote{就是说，$\partial_a(g_{rt}R^{r}_{kij})=g_{rt}\partial_aR^r_{kij}+R^r_{kij}\partial_ag_{rt}$．}，再把\autoref{RicciC_eq5} 和\autoref{RicciC_eq4} 代进去，即可得证．
