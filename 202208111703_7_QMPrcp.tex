...
以一维单粒子的连续情况为例：位置表象下的波函数$\psi(x)$，可以视为取位置算符的本征矢量（在位置表象下的波函数表示为$\delta(x-x_0)$\footnote{这是\textbf{狄拉克 delta 函数}\upref{Delta}．}，在动量表象下则为$\exp(x_0p/\I\hbar)$．）线性组合而成；动量表象下的波函数$\phi(p)$，可以视为取动量算符的本征矢量（在位置表象下表为$\exp{(\I p_0 x/\hbar)}$，在动量表象下则为$\delta(p-p_0)$．）线性组合而成．你可以粗略理解为，$\psi(x_0)$就是线性组合中基向量$\delta(x-x_0)$的系数，即
\begin{equation}
\psi(x) = \int \psi(x_0)\delta(x-x_0)\dd x_0
\end{equation}
这也确实符合\textbf{狄拉克 delta 函数}\upref{Delta}的性质．
