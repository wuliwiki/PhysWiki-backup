...
当系统处于非平衡态时，会自发地向平衡态过度，从而产生\textbf{动量、能量、质量}等宏观的流动，这些过程统称为\textbf{耗散过程}．输运过程在微观上就是耗散过程．例如当热学平衡条件不满足时，有温度梯度，从而有热传导方程（能量的传递）；力学平衡条件不满足时，有黏滞现象（动量的传递），从而有牛顿黏滞定律；化学平衡条件不满足时，有扩散现象（质量的传递），从而有菲克（Fick）扩散定律．我们先给出这三个定律的表达式，可以从中观察到相似性：
\begin{align}
...
其中牛顿黏滞方程的 $J_p$ 为动量流密度，菲克定律的 $J_m$ 为质量流密度．

经过下面的分析，我们将发现，这三种输运过程在微观上的机制本质是相同的．

先简化模型，我们设系统的各个宏观量（例如质量密度、能量密度、动量密度）在同一高度上处处相同