...
\item 强烈推荐不要用命令行编译而是 CLion 打开 Makefile 然后选择 julia-debug 作为 target。 经过一段时间的加载，所有的符号都可以跳转到定义。编译以后支持逐行调试。当然用命令行编译再用 \enref{Custom Build Targets}{CLionN} 也是可以的。
