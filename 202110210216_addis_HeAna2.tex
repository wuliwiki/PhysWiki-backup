...
\end{equation}
理论上要解本征态 $\ket{H,L,M}$， 就用许多无限长的长方形网格， 每张网格是一个 $(l_1,l_2,L)$ 分波， 网格之间被 $1/r_{12}$ 势能耦合． 网格上 $r_1$ 在区间 $[0,a]$， $r_1 = a$ 处边界条件是波函数为零， 另外两条边 $r_1 = r_2 = 0$ 也有波函数为零． $r_2$ 的区间为 $[0,\infty)$， 这样才能保证 $E$ 可以连续取值而不是离散的． 所以我们无法控制 $r_2\to \infty$  的渐进相位， 可以记为 $\delta_l(E)$．
