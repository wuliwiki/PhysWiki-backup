...
当建立起某一对象的数学语言时，我们应当首先借助集合论的基本工具。首先，图包含两个代表点和边的集合。而边是连接两个端点的边，因此边可以通过指定它的端点确定。也就是说，可以把边认为是点集的二元子集。当边有向时，只需用有序的点对来表示即可。为方便描述起见，我们将用一个记号来表示集合的 $n$ 元子集构成的全体。
