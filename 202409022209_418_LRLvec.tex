...
解得（选取特殊的$\theta$起点）
\begin{equation}
u = \frac{mk}{L^{2}} + A \cos \theta$，即$r = \frac{1}{u} = \frac{1}{\frac{mk}{L^{2}} + A \cos \theta} = \frac{\frac{L^2}{mk}}{1 + \frac{L^2}{mk} A \cos \theta} ~~
\end{equation}
借此我们可以表示出速度
\begin{align}
\bvec v &= \dot r \hat r + r \dot \theta \hat \theta ~,\\
&=-\frac{L}{m}{du}{d\theta} \hat r + \frac{1}{u} \frac{Lu^{2}}{m} \hat \theta ~,\\
