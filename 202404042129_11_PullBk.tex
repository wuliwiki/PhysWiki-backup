...
\begin{equation}
f^*(\omega\wedge \eta)(X_{1},X_{2}...X_{k+l})=\omega\wedge \eta(f_*(X_{1}),f_*(X_{2})...f_*(X_{k+l})})~.
\end{equation}
根据张量的外积定义，可简化右侧的表达式为：
\begin{equation}
\begin{aligned}
f^*(\varepsilon^I)\wedge f^*(\varepsilon^J)(e_{p_1},e_{p_2}...e_{p_{k+l}})=\frac{(k+l)!}{k!l!}\opn{Alt}(f^*(\varepsilon^I)\otimes f^*(\varepsilon^J))(E)
\end{aligned}
\end{equation}
现证第二条。由第一条得：
\begin{equation}
