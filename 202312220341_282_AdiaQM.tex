...
\addTODO{第一项在绝热近似下也很小呀，为什么忽略第二项而不是第一项？因为相对于第二项，第一项是 1/0 （把\autoref{eq_AdiaQM_4} 代入）？}
...
\end{equation}
是一个块对角的酉矩阵\upref{UniMat}，也就是每个对角块都分别是一个酉矩阵。这类似于薛定谔方程的演化子，且每个子空间独立演化，概率保持不变。
