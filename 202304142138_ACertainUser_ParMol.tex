...
我们还是以内能为例：
$$ U (p, T, n_1, n_2, ...) = \sum_i n_i u_i(p, T, x_1, x_2, ...) $$
对集合公式求关于$n_i$的偏导数\footnote{这个做法其实是不严谨的，因为$x_i$其实也是隐含$n_i$的。事实上，严谨的做法是先定义偏摩尔量为 ${u_i} = \left(\pdv{U}{n_i}\right)_{p,T,n_j, j \neq i}$，然后根据齐次方程欧拉定理得到上述的集合公式 $ U = \sum_i n_i u_i $}，
$$
