...
这意味着：
\begin{itemize}
\item $\tan x$ 左移 $\frac{\pi}{2}$ 变为 $-\cot x$，而 $-\cot x$ 仍是奇函数，因此 $\left(\frac{\pi}{2},0\right)$ 是 $\tan x$ 的对称中心；
\item 同理，$\left(-\frac{\pi}{2},0\right)$ 是 $\cot x$ 的对称中心。
\end{itemize}
值得注意的是，类似于 $y=\frac{1}{x}$ 在 $x=0$ 处的情况，$x=\frac{\pi}{2}$ 并不属于 $\tan x$ 的定义域，而这也使得这个对称中心容易被忽略。
