...
可以参考 \href{https://code.google.com/archive/p/femtolisp/wikis/Manual.wiki}{语法手册（非官方）} 和 GPT。

\subsection{背景}
\begin{itemize}
\item 见 Julia 第一次演讲
\item Lisp 适合用于做编译器，C 代码生成，符号计算等
\end{itemize}
...
\item \verb`aliases.scm` 中定义了许多 scheme 中的函数别名等。 所以 \verb`.scm` 结尾的文件都需要先运行该文件才可以正常工作。
\item 本文中若讲解 \verb`aliases.scm` 定义的语法需要注明。
\end{itemize}

\subsection{类型}
\begin{itemize}
\item \verb`(typeof 123)` 和 \verb`(typeof '123)` 都是 \verb`fixnum`，数字的符号和数字不区分
...
\item 暂时没有找到幂算符 \verb`^` 或函数，可以自己定义 \verb`(define (expt a b) (exp (* b (log a))))`。 现在就可以 \verb`(expt 2 4)` 得 \verb`16`。
\end{itemize}
...
\item \verb`(cadr 变量)` 等效于 \verb`(car (cdr 变量))`，以此类推。 所以取第 n 个元素（从 0 数）就是 \verb`cadd...dr` 中就有 n 个 d。
\item \verb`(list 1 2 3)` 等效为 \verb`(cons 1 (cons 2 (cons 3 '())))`， 本质上是用二叉树表示单链表。 类型仍然是 \verb`pair`。
\item \verb`(list* 1 2 3)` 会把最后一个入参直接放在二叉树的末尾，不以 null 结尾，显示为 \verb`(1 2 . 3)`。 一般用法是 \verb`(list* 1 2 '(3 4))` 得到正常的列表 \verb`(1 2 3 4)`
...
\item 全局变量 \verb`*argv*` 里面是命令行参数的列表。REPL 中第一个是 \verb`flisp` 命令本身。 若 \verb`./flisp my_file.lsp arg1 arg2` 则第一个是 \verb`my_file.lsp`。
\item \verb`(load "路径/文件名")` 可以执行另一个文件中的 lsp 源码，返回最后一个语句的值（若为空则返回 \verb`#t`）
\item \verb`(make-system-image "文件名")` 可以把当前的环境（已经定义的函数、全局变量等）保存到一个二进制文件中。
...
\item 函数 \verb`and, or, not` 是逻辑算符，只有入参是 \verb`#t, #f` 才能正常工作。
...
\item \verb`(memq 元素 列表)` uses \verb`eq?` comparison — identity comparison. 即判断一个元素是否在列表中。
\item \verb`(memv 元素 列表)` uses \verb`eqv?` comparison — similar but a bit looser (numbers, characters).
\item \verb`(member 元素 列表)` uses \verb`equal?` comparison — deep structural equality.
\item \verb`(reverse! '(1 2 3))` 反转列表得到 \verb`(3 2 1)`
\item \verb`(unwind-protect (princ "doing work\n") (princ "cleanup\n"))` 在执行第一个入参的命令时无论是正常退出还是抛出异常，都会执行第二个入参的命令。
\item \verb`(with-bindings ((全局变量 值) ...) 语句 ...)` 和 \verb`(let ...)` 类似， 但是临时改变全局变量但语句结束后全局变量又恢复原来的值。 全局变量叫做 Dynamic binding，局部变量叫做 Lexical binding。
\end{itemize}

\subsubsection{字典}
\begin{itemize}
\item \verb`(define t (table))` 创建空字典
\item \verb`(define t (table :a 1 :b 2 :name "Ada"))` 初始化字典，注意任何对象都能作为 key
\item \verb`(table? t)` 检查类型，\verb`(typeof t)` 返回 \verb`table`
\item \verb`(put! t :a 42)` 插入或更新，返回字典本身
\item \verb`(get t :a)` 查询
\item \verb`(get t :some_name 默认值)` 
\item \verb`(has? t :a)` 是否存在 key
\item \verb`(del! t :a)` 删除键值对
\end{itemize}
...
\subsubsection{循环}
\begin{itemize}
\item \verb`(for-each print '(1 2 3))`，也可以支持多入参的函数
\begin{lstlisting}[language=none]
(for-each (lambda (a b)
  (print (+ a b))) '(1 2 3) '(10 20 30))
\end{lstlisting}
\item 循环一般用函数递归，应该熟悉。例如计算 \verb`1+2+3+4`
\begin{lstlisting}[language=none]
(define (sum-up i sum)
  (if (> i 5)
      sum
...
\subsubsection{异常}
\begin{itemize}
\item \verb`(trycatch 要执行的命令 异常处理函数)`
\begin{lstlisting}[language=none]
(trycatch
  (begin
    (princ "Before error\n")
...
\item 任意数量参数函数：\verb`(define (函数名 参数1 参数1 . 参数n) 函数体)`。 \verb`参数n` 相当于 matlab \verb`varargin`。 例如 \verb`(define (show first . rest) (princ "First: " first "\n") (princ "Rest: " rest "\n"))`， 然后 \verb`(show 1 2 3 4)` 会显示 \verb`First: 1 Rest: (2 3 4)`。
...
\subsection{路径与文件读写}
\begin{itemize}
\item \verb`(path.cwd)` 返回当前目录， \verb`(path.cwd 新目录)` 改变当前目录 \verb`新目录` 可以是相对的。
...
\item 例如 \verb`aliases.scm` 定义了一个类似于 \verb`std::stringstream` 的构造函数
\begin{lstlisting}[language=none]
