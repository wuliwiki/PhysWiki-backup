...
\item 强烈推荐 CLion 打开 Makefile 然后选择 debug 作为 target。Executable 填写 \verb`G:\OneDrive\github\etc-3rd-party\julia\usr\bin\julia-debug` 经过一段时间的加载，所有的符号都可以跳转到定义。编译以后支持逐行调试。当然用命令行编译再用 \enref{Custom Build Targets}{CLionN} 也是可以的。
...
\end{lstlisting}
\item 最新的 v1.12.0 中 \verb`make debug` 失败，提了 \href{https://github.com/JuliaLang/julia/issues/57675}{issue} 正在修复。撤回到 v1.10.10 编译成功
...
\item \verb`jl_fl_parse()` 负责调用 \verb`jlfrontend.scm` 中的 \verb`jl-parse-all或one` 函数
\end{itemize}
