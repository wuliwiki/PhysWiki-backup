...
就像其他类型的方程一样，只要一个表达式符合双曲线标准方程的形式，就可以确定它的图像是双曲线。有了学习圆和椭圆时的经验，自然也会希望在这里了解双曲线的参数方程。不过，在已知的三角函数关系中，并没有可以直接推导出双曲线参数方程的公式。在双曲线的参数方程中，使用了\enref{双曲函数}{TrigH}，但在高中阶段并不要求掌握。这里给出相关内容，主要是为了知识的完整性。
...
相信读者在学习过程中一定会产生两个问题：第一，为什么双曲线会有渐近线？第二，这些渐近线究竟是什么样的？或者说，它们的表达式是什么？

先来看第二个问题。其实在\autoref{fig_Hypb3_8} 中，就已经能够初步观察到渐近线的存在。从图中可以看到，双曲线的两条渐近线都经过一个由实顶点和虚顶点确定的矩形的四个顶点。具体来说，这两条渐近线经过点 $(a,b)$ 和 $(a,-b)$，并且也通过原点。因此，可以写出它们的方程为：
在前面的内容中已经可以看出，渐近线是双曲线区别于椭圆、圆等曲线的重要特征之一。下面简单介绍一下双曲线的渐近线及其性质。

简单来说，\textbf{渐近线（asymptotes）}可以理解为图像在某个方向上靠近但永远不会到达的“边界线”或“吸引线”。当沿着某个方向不断延伸时，双曲线和渐近线之间的距离会越来越小，在无穷远处趋近于零。关于双曲线的渐近线，相信读者一定会有这样两个问题。一则，为什么双曲线会有渐近线？二则，这些渐近线到底是什么样的？或者说，它们的表达式是什么？

关于第二个问题，其实在\autoref{fig_Hypb3_8} 中就已初现端倪。从图中可以发现，双曲线的两条渐近线都经过一个由实顶点和虚顶点确定的矩形的四个顶点。具体来说，渐近线经过点 $(a,b)$ 和 $(a,-b)$。又从图中可以看到它们也经过原点，因此可以写出渐近线的方程为：
\begin{equation}\label{eq_Hypb3_16}
y=\pm\frac{b}{a}x~.
\end{equation}

当然，上面只是通过图像观察得出的结论，严谨讲来仅仅能作为一种猜想。如果感兴趣的话，还可以越过高中知识的藩篱，到无穷远处看一看。

对于\autoref{eq_Hypb3_4} ，如果想要探究$x$趋于正无穷时的样子，需要把它放到分母上来考虑，即变形为：
\begin{equation}
\frac{1}{a^2} - \frac{y^2}{x^2b^2} =  \frac{1}{x^2} ,\qquad(a>0,b>0)~.
\end{equation}
当 $x$ 趋于无穷大时，式中等号右侧的部分会趋于0，可以忽略不计，而等号左侧由于y在这时也会趋于无穷大，所以这一项不确定。不过把右侧变成0之后，就可以移项化简了，即：
\begin{equation}
...
这里在开方时，为了保证结果的完整性，于是有了正负号。这样就得到了\autoref{eq_Hypb3_16} 。
...
当然，这样的推导也并不严谨，在学习了\enref{泰勒展开}{Taylor}”后，可以稍微严谨地给出证明：由\autoref{eq_Hypb3_4} 得
\begin{equation}
...
当然，对于渐近线的深入研究也可以参考\enref{渐近线}{Asmpto}，在此就不予赘述了。
