...
可以通过将带有维度的因子 \( \varepsilon_0 \) 和 \( \mu_0 \) 吸收到单位中（从而重新定义它们），来简化电荷、电场和磁场的定义，以便于理论计算。配合洛伦兹力定律中的量值变化，这将产生相同的物理效果，例如带电粒子的轨迹或电动机所做的功。这些定义在理论物理和高能物理中更为常用，因为此时将电场和磁场设置为相同单位可以简化电磁张量的表示：统一电场和磁场的洛伦兹协变对象将包含具有统一单位和维度的分量。\(^\text{ 这样的修正定义通常在高斯（CGS）单位制中使用。使用这些定义，俗称“在}\}
[9]:vii高斯单位制中”，麦克斯韦方程变为：\(^\text{[11]}\}
