...
\item \verb`%"my_var"`: This is the name of the parameter. The \% symbol is used to denote local entities (like parameters and local variables) in LLVM IR. The name "my_var" is a descriptive name that indicates the parameter is a 64-bit integer named abc.
...
\item top: is a label. In LLVM IR, labels are used to mark specific points in the code, often used as targets for branches or jumps. The label top typically marks the entry point of the function, though the name top is arbitrary and could be anything.
\end{itemize}
