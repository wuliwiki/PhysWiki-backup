...
下面的内容均研究函数$y=f(x)$，假设点$(x,y)$代表着函数上的一个点，函数$y_1=f(x_1)$和点$(x_1,y_1)$分别代表着变换后的函数和上的一个点。介绍也会在“坐标系在变换”的这个视角上进行。如果觉得比较困难，可以先只了解平移部分，待习惯后再了解后面的两个部分。
...
\subsubsection{伸缩}
有了是上面的经验伸缩就容易得多了，如果我们希望把函数的图象在y方向上拉伸，那么就要把坐标系在y方向上压缩，因此需要y
\subsubsection{*旋转}
