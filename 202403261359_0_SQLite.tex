...
\subsection{CRUD}
CRUD 就是 create, read, update, delete。 总的来说是一件很繁琐的事情。

例如很常见的一个繁琐情况是，程序里面有一个表的完整的新版本，想把表中所有记录（也可以是表中满足某个条件的所有记录）删除再重新写。 但这样会导致程序很慢，尤其是如果有很多记录且新版本只修改了少数几条记录。另一个坏处就是会导致 dump 出来的 sql 文本文件有大量不一致，即使一摸一样的记录 dump 位置也改变了，不利于版本管理。 sql 没有一个稳定的 dump 算法，需要借助第三方工具。

高性能的做法需要多得多的编程： 首先把整个表读到程序中（读比写快得多）， 在程序中鉴定三种情况：新纪录、需要更新的记录、需要删除的记录。 然后把使用 \verb`INSERT`， \verb`UPDATE`， \verb`DELETE` 三种不同的命令分别处理即可。 由于数据结构的问题， 很难把这个过程用一个函数或函数模板实现（或许可以）。
