...
\end{equation}
对于任何初值$u(x_0)=u_0$都可积, 当且仅当$U\times V$中存在由图像给出的, 经过点$(x_0,u_0)$的$n$维子流形$\{(x,u(x)):x\in U\}$, 而且与向量场
$$
X_i=\frac{\partial}{\partial x^i}+f_i^\alpha(x,u)\frac{\partial}{\partial u^\alpha}
$$
相切. 根据费罗贝尼乌斯定理, 可以给出可积性的充分必要条件: \autoref{FrobTh_eq1}的可积性等价于$U\times V$上的$n$维分布$\text{span}(X_1,...,X_n)$的可积性. 