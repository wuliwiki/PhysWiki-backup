...
前面我们已经知道了，$\delta_b^a$在指标表示法中的作用是“改变指标名称”，因此在彭罗斯记号中表现为一根导线，这样其它张量和它相接就直接改变了导线另一端的指标名称，如\autoref{fig_PnrsGN_5} 。类似地，度规$g_{ab}$是给定流形上最重要的一个双线性函数，因此特别地表示为一根端口都朝下（即指标升降）的导线，如图
...
\caption{$\delta$符号和度规的彭罗斯图形表示法。$\delta^a_b$被表示为一根导线，上端口是$a$，下端口是$b$。图中左上角} \label{fig_PnrsGN_5}
\end{figure}
