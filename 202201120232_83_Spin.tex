...
解：首先我们和之前处理$S_x$一样，先要找到$S_y$的本征值和本征旋量：
\begin{equation}
\frac{\hbar}{2}\pmat{0\quad \I\\-\I\quad0}\pmat{\alpha\\\beta}=\pm\frac{\hbar}{2}\pmat{\alpha\\\beta} \Rightarrow \pmat{\beta\\\alpha}=\pm\pmat{\alpha\\\beta}
\end{equation}
所以$\beta=\pm\alpha$，显然$\bvec S_x$的对应本征值为$+\hbar/2$的归一化的本征旋量为：
\begin{equation}
\chi_+^{(x)}=\frac{1}{\sqrt{2}}\pmat{1\\1}
