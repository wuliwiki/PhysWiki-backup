...
\item 作为一个内容冲突的防范， 每次以任何形式保存词条后， 记录下后台词条文件的 hash， 下次保存再次检查该文件的 hash． 如果和记录的不同， 那么提示 \verb|服务器上该文件发生了修改， 和这里的内容冲突， 这是一个异常， 冲突版本已保存， 请马上停止编辑并联系管理员|． 并把当前编辑器的内容保存到 \verb|文件名-conflict.tex|． 如果程序没有 bug， 这个异常就不会被触发．
