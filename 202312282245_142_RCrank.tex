...
令 $X, Y$ 为有限维线性空间， 线性算符 $A:X \to Y$ 的零空间为 $X_0$， 其厄米共轭\upref{HerMat}（也叫自伴算符） $A\Her: Y \to X$ 的零空间（\autoref{def_LinMap_2}~\upref{LinMap}）为 $Y_0$； 令两算符的值空间分别为 $Y_1 = A(X)$， $X_1 = A\Her(Y)$。 那么 $X_0, X_1$ 是 $X$ 中的正交补（\autoref{def_OrthSp_1}~\upref{OrthSp}）， $Y_0, Y_1$ 是 $Y$ 中的正交补。 即
\begin{equation}
