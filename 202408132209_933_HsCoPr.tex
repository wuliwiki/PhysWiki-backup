...
若逻辑连接词连接的两个命题$A,B$只有同时成立，整个复合命题才为真，则称这个逻辑连接词为\textbf{且}（and，也称为\textbf{而且}、\textbf{同}、\textbf{合取}），记作$A\land B$\footnote{符号不做硬性要求，下同。}。
...
\item 在集合论中，$A \cap B$表示集合$A$和$B$的交集，其中的元素必须既属于$A$“且”属于$B$。“且”就出现在交集的定义中。
\item “且”常被用来描述多重条件，比如证明某个四边形是矩形，就需要满足以下两个条件同时成立：四边形的对角线相等、四个角都是直角。这两个条件就是“且”的关系。
\end{itemize}
