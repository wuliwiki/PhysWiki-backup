...
\item  特别注意显示成小数形式的 \verb|sym| 类型数字可能是不精确的． 例如运行 \verb|x = sym('12.3')| 或者 \verb|x = sym('1.23e1')|， 输出显示 \verb|x = 12.3|， 这说明 \verb|x| 是一个变精度浮点数， 存在误差． 若把它进行运算， 则该误差会传递且会在计算中产生新的误差． 而 \verb|x = sym('123/10')| 显示为 \verb|x = 123/10| 该数无论参与任何计算都没有丝毫误差．
\end{itemize}
