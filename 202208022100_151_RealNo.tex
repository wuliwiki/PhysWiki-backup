...
有理数集 $\mathbb{Q}$ 的一个非空真子集 \alpha  称作一个戴德金分割，若满足：
 \text{a}) 对于 r\in \alpha ，任给 s\in\mathbb{Q} ，若 s<r 则 s\in\alpha 
 \text{b}) 对于 r\in \alpha ，都存在 s\in\alpha 使得 s>r 
我们把每个分割称为一个实数，全体实数的集合称作实数集，记作 \mathbb{R} 
\end{definition}
戴德金分割实际上就是对引言中的“左”的刻画， \text{a}) 刻画了“左”向左无限延伸， \text{b}) 刻画了“左”中无最大数．其几何意义很明显：向左无限延伸，向右无限逼近某个点却无法达到这个点
...
若 b 为 D 的一个下界，且对于任意 \varepsilon>0 都存在 x\in D 使得 x<b+\varepsilon ，则称 b 为 D 的上确界，记作：
 b=\inf D\\ 
引理：上确界是上界中的最小者，下确界是下界中的最大者

定义 3  实数的序关系
\subseteq 就是 \mathbb{R} 上的序关系
为什么能够这样定义？下面我们来证明 \subseteq  满足序关系的基本性质：
1. 完全性： \forall \alpha,\beta \in\mathbb{R} ， \alpha\subseteq \beta   与 \beta\subseteq \alpha  必有一项成立
...
2. 传递性： \forall \alpha,\beta,\gamma \in \mathbb{R} ，若 \alpha\subseteq\beta 且 \beta\subseteq\gamma ，则 \alpha\subseteq\gamma 
因此， \subseteq 是 \mathbb{R} 上的序关系，而 \subseteq 又对 \mathbb{R} 满足：
3. 自反性： \forall \alpha\in\mathbb{R} ， \alpha\subseteq\alpha 
4. 反对称性： \forall \alpha,\beta\in\mathbb{R} ，若 \alpha\subseteq\beta 且 \beta\subseteq\alpha ，则 \alpha=\beta 
于是，\subseteq 还是 \mathbb{R} 上的偏序关系
...
加法具有我们熟悉的性质：
 1. 交换律： \forall\alpha,\beta\in\mathbb{R} ， \alpha+\beta=\beta+\alpha 
2. 结合律： \forall \alpha,\beta,\gamma\in\mathbb{R} ， (\alpha+\beta)+\gamma=\alpha+(\beta+\gamma) 
3. 加法单位元（零元）：存在唯一的 O\in\mathbb{R} 使得 \forall \alpha\in\mathbb{R} 有 \alpha+O=\alpha 
证明：我们考虑全体负有理数的集合 \mathbb{Q}^-=\{p\in\mathbb{Q}\mid p<0\} ，容易断定其为实数，考察任意 \alpha\in\mathbb{R} 
...
4. 加法逆元： \forall\alpha\in\mathbb{R} ，存在唯一的 \beta\in\mathbb{R} 使得 \alpha+\beta=O 
