...
解析函数和矢量分析有紧密的关联， 如果把复平面对应到 $x$-$y$ 直角坐标系， 那么 $f(z)$ 的实部和虚部 $u(z), v(z)$ 看作矢量的 $x,y$ 分量， 那么 $f(z)$ 就定义了复平面上的矢量场\upref{Vfield}． 另外， $f(z)$ 的复共轭函数 $f^*(z)$ 对应的二维矢量场为
\begin{equation}
\bvec f(x,y) = u(x,y) - v(x,y)\I 
\end{equation}
