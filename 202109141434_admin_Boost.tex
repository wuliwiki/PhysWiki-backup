...
但如果想获取最新版本可以直接\href{https://www.boost.org/users/history/version_1_76_0.html}{下载}． Boost 绝大部分库都可以是 header only 的． 用 \verb|tar -xzvf boost_1_76_0.tar.gz| 解压以后， 在解压文件夹中运行 \verb|sudo ./bootstrap.sh|， 然后 \verb|sudo ./b2 install| 即可（默认需要 \verb|gcc| 编译器， 注意编译时间可能较长）． 如果只需要头文件， 用 \verb|./b2 headers|．
