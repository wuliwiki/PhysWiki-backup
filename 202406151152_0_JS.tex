...
\item \textbf{default export} 如 \verb|export default {age, name};| 也可以在定义前面直接用 \verb`export default const age = 40;`。 一个模块文件中只能有一个 default export。
\item 也可以个变量或函数重命名 \verb`import { greet as sayHello } from './myModule.js';`
