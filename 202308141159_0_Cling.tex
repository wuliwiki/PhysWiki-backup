...
\item 如果想在本地使用 Jupyter Notebook 中的 Cling （推荐）， 用 \verb|conda|\upref{CondaN} 安装即可（不支持\footnote{最后测试时间 2023/8/13} Windows 版的 conda， 或者 WSL2\footnote{2023/8/13： WSL1 不行，安装成功，但启动 jupyter notebook 失败。} 安装 conda。
