...
首先要提出来的，是所有相差$2k\pi$的角都是相等的。这一点之前提过很多次，但后面还会用到，因此先提一下。

因为始边是在x轴上的，所以所有关于x轴对称的点$P,Q$，都可以看作是角的终边往相反方向旋转的角。（这里我希望再细讲讲），因此，所有所有关于x轴对称的点$P,Q$对应的角$\alpha,\beta$的关系是：
\begin{equation}
\alpha=-\beta+2k\pi,\qquad(k\in\mathbb{Z})~.
\end{equation}

对于关于y轴对称的点，则（补充），因此所有关于y轴对称的点$P,Q$对应的角$\alpha,\beta$的关系是：
\begin{equation}
\alpha=\pi-\beta+2k\pi,\qquad(k\in\mathbb{Z})~.
\end{equation}

对于关于原点对称的点，则（补充）相当于是分别沿x和y对称一次，因此所有关于原点对称的点$P,Q$对应的角$\alpha,\beta$的关系是：
...
首先来看关于 $x$ 轴对称的情况。由于三角函数的角是以 $x$ 轴正方向为始边定义的，因此若两个角 $\alpha, \beta$ 对应的点 $P, Q$ 关于 $x$ 轴对称，则 $Q$ 的位置可以看作是 $P$ 沿 $x$ 轴翻转，即 $P$ 的终边逆时针旋转 $\beta$ 所在的角度，而 $Q$ 的终边则顺时针旋转相同的角度。这意味着，若 $P$ 对应的角为 $\beta$，则 $Q$ 对应的角为 $-\beta$。由于单位圆的周期性，相差 $2k\pi$ 的角等价，此时有：

\begin{equation}
\alpha = -\beta + 2k\pi, \qquad (k\in\mathbb{Z})~.
\end{equation}

下面再来推导关于 $y$ 轴对称的角度关系，这可以分成两终边在上半平面或下半平面两种情况。

若角 $\alpha, \beta$ 的终边对应的点 $P, Q$ 关于 $y$ 轴对称，且两者都位于上半平面，则此时二者是早已熟悉的互补关系，即$\alpha+\beta=\pi+2k\pi$，从而：
\begin{equation}
\alpha = \pi-\beta + 2k\pi, \qquad (k\in\mathbb{Z})~.
\end{equation}
...
\begin{equation}
\alpha{\prime} = -\alpha + 2m\pi, \quad \beta{\prime} = -\beta + 2n\pi~.
\end{equation}
此时，$\alpha’, \beta’$ 在上半平面，它们满足互补关系$\alpha’+\beta’=\pi+2k\pi$
...
由于 $k$ 的任意性，我们可以通过 $k$ 变换，使得：
￼
因此，两种情况可以合并为统一表达式：
\begin{equation}
\alpha = \pi - \beta + 2k\pi, \qquad (k\in\mathbb{Z})~.
\end{equation}
这说明无论 $\alpha, \beta$ 处于上半平面还是下半平面，它们关于 $y$ 轴对称的角度关系都是相同的。




若两个角 $\alpha, \beta$ 对应的点 $P, Q$ 关于 $y$ 轴对称，则 $Q$ 的位置可以看作是 $P$ 的终边绕 $y$ 轴翻转。
则有两种情况，一种情况是二者在上半平面，则此时是早已熟悉的二者互补，即$\alpha+\beta=\pi+2k\pi$，从而：
\begin{equation}
\alpha = \pi-\beta + 2k\pi, \qquad (k\in\mathbb{Z})~.
\end{equation}
如果二者在下半平面，可以认为二者是原本互补的角改为顺时针旋转，即$-(\alpha+\beta)=\pi+2k\pi$，从而：
\begin{equation}
\alpha = -\pi-\beta + 2k\pi, \qquad (k\in\mathbb{Z})~.
\end{equation}
由于$k$的任意性，可以将两种情况合并记作：
\begin{equation}
\alpha = \pi-\beta + 2k\pi, \qquad (k\in\mathbb{Z})~.
\end{equation}
这意味着，若 $P$ 对应的角为 $\beta$，则 $Q$ 对应的角可以看作 $\pi - \beta$，即：

\begin{equation}
\alpha = \pi - \beta + 2k\pi, \qquad (k\in\mathbb{Z})~.
\end{equation}

这一关系表明，关于 $y$ 轴对称的角，其三角函数值可能保持不变或变号，具体取决于三角函数的定义。

关于原点对称

若两个角 $\alpha, \beta$ 对应的点 $P, Q$ 关于原点对称，则可以看作是 $P$ 先关于 $x$ 轴对称，再关于 $y$ 轴对称，或者反之。因此，若 $P$ 对应的角为 $\beta$，则 $Q$ 对应的角为 $\pi + \beta$，即：

\begin{equation}
\alpha = \pi + \beta + 2k\pi, \qquad (k\in\mathbb{Z})~.
\end{equation}

这一关系表明，关于原点对称的角，其三角函数值可能完全变号（即 $u, v$ 均取相反数）。

这些角度变换公式是诱导公式的基础，它们体现了三角函数在单位圆上的对称性，并且可以帮助我们快速推导出不同象限角的三角函数值。
