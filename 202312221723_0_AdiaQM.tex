...
\end{equation}
其时间导数恒为零，根据（\autoref{eq_AdiaQM_10}） $\mat A$ 恒为零。 所以即使开始时存在简并，也能使\textbf{绝热近似精确成立}（\autoref{eq_AdiaQM_2} ）！ 把\autoref{eq_AdiaQM_12} 代入\autoref{eq_AdiaQM_8} 得
\begin{equation}
...
\end{equation}
其中 $\mat I$ 是单位矩阵， $\mat H^1$ 与时间无关。 \textbf{那么绝热近似就是精确的}。
...
通解为
\begin{equation}
\Psi(t) = \sum_n C_n \bvec\psi_n \E^{\I\theta_n(t)}~.
\end{equation}
