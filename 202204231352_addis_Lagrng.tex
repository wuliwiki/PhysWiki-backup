...
为了进一步明确这里的动能和势能函数的定义， 令系统中每个质点的位置矢量为 $\bvec r_j$， 都可以看作 $(q, t)$ 的函数． 那么质点的速度 $\dot{\bvec r}_j$ 就是 $(q,\dot q, t)$ 的函数（见\autoref{TotDer_eq1}~\upref{TotDer}）． 所以系统动能定义为
\begin{equation}
T(q, \dot q, t) = \frac{1}{2}\sum_j m_j\dot {\bvec r}_j^2
\end{equation}
而势能 $V$ 可以先看成所有 $\bvec r_j$ 和 $t$ 的函数， 在直角坐标系中， 我们要求势能函数和每个质点受到由势能产生的力满足
\begin{equation}
\bvec F_j^{(V)} = - \grad_j V(\bvec r_1,\bvec r_2, \dots, t) = -\pdv{V}{x_j}\uvec x - \pdv{V}{y_j}\uvec y - \pdv{V}{z_j} \uvec z
\end{equation}
由于每个 $\bvec r_j$ 是 $(q, t)$ 的函数， 所以 $V$ 可以进一步看成 $(q, t)$ 的函数． 注意该式并不是每个质点的合力， 质点除了可能受到势能产生的力， 还可能受约束力， 我们将在下文讨论．
...
我们以下先通过一些例子来学习如何使用拉格朗日方程， 并引入一些常见的概念， 而把方程的证明或推导留到 “达朗贝尔定理\upref{dAlbt}” 以及 “哈密顿原理\upref{HamPrn}” 中．
