...
网页版百科分为两个界面：
\begin{itemize}
\item \textbf{原界面} \verb|wuli.wiki/online|，文章以 html 方式存储。文章 \verb|wuli.wiki/online/AU.html| 中的 AU 是文章的 id， 每个文章都有自己的唯一 id， 即使文章被重命名也不会变。  
\item \textbf{新界面} \verb|wuli.wiki/book|，文章存储在数据库中，增加了评论，搜索等功能，目录也更美观。文章内容全部来自原界面，后台定时爬取。新界面文章 \verb|wuli.wiki/book/AU|。
