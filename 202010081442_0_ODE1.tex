...
\subsection{线性}
若干函数 $f_1(x), f_2(x), \dots$ 的线性组合\footnote{线性组合是线性代数中的概念， 线性代数中， 线性空间都可以做线性组合， 例如几何矢量（\autoref{GVecOp_eq1}~\upref{GVecOp}）． 特定函数的集合可以看作线性空间（\autoref{LSpace_ex2}~\upref{LSpace}）．}定义为他们分别乘以一个常数再相加
\begin{equation}
c_1 f_1(x) + c_2 f_2(x) + \dots
\end{equation}
由于求导操作是线性的
