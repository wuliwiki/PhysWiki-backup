...
接下来，为了更清晰地理解六个三角函数的几何含义，可以通过对比分析锐角情况下任意角的三角函数定义，并探讨这些函数名称的由来。

首先，研究的角 $\alpha$ 如 \autoref{fig_HsTrFu_5} 所示，称为正角。在此前的学习中，已经了解两个角互为余角的条件，即它们的角度之和为 $90^\circ$。换句话说，余角是 $90^\circ$ 减去给定正角后剩余的角。

接下来，引入弦、切、割的概念。对于圆而言\footnote{关于曲线的切线和割线，可以参见\enref{切线与割线}{TanL}。}：
\begin{itemize}
\item 连接圆上两点的线段，称为弦\footnote{如果将弦与其对应的弧看作一把弓，那么矢的位置正好对应于这把弓上的箭。}。
\item 过圆外一点作直线，与圆相交于两个不同的点，该直线称为割线。
\item 过圆外一点作直线，仅与圆相切于唯一一点，该直线称为切线。
\end{itemize}

这些几何概念在三角函数的定义和命名中具有重要作用。例如，正切与余切函数的名称来源于切线的概念，而正割与余割函数则与割线的概念密切相关。这些命名方式反映了三角函数与几何的紧密联系。

接下来，为了更清晰地理解这六个三角函数的几何含义，通过对照分析在锐角情况下任意角的三角函数的几何定义，并探讨这些函数名称的由来：

首先，研究的角$\alpha$如\autoref{fig_HsTrFu_5} 所示，称作正角，在之前学习过，两个角互余就是他们的角度之和为$90^\circ$，也就是说余角是垂角减去正角后剩余的角。
然后是弦、切、割的概念，对于圆而言\footnote{关于曲线的切线和割线可以参见\enref{切线与割线}{TanL}。}：
\begin{itemize}
\item 连接圆上两点的线段，称作\textbf{弦}\footnote{如果把弦和弧的组合看做一把“弓”的话，矢的位置正好就是这把弓上的“箭”。}。
\item 过圆外一点作直线，与圆交于相异两点，称作\textbf{割线}。
\item 过圆外一点作直线，与圆交于唯一一个点时，称作\textbf{切线}。
\end{itemize}
...
它们在坐标系中标示，如\autoref{fig_HsTrFu_3} 所示。参照上面的介绍可以清晰地看到：
sin\theta
