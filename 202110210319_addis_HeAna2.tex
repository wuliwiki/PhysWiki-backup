...
但波函数有 6 维度， 我们只确定了三个量子数 $H,L,M$， 所以还可以规定在 $r_2\to\infty$ 时， 某个 $(l_1,l_2)$ 分波是束缚态和库仑波函数 $Z=1$ 的乘积 $\ket{n_1,l_1}\ket{F_{l_2}(r_2)}$， 所有其他的 $(l_1,l_2)$ 分波在 $r_2\to\infty$ 波函数都消失． 这样就可以把这个态记为 $\ket{E,L,M; l_1,l_2,k_2}$． 分号用于提醒后面的量子数只是无穷远处的边界条件． 如果把这些态用 CG 系数线性组合一下， 就可以得到 $\ket{E,L,M; l_1,l_2,k_2}$．
