...
\subsubsection{电荷守恒}
同质量一样\footnote{防杠声明：严格来说，在相对论中是质量-能量守恒。}，\enref{电荷是守恒的}{ChgCsv}。也就是说，如果一个区域内有电荷的流出(电流)，那么这个区域内的电荷量就会相应地减小。
$$
\oint \bvec j \vdot \dd{\bvec s}  =  - \dv{t} \int \rho \dd{V} \qquad \sum_i I_i = -\dv{q}{t}~,
$$
其中，$\bvec j$是电流密度，$\rho = \dv{q}{V}$是电荷密度，$I>0$表示流出该区域的电流，否则是流入的电流。上式的微分形式为
$$
