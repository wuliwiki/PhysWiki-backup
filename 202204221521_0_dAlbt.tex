...
\pentry{广义力\upref{LagEqQ}， 矢量的求导法则\upref{DerV}， 全导数\upref{TotDer}}
...
注意在这个例题中， 约束力无论是虚功还是真实的做功都为零．


事实上， 对于不含时约束（即约束方程中不出现时间， 即 $\bvec r_j$ 不显含时间， $\pdv*{\bvec r_j}{t} = \bvec 0$）， 约束力的总时功也为零． 证明： 约束力的功率为
\begin{equation}
\dot W^{(c)} = \sum_j \bvec F_j^{(c)} \vdot \dot {\bvec r}_j = \sum_j \bvec F_j^{(c)} \vdot \sum_i \pdv{\bvec r_i}{q_i} \dot q_i
\end{equation}
对不含时约束， ， 所以使用\autoref{dAlbt_eq3} 可知 $\dot W^{(c)} = 0$．
