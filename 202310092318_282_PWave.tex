...
k = \frac{2\pi}{\lambda}~.
\end{equation}
式中 $\cos$ 函数的参数 $\phi = (k x - \omega t + \phi_0)$ 叫做\textbf{相位（phase）}， 相位取决于位置和时间。 令 $x=0$， $t=0$， 相位为 $\phi_0$， 是一个常数， 叫做\textbf{初相位}。 即使对同一个平面简谐波， 初相位也和坐标系的选取有关： 例如把坐标系平移后， 令新坐标为 $x' = x - \Delta x$， 那么新坐标系中的初相位就是 $\phi'_0 = \phi_0 + k\Delta x$。 由 $\cos$ 的周期性， 理论上可以给初相位 $\phi_0$ 加减 $2\pi$ 的任意整数倍而不影响波的形状。
