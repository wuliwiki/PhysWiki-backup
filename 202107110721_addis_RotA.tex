...
\pentry{空间旋转矩阵\upref{Rot3D}，圆周运动的速度\upref{CMVD}， 右手定则\upref{RHRul}}

\footnote{参考 Wikipedia \href{https://en.wikipedia.org/wiki/Rodrigues'_rotation_formula}{相关页面}．}直角坐标系中，某点 $\bvec r=(x,y,z)\Tr$ 以单位矢量 $\uvec A=(A_x, A_y, A_z)\Tr$ 为轴按右手定则转动 $\theta$ 角的得到的点 $\bvec r'=(x',y',z')\Tr$ 可用\textbf{罗德里格斯旋转公式（Rodrigues' rotation formula）}计算
\begin{equation}
\bvec r' = \bvec r \cos\theta + \uvec A \cross \bvec r \sin\theta + \uvec A (\uvec A \vdot \bvec r) (1 - \cos\theta)
\end{equation}

不难证明这是一个线性变换\upref{LinMap}， 可以用矩阵 $\mat R_\theta$ 表示：
\begin{equation}
