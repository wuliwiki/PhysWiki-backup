...
海森堡绘景要求，态右矢$\ket{s}$不变，而算符和基都改变，并且同一个算符在给定基下的矩阵与薛定谔绘景中相同，即
\begin{equation}\label{HsbPic_eq16}
\bra{a_i; 0}X^{(S)}\ket{a_j; 0} = \bra{a_i; t}X^{(H)}(t)\ket{a_j; t}
\end{equation}
...
利用\autoref{HsbPic_eq16} 可得
\begin{equation}
\bra{a_i; 0}X^{(S)}\ket{a_j; 0} = \bra{a_i; 0}\mathcal{U}(t)X^{(H)}(t)\mathcal{U}(t)\ket{a_j; 0}
\end{equation}
