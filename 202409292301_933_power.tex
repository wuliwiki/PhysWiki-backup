...
\frac{f(x_1)-f(x_2)}{x_1-x_2}=\frac{(x_2)^a(\left(\frac{x_1}{x_2}\right)^a-1)}{(x_1-x_2)}~.
\end{equation}

由于\autoref{eq_power_1} 中的分母和$x_2^a$为正，因此讨论分子的符号。设$\displaystyle p=\frac{x_1}{x_2}$，由于$x_1>x_2$，可知$p>1$。从而，$a>0$时，$p^a>1$，$a<0$时，$\displaystyle p^a=\left(\frac{1}{p}\right)^{|a|}<1$。可知$a>0$时，\autoref{eq_power_1} 的值大于$0$，函数在第一象限是递增的；反之$a<0$时，函数则是递减的。
