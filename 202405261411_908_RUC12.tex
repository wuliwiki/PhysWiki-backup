...
\begin{enumerate}
    \setcounter{enumi}{5}
    \item 证明 $\\frac{C_V}{Nk}$ 是常数
    \item 求 $\\lambda$ 与 $T$ 的关系，验证 $n \lambda^3$ 能否满足经典极限条件。($k=1.38 \times 10^{-38} \\ \text{J/K}$, $T=300 \\ \\text{K}$, $c=3 \\times 10^8 \\ \\text{m/s}$, $m=9.1 \\times 10^{-31} \\ \\text{kg}$ 计算时只需求出 $n \\lambda^3$ 数量级即可)
\\end{enumerate}
...
\\begin{enumerate}
    \\item 态密度 $D(\\epsilon)$
    \\item 当 $a=1$ 和 $a=2$ 时，分别求费米能级
    \\item $U$ 与 $T$ 的关系，高温、低温下的极限
    \\item $C_v$ 与 $T$ 的关系，高温、低温下的极限及物态方程