...
\item \verb`lstlisting` 环境（见 sample）转换后开头为：\verb|```语言 标题|，结尾 \verb|```|。 其中 \verb`标题` 可选。
\item \verb`\begin{equation}\begin{aligned} ... \end{aligned}\end{equation}` 环境开头变为 \verb`:::eqa`
\item \verb`equation` 环境开头变为 \verb`:::eq`， 结尾 \verb`:::`。 如果里面直接嵌套了 \verb`aligned` 环境，即 ， 那么转换时就把 \verb`\begin{aligned}` 和 \verb`\end{aligned}` 删掉。 转换回去时， 如果公式代码中有单独的 \verb`\\`（不包括连续超过两个的），就把 \verb`\begin{aligned}` 和 \verb`\end{aligned}` 再加回去。
