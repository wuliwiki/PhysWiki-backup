...
矩阵最常见的运算是矩阵的乘法。 事实上，上面的 “矩阵乘以列向量” 就是矩阵乘法的一种特殊情况。 如果我们把同一矩阵乘以多个列向量
\begin{equation}
\bvec y_i = \mat A \bvec x_i \qquad (i=1,\dots, N)~,
\end{equation}
那么把所有列向量 $\bvec y_i$ 从左到右排列得到矩阵 $\mat Y$，把所有 $\bvec y_i$ 从左到右排列得到 $\mat X$，那得到了一般的矩阵乘法
\begin{equation}
\mat Y = \mat A\mat X~.
\end{equation}
令 $\mat X$，$\mat Y$ 分别为 $n \times l$ 和 $m \times l$ 的矩阵
\begin{equation}
\mat X =
\begin{pmatrix}
x_{11} & \cdots & x_{1l} \\
