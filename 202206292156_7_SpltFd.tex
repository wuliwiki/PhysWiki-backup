...
但这些自同构不是全部，因为只是针对一组基构造出来的．$\mathbb{F}_1$上所有的保$\mathbb{F}$自同构，应该是上段构造的自同构和全体保$\mathbb{F}(\alpha_1)$复合的结果．这是因为$\mathbb{F}_1$的保$\mathbb{F}$自同构只有两种类型，保$\mathbb{F}(\alpha_1)$和不保$\mathbb{F}(\alpha_1)$的，而$\sigma(\mathbb{F}(\alpha_1))$只取决于$\sigma(\alpha_1)$，所以任挑一个
