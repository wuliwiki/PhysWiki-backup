...
如果给$V_1$和$V_2$进行基变换，过渡矩阵分别为$\bvec{P}$和$\bvec{Q}$，则在新的基下，两向量的坐标分别为$\bvec{c}'(\bvec{v}_1)=\bvec{P}^{-1}\bvec{c}(\bvec{v}_1)$和$\bvec{c}'(\bvec{v}_2)=\bvec{Q}^{-1}\bvec{c}(\bvec{v}_2)$，于是有$f$的矩阵变成了$\bvec{Q}^{-1}\bvec{F}\bvec{P}$．

为了方便将坐标变换推广，我们把二阶张量$f$的坐标表示为一个\textbf{行向量的行矩阵}$\bvec{F}$，也就是说，选定了$V_1$和$V_2$的基以后，用$\bvec{c}(\bvec{v}_1)$和$\bvec{c}(\bvec{v}_2)$分别表示$\bvec{v}_1\in V_1$和$\bvec{v}_2\in V_2$在这两组基下的坐标列矩阵，那么$f(\bvec{v}_1, \bvec{v}_2)=\bvec{F}\bvec{v}_1\bvec{v}_2$．此时有$f(\bvec{v_1},\bvec{v_2})=\bvec{c}(\bvec{v_2})\Tr\bvec{F}\bvec{c}(\bvec{v_1})=\bvec{c}'(\bvec{v_2})\Tr\bvec{Q}\Tr\bvec{F}\bvec{P}\bvec{c}'(\bvec{v_1})$
