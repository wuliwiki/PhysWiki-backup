...
“$f_n(x)$\textbf{不收敛}于$f(x)$”等价于“存在$\epsilon>0$，使得对于任意$N\in\mathbb{Z}^+$，都存在$n>N$，使得$\abs{f_n(x)-f(x)}\geq\epsilon$”\footnote{到这里，有的读者可能已经看出来怎么证明了：“存在$\epsilon>0$”对应的就是\autoref{EgrfTh_eq1} 中的$\bigcup^\infty_{k=1}$，“对于任意$N$”对应的是$\bigcap^\infty_{N=1}$，“存在$n>N$”}．
