...
\end{equation}
其中 $N$ 称为\textbf{时移函数}（\textbf{lapse function}），$N^i$ 称为\textbf{位移函数}（\textbf{shift function}）．其几何解释为：$N\dd t$ 是相邻两超曲面的间的固有时，$N^i\dd t$ 是上超曲面 $X^\mu(x^i,t+\dd t)$ 上空间坐标为 $x^i$ 的点到下超曲面 $X^\mu(x^i,t)$ 上具有同一zu点 $x^i$ 的法矢量的距离（即从下超曲面 $X^\mu(x^i,t)$ 上通过点 $x^i$ 的法矢量进行的）．
