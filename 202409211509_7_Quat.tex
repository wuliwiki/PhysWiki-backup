...
由于每个四元数都可以写成$1, \I, \J, \K$四个元素分别乘以一个实数后相加，且四元数乘法具有分配性，故要证明结合性，只需要证明这四个元素的乘法总满足结合性即可；又因为$1$是乘法单位元，与任何其它两个四元数做乘法的时候必定满足结合性，故我们只需要证明$\I, \J, \K$之间有结合性杰克。最后，因为$\I\J=\K$、$\J\K=\I$和$\K\I=\J$的“\textbf{$3$-轮换对称性}”\footnote{即把$\I, \J, \K$放在等边三角形的三个顶点上，通过逆时针\textbf{旋转}三角形$0^\circ$、$60^\circ$或$120^\circ$来改变这几个元素的名字，其乘法结构不变。比如，如果$\I, \J, \K$依次按逆时针排列，则三角形逆时针旋转$60^\circ$后，$\I$变为$\K$，$\K$变为$\J$，而$\J$变为$\I$，此时$\I\J=\K$就变成了$\K\I=\J$，变换结果依然是成立的。}，以及$\I\J=-\J\I$的\textbf{反对称}性，我们只需要证明以下三个结合性：
\begin{equation}
\left\{
    \begin{aligned}
...
\right. ~
\end{equation}
即可得证四元数乘法的结合性\footnote{比如说，要证明$\K(\K\J)=(\K\K)\J$，只需要利用轮换对称性，把$\I$变为$\J$、$\J$变为$\K$、$\K$变为$\I$（全部“向后移动一位”），即可将问题化为证明$\I(\I\K)=()$}。
\textbf{证毕}。
