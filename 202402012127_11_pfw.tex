...
具体来说，设$f:M\rightarrow N$是光滑映射，$c(t):[0,1]\rightarrow M$是光滑道路。$V$是$M$上沿着该道路的任意切场：$V(t)=v^i(t)e_{i,c(t)}$，其中$\{e_{i,c(t)}\}$为点$c(t)$邻域上的一组基。为方便计，重新表示$V(t)=v^ie_i$。则切场$V()$沿着道路$c(t)$的前推为$f_*V(t)=v^i\widetilde e_{i,f\circ c(t)}$，其中$\widetilde e_i=f_*e_i$，是$f\circ c(t)$上的一组基。
