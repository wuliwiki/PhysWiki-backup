...
\item \verb|find_library(变量 NAMES 库名 PATHS /usr/lib /usr/local/lib HINTS $ENV{LIBRARY_PATH} $ENV{LD_LIBRARY_PATH})| 可以寻找 \verb|lib库名.so或a[.版本号]|。 若要指定版本可以用例如 \verb|gfortran.3|， 也可以指定 \verb|gfortran.so或a|。 注意指定目录的子目录不会被搜索。
\item \verb|set(CMAKE_FIND_LIBRARY_SUFFIXES ".a或.so或.lib")| 可以让 \verb|find_library()| 更倾向于某种类型的 lib。 也可以用 \verb|".a;.so"| 来指定优先级。
\item \verb|find_library()| 的默认搜索路径如 \verb`/usr/lib`，\verb`/usr/local/lib`，以及环境变量 \verb`LD_LIBRARY_PATH` 中的路径。 \verb`CMAKE_LIBRARY_PATH` 变量可以增加 \verb`find_library` 的默认搜索路径，且会优先搜索。
\end{itemize}
