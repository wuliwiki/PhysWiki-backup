...
\textbf{柯西-施瓦茨不等式（Cauchy-Schwarz Inequality）}，也称\textbf{柯西不等式}，为两个向量的内积（inner product）与它们的模长（norm）之间建立了重要的不等关系。柯西不等式是“内积”这种运算定义时自带的属性，因此所有满足“内积”定义要求的运算，或者说可以称为“内积”的运算，都会满足这个不等式\footnote{关于内积更详细的内容可以参考\enref{内积、内积空间}{InerPd}。}。

下面以高中常见的二维向量来做一个初步证明。对二维向量$\bvec{x}=(a,b),\bvec{y}=(c,d)$，它们的内积可以表示为：
