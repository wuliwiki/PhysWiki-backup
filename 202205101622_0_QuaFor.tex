...
\begin{equation}
\begin{aligned}
q_f(-{x})&=f(-{x},-{x})=f({x},{x})=q_f({x}) \quad \forall x\in V\\
...
称与 $q$ 配极的双线性型 $f$ 在空间 $V$ 的基底 $(e_1,\cdots,e_n)$ 之下的矩阵 $F$ 是二次型 $q=q_f$ 的矩阵．
\end{definition}
\begin{example}{}
设 $F=(f(e_i,e_j))=(f_{ij})$ ，则
\begin{equation}
f_{ij}=\frac{1}{2}[q(e_i+e_j)-q(e_i)-q(e_j)]\quad i,j=1,2,\cdots n.
\end{equation}
任意一个对称的矩阵 $F=(f_{ij})$ 也适应由关系式\autoref{Tensor_exe1}~\upref{Tensor}
\begin{equation}\label{QuaFor_eq2}
q(x)=X^TFX=\sum_{i,j} f_{ij}x_ix_j\quad i,j=1,\cdots ,n
\end{equation}
