...
\end{equation}
这里的 $N$ 个有序实数 $(x1, x2, \cdots, x_N)$ 就是 $\bvec v$ 关于基底 $\qty{\bvec \beta_i}$ 的\textbf{坐标（coordinates）}． 我们说该坐标属于集合 $\mathbb R^N$， 即实数集 $\mathbb R$ 自身的 $N$ 次笛卡尔积（\autoref{Set_eq1}~\upref{Set}）．

在另一些情况下， 尤其是对坐标进行线性变换时， 我们也会把它记为\textbf{列矢量}的形式． 列矢量是 $N$ 行 $1$ 列的特殊矩阵\upref{Mat}．
...
\end{equation}
列矢量可以看成 $N$ 行 $1$ 列的特殊矩阵\upref{Mat}． 在列矢量的右下角声明基底是较为严谨的做法， 但为了书写简洁， 在不至于混淆的情况下我们可以将其省略． 另外在正文中， 为了节约空间， 我们将\autoref{Gvec2_eq7} 记为 $\bvec v = (x, y, z)_{\{\bvec\beta_i\}}\Tr$，其中$\Tr$表示矩阵的转置（见“矩阵\upref{Mat}” \autoref{Mat_eq2} ）．同样， 我们时常省略 $\{\bvec\beta_i\}$．
...
要特别注意的是， 当定义了多组基底时， 只有基底相同的两个列矢量按照\autoref{Gvec2_eq8} 相加才有意义．
