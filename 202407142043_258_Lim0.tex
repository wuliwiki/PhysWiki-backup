...
也许读者已经接触过数列的单调性这个概念，例如，对于数列$ b_n= \bvec n$,$n$越大$b_n$就越大。要想$b_n$足够大(大于一个预先指定的正数，比如一千万)，只需n足够大。

例如，要让$b_n$比一千万亿大，只需n大于一千万亿。要让$a_n$大于$10^{100}$，只需$n$大于$10^{100}$。从这个意义上讲，我们可以说数列${a_n}$是一个“无穷大量”。需要注意的是，这并不意味着$b_n = \infty $，事实上，$\infty$不是一个实数，不能被变量等于。无论$n$有多大，$a_n$都只能任意地大，只要$n$够大，$a_n$就可以超过任何正数，但不会等于$\infty$，因为$\infty$并不存在于实数系，它不是一个数。
...
数列值$  a_n  $不会小于$0$，但可以任意接近$0$，换言之，$0$是数列$\begin{Bmatrix} a_n \end{Bmatrix}$不可逾越的“极限”。
