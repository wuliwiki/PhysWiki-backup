...
有些读者可能会觉得不必区分这四种形式，认为只需记住两种基本形式，再结合 $p$ 的正负来判断开口方向即可，并把 $|p|$ 看作抛物线的焦准距。这样理解记忆是可以接受的。但一般采用上述形式，一方面是与教材体系保持一致，另一方面是因为 参数$p$ 并不只出现在此处，为了统一定义与表达，避免混淆，选用了固定的正值 $p$ 并通过符号体现开口方向。
...
此外，抛物线还可以使用参数方程来描述，尤其在物理和工程问题中更为常见。例如，在研究抛体运动或光线反射路径时，参数形式常具有更清晰的表达优势。
...
这里，相信有读者会认为有些麻烦，只要有两种形式，然后根据p的正负来判定就可以了，然后认为|p|是焦准距就可以了。当然，如何理解是读者的自由，但这里一则是与教科书统一，二则$p$这个参数本身并不在此出现，为了保持定义的统一以免混淆，因此此处采用了这种方法。\autoref{the_Para3_1} 中的四个方程，分别对应\autoref{fig_Para3_2} 中开口向上、下、右左的四条抛物线。

当然，抛物线也可以使用参数方程，但由于它本身变量间的关系比较简单，

以\autoref{eq_Para3_4} 为例，当 $t$ 从负变正时，$y$ 从负变正，表示点沿抛物线从左侧向右侧移动。这种表示在物理、工程中常用于描述抛射体的运动轨迹或光线反射路径。你也可以根据具体问题，将参数方程稍作变化来适应不同的抛物线形式。
