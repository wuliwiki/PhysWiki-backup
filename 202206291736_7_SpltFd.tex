...
不妨设$f$的各不可约因子互不相同．设$\sigma:\mathbb{K}\to\mathbb{K}$是保$\mathbb{F}$自同构，$h$是$f$在$\mathbb{F}$上的一个不可约因子，其在$\overline{\mathbb{F}}$上的全体根为$\{\alpha_i\}_{i=1}^m$，$\opn{deg}h=n\geq m$．
...
由前面的讨论，单扩张$\mathbb{F}(\alpha_1)$的保$\mathbb{F}$自同构数量，等于$\mathbb{F}(\alpha_1)$中所包含的$h$的根的数量，设该数量为$k$．则
