...
\begin{itemize}
\item 令 $X$ 为 $\mathbb K$ 上的希尔伯特空间， 令 $X^*$ 为 $X$ 的对偶空间． 那么 $f\in X^*$ 当且仅当存在 $v\in X$ 使得 $f(u) = (v|u)$ 对所有 $u\in X$． $v$ 可以由 $f$ 唯一确定， 且 $\norm{f} = \norm{v}$
...
\begin{itemize}
\item 以下三个条件互相等价： (1) 二次最小值问题的 existence principle
\end{itemize}


\subsection{Problems}
\begin{itemize}
\item \textbf{special tensor product}： 经典分析中， 两个函数的张量积定义为 $(\phi\otimes\psi)(x,y) := \phi(x)\psi(y)$． 令广义函数 $U, \delta \in \mathcal D'(\mathbb R)$． 定义 $(U\otimes\delta^{(n)}(\xi))$
\end{itemize}
