...
我们来观测一个一共 $4$ 层的搜索树（算上根节点），并在模拟一部分搜索的过程中了解 Alpha-Beta 剪枝的原理。
我们仍然考虑刚才的搜索树并忽略最后一层，则已知的搜索完成后的各结点存储的胜率如下图
\begin{figure}[ht]
\centering
\includegraphics[width=14cm]{./figures/a094470dd5f25419.png}
\caption{模拟搜索树} \label{fig_mmsab_9}
\end{figure}

首先标注出每一层是求最小值（Min 层）还是最大值（Max 层），然后模拟先序遍历搜索到第一个叶子结点。此时根据求最小值与最大值的性质可以得到父亲结点的范围。
\begin{figure}[ht]
\centering
\includegraphics[width=14cm]{./figures/397aad508bb7566c.png}
\caption{搜索到第一个叶子节点的搜索树} \label{fig_mmsab_12}
\end{figure}

接下来访问下一个叶子结点，注意到他的值是 $58\%$，此时这两个结点的fu'qin'jie