...
接下来考虑极限 $\Delta t\to 0$， 此过程中保持 $\lambda$ 为常数． 那么根据\autoref{E_eq3}~\upref{E}， 上式中方括号等于 $1/\E$， 所以第一次中奖时间的概率密度函数等于每次中奖的概率除以开奖时间间隔\footnote{\autoref{RandF_eq16} 中隐含了一个假设就是 $\lim [f(x)g(x)] = \lim f(x) \lim g(x)$， 严格来说这是有例外的， 但这里并不深究， 详见（链接未完成）．}
