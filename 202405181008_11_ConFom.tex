...
\begin{equation}
\begin{aligned}
\nabla_X\nabla_Y\bvec e_j& =\nabla_X(\omega_j^k(Y)\bvec e_k) \\
&=X\omega_j^k(Y)\bvec e_k+\omega_j^k(Y)\nabla_X\bvec e_k \\
&=X\omega_j^i(Y)\bvec e_i+\omega_j^k(Y)\omega_k^i(X)\bvec e_i.
\end{aligned}~,
\end{equation}
根据对称性，同理可得
\begin{equation}
\nabla_Y\nabla_X\bvec e_j =Y\omega_j^i(X)\bvec e_i+\omega_j^k(X)\omega_k^i(Y)\bvec e_i~.
\end{equation}
接着根据曲率的定义，我们有
\begin{equation}
\begin{aligned}
R(X,Y)e_j& =\nabla_X\nabla_Y\bvec e_j-\nabla_Y\nabla_X\bvec  e_j-\nabla_{[X,Y]}\bvec e_j \\
&=(X\omega_j^i(Y)-Y\omega_j^i(X)-\omega_j^i([X,Y]))\bvec e_i \\
&+(\omega_k^i(X)\omega_j^k(Y)-\omega_k^i(Y)\omega_j^k(X))e_i \\
&=\dd\omega_j^i(X,Y)e_i+\omega_k^i\wedge\omega_j^k(X,Y)\bvec e_i \\
&=(\dd\omega_j^i+\omega_k^i\wedge\omega_j^k)(X,Y)\bvec e_i.
\end{aligned}~.
\end{equation}
