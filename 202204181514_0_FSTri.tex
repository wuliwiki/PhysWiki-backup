...
\subsubsection{复值函数}
若上述傅里叶展开公式对实值函数 $f(x)$ 成立， 当 $f(x)$ 的函数值变为复数时是否仍然成立呢？ 答案是肯定的．

我们可以令
\begin{equation}
f(x) = u(x) + \I v(x)
\end{equation}
其中 $u(x), v(x)$ 是相同周期的实函数， 且都满足迪利克雷条件． 那么， 可以把它们分别做傅里叶展开． 令 $u(x)$ 的展开系数为 $a'_n, b'_n$， 而 $v(x)$ 的系数为 $a''_n, b''_n$． 分别对 $u(x), v(x)$ 列出\autoref{FSTri_eq1} 到\autoref{FSTri_eq3}， 且令 $a_n = a'_n + \I a''_n$， $b_n = b'_n + \I b''_n$， 会发现展开公式同样成立．
