...
\item 由de Morgan公式，$\bigcap_{i=1}^\infty E_i=[\bigcup_{i=1}^\infty E_i^C]^C$，因此只需证明$\bigcup_{i=1}^\infty E_i$是可测集即可．由已经证明的\autoref{MsbSet_eq4} ，可推论得对于任意$A\in\mathbb{R}^n$和\textbf{两两不交}的可测集$E_i$有\footnote{第一个等号处，将$E_1^C$也看成“一列可测集”的一员，应用了一次\autoref{MsbSet_eq2} ；第二个等号则只对等号左侧（第一个等号右侧）第二项应用\autoref{MsbSet_eq2} .}
\begin{equation}
\begin{aligned}
\opn{m^*}A&=\opn{m^*}(A\cap E_1^C)+\sum_{i=1}^\infty \opn{m^*}(A\cap E_i)\\
&=\opn{m^*}(A\cap E_1^C)+\opn{m^*}(A\cap\bigcup_{i=1}^\infty E_i)\\
&=\opn{m^*}(A\cap [\bigcup_{i=1}^\infty E_i]^C)+\opn{m^*}(A\cap\bigcup_{i=1}^\infty E_i)
\end{aligned}
\end{equation}
