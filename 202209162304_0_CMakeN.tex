...
\item 在 \verb|CMakeLists.txt| 的路径下， \verb`cmake .` 生成 Makefile， 然后 \verb|make -j12| 多线程编译． \verb|make VERBOSE=1 -j12| 输出编译命令．
\item 要指定目录， 用 \verb|cmake -S CMakeList的目录 -B 生成Makefile的目录|， 然后再到 Makefile 的目录 \verb|make -j12| 即可．
\item 当然， 也不是什么平台都可以用 \verb|make|， 所以也可以用 \verb|cmake --build 生成Makefile的目录 [--target 可执行文件] -- -j 12| 其中 \verb|--| 后面的选项会传给 \verb|make|
