...
\autoref{PLaw1_eq5} 反映了什么样的物理内容呢？去思考！

由质点匀速圆周运动容易知道，在同样的时间内质点径矢方向 $\uvec{r}$ 转过的角度 $\theta$ 是不变量．这表明，若将矢量 $\uvec r(t)$ 头尾相连，则$\uvec r(0)$到 $\uvec r(t)$ 将画出一个正多边形的轮廓，如
\begin{figure}[ht]
\centering
\includegraphics[width=5cm]{./figures/PLaw1_3.pdf}
\caption{单位径矢$\uvec r$ 画出的正多边形} \label{PLaw1_fig3}
\end{figure}
那么将此多边形边长乘以 $\dd t$ 被， 得到的正多边形便是由矢量 $\uvec r(t)\dd t$ 头尾相连画出的正多边形轨迹．而 $\dd t$ 实际上是无穷小量，故这个正多边形事实上成为一个圆．在一个周期内将画出一个完整的圆（请思考？）．由矢量加法的几何图像可知，那么积分
\begin{equation}
\int_{0}^{t}\uvec r(t) \dd{t}
\end{equation}
的结果将是从 $\dd r(0)$ 起点指向 $\dd r(t)$ 的终点的向量．那么这个矢量微元 $\dd r$ 首尾相连构成的圆的半径是多少呢？在四分之一周期这个圆画了1/4，所以由\autoref{PLaw1_eq5} 可知，该圆半径便是 $\frac{1}{\omega}$.
\end{example}
