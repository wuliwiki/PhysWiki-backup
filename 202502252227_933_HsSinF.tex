...
之前学习过 $(\sin x)’ = \cos x$，以及 $(\cos x)’ = -\sin x$，可以看到 $\sin x$ 在求两次导数后会变成与原函数相反的形式。同样，继续求导可以得到 $(-\sin x)’ = -\cos x$ 以及 $(-\cos x)’ = \sin x$，也就是说，$\sin x$ 经过四次求导后，又回到了它自身。这种求导的循环性质十分有趣，下面对其一般形式进行总结。
...
对于任意整数 $n$，正弦函数与余弦函数的 $n$ 阶导数满足：
\begin{equation}
\frac{d^n}{dx^n} \sin x = \sin\left(x + n\frac{\pi}{2}\right), \quad
\frac{d^n}{dx^n} \cos x = \cos\left(x + n\frac{\pi}{2}\right)~.
\end{equation}
\end{corollary}
...
通过直接计算 $\sin x$ 和 $\cos x$ 的前几阶导数，可以发现如下规律：
\begin{align*}
...
\end{align*}
这表明，$\sin x$ 和 $\cos x$ 的高阶导数是以 4 为周期循环变化的。对于一般的 $n$，我们有：
\begin{align*}
\frac{d^{4k}}{dx^{4k}} \sin x &= \sin x, \
\frac{d^{4k+1}}{dx^{4k+1}} \sin x &= \cos x, \
\frac{d^{4k+2}}{dx^{4k+2}} \sin x &= -\sin x, \
\frac{d^{4k+3}}{dx^{4k+3}} \sin x &= -\cos x.
\end{align*}
其中 $k$ 为非负整数。这个循环关系同样适用于 $\cos x$。
...
高阶导数的周期性在物理学中有着广泛的应用。例如，在简谐振动系统中，位置 $x(t)$ 通常满足：
\begin{equation}
