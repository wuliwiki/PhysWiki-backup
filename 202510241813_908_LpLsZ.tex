...
~
\]
作为一个二阶微分算子，拉普拉斯算子将$C^{k}$类函数映射为$C^{k-2}$类函数（当 $k \ge 2$ 时）。它是一个线性算子 $\Delta : C^{k}(\mathbb{R}^{n}) \to C^{k-2}(\mathbb{R}^{n})$，或更一般地，$\Delta : C^{k}(\Omega) \to C^{k-2}(\Omega)$，其中 $\Omega \subseteq \mathbb{R}^{n}$ 为任意开集。
