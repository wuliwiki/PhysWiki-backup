...
\item $\cos x, \cot x$ 的零点为$x = \frac{\pi}{2} + k\pi, (k\in\mathbb{Z})$；
...
\end{itemize}
然而，还不仅止于此。根据\aref{诱导公式}{eq_HsTrFu_16}：
\begin{equation}
\cos(x+\pi) = -\cos x~.
\end{equation}
这表明 $\cos x$ 向左平移 $\pi$ 个单位后变为 $-\cos x$，而 $-\cos x$仍满足偶函数的定义，因此$x=\pi$ 也是 $\cos x$ 的对称轴。

类似地，根据\aref{诱导公式}{eq_HsTrFu_14}：
\begin{equation}
\sin(x+\frac{\pi}{2}) = \cos x, \quad \sin(x-\frac{\pi}{2}) = -\cos x~.
\end{equation}
这意味着 $\sin x$ 向左平移 $\frac{\pi}{2}$ 得到 $\cos x$，向右平移 $\frac{\pi}{2}$ 得到 $-\cos x$，从而可以推出 $\sin x$ 具有对称轴$\displaystyle x=\frac{\pi}{2}$和$\displaystyle x=-\frac{\pi}{2}$。当然，反过来也可以认为$\cos x$向右平移 $\frac{\pi}{2}$ 得到$\sin x$
...
另外根据\aref{诱导公式}{eq_HsTrFu_14}，sinx向左平移$pi/2$个单位可以得到cosx，sinx向右平移$pi/2$个单位可以得到-cosx，使得sinx本身也存在两个对称轴x=pi/2和x=-pi/2。而由于tanx和cotx都是奇函数，因此不论如何变换都没有对称轴。

上面分析的都是一个周期内的情况，结合各个函数的周期性可以得到：
sin是奇函数，它的对称中心是，对称轴是
cos
tan
cot
sec
...
由于三角函数具有周期性，其对称中心和对称轴会随周期性重复。因此可以总结如下：
	•	$\sin x$ 是奇函数，对称中心为 $x = k\pi$，对称轴为 $x = \frac{\pi}{2} + k\pi$。
	•	$\cos x$ 是偶函数，对称轴为 $x = k\pi$。
	•	$\tan x$ 是奇函数，对称中心为 $x = k\pi$，无对称轴。
	•	$\cot x$ 是奇函数，对称中心为 $x = \frac{\pi}{2} + k\pi$，无对称轴。
	•	$\sec x$ 是偶函数，对称轴为 $x = k\pi$。
	•	$\csc x$ 是奇函数，对称中心为 $x = \frac{\pi}{2} + k\pi$，无对称轴。
