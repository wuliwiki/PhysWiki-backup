...
\item 自动补全括号 \verb|()[]{}| 的算法需要和 vscode 的行为一样， 以 \verb|{| 为例： 1. 插入 \verb|{| 时， 只有右边是空格、换行符、enf of file 或者 \verb|)]}| 之一时才补全右括号． 2.  作为 1 的例外， 在插入 \verb|}| 时， 如果右边的字符是自动配对的 \verb|}|， 则不插入并将光标向右移动一个（或者理解为覆盖）． 3. 选中几个字符并插入 \verb|{| 时， 自动在选中字符的左边插入 \verb|{|， 右边插入\verb|}|．
