...
\autoref{Lagrng_ex2}~\upref{Lagrng} 中， 若两个滑块受到于速度相反， 大小恒定的摩擦力 $\bvec f$， 求运动方程．

这里有一点微妙的地方在于摩擦力是取决于运动方向的． 把 $\dot X$ 的正负号记为 $\opn{sgn}(\dot X)$， 那么广义力为
\begin{equation}
Q_x = -2f\opn{sgn}(\dot x)~, \qquad
Q_X = -2f\opn{sgn}(\dot X)
\end{equation}
求得运动方程为
\begin{equation}
m\ddot x =  - 3kx - 4f \opn{sgn}(\dot x)~, \qquad
m\ddot X =  - kX - f\opn{sgn}(\dot X)
\end{equation}
\addTODO{如何验证是否正确？}
\end{example}
