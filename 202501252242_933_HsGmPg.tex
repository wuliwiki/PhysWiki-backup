...
当将等比数列和的项数取极限 $n \to \infty$ 时，称为\textbf{等比级数（geometric series）}。此时，若公比满足 $0 < |q| < 1$，可以将 $S_n$ 取极限，当 $n \to \infty$ 时，项 $q^n$ 的绝对值趋于$0$\footnote{极限的求解方法可参考“\enref{数列的极限（简明微积分）}{Lim0}”。}，因此：
\begin{equation}
S=\lim_{n \to \infty} S_n = \lim_{n \to \infty} \frac{a_1(1 - q^n)}{1 - q} = \frac{a_1(1 - 0)}{1 - q}=\frac{a_1}{1 - q}~.
\end{equation}
若 $|q| \geq 1$，等比级数将会趋于无穷大，即发散。这也使得人们通常更关注$|q|<1$的等比数列。这一结果展示了数列和的性质在特定条件下如何转化为收敛级数的形式，并为数列向函数形式的推广提供了重要的数学依据。