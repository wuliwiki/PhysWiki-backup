...
\item 把 list 作为 \verb|command()| 的参数相当于多个 \verb|arg|． 例如 \verb`command(${Foo})` 相当于 \verb`command(aaa bbb ccc)`． 另外 \verb`command("${Foo}")` 相当于 \verb`command("a b c")` （双引号中的是一个字符串不是 list）
