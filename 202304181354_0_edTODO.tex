...
\subsection{使用 Git 协同编辑}
\begin{itemize}
\item 使用 git 的方式管理协作。 从概念上每个人的每次新编辑（可以修改任意多个词条）都创建整个百科的一个新分支（以账号 ID 命名）。 完成后提交 pull request（PR）。 审稿人同意后 merge 到 master。 每个新分支都有自己的数据库（概念上）。
\item 每个人都可以看到每个词条的占用和修改情况， 鼓励同时编辑两个词条的人进行沟通。
\item 为避免冲突每个词条一次只能有一个 PR， 这样抢锁就成了抢 PR。 与其这样就把编辑者抢锁的负担转移给审稿人了（必要时手动解决冲突）。
\item 鼓励尽量把非 trivial 的公式用公式按钮添加， 按钮自动生成 label。 其他环境一律强制要求有 label。
\item 实现重命名 label 功能（某些重要的 label 可能希望改成有意义的 id）， 可以自动把所有分支的某个 label 都重命名（要确保都没有冲突）。
\item 数据库的实现上， 给每个分支新建一个临时数据库， 但每个表格只保存变化、删除和新添加的记录（但为了偷懒也可以先复制整个数据库）。 PR 通过以后删除数据库。 文件也是只有改变的才进行复制（为了偷懒也可以先复制所有词条文件）。
\end{itemize}

\addTODO{会员制需要考虑一个问题就是， 如果大家都可以用编辑器， 那不开会员也可以打开编辑器查看所有内容。 所以编辑者必须开通会员， 有足够的贡献才能抵扣会员。}
