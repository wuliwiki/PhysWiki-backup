...
\subsection{RAM 和 SSD 缓存}
\begin{itemize}
\item ZFS 直接支持 SSD 缓存（未完成：命令），也可以调整内存缓存的大小。
\item 内存相当于一级缓存， SSD 是第二级
\item ARC (Adaptive Replacement Cache) 就是 RAM 缓存
\item \verb`cat /proc/spl/kstat/zfs/arcstats | grep c_max` 可以查看当前最大允许的 arc 缓存。默认值一般是系统内存
\item 要让设置在重启后生效， 要修改 \verb`/etc/modprobe.d/zfs.conf`
\end{itemize}
