...
\end{equation}
把\autoref{ScaNrm_eq3} 做积分换元 $E = k^2/(2m)$ 得



只有满足？ ， 才能把任意波函数分解为许多 $\psi_{k}$ 的叠加（积分）

使得
\begin{equation}
C(k) = \int_{-\infty}^{+\infty} \psi_{k}(x)^* \psi(x) \dd{k}
\end{equation}
注意一维定态薛定谔方程对于同一个能量 $E = k^2/(2m)$ 有两个不同的解， 一个能量对应正负两个 $k$ 的平面波， 他们都被包含在了\autoref{ScaNrm_eq2} 的积分中．
