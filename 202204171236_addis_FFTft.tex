...
这里使用的算法见\autoref{DFT_sub1}~\upref{DFT}． 给出任意等间距的 $x$ 坐标格点 \verb|[x0, x0+dx, x0 + 2*dx,...]|， 以及对应的函数值 \verb|f = [f(1), f(2), ...]|

\verb|f| 是一个向量， \verb|f(i)| 就是函数值 \verb|f(x0+(i-1)*dx)|． \verb|Nk| 是可选的， 若 \verb|Nk| 大于 \verb|f| 个个数， 输出中 \verb|k| 的步长将会相应变小使 \verb|k| 的长度为 \verb|Nk|， 但区间不会变． \verb|k| 的区间是由 \verb|dx| 决定的． 
