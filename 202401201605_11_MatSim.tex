...
若相似变换可以使矩阵变为对角矩阵， 我们把这个过程称为对角化，并称$A$可对角化。从\autoref{the_MatEig_1}~\upref{MatEig}可知，$A$可对角化的充要条件是$A$有$n$个线性无关的特征向量。结合\autoref{the_MatEig_2}~\upref{MatEig}“不同特征值的特征向量线性无关”和\autoref{cor_MatEig_1}~\upref{MatEig}“不同特征值的特征向量组线性无关”可知$A$如果满足以下两种情况，则$A$可对角化。
\begin{corollary}{}
