...
\begin{equation}
E_n^{(0)} = n^2 \frac{\pi^2 \hbar^2}{2\mu a^2}, \quad \psi_n^{(0)} = \sqrt{\frac{2}{a}} \sin \frac{n \pi x}{a}
\end{equation}

求到二级，矩阵元一般形式
\\begin{equation}
\langle n | H' | 1 \\rangle = \\frac{2}{a} \\int_0^a \\sin \\frac{n \\pi x}{a} \\lambda \\delta \\left( x - \\frac{a}{2} \\right) \sin \\frac{\\pi x}{a} dx = \\frac{2\\lambda}{a} \\sin \\frac{n \pi}{2} \\sin \\frac{\\pi}{2}
\end{equation}
...
\\textbf{二级修正}
\\begin{equation}
E_1^{(2)} = \\sum_{l \\neq 1} \\frac{|\\langle 1 | H' | l \\rangle |^2}{E_1^{(0)} - E_l^{(0)}} = \\sum_{l \\neq 1} \\frac{\\lambda^2 \\cdot 4}{a^2} \\times \\frac{\\sin^2 \\frac{l \\pi}{2}}{E_1^{(0)} (1 - l^2)}
\\end{equation}
...
由 $|E_1^{(2)}| \\ll |E_1^{(1)}| \\ll |E_1^{(0)}|$，可得 $\\lambda \\ll \\frac{\\pi^2 \\hbar^2}{\\mu a}$
