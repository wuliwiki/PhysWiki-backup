...
\subsection{证明}
根据（链接未完成）以及\autoref{HlmPr2_the2}~\upref{HlmPr2}， \autoref{HelmTh_eq1} 和\autoref{HelmTh_eq2} 中定义的 $\bvec F_{d}(\bvec r)$ 和 $\bvec F_{c}(\bvec r)$ 分别是无旋场和无散场， . 满足
\begin{equation}
\div \bvec F_d(\bvec r) = \div \bvec F(\bvec r)
\end{equation}
\begin{equation}
\curl \bvec F_c(\bvec r) = \curl \bvec F(\bvec r)
\end{equation}
容易证明剩下的 $\bvec H(\bvec r)$ 必定是调和场：
\begin{equation}
