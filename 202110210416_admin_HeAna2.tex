...
\end{equation}
所有其他的 $(l_1,l_2)$ 分波在 $r_2\to\infty$ 波函数都消失． 这样就可以把这个态记为 $\ket{L,M; l_1,l_2, n_1, k_2}$． 分号用于提醒后面的量子数只是无穷远处的边界条件， 这个态仍然只有 $E,L,M$ 是 well defined． 如果把这些态用 CG 系数线性组合一下， 就可以得到 $\ket{n_1,l_1,m_1, l_2,m_2,k_2}$， 注意此时除了能量， 里面的量子数都只是在 $r_2\to+\infty$ 处是 well defined． 要获得第二个粒子的平面波（平面波出射）， 就做线性组合
\begin{equation}
\ket{n_1, l_1, m_1, \bvec k_2} = \sum_{l_2,m_2}\frac{\I^{l}}{k} \E^{-\I(\sigma_{l_2}+\delta_{l_2})} Y_{l,m} (\uvec k) \ket{n_1,l_1,m_1, l_2,m_2,k_2}
\end{equation}

微扰理论中的矩阵元可以是（注意对称化） $\mel{n_1,l_1,m_1, l_2,m_2,k_2}{z_1 + z_2}{i}$， 剩下的论述就和氢原子的一样了． 把一阶微扰的末态投影到上面后， 光电离的总延时就是
\begin{equation}
