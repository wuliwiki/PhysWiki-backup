...
一下子引入了多个参数，可能让人感到眼花缭乱，但它们的核心作用是描述正弦型函数与标准正弦函数 $\sin x$ 之间的关系。所有这些参数的引入，都是为了找到变化后的函数与 $\sin x$ 之间的对应关系，使其规律更加清晰。接下来，将对 $f(x)$ 中的各个参数逐一进行介绍：
\begin{itemize}
\item $A$ 决定了 $f(x)$ 的值域 $\left[-A, A\right]$。如果将正弦函数视为一种振动，那么 $A$ 代表的是函数的最大偏离值，这也是“振幅”一词的来源。换句话说，$A$ 控制了函数图像在垂直方向上的伸缩，最高点与最低点的差为 $2A$。
\item 相位$\omega x + \varphi$ 引入了一种新的视角来看待三角函数。具体来看，$\sin x$ 是一个非线性函数，直接分析其变化规律可能并不直观。于是，就像指数函数指数部分那样，将 $\sin(\omega x + \varphi)$ 视为一个复合函数，其中 $\omega x + \varphi$ 相当于对 $x$ 进行线性变换，控制输入值的增长速度（由 $\omega$ 决定）以及起始位置（由 $\varphi$ 决定），这决定了 $\sin$ 函数的输入值。然后 $\sin$ 作为外层函数，仅起到将输入映射到 $[-1,1]$ 之间的作用。

这种方式可以避免三角函数带来的非线性直觉困难，使分析更加直观。


	2.	再通过 $\sin$ 作用到这个新变量上，将其非线性映射到 $[-1,1]$ 之间。

这样，$\omega$ 直接影响函数的变化频率，而 $\varphi$ 决定了相位偏移。这种分解方式有助于分离正弦函数的周期性行为与输入变量的线性变化，使得分析更加直观。

\item $\omega$ 称为\textbf{角频率（angular frequency，也称作圆频率）}，反映了函数的周期性变化速率。此前已经学习过\textbf{周期（period）}的概念，这里给出 $T$ 与 $\omega$ 的关系：
\begin{equation}
T = \frac{2\pi}{\omega}~.
\end{equation}

有时也会直接在正弦型函数中使用 $T$ 来表示周期，例如：

\begin{equation}
f(x) = A\sin\left(\frac{2\pi}{T} x + \varphi\right)~.
\end{equation}
此外，在日常生活中经常提到的\textbf{频率（frequency）}，通常用字母 $f$ 表示\footnote{这一符号与函数 $f(x)$ 的符号相同，容易混淆。}，其定义如下：
\begin{equation}
f = \frac{1}{T} = \frac{\omega}{2\pi}~.
\end{equation}
（角频率与频率的具体名称来源可以从圆周运动的角速度角度理解。）此前曾提及三角函数也被称为\textbf{圆函数（circular function）}，在这里这一特点得到了体现。
...
\begin{itemize}
\item $A$表示了函数的最大偏离值，它决定了 $f$ 的值域，即$\left[-A,A\right]$ ，也就是 $f$ 的图像的垂直高度，最高与最低点的差为$2A$。
\item $\omega x + \varphi$是之前没有见过的形式，他的思路就是，将外面的sin当作一个黑箱，只要算出内部的值是多少，就可以用sinx映射过去，相当于看成一个复合函数，这样就避免了sin引起的不直观的非线性的问题。用于描述函数在不同 $x$ 位置的状态。
