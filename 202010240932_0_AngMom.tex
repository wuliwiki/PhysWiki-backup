...
\end{equation}
变换到与 $A$ 相对静止的坐标系 $B$ 中， 从 $B$ 原点指向 $A$ 原点的矢量为 $\bvec r_{BA}$， 那么总角动量为
...
&= M \bvec r_c \cross\bvec v_c + \sum_i m_i \bvec r_{ci}\cross \bvec v_{ci} + \sum_i m_i \bvec r_c \cross \bvec v_{ci} + \sum_i \bvec r_{ci} \cross \bvec v_c
\end{aligned}
\end{equation}

根据\autoref{SysMom_eq2}~\upref{SysMom}， 系统总动量可以表示为系统总质量 $M = \sum_i m_i$ 乘以质心速度 $\bvec v_c$， 所以我们可以定义所谓的 “质心角动量”
\begin{equation}
\bvec L_c = \sum_i \bvec p_i = M \bvec v_c
\end{equation}
