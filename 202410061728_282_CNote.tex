...
\end{lstlisting}
\item \verb`size_t fread(void *ptr, size_t size, size_t num, FILE *stream)` 读取指定 \verb`size*num` 字节的内容到 \verb`ptr`（不会在结尾写 \verb`\0`！需要手动添加）。 函数返回成功读取的元素个数，如果返回值小于 \verb`num`，说明达到了文件末尾，或发生了错误。
\item \verb`w+` 模式的例子：
\begin{lstlisting}[language=cpp]
#include <stdio.h>
#include <stdlib.h>
