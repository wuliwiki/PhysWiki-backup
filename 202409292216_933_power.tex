...
幂函数的定义是基于乘方运算的。乘方运算是继加法、减法、乘法和除法之后的另一种重要的数学运算。它最开始的意思是表示一个数自乘若干次，这也是初中学过的含义。但随着各自定义的扩张，乘方运算包含了一些其他内容。

\begin{definition}{乘方运算}\label{def_power_1}
形如
\begin{equation}
a^b=c~.
\end{equation}
的运算称为\textbf{乘方运算（exponentiation，或幂运算）}，其中a称为\textbf{底数（base）}，b称为\textbf{指数（exponent）}，c称为\textbf{幂值（result）}。$b=2$时称为\textbf{平方}，$b=3$时称为\textbf{立方}。
\end{definition}

下面是一些乘方运算的法则，它是对初中已有的整数幂次的如果尚不了解原理或感到陌生，直接默认其成立熟练使用即可：
\begin{itemize}
...
f(x) = x^a~.
\end{equation}
的函数称作\textbf{幂函数（Power function）}，其中 $a\in\mathbb Q$。
\end{definition}
