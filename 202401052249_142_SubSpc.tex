...
如果一个向量空间 $\mathcal S$ 中的所有向量都属于另一个向量空间 $\mathcal V$， 且两个向量空间中加法和数乘运算的定义相同。 那么前者就是后者的\textbf{子空间}。

% Giacomo：没有但是，向量空间本身当然是它的子空间
% 严格来说所有向量空间都是它本身的子空间（即 $N_S = N$ 的情况）， 但我们一般是指 $N_S < N$ 的情况。
