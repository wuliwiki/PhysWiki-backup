...
\end{equation}
注意 $n=m$ 时该式两边恒为零，\autoref{eq_AdiaQM_10} 中 $\mat A$ 的对角元也都恒为零。 要计算非对角元， 由\autoref{eq_AdiaQM_4} 得
\begin{equation}
\braket*{\psi_m}{\dot\psi_n} = \frac{\mel*{\psi_m}{\dot H}{\psi_n}}{E_n-E_m} \qquad (m\ne n)~.
\end{equation}
这可以帮我们精确计算矩阵 $\mat A$。

到现在为止，所有推导都是精确的。

绝热近似的关键就在于假设 $H$ 随时间变化缓慢， $\dot H$ 非常小，以至于两个能级 $E_n$ 和 $E_m$ 如果相距不是太近，可以近似认为。
...
\end{equation}
其中第一个求和中的 $n$ 满足是对所有在 $t$ 取值范围内可能使 $E_n(t)$ 和 $E_m(t)$ 相等或非常接近；而所有剩下的 $n$ 都放到第二项的求和中。 现在还没有使用任何近似。  注意当 $E_n(t)$ 和 $E_m(t)$ 非常接近时该近似可能会失效， 所以我们把它保留到第一个求和中不予忽略。 下文我们将详细讨论这点。
