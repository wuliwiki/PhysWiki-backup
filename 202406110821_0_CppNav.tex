...
\subsection{C++ 可以做什么}
C++ 属于较为底层的语言。 适合执行对性能要求较高的任务。 例如 Linux 本身是 C 写的（C 基本可以看做是 C++ 的一个子集）， \enref{Python}{Python} 和 \enref{Matlab}{Matlab} 的解释器也是 C++ 写的， 网站服务器（Nginx，Apache），数据库也一般是 C++ 写的。

一般来说，学习 C++ 的相当长一段时间内你都做不出任何图形界面，也就是你打交道的都是命令行输入和输出。甚至许多应用（尤其是科学计算）根本不需要图形界面，只使用命令行。 在科学计算中，如果你需要画图，一般是用 C++ 导出数据文件，然后用其他工具画图（如 \enref{Matlab}{MatPlt}，Matplotlib， gnuplot 等）。
