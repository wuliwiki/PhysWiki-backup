...
\item 键盘左边只有三个按键，可以用 Karabiner 把空格右边那个键（也就是 right_option 映射到 fn）空格右边其他键暂时不会弄。
\item 中文输入法下，如果要临时切换到英文输入可以用 Capslock 键，并不会输入大写。
\end{itemize}
...
\item \verb|Cmd + Tab| 切换程序（相当于 Windows 的 \verb`Alt + Tab`），但是最小化的窗口仍然会保持最小化。如果要让它出现，松开 \verb`Tab` 以后按住 \verb`Opt`，松开 \verb`Cmd` 再松开 \verb`Opt` 即可…… （这是什么神仙设计？）。 
