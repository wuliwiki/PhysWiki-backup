...
\item 若三角形以$A$、$B$、$C$ 为顶点，则称为圆的\textbf{内接三角形（inscribed triangle）}，此时三角形的三个内角为三个圆周角，三角形的（什么心）为圆心。
\item 若三角形的三边分别与圆相切于$A$、$B$、$C$，则称为圆的\textbf{外切三角形（circumscribed triangle）}，此时三角形的（什么心）是圆心。
\end{itemize}
