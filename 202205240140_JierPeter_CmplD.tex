...
换言之，存在$g(x)\in\mathbb{F}[x]$，使得$f(x)=g(x^p)$．

由于“$g(x)$可约” $\Rightarrow$ “$g(x^p)$可约” $\iff$ “$f(x)$可约”，可知“$f(x)$可约” $\Rightarrow$ “$g(x)$可约”． 

如果$g(x)$也不可分，那么也可以进行相同的挖去操作，得到$g_1(x)$，其中$g(x) = g_1(x^p)$．由于多项式中只有有限个单项式，因此这一方法可以在有限步内得到一个可分的不可约多项式．为了方便，就设$g(x)$是最终得到的\textbf{可分的}不可约多项式．
