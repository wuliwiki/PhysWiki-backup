...
我们已经知道用焦点和准线如何定义椭圆\upref{Cone}， 下面介绍另外两种．
...
在直角坐标系 $x$-$y$-$z$ 中， 圆锥的方程为
\begin{equation}\label{Elips3_eq4}
\gamma^2 \cdot z_1^2 = x_1^2 + y_1^2
\end{equation}
其中 $\gamma$ 是圆锥角的 $\tan$． 我们可以再列出一个斜面方程得到方程组， 但这样解出来的椭圆将与 $x$-$y$ 平面不平行． 所以更好的办法是把圆锥旋转一下， 再用某个方程为 $z = z_0$ 的平面去截出椭圆． 关于 $y$ 轴做旋转变换\upref{Rot2DT}
\begin{equation}
...
\begin{equation}
(\sin\theta\cdot x - \cos\theta\cdot \gamma z)^2 = (\cos\theta\cdot x + \sin\theta\cdot \gamma z)^2 + y^2
\end{equation}
\addTODO{待验证}
化成标准形式为
\begin{equation}
\frac{(x + \tan2\theta \cdot \gamma  z)^2}{(\gamma  z/\cos2\theta)^2} + \frac{y^2}{(\gamma z)^2/\cos2\theta} = 1
\end{equation}
使用\autoref{Elips3_eq8} 得离心率为 $\sqrt{2}\sin\theta$．
