...
\end{equation}
那么\autoref{Hanaly_eq6} 中内积的具体计算方法见\autoref{CulmWf_eq6}~\upref{CulmWf}．
...
P(E) = \frac{1}{k}P(k)
\end{equation}
但是类氢原子的波函数一般用球谐函数表示， 我们试图直接用（径向）库仑函数来计算． 将波函数投影到归一化得库仑球面波\autoref{CulmWf_eq1}~\upref{CulmWf}得
\begin{equation}
...
\begin{equation}\label{Hanaly_eq5}
\sum_{l,m} \int P(l, m, k) \dd{k} = 1
\end{equation}
对比\autoref{Hanaly_eq2} 和\autoref{Hanaly_eq5}  得
\begin{equation}
