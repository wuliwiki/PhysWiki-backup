...
\begin{enumerate}
    \item 对于中心力场中的无自旋粒子，能级 $E_{nl}$ 的简并度为 ——————。
    \item 设 $t=0$ 时刻的量子态 $\left|\psi(0)\right\rangle$ 恰好为能量本征态，即 $H\left|\psi(0)\right\rangle = E\left|\psi(0)\right\rangle$ ，则 $t$ 时刻的量子态可表为 $\left|\psi(t)\right\rangle = $\underline{\hspace{3cm}} 。
    \item 带电粒子在电磁场中的哈密顿算符为 $H = \left[(\vec{p} + q\vec{B}/c)^2 + p_z^2 + p_\perp^2\right]^{1/2} \mu - \alpha c \hat{\jmath}$ ，除去，并列举两个守恒量： \underline{\hspace{3cm}} 。
    ...
    \item 设 $a$ 为湮灭算符，$\hat{a}\left| n \right\rangle = A_n\left| n-1 \right\rangle$，$\hat{a}^\dagger \left| n \right\rangle = A_{n+1}\left| n+1 \right\rangle$，$A_n \ne A_{n+1}$ ，则 $\left\langle m \left| n \right\rangle\right. = $\underline{\hspace{3cm}} 。
    \item 设一粒德布罗意子的概率分布为 $\rho(\mathbf{r})$ ，则能量本征值为 \underline{\hspace{3cm}} 。
