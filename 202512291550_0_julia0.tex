...
\item typed ir 中常见的 \verb`Core.Intrinsics` 模块中的 \verb`Core.IntrinsicFunction` 类（\verb`Core` 中定义的类）函数都有唯一的编号（例如 \verb`add_int (intrinsic function #2)`）这些函数在 \verb`Base` 模块中也放了一份： \verb`add_int (+), add_ptr (指针+偏移量) , and_int (按位与), bitcast (类型转换), checked_smul_int (有符号乘法，带上限检查), lt_float (<), mul_float (*), not_int (按位非), or_int (按位或), sle_int (<=), slt_int (<), sub_int (-), ule_int (<=), ult_int (<), xor_int (按位异或)`。
