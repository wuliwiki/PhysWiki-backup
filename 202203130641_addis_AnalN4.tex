...
\item 2.14 \textbf{里斯表示定理}： 设 $X$ 是局部紧的豪斯多夫空间, $\Lambda$ 是 $C_{c}(X)$ 上的正线性泛函, 则在 $X$ 内存在一个包含 $X$ 的全体博雷尔集的 $\sigma$-代数 $\mathfrak{M}$, 并存在 $\mathfrak{M}$ 上的唯一一个正测度 $\mu, \mu$ 在下述意义上表示了 $\Lambda$: 
