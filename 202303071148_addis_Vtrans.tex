...
一般情况下， 相对速度 $\bvec v_r$ 可以拆分成 $S'$ 的原点 $O'$ 在 $S$ 中的速度， 以及 $S'$ 相对 $S$ 旋转产生的速度（\autoref{CMVD_eq5}~\upref{CMVD}）两部分， 即（下面将证明）
...
\subsubsection{证明}
现在用旋转变换矩阵\upref{Rot3D}在 $S$ 系中证明\autoref{Vtrans_eq2} 和\autoref{Vtrans_eq4}。 令 $S'$ 中坐标到 $S$ 坐标的旋转变换矩阵\upref{Rot3D}为 $\mat R$， 即对任意几何矢量 $\bvec A$ 有 $\mat R (\bvec A)_{S'} = (\bvec A)_S$， 那么令 $\bvec r_{O'}$ 为 $S$ 的原点指向 $S'$ 的原点的几何矢量， 令 $\bvec r, \bvec r'$ 分别为质点在两参考系中的位置矢量， 有
\begin{equation}
...
\begin{equation}
\begin{aligned}
(\bvec v)_S &= ({\bvec v}_{O'})_S + \dot{\mat R}(\bvec r_{S'})_{S'} + \mat R (\bvec v')_{S'}\\
&= ({\bvec v}_{O'})_S + \dot{\mat R}(\bvec r_{S'})_{S'} + (\bvec v)_S~.
\end{aligned}
\end{equation}
右边第三项为 $(\bvec v')_S$。 这样就得到了\autoref{Vtrans_eq5} 。 所以只需要证明右边前两项就是相对速度\autoref{Vtrans_eq4}， 即证明：
\begin{equation}
(\bvec \omega \cross \bvec r')_S = \dot{\mat R}(\bvec r_{S'})_{S'}
\end{equation}
