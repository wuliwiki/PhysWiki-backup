...
对于任意$g\in\mathbb{F}[x]$和任意$k\in\mathbb{K}$，必有$\sigma(g(k))=g(\sigma(k))$．因此，$h$的根必须被$\sigma$映射为$h$的根．或者说，$\sigma$是$\{\alpha_i\}$上的一个置换．由\autoref{FldExp_the4}~\upref{FldExp}第2条，便历所有$\sigma$，则$\sigma(\alpha_1)$便历所有$\alpha_i$．
