...
\item \verb`weak_ptr` 不提供 \verb`get()` 成员，无法直接返回裸指针。 因为这是线程不安全的，因为有可能 \verb`get()` 调用以后、裸指针使用以前，其他线程会把对象销毁。 所以必须要先用 \verb`lock()` 返回一个 \verb`shared_ptr`，把这个对象锁住，再返回裸指针才能安全使用。 这也是 \verb`lock()` 名字的由来：把对象锁住不让它释放。
\end{itemize}
