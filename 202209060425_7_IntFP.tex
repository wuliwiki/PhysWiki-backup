...
\frac{\dd }{\dd t} \qty[-\qty(m+g\phi)\frac{-\dot{x}^k}{\sqrt{1-\dot{x}^\mu \dot{x}^\nu g_{\mu\nu}}}] = -g\frac{\partial \phi}{\partial x^k}\sqrt{1-\dot{x}^\mu \dot{x}^\nu g_{\mu\nu}}
\end{equation}

如果取\textbf{非相对论}和\textbf{弱场}极限，即$\dot{x}^k\ll c$且$g\phi$和$\dot{x}^\mu \dot{x}^\nu g_{\mu\nu}$相当，那么\autoref{IntFP_eq1} 化为
...
以上一小节讨论的\autoref{IntFP_eq1} 为例，我们希望$g\phi\sqrt{11\dot{x}^\mu \dot{x}^\nu g_{\mu\nu}}$
