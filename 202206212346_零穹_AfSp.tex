...
设 $V$ 是域 $\mathbb F$ 上的矢量空间．且 $\mathbb A$ 是一个集合，其元素称着\textbf{点}，并用 $\dot{p},\dot{q},\dot{r},\cdots$ 表示．称 $\mathbb A$ 是和 $V$ \textbf{相伴（连带的）的仿射空间}，若存在笛卡尔积 $\mathbb A\times V$ 到 $\mathbb A$ 的映射：
\begin{equation}
