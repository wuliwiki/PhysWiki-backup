...
$k$ 维向量空间 $V$ 的 $n$ 次张量幂空间 $V^{\otimes n}$ 上存在一个 $n$ 阶对称群 $S_n$ \upref{Perm}的群作用：
\begin{equation}
\begin{aligned}
...
特别的，我们把 $n$ 次张量幂空间 $V^{\otimes n}$ 的不动点集 $(V^{\otimes n})^{S_n}$ （\autoref{def_Group3_2}~\upref{Group3}）称为 $V$ 的 $n$ 阶\textbf{对称幂空间}，记做 $\opn{Sym}^n V$ 或者 $S^n(V)$。
...
$k$ 维向量空间 $V$ 的 $n$ 次张量幂空间 $V^{\otimes n}$ 的维度为 $k^n$，巧的是作为它的子空间，对称幕空间 $\opn{Sym}^n(V)$ 和反对称幂空间 ${\large \wedge}^n V$ 的维度之和为
\begin{equation}
\begin{aligned}
\pmat{k + n - 1 \\ n} + \pmat{k \\ n} &= \frac{n! (k - 1)!}{(k + n - 1)!} + \frac{n! (n - k)!}{k!} \\
&= \frac{n! (k - 1)!}{(k + n - 1)!} + \frac{n! (n - k)!}{k!}~ \\
\end{aligned}~
\end{equation}
