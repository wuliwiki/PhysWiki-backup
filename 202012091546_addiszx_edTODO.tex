...
\item 自动补全括号 \verb|()[]{}| 的算法需要和 vscode 的行为一样， 尤其是给几个已有的字符（两端不是空格）加上括号时， 不需要自动补全． 选中几个字符插入左括号时， 自动在选中字符的右边插入右括号．

\item \verb|$| 的补全使用和括号不一样的算法： 如果插入 \verb|$| 时两边都是空格， 那么补全另一个 \verb|$|， 否则不补全．
