...
垂直抛球的运动。当球达到某个高度时，它可能正处于上升阶段，也可能处于下降阶段。仅凭高度本身无法判断球的运动趋势，而必须结合其方向信息。
...
在$\sin x$ 的一个周期中，同一个 $y$ 值通常对应两个不同的 $x$ 值。然而，这两个 $x$ 互补，使得他们的导数符号相反——即在相同的 $y$ 处，一个点的斜率为正，而另一个点的斜率为负。因此，尽管 $\sin x$ 在一个周期内并非单调，每个 $(y, y’)$ 对应的 $x$ 仍然是确定的，不会出现 $\sin x$ 和 $\sin(x + 2\pi)$ 这样的状态完全重叠而无法区分的情况。

这一点可以类比于同样，在 $\sin x$ 的周期内，虽然两个 $x$ 可能对应相同的 $y$ 值，但由于导数不同，它们在周期中的位置依然是可以区分的。因此，完整描述一个波动状态，不能仅看振幅 $y$，还需要知道它处于上升还是下降的阶段。
