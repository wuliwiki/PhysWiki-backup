...
从台阶的故事中可以看出，不论是每次跨一步、两步还是三步，这些方式都有一个共同的特点：每次跨的台阶数是固定的，即两次动作之间的差始终相等。这种规律不仅出现在上台阶这样的情景中，还广泛存在于其他现象中，例如每天增加固定的储蓄金额，或者不断调整音量时每次调节相同的数值。无论这些场景多么不同，它们的本质都是一种“均匀增加或减少”的过程，由固定的差值串联起来，使得整个变化过程充满了可预测性和规律性。
...
对等差数列 $\{a_n\}$，根据等差数列的递推公式不断迭代，可以得到$n>1$时有：
...
\end{aligned}
\end{equation}
然后把它们左侧和右侧分别全加起来，这时左侧的很多项会因为符号的正负而消掉，剩余的部分只有$a_n-a_1$，左侧则是$n-1$项和，即：
\begin{equation}\label{eq_HsAmPg_8}
a_n -a_1=(n-1)d~.
\end{equation}
\autoref{eq_HsAmPg_6} 的推导过程与\autoref{eq_HsAmPg_7} 推导出 \autoref{eq_HsAmPg_8} 的过程是研究数列的两个重要构造方法。
