...
ROC曲线分析可以帮助我们做模型选择，选择最优模型，抛弃次优模型。曲线下面积（Aera under curve, AUC）是ROC曲线最基本的评估指标，能表示分类器预测能力的一项常用的统计值。顾名思义，表示的是曲线下方，横轴上方的面积。通常把整图的面积定义为1，则AUC的值在$0$和$1$之间。该面积值越大，则表示分类模型效果越好；反之，则越差。

前面提到，ROC曲线越靠近右上方越好，因此，ROC曲线下的面积越大越好，代表模型的效益越高。
\begin{itemize}
