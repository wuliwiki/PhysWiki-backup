...
可以证明， 如果 $V(x,t)$ 关于 $x$ 可以被泰勒展开\upref{Taylor}， 那么动量表象的薛定谔方程为
\begin{equation}
\frac{k^2}{2m}\varphi(k, t) + V\qty(\I \pdv{k}, t)\varphi(k, t) = \I \pdv{t} \varphi(k,t)
\end{equation}

\textbf{证明}： \autoref{moTDSE_eq2} 代入\autoref{moTDSE_eq1} 得
\begin{equation}
\mathcal{F}^{-1}\qty[1] + V(x, t)\mathcal{F}^{-1}\qty[2] = \I \pdv{t}\mathcal{F}^{-1}\qty[3]
\end{equation}


， 再利用\autoref{FTExp_eq9}~\upref{FTExp} （\autoref{FTExp_eq9}~\upref{FTExp} 两边取反傅里叶变换， 令 $f_1(x) = V(x, t)$， $g_2(k) = \varphi(k, t)$）即可． 证毕．
