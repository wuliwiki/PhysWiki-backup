...
\begin{issues}
\issueDraft 统一标势：phi,lambda->varpsi
\end{issues}

\pentry{电磁场标势和矢势\nref{nod_EMPot}}{nod_4d5d}

\footnote{参考 Wikipedia \href{https://en.wikipedia.org/wiki/Gauge_fixing}{相关页面}。}虽然标势和矢势可以唯一确定电磁场，但是同一个电磁场却可能对应不同的标势和矢势。 这是因为在麦克斯韦方程中，有物理意义的是$\bvec E,\bvec B$。我们只需保证：尽管改变四维电磁矢势$(\phi,\bvec A)$，也对应相同的$\bvec E,\bvec B$即可。

由于$\curl  A=\bvec B$，利用函数的梯度是一个无旋场，我们可以令$\bvec A'=A+\curl\phi$
