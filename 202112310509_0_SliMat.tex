...
\subsection{Vec 矢量}
VecDoub 矢量继承 VbaseDoub， 为了避免歧义， 任何两个矩阵之间禁止使用等号． 如果要复制使用 \verb|copy()| 函数．
\begin{lstlisting}[language=cpp]
...
\subsection{Cmat 矩阵}
CmatDoub 矩阵同样继承 VbaseDoub． 在此基础上增加一个行数 \verb|m_N0|， 列数 \verb|m_N1|， 双索引 \verb|a(i, j)|
\begin{lstlisting}[language=cpp]
...
public:
    ...
    CmatDoub(Long_I N1, Long_I N2);  // 初始化， 分配 N1 x N2 个元素
    ...
    Long n0() const; // 获取行数
    Long n1() const; // 获取列数
    void resize(Long_I N1, Long_I N2); // 重新分配
    void reshape(Long_I N1, Long_I N2); // 重新
};
\end{lstlisting}
