...
\item 如果所有 limit point 都属于集合， 这个集合就是\textbf{闭（closed）}的．

\item 如果关于点 $p$ 的某个邻域是集合 $E$ 的子集， $p$ 就是 $E$ 的 \textbf{interior point}

\item 如果 $E$ 中的任意一点都是 interior point， $E$ 就是 \textbf{开（open）} 的

\item \textbf{补集（complement）}

\item 如果一个闭集合中每一点都是 limit point， 那么集合就是 \textbf{perfect} 的
...
\item \textbf{紧集（compact set）}： if $\{G_\alpha\}$ is an open cover of $K$, then there are finitely many indices $\alpha_1,\dots, \alpha_n$ such that $K \subset G_{\alpha_1} \bigcup \dots \bigcup G_{\alpha_n}$
