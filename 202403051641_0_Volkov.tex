...
要求解该问题， 可以用 “一维自由粒子高斯波包\upref{GausWP}” 相似的思路，先把初始波函数拆解为无数平面波， 再分别把每个平面波在电场 $\bvec {\mathcal E}(t)$ 中演化到任意时间 $t$， 最后重新组合成为 $\Psi(x,t)$。 该过程中，把平面波在电场中演化到某时刻的结果就称为 \textbf{Volkov 波函数}。

以下我们分别在长度规范\upref{LenGau}、速度规范\upref{LVgaug}、加速度规范\upref{AccGau}中求解 Volkov 波函数。 在 $t \le t_0$ 时， 令 $\bvec {\mathcal E}(t) = \bvec A(t) = \bvec 0$。 令三种规范下 $t \le t_0$ 的波函数都是如下平面波（不同规范仅当电场出现后才会不同）
