...
（A）$r(\mat A)=2,r(\overline{\mat A})=3 \qquad$ 
（B）$r(\mat A)=2,r(\overline{\mat A})=2 \qquad$\\
（C）$r(\mat A)=1,r(\overline{\mat A})=2 \qquad$ 
（D）$r(\mat A)=1,r(\overline{\mat A})=1 \qquad$ 

7.设$A,B$为随机事件，则$P(A)=P(B)$的充分必要条件是（）\\
（A）$P( A \cup B)=P(A)+P(B) \qquad$  （B）$P(AB)=P(A)P(B)$\\
（C）$P(A\overline{ B })=P(B\overline{A}) \qquad$  (D)$P(AB)=P(\overline{A} \overline{B})$