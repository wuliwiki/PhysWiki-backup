...
由于 Git 的分支非常轻量级，这使得用户它时常会被作为草稿使用。例如一个程序仓库的 master 分支只用于储存比较重要的 commit， 而日常修改程序时创建一些草稿分支，里面的每次 commit 可能都是一些微小的修改，等到这些修改告一段落，再把该分支所有 commit 并入到 master 的一次 commit 并分支删除。
