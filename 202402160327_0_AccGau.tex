...
\footnote{本文参考 \cite{Bransden}。}本文使用原子单位制\upref{AU}。 首先注意\textbf{加速度规范（acceleration gauge）}并不是一种规范而只是薛定谔方程的一种变换， 说它是规范只是习惯上的叫法。 该变换也叫做 \textbf{Kramers-Henneberger 变换} 或 \textbf{K-H 变换}。

在速度规范\upref{LVgaug}下， 电场和矢势满足和库仑规范和速度规范同样的关系（\autoref{eq_Cgauge_2}~\upref{Cgauge}）
