...
\item 如果说用 key \verb`ssh` 的时候说 \verb`permission id_rsa too open`， 把 \verb|.ssh| 文件夹的权限修改为 \verb|700|， 里面的 \verb|authorized_keys| 修改为 \verb|600|， \verb|id_rsa.pub| 和 \verb|known_host| 改为 \verb|644|。 如果服务器端文件的权限不对会提示 \verb|permission denied|。
