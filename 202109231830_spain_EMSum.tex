...
Euler-Maclaurin求和公式是有关定积分的一种数值计算公式,它建立了函数的积分与其导数的联系.在数值积分理论与级数求和法中,Euler-Maclaurin求和公式是一个极有用的工具.前面我们已经得到了一个较为不错的求和式，为了得到更精确的结果，我们需要深入分析\autoref{EMSum_eq1} 右端积分的一些性质，因此下面引入Bernoulli数和Bernoulli多项式．
...
下面将介绍本篇的求和最终公式，它能为常见初等函数的求和提供极为精确的结果
\begin{theorem}{}
...
实际上，利用本篇内容可以较好地估计数值积分（梯形法）\upref{NumInt}的误差．
