...
频率为 $\omega$ 的一维线性谐振子，状态为 $\psi(x)=c_0 \psi_0(x)+c_3\psi_3(x)$ ，其中 $\psi_n(x)$ 为第 $n$ 个能量本征态，系数 $c_1$, $c_3$ 已归一化 ．给出能量的平均值．

由\autoref{QMavg_eq1}
\begin{equation}\label{QMavg_eq8}
\ev{E}=|c_0|^2 E_0+|c_3|^2 E_3
\end{equation}

其中 $E_n$ 为第 $n$ 个能量本征值：$E_n=\omega(n+\frac{1}{2})$，其中 $\hbar$ 已取为1．代入\autoref{QMavg_eq8} 即得该态的能量平均值．
\end{example}
...
&= -i|C|^2\int_{-a}^a (a^2-x^2)(-2x)\dd{x}\\
&= 0
\end{aligned}
\end{equation}
\end{example}
