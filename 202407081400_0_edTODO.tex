...
\item 所有 \verb`\autoref{标签}` 变为 \verb`@标签`，后面加一个空格（无论 \verb`\autoref{标签}` 后面是否已经有空格）。 转换回去的时候后面的空格删掉。
...
\item \verb|\verb`代码`|（注意 delimiter 也可能是其他任意单个符号如 \verb`\verb+代码+` 变为 \verb|`代码`|， 如果代码中已经有 \verb|`| 符号， 就使用多个作为 delimiter 如 \verb|``代码``|。 如果 \verb`代码` 的第一个或最后一个字符是 \verb|`|， 就用一个空格和 delimiter  隔开， 但该空格转换回 latex 时删掉。
