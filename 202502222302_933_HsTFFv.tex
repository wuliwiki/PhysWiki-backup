...
\begin{itemize}
\item 在右半平面（第一、第四象限）内，$\cos x > 0$，故 $\sin x$ 递增。
\item 在左半平面（第二、第三象限）内，$\cos x < 0$，故 $\sin x$ 递减。
\end{itemize}

同理，$(\sin x)'=\cos x$，结合 $\sin x$ 在不同象限的符号，可得：
\begin{itemize}
\item 在上半平面（第一、第二象限）内，$\sin x > 0$，故 $\cos x$ 递减。
\item 在下半平面（第三、第四象限）内，$\sin x < 0$，故 $\cos x$ 递增。
\end{itemize}

而 $(\tan x)' = \sec^2 x$，由于 $\sec x$ 在其定义域内无零点，故 $\sec^2 x > 0$ 恒成立。这表明 $\tan x$ 在每个连续区间上严格递增。然而，需要注意的是，导数恒为正只能保证函数在各个连续区间内递增，但不能直接推出在整个定义域内任取两个点都满足递增关系，因为 $\tan x$ 存在间断点。

另外，此处直接给出其余三个三角函数的导数和单调性：
\begin{itemize}
\item cot x
\item $sec$
\item csc
