...
\end{figure}
现在我们来看一看态$\phi(\mathbf x)|0\rangle$的物理意义．从\autoref{quanti_eq3}~\upref{quanti}可以看出
\begin{equation}
...
\begin{equation}
\langle 0 | \phi(\mathbf x) | \mathbf p \rangle = \langle 0 | \int \frac{d^3 p'}{(2\pi)^3} \frac{1}{\sqrt{2E_{\mathbf p'}}}(a_{\mathbf p'}e^{i \mathbf p'\cdot \mathbf x} + a^\dagger_{\mathbf p'} e^{-i \mathbf p' \cdot \mathbf x} ) \sqrt{2E_{\math}} 
\end{equation}
