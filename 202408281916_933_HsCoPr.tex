...
这里需要注意几点：
\begin{itemize}
\item 否定条件命题时，只否定后件，前件不用动。
\item “$P$不能推出$Q$”是一种符合语言习惯的表达，但很多同学会因为这个表达而对条件命题的否定造成混淆，不知道应该否定哪个部分。
\item 这里的否定和后面的否命题不是同一回事，辨析参见\autoref{ex_HsCoPr_1} 。
\item 到此为止，给出的命题都不涉及真值的判定，事实上条件命题既可以是真命题，也可以是假命题。但在高中教材中，有一些语焉不详，造成初次接触时，容易对条件命题的否定以及充分、必要条件的理解出现问题。因此，请注意文本的表达和教材表达的区别。具体说来，教材中认为如果$P$能推出$Q$，则记作$P\Rightarrow Q$，否则记作$P\not\Rightarrow Q$，也就是认为“$P$能推出$Q$”这句话或者是$\Rightarrow$这个符号是包含了真值判断的。本文则采用命题“如果$P$能推出$Q$”，记作$P\Rightarrow Q$，否定记作$P\not\Rightarrow Q$，然后再讨论命题的真值：如果命题为真，则否定为假，如果命题为假，则否定为真，的方式。
\end{itemize}
