...
\item 所以作为系统管理员，要想让不同组的用户可以访问不同的文件夹，那就首先把这个文件夹本身的权限设为 \verb|2750|， owner 设为 \verb|root|， 组设为可以访问的组。 所有子文件夹权限设为 \verb|775| 每个用户的 \verb|~/.bashrc| 中 \verb|umask| 设为 \verb|002| （默认为 022）。 然后把每个文件夹的所有子文件夹和子文件设置为同一个组， 文件权限设为 \verb|664|，
