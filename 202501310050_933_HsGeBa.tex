...
考虑一个直角三角形，其中一个锐角为 $\theta$，则三角形的三个边分别称作斜边、邻边和对边，具体说来：
\begin{itemize}
...
初中接触的三角函数主要包括\textbf{正弦（sine）}、\textbf{余弦（cosine）}和\textbf{正切（tangent）}。

\paragraph{正弦、余弦、正切的定义}

对直角三角形$ABC$，角$C$为直角，则对角$A$，对边、邻边和斜边分别为$BC,AC,AB$。则三角函数的定义如下：
\begin{equation}
\begin{split}
