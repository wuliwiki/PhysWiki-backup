...
\item 作为一个 bug 的报警， 每次以任何形式保存时， 记录下文件的 hash， 如果下次保存前发现文件的 hash 和记录的不同， 那么提示 \verb|服务器上该文件发生了修改， 和这里的内容冲突， 这是一个异常， 冲突版本已保存， 请马上停止编辑并联系管理员|． 并把当前编辑器的内容保存到 \verb|文件名-conflict.tex|．
