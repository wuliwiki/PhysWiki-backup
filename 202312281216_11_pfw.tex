...
设$f:(M,\triangle)\rightarrow(N,\widetilde {\triangle})$是保联络的微分同胚映射。$c(t)$是$M$上的光滑曲线，$\frac{D}{dt}$和$\frac{\widetilde D}{dt}$分别是$M$上沿$c(t)$与$N$上沿$f\circ  c(t)$的协变导数。如果$V(t)$是$M$上沿$c$定义的切向量场（即每一点的切向量不一定是曲线上该点的切向量）。那么我们有：
\begin{equation}
