...
\end{corollary}
\textbf{证明：}
若周期集 $F$ 中无最小正数，也即对任意 $\epsilon$, 有正周期 $c<\epsilon$，由 $F$ 是加法群， 所以 $mc$ （$m$ 是整数）是周期。任意实数 $c_0$，若选取满足
\begin{equation}
(c_0-\epsilon)/c<c_0/c-1<m<c_0/c+1<(c_0+\epsilon)/c~
\end{equation}
的整数，就有 $\abs{mc-c_0}<\epsilon$。于是 $c_0$ 是 $F$ 的收敛点（\autoref{def_realat_2}~\upref{realat}）。由 $F$ 是闭的，有 $c_0\in F$，由于 $F$ 的元素必定是实数，而现在任意实数都在 $F$ 中，所以 $F=\mathbb R$。
...
\end{equation}
由此，区间 $(m_1-x_1+x_2,m_2-x_1+x_2)$ 应合于 $(m_1,m_2)$，于是 $m_1=-\infty,m_2=+\infty$。\autoref{eq_EqliP_3} 也表明 $x_1-x_2$ 是解的周期。
