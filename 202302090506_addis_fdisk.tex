...
\item 先随便创建一个文件， 文件大小就是硬盘的初始大小。 如 \verb|sudo dd if=/dev/zero of=/路径/虚拟硬盘名.img bs=1M count=1000|。 其中 \verb|.img| 是不必须的。
\item 格式化为 ext4： \verb|sudo mkfs -t ext4 /路径/虚拟硬盘名.img|。 注意运行完以后， 镜像文件会自动缩小的刚好够用！
...
\item \verb|resize2fs 虚拟硬盘名.img 100M| 可以改变虚拟硬盘大小。
\end{itemize}
