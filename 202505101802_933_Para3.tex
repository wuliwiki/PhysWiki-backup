...
这个结论其实并不难证明。设抛物线的标准形式为 $y^2 = 2px$，其焦点为 $F\left( \frac{p}{2}, 0 \right)$，准线为 $l: x = -\frac{p}{2}$。

在抛物线上取一点 $P(x_0, y_0)$，连接 $PF$，并作 $P$ 到准线 $l$ 的垂线，垂足记作 $Q\left( -\frac{p}{2}, y_0 \right)$。



这证明起来也并不困难。设抛物线的标准形式为 $y^2 = 2px$，则其焦点为 $\displaystyle F\left(\frac{p}{2}, 0\right)$，准线为 $\displaystyle l: x = -\frac{p}{2}$。取抛物线上一点 $P(x_0, y_0)$，连接 $PF$，作点 $P$ 到准线 $l$ 的垂线，垂足为 $\displaystyle Q\left(-\frac{p}{2}, y_0\right)$。
...
|FP| = |PQ|~.
\end{equation}
即，$\triangle PQF$是一个等腰三角形。设$R$是底边中点，根据中点公式可知$R$点坐标为$\displaystyle (0, \frac{y_0}{2})$。根据等腰三角形的性质有$PR$是$\angle FPQ$的角平分线，或者说，$PQ$和$PF$关于$PR$对称。

根据\autoref{eq_Para3_7} ，点 $P$ 处的切线为：
\begin{equation}
y-y_0={p\over y_0}(x-x_0)~.
\end{equation}
代入$x=0$，并根据抛物线方程化简，可得其$y$轴截距为：
\begin{equation}
\begin{split}
y&=y_0-{px_0\over y_0}\\
...
\end{split}
\end{equation}
即$P$处切线过点$R$，即$PR$是点$P$处的切线。于是 $FP$ 与 $PQ$ 关于切线对称。综上，抛物线的反射性质得证。
