...
其实等比数列的通项公式很显然就是一个指数函数，根据等差数列的经验，等比数列实际上是指数函数的离散形式。换句话说，在直角坐标系中，如果将数列的项数 $n$ 作为横坐标，数列的值 $a_n$ 作为纵坐标，则点 $(n, a_n)$ 将分布在一条过$\displaystyle(0,{1\over q})$和$(1,a_1)$的指数函数的曲线上。
...
这一公式为分析数列增减性提供了基础，可以针对这个关系分以下几种情况讨论：
	1.	当 $q < 0$ 时：此时 $a_n a_{n+1} < 0$，说明相邻两项的符号相反，数列中任意相隔的项符号相同。这种数列称为\textbf{摆动数列}，表现为各项符号交替变化。
	2.	当 $q > 0$ 时：所有项的符号与首项 $a_1$ 的符号一致，具体分以下两种情况：
	•	如果 $q > 1$，则 $\frac{a_{n+1}}{a_n} > 1$。这意味着相邻两项的比值大于 1。如果 $a_1 > 0$，则 $a_{n+1} > a_n$，数列递增；如果 $a_1 < 0$，则 $a_{n+1} < a_n$，数列递减。
	•	如果 $0 < q < 1$，则 $\frac{a_{n+1}}{a_n} < 1$。这意味着相邻两项的比值小于 1。如果 $a_1 > 0$，则 $a_{n+1} < a_n$，数列递减；如果 $a_1 < 0$，则 $a_{n+1} > a_n$，数列递增。


如果$q<0$，那么$a_na_{n+1}<0$，即二者异号，这时数列相隔的项符号相同，数列是摆动数列。反之，$q>0$时，则所有项的符号与$a_1$相同。此时若$q>1$，则有$\displaystyle{a_{n+1}\over a_n}>1$，若$a_1>0$，则有$a_n>0$从而$a_{n+1}>a_n$，数列递增，反之若$a_1>0$，则有$a_{n+1}<a_n$，数列递减。若$0<q<1$，则有$\displaystyle{a_{n+1}\over a_n}<1$，若$a_1>0$，则有$a_n>0$从而$a_{n+1}<a_n$，数列递减，反之若$a_1>0$，则有$a_{n+1}>a_n$，数列递增。
...
G = \pm \sqrt{ab}~.
\end{equation}
这说明，$a$ 与 $b$ 为正的等比中项正好是他们的\textbf{几何平均值（geometric mean）}\footnote{关于几何平均值参见\aref{基本不等式}{sub_HsIden_2}。几何平均值的英语也和等比数列的英语使用了一样的单词 “geometric”。}。反过来，从\autoref{eq_HsAmPg_10} 也可以得到：
\begin{equation}
2A = a+b~.
\end{equation}
