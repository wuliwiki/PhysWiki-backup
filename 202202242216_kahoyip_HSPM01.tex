...
对称性：竖直上抛运动的物体在上升和下落阶段会经过同一位置，其运动具有对称性．上升和下落阶段经过同一位置时速度等大反向，即$v_{up}=-v_{down}$（速度对称）．上升和下落阶段经过同一段竖直距离所用的时间相等，即$t_{up}=t_{down}$（时间对称）．上升阶段为减速到0的过程，下落阶段为自由落体运动回到抛出点，上升阶段可以看作下落阶段的逆过程（过程对称）．