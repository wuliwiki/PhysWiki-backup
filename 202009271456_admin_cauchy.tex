...
度量空间 $X$ 中的任意柯西序列 $\qty{x_i}$ 都会收敛到某一点 $x$． 若 $x \in X$， 则 $x$ 是序列 $\qty{x_i}$ 的极限（\autoref{Metri2_def1}~\upref{Metric}）； 若 $x \notin X$， 则可以定义 $x$ 和任意 $y\in X$ 的距离为（该极限必存在）
\begin{equation}
...
在\autoref{cauchy_the1} 中我们介绍了一种方法， 可以给不完备度量空间 $X$ 不断加入新的元素． 如果我们不断重复这一过程， 就可以在 $X$ 的基础上得到一个唯一的完备空间 $\bar X$ 称为 $X$ 的完备化．
