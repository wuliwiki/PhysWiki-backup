...
那么一般来说我们如何定义双粒子态空间中的交换算符呢？ 首先我们要确定单粒子空间， 并找到一组单粒子的基底 $\qty{\ket{i}}$． 然后把两个单粒子空间做张量积得到双粒子的太空间， 基底取 $\qty{\ket{i}\ket{j}}$． 将双粒子态根据该基底展开， 再将每个基底替换为 $\qty{\ket{j}\ket{i}}$ 即可．
...
\end{equation}
该式也可以拓展到连续本征矢的情况． 例如在\autoref{ExchOp_eq3} 中， 单粒子的位置本征态基地记为 $\ket{\bvec r}$， 张量积空间的基底记为 $\ket{\bvec r_1}\ket{\bvec r_2}$， 那么\autoref{ExchOp_eq4} 中求和变为积分
\begin{equation}
...
$N > 2$ 个全同粒子波函数的交换算符不止一个， 我们可以把交换粒子 $i,j$ （$i\ne j$） 的算符记为 $P_{i,j}$， 共有 $C_N^2 = n(n-1)/2$ 个． 这些算符中有一些是对易的（如 $P_{1,2}$ 和 $P_{3,4}$）， 一些是不对易的（如 $P_{1,2}$ 和 $P_{2,3}$）．

玻色子的态矢只能处于所有交换算符的对称子空间的交空间中\footnote{容易证明两个不同子空间的交集仍然是矢量空间}； 费米子的态矢只能处于所有交换算符的反对称子空间的交空间中．
...
\subsection{对称化和反对称化}
如果给定一个多粒子波函数处于对称或反对称子空间中， 我们可以将其\textbf{对称化}或\textbf{反对称化}． 先看有限维空间中的例子：

\begin{example}{}\label{ExchOp_ex2}
假设单个粒子的一组单位正交基底是 $\qty{\ket{i}}$， 两个粒子某时刻的态矢可以用单个张量积表示 $\ket{i}\ket{j}$ 且 ． 显然若 $i = j$， 则这个态已经是对称的．  若 $i \ne j$， 则态矢是不对称的， 我们可以通过乘以 $(1 \pm P_{12})/\sqrt{2}$ 的方法对称和反对称化
\begin{equation}\label{ExchOp_eq5}
\frac{1}{\sqrt{2}}(1 \pm P_{12})\ket{i}\ket{j} = \frac{1}{\sqrt{2}}(\ket{i}\ket{j} \pm \ket{j}\ket{i})
\end{equation}
其中正号和负号分别代表对称和反对称化． 由于等式中的两项也是正交归一的， 我们需要另乘归一化系数 $1/\sqrt 2$． 可以验证， 该式就是 $P_{1,2}$ 的本征矢， 本征值分别为 $\pm 1$． 以上的（反）对称化方法利用了交换算符的性质 $P_{1,2}^2 = 1$．
\end{example}
