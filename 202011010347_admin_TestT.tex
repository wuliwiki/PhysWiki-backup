...
That way, we have $f(\bvec{v}, \bvec{u})= \bvec c_v^T\bvec{M} \bvec c_u$．Note that $\bvec c_v^T$ means the transpose of the matrix $\bvec c_v$\footnote{If we use other bases, then the representations of $\bvec{v}$, $\bvec{u}$ and $f$ may be different matrices, but the calculation of $\bvec c_v^T\bvec{M} \bvec c_u$ doesn't change}．
...
This observation suggests a simple way to determine the order of a tensor: the order is just the number of vector spaces involved. For example, if a tensor maps 3 vectors to 2 vectors, then its order is 5; if a tensor maps 2 vectors to a number, then its order is 2. 
