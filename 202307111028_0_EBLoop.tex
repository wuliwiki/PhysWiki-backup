...
\end{equation}
\addTODO{引用 $\oint \dd{\bvec r}$ 的出处}

\subsection{证明}

下面为了书写方便， 使用 $\bvec x_1, \bvec x_2, \bvec x_3$ 代替 $\bvec x, \bvec y, \bvec z$。

若磁场是任意的， 那么（$i=1,2,3$， 下同）
\begin{equation}
\ali{
...
其中用到了斯托克斯定理（\autoref{eq_Stokes_1}~\upref{Stokes}）， $\Sigma $ 是以闭合曲线 $L$ 为边界的曲面。上式中
\begin{equation}
\begin{aligned}
