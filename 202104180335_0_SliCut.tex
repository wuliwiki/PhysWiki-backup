...
delete a[];
\end{lstlisting}
再看一个例子， 如何对子矩阵求和呢？ 我们需要一个参数叫 leading dimension， 该参数表示列标（行主元时：行标）增加 1 时单索引增加的数量．
\begin{lstlisting}[language=cpp]
...
\end{lstlisting}
可见直接使用裸指针给函数传递数组是非常灵活的， 在 C 语言中这也是很常见的做法， 见 BLAS 和 LAPACK． 然而这么做的缺点是容易出错， 例如索引超出分配的内存， 这样的错误调试起来会非常困难．
