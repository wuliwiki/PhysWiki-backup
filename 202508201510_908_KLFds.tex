...
在实数情形下，这一结果特别意味着：
$$
\operatorname{Cl}_{p+2,\,q}(\mathbb{R}) \;\cong\; M_2(\mathbb{R}) \otimes \operatorname{Cl}_{q,\,p}(\mathbb{R}),~
$$
$$
\operatorname{Cl}_{p+1,\,q+1}(\mathbb{R}) \;\cong\; M_2(\mathbb{R}) \otimes \operatorname{Cl}_{p,\,q}(\mathbb{R}),~
$$
$$
\operatorname{Cl}_{p,\,q+2}(\mathbf{R}) \;\cong\; \mathbb{H} \otimes \operatorname{Cl}_{q,\,p}(\mathbf{R}),~
$$
其中 $M_2(\mathbf{R})$ 表示 $2 \times 2$ 实矩阵代数，$\mathbb{H}$ 表示四元数代数。这些公式可以用于求出所有实克利福德代数与复克利福德代数的结构；详见克利福德代数的分类。
