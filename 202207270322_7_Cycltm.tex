...
任取$m=\prod_{k=1}^r p_k^{g_k}$，其中$1\leq g_k\leq f_k$
\end{theorem}
...
设$\varepsilon$是任意的$p$次本原单位根．

对于$p^k$次单位根$\alpha$，如果$\alpha$是本原的，那么存在正整数$s$使得$\alpha^s=\varepsilon$；反过来，如果存在正整数$s$使得$\alpha^s=\varepsilon$，那么$\alpha$的幂次遍历所有$\varepsilon$的幂次，即遍历所有$p^k$次单位根，故$\alpha$本原．因此，$p^k$次单位根$\alpha$是本原的，当且仅当存在正整数$s$使得$\alpha^s=\varepsilon$．
...
\begin{example}{\autoref{Cycltm_the3} 的实例}
\begin{equation}
\leftgroup{
    \Phi_2(x) &= x+1\\
    \Phi_4(x) &= x^2+1\\
