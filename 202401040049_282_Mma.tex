...
\item \verb|Eigenvectors[{{1,2},{3,4}}]|， \verb|Eigenvalues[{{1,2},{3,4}}]| 计算矩阵本征值。 或者 \verb|EigenSystem[矩阵]|， 一次返回 \verb|{本征值，本征矢}|，和分别计算是一样的。 注意返回的本征矢矩阵每行是一个本征矢而不是每列。 注意符号计算时返回的本征矢未必是归一化也\textbf{未必是正交}的（因为这往往需要很久表达式）， 数值计算时返回的是正交归一本征矢。
