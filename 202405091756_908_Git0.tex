...
\footnote{可以参考\href{https://git-scm.com/book/en/v2}{官方教程}。}Git 最初是一个 \enref{Linux}{Linux0} 上的命令行程序， 也就是说使用者需要在 Linux 系统的命令行中手动输入 \verb|git| 命令和相应的参数进行所有操作。 GitHub Desktop\upref{GitHub} 这样的程序仅仅提供了图形界面（GUI） 但底层仍然通过调用 \verb|git| 命令实现具体操作。
