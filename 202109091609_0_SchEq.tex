...
可以证明只有对于某些离散的 $E$ 我们才能解出这些波函数． 我们把每个 $E_i$ 叫做\textbf{能级（energy level）}， 对应的波函数叫做\textbf{束缚态（bound state）}．
...
\subsubsection{简并}
能量简并是指一个能量本征值 $E$ 对应定态薛定谔方程的多个线性无关的归一化波函数的情况． 由施图姆—刘维尔定理\upref{SLthrm}可知一维束缚态不存在简并．
\addTODO{如何证明？}
...
由于散射态不能归一化， 它们并不是物理的， 即粒子不能处于某个散射态． 然而散射态的线性组合\footnote{这里指积分}却可能可以归一化． 例如自由粒子（$V(x) \equiv 0$）的高斯波包\upref{GausPk} 就可以看作由平面波线性叠加而来， 即反傅里叶变换．
