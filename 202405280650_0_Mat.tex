...
矩阵概念最早出现于\textbf{线性方程组}中，因此其概念不可避免地涉及矩阵元素之间相乘、相加的运算，即矩阵乘法，见本文\autoref{sub_Mat_1} 。事实上，矩阵的威力正来自矩阵的乘法运算（\autoref{sub_Mat_1}），因此即便某个矩阵的元素不是数字，我们也希望这些元素能拿有乘法、加法等运算。目前，我们先默认矩阵的元素都是数字。\footnote{延伸阅读：\enref{指标与求和约定}{EinSum}（这是矩阵的另一种表达方式）。}
...
可用矩阵与列向量的乘法表示为（有时候也会用方括号表示矩阵和向量）
...
令列向量 $\mat x = \pmat{x_1& \dots& x_n}\Tr $，  $\mat y = \pmat{y_1&y_2& \dots& y_n}\Tr$， 系数矩阵为 $\mat A$， 上式可记为
