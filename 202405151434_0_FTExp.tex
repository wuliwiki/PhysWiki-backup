...
g^*(k) = g(-k)~.
\end{equation}
注意 $g(k)$ 的实部是偶函数，虚部是奇函数，因此往往只需要 $k$ 的正半轴即可得到所有信息。 上式就是 $f(x)$ 为实函数的充要条件， 要证明充分性， 将其代入\autoref{eq_FTExp_1} 可得 $f(x) = f^*(x)$。

另外容易证明如果 $f(x)$ 是偶函数或奇函数，那么 $g(x)$ 将也是。

由此可得，若 $f(x)$ 同时是实函数和偶（奇）函数，那么 $g(x)$ 将也是。

\begin{equation}
g(-k) = \int f(-t)e^{-ikt} dt = \int_{-b}^{-a} f(-x) dx~.
\end{equation}
