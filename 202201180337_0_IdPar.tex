...
\begin{example}{}
若某一维势阱中有两个互不作用的全同粒子， 单个粒子在该势阱中可能有 $N$ 个正交归一的束缚态 $\psi_i(x)$ （$i=1,2,\dots, N$）， 它们可以看作是 $N$ 维希尔伯特空间中的基底．

那么双粒子波函数的一组基底为 $\psi(x_1, x_2) = \psi_1(x_1) \psi_2(x_2)$ 对不同的函数 $f, g$ 既不是对称也不是反对称， 因为 $f(x_2)g(x_1) \ne \pm f(x_1)g(x_2)$．
\end{example}
