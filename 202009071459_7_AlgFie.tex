...
外代数最重要的例子是外微分\upref{ExtDif}，它是微分形式作为线性空间的外代数．$n$次外代数可以视为线性空间上的$n$维体积，在$n=2$的时候就是我们所熟悉的行列式：取定线性空间$V$的基$\{\bvec{e}_i\}$，那么$\sum\limits_{i}(a_{i}\bvec{e}_i)\wedge\sum\limits_{j}(b_{j}\bvec{e}_j)=\sum\limits_{i, j}(a_ib_j-a_jb_i)$
