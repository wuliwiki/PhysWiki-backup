...
接下来设$n>2$。若$A$无特征值，则必有一二维不变子空间，设为$S$,$A|_{S}$为\autoref{eq_ortho_2}。$A|_{S^{\bot}}$为正交矩阵，若$\lambda_1$为其特征值，则必定有一维不变子空间，设对应的特征向量为$\bvec x_1$，则可以扩展为$S^{\bot}$上的标准正交基，设$\bvec x_2$的正交补为$S_1$，则
\begin{equation}
