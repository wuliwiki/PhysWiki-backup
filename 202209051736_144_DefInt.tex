...
每一本高数教材\cite{同济高}\cite{Thomas}都会列出一张长长的定积分运算律的表格．这里只列举一些比较有意义的．尽管这些定理看起来都\textsl{非常显然}，但在实际做题时往往会给初学者带来不少困惑．
\begin{itemize}
\item 仅仅改变积分变量的名称不影响积分的结果．
$$\int^b_a f(x) \dd x = \int^b_a f(t) \dd t$$
\item 被积函数可加性．注意乘法可没有这么好的性质，不然我们也不需要大费周章地引入各种方法计算积分了！
...
\item 积分中值定理
$$\exists \xi \in [a,b], \int^b_a f(x) \dd x = f(\xi)(b-a)$$
\end{itemize}
