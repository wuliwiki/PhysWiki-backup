...
\subsubsection{新备份算法}
\begin{itemize}
\item editor 备份步骤： 更新 \verb|contents/文件名.tex|， 然后运行 \verb|PhysWikiScan --backup 文件名|，不需要修改数据库。如果新增了备份，就会在 stdout 输出 \verb|added 文件名|。 若已存相同 hash 的备份文件则输出 \verb|exist 已存在的文件名|。
