...
需要注意，尽管在几何问题中，三角函数通常仍然使用角度作为单位，但为了使三角函数真正成为“函数”，并与当前学习的实数函数体系结合，通常从函数的视角来看待三角函数时，需要引入弧度制，使角度成为三角函数的自变量。引入弧度制会让三角函数的性质更好，例如，在弧度制下，$\sin x$ 在 $x=0$ 处的导数为 $1$，这一性质使得微积分中的运算更加简洁自然。

如之前介绍任意角和弧度制时所说，将三角函数的研究通常也放在单位圆中，此时有一个重要的优势：由于单位圆的半径恒为 $1$，定义中不会因半径的不同而影响正弦和余弦的值（而正切 $\tan\alpha$ 的表达式中本身就没有 $r$，因此它与半径无直接关系）。这一推广方式不仅使三角函数的定义更加一般化，也更有利于后续的函数分析和计算。
...
此外，如之前介绍任意角和弧度制时所述，通常将三角函数的研究放在单位圆中，这样做有一个重要的优势：由于单位圆的半径恒为 $1$，定义中不会因半径的不同而影响正弦和余弦的值（而正切 $\tan\alpha$ 的表达式中本身不涉及 $r$，因此它与半径无直接关系）。这一推广方式不仅使三角函数的定义更加一般化，也更有利于后续的函数分析和计算。
