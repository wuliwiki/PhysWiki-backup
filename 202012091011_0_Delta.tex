...
$\delta(x)$ 在泛函分析中并不是数学中一个严格意义上的函数， 而是被称为\textbf{广义函数（generalized function）}或\textbf{分布（distribution）， 详见泛函分析}． 但这里我们给出另含有 $\delta(x)$ 等式的一种严谨的理解． 令一个含有参数 $\alpha$ 的一元函数 $f_\alpha(x)$． $\alpha$ 可以是连续的或离散的． 如果 $\alpha$ 是离散的， 我们可以将其看作一个由无穷多个函数组成的函数序列 $f_1(x), f_2(x), \dots$． 若 $f_\alpha(x)$ 在所有 $x \ne 0$ 处满足
\begin{equation}
