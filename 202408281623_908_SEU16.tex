...
\textbf{7.(15 分)}质量为 $\mu$ 带电 $q$ 的粒子限制在半径为 $r$ 的圆环上运动，圆环处在一条细长的磁通管中，磁场限制在细管内，圆环上及其近区域的磁场为 $0$。设通过圆环的磁通量为 $\Phi$，则圆环近区域的电势可取为（在适当的标示中）
\[
\Lambda(\rho, \phi, z) = \left( \frac{\Phi}{2\pi\rho} \right) e_\phi.~
\]

\begin{enumerate}
    \item [(1)] 写出粒子的哈密顿算符；
    \item [(2)] 求能量本征值，并讨论能级简并情况。
\end{enumerate}