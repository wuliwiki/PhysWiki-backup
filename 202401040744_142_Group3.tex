...
\begin{example}{置换矩阵}
参考\autoref{def_Perm_1}~\upref{Perm}，$\rho(\pi): = R_\pi$

向量空间 $V$ 的 $n$ 次笛卡尔幂空间 $V^{\times n}$ 上存在一个 $n$ 阶对称群 $S_n$ \upref{Perm}的群作用：
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
被称为\textbf{置换作用}（或者\textbf{置换表示}，参考群表示\upref{GrpRep}）。
\end{example}
