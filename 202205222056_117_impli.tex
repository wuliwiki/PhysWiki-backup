...
② $F\in\mathrm{C}(D\times E)$, 即 $F(x,y)$ 在 $D\times E$ 上连续；
% \footnote{\textsl{注}：此条件指 $F(x,y)$ 关于 $y$ 的偏导数 $F_{y}(x,y)$
% 在 $D\times E$ 上连续. 注意, 这里加强了条件 ③， 要求 $F_{y}(x,y)$ 必须对两个变元连续, 不可以只对 $y$
% 变元连续, 更不可只对 $(x_0,y_0)$ 连续 因为推导公式 ( \autoref{impli_eq2} ) 时会用到.} 
...
都存在\footnote{条件 ④ 若只是对某个偏导数成立也有相应结论. }, 且 $F_y\in\mathrm{C}(D\times E)$,
