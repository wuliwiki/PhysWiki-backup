...
\begin{itemize}
\item 我们常常遇到两个或多个方程需要同时成立的情况，这时会把他们放在一起称为“方程组”，求解方程组的方法称为“消元”。
\item 证明某个四边形是矩形，就需要满足以下两个条件同时成立：四边形的对角线相等、四个角都是直角。
\item 在集合论中，$A \cap B$表示集合$A$和$B$的交集，其中的元素必须既属于$A$，又属于$B$。
\end{itemize}

若逻辑连接词连接的两个命题$A,B$只有同时成立，整个复合命题才为真，则这个逻辑连接词称为\textbf{且}（and，也称为\textbf{而且}、\textbf{同}、\textbf{合取}），记作$A\land B$\footnote{符号不做硬性要求，下同。}。在上面的例子中，一个方程组中方程的关系、用来描述某个事物的多个条件的关系，交集的元素和原始集合的关系，就是“且”。“消元”依据的也就是“且”这个前提。

\textbf{真值表}是一种用来系统地列出逻辑命题在各种情况下的真值的工具。通过真值表，我们可以清楚地看到复合命题在不同输入条件下的结果。下面是“且”的真值表。
...
通过看真值表可以看出只有当$A$和$B$都是真的时候$A\land B$（$A$且$B$）才是真，否则只要$A$和$B$里面有任何一个是假（当然也包括两个全是假），$A\land B$就是假。
...
\item 如果一个实数$x\neq0$，则要么$x<0$，要么$x>0$。
\item 在集合论中，$A \cup B$表示集合$A$和$B$的并集，其中的元素bu s在$A$中，要么在$B$中，要么同时在两者之中。
\end{itemize}
若逻辑连接词连接的两个命题$A,B$只要有一个成立，整个复合命题就为真，则称这个逻辑连接词为\textbf{或}（or，也称为\textbf{或者}、\textbf{析取}），记作$A\lor B$。在上面的例子中，一个方程组中方程的关系、用来描述某个事物的多个条件的关系，并集的元素和原始集合的关系，就是“且”。“消元”依据的也就是“且”这个前提。
...
“或”的实例：
\begin{itemize}
\item 我们常常遇到两个或多个方程需要同时成立的情况，这时会把他们放在一起称为“方程组”，而一个方程组中的方程就是“且”的关系。求解方程组的消元方法，依据的就是“且”这个前提。
