...
\begin{equation}
\begin{aligned}
\hat u(k)&=h\sum_{i=-\infty}^{\infty}e^{-\I kx_i}\qty(\sum\limits_{j}a_jf_j(x))\\
&=\sum\limits_{j}a_j\qty(h\sum_{i=-\infty}^{\infty}e^{-\I kx_i}f_j(x))\\
...
\end{aligned}\right.~
\end{equation}
表示为 $u(x_j)=\sum\limits_{m=-\infty}^{\infty}u(x_m)\delta(x_j-x_m)$。因此，若知道 $\delta(x)$ 的傅里叶变换 $\hat\delta(k)$，则\autoref{eq_InfDM_3} 的第一式就为 
\begin{equation}
\hat u(k)=\sum\limits_{m=-\infty}^\infty u(x_m)\hat\delta(x-x_m).~
\end{equation}
从而\autoref{eq_InfDM_4} 的 $p(x)$ 成为
\begin{equation}
\begin{aligned}
p(x)&=\frac{1}{2\pi}\int_{-\pi/h}^{\pi/h} e^{\I kx}\hat u(k)\dd k\\
&=\sum\limits_{m=-\infty}^\infty u(x_m)\frac{1}{2\pi}\int_{-\pi/h}^{\pi/h} e^{\I kx}\hat\delta(x-x_m)\dd k
\end{aligned}~
\end{equation}
