...
设 $V$ 是一个集合，$E\subset [A]^2$，则称二元组 $G=(V,E)$ 为\textbf{图}（graph），$V$ 的元素称为 $G$ 的\textbf{顶点}（vertex），$E$ 的元素称为 $G$ 的\textbf{边}（edge）。
\end{definition}

当然顶点也可以称为节点（node）或点（point），边还能叫做线（line），这都是习惯的问题。

有些地方会约定 $E\cap V=\emptyset$ 来避免记号的混淆。这里不这样做，因为 $E$ 是 $V$ 的二元子集构成的集，其元素是二元子集，不是二元子集的元素本身。
