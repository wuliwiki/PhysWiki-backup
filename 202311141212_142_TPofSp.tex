...
\pentry{张量积\upref{TsrPrd}，对偶空间\upref{DualSp}}
...
\red{新：}对于有限维度矢量空间来说，我们可以用对偶空间\upref{DualSp}和多线性映射空间\upref{MulMap}来定义来定义它们之间的张量积。
...
在张量的张量积\upref{TsrPrd}中我们已经定义过线性映射（对偶向量）的张量积了：考虑 $f \in V_1^*,g \in V_2^*$，我们有 $(f \otimes g)(v_1, v_2) = f(v_1) f(v_2)$，
...
\end{equation}
是双线性映射空间 $\mathcal L(V_1,V_2;\mathbb F)$ \upref{MulMap}的一组基。
\end{lemma}
...
又因为 $\mathcal L(V_1,V_2;\mathbb F)$ 的维数为 $nm$，由基的定义（\autoref{def_VecSpn_2}~\upref{VecSpn}），所以\autoref{eq_TPofSp_1} 就是它的一组基。
...
注意：上述的引理和推论，将矢量空间和其对偶空间互调仍然成立，因为彼此是对方的对偶空间（参考对偶空间\upref{DualSp}）。
