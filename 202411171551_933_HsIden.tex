...
\textbf{柯西-施瓦茨不等式（Cauchy-Schwarz Inequality，或柯西不等式）}为两个向量或数列的内积与它们的模长之间建立了不等关系。

\textbf{柯西-施瓦茨不等式（Cauchy-Schwarz Inequality）}，也称\textbf{柯西不等式（Cauchy Inequality）}，为两个向量或数列的\textbf{内积（inner product）}与它们的模长（norm）之间建立了重要的不等关系。在数学上，柯西-施瓦茨不等式的基本形式为：
