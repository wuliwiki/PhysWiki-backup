...
\end{equation}
\end{definition}
事实上， 教材上（如\cite{Arfken}）常使用另一种定义， 但笔者认为这两种定义是等价的\footnote{笔者不会证明，欢迎指正}， 且前者更容易理解
...
\end{equation}
注意该式中的极限并不总是存在， 等号也不是通常意义的两函数相等， 而是隐含了\autoref{Delta_eq4} 和\autoref{Delta_eq5} 两个条件． 另外在一些教材会把 $\delta(x)$ 的性质简单记为
