...
接下来的问题是，如何在单位圆中构造包含角 $\alpha$ 的直角三角形，既能够利用原本直角三角形的定义，又能够让新定义的概念天然地符合扩展之前的。在直角三角形中，正弦函数的定义为$\displaystyle\sin\alpha = \frac{\text{对边}}{\text{斜边}}$

在坐标系中，过点 $P$ 向 $x$ 轴作垂线，可以构造出一个直角三角形。此时，斜边即单位圆的半径 $r$，角 $\alpha$ 的对边是点 $P$ 的纵坐标 $v$，邻边是横坐标 $u$。由于这三个量对于任意角 $\alpha$ 都是确定的，因此可以自然地推广三角函数的定义：
