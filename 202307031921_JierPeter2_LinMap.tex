...
比如说，考虑三维几何向量的\textbf{投影变换}，记为$\pi$。任取若干向量$\bvec{v}_i$，它们的线性组合$\sum_{i}a_i\bvec{v}_i$也是一个向量。你可以先将$\bvec{v}_i$组合起来，再将结果用$\pi$投影，得到$\pi\qty(\sum_{i}a_i\bvec{v}_i)$；也可以先将每个$\bvec{v}_i$投影为$\pi(\bvec{v}_i)$，再用同一组系数$a_i$做组合，得到$\sum_{i}a_i\pi(\bvec{v}_i)$。验算一下，两条路是不是殊途同归
