...
显然上面举出的三个判别法一个比一个强. 例如, 假若存在常数$0<q<1$使得$a_{n+1}/a_{n}\leq q$恒成立, 那么相应的拉阿贝序列$na_n/a_{n+1}\geq nq^{-1}$会增长至无穷, 由此可见比值判别法只是拉阿贝判别法的非常弱的特殊情形. 如果$\lim_{n\to\infty}a_{n+1}/a_{n}=1$, 那么拉阿贝判别法和贝特朗判别法实际上就是在更加精细地计算比值$a_{n+1}/a_{n}$趋于1的速度. 
