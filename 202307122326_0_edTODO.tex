...
\subsubsection{图片相关}
\begin{itemize}
\item 上传新的图片时， 插入信息到 \verb|images| 中的 \verb|hash, ext, figures| \verb|figures| 表格（只需要 \verb|id,entry|）。 \verb|PhysWiki/figures/| 中的文件使用 \verb|images.hash| + 拓展名命名。 \verb|\label{}| 格式为 \verb|fig_词条名_序号|。
