...
\end{equation}
其中 $c_i$ 是 $d$ 个任意常数（当方程有唯一解时 $d = 0$）， 无论这些常数取什么值， $\bvec x$ 都是方程的解． 另一方面， 给出方程的任意一个解， 总能找到一些常数 $c_i$ 与之对应． \autoref{GAUSS_eq18} 叫做方程的\textbf{通解（general solution）}， 通解中的任意一个就做方程的\textbf{特解（special solution）}（例如上式中所有的 $\bvec x_i$ 都是特解）．
