...
设$M(n,\mathbb R)$是一个$\mathbb{R}$上的$n$阶矩阵李群，同时也可视作一个李代数，李括号由矩阵乘法导出。可以证明其李代数$\mathfrak gl(n,R)$同构于$\opn{Lie}GL(n,\mathbb R)$，因此可以用来导出左不变切场的积分曲线。
\begin{equation}\label{eq_LieGA_3}
c(t) = \{ \mathcal{X}\in \opn{GL}(n, \mathbb{C}) \mid \E^{t\mathcal{X}}\in M, \forall t\in\mathbb{R} \}~.
\end{equation}
...
同\autoref{eq_LieGA_3} ，$GL(n,\mathbb R)$的李代数是全体实n阶矩阵$M(n,R)$
