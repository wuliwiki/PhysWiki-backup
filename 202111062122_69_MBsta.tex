...
\end{equation}
$\omega_l$ 是简并度，那么对于单个量子态，粒子数为 $e^{-\alpha-\beta \epsilon_l}$．现在我们列举一切能级中的一切量子态，它们的能级依次是 $\{\epsilon_s\}$．我们有
\begin{equation}
N=\sum_s e^{-\alpha-\beta \epsilon_s}
\end{equation}

这种分布出现的概率最大．下面我们来确定 $\alpha,\beta$ 的值．

\subsection{玻尔兹曼分布}
我们定义配分函数 $Z(\beta)=\sum_l e^{-\beta \epsilon_l}$