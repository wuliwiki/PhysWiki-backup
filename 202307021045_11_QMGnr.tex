...
设$ F$为多粒子体系的总单体算符，对应的单粒子算符为$f$，那么总的测量结果为单粒子测量结果之和。引入粒子数算符$N(f_i)=a(f_i)^\dagger a(f_i)$，其中${f_i}$为单体算符的本征值谱，该粒子数算符统计处于该本征值的粒子数目。那么$F=\sum ^i f_i a(f)^\dagger a(f) $。产生和湮灭算符的定义是在动量表象的，现在需要得到$q$表象下的形式，利用真空态和完备性关系，我们可以得到在任意两个表象下，产生和湮灭算符的关系
\begin{equation}
a^{\dagger}(f)\ket{0}=\ket{f}=\sum ^{h}\braket{h}{f}a^{\dagge}(f)\ket{0}~,
\end{equation}
\begin{equation}
