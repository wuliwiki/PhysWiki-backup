...
其中 $\langle a \rangle_0$ 表示 $a$ 的标量部分（即在 $\mathbb{Z}$-分次中的次数为 0 的部分）。可以证明，对于 $v_i \in V$：

$$
Q(v_1 v_2 \cdots v_k) = Q(v_1) Q(v_2) \cdots Q(v_k),
$$

但这一恒等式并不对 $\operatorname{Cl}(V, Q)$ 的任意元素成立。
...
此外，**左（或右）克利福德乘法**与**转置**之间满足伴随关系。具体地，元素 $a \in \operatorname{Cl}(V, Q)$ 的转置 $a^{\mathrm{t}}$ 定义了左（或右）乘算子的伴随：
...
要不要我接下来帮您翻译 **“Center”** 这一节？它一般紧跟在 “Clifford scalar product” 之后。
