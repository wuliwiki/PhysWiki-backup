...
F(r) = \frac{k}{r^2}
\end{equation}
其中 $k$ 为非零实数． $k>0$ 表示斥力， $k<0$ 表示引力． 由于质点的运动只和初始位置、速度和 $k$ 有关， 与质点的质量无关， 所以我们令 $m=1$ 并不影响一般性． 令发射位置的极坐标\upref{Polar}为 $(R,\alpha)$， 初速度为 $v_0$， 发射仰角为 $\beta$ （顺时针增量为正， $\beta=0$ 时质点延逆时针方向水平发射）．
