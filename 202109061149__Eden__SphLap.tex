...
\textbf{解：}由于空间各点不存在自由电荷分布，我们得知静电势 $\phi$ \upref{QEng} 在全空间（除了球壳上）满足拉普拉斯方程．又因为该问题关于 $\phi$ 对称（绕 $z$ 轴对称），我们的解中将仅涉及到 $m=0$ 的球谐函数．我们将静电势写成：
...
\begin{equation}
\phi_{in}(\bvec x)=A_1r\cos x,\ \
\phi_{out}(\bvec x)=(B_1r+\frac{C_1}{r^{2}})\cos\theta
\end{equation}

根据介质中的麦克斯韦方程\upref{MWEq1}，可以知道电位移矢量的法向分量连续（$$，