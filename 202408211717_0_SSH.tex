...
把 \verb`.ssh` 文件夹的权限修改为 \verb`700`， 里面的 \verb`id_rsa` 和 \verb`authorized_keys` 和 \verb`config` 修改为 \verb`600`， \verb`id_rsa.pub` 和 \verb`known_host` 改为 \verb`644`。 \verb`.ssh` 文件夹以及其中文件的 owner 都必须是登录者的。 如果服务器端文件的权限不对会提示 \verb`permission denied`。
