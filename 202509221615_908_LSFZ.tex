...
设$f: \mathbb{R}^k \to \mathbb{R}$,并且对每个
...
需要用到 $f$ 对其 $k$ 个自变量的偏导数。通常偏导符号会涉及函数自变量的名称，但在上式中自变量并没有被命名，因此用 **D-记号** 更为简洁和清晰。记

* $D_i f$：$f$ 关于第 $i$ 个自变量的偏导；
* $D_i f(z)$：该偏导在点 $z$ 的取值。
...
要不要我接下来帮你翻译 **“梯度与向量形式”** 的版本，把上式改写成更紧凑的点积形式？
