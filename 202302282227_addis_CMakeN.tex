...
\item \verb|string(REPLACE "老字符串" "新字符串" HEADER_OUT_FILE ${HEADER_IN_FILE})|
...
\item \verb|foreach(变量 IN 列表) ... endforeach()| 可以循环。 \verb|foreach(变量 变量1 变量2 ...) ... endforeach()| 可以循环。 \href{https://cmake.org/cmake/help/latest/command/foreach.html}{文档}。
\item \verb|find_program(变量 程序名 [路径1 路径2 ...])| 可以在指定路径或者 \verb|$PATH| 中寻找某个可执行文件， 并把绝对路径写入\verb|变量|。
\end{itemize}
