...
\end{split}
\end{equation}
因此，$a_2-a_1=3A+B-A-B-C=2A-C$，而对$n>2$有
\begin{equation}
a_n-a_{n-1} = (2n+1)A+B-(2n-1)A-B=2A~.
\end{equation}
由于$ABC\neq0$，即$C\neq0$，可以看出，$\{a_n\}$除$a_1$之外的项构成等差关系，但由于$2A-C\neq2A$，因此整体不构成等差数列。或者说，如果项构成等差数列，则要求其数列和必须为$S_n=An^2+Bn$的形式。
\end{example}