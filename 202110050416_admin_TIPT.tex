...
但考虑到 $H_0$ 简并， 它的一套本征矢（正交归一基底） $\psi_n^0$ 并不是完全确定的， 每个简并子空间中正交归一基底的选取都是有一定自由的． 为了让问题更简单， 我们在每个子空间中也要求 $\psi_n^0$ 基底下 $H^1$ 是对角化的， 即 $\mel{\psi_m^0}{H^1}{\psi_n^0}$ 的对角块都是对角化的． 注意这并不要求 $[H^0, H^1] = 0$ 对易， 因为对易要求矩阵 $\mel{\psi_m^0}{H^1}{\psi_n^0}$ 可以被彻底对角化而不只是每个对角块对角化． 这样， 通常就能唯一地确定 $\psi_n^0$， 但不唯一也关系不大．
