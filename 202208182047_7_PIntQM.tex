...
\begin{equation}
S(t_{n}-t_{n-1})=\int^{t_n}_{t_{n-1}}\dd t L(x, \dot{x})
\end{equation}
...
\begin{equation}
\ali{
    \bra{x_1}\E^{\I H \dd t}\ket{x_0} &= \qty(\frac{1}{\omega(\dd t)})\exp\qty(\frac{\I}{\hbar}S(\dd t))\\
    &= \qty(\frac{1}{\omega(\dd t)})\qty(1+\frac{\I}{\hbar}S(\dd t))
}
\end{equation}
其中$\omega$是比例和量纲的修正项．

我们对自由粒子应用上述猜想．注意到自由粒子的拉格朗日函数为$L=mv^2/2$，故无穷小段的作用量为（时间差$\Delta t$，从$x_0$到$x_1=x_0+v \Delta t$）：
\begin{equation}
\ali{
    S(\dd t) &= \int^{\Delta t}_0 \dd t\qty(\frac{mv^2}{2})\\
    &= \frac{mv^2}{2}\Delta t\\
    &= \frac{m}{2}\qty(\frac{}{})^2\Delta t
}
\end{equation}
