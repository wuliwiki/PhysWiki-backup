...
\end{aligned}
\end{equation}
其中$L^{\mu\nu}\equiv=\I(x^{\mu}\partial^{\nu}-x^{\nu}\partial^{\mu})$。显然这就是我们需要的，$SO^+(1,3)$线性算符表示的李代数。当然，我们还需要用李括号验证一下。类似\autoref{eq_PoinGr_2}  ，$L_i=(L_1,L_2,L_3)\equiv(L^{23},L^{31},L^{12})$。则
\begin{equation}
\begin{aligned}
...
设$\Lambda L(a)$为时空平移群的线性算符表示，假设同样可以用指数映射表示对坐标系平移任意时空四矢量，
