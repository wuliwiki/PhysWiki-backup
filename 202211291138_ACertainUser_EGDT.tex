...
\begin{figure}[ht]
\centering
\includegraphics[width=12cm]{./figures/EGDT_1.pdf}
\caption{球面两点间的最短路径} \label{EGDT_fig1}
\end{figure}

众所周知，球面上两点间的最短路径不再是一条直线段，而是一条圆弧曲线段．这也是为什么在平面地图上两点间的最短路径不是两点的直接连线，而是一条有弧度的曲线段．

以下是绘制地球上两点间最短路径的octave/matlab代码．该程序同时将返回该路径的长度，单位为$km$. 本代码的运行需要函数\verb|Sph2Cart| \upref{SphCar}．这个简略的模型只适用于演示，并且可能存在bug．%代码还写的很丑．．．
