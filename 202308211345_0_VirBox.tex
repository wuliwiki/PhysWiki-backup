...
\item 若没有快照，向硬盘中写入全 0 的文件不会增加 \verb|.vid| 文件的大小。 \verb|--compact| 也会把全零的 blocks 进行压缩。 所以用 \verb|dd if=/dev/zero of=zeros.txt status=progress;| 就可以把所有没有用到的硬盘都清零，便于 \verb||
\end{itemize}
