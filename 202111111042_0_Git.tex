...
\item 私有仓库用网址如果 clone, 每次连接服务器需要输入用户名和 token, 包括 \verb|clone, pull| 等． 要记住账户密码, 用 \verb|git config --global credential.helper store| 如果 github 修改了密码, 再用一次该命令即可, 要测试是否成功可用 \verb|git pull|， 如果成功则不需要密码
