...
\textbf{周期性情形}
当限制 $u(x_i)$ 对 $x_i$ 具有周期 $2L$ 时，即 $u(x_i+2L)=u(x_i)$，那么为了 $u$ 的单值性，限定 $x_i\in[-L,L]$。设 $x_i$ 有 $N$ 个，于是 $x\in \{h,\cdots,Nh\}$，其中 $h:=\frac{2L}{N}$。对 $u(x)$ 用正交函数系来展开应当将函数系的函数限制在区间 $[-L,L]$ 上。从而用
