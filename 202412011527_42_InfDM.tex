...
\end{aligned}\right.~
\end{equation}
表示为 $u(x_j)=\sum\limits_{m=-\infty}^{\infty}u(x_m)\delta(x_j-x_m)$。因此，若知道 $\delta(x)$ 的傅里叶变换，就知道了 $u(x)=\sum\limits_{m=-\infty}^\infty u(x_m)\delta(x-x_m)$ 的傅里叶变换，显然 $u(x_i)=u_i$。
