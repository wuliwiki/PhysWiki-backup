...
$\mathrm{Sp}(n, \mathbb{C})$是实群$\mathrm{Sp}(2n, \mathbb{R})$ 的复化。$\mathrm{Sp}(2n, \mathbb{R})$是一个实的、非紧致的、连通的单李群。[4]它的基本群同构于加法下的整数群。作为单李群的实型，它的李代数是可分解李代数。

$\mathrm{Sp}(2n, \mathbb{R})$ 的进一步性质：
\begin{itemize}
\item 从李代数$\mathfrak{sp}(2n, \mathbb{R})$ 到群 $\mathrm{Sp}(2n, \mathbb{R})$的指数映射不是满射。然而，该群的任意元素都可以表示为两个指数的乘积。[5]换句话说：
   $$
   \forall S \in \operatorname{Sp}(2n,\mathbb{R}) \;\; \exists X, Y \in \mathfrak{sp}(2n,\mathbb{R}) \;\; S = e^{X} e^{Y}.~
   $$
\item 对于所有$S \in \mathrm{Sp}(2n, \mathbb{R})$，有：
   $$
   S = O Z O',~
   $$
   其中
   $$
   O, O' \in \operatorname{Sp}(2n, \mathbb{R}) \cap SO(2n) \cong U(n),~
   $$
   且
   $$
   Z = \begin{pmatrix} D & 0 \\ 0 & D^{-1} \end{pmatrix}.~
   $$
   这里 $D$ 是正定对角矩阵。所有这样的 $Z$ 构成 $\mathrm{Sp}(2n, \mathbb{R})$ 的一个非紧子群，而 $U(n)$ 构成一个紧子群。这种分解被称为欧拉分解或Bloch–Messiah 分解。[6] 更多关于辛矩阵的性质可参见维基百科相关页面。
\item 作为李群，$\mathrm{Sp}(2n, \mathbb{R})$具有流形结构。该流形与酉群$U(n)$和一个维数为$n(n+1)$的向量空间的笛卡尔积微分同胚。[7]
\end{itemize}
