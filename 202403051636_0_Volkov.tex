...
本文使用原子单位制\upref{AU}。 若初始时刻 $t_0$ 一个自由粒子的波函数 $\Psi(x,t_0)$ 已知，接下来空间中出现了随时间变化的电场 $\bvec {\mathcal E}(t)$ （我们采用偶极子近似\upref{DipApr}， 即假设电场不随位置变化， 感生磁场可以忽略）\upref{DipApr}， 那么接下来波函数会如何变化呢？

要求解该问题， 可以用 “一维自由粒子高斯波包\upref{GausWP}” 相似的思路，先把初始波函数拆解为无数平面波， 再分别把每个平面波在电场 $\bvec {\mathcal E}(t)$ 中演化到任意时间 $t$， 最后重新组合成为 $\Psi(x,t)$。 该过程中，把平面波在电场中演化到某时刻的波函数就称为 \textbf{Volkov 波函数}。

以下我们分别在长度规范\upref{LenGau}、速度规范\upref{LVgaug}、加速度规范\upref{AccGau}中求解 Volkov 波函数。 在 $t \le t_0$ 时， 令 $\bvec {\mathcal E}(t) = \bvec A(t) = \bvec 0$。 令三种规范下 $t \le t_0$ 的波函数都是如下平面波（不同规范仅当非零电场出现后才有区别）
...
容易验证\autoref{eq_Volkov_4} 是长度规范薛定谔方程（\autoref{eq_LenGau_3}~\upref{LenGau}）
\begin{equation}
