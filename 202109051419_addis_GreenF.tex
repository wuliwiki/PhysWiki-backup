...
当我们像定积分那样用许多小长方形来代替函数 $f(x)$， 那么方程的解就是每个小长方形对应的解 $G_i(x)$ 做同样的线性组合． 当小长方形越来越细， $f_i(x)$ 就 以上的求和就变为了积分．

\autoref{GreenF_eq7} 就是\autoref{GreenF_eq8} 的连续化拓展． 我们可以把每个 $f_i(x)$ 看成一个宽为 $\Delta x$， 中心位置在 $x'$ 的长方形函数， 面积为 1． 当 $\Delta x\to 0$ 时就得到了 $\delta(x - x')$．

而\autoref{GreenF_eq6} 就是\autoref{GreenF_eq9} 和\autoref{GreenF_eq10} 的连续化． 
