...
\begin{equation}
\tau = \pdv{E} \arg[\mel{F_l(k)}{H'}{\psi_0} + \sigma_l(k) - kr\ln(2kr)]
\end{equation}
前两项是瞬间的， 而第三项是波包移动过程中缓慢积累且不收敛的． 在 Streaking 实验中， 前两项都会直接转换为 streaking 延迟． 一定要强调光电离延迟 $\tau$ 和 streaking 延迟是不一样的． 第三项在上式中不收敛， 但 streaking 延迟却总是收敛的．
