...
当 $n$ 是正整数时成立。（事实上，该法则在 $n$ 不是正数甚至不是整数时依然成立，但那时的证明必须依赖其他方法。）

证明方法是对指数 $n$ 使用数学归纳法。

基例：若 $n=0$，则 $x^n$ 是常数，而 $n x^{n-1} = 0$。在此情况下，法则成立，因为常函数的导数为 0。

归纳步骤**：假设该法则对某个特定的指数 $n$ 成立，则对下一个指数 $n+1$，有：
$$
\begin{aligned}
\frac{d}{dx} x^{n+1} 
...
要不要我帮你把这个翻译版本和原始英文一起排版成 **对照形式**？这样方便比读和学习。
