...
&= -\frac{\hbar^2}{2m} \laplacian + \I\hbar \frac{q}{2m} (\bvec A \vdot \Nabla + \Nabla \vdot \bvec A) + \frac{q^2}{2m} \bvec A^2 + q\varphi~,
}\end{equation}
注意算符 $\Nabla \vdot \bvec A$ 是指先把波函数乘以矢势再取散度而不是直接对 $\bvec A$ 取散度（想想量子力学中算符相乘的定义）。

另外要注意 $\bvec p = -\I\hbar\Nabla$ 代表的是\autoref{eq_QMEM_6} 的\textbf{广义动量}而不是 $m\bvec v$。 所以一般规范下的平面波 $\exp(\I \bvec k \vdot \bvec r)$ 的能量是
\begin{equation}
E = \frac{(\hbar\bvec k - q\bvec A)^2}{2m}~.
\end{equation}
在长度规范\upref{LenGau}下， $\bvec A \equiv \bvec 0$， 这时才有常见的 $E = \hbar^2\bvec k^2/(2m)$。
