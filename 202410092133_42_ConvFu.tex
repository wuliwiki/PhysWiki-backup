...
\end{equation}
那么称 $f(x)$ 为 $I$ 上的\textbf{凸函数}。如果将 $\le$ 改为 $<$，那么称 $f(x)$ 为\textbf{严格凸函数}。类似地可以定义\textbf{凹函数}（$\geq$）和\textbf{严格凹函数}($>$)。
\end{definition}


\subsection{凸函数的几何意义}
由凸集的\autoref{sub_ConSet_1} 可知，不等式\autoref{eq_ConvFu_1} 的左边的 $tx_1+(1-t)x_2,t\in(0,1)$ 是连接定义域上点 $x_1$ 和 $x_2$ 的开线段，而右边是连接函数值域上点 $f(x_1)$ 和 $f(x_2)$ 的开线段。

\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/64b205754a96ca8b.pdf}
\caption{凸函数的几何意义示意图} \label{fig_ConvFu_1}
\end{figure}

由\autoref{fig_ConvFu_1} ，利用相似性，可得
\begin{equation}
\frac{}{}
\end{equation}
