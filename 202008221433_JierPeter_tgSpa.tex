...
给定流形$N$，则在其上一点$p\in N$处的一个切向量就是从$p$出发的一条道路$r$所在的等价类$[r]$．其中，两道路$r_1$和$r_2$等价当且仅当存在$p$处的一个图$(U, \varphi)$，使得道路$\varphi\circ r_1$和$\varphi\circ r_2$都收敛于$\varphi(U)$中的同一个切向量．
\end{definition}
