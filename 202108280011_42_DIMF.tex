...
\end{equation}
\end{theorem}
\textbf{证明：}记单位制为 $\mathcal{Z}$ ，为便于陈述，设 $\mathcal{Z}$ 制有3个基本量类——$\tilde{\boldsymbol{l}}$、$\tilde{\boldsymbol{m}}$和$\tilde{\boldsymbol{t}}$．选定基本单位$\hat{\boldsymbol{l}}$、$\hat{\boldsymbol{m}}$和$\hat{\boldsymbol{t}}$ ．任一导出量类 $\tilde{}$ 的导出单位 $\hat{\boldsymbol{C}}$ 的终定方程为
\begin{equation}
C=f(l,m,t)
\end{equation}
