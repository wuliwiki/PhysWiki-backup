...
\end{equation}
现在我们可以按\autoref{RandF_eq14}~\upref{RandF}定义 $Q$ 的平均值为
...
若 $Q$ 的本征态是连续的， 记为 $\ket{q}$ （$q\in [a,b]$）， 且满足广义的正交归一条件\autoref{COrNoB_eq8}~\upref{COrNoB}
...
c(q) = \braket{q}{\psi}
\end{equation}
而 $\abs{c(q)}^2$ 则是测量\upref{QM0}得到 $q$ 的概率密度\upref{RandF}， 满足归一化条件
\begin{equation}
...
\end{equation}
由\autoref{Delta_eq8}~\upref{Delta}，可证明其满足正交归一化条件
\begin{equation}
...
特殊地， 若 $\psi(x)$ 是一个实函数（或者乘以一个任意相位因子 $\E^{\I \theta}$）， 那么其频谱 $\abs{c(p)}^2$ 必定是一个偶函数\upref{FTExp}， 可得动量平均值为零． 所以
\begin{theorem}{}
