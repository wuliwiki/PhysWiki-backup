...
\begin{lemma}{}\label{SpltFd_lem1}
给定域$\mathbb{F}$和其上一个可分的不可约多项式$f$，其全体根构成集合$\{\alpha_i\}_{i=1}^n$．
\end{lemma}
...
把$f$的所有不可约因式都考虑上，则总的保$\mathbb{F}$同构数目应该是各因式的根的数目之积．由\autoref{SpltFd_lem1}  ，$f$的分裂域可以看成是$f$的各不可约多项式的根依次进行单扩张的结果，因此由\autoref{FldExp_the1}~\upref{FldExp}和\autoref{FldExp_the3}~\upref{FldExp}，即可得证明．
