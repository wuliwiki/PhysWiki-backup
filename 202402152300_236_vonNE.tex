...
&= \sum_i\left( p_i\log p_i - \sum_jp_i\log q_j \braket{u_i}{v_j}\braket{v_j}{u_i} \right) \\
&= \sum_i\left(p_i\log p_i - \sum_j p_i\log q_j \abs{\braket{u_i}{v_j}}^2\right)
\end{aligned}~
\end{equation}

令$P_{ij} = \abs{\braket{u_i}{v_j}}^2 \geqslant 0$。易得$\sum\limits_i P_{ij} = \sum\limits_j P_{ij} = 1$（一标准正交基$u_i$在ling'yi）
