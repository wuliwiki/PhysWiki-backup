...
设$E$是$\mathbb{R}^n$上的一个可测集，$f$是其上一个函数．如果$E$能分解为\textbf{互不相交}的可数个$E_k$，其中$k$取值为正整数，使得$E=\bigcup_{k=1}^\infty$，且$f$在各$E_k$上都是\textbf{常数}，那么我们称$f$是一个$E$上的\textbf{简单函数（simple function）}.
\end{definition}

显然，简单函数都是可测函数．你甚至可能留意到了，简单函数可以直接定义Lebesgue积分了：如果$f$是\autoref{MsbFSt_def1} 中所说的那种简单函数，那么可以定义其Lebesgue积分为：

\begin{equation}
\int_E f \dd x = \sum_{k=1}^\infty f(x_k) \opn{m}(E_k)
\end{equation}
其中$x_k\in E_k$．
