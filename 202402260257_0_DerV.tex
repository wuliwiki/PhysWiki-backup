...
\begin{theorem}{}
参数曲线 $\bvec r(t)$ 在任意 $t=t_0$ 处存在切线\upref{TanL}的充分必要条件是导数 $\dot{\bvec r}(t_0)$ 存在，且切线的方向就是导数的方向。
\end{theorem}

从物理上这是容易理解的，若 $t$ 是时间， $\bvec r$ 是一点的位置矢量\upref{Disp}，那么 $\dot{\bvec r}(t)$ 就是这点的速度矢量\upref{VnA}，速度矢量总是沿运动轨迹的切线方向。

证明思路可以使用\autoref{eq_DerV_1}：若不取极限，对每个具体的 $\Delta t$，分子 $\bvec v(t+\Delta t)-\bvec v(t)$ 的方向就是曲线的\textbf{割线}的方向。而取极限 $\Delta t\to 0$ 时， 若 $\bvec v(t+\Delta t)$ 无限接近 $\bvec v(t)$，割线的极限就是切线。 若不是无限接近，说明曲线在该处不连续，切线也不存在。 注意 $\Delta t\to 0$ 的极限存在要求从正负两个方向趋近于零时极限都存在且相等， 所以类似\autoref{fig_TanL_3}~\upref{TanL}拐角处的情况不满足该条件。
