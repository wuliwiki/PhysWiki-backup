...
\begin{equation}
\lambda^{kl}=\qty(\sum_{i,j}\lambda^{ij}\mu_i\otimes\nu_j)(\mu_k,\nu_l)=0(\mu_k,\nu_l)=0
\end{equation}
其中，最后的 $0$ 是域 $\mathbb F$ 的零元．所以得到\autoref{TPofSp_eq1} 的线性无关性．

又 $\mathcal L(V_1,V_2;\mathbb F)$ 的维数为 $nm$，由基的定义（），所以\autoref{TPofSp_eq1} 就是它的一组基．
