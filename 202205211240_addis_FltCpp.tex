...
另外测试一下双精度的二进制表示（\autoref{CmArit_fig3}~\upref{CmArit}）， 例如双进度的 10 表示为
\begin{lstlisting}
01000000 00100100 (48 个 0)
\end{lstlisting}
第一位 0 说明是正数， 接下来 11 位指数的第一个 \verb|1| 说明指数是正数（比较奇怪）， 11 位的最后两位是 2， 于是指数项为 $2^{2+1} = 8$ （也比较奇怪）． 小数项应该是 $10/8 = 1+1/4$． 第
