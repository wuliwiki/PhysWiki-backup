...
在角动量理论里，$\E^{-\I J_i\phi}$总表示\textbf{系统}绕$i$轴逆时针转动$\phi$角。当$J_i\in \mathfrak{so}^+(1,3)$时，若令$\bvec v,\bvec {v'}$分别表示系统$i$轴转动前后的时空坐标，则$\bvec {v'}=\E^{-\I J_i\phi} \bvec v$。同理可证满足角动量关系的$S_i$是$SU(2)$的生成元，
