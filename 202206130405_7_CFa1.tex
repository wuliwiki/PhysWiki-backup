...
考虑$\mathbb{R}^4$上的一维向量丛的截面，也就是一个标量场，记为$\phi$．

时空是一个流形，上面定义了一个度量张量$g^{\mu\nu}$．我们可以仿照\autoref{CFa1_eq2} ，构造标量场$\phi$的一个拉格朗日函数，它必须是一个标量函数：
...
接下来的问题是，该如何推广\autoref{CFa1_eq4} ，得到作用量的表达式呢？答案很简单，“积分区域是时空上的给定区域”．
\begin{equation}
