...
现在来考虑对称性的问题， 若两个粒子是可区分的， 那么波函数可以在这个 9 维空间中任意选取， 再进行归一化． 若波函数恰好处于 9 个基底中的一个， 那么此时的物理意义是： 第一个粒子处于 $i$ 态而第二个粒子处于 $j$ 态．
...
和可区分粒子的情况不同， \autoref{IdPar_eq8} 和\autoref{IdPar_eq9} 中新基底的物理意义变为： 有一个粒子处于 $i$ 态
\end{example}
