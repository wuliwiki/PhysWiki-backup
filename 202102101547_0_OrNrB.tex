...
\end{equation}
最后一步使用了\autoref{Kronec_eq1}~\upref{Kronec}． 所以\autoref{OrNrB_eq5} 中的系数有唯一确定的值 $c_k = \bvec v \vdot\uvec x_k$． 证毕．

\autoref{OrNrB_eq2} 的过程可以看作是用 “点成 $\uvec x_k$” 的操作把\autoref{OrNrB_eq5} 求和中需要的一项筛选出来． 注意只有使用正交归一基底才可以进行这样的筛选． 一个反例是斜坐标系（\autoref{Gvec2_ex1}~\upref{Gvec2}）不能使用\autoref{OrNrB_eq1} 计算坐标．
