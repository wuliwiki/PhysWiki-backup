...
先考虑$h$无重根的情况．此时，$h$的根的数目$m=\opn{deg}f=n$．由\autoref{FldExp_the1}~\upref{FldExp}，$[\mathbb{F}(\alpha_1):
\mathbb{F}]=n$．设$\mathbb{F}(\alpha_1)$含有$h$的部分根，则只要确定了$\alpha_1$的映射便确定了
