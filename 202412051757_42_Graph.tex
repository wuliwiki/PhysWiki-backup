...
设 $G=(V,E)$ 是图，$e=\{x,y\}\in E$，则称 $x$（$y$ 也是同样的） 和 $e$ \textbf{关联}（incident），而 $v$ 称为在 $x$ 上的边。$x$ 和 $y$ 也称为 $e$ 的\textbf{端点}（endvertex）或\textbf{顶端}（end），$e$ 也称为\textbf{连接}（join）$x$ 和 $y$ 的边。$\{x,y\}$ 也记作 $xy$ （或 $yx$）。若 $x,$
\end{definition}
\subsection{2.特殊图元素}
