...
\autoref{the_nullpl_1}~\upref{nullpl}的$f_i(A)=(A-\lambda_i I )^k$就是限制在其核上的幂零变换。与上述不同的是，该幂零变换的幂指数未必是全空间幂零变换的指数。如果我们能证明$V$是$B$的循环子空间之直和，则我们可以依据幂指数写出每一个阶数与指数相同，形如\autoref{eq_Jordan_1} 的简单对角块。并根据$B|_{\opn{ker}f_i(A)}=A-\lambda_i I$得到$A$的对角块形式，即：
