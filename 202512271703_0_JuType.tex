...
\end{lstlisting}
\begin{itemize}
\item 要获取成员的值， 用 \verb`foo.bar`, \verb`foo.baz` 等。 用 \verb`struct` 声明的复合类型的变量在定义后就不能被改变。 如果 struct 里面有矩阵等， 那么矩阵元仍然是 mutable 的（没有 lower level const）。
...
\item \verb`getproperty(a, b)` 相当于算符 \verb`a.b`，会根据 \verb`a, b` 的类型调用 \verb`getglobal(::Module, ::Symbol)`， \verb`getfield(::Type, ::Symbol)`（结构体获取字段）， \verb`getfield(::Tuple, ::Int)`（Tuple 获取元素）
