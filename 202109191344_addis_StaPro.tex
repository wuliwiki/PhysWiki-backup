...
一个具体的例子是力场对单个质点的做功． 在分析力学中， 此时状态空间是 $(\bvec r, \bvec p)$． 如果力场（只是 $\bvec r$ 的函数）是保守场， 那么做功就是势能之差； 如果是非保守场， 做功只能由具体路径决定， 此时 “功”（过程量） 和 “能”（状态量） 的区分就很重要了． 例如动能总可以表示为状态 $\bvec p$ 的函数， 但做功却不行， 因为它不是状态量．
