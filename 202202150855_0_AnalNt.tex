...
\item 3.8 度量空间 $X$ 中的序列 $\{p_n\}$ 叫做 Cauchy 序列， 如果对于任何 $\epsilon>0$ 存在着正整数 $N$， 只要 $m,n\geqslant N$ 就有 $d(p_n, p_m)<\epsilon$．

\item 3.9 设 $E$ 是度量空间 $X$ 的子集， 又设 $S$ 是一切形式为 $d(p,q)$ 的实数集， $p,q\in E$． $\sup S$ 叫做 $E$ 的直径， 记为 $\opn{diam} E$．

\item 3.10 如果 $E$ 是度量空间 $X$ 中的集， 那么闭包满足 $\opn{diam}\bar E = \opn{diam} E$．
\end{itemize}
