...
可以看到，推导得到的 \autoref{eq_Hypb3_3} 与 \autoref{eq_Hypb3_5} 在形式上完全一致\footnote{即此时$\left(\displaystyle\frac{m}{2}\right)^2=\displaystyle c^2-\left(\frac{m}{2}\right)^2=2\implies m=2\sqrt{2},c=4$}。而前面由果及因地用反比例函数来认识双曲线，现在正是从双曲线的几何定义出发得到了他的表达式。需要注意的是，题目中隐含了一个默认条件，即 $|PF_1| > |PF_2|$，这使得最终得到的表达式只对应图像的右半部分，即 $x > 0$ 的区域。如果将条件改为 $||PF_1| - |PF_2|| = m$，其中 $0 < m < 2c$，则会对应完整的 \autoref{eq_Hypb3_3} 表达式，图像也将呈现左右对称的完整双曲线。而题目所给出的，正是双曲线的几何定义，也被称为\textbf{双曲线的第一定义}。
...
\autoref{eq_Hypb3_3} 正被称为“双曲线”的标准代数表达式。
