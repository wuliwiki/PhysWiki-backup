...
a_n=dn+(a_1-d)~.
\end{equation}
在\enref{数列}{HsSeFu}中曾介绍过，这一形式实际上是一次函数的离散形式。换句话说，在直角坐标系中，如果将数列的项数 $n$ 作为横坐标，数列的值 $a_n$ 作为纵坐标，则点 $(n, a_n)$ 将分布在一条直线上。

事实上，对于等差数列，可以证明，只需知道任意一项及公差，就能够推出所有其他项，满足以下关系式：
\begin{equation}
a_n=a_k+(n-k)d~.
\end{equation}
进而，可以推导出另一个重要关系：
...
d={a_n-a_k\over n-k}~.
\end{equation}
这与\aref{平均变化率}{def_HsFunC_3}或\aref{斜率}{def_HsFunC_5}的定义极为相似，也符合之前一次函数的判断。根据\autoref{eq_HsAmPg_1} 还能得到等差数列的一个重要性质，取四个整数，满足$m+n=p+q$，即$p-n=m-q$，那么根据\autoref{eq_HsAmPg_1} 有：
\begin{equation}
