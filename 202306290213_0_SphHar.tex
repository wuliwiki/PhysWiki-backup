...
与连带勒让德多项式相同， 在定义球谐函数时我们也可以选择是否包含 \textbf{Condon–Shortley 相位} $(-1)^m$。 物理中一般选择包含， 小时百科的物理词条中也选择包含。 如果包含， 我们可以选择将其包含在连带勒让德多项式中（如\autoref{eq_AsLgdr_1}~\upref{AsLgdr}）， 或者包含在球谐函数的定义中。 如果不包含， 该相位在两个定义中都不出现。
