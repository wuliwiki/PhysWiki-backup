...
请注意，尽管这部分内容已经在当前的高中教材中删除，但事实上无处不在。

\subsubsection{且}

若逻辑连接词连接的两个命题$A,B$只有同时成立时整个复合命题才为真，则称这个逻辑连接词为\textbf{且}（and，也称为\textbf{同}、\textbf{而且}、\textbf{合取}），记作为$A\land B$。

\subsubsection{或}

若逻辑连接词连接的两个命题$A,B$只有同时成立时整个复合命题才为真，则称这个逻辑连接词为\textbf{或}（or，也称为\textbf{或者}、\textbf{析取}），记作$A\lor B$。。
