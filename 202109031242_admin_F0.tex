...
\end{equation}
或者简写为 $\int_{-\infty}^{\infty} f^*(k_1, x) f(k_2, x)\dd{x} = \delta(k_2 - k_1)$． 那么它们就是正交归一的． 正交归一的好处是， 若一个平方可积函数可以用一组 $f(k, x)$ 展开函数
\begin{equation}
...
可见 $\sin(kx)$ 和 $\cos(kx)$ 在 $[0,\infty)$ 上并不是正交的． 但在 $(-\infty, \infty)$ 上却是， 因为奇函数乘以偶函数在区间 $(-n, n)$ 的积分始终为零．
\end{example}

一个量子力学的例子可以参考 “有限深方势阱\upref{FSW}” 或者其他势阱中的两组散射态波函数．
