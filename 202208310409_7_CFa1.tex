...
时空是一个流形，上面定义了一个度量张量$g^{\mu\nu}$．假设给定了一个拉格朗日函数$\mathcal{L}(r, \phi, \partial_0\phi, \partial_\mu\phi)$，该如何推广\autoref{CFa1_eq4} ，得到作用量的表达式呢？答案很简单，“积分区域是时空上的给定区域”．
\begin{equation}
...
粒子的哈密顿函数$H(t, q_r, p_r)$是对拉格朗日函数$L(t, q_r, \dot{q}_r)$作\textbf{勒让德变换}\upref{TrLgdr}，将自变量$\dot{q}_r$都替换为$p_r=\frac{\partial L}{\partial \dot{q}_r}$得来的．粒子的行为由正则方程约束：
\begin{equation}
\leftgroup{
