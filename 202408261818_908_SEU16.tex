...
\textbf{5.(15 分)}质量为 $m$ 的粒子以一定的能量 $E (E > 0)$ 沿 $x$ 轴方向入射。假定势场如下图所示：
\[
V(x) = 0, (x < 0), \quad V(x) = \frac{V_0 x}{a}, (0 \leq x \leq a), \quad V(x) = V_0, (x > a).~
\]

\begin{enumerate}
    \item [(1)] 分别写出入射区 ($x < 0$) 和透射区 ($x > a$) 的定态薛定谔方程，并给出定态波函数 $\psi(x)$ 的形式。
    
    \item [(2)] 利用几率流密度公式
    \[
    j = -\frac{i\hbar}{2m} \left(\psi^* \frac{d\psi}{dx} - \psi \frac{d\psi^*}{dx} \right)
    \]
    求出入射区几率流密度 $j_i$、反射区几率流密度 $j_r$、透射区几率流密度 $j_t$，并给出反射系数 $R$ 和透射系数 $T$ 的表达式。
\end{enumerate}
