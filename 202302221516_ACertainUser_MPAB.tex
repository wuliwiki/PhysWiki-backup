...
\end{figure}
有时我们需要改变参考系来观察同一质点。伽利略变换\upref{GaliTr}告诉我们应该如何相应地转换物理量。
\begin{table}[ht]
\centering
...
\hline
加速度（平动）\upref{AccTra} & $\bvec a_{K1} = \bvec a_{K2} + \bvec a_r$ \\
\hline
\end{tabular}
\end{table}
两个相对转动的参考系的加速度变换更为复杂，需要参考下文“非惯性参考系问题”。

\subsection{相对性原理}
在任意惯性系中，上述动力学定理始终成立。亦即在不同惯性参考系中可能观察到不同的物理量，但不会观察到不同的物理规律。\upref{RELTHM}

\subsection{非惯性参考系问题}
牛顿定律仅适用于惯性参考系。如果要在非惯性系中使用这些定律，则应该引入额外的假想力（惯性力\upref{Iner}、离心力\upref{Centri}、科氏力\upref{Corio}）。
