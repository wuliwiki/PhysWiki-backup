...
\caption{圆锥的有限截面是一个椭圆（来自 Wikipedia）}} \label{Elips3_fig1}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=6cm]{./figures/Elips3_2.png}
\caption{\autoref{Elips3_eq4} 表示的圆锥面（修改自 Wikipedia）} \label{Elips3_fig2}
\end{figure}

我们可以再列出一个平面方程与\autoref{Elips3_eq4} 联立得到方程组， 但这样解出来的椭圆将与 $x$-$y$ 平面不平行． 所以更好的办法是先把圆锥旋转一下， 再用某个和 $x$-$y$ 平面平行的平面 $z = z_0$ 去截出椭圆． 关于 $y$ 轴的旋转变换\upref{Rot2DT}为
\begin{equation}
