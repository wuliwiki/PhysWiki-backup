...
\item \textbf{位移}：初位置指向末位置的有向线段，是描述质点位置变化的物理量，是一个\textbf{矢量}．常用符号为 $\bvec x$，在强调“变化”时可用$\Delta \bvec x$，单位为 $\mathrm{m}$．\footnote{在大学物理中，位置常使用$\bvec r$, 位移常使用$\Delta \bvec r$}
\end{itemize}
