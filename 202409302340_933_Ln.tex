...
\begin{equation}
L_n={L_{n}\over L_{n-1}}\cdot{L_{n}\over L_{n-1}}\cdots{L_{2}\over L_{1}}{L_{1}\over L_{0}}\cdot L_0=L_{0}(1 - \frac{1}{L})^n~.
\end{equation}

John Napier根据自己的思路提出了他的对数表，顺带还正式使用了小数点。这个对数，其实是比例对数的意思，指的就是“按照\textbf{比例}制作的，\textbf{对}着查\textbf{数}的表”。也就是上表中$n$是$L_n$的对数。实际使用时，比如要计算两个数$L_n,L_m$的乘积，只需要根据他的表格找到对应的$n,m$，求和后再找到对应的$L_{n+m}$就可以得到计算的积了。
