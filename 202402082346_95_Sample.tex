...
a + b = c~, \quad d + e = f~.
\end{equation}
正文中空行代表分段， 所以\textbf{不要为了代码好看在公式前后习惯性加上一个空行}。 另外行间公式同样属于句子的一部分，需要在中间和末尾加上适当的标点符号，否则会提示错误。 如果根据语法，的确不需要加标点，可以在 \verb|\end{equation}| 之前加一个 \verb|~| 防止报错。  批量对所有公式加 \verb|~|，小心被骂。 公式中标点的用法参考本文。
