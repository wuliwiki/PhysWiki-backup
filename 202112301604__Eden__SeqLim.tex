...
- #### 唯一性
  ...
  **证：** 反证法．假设收敛序列存在两个极限 $a$ 和 $b$，即 $\lim\limits_{n\rightarrow \infty}x_n=a$ 且 $\lim\limits_{n\rightarrow \infty} x_n=b$ 且 $a\neq b$．不失一般性，不妨设 $a<b$．现在取 $\epsilon_0=(b-a)/2$，则由极限定义知，存在正整数 $N_1$ 和 $N_2$，使得：
  $$
  \begin{aligned}
...
- #### 极限的四则运算
  ...
  **证：**略．
  ...
  若 $\lim\limits_{n\rightarrow \infty} x_n=1$；序列 $\{y_n\}$ 的元素是整数．那么序列 $\{x_n^{y_n}\}$  的极限何时存在？何时等于 $1$ ？
