...
\item C++ 对浮点数微操的函数有 \verb|bool signbit(x)| 获取符号位； \verb|ldexp(x, n)| 计算 $x \times 2^n$； \verb|double man = frexp(x, &exp2)| 可以拆分 mantissa 和 exp。 其中 \verb|0.5 <= man < 1|； \verb|fpclassify| 可以用来给浮点数归类。
\end{itemize}
