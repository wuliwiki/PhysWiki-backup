...
令定态薛定谔方程的两个线性无关解为 $\psi_{k,1}(x), \psi_{E,2}(x)$， 我们使用归一化条件（\autoref{EngNor_eq3}~\upref{EngNor}）% 链接未完成
\begin{equation}
\int_{-\infty}^{+\infty} \psi_{E',i'}(x)^*\psi_{E,i}(x) \dd{x} = \delta_{i',i}\delta(E' - E) \qquad (i = 1, 2)
\end{equation}
