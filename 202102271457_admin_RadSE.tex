...
\end{equation}
角动量平方算符为（\autoref{SphAM_eq1}~\upref{SphAM}）
...
-\frac{1}{2m} \dv[2]{\psi_l}{r} + \qty[V(r) + \frac{1}{2m}\frac{l(l + 1)}{r^2}]\psi_l = E\psi_l
\end{equation}
这就是径向薛定谔方程（实际上是不同 $l$ 的一系列方程）， 方括号中可以看作\textbf{一维等效势能}（类比经典力学的情况\autoref{CenFrc_eq6}~\upref{CenFrc}）． 该方程的解取决于 $V(r)$ 的具体形式．
