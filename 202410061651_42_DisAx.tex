...
正规性公理是指任意两个不相交的闭集都各自有一邻域存在，使得两邻域不相交。

\begin{theorem}{}
正规性公理成立的充要条件是：任意闭集 $A$ 和其邻域 $O_A$，都存在 $A$ 的邻域 $ O'_A$，使得 $[O'_A]\subset O_A$。
\end{theorem}

\textbf{证明：}
\textbf{必要性：}假设正规性公理成立。对任意闭集 $A$ 和其邻域 $O_A$，存在开集 $U\subset O_A$ 包含 $A$。于是 $C(U)$ 是闭的，因此由正规性公理，存在开集 $U_A, O$，使得
\begin{equation}
