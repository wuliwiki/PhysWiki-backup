...
压缩映射是个连续映射。
\end{theorem}
\textbf{证明：}类比函数的连续性\upref{contin}，度量空间 $M$ 到度量空间 $M'$ 上的连续映射可以描述为：对任意正数 $\epsilon$ ，若存在数 $\delta$，只要 $x$ 满足 $d(x,x_0)<\delta$，就有 $d(f(x),f(x_0))<\epsilon$，则称映射 $f$ 在点 $x_0$ 连续。下面我们来证明压缩映射确是在度量空间上为连续。
