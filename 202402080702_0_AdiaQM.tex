...
考虑任意两个 $E_m\ne E_n$ 不同的简并空间在任何时间都具有不同能量且能使\autoref{eq_AdiaQM_11} 右边忽略的情况。 此时 $\tilde{\mat H}$ 是块对角厄米矩阵\upref{BHeig}，每个对角块代表一个本征子空间（或者若干个可能在某时刻本征值相同的本征子空间张成的空间），不同子空间之间不存在耦合。 $\mat U$ 也是结构相同的分块酉矩阵，且每个对角块都分别是一个酉矩阵。每个本征子空间独立演化，投影概率保持不变。
...
\end{equation}
其中 $\mat H^0$ 和 $\mat H^1$ 是厄米矩阵且与时间无关。 \textbf{那么绝热近似（\autoref{eq_AdiaQM_2} ）就是精确的}。
\end{theorem}
注意 $\mat H^0$ 和 $\mat H^1$ 都允许存在简并，且 $\mat H^1$ 可能导致 $\mat H^0$ 的能级分裂。 注意不要求 $\alpha(t)$ 缓慢变化。

\textbf{证明}：记 $\mat H$ 的本征值和本征矢为（都与时间无关）
\begin{equation}
