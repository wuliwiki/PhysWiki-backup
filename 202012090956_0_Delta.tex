...
$\delta$ 函数的另一个性质是
\begin{equation}
\delta[f(x)] = \frac{1}{f'(0)} \delta(x)
\end{equation}
我们不妨从几何上来证明这个性质， 与 $\delta(x)$ 相比较， $\delta[f(x)]$ 的图像在 $x$ 方向缩小了 $\abs{a}$ 倍（当 $a < 0$ 时， 还另需要关于 $y$ 轴翻转）， 所以 $\delta(ax)$ 下的面积是 $1/\abs{a}$（注意翻转并不影响面积）， 故 $\abs{a}\delta(ax)$ 下的面积是 $1$， 且满足\autoref{Delta_eq2} 和\autoref{Delta_eq3}， 所以有 $\abs{a}\delta(ax) = \delta(x)$． 证毕．
