...
理论上指数部分可以表示 $-1023$ 到 $1024$ 之间的数， 但首尾两个数有特殊用途， 所以只能表示 $-1022$ 到 $1023$。 当指数为 $-1023$ 时， 仍然相当于指数为 $-1022$ 但小数部分前面不加 1（如果小数也都是零， 那就表示 0）。 当指数为 $1024$ 时， 表示 \verb|inf|（要求小数部分都是零， 正负号仍然有效）或者 \verb|nan| （\verb|0b1...| 表示安静的 \verb|qNaN|， \verb|0b0..1..| 表示会发错误信号的 \verb|sNaN|）。
