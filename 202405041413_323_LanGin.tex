...
当$\xi = 0$时，我们可以认为在界面处序参量没有平缓的变化，对于$x>0,\psi(x) = \psi_{\infty}$，对于$x<0,\psi(x)=0$。根据\autoref{eq_magnetization} (这里有一点不同单位制之间的混用，但可以看到$H$也是指数衰减，且衰减的特征长度也是$\lambda$)，可以得到当$x>0$时，磁场强度的解为$H(x) = H_c e^{-x/\lambda}$。同样将这一解带入到\autoref{eq_free_energy} 中，计算得到界面能为$$
\end{example}
