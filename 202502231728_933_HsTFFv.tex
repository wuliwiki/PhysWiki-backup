...
之前学习过(\sin x)'=\cos x，而(\cos x)'=-\sin x，这说明sinx如果求两次导就会变成与原本相反的样子，同样，如果再求两次导就会得到$(-\sin x)'=-\cos x$以及$(-\cos x)'=\sin x$，也就是说，$\sin x$求四次导数之后，就得到了它自己。这种求导上的循环规律很有趣，下面大概介绍一下。

\begin{corollary}{正弦函数与余弦函数的$n$阶导数}
\begin{equation}
(\sin x)^n=,\quad(\cos x)^n=~.
\end{equation}
\end{corollary}