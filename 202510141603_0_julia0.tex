...
\item 传递给 \verb`jl_toplevel_evel_flex()` 的节点是 \verb`jl_toplevel_sym`，要递归对每个分支执行 \verb`jl_toplevel_evel_flex()`
\item \verb`body_attributes()` 用于总结要执行的 Expr 的一些属性，例如是否有函数定义， 是否有 \verb`ccall()`，然后用 \verb`if, else` 立即判断是否需要编译该 AST， 还是使用解释器直接执行。
\item 要是决定解释器直接执行，则 \verb`jl_interpret_toplevel_thunk()` 会调用 \verb`interpreter.c: eval_body()`。 注意第二个参数 \verb`jl_code_info_t src` 中含有 SSA。（是否执行 SSA 而不是语法树？）， \verb`jl_array_t *stmts = src->code;`（类型必须为 \verb`Array{Any}`） 就是所有 IR 命令？
