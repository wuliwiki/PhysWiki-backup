...
线性算子是矢量空间上的线性映射并且所有线性算子构成一矢量空间（\upref{LiOper}），其不仅能在加法和数乘的定义下构成代数，而且能在其上定义范数使其变成完备度量空间。本节旨在说明后一论断。下面我们约定 $V^* \equiv V - \{0\}$，其中 $0$ 为矢量空间 $V$ 的零矢量。
...
对于有限维度赋范空间来说，$\{x \in V \mid |x| = 1\}$是一个紧空间，因此$\norm{\mathcal A}$有限，称为\textbf{有界算子}；无限维度赋范空间可以存在不有界的算子。
