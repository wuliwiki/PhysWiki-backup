...
这一证法是先通过旋转法求出余弦的加法公式\autoref{HsAnTf_eq3} ，然后进行简单变换得到剩下三个式子．思路来自П. М. Котельников的学位论文\footnote{据刘培杰工作室出版的《世界著名三角学经典著作钩沉 平面三角卷I》第22节．笔者按此名字搜索，并未找到出处，特此声明．}．
...
计算$\cos\qty(\alpha-(-\beta))$即可得\autoref{HsAnTf_eq4} ．将$\sin x=\cos(x-\pi/2)$代入这两个余弦加法公式，即可得到正弦加法公式\autoref{HsAnTf_eq5} 与\autoref{HsAnTf_eq6} ．
