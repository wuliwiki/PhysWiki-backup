...
\subsubsection{复值函数}
若上述傅里叶展开公式对实值函数成立， 当\autoref{FSTri_eq1} 中的 $f(x)$ 的函数值是复数时是否仍然成立呢？ 我们可以令
\begin{equation}
f(x) = u(x) + \I v(x)
\end{equation}
其中 $u(x), v(x)$ 是相同周期的实函数， 且都满足迪利克雷条件． 那么， 可以把它们分别做傅里叶展开， $u(x)$ 的展开系数为 $a'_n, b'_n$， 而 $v(x)$ 的系数为 $a''_n, b''_n$． 分别对 $u(x), v(x)$ 列出\autoref{FSTri_eq1} 到\autoref{FSTri_eq3}， 且令 $a_n = a'_n + a''_n$， $b_n = b'_n + a''_n$




实函数的展开系数 $a_n, b_n$ 也是实数， 复函数的展开系一定不全是实数．
