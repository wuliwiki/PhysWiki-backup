...
也许读者已经接触过数列的单调性这个概念，例如，对于数列$ a_n= \bvec n$,n越大$a_n$就越大。要想$a_n$足够大，只需n足够大。

例如，要让$a_n$比一千万亿大，只需n大于一千万亿。要让$a_n$大于$10^{100}$，只需n大于$10^{100}$。从这个意义上讲，我们可以说数列${a_n}$是一个“无穷大量”。需要注意的是，这并不意味着$a_n = \infty $，事实上，$\infty$不是一个实数，bu'nen
