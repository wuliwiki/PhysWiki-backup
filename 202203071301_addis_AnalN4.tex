...
\item 1.11 \textbf{博雷尔集}： 设 $X$ 为拓扑空间． 由定理 1.10, 在 $X$ 内存在一个最小的 $\sigma$-代数 $\mathscr B$， 使得 $X$ 内每一个开集都属于 $\mathscr B$． 就称 $\mathscr B$ 的元素为 $X$ 的博雷尔集．
...
\item 2.14 \textbf{里斯表示定理}： 设 $X$ 是局部紧的豪斯多夫空间, $\Lambda$ 是 $C_{c}(X)$ 上的正线性泛函, 则在 $X$ 内存在一个包含 $X$ 的全体博雷尔集的 \textbf{$\sigma$-代数} $\mathfrak{M}$, 并存在 $\mathfrak{M}$ 上的唯一一个正测度 $\mu, \mu$ 在下述意义上 表示了 $\Lambda$: 
