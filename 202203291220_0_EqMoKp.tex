...
\begin{equation}\label{EqMoKp_eq2} % 对 k<0 已经验算正确
...
\begin{equation}\label{EqMoKp_eq4} % 对 k<0 已经验算正确
t = \sqrt{\frac{ma^3}{\abs{k}}} (e\sinh\xi - \xi)
\end{equation}

对于\textbf{双曲线}（$e>1, k>0$）， 类似有\footnote{推导见\lstinline|chrome-extension://efaidnbmnnnibpcajpcglclefindmkaj/viewer.html?pdfurl=https%3A%2F%2Fcomethunter.lamost.org%2Fscwrk%2FTHECAL%2Ffkepler.pdf&clen=69977&chunk=true|}
\begin{equation}
r = -a(e\cosh\xi + 1) \qquad (\xi \in \mathbb R)
\end{equation}
此时若仍然定义近日点 $\Delta \theta = 0$， 则 $\Delta\theta = \theta \in (-\gamma, \gamma)$， $r < 0$
\begin{equation}
t = \sqrt{\frac{ma^3}{\abs{k}}} (e\sinh\xi + \xi)
\end{equation}
