...
\item \verb`get_filename_component(变量 "文件名含路径" NAME或DIRECTORY)` 会从 \verb`"文件名含路径"` 中提取文件名或路径， 赋值给\verb`变量`。 例如 \verb`get_filename_component(NORMALIZED_PATH "${PROJECT_SOURCE_DIR}/../.." REALPATH)`
