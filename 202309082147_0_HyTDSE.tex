...
事实上，注意到 $\exp(\I q\bvec E\vdot\bvec r\dd t)$ 不过是一个普通函数（一个平面波）而不是微分算符， 所以我们只需要把它和波函数相乘： $\exp(\I q\bvec E\vdot\bvec r\dd t)\Psi$。 为了使相乘后的函数仍然具有\autoref{eq_HyTDSE_3} 的形式， 可以先根据\autoref{eq_Pl2Ylm_1}~\upref{Pl2Ylm} 对其展开为分波（$\exp(\I q\bvec E\vdot\bvec r\dd t) = \sum_{l'',m''}R_{l'',m''}Y_{l'',m''}$）。 所以演化后的每个分波的径向波函数就是
\begin{equation}
\begin{aligned}
...
&= \sum_{l'',m''}\sum_{l',m'}R_{l'',m''} \psi_{l',m'} \mel{Y_{l,m}}{Y_{l'',m''}}{Y_{l',m'}}
\end{aligned}~
\end{equation}
现在可以用\autoref{eq_SphHar_5}~\upref{SphHar} 把三个球谐函数的积分变为两个 3j 符号\upref{ThreeJ} 的乘积，再根据选择定则，排除两个求和中等于零的项。



由于算符只混合每个位置 $\bvec r$ 处不同的分波， 所以只需要计算 $\mel{Y_{l,m}}{\exp(\I q\bvec E\vdot\bvec r\dd t)}{Y_{l',m'}}$ 即可。 但话又说回来，做 $N_l^2/2$ 个球面积分，真的会比例如 Crank-Nicolson 要快吗？我看不见得。
