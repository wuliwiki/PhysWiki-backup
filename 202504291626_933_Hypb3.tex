...
综上，通径的长度为$\displaystyle\frac{2b^2}{a}$。
\end{example}

另外，类似地也可以证明，在双曲线上的所有点中，到某一焦点距离最近的位置就是它同侧的顶点。

\begin{example}{对双曲线$x^2$右焦点 $F(c,0)$ 为例，上的所有点中，到某一焦点距离最近的位置就是它同侧的顶点。}
以显然左支上的任意点到$F$的距离都大于它关于$y$轴对称的点到$F$的距离，设最近的点为$P$则P一定在右支上。
\end{example}
