...
$\mathbb{Z}_p(x, y)$作为$\mathbb{Z}_p(x^p, y^p)$的线性空间，基向量组为$\{x^iy^j\mid i, j=0, 1, 2, \cdots, p-1\}$．

比如说，向量$y^{-1}\in\mathbb{Z}_p(x, y)$就可以表示为$\frac{1}{y^p}y^{p-1}$，其中$\frac{1}{y^p}\in\mathbb{Z}_p(x^p, y^p)$是系数，$y^{p-1}$是基向量．下面的\autoref{PrmtEl_ex1} 进一步展现了如何用上述向量组和$\mathbb{Z}_p(x^p, y^p)$作为基域来表示$\mathbb{Z}_p(x, y)$中的元素．
...
\end{equation}\footnote{想到这一点的思路如下：}

\begin{equation}
\frac{1}{x^2+y}(x^{2p}+y^p) = x^px^{p-2}+y^{p-1}
\end{equation}
