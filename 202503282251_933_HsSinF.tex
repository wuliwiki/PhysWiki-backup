...
其中，\textbf{幅值（amplitude）} $A$ 表示图像的振幅，\textbf{角频率（angular frequency）} $\omega$ 控制周期，\textbf{初相位（initial phase）} $\varphi$ 决定图像的水平平移，而参数 $b$ 是函数图像的垂直平移量。从线性变换的视角看，$\omega x + \varphi$ 是对自变量的线性变换，而$A$和$b$是对函数值的线性变换。
