...
\item 若三角形以 $A$、$B$、$C$ 为顶点，则称为圆的\textbf{内接三角形（inscribed triangle）}，圆称为三角形的\textbf{外接圆}，此时三角形的三个内角均为圆周角，圆心就是三角形的\textbf{外心（circumcenter）}。
\item 若三角形的三边分别与圆相切于 $A$、$B$、$C$，则称为圆的\textbf{外切三角形（circumscribed triangle）}，圆称为三角形的内切圆。三角形的\textbf{内心（incenter）}也是圆心。
\end{itemize}
