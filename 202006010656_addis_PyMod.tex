...
\subsection{创建模块}
事实上， python 的模块就是一个普通的 \verb|.py| 拓展名的文件． 这个文件中可以定义全局变量， 函数， 甚至是可执行的命令． 当我们要在其他 \verb|.py| 文件中使用这个文件中的函数或全局变量， 只需要生命

如需创建模块，只需将所需函数代码保存在文件扩展名为 \verb|.py| 的文件中． 例如在名为 \verb|mymodule.py| 的文件中保存代码：
