...
\begin{equation}
\sin^{-2}\theta_N \Nabla^2_{S^{N-1}}{W_N}+\qty[(N-1)\cot\theta^N\pdv{W_N}{\theta^N}+\pdv[2]{W_N}{\theta^N}]=-\lambda_N W_N.~
\end{equation}
显然，现在可以继续进行变量分离，令
\begin{equation}
W_N(\theta_1,\cdots,\theta_N)=W_{N-1}(\theta_1,\cdots,\theta_{N-1})y_N(\theta_N),~
\end{equation}
那么
\bq
