...
\end{equation}
当电荷分布关于极轴对称时， 求和中只需要 $m=0$ 的项，也就是说只有 $C_{l,0}$ 不为零，且为实数。 式中 $l = 0$ 的项就是\textbf{电单极子（electric monopole）} 具有球对称的电荷分布和势能分布， $l = 1$ 的项就是\enref{电偶极子（dipole）}{eleDP2}（$\propto\cos \theta$）， $l = 2$ 的项是\textbf{电四极子（quadrupole）}……  $l = N$ 的项叫做电 $2^N$ 极子。
