...
与椭圆或双曲线中可以通过独立的参数控制长轴、短轴不同，标准形式的抛物线只有一个参数 $p$来控制开口的“张度”。方向的转换则变换必须通过改变二次项的变量来实现。

抛物线本身只有一个参数，不同形态之间的变换必须通过改变二次项的变量来实现，所以，没有它们那种可以直接通过参数来改变开口取向的方法。但既然只有一个参数，说明所有对称轴共线的抛物线本身就构成一个抛物线族。这没什么好说的。

有趣的是，事实上，所有的抛物线都是一样的，他们只是在尺度上有差别，或者说他们都是相似的。这一点和椭圆、双曲线非常不同，双曲线的相似性需要相同渐近线作为约束，椭圆则（……补充）。

这一点也非常好证明，对于两个共轴的抛物线，p1,p2，设k=p1/p2，则显然他就是在x方向上伸缩了k倍。而不共轴的抛物线，则可以直接旋转到共轴上。
