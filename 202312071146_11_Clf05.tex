...
如果几何代数的系数取自实数域，非退化的几何代数里总存在标准正交基\footnote{1}。在取了标准正交基后，其互反基实际上就是这组基本身：$e^i=e_i$。因此，互反基相当于利用二次型，将对偶空间同构回原空间。
...
proof.
由几何代数非退化，可以取$\bar A$上的一组标准正交基$\{e_i\}^k_i=1$，并扩展为全空间的标准正交基$\{e_i\}^n_i=1$。由互反基定义得，$v^{k+m}$则
\begin{equation}
\begin{aligned}
