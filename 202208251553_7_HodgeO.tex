...
给定正整数指标集合$\Gamma=\{1, 2, \cdots, n\}$．设$I$是$\Gamma$的\textbf{有序}子集，即存在一个置换$\sigma\in S_n$和一个正整数$k\leq n]$，将$I$表示为$(\sigma(1), \sigma(2), \cdots, \sigma(k))$．再定义$I$的补集$\bar{I}=(\sigma(k+1), \cdots, \sigma(n))$，如果$k=n$则$\bar{I}=\varnothing$．

微分形式$\omega_{\sigma(1)}\dd x^{\sigma(1)}\wedge\omega_{\sigma(2)}\dd x^{\sigma(2)}\wedge\cdots\wedge\omega_{\sigma(k)}\dd x^{\sigma(k)}$用复杂指标表示为$\omega_{I}\dd x^{I}$．
\end{definition}
