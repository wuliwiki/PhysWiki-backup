...
\subsection{关于面积的讨论}
叉乘的另一个几何意义是， 叉乘的绝对值等于面积，  然而 $\bvec A \cross \bvec D$ 的面积（模长）通常并不等于 $\bvec A \cross \bvec B$ 的面积（模长）加上 $\bvec A \cross \bvec C$ 的面积（模长）， 因为\autoref{CrossP_eq1} 中的加法是矢量相加． 从这个角度来说， 叉乘所定义的 “面积” 也是一个带方向的矢量． 

要使用用面积的几何意义来证明叉乘的分配律， 思路也和上面的证明同理． \autoref{CrossP_eq2} 告诉我们三个平行四边形的面积可以变为三个长方形的面积． 但 “矢量面积” 的加法同样需要通过平行四边形法则， 而不是简单地把数值相加．
