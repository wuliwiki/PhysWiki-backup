...
设 $V$ 是一个集合，$E\subset [A]^2$，则称二元组 $(V,E)$ 为\textbf{图}（graph），记作 $G(V)$。$V$ 的元素称为 $G$ 的\textbf{顶点}（vertex），$E$ 的元素称为 $G$ 的\textbf{边}（edge）。
\end{definition}

当然，顶点也可以称为节点（node）或点（point），边还能叫做线（line），具有顶点集 $V$ 的图也能称为 $V$ 上的图（a graph on $V$）等等， 这都是习惯的问题。
