...
\item 同理， \verb|CMAKE_CXX_FLAGS_RELEASE| 可以设置 release mo
...
\item \verb|CMAKE_BUILD_TYPE| 可以被用户设置为 \verb|Release|， \verb|Debug| 等。 例如 \verb|cmake -DCMAKE_BUILD_TYPE=Debug| 用 debug 模式编译。 \verb|Debug| 模式下，编译器优化会被自动关闭， \verb|-g| 选项会自动加上， \verb|NDEBUG| 不会。
