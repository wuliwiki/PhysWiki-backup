...
一般地， 对于自旋量子数为 $s$ 的两个全同粒子， 我们可以将双粒子自旋态记为 $\chi_{1,2}$， 是 $N = (2s+1)^2$ 维希尔伯特空间中的态矢． 那么全同粒子要求
\begin{equation}\label{IdPar_eq3}
\chi_{2,1} = \pm\chi_{1,2}
\end{equation}
其中 $\chi_{1,2}$ 和 $\chi_{2,1}$ 的转换就是把每项中第一个箭头和第二个箭头互换， 例如 $\downarrow\uparrow - \uparrow\downarrow$ 变为 $\uparrow\downarrow - \downarrow\uparrow$． 其中对称子空间是 $N(N+1)/2$ 维的， 反对称子空间是 $N(N-1)/2$ 维的， 他们互为互补的子空间． 我们把推导留到 “粒子交换算符\upref{ExchOp}” 中．
