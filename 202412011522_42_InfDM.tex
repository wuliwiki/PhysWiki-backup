...
\subsection{无穷阶差分矩阵}
由\autoref{eq_InfDM_1} 可知，若 $u(x)=\sum\limits_{i}a_if_i(x)$，$a_i\in\mathbb C$， 那么
\begin{equation}
\begin{aligned}
\hat u(k)=\sum_{i}a_i\int_{-\infty}^{\infty} e^{-\I kx}f_i(x)\dd x=\sum_{i}a_i\hat f_i(k).
\end{aligned}~
\end{equation}
即傅里叶变换是线性的。而任意的离散点 $u_j$ 都可以用Kronecker Delta 函数（$x_j=jh$） 
\begin{equation}
\delta_j=\delta(x_j)=\left\{\begin{aligned}
&1,&j=0,\\
&0,&j\neq0.
\end{aligned}\right.~
\end{equation}
表示为 $u_j=\sum\limits_{m}u_m\delta_{j-m}$。因此，若知道 $\delta_j$ 的傅里叶变换，就知道了 $u(x)=\sum\limits_{m=-\infty}^\infty u_m\delta(x-x_m)$ 的傅里叶变换，显然 $u(x_i)=u_i$。
