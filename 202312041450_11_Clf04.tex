...
取$\bar A$上一组正交基$\{e_i\}^k_{i=1}$，并扩张到全空间的正交基。非退化性使得这样的扩张总存在。利用这组基计算，我们有：$f(u)\wedge f(A)=0\Rightarrow f(u)\in\overline{f(A)}=f(\bar{A})$
综合以上讨论便有

\begin{equation}
\begin{aligned}
u\wedge A=0 &\Longleftrightarrow f(u)\wedge f(A)=0\\
&\Rightarrow f(u)\in\overline{f(A)}=f(\bar{A})\\
&\Longleftrightarrow u\in\Longleftrightarrow \bar{A}~.
\end{aligned}
\end{equation}
