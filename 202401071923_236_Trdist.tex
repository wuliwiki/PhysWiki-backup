...
上式中$P$代表任意半正定算子，我们首先证明\autoref{eq_Trdist_3} 与\autoref{eq_Trdist_4} 的等价性。
...
\end{aligned}~
\end{equation}
则有$D\left(\rho,~\sigma\right) = \frac{1}{2}\left(\opn{tr}M_+ - \opn{tr}M_-\right)$。

考虑$\opn{tr}\rho = \opn{tr}\sigma = 1$，则$\opn{tr}M_+ + \opn{tr}M_- = \opn{tr}\left(\rho - \sigma\right) = 0$，即$\opn{tr}M_+ = -\opn{tr}M_-$，$D\left(\rho,~\sigma\right) = \opn{tr}M_+$。
