...
\subsection{列矩阵和行矩阵}
我们把一个 $n$ 行 $n$ 列的矩阵记作\textbf{方矩阵}， $1 \times n$ 矩阵记作\textbf{列矩阵}（列向量），$n \times 1$ 矩阵记作\textbf{行矩阵}（行向量），$1 \times 1$ 矩阵记作\textbf{数}（标量）．根据矩阵的乘法\autoref{Mat_sub1}~\upref{Mat}，我们有以下六种合法的乘法（注意顺序不能颠倒）：
