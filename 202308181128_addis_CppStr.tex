...
\item \verb`cin.getline(char* s, streamsize n, char delim)cin.getline(name, max, '\n');` 用于读取一个字符串, 不超过 max 字符, 然后存在数组 \verb`char[] name` 里面, 后面加上 \verb`\0`.
\item \verb|std::getline(std::istream& input, std::string& str, char delim);| 功能也类似
