...
\item 分部积分的高拓展： $\int_G (\partial_j u) v\dd{x} = \int_{\partial G} uvn_j \dd{O} - \int_G u \partial_j v\dd{x}$． 其中 $x = (\xi_1, \dots, \xi_N)$， $\partial_j u := \partial u/\partial\xi_j$， $n_j$ 是边界 $\partial G$ 的法向量 $n = (n_1, \dots, n_N)$． $\dd{O}$ 代表边界上的积分．
\end{itemize}
