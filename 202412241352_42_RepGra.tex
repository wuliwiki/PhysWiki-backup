...
\end{aligned}\right.~
\end{equation}
其中 $\mu(x_i,x_j)$ 表示以 $v_i$ 为起点 $v_j$ 为终点的边数。由 $N$ 的定义知 $N$ 是对称的。且 
\begin{equation}
\sum_{j=1}^{\abs{V}}n_{ij}=d_C(v_i)+2r-\qty(\sum_{j=1,j\neq i}^{\abs{V}}\mu(x_i,x_j)+\sum_{j=1,j\neq i}^{\abs{V}}\mu(x_j,x_i)).~
\end{equation}
