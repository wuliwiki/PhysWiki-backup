...
\begin{theorem}{}
设 $M$ 是由依赖于参数 $x$ ，定义域为柱体 $\abs{x-x_0}\leq b',\abs{t-t_0}\leq a'$，满足 $\abs{\varphi(x,t)}\leq K\abs{t-t_0}$ 的连续函数 $\varphi(x,t)$ 构成的度量空间，其上的度量 $d$ 定义为
\begin{equation}
...
\end{theorem}
\textbf{证明：}首先验证 $A$ 将 $M$ 映射到自身，就是要验证对任意 $\varphi$，$A\varphi$ 连续且 $\abs{A\varphi}(x,t)\leq K\abs{t-t_0}$：
\begin{equation}
\begin{aligned}
\abs{A\varphi}(x,t)}&=\abs{x+\int_{t_0}^t v(\varphi(x,\tau),\tau)\dd \tau}\leq\abs{x+\int_{t_0}^t v(\varphi(x,\tau),\tau)\dd \tau}\\
&\leq\abs{x}+\abs{\int_{t_0}^t C\dd \tau}\leq\abs{x+\int_{t_0}^t v(\varphi(x,\tau),\tau)\dd \tau}
\end{aligned}
\end{equation}
