...
写作参考\href{https://chaoli.club/index.php/4541/last}{这篇帖子}． 类似于平面波， 一维散射态也有不同的归一化方式， 但情况要更为复杂． 我们先考虑如何能量归一化． 令定态薛定谔方程的两个线性无关解为 $\psi_{E,1}(x), \psi_{E,2}(x)$， 能量归一化条件为% 链接未完成
\begin{equation}
\int_{-\infty}^{+\infty} \psi_{E',i'}(x)^*\psi_{E,i}(x) \dd{x} = \delta_{i',i}\delta(E' - E)
\end{equation}

为了方便先假设 $V(x)$ 关于原点对称， 且 $V(x)$ 只在区间 $[-L,L]$ 内不为零．令 $k = \sqrt{2mE} > 0$． 由于 $V(x)$ 的对称性， 我们必定能找到实值的奇函数和偶函数两种解， 所以在区间外， 波函数就是正弦函数加上一个相移
\begin{equation}
\psi_k(x) \propto \sin(kx + \phi) \qquad (x > L)
\end{equation}
其中$\phi$ 是 $k$ 的函数， 为方便书写下文把 $\phi(k),\phi(k')$ 分别记为 $\phi, \phi'$．
