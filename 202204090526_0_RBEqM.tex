...
\end{equation}
其中 $\dot{\bvec\omega} = \dv*{\bvec\omega}{t}$ 是矢量\textbf{角加速度}， 本文中符号上方一点都表示对时间求导， 另外对矩阵和列矢量求导就是对每个元素分别求导． 角速度和角加速度的关系可以类比速度和加速度\upref{VnA}． 对比平面运动的\autoref{RBEM_eq2}~\upref{RBEM}， 转动惯量变为了惯性张量， 且多了一项角速度叉乘角动量． 当二者共线时， 叉乘为零， 就回到了平面运动的式子．
