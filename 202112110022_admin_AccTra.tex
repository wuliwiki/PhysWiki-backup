...
向心加速度为
\begin{equation}
\bvec a_{S'} = -\omega_1^2 \bvec r = -\qty(\frac{R}{r} + 1)^2 \omega^2\bvec r
\end{equation}
...
\end{equation}
其中 $\uvec R = \bvec R/R$ 是单位矢量． 代入\autoref{AccTra_eq1} 得
\begin{equation}
\bvec a_S = \bvec a_{S'} + \bvec a_r = -\qty(\frac{R}{r} + 1)^2 \omega^2\bvec r -\omega^2 (R + r)\uvec R
\end{equation}
例如当 $\bvec r$ 和 $\bvec R$ 同方向时（点 $P$ 离 $O$ 最远）， $\bvec r = $
\begin{equation}
