...
\textbf{证毕！}
\subsection{向量场}
常微分方程研究的过程是可微过程，即要求相空间 $M$ 是微分流形，状态随时间的变化由可微函数描述．由于状态x经过t时间后变为 $g^tx$，要这个有可微函数描述的映射就是 $g(t,x)=g^tx$，由\autoref{GofODE_def1} ，$\{g^t|t\in\mathbb R\}$ 就是 $M$ 的单参数微分同胚群．因此，以后提到相流 $\{M,\{g^t\}\}$ ，都有将 $\{g^t\}$ 理解成单参数
