...
若给出两个对易的厄米矩阵 $\mat A, \mat B$， 先对 $\mat A$ 对角化， 得到本征列矢量组成的矩阵 $\mat P$， 使得 $\mat P \mat A\mat P^{-1}$ 为对角矩阵． 此时计算 $\mat P \mat B\mat P^{-1}$ 得块对角矩阵， 其本征列矢量组成的矩阵可以排成块对角矩阵 $\mat R$， 每个块都是酉矩阵\footnote{事实上只要分别对 $\mat P \mat B\mat P^{-1}$ 的每个块分别解本征shi'lia问题即可．}． 此时 $\mat B$ 的对角化为
\begin{equation}
\mat R\mat P \mat B\mat P^{-1}\mat R^{-1} = (\mat R\mat P) \mat B (\mat R\mat P)^{-1}
\end{equation}

那么， $\mat R\mat P$ 的每一列就是 $\mat A, \mat B$ 的共同本征矢．
