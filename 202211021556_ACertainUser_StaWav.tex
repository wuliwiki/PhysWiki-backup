...
根据\autoref{StaWav_eq1} ，如果我们把$\sin(kx)$一项视为振幅的一部分，那么我们会发现振幅大小与位置有关，即$A(x) = A \sin(kx)$. 也就是说，当$$\sin(kx)=0, kx=n\pi \qquad n=1,2,3,...$$时（图中黑色点），振幅始终为零，此处的质元从不运动，称为波节；当$$\sin(kx)=\pm, kx=\frac{\pi}{2} (2n-1)$$时，振幅达到最大值$A$，称为波腹（图中红色点）．
...
在简谐波中，频率、波长是可以任意取值的；但是在驻波中，频率、波长只能取一系列特定的、离散的值．这是由于驻波被弦边界所约束．具体的求解需要运用\textsl{烦闷复杂的}数学物理方法（分离变量法解偏微分方程\upref{SepVar}）的相应知识，此处仅做简单介绍．
