...
先看不含 bridge function 的矩阵元：
\begin{equation} \ali{
D_{(im), (in)} &= -\frac{\sqrt{w_{0m}}}{a_i^2} f'_n(t_m)
\qquad (1 < m \leqslant n < N)~.
} \end{equation}
再看含 bridge function 的矩阵元：
\begin{equation} \ali{
D_{(im), (iN)} &= \frac{-\sqrt{w_{0m}}}{a_i^{3/2} \sqrt{a_i + a_{i+1}}} f'_N(t_m)
\qquad (1< m < N)~,
} \end{equation}

\begin{equation} \ali{
D_{(iN), (iN)} &= \frac{-1}{(a_i + a_{i+1})} w_{0k} \qty[\frac{1}{a_i} f'_N(t_N) + \frac{1}{a_{i+1}} f'_1(t_m)]~,
} \end{equation}
