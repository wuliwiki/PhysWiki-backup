...
\end{equation}
现在考虑单个静质量为 $m$ 的粒子对能动张量 $T^{\mu\nu}$ 的影响，设它的运动轨迹为 $\bvec r_0(t)$．设 $\epsilon=m\delta(\bvec r-\bvec r_0(t))$．根据能动张量的物理意义，$T^{00}=\gamma \epsilon$，$T^{0i}=T^{i0}=\gamma m \frac{\dd x^i}{\dd t}$．由 $\gamma=\frac{\dd t}{\dd \tau}$，并根据 $F^{\mu\nu}$ 的洛伦兹协变性，可以得到单个带电粒子的能动张量的表达式：
\begin{equation}
T^{\mu\nu}=\frac{\epsilon}{\gamma}\frac{\dd x^\mu}{\dd \tau}\frac{\dd x^\nu}{\dd \tau}
\end{equation}

现在重写 $\partial_\mu T^{\mu\nu}=0$，可以得到
\begin{equation}
\partial_{\mu}T^{\mu\nu}=
\end{equation}