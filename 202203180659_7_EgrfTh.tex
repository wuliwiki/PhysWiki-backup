...
固定$\epsilon_k$，记$\{x\in E|\abs{f_n(x)}\geq\epsilon_k\}=D_{k, n}$．由于$\{f_n\}$处处趋近于$0$，故$D_{k, n}$的上极限是$\varnothing$．由\autoref{EgrfTh_lem1} ，$\lim\limits_{k\to\infty}\opn{m}D_{k, n}=0$．因此，我们可以取一个$N_k$，使得对于任意$i>N_k$，都有$\opn{m}D_{k, i}$
