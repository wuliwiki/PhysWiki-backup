...
\end{equation}
得证。
这两条引理说明，正交变换$A$的每个复特征值都对应一个二维不变子空间，且$A$限制在该子空间上的形式总如\autoref{eq_ortho_2} 所示。若$A$为二阶矩阵且实数域上无特征根，则$A$唯一bia
\begin{lemma}{}
...
\end{theorem}
\textbf{证明：}
若$A$没有特征值，由\autoref{lem_ortho_1} 可知，$A$在$V$上有一二维不变子空间，设为$S$。$A_{S^{\bot}}$是对应子空间的正交变换，由于没有特征值，因而
\subsection{对称变换}
