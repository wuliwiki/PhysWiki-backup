...
同样，在 $\sin x$ 的周期内，虽然同一个 $y$ 值通常对应两个不同的 $x$ 值，但这两个点的导数符号相反，意味着它们的运动趋势不同。因此，$(y, y')$ 这对信息可以唯一地确定周期运动中的某个状态，即“相”\footnote{特别地，将他们作为坐标构成的平面称作相平面。}，而 $x$ 则是指向该状态的唯一“相位”。在正弦型函数中，$x$ 的位置由 $\omega x + \varphi$ 代替，因此确定 $\omega x + \varphi$ 也就等同于确定 $\sin x$ 在周期内的具体位置，从而确保状态信息的完整性。在相位的表达式中：
\begin{itemize}
...
\item 若 $\Delta \varphi > 0$，则 $f_1(x)$ 相对于 $f_0(x)$ 向左平移，即 $f_0(x)$ 领先于 $f_1(x)$。
