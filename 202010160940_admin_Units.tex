...
这样，公式中就多出了一个常数．
\end{example}
在这个简单的例子中， 所有的转换常数虽然具有不同的数值， 但本质上都还是无量纲的 1． 但在一些单位制中， 同一个物理量的量纲却有可能不同．
\begin{example}{}
...
把 $F = \beta_F F'$， $q = \beta_q q'$， $r = \beta_x r'$ 代入上式左边， 对比得
\begin{equation}
\frac{\beta_q^2}{4\pi\epsilon_0\beta_x^2 \beta_F} = 1
\quad \text{即} \quad
\beta_q = \sqrt{4\pi\epsilon_0 \beta_F} \beta_x
\end{equation}
注意 $\beta_q$ 的量纲为 $\Si{C\cdot s/\sqrt{g\cdot cm^3}}$ 而不是 1．
\end{example}
