...
这样， 我们就通过矩阵 $\mat A$ 建立了从 $X$ 空间到 $Y$ 空间的一个线性映射\upref{LinMap}
\begin{equation}
A:X\to Y
\end{equation}


即 $X$ 空间的任意矢量 $\bvec x$， 都可以映射到 $Y$ 空间中唯一矢量 $\bvec y$． 注意映射并不要求不同的 $\bvec x$ 映射到不同的 $\bvec y$， 就像函数 $y = f(x)$ 中一个 $x$ 值只能对应一个 $y$， 值， 但反之则不一定． 这个映射也未必是一个满射， 即未必 $Y$ 空间的每个矢量都能被 “射中”． 这些都取决于 $\mat A$ 的具体性质．
