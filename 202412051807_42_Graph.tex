...
设 $G=(V,E)$ 是图，$e=\{x,y\},f\in E$。则：
 ...
 $x$（$y$ 也是同样的） 和 $e$ \textbf{关联}（incident）；

  $v$ 称为在 $x$ 上的边。
  
  $x$ 和 $y$ 也称为 $e$ 的\textbf{端点}（endvertex）或\textbf{顶端}（end）；
  
  $e$ 也称为\textbf{连接}（join）$x$ 和 $y$ 的边；
  
  $x$ 和 $y$ 称为\textbf{相邻的}(adjacent)。
  
  $\{x,y\}$ 也记作 $xy$ （或 $yx$）。
  
  若 $x\in X,y\in Y$，则称边 $xy$ 为 $X-Y$ 边（$X-Y$ edge）。
  
  所有 $X-Y$ 边的全体记作 $E(X,Y)$。而 $E(\{x\},Y),E(X,\{y\})$ 则分别简记作 $E(x,Y),E(X,y)$。所有和 $v\in V$ 关联的边的全体记作 $E(v)$。
