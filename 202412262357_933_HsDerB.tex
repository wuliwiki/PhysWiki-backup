...
证明过程中，使用了一步代换，尽管这里是在算极限，但这也是求导时经常使用的手段，将复杂的形式设为一个新变量，从而降低计算的难度。当$\Delta x$ 趋于 $0$时，$\E^{\Delta x}-1$也趋于$0$，即此时$t$趋于$0$。而这里使用了一个重要极限，也就是$\E$的\aref{定义}{eq_Ln_3}，变形得到了$\displaystyle\lim_{t \to 0}(1+t)^{1\over t}=\E$。对$y=\ln x$，变形有$x=\E^y$，两侧同时求导有：
\begin{equation}
1=\E^y\cdot y'~.
\end{equation}
