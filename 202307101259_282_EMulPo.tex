...
\end{equation}
注意由于势能函数是一个实数， 那么可以限制 $C_{l,m}$ 都取实数且\autoref{eq_SphHar_13}~\upref{SphHar}
\begin{equation}
C_{l,-m} = (-1)^m C_{l,m}~.
\end{equation}

式中 $l = 0$ 的项就是\textbf{电单极子（electric monopole）} 具有球对称的电荷分布和势能分布， $l = 1$ 的项就是\textbf{电偶极子（dipole）}\upref{eleDP2}（$\propto\cos \theta$）， $l = 2$ 的项是\textbf{电四极子（quadrupole）}……  $l = N$ 的项叫做电 $2^N$ 极子。
