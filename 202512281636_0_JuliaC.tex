% Julia 调用 C 语言（笔记）
% license Xiao
% type Note

\begin{issues}
\issueDraft
\end{issues}

\begin{itemize}
\item 参考\href{https://docs.julialang.org/en/v1/manual/calling-c-and-fortran-code/}{这里}。 以及\href{https://discourse.julialang.org/t/how-to-make-julia-call-c-c-coded-function/54780/5}{这里的例子}。
\item 基本没有 overhead （如果不考虑 inline function)
\item 下面是 C 语言的例子， 如果是 C++ 需要把要导出的函数定义在 \verb`extern "C" {}` 中。
\end{itemize}

\begin{lstlisting}[language=cpp]
int myfun () {
 return 2;
}
double myfun2 (float i, float j) {
 return i+j;
}
\end{lstlisting}

编译动态链接库：
\begin{lstlisting}[language=bash]
gcc -o test.o -c test.c
gcc -shared -o test.so test.o -fPIC
\end{lstlisting}

在 Julia 中调用
\begin{lstlisting}[language=julia]
test = joinpath(@__DIR__, "test.so") # String 类型
a = ccall((:myfun,test), Int32, ())
b = ccall((:myfun2,test), Float64, (Float32,Float32), 2.5, 1.5)
c = ccall(:myfun3, Float64, (Float32,Float32), 2.5, 1.5) # 若 myfun3 的库已加载则无需路径
\end{lstlisting}
\begin{itemize}
\item 其中 \verb`@__DIR__` 是当前路径。 注意 \verb`ccall` 是一个关键字， 不是函数。
\item \verb`ccall((:函数名, 路径), 出参类型, 入参类型表::Tuple, 入参1, 入参2)`
\item 如果只有一个参数， 用 \verb`(类型,)` 即 1-Tuple。 没有参数时， \verb`()` 表示 0-Tuple。
\end{itemize}
