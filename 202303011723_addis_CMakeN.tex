...
\item \verb`add_executable(exe_targ_name source_name)` 前者是可执行文件名， 后者是所有需要 link 的 \verb|c/c++| 文件名，不需要头文件。 cmake 会自动分析代码得到哪个 cpp 调用哪个头文件， 如果头文件改变了， 只有调用它的 cpp 会重新编译。 注意 \verb|exe_targ_name| 可以是任意名字不一定是可执行文件名。
...
\item \verb|get_filename_component(变量 文件名含路径 NAME)|
\end{itemize}
