...
\end{aligned}
\end{equation}
其中对于偶置换$\sigma = 1$、奇置换$\sigma = -1$；我们把它的不动点集称为 $V$ 的 $n$ 阶\textbf{反对称幂}或称\textbf{交错幂}、\textbf{外幂}），记做 ${\large \wedge}^n V$。
...
我们知道张量积空间 $V^{\otimes n}$ 的维度为 $k^n$，巧的是作为它的子空间，对称幕空间和反对称幂空间的维度之和为
\begin{equation}
\pmat{k \\ n}
\end{equation}
