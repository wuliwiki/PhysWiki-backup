...
由于自然数任何形式下的求和公式中的和函数一般并不容易求出，因此数学家Euler利用原函数来拟合和函数，下面我们先介绍一个较为简单的Euler求和公式，它在一般情况下已经是一个较好的结果．
...
\]
注意当$K$较大时，相应的余项的绝对值也会非常大，因此在近似计算时不宜将\autoref{EMSum_eq20} 的项数取的过多．Euler-Macluarin求和公式分别由Euler和Maclaurin独立发现，前者主要应用于求和函数，后者利用它来处理数值积分．
\begin{lemma}{}
