...
为方便起见，以 $n$ 为偶数为例，即可设为 $2n$ 阶多项式插值，其过 $2n+1$ 个点 $u_{i+j},j=-n,-n+1,\cdots ,n$ 只需注意 $p_i(x_{i-j})=u_{i-j},u_{i}=u_{i+2n}$，注意\autoref{eq_DifMa_3} 的规律，因此 $2n$ 阶多项式插值公式为（设 $x_{i+2n}=x_i$）
\begin{equation}
...
\end{aligned}~
\end{equation}
只要求出 $p'(x_i)$ 即可得到对应的 $n$ 阶差分方程。例如，对 $n=4$，就有
\begin{equation}
