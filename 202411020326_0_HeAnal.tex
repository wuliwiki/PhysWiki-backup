...
下面计算单频截面或微分截面，令双电离过程中从波包中单位频率吸收的能量为
\begin{equation}
...
\bar E = \frac12 \iint \omega(k_1,k_2) P_{j\to {(\bvec k_1,\bvec k_2)}} \cdot k_1^2k_2^2 \dd{k_1}\dd{k_2}\dd{\Omega_1}\dd{\Omega_2}~.
\end{equation}
对比\autoref{eq_HeAnal_27} 和\autoref{eq_HeAnal_28} 可以把双电离微分截面表示为
\begin{equation}
\pdv{\sigma_j}{\Omega_1}{\Omega_2}  = \frac{2\pi m}{s(\omega)} \ev{\omega k_1^2k_2^2 P_{j\to {(\bvec k_1,\bvec k_2)}}}~.
\end{equation}
其中 $\ev{\dots}$ 是在 $(k_1,k_2)$ 平面的第一象限的由\autoref{eq_HeAnal_20} 定义的四分之一圆上取平均值。 做角向积分得
\begin{equation}
