...
\end{equation}
其中我们假设两个原子间的相互作用只跟他们的相对距离有关，同时也假定两个粒子的质量相同。注意到关系式$(x+y)^2 +(x-y)^2 = 2x^2 + 2y^2$,我们令$\hat{P} = (\hat{p}_1 + \hat{p}_2)/2$, $\hat{p} = (\hat{p}_1 - \hat{p}_2)/2$,此时就有
\begin{equation}
\frac{\hat{p}_1^2}{2m} + \frac{\hat{p}_2^2}{2m} = \frac{\hat{P}^2}{m} + \frac{\hat{p}^2}{m}~.
\end{equation}
另外，可以证明，对动量的变换等价于对坐标的变换$R = (r_1+r_2)/2,r=(r_1-r_2)/2$，或$r_1 = (R+r),r_2 = R-r$
\begin{equation}
\frac{\partial \psi}{\partial r} = \frac{\partial \psi}{\partial r_1} \frac{\partial r_1}{\partial r} + \frac{\partial \psi}{\partial r_2} \frac{\partial r_2}{\partial r} = \frac{\partial \psi}{\partial r_1} -\frac{\partial \psi}{\partial r_2}~.
\end{equation}
对于$R$的导数类似，可以证得
\begin{equation}
(\frac{\partial \psi}{\partial r})^2 + (\frac{\partial \psi}{\partial R})^2 = 
\end{equation}
