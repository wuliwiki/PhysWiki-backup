...
\item \textbf{线性性（分配律）：}$a\bvec x\wedge(b\bvec y+c\bvec z)=ab\bvec x\wedge \bvec y+ac\bvec x\wedge\bvec z$；
\item \textbf{反对称性：} $\bvec x\wedge \bvec y=-\bvec y\wedge\bvec z$；
\item \textbf{非平凡性：} 若$\bvec x\neq\bvec y$，则$x\wedge y\neq \bvec 0 $。
\end{enumerate}

故构成 $\mathbb{F}$ 上的有限维代数。称这个代数为 $V$ 上的\textbf{外代数（exterior algebra）}、\textbf{楔积（wedge product）}或\textbf{格拉斯曼代数（Grassmann algebra）}。
\end{definition}
