...
\item \verb|(x,y) = (1,"ab")| 或者 \verb|x,y = (1,"ab")| 会分别把元素赋值给 \verb|x, y|． 这可以用于
\end{itemize}
...
\subsubsection{Dict}
\begin{itemize}
\item \verb|d = Dict{key类型,Val类型}()| 创建一个字典（哈希表）
\item \verb|push!(d, key=>val)| 添加一个元素， 如果 \verb|key| 已经存在， 就覆盖 \verb|val|．
...
\end{lstlisting}
\item 添加新元素可以用 \verb|a[key] = val|．也可以 \verb|push!(a, key => val)|
