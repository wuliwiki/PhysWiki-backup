...
\subsection{搜索顺序}
2. sys.path
If the module is not a built-in module, Python searches for it in the directories listed in sys.path. The sys.path is a list of directory locations that Python uses to look for modules and packages.

sys.path is initialized from the following sources:

The directory containing the input script (or the current directory if no script is specified).

The PYTHONPATH environment variable (a list of directories, similar to the system PATH variable).

Installation-dependent default paths (e.g., the site-packages directory where third-party packages are installed).

3. Module Search Order
Python searches for the module in the following order:

The current directory (or the directory of the script being executed).
