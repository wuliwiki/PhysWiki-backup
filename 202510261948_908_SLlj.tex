...
并被\textbf{大卫·希尔伯特}（David Hilbert）列为 1900 年著名的 23 个问题中的第一个。  
\textbf{哥德尔}（Kurt Gödel）通过构造\textbf{可构成宇宙}（constructible universe, \(L\)），
证明了在 ZF（无论是否包含选择公理）的前提下，  
连续统假设\textit{无法被否证}。  
而在 1963 年，\textbf{保罗·科恩}（Paul Cohen）使用\textbf{强迫方法}（forcing）证明，
连续统假设\textit{无法从 ZF 公理系统中被证明}\footnote{参见文献 [27]}。  
这一独立性结果虽未完全解决希尔伯特的问题，
但表明可能需要引入新的集合论公理以决定该假设的真伪。  
近年，\textbf{W. Hugh Woodin} 在此方向上进行了新的研究，
然而其最终意义仍有待评估\footnote{参见文献 [42]}。

\paragraph{当代集合论研究（Contemporary Research in Set Theory）}

现代集合论的研究重点包括：
\begin{itemize}
  \item \textbf{大基数理论}（large cardinal theory）：研究具有强大性质的基数。  
  这些“\textbf{大基数}”（large cardinals）的存在无法在 ZFC 中被证明，  
  而最小的一类——\textbf{不可达基数}（inaccessible cardinal）——
  的存在就已蕴含了 ZFC 的一致性。
  尽管这些基数具有极高的势，但其存在对实数轴结构具有深远影响。

  \item \textbf{可判定性理论}（determinacy）：  
  研究某些双人无限博弈中是否存在一方的制胜策略。  
  若这些游戏被称为“可判定”（determined），
  则制胜策略的存在会蕴含实数轴及其他\textbf{波兰空间}（Polish spaces）的结构性质。
\end{itemize}
