...
\subsection{直接数值积分}
为了作为参照我们先实现直接用数值积分计算傅里叶变换（\autoref{FTExp_eq6}~\upref{FTExp}）， 调用时需要提供一元函数 \verb|f|， 而不是一系列散点． \verb|xspan| 是对 \verb|f| 积分的区间， 而 \verb|kspan| 是输出中 \verb|k| 的区间， \verb|Nk| 是 \verb|k| 的长度． 这么做虽然直观且精确， 但计算量较大， 所以一般还是用下一节中的 FFT 方法．

另外， 如果 \verb|f| 并不是通过函数给出， 而只是一系列等间距的散点， 那么与其先插值再做数值积分， FFT 方法是最适合的．
...
用 FFT 计算傅里叶变换， \verb|f| 是一个向量， \verb|f(i)| 就是函数值 \verb|f(x0+(i-1)*dx)|． \verb|Nk| 是可选的， 若 \verb|Nk| 大于 \verb|f| 个个数， 输出中 \verb|k| 的步长将会相应变小使 \verb|k| 的长度为 \verb|Nk|， 但区间不会变． \verb|k| 的区间是由 \verb|dx| 决定的． 算法见（\autoref{DFT_sub1}~\upref{DFT}）
