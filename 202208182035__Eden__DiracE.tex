...
\end{aligned}
\end{equation}
其中将 $A_\mu$ 拆成了 $(A_0,-\bvec A)^T$．或者也可以改用标准表示（从 \autoref{qed4_eq6}~\upref{qed4} 出发得到 $\gamma$ 矩阵的标准表示．与 weyl 表示不同的是，这里的 $\gamma^0$ 为 $\pmat{&I&0\\&0&-I}$．设标准表示下 $\psi=\pmat{\varphi\\ \chi}$，则
\begin{equation}\label{DiracE_eq1}
\begin{aligned}
&(i\partial_0-qA_0-m)\varphi+\bvec \sigma\cdot (\bvec p-q\bvec A)\chi=0\\
&(i\partial_0-qA_0+m)\chi+\bvec \sigma\cdot (\bvec p-q\bvec A)\varphi=0
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
&(i\partial_0 - q A_0)\varphi = -\bvec \sigma\cdot (\bvec p-q\bvec A) \chi\\
&(i\partial_0 - q A_0 + 2m) \chi = \bvec \sigma\cdot (\bvec p-q\bvec A)  \varphi
\end{aligned}
\end{equation}
对比\autoref{DiracB_eq1}~\upref{DiracB}可以发现两者的相似之处．在非自然单位制下，上式可以改写为
\begin{equation}
\begin{aligned}
&i\hbar \pdv{t} \varphi = -c\bvec \sigma\cdot \qty(\bvec p-\frac{e}{c}\bvec A)\chi + q A_0 \varphi\\
&i\hbar\pdv{t} \chi = c \bvec \sigma\cdot \qty(\bvec p-\frac{e}{c}\bvec A) \varphi + (q A_0-2mc^2) \chi
\end{aligned}
\end{equation}
在非相对论极限下，