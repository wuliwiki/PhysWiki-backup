...
\Psi_{\bvec k}(\bvec r, t \le t_0) = (2\pi)^{-3/2} \exp\qty(\I\bvec k \vdot \bvec r - \I E_0 t)
\end{equation}
其中 $E_0 = \bvec k^2/(2m)$． 注意以下所有规范中， 广义动量算符都是 $\bvec p = -\I \grad$．
...
\end{equation}
注意 $E_0$ 只是初始时的动能， 电场中的粒子能量不守恒． 任意时刻， 波函数都是 $m\bvec v$ 和动能 $E_k$ 的本征矢， 本征值和经典粒子的动量动能相同．
...
\end{equation}
其中 $\bvec p(t)$ 是经典粒子的动量（\autoref{Volkov_eq5} ）． 注意长度规范下广义动量就是 $m\bvec v$（\autoref{LenGau_eq6}~\upref{LenGau}）．
\begin{equation}
\bvec p(t) = m\bvec v(t) = \bvec k - q\bvec A(t)
\end{equation}
注意本文中的 $\bvec A(t)$ 都是指库仑规范或速度规范中的矢势而不是长度规范\footnote{长度规范下的矢势恒为零（\autoref{LenGau_eq4}~\upref{LenGau}）}．
