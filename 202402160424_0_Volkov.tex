...
% 已代入薛定谔方程验证
\Psi_{\bvec k}^V(\bvec r, t) = (2\pi)^{-3/2} \exp\qty{\I \bvec k_0 \vdot [\bvec r - \bvec \alpha(t)] - \I E_0 t}~,
\end{equation}
...
\end{equation}
注意 $E_0$ 只是初始时间的动能 $E(t_0)$， 电场中的粒子能量不守恒。 任意时刻， 波函数都是 $m\bvec v(t)$ 和动能 $E_k(t)$ 的本征矢， 本征值和经典粒子的动量动能相同。
...
\end{equation}
其中 $\bvec p(t)$ 就是的经典粒子的动量（\autoref{eq_Volkov_5}）。 注意本文中的 $\bvec A(t)$ 都是指速度规范中的矢势， 长度规范下的矢势恒为零（\autoref{eq_LenGau_4}~\upref{LenGau}），广义动量就是 $m\bvec v$（\autoref{eq_LenGau_6}~\upref{LenGau}）。
