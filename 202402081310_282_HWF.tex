...
\caption{氢原子波函数的概率密度 $\abs{\psi_{nlm}(\bvec r)}^2$ 的 $x$-$z$ 截面， 大小成比例。 每个图中的三个数字分别是量子数 $n, l, m$， 电子出现在白色圆圈内部的概率为 0.95\footnote{为什么bu}。 色条对应的数值是线性的， 每个子图中的色条取值范围不相同。 源码见 “氢原子波函数画图程序（Matlab）\upref{Hplot}”。} \label{fig_HWF_2}
\end{figure}
\addTODO{在上图中加入比例尺}
