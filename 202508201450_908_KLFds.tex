...
$$
\operatorname{Cl}^{[0]}_{p,q}(\mathbb{R}) \;\cong\;
\begin{cases}
\operatorname{Cl}_{p,\,q-1}(\mathbb{R}), & q > 0, \\[6pt]
...
$$
\operatorname{Cl}_{0,\,n-1}(\mathbb{R}) \;\subset\; \operatorname{Cl}_{0,\,n}(\mathbb{R}),
$$
...
$$
\operatorname{Cl}^{[0]}_n(\mathbb{C}) \;\cong\; \operatorname{Cl}_{n-1}(\mathbb{C}).
$$

---

要不要我在这段的基础上，帮您绘制一个 **层次结构图/包含关系图**（例如 $\mathbb{R} \subset \mathbb{C} \subset \mathbb{H} \subset \cdots$）？这样更直观地展示克利福德代数在低维的嵌套关系。
