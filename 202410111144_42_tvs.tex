...
既然它们的拓扑由度量空间来保证，那么可以共同用度量定义的拓扑来验证。任意 $z_0=x_0+y_0$，设 $O_{z_0}$ 是 $z_0$ 邻域，于是其包含某一开球  $B(z_0,\epsilon)$， 选取 $x_0$ 的邻域 $B(x_0,\epsilon/2)$ 和 $y_0$ 的邻域 $B(y_0,\epsilon/2)$，于是任意 $x\in B(x_0,\epsilon/2),y\inB(y_0,\epsilon/2)$，成立 $d(x+y,z_0)$
\end{example}
