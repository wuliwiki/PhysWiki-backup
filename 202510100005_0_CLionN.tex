...
\item 按 \verb`+`，选择刚创建的 "Custom Build Application"，点击 Apply,
...
（其实更方便的办法是用命令行编译，\aref{仅用 IDE 调试}{sub_CLionN_1}）
...
\item \verb`Settings > Build,Execution,Development > CMake` 可以进行 cmake 设置。 可以用 \verb`-G` 选项指定构造文件（\verb`ninja/makefile/vs code`）。
