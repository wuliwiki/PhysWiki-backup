...
两个 \verb`compare_exchange` 函数的区别在于，\verb`compare_exchange_weak` 可能操作不成功，需要使用 do-while 循环执行直到成功；\verb`compare_exchange_strong` 则一定一次成功。\textbf{推荐使用强形式的函数 \verb`compare_exchange_strong`。}


下面通过介绍一些使用 \verb`atomic` 类型的例子来介绍其用法，并给出一些例程。

\subsection{自定义类型的原子化}
对于合法的自定义类，例如下面的 \verb`counter`，
