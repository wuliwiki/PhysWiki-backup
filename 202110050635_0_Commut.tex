...
若给出两个对易的厄米矩阵 $\mat A, \mat B$，
\begin{enumerate}
\item 先对 $\mat A$ 对角化， 得到本征列矢量组成的矩阵 $\mat P$
\item 计算 $\mat B_1 = \mat P \mat B\mat P^{-1}$ 得块对角矩阵
\item 解 $\mat B_1$ 的本征列矢量， 排列成块对角矩阵\footnote{只要分别对 $\mat P \mat B\mat P^{-1}$ 的每个块分别解本征矢量， 再拼成块对角矩阵即可得 $\mat R$．} $\mat R$
\item $\mat R\mat P$ 的每一列就是 $\mat A, \mat B$ 的共同本征矢
\end{enumerate}

使得 $\mat P \mat A\mat P^{-1}$ 为对角矩阵．
