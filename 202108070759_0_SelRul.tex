...
\end{aligned}
\end{equation}
把三个球谐函数之积的积分用 3j 符号表示（\autoref{SphHar_eq5}~\upref{SphHar}）
...
由三角约束（\autoref{AMAdd_eq2}~\upref{AMAdd}） $\abs{l-l'} \leqslant 1 \leqslant l + l'$ 得 $\Delta l = 0, \pm 1$． 但由\autoref{ThreeJ_eq8}~\upref{ThreeJ} 得 $l + l' + 1$ 为奇数时\autoref{SelRul_eq1} 为零， 所以只有满足
...
\subsubsection{物理意义}
$z$ 方向的电场不会改变电子 $z$ 方向的角动量， 所以 $L_z$ 守恒\upref{QMcons}， $\Delta m$ 为 0． 另外， 虽然我们的计算中并未使用光子的概念（详见量子电动力学）， 但 $\Delta l = 0, \pm 1$ 暗示了\textbf{光子的自旋}（链接未完成：自旋）为 $l=1$， 而事实的确如此．
