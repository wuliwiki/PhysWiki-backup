...
以下我们分别在长度、速度、加速度规范中求解 Volkov 波函数， 电场和矢势满足\footnote{未完成： 引用 “电场不含直流分量” 的结论．} $\bvec E(\pm\infty) = \bvec A(\pm\infty) = \bvec 0$． 三种规范下的初始波函数都是（不同规范仅当存在电磁场时才有区别）
...
\end{equation}
这是以下薛定谔方程（见\autoref{LVgaug_eq7}~\upref{LVgaug}）的解
\begin{equation}
...
\autoref{Volkov_eq4} 是以下薛定谔方程的解
\begin{equation}
\frac{\bvec p^2}{2m} \Psi^L - q\bvec {\mathcal E}(t) \Psi^L = -\I \pdv{t} \Psi^L
\end{equation}
