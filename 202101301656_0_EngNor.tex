...
回顾一维自由粒子\upref{FreeP1}中的分析， 我们需要把能量本征态即平面波进行动量或者能量归一化% 链接未完成
...
我们可以令 $\psi_{E,1}, \psi_{E,2}$ 分别为向右和向左的平面波（$k$ 为正和负值），只是归一化系数不同． 只需要使用 $\delta$ 函数得性质（\autoref{Delta_eq12}~\upref{Delta}）， 把 $E'-E$ 看成 $k$ 的函数（$k'$ 视为常数）， 根为 $k'$
\begin{equation}
...
\end{equation}
代入\autoref{EngNor_eq4} 可以猜出一种可能的关系是
\begin{equation}
