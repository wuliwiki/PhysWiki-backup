...
粒子的哈密顿函数$H(t, q_r, p_r)$是对拉格朗日函数$L(t, q_r, \dot{q}_r)$作\textbf{勒让德变换}\upref{TrLgdr}，将自变量$\dot{q}_r$都替换为$p_r=\frac{\partial L}{\partial \dot{q}_r}$得来的：
\begin{equation}
...
\begin{equation}
\ali{
    -\frac{\partial H}{\partial \phi} &= -\frac{\partial}{\partial \phi}\qty( \partial_\mu\phi\cdot\frac{\partial L}{\partial (\partial_\mu\phi)}-L(x^\mu, \phi, \partial_\mu\phi) )\\
    &= -\partial_\mu\phi\cdot\frac{\partial^2 L}{\partial \phi\cdot\partial (\partial_\mu\phi)}-L(x^\mu, \phi, \partial_\mu\phi)
}
\end{equation}
