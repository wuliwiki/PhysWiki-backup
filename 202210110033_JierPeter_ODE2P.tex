...
$J_n$ 和 $J_{-n}$ 统称为\textbf{第一类贝塞尔函数}．利用\textbf{阿贝尔微分方程恒等式}\upref{AbelID}计算这两个函数的朗斯基行列式\autoref{ODEb1_def1}~\upref{ODEb1}得
\begin{equation}
W[J_n, J_{-n}](x) = W[J_n, J_{-n}](x_0)\cdot \exp\qty(-\int _{x_0}^x p(z)\dd z)
\end{equation}




它们线性无关，因而可以用来将贝塞尔方程\autoref{ODE2P_eq18} 的通解表示为
\begin{equation}
