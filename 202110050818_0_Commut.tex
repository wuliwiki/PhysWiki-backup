...
\item 解 $\mat B_1$ 的本征列矢量， 排列成块对角矩阵\footnote{要求块对角矩阵的本征矢矩阵\autoref{HerEig_sub2}~\upref{HerEig}， 只要对每个对角块分别解本征矢量矩阵， 再拼成块对角矩阵即可．} $\mat Q$．
...
\addTODO{结合上文说明和证明}
改用 $\mat P$ 中的基底后， $\mat A$ 变为对角矩阵， $\mat B_1$ 变为块对角矩阵． $\mat B_1$ 中每个块的维数就是对应本征子空间的维数． 使 $\mat A$ 对角化的 $\mat P$ 的选择不止一种， 对本征值相同的所有列进行幺正变换同样能使 $\mat A$ 对角化， 而 $\mat Q\mat P$ 就是一个这样的变换．
