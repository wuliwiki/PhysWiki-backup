  ...
  args: Array{Any}((3,))
    ...
    3: Expr
      ...
      args: Array{Any}((3,))
        ...
        3: Symbol y
\end{lstlisting}
\item \verb`Expr` 类型是内建的，似乎无法查看源码。\verb`dump(Expr)` 会打印 
\begin{lstlisting}[language=julia]
Expr <: Any
  head::Symbol
  args::Vector{Any}
...
也可以把函数定义写到 jl 脚本中，然后在 REPL 中 \verb`include("脚本名.jl")`， 再 \verb`@code_xxx add(1, 2)`。 \verb`include()` 相当于把脚本代码直接插入到该位置。 如果找不到，就用 \verb`using InteractiveUtils
a = InteractiveUtils.@code_lowered f1();`
