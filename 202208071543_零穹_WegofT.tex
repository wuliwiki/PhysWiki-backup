...
\end{equation}
注意 ，就有 $A(Q\otimes R)\in\Lambda(V)$．
...
\end{theorem}
\textbf{证明：}1．设 $Q$ 为 $q$ 矢量，$R$ 为 $r$ 矢量，那么由\autoref{CofTen_the2}~\upref{CofTen}， $Q\otimes R\in\mathbb T_0^{q+r}$ ．由\autoref{SIofTe_the1}~\upref{SIofTe} ，对 $\mathbb T_0^{q+r}(V)$ 上的 $A$，其像 $\Im A=\Lambda ^{q+r}(V)$ ．于是 成立．
...
对一般情形，可由 和外积的结合性证得，见\autoref{AntMap_ex1}~\upref{AntMap}．
...
从上面知道，由 $Q \wedge R=A(Q\otimes R)$ 定义了 $\Lambda(V)$ 上的乘法运算．其满足封闭性、结合性、纯量和张量外积的补充定律（ ）并且还有单位元 $1\in\mathbb F$．这就使得 $\Lambda(V)$ 成为了域 $\mathbb F$ 上的一个代数（\autoref{AlgFie_def1}~\upref{AlgFie}）
\begin{definition}{外代数}
称域 $\mathbb F$ 上的代数 $\Lambda(V)$ 是空间 $V$ 的\textbf{外代数}（或\textbf{格拉斯曼代数}或\textbf{G代数}），其上的乘法由外积 “$\wedge$” （ ）所定义．
\end{definition}
\begin{theorem}{}
...
\end{equation}
由\autoref{SIofTe_eq11}~\upref{SIofTe} 和 $A$ 的线性性及 
\begin{equation}
...
\end{equation}
由 ，任意 $e_{j_1}\wedge\cdots\wedge e_{j_p}$ 都可按升序排序来代替， 即任一 $p$ 矢量都可由形如\autoref 的矢量展开．所以现在只需证明它们的线性无关即可．设


\begin{equation}
\sum_{1\leq i_1<\cdots<i_p\leq n} \lambda^{i_1\cdots i_p} e_{i_1}\wedge\cdots\wedge e_{i_p}=0
\end{equation}
由 和\autoref{SIofTe_eq3}~\upref{SIofTe}
\begin{equation}
\begin{aligned}
...
由\autoref{CofTen_the2}~\upref{CofTen}，$e_{i_1}\otimes\cdots\otimes e_{i_p}$ 构成 $\mathbb T_0^p(V)$ 上的一个基底．所以对不同的 $\pi$ ，$e_{i_{\pi^{-1} 1}}\otimes\cdots\otimes e_{i_{\pi^{-1} p}}$ 是线性无关的．所由 ，只能是
\begin{equation}
...
\end{equation}
\end{corollary}
\textbf{证明：}形如 的 $p$ 矢量的个数等于从 $n$ 个中一次取 $p$ 个组合的个数（\autoref{APC_eq1}~\upref{APC}），所以根据
\begin{equation}
\dim\Lambda^p(V)=\left(\begin{aligned}
n\\p
\end{aligned}\right)
\end{equation}
由 ，$\Lambda(V)$ 是各 $\Lambda^p(V)$ 的直和，所以
\begin{equation}
...
\end{aligned}
\end{equation}
的式子分别将 $Q,R$ 展开．然后由 ，可将 $R$ 的展开中每一项的基底的矢量一个个和前面矢量对换到 $Q$ 的基底前，最后就得到.
\end{example}
