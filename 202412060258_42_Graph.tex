...
设 $G=(V,E,\varphi)$ 是图，$e_1,e_2\in E$。若 $\varphi(e_1)=(v,v)$，则称 $e$ 为\textbf{环}（loop）。若 $\varphi(e_1)=(v_1,v_2),\varphi(e_2)=(v_2,v_1)$，则称 $e_1,e_2$\textbf{对称}（symmetric）。若$\varphi(e_1)=\varphi(e_2)$，则称 $e_1,e_2$ 为\textbf{平行}（parallel）或\textbf{重边}（multi-edges）。
\end{definition}
