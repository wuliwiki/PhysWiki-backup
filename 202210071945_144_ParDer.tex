...
\pentry{导数\upref{Der}}
\subsection{一点处的偏导数}
与一元函数的导数\upref{Der}类似，我们先从一点处开始，引入偏导数的含义．对一个多元函数 $y = f(x_1, x_2 \dots x_i \dots)$，如果求导时只把 $x_i$ 看成自变量，剩下的 $x_{j \ne i}$ 都看做常数，得到的导数就叫函数（关于 $x_i$）的\textbf{偏导数}．

\begin{figure}[ht]
\centering
\includegraphics[width=14cm]{./figures/ParDer_1.png}
\caption{一点处的偏导数} \label{ParDer_fig1}
\end{figure}
如以二元函数 $z=f(x,y)$ 为例，对 $x$ 的偏导数常记为
