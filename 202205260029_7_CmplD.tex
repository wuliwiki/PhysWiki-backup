...
由于$\mathbb{F}$是完备域，所以可以取$b_i\in\mathbb{F}$使得$b_i^p=a_i$．定义$h(x) = \sum_{i=0}^n b_ix^i$．由于$\opn{ch}\mathbb{F}=p$，知
\begin{equation}
\qty(f(x))^p=(\sum_{i=0}^n a_ix^i)^p=\sum_{i=0}^n a_i^p x^{ip}
\end{equation}
