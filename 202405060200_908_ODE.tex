...
有一些微分方程的解法是显然的， 例如描述{自由落体运动}{ConstA} 的微分方程为 $\dv*[2]{y}{t} = g$ （假设 $y$ 轴竖直向下）。 要解这个方程， 只需对等式两边进行两次不定积分即可得到通解为 $y = C_1 + C_2 t + gt^2/2$。 一般来说， 如果 $N$ 阶微分方程具有 $y^{(N)} = f(x)$ 的形式， 只需进行 $N$ 次积分即可得到通解。
