...
初中接触的三角函数主要包括\textbf{正弦（sine）}、\textbf{余弦（cosine）}和\textbf{正切（tangent）}，角$\theta$的这三个三角函数分别记作$\sin\theta,\cos\theta,\tan\theta$。设直角三角形$ABC$，角$C$为直角，记角$A$对边、邻边和斜边的长度分别为$|BC|,|AC|,|AB|$。则角$A$的三角函数的定义如下：
\begin{equation}
