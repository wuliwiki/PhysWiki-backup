...
\end{pmatrix}.~
$$
更正式地说，该映射定义了从 $\mathbb{R}^3$ 到迹为零的厄米 $2\times 2$ 矩阵向量空间的映射。该映射通过矩阵函数编码了 $\mathbb{R}^3$ 作为赋范向量空间和**李代数（其中向量叉乘作为李括号）的结构，使得该映射成为一个李代数同构。
...
$$
这实际上是映射$\vec{a} \;\mapsto\; \vec{a} \cdot \vec{\sigma}$的逆映射**，从而明确表明该映射是一个**双射**。
