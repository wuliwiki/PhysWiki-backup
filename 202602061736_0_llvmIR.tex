...
\item \verb`getelementptr` （GEP）的格式为 \verb`指针 = getelementptr 基类型, 基类型* 指针, 指标类型1 ind1, 指标类型2 ind2 ...`。 在取数组第 3 个元素时，最后是 \verb`i64 0, i64 2`， 第一个从 \verb`数组的指针` 中获取数组 \verb`array`， 第二个从 \verb`array` 中获取获取第 3 个元素。
\end{itemize}
