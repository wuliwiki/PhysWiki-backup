...
在物理中我们首先要区分两类变量， 一种是只有数值没有单位， 例如“甲的身高是乙的 $1.27$ 倍”． 另一种既有数值也有单位， 例如“甲的身高是 $1.8\Si{m}$”． 我们把前者叫做一个\textbf{数}， 把后者叫做\textbf{物理量}． 我们可以把物理量和一维几何矢量\upref{GVec}的部分性质做类比： 一个一维几何矢量本身也不能用一个数描述， 而是需要先选取一个矢量基底， 然后用一个数（即坐标）乘以这个基底表示这个矢量． 例如若甲选择的基底是乙的基底的两倍， 那么甲的坐标数值将会是乙的一半．
...
然而， 几个相同或不同量纲的物理量相乘或相除却是很常见的． 例如长度乘以长度等于体积， 长度除以时间等于速度． 相乘或相除后所得物理量的单位也可以写成若干个基本单位相乘或者相除． 例如
\begin{equation}
1\Si{m} \times 1\Si{m} = 1\Si{m\cdot m} = 1\Si{m}^2
\end{equation}
\begin{equation}
1\Si{m} / 1\Si{s} = 1 \Si{m/s} = 1 \Si{m s^{-1}}
\end{equation}
