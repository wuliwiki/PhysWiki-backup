...
一种 $\psi_{k,1}$ 常用的行波边界条件为 $D_1 = 0$， 它的物理意义是粒子从左边入射， 发生反射和透射． 满足归一化条件（链接未完成）
\begin{equation}
\abs{A_1}^2 + \abs{B_1}^2 + \abs{C_1}^2 = \frac{1}{\pi}
\end{equation}
又根据概率流\upref{PrbJ}守恒得 $\abs{A_1}^2 = \abs{B_1}^2 + \abs{C_1}^2$， 得 $\abs{A_1} = 1/\sqrt{2\pi}$． 为了方便我们一般直接规定
\begin{equation}\label{ScaNrm_eq10}
A_1 = \frac{1}{\sqrt{2\pi}}
\end{equation}
一般情况下我们通过施密特正交化得到与之正交归一的 $\psi_2$． 我们先令不一定正交的 $\psi'_2$ 满足边界条件 $A_2 = 0$， 即粒子从右边入射， 投影得
\begin{equation}
