...
\subsection{形式逻辑系统}
数学逻辑的核心在于以\textbf{形式逻辑系统}（formal logical systems）为工具来刻画数学概念。  
这些系统虽然在细节上各不相同，但它们的共同点是：只研究在一个固定的\textbf{形式语言}（formal language）中表达的符号与命题。  
其中，\textbf{命题逻辑}（propositional logic）与\textbf{一阶逻辑}（first-order logic）是当今研究最为广泛的两种系统，  
原因在于它们既能有效地应用于数学基础研究，又具有良好的证明论性质。\footnote{参见注释 [c]}  
此外，学者们还研究更强的经典逻辑系统，如\textbf{二阶逻辑}（second-order logic）与\textbf{无限逻辑}（infinitary logic），  
以及\textbf{非经典逻辑}（non-classical logics），如\textbf{直觉主义逻辑}（intuitionistic logic）。

\subsubsection{一阶逻辑}

（参见主条目：\textit{First-order logic}）

一阶逻辑是一种特殊的形式逻辑系统，其\textbf{句法}（syntax）仅允许有限表达式构成良构公式（well-formed formulas），  
而其\textbf{语义}（semantics）则以“所有量词皆限定于一个固定论域”（fixed domain of discourse）为特征。

形式逻辑的早期成果揭示了一阶逻辑的若干局限。  
1919年，\textbf{洛文海姆–斯科勒姆定理}（Löwenheim–Skolem theorem）表明：  
若一个可数一阶语言中的句子集合存在一个无限模型，则它在每一个无限基数上至少都有一个模型。  
这意味着——\textit{不存在一组一阶公理可以唯一地刻画自然数系、实数系或任何其他无限结构的同构类}。  
由于早期数学基础研究的目标是为整个数学建立统一的公理化理论，这一限制尤其令人震撼。

\textbf{哥德尔完备性定理}（Gödel’s completeness theorem）建立了一阶逻辑中语义与句法之间的等价关系。\footnote{参见文献 [31]}  
该定理指出：若某命题在所有满足某组公理的模型中皆为真，则必存在一段有限的推导可由这些公理导出该命题。  
而\textbf{紧致性定理}（compactness theorem）最初作为哥德尔完备性证明中的引理出现，  
但直到多年后逻辑学家才认识到其重要性并广泛应用。  
紧致性定理断言：一个句子集合有模型，当且仅当其每个有限子集都有模型；换言之，  
若一个公式集不一致，则其必存在某个有限子集亦不一致。  
完备性与紧致性这两项定理，使得对一阶逻辑中“逻辑蕴涵”的研究变得极为精致，并奠定了模型论发展的基础。  
这也是一阶逻辑在整个数学中占据核心地位的重要原因之一。

\textbf{哥德尔不完备定理}（Gödel’s incompleteness theorems）进一步揭示了一阶公理化体系的根本限制。\footnote{参见文献 [38]}  
第一不完备定理指出：对于任何一致的、\textit{有效给定}（effectively given，定义见下文）且能解释算术的一阶逻辑系统，  
总存在一个命题在自然数意义下为真，但在该系统内却不可被证明（并且在某些与该系统一致的非标准模型中甚至可能为假）。  
例如，在任何能表达\textbf{皮亚诺公理}（Peano axioms）的逻辑系统中，  
哥德尔句（Gödel sentence）在自然数上成立，却无法在该系统内被证明。

这里，一个逻辑系统称为“有效给定”，  
是指能否机械地判断任意给定公式是否为该系统的公理；  
若系统还能表达皮亚诺算术，则称其“足够强”（sufficiently strong）。  
当该定理应用于一阶逻辑时，  
它意味着任何足够强、一致、有效的一阶理论都存在非初等等价的模型，  
这一限制比洛文海姆–斯科勒姆定理所揭示的更为严格。  

\textbf{第二不完备定理}进一步指出：  
任何足够强、一致、有效的算术公理系统都无法在自身内部证明其自身的一致性。  
这一结论通常被解释为表明\textbf{希尔伯特计划}（Hilbert’s program）无法彻底实现。
