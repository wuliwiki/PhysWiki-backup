...
\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{./figures/20a723f6591249f5.png}
\caption{} \label{fig_interf_5}
\end{figure}

如上图所示，$P_1$为光屏上的任意点。在该点以$r_1$为半径画圆，交$S_2P_1$于$M$。考虑到双缝间距$d$远小于屏距$l$，$\bigtriangleup S_1S_2M$通常近似于直角三角形，即$\angle S_1MS_2=\pi/2$。 此时这两道光近似于平行光，$\angle P_1OP_0 \approx \theta$，联立方程
\begin{equation}
\begin{aligned}
...
r_2-r_1=\frac{dx}{l}~.
\end{equation}
因此，我们可以得到双缝干涉实验的两个规律。
\begin{itemize}
\item 亮纹中心位置为
\begin{equation}\label{eq_interf_1}
x=\frac{nl}{d}\lambda,(n\in\mathbb N)~.
\end{equation}
\item 相邻亮纹或暗纹的中心间距为
\begin{equation}
...
\end{itemize}
由\autoref{eq_interf_1} 可知，波长越长，同级亮纹距离光屏中心越远，相邻亮纹的中心间距也越大。因此，红光亮纹图样相当于紫光上移，且更为稀疏。

我们也可以用白光做实验。也就是没有在单缝前面加滤光片，从而在光屏上得到彩色条纹。对于一级条纹，显然红光条纹最靠外，紫光最靠内。从二级条纹开始，不同级且颜色不同的条纹开始重叠，所以颜色逐渐变淡，最后呈现白色。
\subsubsection{实验装置}
