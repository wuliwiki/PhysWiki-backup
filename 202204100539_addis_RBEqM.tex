...
\end{equation}
注意此时式中的 $\bvec\omega$ 和 $\dot{\bvec\omega}$ 仍然是刚体相对于实验室坐标系的角（加）速度， 只是使用了体坐标系中的基底来计算坐标． 第二条中特地使用 $\bvec\alpha_0$ 表示角加速度而不是 $\dot{\bvec\omega}_0$， 因为后者容易被误解为 $\bvec\omega_0$ 的时间导数， 这显然是错误的． 那么欧拉方程\autoref{RBEqM_eq6} 可以简化为
...
\end{equation}
容易发现 $\mat\Omega_0$ 就是体坐标系中 $\bvec\omega_0\cross$ 代表的矩阵． 那么易得\autoref{RBEqM_eq4} 变为
\begin{equation}
123
\end{equation}
...
\mat\Omega_0 = \mat R\Tr\mat\Omega\mat R
\end{equation}
容易发现 $\mat\Omega_0$ 就是体坐标系中 $\bvec\omega_0\cross$ 代表的矩阵． 代入后就得到\autoref{RBEqM_eq9}．
