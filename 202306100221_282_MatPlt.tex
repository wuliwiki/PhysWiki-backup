...
如果直接通过菜单保存图片， 会默认使用显示器的分辨率， 要按指定的分辨率保存图片例如 \verb|exportgraphics(gcf, '文件名.png', 'Resolution', 300)|。 另外也可以用菜单或者 \verb|saveas()| 保存为矢量图等格式。 另外还有 \verb|print| 函数也可以： \verb|print('figurename', '-dpng', '-r300');|
