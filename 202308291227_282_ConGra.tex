...
同理，若 $\mat A$ 是一个厄米矩阵\upref{HerMat}且正定，把\autoref{eq_ConGra_2} 中的转置改为厄米共轭\upref{HerMat}即可（未完成：需要求证）。

\subsection{双共轭梯度法}

梯度法可以拓展为\textbf{双共轭梯度法}， 以适用于任意线性方程组。 该方法的优势在于用户只需要向解算器提供矩阵乘矢量的函数， 而不需要提供矩阵本身。 这样， 矩阵可以具有任意的数据结构， 例如各种稀疏矩阵\upref{SprMat}。
