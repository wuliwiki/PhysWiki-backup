...
另外，如果把一个正方形的四个顶点顺时针依次编号为$1, 3, 2, 4$，则不难看出$C$实际上就是正方形的对称群$D_4$．类似地，$S_4$的每个Sylow-$2$子群都是$D_4$．
...
如果$[\mathbb{K}:\mathbb{F}]=2$，而$2$是素数，故同理可知$\abs{\opn{Gal}(\mathbb{K}:\mathbb{F})}=2$，进而$$
