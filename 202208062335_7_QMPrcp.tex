...
在给定基下，$\bra{s_i}$的坐标是一个列矩阵$c$，除了第$i$行为$1$，其它行都为$0$；类似地，$\ket{s_j}$的坐标是一个行矩阵，除了第$j$列为$1$，其它列都为$0$．设$X$的坐标是$M$，其第$i$行$j$列的坐标为$m^i_j$．则有

\begin{equation}
\bra{s_i}X\ket{s_j} = m^i_j
\end{equation}
