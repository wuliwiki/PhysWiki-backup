...
显然，$s,S$ 分别是对应该分划的积分和中的下界和上界（\autoref{def_SupInf_1}~\upref{SupInf}）。如果 $f(x)$ 是连续函数，$s,S$ 则是\textbf{下确界}和\textbf{上确界}(\autoref{def_SupInf_2}~\upref{SupInf})，此时达布下、上和可直接记作$\underline \sigma(f, \mathcal P),\bar \sigma(f, \mathcal P) $。
...
\begin{equation}
m_i(x_{i+1}-x_i),\qquad m1_i(x'-x_i)+m2_i(x_{i+1}-x'),
\end{equation}
其中 $m1_i,m2_i$ 分别是函数 $f(x)$ 在区间 $[x_i,x'],[x',x_{i+1}]$ 山的最小值，由于 $m_i$ 是q
\textbf{证毕！}
