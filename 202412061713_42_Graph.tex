...
\begin{definition}{$n$ 部分图}
设 $G=(V,E,\varphi)$ 是图。若 $V$ 可分为 $n$ 个不相交的点集 $V_1,\cdots,V_n$，且任意 $V_i$ 上的不同点是不相邻的，则称 $G$ 是 $n$ \textbf{部分图}（partite graph）。
\end{definition}

\begin{definition}{完全 $n$ 部分图}
设 $G=(V,E,\varphi)$ 是简单 $n$ 部分无向图，$V_1,\cdots,V_n$ 是对应的划分。若 $\forall x\in V_i,y\in V_j,i\neq j$，都有 $e\in E$，使得 $x,y$ 相邻，则称 $G$ 为\textbf{完全 $n$ 部图}。
\end{definition}
