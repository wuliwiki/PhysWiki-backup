...
\begin{enumerate}
\item \textbf{负能解}
\autoref{QMPrcp_eq1} 的平面波解为 $\phi_{\boldsymbol{p}}(\boldsymbol{x}, t)=\frac{1}{\sqrt{V}} e^{- \I\left(E_{\boldsymbol{p}} t-\boldsymbol{p} \cdot \boldsymbol{x}\right)}$．质壳条件并没有对能量的正负作出约束，其本征值为
...
\end{equation}
其中，$\boldsymbol {\alpha}$和$\beta$是待定系数，它们一定不能是常数，否则不能满足K-G方程，假设这两个待定系数是矩阵．\autoref{QMPrcp_eq2} 平方后有
...
\end{equation}
同时满足\autoref{QMPrcp_eq4} 和\autoref{QMPrcp_eq5} 的不同表示\textbf{(representation）}之间通过相似变换联系（）．
常用的表示有以下两种：
...
\end{array}\right)
\end{equation}
对\autoref{QMPrcp_eq2} 进行一次量子化．动量被动量算符代替，由含时薛定谔方程得：\begin{equation}\label{QMPrcp_eq9}
...
\begin{equation}\label{QMPrcp_eq10}
...
\textbf{标准表示}
采取静止坐标系，此时粒子动量为0，\autoref{QMPrcp_eq9} 变为
\begin{equation}\label{QMPrcp_eq11}
 \I \frac{\partial}{\partial t} \psi= \beta m\psi
\end{equation}
令 $\psi=\left(\begin{array}{l}\psi_{+} \\ \psi_{-}\end{array}\right)$，代入 $\beta$ 得：
\begin{equation}\label{QMPrcp_eq14}
 \I \frac{\partial}{\partial t} \psi_{+}=+m \psi_{+}
\end{equation}
\begin{equation}\label{QMPrcp_eq15}
 \I \frac{\partial}{\partial t} \psi_{-}=-m \psi_{-}
\end{equation}
以上两式表明 $\psi_{+}$ 与 $\psi_{-}$ 分别对应能量本征值为 $m$ 与 $-m$ 的波函数解．

\textbf{手征表示} 同样采取静止坐标系，令 $\psi=\left(\begin{array}{l}\psi_{L} \\ \psi_{R}\end{array}\right)$,代入该表示中的 $\beta$ 于\autoref{QMPrcp_eq11} ，则：
\begin{equation}\label{QMPrcp_eq12}
 \I\frac{\partial}{\partial t} \psi_{L}=m \psi_{R}
\end{equation}
\begin{equation}\label{QMPrcp_eq13}
...
\psi_{-}=\frac{1}{\sqrt{2}}\left(\psi_{L}-\psi_{R}\right)
\end{equation}
代入\autoref{QMPrcp_eq12} 与\autoref{QMPrcp_eq13} ，并进行相加减，最后可得到\autoref{QMPrcp_eq14} 与\autoref{QMPrcp_eq15} ，意味着不同表示并没有改变能量本征值与对应的本征态．
...
将轨道角动量 $L$ 代入算符的运动方程有：
\begin{equation}\label{QMPrcp_eq16}
\frac{d {L}}{d t}=- \I[L,H]
\end{equation}

由于狄拉克方程描述的自由粒子角动量守恒，令上式 为0，代入狄拉克方程的哈密顿量后有：
\begin{equation}\label{QMPrcp_eq18}
- \I[L,H]=
...
\begin{equation}\label{QMPrcp_eq17}
...
\begin{equation}\label{QMPrcp_eq19}
[H, \mathbf{\Sigma}]=2  \I \boldsymbol{\alpha} \times \boldsymbol{p}
\end{equation}
对比\autoref{QMPrcp_eq18}  和\autoref{QMPrcp_eq19} ，我们可以得到：
\begin{equation}
...
\begin{equation}\label{QMPrcp_eq22}
\left(\mathrm{i} \gamma^{\mu} \frac{\partial}{\partial x^{\mu}}-m\right) \psi(x)=0
\end{equation}
若引入费曼斜线标记，即 $\not A$ 表示 $\gamma^{\mu}A_{\mu}$，狄拉克方程可以更加简便地写为：
\begin{equation}\label{QMPrcp_eq21}
( \not p-m) \psi=0
\end{equation}
\subsubsection{拉氏量}
狄拉克方程作为描述旋量的经典场方程，理应能从拉格朗日方程或由哈密顿原理导出，以此可作为拉氏量的一种验证方式．方程的洛伦兹协变性要求拉氏量也具有相同的协变性．在无相互作用的条件下，Dirac场的拉氏密度如下：
\begin{equation}\label{QMPrcp_eq20}
\mathcal{L}=\mathrm{\I} \bar{\psi} \gamma^{\mu} \partial_{\mu} \psi-m \bar{\psi} \psi \quad\left(\bar{\psi}=\psi^{\dagger} \gamma^{0}\right)
\end{equation}
对 $\bar\psi$ 进行变分可得到Dirac旋量的运动方程，即\autoref{QMPrcp_eq22} ．

\subsection{狄拉克方程的解}
现在我们在手征表象下，对\autoref{QMPrcp_eq21} 进行求解．由于狄拉克方程可变换为K-G方程，所以狄拉克方程的解亦是平面波的叠加，即 $\Psi(x)=u(p)e^{-ipx}+v(p)e^{ipx}$，其中 $u(p)$ 和 $v(p)$ 都是四分量旋量．将正能部分和负能部分分别代入狄拉克方程后可得：
\begin{equation}\label{QMPrcp_eq23}
(\not p-m) u(p)=0 
\end{equation}
\begin{equation}
(\not p+m) v(p)=0
...
我们先对正能部分进行讨论．为了方便，对粒子采取静止坐标系，则 $p^\mu=(m,0,0,0)$．（非静坐标系下的解可通过对静坐标系下的解作boost得到）．\autoref{QMPrcp_eq23} 在手征表象下为：
\begin{equation}
...
\end{aligned}
\end{equation}
$\bar\psi\times$\autoref{QMPrcp_eq22} +\autoref{QMPrcp_eq24} $\times\psi$，可得:
\begin{equation}
i \bar{\psi} \gamma^{\mu}\left(\partial_{\mu} \psi\right)+i\left(\partial_{\mu} \bar{\psi}\right) \gamma^{\mu} \psi=0
\end{equation}
可以验证，式45可改写为$\partial_{\mu}\left(\bar{\psi} \gamma^{\mu} \psi\right)=0$，因此我们得到了四维矢量流密度的形式．概率荷密度是0分量，即：\begin{equation}\label{QMPrcp_eq24}
