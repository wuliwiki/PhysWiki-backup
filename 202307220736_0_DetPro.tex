...
\begin{corollary}{对角矩阵和三角矩阵的行列式}
\begin{enumerate}
\item 对角矩阵的行列式等于所有对角元相乘。
\item 上三角矩阵或下三角矩阵（\autoref{eq_Mat_7}~\upref{Mat}）的行列式的值等于其对角线上的值相乘。
\end{enumerate}
\end{corollary}
证明：根据行列式定义可证明 1； 把 2 中的三角矩阵做行变换或列变换\upref{GAUSS}可得对角矩阵，根据\autoref{the_DetPro_5}，行列式的值不变，命题变为 1。 证毕。 
