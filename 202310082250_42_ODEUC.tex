...
注意 $(g_{0})_{*}=f_0$，于是由数学归纳法可从\autoref{eq_ODEUC_8} 推得 $(g_{n+1})_{*}=f_{n+1}$。因此，序列 $\{f_n\}$ 是 $\{g_n\}$ 的导数序列。对充分小的 $\abs{t-t_1}$，\autoref{eq_ODEUC_8} 一致收敛（因为皮卡映射的压缩性要求其不动点存在，$f_$）。因此序列 $\{g_n\}$ 及其关于 $x_1$ 的导数都一致收敛，因此极限函数
\begin{equation}
