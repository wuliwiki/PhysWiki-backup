...
\item 如果上传时发现图片文件的 hash 已存在于 \verb|images.hash|， 则检查该文件的 \verb|images.figures| 中 \verb|figures.aka| 为空的图片环境（只有一个或者没有，否则就是 bug）。 如果有且 \verb|figures.deleted==0|， 有就提示图片已存在， 让用户选择引用（\verb|\autoref{}|）（推荐）或者重复使用图片。 若有多个就把标签全部列出。 如果没有， 或者用户选择重复使用图片， 就检查 \verb|images.figures| 中被标记 \verb|figures.deleted| 的环境有没有原本属于当前词条的， 如果有就更新 \verb|figures.deleted=0| 然后根据该记录中的信息插入图片环境代码； 如果没有。


未标记 \verb|figures.deleted| 的图片环境 提示， 或者重复使用图片（等效于上传新图片， 但更新 \verb|figures.aka| 指向已存在的图片）。 
