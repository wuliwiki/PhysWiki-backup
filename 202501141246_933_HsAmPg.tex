...
A = \frac{a+b}{2}~.
\end{equation}
这说明，$a$ 与 $b$ 的等差中项正好是他们的\textbf{算术平均值（arithmetic mean）}\footnote{关于算术平均值参见\aref{基本不等式}{sub_HsIden_2}}。反过来，从\autoref{eq_HsAmPg_10} 可以得到：
\begin{equation}
2A = a+b~.
\end{equation}
结合\autoref{cor_HsAmPg_1} 可以知道，对于一个已知的等差数列，如果某两项的项数和为另一个项数的两倍，那么后者就是前者的等差中项。显然，这要求那两项的项数和不能为奇数，否则无法从等差数列中找到这项\footnote{对于直线，无论什么情况，都能够找到这个点。}。

从上面的例子可以看出，有两种视角可以理解“等差中项”这一名称：它既可以指在两个数之间插入的数，与它们构成等差关系；也可以指等差数列中位于某两项中间位置的那项。
