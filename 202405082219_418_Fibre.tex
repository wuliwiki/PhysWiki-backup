...
\begin{definition}{纤维丛}
    ...
    \begin{CD}
		...
		U @= U
\end{CD}\\
	应是可交换的（我们说交换图是可交换的即图上任意两点间的各路径分别对应一个复合映射，这些复合映射应得到同样的结果）。而 $\pi^{-1}[U]$ 给出一个子空间拓扑。 对于 $\operatorname{proj}_1 : (U \times F) \rightarrow U$ 是自然映射，其考虑为笛卡尔积的投影，或者说 $\forall (u, f) \in (U \times F)$ 其给出 $u$。
