...
\item \verb|sudo zfs set mountpoint=/挂载目录 pool名/fs名| 改变挂载目录。
\item \verb|sudo zfs unmount pool名| 弹出 pool， 用 \verb|zfs mount| 重新挂载。
\item \verb|sudo zfs set compression=on pool名/fs名| 开启压缩。 可以随时打开或者关闭。
\item \verb|sudo zfs set dedup=on pool名/fs名| 开启查重（重复文件可以节约空间）。
