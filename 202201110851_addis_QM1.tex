...
首先要注意的是在讨论量子力学的一维问题时， 我们不能完全假设粒子（质点）在三维空间中延某条直线运动． 如果假设粒子延 $x$ 轴运动， 那么在 $y$ 和 $z$ 方向的动量和坐标就可以同时确定， 而这是违反不确定性原理的． 所以最好的办法是假设空间只有一个维度．
...
事实上， 同一个算符在不同的\textbf{表象}下具有不同的形式， 这可以类比同一个矢量用不同的基底可以得到不同的坐标\upref{Gvec2}， 同一个线性算符也表示为不同的矩阵．% 链接未完成
