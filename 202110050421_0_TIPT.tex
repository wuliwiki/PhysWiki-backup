...
我们只需要通过某种方法找到上式的某个解即可． 观察等式右边可以发现矩阵的对角块都是对角矩阵， 但等式左边在我们确定 $\psi_n^0$ 时就已经确定了， 且一般来说并不满足这一要求．

但考虑到 $H_0$ 简并， 它的一套本征矢（正交归一基底） $\psi_n^0$ 并不是完全确定的， 每个简并子空间中正交归一基底的选取都仍有一定自由——可以进行任意基底变换． 这使我们总可以通过适当的基底变换得到合适的 $\psi_n^0$ 使上面的等式成立． 换言之， 我们希望在每个本征子空间中， $H^0$ 和 $H^1$ 可以同时对角化． 注意这并不要求 $[H^0, H^1] = 0$ 对易， 因为对易要求整个 $\mel{\psi_m^0}{H^1}{\psi_n^0}$ 矩阵可以被彻底对角化而不只是每个对角块对角化． 这样， 通常就能唯一地确定 $\psi_n^0$， 但不唯一也关系不大．
