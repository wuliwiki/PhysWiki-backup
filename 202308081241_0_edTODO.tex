...
\subsubsection{新备份算法}
\begin{itemize}
\item editor 备份步骤： 更新 \verb|contents/文件名.tex|， 然后运行 \verb|PhysWikiScan --backup 文件名|，不需要修改数据库。如果备份成功且没有覆盖备份文件，就会在 stdout 输出 \verb|added 文件名|。 若备份成功且覆盖了已有文件，就会输出 \verb|replac| 若已存相同 hash 的备份文件则输出 \verb|exist 已存在的文件名|。 事实上，后者不应该出现，因为编辑器前端应该检测到内容改变才进行备份。
