% 刚体定轴转动 转动惯量
% keys 转动惯量|角动量定理|刚体|绕轴转动|角速度|角加速度
...
要讨论刚体的定轴转动和力矩之间的关系， 我们需要先来看\textbf{角动量矢量在延轴方向的分量}． 以后会知道若定轴旋转的刚体的质量分部关于转轴有某种旋转对称\footnote{例如圆盘、长方形、正三角形等绕对称轴旋转}， 那么刚体的角动量矢量必定是平行于转轴的， 然而对于更一般的刚体（如\autoref{RigRot_fig2}）， 定轴转动时的角动量矢量就未必与转轴平行（如\autoref{RBrot2_ex1}~\upref{RBrot2}）． 所以为了简单起见我们先讨论其延轴的分量， 完整的矢量关系以后会在 “惯性张量\upref{ITensr}” 中看到．

我们把转轴的某个正方向定义为 $z$ 方向． 对于刚体上的单个质点， 角动量在 $z$ 方向的分量为
\begin{equation}
...
\subsection{角加速度与力矩}
要分析刚体的转动与刚体所受外力的关系， 就要对系统使用角动量定理\upref{AMLaw}． 在这里 “系统” 指的就是刚体本身， 不包含任何相对刚体运动的物体． \autoref{RigRot_fig2} 中所画的转轴可以认为是一个数学上的直线而不是真实中有粗细有质量的机械转轴， 也不对刚体提供任何作用力． 现实中约束刚体延该数学转轴旋转可能会使用非常不同的机械结构， 例如通过轴承将物体固定在一根不旋转的杆上， 此时杆则不属于刚体的一部分， 杆对旋转体的所有作用力都视为系统外力． 另一种大同小异的情况例如杆与旋转体相对固定一起转动， 且杆两端通过轴承固定， 此时杆可以视为刚体的一部分， 故无需分析杆和物体之间的相互作用， 轴承对杆的任何作用力都视为系统外力． 总之无论机械结构有多么复杂， 我们只需要把机械结构中不形变的、整体绕数学轴旋转的部分看成刚体， 刚体外部对刚体的力视为系统外力， 那么以下的分析就是成立的．

可以对系统使用 “角动量定理\upref{AMLaw}” 的\autoref{AMLaw_eq1}， 注意等号两边是矢量， 所以各个分量必须相等， 我们有
...
可见和 “匀速运动不需要外力维持” 一样， 匀速转动也并不需要延轴方向的外力矩维持． 然而在日常生活中， 正如水平直线轨道上具有初速度的滑块会由于与轨道的摩擦力， 空气阻力等各种外力最终停止运动， 具有初始角速度的物体也会在机械摩擦、空气阻力等外力矩的作用下最终停止转动． 所以这与上述动量守恒并不矛盾．
