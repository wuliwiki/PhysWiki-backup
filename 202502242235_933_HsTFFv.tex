...
\item $\varphi$ 称为\textbf{初相（initial phase）}，设定初始相位，决定了函数图像的水平偏移量。
\end{itemize}

这种分解方式将三角函数的周期性行为与输入变量的线性变化分开，使得分析更加直观，也更容易理解各参数对函数图像的影响。在数学建模中，许多涉及非线性的研究，如神经网络或回归分析，也常采用类似的方法，即先处理线性部分，再通过非线性映射得到最终结果，是一种很重要的方法。
...
\hline
$x$ & $0$ &$\displaystyle {T\over4}$& $\displaystyle {T\over2}$ & $\displaystyle {3\over4}T$ & $\displaystyle T$\\
\hline
$\sin x$ & $0$&$1$&$0$&$-1$&$0$ \\
\hline
$f(x)$ & $b$&$b+A$&$b$&$b-A$&$b$ \\
...
\end{tabular}
\end{table}
这五个点包含了一个周期内的零点（对称中心）和极值点（对称轴）的信息，利用周期性扩展基本就能获得完整的函数图像。另外如前所述，内部是线性的，因此经过$\sin x$映射后，仍然会呈现出sinx的形态，外部的A和b是对结果进行第二次线性变换。而五点本质上就是在描述sinx的非线性。
