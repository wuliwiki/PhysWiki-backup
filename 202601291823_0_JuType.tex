...
\subsubsection{符号}
\begin{itemize}
\item 用户可以用 \verb`:名字` 表示任意名字的 \verb`Symbol`，常用于字典的 key，类似于 C++ 的 enum。
\item \verb`Symbol` （又例如 \verb`:abc`， \verb`:a`）注意 \verb`:pi` 和 \verb`:π` 是两个不同的 \verb`Symbol`。
\item 如果是 Symbol 名字含有较复杂的字符，可以用 \verb`Symbol("#3ac!3f")` 创建。
\item \verb`Symbol(:abc, :def, :ghi)` 可以合并符号名，得到 \verb`:abcdefghi`
\end{itemize}


\subsubsection{无理数}
\begin{itemize}
\item \verb`pi` 的类型是 \verb`Irrational{:π}` 其中 \verb`:π` 的类型是 
...
\item \verb`append!(数组, 元素)` 或者 \verb`push!(数组, 元素)` 可以在列向量最后添加一个元素。
\item \verb`push!([1, 2, 3], 4, 5)` 或者 \verb`append!([1, 2, 3], [4, 5])`
\item \verb`类型[]` 创建空数组
...
\item \verb`objectid(1.23)` 会返回一个哈希如 \verb`0xae6a147d33244922`。 \verb`objectid(x) == objectid(y)` 一定有 \verb`x === y`，反过来大部分情况成立，但不保证。
...
\subsubsection{SimpleVector}
\begin{itemize}
\item \verb`v = Core.svec(1, "a", :x)` 创建一个简单矩阵，通常不被用户使用而在内核内部使用。可以用 \verb`v[i]` 获取值，但不能改变（unmutable）
\item 可以用 \verb`length(v)` 检查长度
\item \verb`collect(v)` 变为普通数组
\end{itemize}


\subsection{Array 的内部}
较新版的 Julia \verb`dump(Matrix{Float64})`
\begin{lstlisting}[language=julia]
...
\end{lstlisting}
\begin{itemize}
\item 要获取成员的值， 用 \verb`foo.bar`, \verb`foo.baz` 等。 用 \verb`struct` 声明的复合类型的变量在定义后就不能被改变。 如果 \verb`struct` 里面有矩阵等， 那么矩阵元仍然是 mutable 的（没有 lower level const）。
\item 如果要支持成员改变， 那么用 \verb`mutable struct` 来声明即可。 mutable struct 通常存在 heap 中而不是在 stack 中。
\item 查看成员名称用 \verb`fieldnames(类型)`， 如果还要显示成员类型， 用 \verb`dump(类型)`。
\item \verb`getglobal(module::Module, name::Symbol, [order::Symbol=:monotonic])` 可以从模块中获取符号。
\item \verb`getproperty(a, b)` 相当于算符 \verb`a.b`，会根据 \verb`a, b` 的类型调用 \verb`getfield(::Type, ::Symbol)`（结构体获取字段）， \verb`getfield(::Tuple, ::Int)`（Tuple 获取元素），\verb`getfield(::Array, ::Int)`，\verb`getfield(自定义struct, 字段序号::Int)`，\verb`getglobal(::Module, ::Symbol)`（从模块获取符号）
\item \verb`isdefined(对象, :字段名)` 可以判断对象的字段是否存在
\item \verb`ismutable(Foo)` 可以判断是否 \verb`mutable struct`。注意 \verb`ismutable(Matrix{Int64})` 是 \verb`true` 而 \verb`ismutable(Matrix)` 是 \verb`false`。
\item 要判断一个类型是 primitive 还是 struct， 可以先打个 \verb`?` 进入 help 模式， 然后再输入 \verb`Foo`。 等效地也可以用 \verb`@doc Foo`。
\item 定义 inner constructor 如：
\begin{lstlisting}[language=julia]
struct Person
    name::String
    age::Int
...
\item 定义后无法动态增减字段。
\item \verb`fieldnames(类型)` 查看所有字段，\verb`fieldname(类型, i)` 查看第 \verb`i` 个，\verb`nfields(对象)` 是数量， \verb`fieldtypes(类型)` 查看对应的类型。 \verb`dump(类型)` 查看详情， \verb`methods(类型)` 查看有哪些函数（似乎只能显示构造函数）。
...
\item named tuple： \verb`x = (a=2, b="abc")` 等效于 \verb`x = NamedTuple{(:a, :b)}((2, "abc"))`， 类型为 \verb`NamedTuple{(:a, :b), Tuple{Int64, String}}`，语法糖为 \verb`@NamedTuple{a::Int64, b::String}`。然后可以使用 \verb`x.a` 和 \verb`x.b` 来获取元素。 和 \verb`Tuple` 一样，创建后值无法改变。 \verb`typeof(x)` 打印出语法糖。
...
\item \verb`NTuple{<长度>, <类型>;}` 等效于 \verb`Tuple{<类型>, <类型>, ...}`（可以用 \verb`===`）
\end{itemize}
...
\subsection{引用}
\begin{itemize}
\item \verb`Ref{类型}` 创造一个安全的引用，保证类型不变，被引用的对象不会被回收。
\item 可以想象为一个容器。
\item 通常用于 \verb`ccall` 调用 C 函数。
\item 如果原变量的对象是 mutable 的（例如 Array），那么改变 ref 也会改变原变量
\item 如果原变量的对象是 immutable 的（例如 Int），那么改变 ref 不会改变原变量，反之亦然。这应该是因为 immutable 对象都是靠 COW 改变的。
\begin{lstlisting}[language=julia]
# immutable
x = 3
...
\subsubsection{Set}
\begin{itemize}
\item \verb`Set` 是无序集合，若要有序，用 \verb`OrderedCollections.jl` 中的 \verb`OrderedSet`
...
\item \verb`Dict("ab"=>12, 12=>"ab")` 的类型是 \verb`Dict{Any, Any}`。
\item \verb`isempty()` 可以检查容器是否为空。
...
\item \verb`@inbounds a[key]` 和数组一样，可以避免边界检查（当你确信 key 一定存在）
\item 使用 Symbol 而不是字符串作为 key 效率更高，也很常见。
\item \verb`haskey(d, 12)` 检查 key 是否存在。
...
\subsubsection{Base.Pairs}
\begin{itemize}
\item 当 keyword arguments 使用 \verb`kw...` 时， \verb`kw` 的类型就是这个。
\item 在函数体中，kw 的使用方法几乎就是一个只读的 dict。 但内部数据类型是 \verb`NamedTuple`。 \verb`NamedTuple(kw)` 可以把它转换。
\item 函数体中， \verb`another_fun(...; kw..., abc = 123)` 可以把它继续传递给别的函数。
\end{itemize}
...
\begin{itemize}
\item 当你把一个类名作为值来使用时，它的类就是 \verb`DataType`。 例如 \verb`my_func(Int32)` 就是把类型值作为入参。 此时 \verb`typeof(Int32)` 是 \verb`DataType`。
\item \verb`DataType` 的超类是 \verb`Type{T}`。
\item \verb`Type{Int32}` 是一个单例类，它的唯一对象就是 \verb`Int32`。
\item \verb`isa()` 的用法是 \verb`isa(值，所属类或超类)`， 例如 \verb`isa(3.1, Float64)` 返回真，\verb`isa(3.1, AbstractFloat)` 返回真。
\item 以下都返回真：\verb`isa(Int32, DataType), isa(Int32, Type), isa(Int32, Type{Int32})` 
\item 注意 \verb`isa(Int32, Type{Integer})` 返回假，\verb`isa(Int32, Type{Float64})` 返回假。
\end{itemize}
...
\subsection{常数类型}
\begin{itemize}
\item \verb`A = rand(2, 2); B = A.^2;` 在 IR 中会翻译成如下代码
\begin{lstlisting}[language=julia]
A = rand(2, 2);
...
\end{lstlisting}
\item \verb`Core.apply_type(类模板, 模板参数 ...)` 可以从模板创建一个类型，模板参数应该需要为编译时常量。
\item \verb`类()` 或者 \verb`(类)()` 调用默认构造函数。
\item 如果是标量，直接执行 \verb`Base.literal_pow(Main.:^, 3, Val{2}())` 可以计算 \verb`3^2 = 9`
\item \verb`Base.broadcasted(算符或函数, 参数1, 参数2, ...)` 其中 \verb`参数*` 本身或者其矩阵元被传入 \verb`算符或函数`。
\item \verb`Val{2}` 的父类是 \verb`Any`
\end{itemize}
