...
\begin{itemize}
\item 编译 pdf 时， 特殊字符（如希腊字母等）会无法显示． 如果有无法显示的字符， 在编译的 .log 文件里面（或者 console）就会出现 \verb|Missing character: There is no ...| 的提示． 这时最好的办法就是替换成公式环境中的命令． 如希腊字母 $\alpha$． 参考 \href{https://www.overleaf.com/learn/how-to/What_file_encodings_and_line_endings_should_I_use\%3F}{overleaf 的这篇文章}． 另外\href{https://www.johndcook.com/unicode_latex.html}{这个}网页可以把 utf 字符转换为 latex 命令．
