...
\item \verb`jl_symbol_name()` 可以获取 symbol 的名字，原理是寻找 \verb`jl_sym_t` 对象在内存中后面的地址并转为 \verb`char*`（后面到底是什么？）
\item \verb`ast.c` 中定义了（所有可能的？）表达式类型，即 \verb`jl_expr_t` 的 \verb`head`。
\begin{lstlisting}[language=cpp]
