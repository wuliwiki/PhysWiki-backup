...
接下来我们从保真度的定义出发，如果$\rho$和$\sigma$可以在同一组基$\left\{\ket{i}\right\}$下写成对角矩阵的形式，$\rho = \sum\limits_ip_i\ketbra{i}$，$\sigma = \sum\limits_iq_i\ketbra{i}$，那么容易验证$F\left(\rho,\sigma\right) = \sum\limits_i\sqrt{p_iq_i}$，可以看到保真度的概念实际上是将经典概率开根号之后做内积的量子力学推广。

\subsubsection{讨论：为什么定义不是$F\left(\rho,\sigma\right) = \sqrt{\rho\sigma}$}
