...
最开始学习的时候，会很容易把它和$0$联想起来，他们的确存在一些联系\footnote{空集的元素数量是0}，但建议你学习时就还是把它当成空塑料袋就好了，尤其不要把空集当成$\{0\}$，后者是一个以数字$0$为元素的集合。
...
由于数轴对应的是实数，所以在提及区间时，一定是在讲某些实数。实数集${\rm \mathbf{R}}$用区间也可以表示，记作$(-\infty,+\infty)$。$\infty$是\textbf{无穷}的记号。关于无穷是什么，具体的学习会在本科阶段进行，现在你只要知道三点：
\begin{enumerate}
\item 无穷不是个数字
\item 无穷不在实数集里
