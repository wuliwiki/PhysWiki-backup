...
理论上若把上面的平面波换成库仑平面波（库仑势能中的精确散射态\upref{CulmWf}）， 那么理论上用不同的规范结果是一样的。先看球面波投影：
...
&= \sum_{l',m'} \frac{\I^{-l'}}{k} \E^{\I\sigma_{l'}} Y_{l',m'}(\uvec k) \int \frac{1}{r} \sqrt{\frac{2}{\pi}} F_{l'}\qty(\eta, kr) Y_{l',m'}^*(\uvec r) \qty(r \sqrt{\frac{4\pi}{3}} Y_{1,0}(\uvec r))R_{n,l}(r) Y_{l,m}(\bvec r) r^2 \dd{r}\dd{\Omega}\\
&= \sqrt{\frac{8}{3}}\sum_{l',m'} \frac{\I^{-l'}}{k} \E^{\I\sigma_{l'}} Y_{l',m'}(\uvec k) \int F_{l'}\qty(\eta, kr) R_{n,l}(r) r^2 \dd{r} \int Y_{l',m'}^*(\uvec r)Y_{1,0}(\uvec r)Y_{l,m}(\bvec r) \dd{\Omega}\\
