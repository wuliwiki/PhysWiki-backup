...
\begin{equation} \ali{
D_{(iN), (iN)} &= \frac{-\sqrt{w_{01}}}{(a_i + a_{i+1})} \qty[\frac{1}{a_i} f'_N(t_N) + \frac{1}{a_{i+1}} f'_1(t_1)]~,
} \end{equation}

\begin{equation} \ali{
D_{(iN), (i+1,n)} &= \frac{-\sqrt{w_{01}}}{a_{i+1}^{3/2} \sqrt{a_i + a_{i+1}}} f'_n(t_1)
\qquad (1< n < N)~,
} \end{equation}

\begin{equation}\label{eq_FEDVR_5} \ali{
D_{(iN), (i+1,N)} &= \frac{-\sqrt{w_{01}}}{a_{i+1}\sqrt{(a_i + a_{i+1})(a_{i+1}+a_{i+2})}} f'_N(t_1)~.
} \end{equation}
