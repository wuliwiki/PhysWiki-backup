...
要计算高斯函数
\begin{equation}\label{eq_FTExp_12}
f(x) = \E^{-ax^2} \quad (a > 0)~
\end{equation}
...
在光学和量子力学中，时常需要给 $f(x)$ 乘以一个简谐波以及平移，即
\begin{equation}
f'(x) = \E^{-a(x-x_0)^2}\E^{\I k_0 (x-x_0)} \quad (a > 0)~.
\end{equation}
利用下文的平移性质（\autoref{eq_FTExp_4} \autoref{eq_FTExp_7} ） 可得
\begin{equation} % 已 AI 验证
g'(x) = \frac{1}{\sqrt{2a}} \exp(-\frac{(k-k_0)^2}{4a})\E^{-\I x_0 k}~.
\end{equation}
另见\enref{一维自由粒子的高斯波包（量子）}{GausWP}。
\end{example}
...
\mathcal F[f(x - x_0)] = g(k) \E^{-\I x_0 k}~.
\end{equation}
...
\end{equation}
其中 $\sigma_x, \sigma_k$ 分别是 $f(x), g(k)$ 的标准差，即
\begin{equation}
\sigma_x^2 = \int \abs{f(x)}^2 (x-\bar x)^2 \dd{x}~.
\end{equation}
\begin{equation}
\sigma_k^2 = \int \abs{f(x)}^2 (k-\bar k)^2 \dd{k}~.
\end{equation}
对高斯函数（\autoref{eq_FTExp_12}），不等式取等号，此时 $\sigma_x^2 = 1/(4a)$。推导参考量子力学的 “\enref{不确定性原理}{Uncert}”。
