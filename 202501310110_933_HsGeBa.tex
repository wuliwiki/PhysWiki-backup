...
(m^2 - n^2, 2mn, m^2 + n^2)~.
\end{equation}
它可以构造所有的原始勾股数（即最大公因数为 1 的勾股数）。之前给出的例子分别是$m=2,n=1$，$m=3,n=2$，$m=4,n=3$的情况。

这些整数边长的直角三角形广泛应用于几何、建筑和工程测量中。例如，在古代建筑中，工匠常利用长度为 $3$、$4$、$5$ 的木杆构造直角，以确保墙壁垂直。

勾股数的生成规律可以通过数学公式推导。例如，若取连续两个正整数 $n$ 和 $n+1$，可以构造出一组勾股数：
...
从而得到勾股数组 $(5,12,13)$。同样，也可以利用更一般的公式：
