...
单粒子的自旋态的一组基底是 $\qty{\ket{s, m_s}}$， 其中 $-s \le m_s \le s$． 对于全同粒子， $s$ 必须相同． 单个粒子的自旋态所在的空间的维数为 $2s+1$． 那么两个全同粒子所在的自旋态就是该空间和自己的张量积空间， 维数为 $(2s+1)^2$， 其中的态矢可以记为（省略 $s$） $\ket{m_{s1}, m_{s2}}$．

为了满足

如果考虑自旋， 单个粒子的状态所在的空间是波函数和自旋态所在的希尔伯特空间的张量积空间\upref{Tensor}， 也是希尔伯特空间． 而多个粒子状态所在的空间又是单粒子态空间的张量积．
