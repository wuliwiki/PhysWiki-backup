...
这一点也可以由极限分析得到，注意下述分析在高中阶段不作要求。由于$\tan x$在连续区间上单调递增，结合周期性，只要分析$\displaystyle x\in(-\frac{\pi}{2},\frac{\pi}{2})$时$\displaystyle\tan x = \frac{\sin x}{\cos x}$，而 $\cos x$ 出现在分母上，关键在于 $\cos x$ 在零点附近的变化情况：
\begin{itemize}
