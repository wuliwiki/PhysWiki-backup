...
由\autoref{Cycltm_the1} ，$x^n-1 = \prod_{d\mid n}\Phi_d(x)$．

由\autoref{Cycltm_lem1} ，令$f(n)=\Phi_n(x)$，其中$x$可以是\textbf{任意}正整数，于是有
\begin{equation}
\begin{aligned}
\Phi_n(x)&=\prod_{d\mid n}\qty(\prod_{k\mid \frac{n}{d}}\Phi_k(x))^{\mu(d)}\\
&=\prod_{d\mid \frac{n}{k}}\prod_{k\mid n} \qty(\Phi_k(x))^{\mu(d)}\\
&=\prod_{d\mid n}\prod_{k\mid n} \qty(\Phi_k(x))^{\mu()}\\
\end{aligned}
\end{equation}
