...
\autoref{eq_GbeamABCD} 的函数形式与\autoref{eq_Gbeam} 一致，这证明了我们的猜测是正确的：\autoref{eq_Gbeam} 这样一个高斯型的函数就是我们要找的，通过光学元件后光斑分布不变的一种光，写回成$E$的形式，也就是：
\begin{equation}
E(x,y,z)=A\E^{-\I k(z+\frac{x^2+y^2}{2q})}~
\end{equation}
