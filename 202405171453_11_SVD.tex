...
现在我们来证明奇异值分解定理。
对于\textbf{任意复矩阵}$M$，我们可以分解为\autoref{lem_SVD_1} 的形式。那么对于\textbf{满秩}$N$矩阵，自然而然的，我们可以利用\autoref{the_SVD_1} 进行进一步的分解。设该分解表示为$N=Q^{\dagger}D'S$。

构造复矩阵$Q_1,S_1$，其中$E_1,E_2$是用作填充的单位矩阵，比如$E_1$使得$Q_1$的行数等于$S^{\dagger}$的行数。
\begin{equation}
Q_1=\pmat{Q^{\dagger}&0\\0&E_1},\quad S_1=\pmat{S&0\\0&E_2}
~.
\end{equation}
该定义是合理的，因为
\begin{equation}
\pmat{N&0\\0&0}=\pmat{Q^{\dagger}&0\\0&E_1}\pmat{D'&0\\0&0}\pmat{S&0\\0&E_2}\equiv Q_1DS_1~.
\end{equation}

易证$Q_1,S_1$是酉矩阵，可见$D$便是定理里要求的对角矩阵，
