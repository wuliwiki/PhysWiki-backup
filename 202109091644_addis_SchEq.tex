...
由于散射态不能用 $\braket{\psi}{\psi} = 1$ 的方式归一化， 它们并不是物理的， 即粒子不能处于某个散射态． 然而散射态的线性组合\footnote{这里指积分}却可能可以归一化． 例如自由粒子（$V \equiv 0$）的高斯波包\upref{GausPk} 就可以看作由平面波线性叠加而来， 即反傅里叶变换． 某种意义上， 给定一个势能函数， \textbf{所有的束缚态和散射态可以构成一组完备的正交归一的函数基底}， 可以展开所有可归一化的波函数． 这里需要用到比较成熟的数学知识（泛函分析）， 我们留到一维散射态的正交归一化\upref{ScaNrm} 简单介绍．
