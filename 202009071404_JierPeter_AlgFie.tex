...
\begin{example}{外代数}
给定域$\mathcal{K}$上的$n$维线性空间$V$，任取其一组基$\{e_i\}^n_{1}$．我们进行以下构造：
\begin{itemize}
\item 对于每两个元素$v,u\in V$，我们定义一个新的元素$v\wedge u$，只要求它满足\textbf{反对称性}：$v\wedge u=-u\wedge v$．这样，对于两个非零且不相等的$v,u\in V$，$v\wedge u$是一个不在$V$中的新元素；当$v=u$或其中之一为$0$时，$v\wedge u$是$0\in V$．
\end{itemize}
