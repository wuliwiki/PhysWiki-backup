...
\begin{itemize}
\item \verb`curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.4/install.sh | bash` 安装 nvm
\item chong
\item \verb`nvm --version` 验证安装成功
\item \verb`nvm install node` 安装 node.js
\item \verb`node -v; npm -v` 查看 node 和 npm 版本
\end{itemize}
