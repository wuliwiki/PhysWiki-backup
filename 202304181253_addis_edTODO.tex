...
\subsection{【未来计划】使用 Git 协同编辑}
\begin{itemize}
\item 使用 git 管理协作， 每个人的新编辑概念上都创建一个新的分支， 然后提交 pull request， 然后管理员批准后 merge 到 master。 每个新 branch 都有自己的数据库。 这样就就把编辑者枪锁的负担转移给审稿人了（必要时手动解决冲突）。
\item 会员制需要考虑一个问题就是， 如果大家都可以用编辑器， 那不开会员也可以打开编辑器查看所有内容。
\end{itemize}
