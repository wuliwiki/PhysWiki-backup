...
\end{theorem}
从每一个（平行于 $W$ 的）仿射集 ${S_{\alpha}}$ （不包括 $W$）里各选一个元素$v_a$，$\{[v_{\alpha}]\}$ 构成商空间$V/W$的一组基底，而 $\{v_{\alpha}\}$ 张成的是$W$的补空间。由等价类划分可知，$[v_{\alpha}]$与$W$没有交集，因此我们只需要证明所有元素都可以表示为$b_{\alpha}v_{\alpha} + W$即可。

设$v$为任意元素，则有$v + W=b_{\alpha} S_{\alpha}$，由于$v_{\alpha} \in S_{\alpha}$，所以$b_{\alpha} v_{\alpha} \in v + W$。也就是说，总可以找到一个元素$v_1 \in W$，使得$v=bv_{\alpha} + v_1$，由于仿射集与子空间互不包含，因而和是直和，证毕。
\begin{exercise}{例子}
