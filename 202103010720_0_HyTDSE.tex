...
\begin{equation}
H_0 \psi_{\lambda}(r) + \sum_{\lambda'} E(t)rF_{\lambda, \lambda'} \psi_{\lambda'}(r) = \I \pdv{\psi_{\lambda}}{t} \quad (\lambda=0,1,\dots)
\end{equation}
其中
进而可以用球谐函数表示（\autoref{SphHar_eq4}~\upref{SphHar}）

\begin{equation}
\begin{aligned}
F_{\lambda,\lambda'} = &\quad\mel{\psi_{n,l,m}}{\bvec{\mathcal E}\vdot\bvec r}{\psi_{n',l',m'}} = \bvec{\mathcal E}\vdot\mel{\psi_{n,l,m}}{\bvec r}{\psi_{n',l',m'}}\\
...
\begin{equation}
\begin{aligned}
F_{\lambda,\lambda'} &= \sqrt{\frac{4\pi}{3}} \int Y_{l,m}^*(\uvec r) Y_{1,0}(\uvec r) Y_{l',m'} \dd{\Omega}\\
&= \sqrt{(2l+1)(2l'+1)} \pmat{l & 1 & l'\\ 0 & 0 & 0}^2\\
&= \sqrt{\frac{2l+1}{2l'+1}} \bmat{l & 1 & l'\\ 0 & 0 & 0}^2
\end{aligned}
\end{equation}
