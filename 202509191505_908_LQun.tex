...
\textbf{定理：\(^\text{[24]}\)}设$G$与$H$是李群，它们的李代数分别为$\mathfrak{g}$与$\mathfrak{h}$，并且$f : \mathfrak{g} \to \mathfrak{h}$是一个李代数同态。若 $G$ 是单连通的，则存在唯一的李群同态$\phi : G \to H$使得$\phi_{*} = f$其中 $\phi_{*}$ 是 $\phi$ 在单位元处的微分。
...
$$
其中$X$为矩阵。若$G$是$\mathrm{GL}(n;\mathbb{C})$的闭子群，则指数映射会将$G$的李代数送入$G$。因此，所有矩阵群都有指数映射。并且，$G$中每一个足够接近单位元的元素都可以表示为李代数中某个矩阵的指数。\(^\text{[26]}\)
...
\item 半单李群是其李代数为单李代数直积的李群。\(^\text{[30]}\)它们是单李群直积的中心扩张。
\end{itemize}
...
\item 还有一些无限维推广，如一般线性群、正交群等。\(^\text{[31]}\)它们的重要方面之一是可能具有更简单的拓扑性质；例如参见Kuiper 定理。在M理论中，一个 10 维的 $\mathrm{SU}(N)$ 规范理论在 $N \to \infty$ 时会变为 11 维理论。
\end{itemize}
\subsection{另见}
\begin{itemize}
\item 李群的伴随表示
