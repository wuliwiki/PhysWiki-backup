...
递归论起源于 1930 年代\textbf{罗莎·彼得}（Rózsa Péter）、\textbf{阿隆佐·丘奇}（Alonzo Church）  
与\textbf{阿兰·图灵}（Alan Turing）的工作，  
并在 1940 年代被\textbf{斯蒂芬·克莱尼}（Stephen Kleene）和\textbf{埃米尔·波斯特}（Emil Post）大幅扩展。\footnote{参见文献 [45]}  

\paragraph{经典递归论（Classical Recursion Theory）}
经典递归论主要研究\(\mathbb{N} \to \mathbb{N}\)函数的可计算性。  
其基本成果建立了一类稳健而规范的可计算函数集合，
这些函数类具有多种独立且等价的刻画形式——包括\textbf{图灵机}（Turing machine）、\textbf{λ 演算}（lambda calculus）等。  
更高阶的结果涉及\textbf{图灵度结构}（structure of Turing degrees）以及\textbf{递归可枚举集格}（lattice of recursively enumerable sets）的研究。

\paragraph{广义递归论（Generalized Recursion Theory）}
广义递归论将递归理论的思想扩展到不再局限于有限计算的情形。  
该分支包括高阶类型（higher types）中的可计算性研究，
以及\textbf{超算术理论}（hyperarithmetical theory）和\textbf{α-递归理论}（α-recursion theory）等领域。

\paragraph{现代递归论研究（Contemporary Research）}
现代递归论的研究方向包括：
\begin{itemize}
  \item \textbf{算法随机性}（algorithmic randomness），
  \item \textbf{可计算模型论}（computable model theory），
  \item \textbf{逆向数学}（reverse mathematics），
  \item 以及纯递归论中的新结果（new results in pure recursion theory）。
\end{itemize}
\subsubsection{算法不可解问题}
递归论的一个重要分支研究\textbf{算法不可解性}（algorithmic unsolvability）。  
若不存在任何可计算算法能对问题的所有合法输入返回正确输出，
则称该\textbf{判定问题}（decision problem）或\textbf{函数问题}（function problem）为算法不可解的。

最早关于算法不可解性的结果由\textbf{丘奇}（Church）与\textbf{图灵}（Turing）于 1936 年独立获得，
他们证明了著名的\textbf{判定问题}（Entscheidungsproblem）是算法不可解的。  
图灵的证明基于\textbf{停机问题}（halting problem）的不可解性，
这一结果在递归论与计算机科学中均具有深远影响。

数学中存在许多著名的\textbf{不可判定问题}实例：
\begin{itemize}
  \item \textbf{群的词问题}（word problem for groups）——
  由\textbf{彼得·诺维科夫}（Pyotr Novikov, 1955）与\textbf{W. Boone}（1959）分别独立证明其算法不可解；
  \item \textbf{忙海狸问题}（busy beaver problem）——
  由\textbf{蒂博尔·拉多}（Tibor Radó）于 1962 年提出，是最著名的不可判定问题之一。
\end{itemize}

\textbf{希尔伯特第十问题}（Hilbert’s tenth problem）要求给出一种算法，
判断给定的多元整系数多项式方程是否在整数中有解。  
\textbf{朱莉娅·鲁滨逊}（Julia Robinson）、\textbf{马丁·戴维斯}（Martin Davis）与\textbf{希拉里·普特南}（Hilary Putnam）  
对该问题作出了部分进展。  
最终，\textbf{尤里·马季亚谢维奇}（Yuri Matiyasevich）于 1970 年证明该问题\textbf{算法不可解}\footnote{参见文献 [46]}，
这标志着希尔伯特第十问题的彻底解决。
