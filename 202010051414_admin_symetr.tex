...
由\autoref{symetr_eq5} 的启发， 我们可以尝试改变\autoref{symetr_eq1} 中每项的正负号来达到反对称化． 事实上行列式的定义\upref{Deter}中已经给出了我们需要规则， 即使用逆序数的奇偶性来决定正负号． 对称化的结果可以用行列式记为
...
要证明反对称性很简单， $P_{i,j}$ 作用在\autoref{symetr_eq3} , 相当于把行列式的两行置换， 而根据行列式的性质（\autoref{DetPro_the6}~\upref{DetPro}）， 这会使结果取相反数．
