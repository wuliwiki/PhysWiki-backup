...
\subsection{线性算符}
令 $N$ 维线性空间 $X$ 到 $M$ 维线性空间 $Y$ 的一组正交归一基底分别为 $\{\xi_i\}$ 和 $\{\eta_i\}$． 若线性映射\upref{LinMap}（线性算符） $A:X\to Y$ 表示成矩阵后， 矩阵元为 $A_{i,j}$， 那么该算符也可以用狄拉克符号表示为
\begin{equation}
A = \sum_{i,j} A_{i,j} \ket{\eta_i}\bra{\xi_j}
\end{equation}
这是因为： 若把 $A$ 作用在任意矢量 $x = \sum_k x_k\ket{\xi_k}$ 上， 有
\begin{equation}
\begin{aligned}
