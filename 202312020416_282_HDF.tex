...
\item DataSet： a matrix in file
\item \verb|DataSet::write(<memory address>, <element type>, <DataSpace for memory>, <DataSpace for dataset>)|

\item DataSpace: where in the matrix (in dataset or in memory)
\item DataSpace::selectHyperslab: select a sub-matrix

\item for row major, we can write the dataset in 1 command
\item for column major, need to use Hyperslab if we don't want to convert to row major first
\end{itemize}
