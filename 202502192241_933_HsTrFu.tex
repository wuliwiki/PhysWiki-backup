...
最后是关于原点对称的情况，若两个角 $\alpha, \beta$ 对应的点 $P, Q$ 关于原点对称，则 $Q$ 可以看作是 $P$ 先关于 $x$ 轴对称，再关于 $y$ 轴对称得到的。

若 $P$ 对应的角为 $\beta$，则关于 $x$ 轴对称后，得到 $P’$，其角为：
\begin{equation}
\beta'= -\beta + 2k\pi, \quad (k\in\mathbb{Z})~.
\end{equation}

若再将 $P’$ 关于 $y$ 轴对称，得到最终的 $Q$，则：

\alpha = \pi - \beta{\prime} + 2m\pi
\end{equation}
代入 $\beta’ = -\beta + 2k\pi$：
￼
化简得：
...
这一关系说明，当一个角 $\beta$ 关于原点对称时，其对应的新角 $\alpha$ 总是等于 $\pi + \beta$（加上 $2k\pi$ 的周期性）。从三角函数的角度来看，这种对称性使得：
￼
即，正弦和余弦都变号。
