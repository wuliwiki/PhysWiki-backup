...
\pentry{椭圆的四种定义\upref{Elips3}， 抛物线的三种定义\upref{Para3}， 双曲线的三种定义\upref{Hypb3}， 平面旋转变换\upref{Rot2DT}}
...
我们可以再列出一个一般的平面方程与\autoref{ConSec_eq4} 联立得到方程组， 但这样解出来的椭圆将与 $x$-$y$ 平面不平行． 所以更方便的办法是先把圆锥旋转一下， 再用某个和 $x$-$y$ 平面平行的平面 $z = z_0$ 去截出椭圆． 这样就可以 关于 $y$ 轴的旋转变换\upref{Rot2DT}为\footnote{\autoref{ConSec_eq1} 和\autoref{ConSec_eq2} 事实上是一个特殊的三维旋转矩阵\upref{Rot3D}．}
