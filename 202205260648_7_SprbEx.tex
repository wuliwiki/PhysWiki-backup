...
设$\mathbb{F}$是域．若多项式$f(x)\in\mathbb{F}[x]$在其分裂域中无重根，则称$f$为$\mathbb{F}$上\textbf{可分（separable）}多项式\footnote{这里采用GTM 242 \textsl{Abstract Algebra}的定义．有些材料中（如孟道骥《抽象代数I-代数学基础》）会把无重根的\textbf{不可约}多项式定义为可分的，而一般的多项式则只要求其不可约因子是可分的，即定义为可分多项式．}．
