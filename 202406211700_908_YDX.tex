...
其中 $\hat{i}$，$\hat{j}$ 和 $\hat{k}$ 是沿着 $X, Y \text{{} Z$ 轴的单位向量。

考虑仅在圆柱 $R(t) = C$ 的表面上移动的粒子 $P$，可以将固定框架中的 $Z$ 轴与圆柱体轴线对齐。然后，$X$ 轴平面内的这个轴的角度 $\theta$ 可以用来定义其轨迹，

$\mathbf{P}(t) = R \cos \theta(t) \hat{i} + R \sin \theta(t) \hat{j} + Z(t) \hat{k}.$

通过引入径向和切向单位矢量，可以简化 $\mathbf{P}(t)$ 的柱坐标:
