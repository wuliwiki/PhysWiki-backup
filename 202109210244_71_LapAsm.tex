...
\le K\int_{\mathbb{R}^+}\E^{[-\operatorname{Re}z-c]t}t^{λN-1}\,\dd t
\]
注意到当$\operatorname{Re}z$时
\[
\int_{0}^{\infty}{e^{-zt}t^{\lambda_n-1}}\,\dd t
=\frac{1}{z^{\lambda_n}}\int_{0}^{\infty}{e^{-u}u^{\lambda_n-1}}\,\dd u
=\frac{\Gamma\left(\lambda_n\right)}{z^{\lambda_n}}
\]
因此
\[
\left|F(z)-\sum_{n=0}^{N-1}{c_n\frac{\Gamma\left(\lambda_n\right)}{z^{\lambda_n}}}\right|\le K\Gamma(\lambda_{N})Rez-cλN=KΓλNzλNzRez-cλN
