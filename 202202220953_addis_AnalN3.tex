...
\item 设 $s(x) = \sum_{n=1}^n c_i K_{E_i}(x)$ （$x\in X, c_i>0$） 可测， 而且 $E\in \mathfrak M$． 我们定义 $I_E(s) = \sum_{n=1}^n c_i \mu(E\bigcap E_i)$． 如果 $f$ 可测且为非负的， 我们定义 $\int_E f\dd{\mu} = \sup I_E(s)$． 这里 $\sup$ 是对所有满足 $0\leqslant s\leqslant f$ de
\end{itemize}
