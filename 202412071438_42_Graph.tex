...
设 $G$ 是简单无向图，若 $G'$ 是它对应的完全图。则称 $(V(G),E(G')\backslash E(G),\varphi)$ 是 $G$ 的\textbf{补图}（complementary graph），记作 $G^c$ 或 $\overline G$。若 $G=\bar G$，则称 $G$ 是\textbf{自补的}（self complementary）。
\end{definition}
