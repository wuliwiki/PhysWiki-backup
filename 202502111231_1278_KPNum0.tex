...
3。把 $x(\Delta t), y(\Delta t)$ 再次代入\autoref{eq_KPNum0_3}， 得到 $x''(\Delta t), y''(\Delta t)$， 再次利用微分近似求出 $x(2\Delta t), y(2\Delta t), x'(2\Delta t), y'(2\Delta t) \dots$ 如此循环下去就可以得到每隔 $\Delta t$ 的数值解。这种方法是前向欧拉法，它的数值离散格式会导致舍入误差不断放大，是不稳定的求解算法，但计算速度非常快，一般用来初步观察解的形状。对于不稳定的数值格式，降低步长往往不会减小误差
