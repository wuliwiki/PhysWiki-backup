...
在推导双曲线的参数方程时，可以使用三角函数中满足双曲线关系的 $\sec x$ 和 $\tan x$，也可以更直接地使用\enref{双曲函数}{TrigH}来表达。不过需要注意的是，双曲线的参数方程在高中阶段并不是必学内容，这里给出相关知识，主要是为了保证内容的完整性。
...
x(\theta)=a\sec{\theta}\\
y(\theta)=b\tan{\theta}
\end{cases},\quad \theta \in [0,2\pi)\text{且} \theta\neq\frac{1}{2}\pi,\frac{3}{2}\pi~.
\end{equation}
...
显然，此处使用三角函数没有双曲函数方便。一则参数的变化并非连续实数，二则当参数单调变化时，
