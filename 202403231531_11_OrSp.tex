...
接下来证明最后一条。$W\cap W^{\bot}=0$，只需证明$V=W+W^{\bot}$即可。正交空间必有正交基，设$\{\bvec e_i\}_{i=1}^k$是$W$上的一组正交基,任意$\bvec z\in V$，$\bvec z'$为任意向量在$W$上的分量，则剩余分量为
\begin{equation}
\bvec z-\bvec z'=\bvec z-\sum{i=1}^k\frac{f(z,\bvec e_i)}{f(\bvec e_i,e_i)}\bvec e_i~.
\end{equation}
