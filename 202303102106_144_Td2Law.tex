...
\footnote{本节的证明方法参考了Schroeder的《热物理学导论》。这也是一种常规的方法} 有很多方法将热力学第二定律方便地运用到等温等容与等温等压系统。此处简要讨论一种笔者最喜欢的方法。

为了进一步论证，我们还需先引入一个十分震撼、可疑又平凡的假设：如果将系统与环境再视为一个“大系统”，那么这个大系统是孤立系统，因为不存在更大的环境与之交换热或功。

我们先探讨等温等容系统（此系统中系统与环境时时保持温度相同，但环境与系统只交换热、不做功$\Delta U = \delta q$）。分析系统与环境这个大系统的熵增：
$$
\begin{aligned}
\Delta {S_{tot}}
 &=\Delta {S_{sys}}+\Delta {S_{env}}\\
 & =\Delta {S_{sys}}+ \frac{\Delta U_{{env}}}{T}\\
 & = \Delta {S_{sys}} - \frac{\Delta U_{{sys}}}{T} \\
 \end{aligned}
$$
现在，大系统的 $\Delta {S_{tot}}$只与原先的系统有关，因此省略下标。
$$
\begin{aligned}
\Delta {S_{tot}}
...
因此，对大系统运用孤立系统的熵判据 
$$\Delta S_{tot} > 0\Rightarrow \Delta F < 0$$，那么我们知道，如果某个过程后$\Delta F < 0$，那么它可以发生。这就是亥姆霍兹自由能\upref{HelmF}

同理，对于等温等压系统，我们有吉布斯自由能\upref{GibbsG} 与相应的Gibbs判据。
