...
\begin{itemize}
\item 三角形的\textbf{中心}，是三条中线的交点。
\item 三角形的\textbf{重心}，是三条高的交点。
\item 三角形的\textbf{外心（circumcenter）}，是三边垂直平分线的交点。
\item 三角形的\textbf{内心（incenter）}，是三条角平分线的交点。
\end{itemize}
三角形的“四心”包括：
\begin{itemize}
    \item 三条中线的交点称作\textbf{重心（centroid）}，重心将每条中线分为 $2:1$ 的比例（靠近顶点的一段较长）。
    \item 三条高的交点称作\textbf{垂心（orthocenter）}。

    \item 三角形的\textbf{外心（circumcenter）}，是三边\textbf{垂直平分线（perpendicular bisector）}的交点。外心是三角形外接圆的圆心，该圆过三角形的三个顶点。

    \item 三角形的\textbf{内心（incenter）}，是三条\textbf{角平分线（angle bisector）}的交点。内心是三角形内切圆的圆心，该圆与三角形的三边相切。
\end{itemize}
...
\item 若三角形以 $A$、$B$、$C$ 为顶点，则称为圆的\textbf{内接三角形（inscribed triangle）}，此时三角形的三个内角均为圆周角，圆心称作三角形的\textbf{外心（circumcenter）}，是三边垂直平分线的交点。
\item 若三角形的三边分别与圆相切于 $A$、$B$、$C$，则称为圆的\textbf{外切三角形（circumscribed triangle）}，此时三角形的\textbf{内心（incenter）}是圆心。
\end{itemize}
