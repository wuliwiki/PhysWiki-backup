...
这两个定理揭示了有限域上极为重要而简洁的性质．我们简单看看两个例子，利用起这个性质：

\begin{example}{}\label{SprbEx_ex3}
考虑$\mathbb{Z}_5$的单代数扩张$\mathbb{Z}_5(\sqrt{2})=\{0, 1, 2, 3, 4, \sqrt{2}, \sqrt{3}\}$．

检查这个扩张的所添加的两个元素，它们的最小多项式分别为$x^2-2$和$x^2-3$，它们在$\mathbb{Z}_5$上分别分解为$(x-\sqrt{2})(x+\sqrt{2})$和$(x-\sqrt{3})(x+\sqrt{3})$．而由于$\sqrt{2}\neq-\sqrt{2}$和$\sqrt{3}\neq-\sqrt{3}$．

注意一个特点：对于任意$a\in\mathbb{Z}_5(\sqrt{2})$，都有$a^5=a$．
\end{example}
...
\autoref{SprbEx_ex2} 中$x^p-\pi$是不可约多项式，但\autoref{SprbEx_ex3} 中如果构造类似的$x^5-\sqrt{2}$，则这是一个可约多项式，因为$(\sqrt{2})^5=\sqrt{2}$，使得$x^5-\sqrt{2}=(x-\sqrt{2})^5$．实际上，如果你先读这两个例子的区别，就会发现为什么数学家会想到用\autoref{SprbEx_the4} 来判断完备域：当且仅当存在$a$的$p$次方根，在$\mathbb{Z}_p$上的多项式$x^p-a$才是可约的．
