...
由于$W$非退化，所以$\opn{Rad} W=W\cap W^{\bot}={\bvec 0}$且$W$是偶数维的辛空间。设$\{\bvec \theta_i,\bvec \theta_{-i}\}_{i=1}^k$是$W$的一组基。扩展其为$V$上的一组基：$\{\bvec \theta_i,\bvec \theta_{-i}\}_{i=1}^k\cup\{\bvec \eta_i\}_{i=1}^{t}$。

设$\bvec \eta'_i=\bvec\eta_i-f(\bvec \eta_i,\bvec \theta_{-i})\bvec\theta_i+f(\bvec\eta_i,\bvec\theta_i)\bvec\theta_{-i}$，则有：
\begin{equation}
f(\bvec \eta'_j,\bvec  \theta_i)=f(\bvec \eta_j,\bvec  \theta_i)-f(\bvec\eta_j,\bvec  \theta_{-i})\bvec \theta_i+f(\bvec\eta_j,\bvec \theta_i)\bvec\theta_{-i}=~.
\end{equation}