...
从三角形的一个顶点出发，向其对边所在直线作的垂线称为三角形的\textbf{高（altitude）}。垂足可能在边上、与定点重合或在边的延长线上，所以高可能位于三角形内部、与边重合或在三角形外部。高所对的边称作这个高所对的\textbf{底（base）}。顶点与对边中点的连线称为\textbf{中线（median）}。三角形的三条中线交于一点，称作\textbf{重心（centroid，或质心）}，重心将每条中线分为 $2:1$ 的比例（靠近顶点的一段较长）。
