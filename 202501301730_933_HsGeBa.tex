...
几何中最基本的用于表示位置的元素称为\textbf{点（Point）}。尽管，人们脑海中点的形象是一个小的黑色实心圆，但事实上，数学中的点没有大小，仅表示位置。通常用大写字母 $A, B, C$ 等表示。

由无数个点组成，是长度无限但没有宽度的几何对象称为\textbf{线（Line）}。通过两点可以确定一条直线。线可以是直线（无限延伸）、线段（有两个端点）、射线（一端有限，一端无限延伸）。由两点确定，可以是不同形式的线形：
	•	直线：无限延伸，不存在端点。
	•	线段：有两个端点，表示有限长度。
	•	射线：从一个端点出发，向另一方向无限延伸。
...
其中，第五公设是欧几里得几何与非欧几里得几何的核心区别。

\subsubsection{度量}

\textbf{长度}，（描述一下。）在欧氏几何中，长度是基础量，是不能被定义的。将线段分成等长的两部分的点称为线段的中点。
