...
回顾伴随映射的定义。设线性空间$V,W$分别对应二次型$q,p$，且线性映射：$f:V\rightarrow W$在这两组基下的矩阵为$F^a_b$，伴随矩阵$f^*$在这两组基下的矩阵为$G^a_b$。那么我们有：$G^a_b =q^{ja}p_{ib}F^i_j$。如果二次型是标准的，上式简化为$q(e_j)p(e_b)F^I_J$
