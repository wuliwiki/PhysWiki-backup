...
\begin{itemize}
\item 每个词条在创建时需要可以选择协议， 选项见数据库的 \verb|entries.license|， 选择后更新数据库。
\item 删除词条以前运行 \verb|./PhysWikiScan --delete 词条名|。 运行成功 只有满足以下条件的词条可以删除（否则提示为什么不能删除）： \verb|entries.ref_by| 为空， \verb|figures| 对应的 \verb|figures.ref_by| 为空， \verb|labels| 对应的 \verb|labels.ref_by| 为空。 删除后更新 \verb|entries.deleted|。
