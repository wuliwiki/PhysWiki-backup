...
由于诱导公式本身就是研究对称性的，而且在之前已经研究的比较透彻，只要结合诱导公式和周期性，就可以简单地得到奇偶性和对称性。
根据诱导公式有：
sin-x=-sinx,cos-x=cosx,tan-x=-tanx
根据奇偶性的\aref{定义}{def_HsFunC_7}可知，sinx,tanx是奇函数，cosx是偶函数。根据倒数关系，cscx,cotx是奇函数，$secx$是偶函数。
...
（直接推结果）
。
然而这还没完，根据\aref{诱导公式}{eq_HsTrFu_16}，cosx向左平移$\pi$个单位后得到-cosx，得到的新函数仍满足偶函数的定义，因此\pi也是cosx的一个对称轴。另外根据\aref{诱导公式}{eq_HsTrFu_14}，sinx向左平移$pi/2$个单位可以得到cosx，sinx向右平移$pi/2$个单位可以得到-cosx，使得sinx本身也存在两个对称轴x=pi/2和x=-pi/2。而由于tanx和cotx都是奇函数，因此不论如何变换都没有对称轴。
