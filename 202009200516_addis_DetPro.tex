...
\end{theorem}
证明： 根据定义（\autoref{Deter_eq4}~\upref{Deter}）， 行列式展开后的每一项都含有每一行（或每一列）的一个元素， 所以当某列全为 0， 则结果为 0．
...
\end{theorem}
证明： 思路和\autoref{DetPro_the1} 的证明一样， 行列式展开后的每一项都含有每一行（或每一列）的一个元素， 所以当某列全部元素乘以常数， 则相加的每一项都乘以该常数．
