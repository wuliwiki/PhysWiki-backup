...
从球谐函数表\upref{YlmTab} 中可知， 上式两个方括号中第一项 $\sim \E^{\I \phi}$， 第二项 $\sim \E^{-\I \phi}$， $(-1)^m$ 是为了抵消 Condon–Shortley 相位（\autoref{SphHar_sub1}~\upref{SphHar}）．所以 $m > 0$ 时 $\mathcal Y_{l,0} \sim \cos(m\phi)$， $m < 0$ 时 $\mathcal Y_{l,0} \sim  \sin(m\phi)$， $m = 0$ 时与 $\phi$ 无关．
