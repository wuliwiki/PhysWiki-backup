...
\pentry{导数\nref{nod_Der}}{nod_9099}
% Giacomo：一元函数的微分\nref{nod_Diff} 要么作为拓展知识，要么作为导数的前置
...
$\sqrt{x^2 + a^2}$ （$a$ 为常数）可以看作幂函数 $f(x) = \sqrt{x}$ 和多项式 $g(x) = x^2 + a^2$ 的复合函数， $f'(x) = 1/(2\sqrt x)$，$g'(x) = 2 x$，代入\autoref{eq_ChainR_1} 得
\begin{equation}
...
\subsection{几何理解}
为了方便表示，我们把 $g$ 的函数值和 $f$ 的自变量记为 $u$， 把 $f$ 的函数值记为 $y$。以下推导用到了微分\upref{Diff}的记号。
