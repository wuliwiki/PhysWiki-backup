...
\item 如果想在本地使用 Jupyter Notebook 中的 Cling， 用 \verb|conda|\upref{CondaN} 安装即可（不支持 Windows\footnote{找不到包， 可以用 WSL 或者 WSL2， 但最好用真正的 Linux。 用 WSL 时，最好}）。 \verb|conda create -n cling|， \verb|conda install xeus-cling -c conda-forge|。 \verb|conda install notebook|。 运行： \verb|conda activate cling|， \verb|jupyter notebook|。
