...
\begin{equation}
I_n(k) = \frac{1}{2\pi}\int_{-n\pi}^{n\pi} \E^{\I kx}\dd{x} = \frac{\sin(n\pi k)}{\pi k} = n \sinc(n\pi k)
\end{equation}
使用\autoref{sinc_eq2}~\upref{sinc}容易证明（\autoref{Delta_def1} ）
\begin{equation}
\lim_{n\to\infty} I_n(k) = \delta(k)
\end{equation}
证毕．
\end{example}
