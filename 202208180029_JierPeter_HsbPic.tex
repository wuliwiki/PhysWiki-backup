...
现在，固定$\ket{s}$不变，而让可观测量算符$X^{(H)}(0)$变为$X^{(H)}(t)$．我们通过转移矩阵$\Omega(t)$实现这个变化：$X^{(H)}(t)=X^{(H)}(0)$．

设有一初始量子态$\ket{s}=\sum_a c_a\ket{a}=\sum_a c_a\ket{a; 0}$．在薛定谔绘景中，一段时间$t$后，这个态变为$\sum_a c_a\mathcal{U}(t)\ket{a}$．

根据\textbf{转移矩阵}\upref{TransM}的\autoref{TransM_sub1}~\upref{TransM}，可知应设转移矩阵为$\mathcal{U}(t)^{-1}=\mathcal{U}^\dagger(t)$．这样，当$\ket{s}$不变时，
