...
如果你已经会用 LaTeX， 请仔细阅读这段话． 我们必须强调这个编辑器只是在 \href{https://www.mathjax.org/}{MathJax} 的基础上做了一个简单的拓展， MathJax 只支持公式环境， 我们另外实现了基本的 LaTeX 命令， 如标题， 粗体， 常用的环境的简单代码以及对他们的引用． 这些功能大部分可以在工具栏上找到（工具栏上没有的很可能不支持） 编辑器只支持唯一个模板且不可改变（见 main.tex）， 除了 main.tex 以外， 其他所有 tex 文件都将作为一个 section 插入 main.tex．
