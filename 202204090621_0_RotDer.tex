...
\bvec r = \mat R \bvec r'
\end{equation}
令 $\bvec\omega = (\omega_x, \omega_y, \omega_z)$ 是 $S'$ 相对于 $S$ 的瞬时角速度． 我们来证明随时间 $t$ 变化的\footnote{事实上 $t$ 不一定需要是时间， 可以是任意参数．}旋转矩阵\upref{Rot3D} $\mat R(t)$ 的导数 $\dot{\mat R}$ （即每个矩阵元对 $t$ 求导）满足
