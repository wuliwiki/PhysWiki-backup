...
\end{equation}
存在，由于 $y$ 线性依赖于 $x$，于是就有 $\mathbb R^n$ 上的线性算子 $\mathcal A$ 存在，使得 $\mathcal Ax=y$。且对 $k>N(\epsilon)$
\begin{equation}
d(\mathcal{A_k,A})=\sup\limits_{x\in{\mathbb R^{n*}}}\frac{\abs{(\mathcal{A_k-A}) x}}{\abs{x}}\leq\norm{\mathcal A_k-\mathcal A}~.
\end{equation}
